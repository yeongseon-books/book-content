...
핵심은 두 개의 다른 결정이 서로 다른 곳에서 일어난다는 것입니다.

| 결정 | 결정자 | 신호 | 결과 |
|---|---|---|---|
| **인스턴스 수**(스케일아웃) | Scale Controller (호스트 외부) | ScaleMonitor·TargetScaler 메트릭 + 호스트 health ping | 인스턴스를 N → N±k 로 |
| **인스턴스 내부 워커 수** | `WorkerConcurrencyManager` (호스트 내부) | 워커 채널 부하 | 같은 인스턴스에서 워커 프로세스 추가/감소 |
...
> 워커 채널이 바쁘면(레이턴시·큐 깊이가 임계치를 넘으면) 같은 인스턴스 안에 워커 프로세스를 하나 더 띄운다. 한가하면 줄인다.

이건 **외부 Scale Controller와 무관합니다.** 인스턴스 수는 그대로 두고 인스턴스 안의 병렬도만 조정합니다. 그래서 같은 1 인스턴스라도 어떤 시점에는 워커가 1개, 어떤 시점에는 3개일 수 있습니다.
...
| 신호 | ScaleMonitor 메트릭 + health ping | 워커 채널 레이턴시·큐 깊이 |
| 영향 | 청구액·시작 시간 | 인스턴스 내 처리량 |
| 플랜 의존성 | 큼 (플랜마다 다름) | 작음 (어디서나 같은 로직) |
...
| Flex Consumption | Scale Controller (신형) | ✅ | 1000 | ✅ | ✅ | ✅ |
| Premium | Scale Controller(+옵션) | ❌ (최소 1) | ~100 | ❌ | pre-warmed | ✅ |
| Dedicated | App Service Auto-Scale | ❌ | 플랜 따라 | ❌ | (항상 켜져 있음) | ✅ |
...
- 스케일 결정은 **호스트 바깥**에서 일어난다. 호스트는 메트릭을 보고하고 health ping에 응답할 뿐이다.
- `IScaleMonitor`(점진)와 `ITargetScaler`(공식 기반)는 SDK·확장 측 인터페이스이고, 호스트는 그 메트릭을 Table Storage에 저장하는 역할만 한다.
- 인스턴스 **수** 결정과 인스턴스 **내부 워커 수** 결정은 다른 메커니즘이다. 후자는 `WorkerConcurrencyManager`가 호스트 안에서 한다.
- 같은 호스트 코드가 Consumption / Flex / Premium / Dedicated에서 **다르게 동작하는 이유는 외부 결정자의 모델이 다르기 때문이다.** 호스트는 그대로다.
- Flex Consumption은 per-function scaling과 Always ready를 도입해 "어느 함수에 얼마나 인스턴스를 둘지"를 그룹 단위로 결정한다.
...
- [입문 5화 — Plan 선택](../../azure-functions-101/ko/05-choosing-a-plan.md)
- [입문 6화 — 스케일링과 콜드 스타트](../../azure-functions-101/ko/06-scaling-and-cold-start.md)
- [심화 4화 — Dispatcher와 Invocation](./04-dispatcher-and-invocation.md)
- [심화 6화 — 콜드 스타트와 Placeholder](./06-cold-start-placeholder.md) (다음 화)
