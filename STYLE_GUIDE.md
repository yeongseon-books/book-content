...
ko 글의 H1/H2/H3는 **한국어 우선**이다.

- 영문 전용 헤딩은 원칙적으로 금지한다.
- 예외: 기술 약어 비교(`MTTR vs MTBF`, `IPv4 vs IPv6`), 클래스/제품명 비교(`StrOutputParser vs JsonOutputParser`, `TestPyPI vs PyPI`), RFC/ISO 참조.
- `### Step N: ...` 형식은 `### 단계 N — ...`으로 번역한다.
