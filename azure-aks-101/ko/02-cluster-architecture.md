...
![클러스터를 반으로 자르면 보이는 것](../../assets/azure-aks-101/02/02-01-cut-the-cluster-in-half.ko.png)
...
![System node pool과 User node pool](../../assets/azure-aks-101/02/02-02-system-node-pool-vs-user-node-pool.ko.png)
### System node pool
...
다음 글에서는 이 구조 위에 실제로 작은 클러스터를 만들고 FastAPI 앱을 배포해 보겠습니다. `az aks create`, `az aks get-credentials`, `kubectl apply -f`가 각각 어디를 건드리는지도 오늘 설명한 Control Plane과 Node Pool 경계 안에서 자연스럽게 읽히게 됩니다.

---

이 글은 Azure Kubernetes Service 101 시리즈의 2화입니다. 1화에서 AKS의 관리 경계를 잡았다면, 이번 화는 그 경계를 Control Plane과 Node Pool 구조로 구체화한 셈입니다. 다음 3화부터는 이 클러스터 위에 실제 FastAPI 앱을 올리면서 개념을 실행 화면으로 바꿔 봅니다.
