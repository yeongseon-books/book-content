...
8. ~~Repository rename (`tech-writing` → `technical-content`)~~ (완료)
...
Repository rename (`tech-blog` → `tech-writing` → `technical-content`)과 멀티채널 파이프라인 구축이 완료되었다.
...
- Phase 9: Repository rename (`tech-blog` → `tech-writing` → `technical-content`) + medium URL 재생성
...
- `technical-content` rename 후 발행된 Medium 글의 old raw URL이 GitHub redirect에 의존 (장기 신뢰 보증 아님)
