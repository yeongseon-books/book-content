...
4. `ARCHITECTURE.md` 추가 (완료 예정)
5. `.sisyphus/medium`과 `scripts/`의 책임 경계 정리
...
Repository rename (`tech-blog` → `tech-writing`)과 멀티채널 파이프라인 구축이 완료되었다.
...
- Phase 9: Repository rename + medium URL 재생성 (`cb179c5`)
...
- `.sisyphus/medium/`과 `scripts/` 통합 정책 미결정
