...
**핵심 질문**: 테스트가 많을수록 좋은 소프트웨어가 되나요?

> 아니요. 적절한 층에 적절한 테스트가 좋은 소프트웨어를 만듭니다.
...
- 단위·통합·E2E 테스트의 역할
- 테스트 피라미드와 그 반대인 아이스크림 콘
- 커버리지가 거짓말하는 순간
- CI에서 테스트를 빠르게 유지하는 법
- 깨지는 테스트를 끄지 않고 고치는 사고법
...
- **단위 테스트(Unit)**: 함수/클래스 단위, 외부 의존 없음.
- **통합 테스트(Integration)**: 여러 컴포넌트가 함께 동작.
- **E2E 테스트**: 실제 사용자 시나리오, 가장 느림.
- **Flaky test**: 같은 코드인데 가끔 실패하는 테스트.
- **Coverage**: 테스트가 실행한 코드 비율(품질 척도가 아님).
...
1. 본인 모듈에서 외부 의존을 fake로 바꿔 단위화 가능한 함수를 찾아 보세요.
2. 가장 느린 E2E 테스트 1개를 단위/통합으로 분할해 보세요.
3. 최근 flaky 테스트의 원인을 5 Whys로 적어 보세요.
