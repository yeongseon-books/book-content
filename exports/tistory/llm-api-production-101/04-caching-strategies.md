...
캐싱 전략을 어떻게 설계해야 비용과 지연을 동시에 줄일 수 있을까요?

이 글은 그 질문에 답하기 위해 캐싱 전략의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- LLM 응답 캐싱은 일반 HTTP 캐싱과 무엇이 본질적으로 다른가?
- 프롬프트 캐싱(provider-side)과 애플리케이션 캐싱은 어떻게 역할이 갈리는가?
- 캐시 키를 만들 때 시스템 프롬프트, 사용자 입력, 모델 버전을 어떻게 다뤄야 하는가?
- Semantic cache(임베딩 기반 유사도)는 언제 도입해야 하고, 위험은 무엇인가?
- 캐시 적중률과 응답 신선도 사이의 트레이드오프를 어떻게 측정하는가?
...
- **프롬프트 정규화가 캐시 적중의 전제** — 공백·순서 차이가 적중률을 떨어뜨립니다.
- **semantic cache는 정확도와 비용 트레이드오프** — 임계값을 의식적으로 결정합니다.
- **TTL을 use case에 맞춘다** — 신선도와 비용의 균형입니다.
- **provider 제공 캐시를 활용** — Anthropic·OpenAI의 prompt cache가 표준이 되어갑니다.
- **캐시 키에 모델·버전 포함** — 버전 변경 시 stale 응답을 막습니다.

## 운영 체크리스트

- [ ] 결정론(temperature, seed) 설정을 캐시 키에 반영했다
- [ ] 모델 버전 변경 시 캐시를 자동 무효화하는 정책을 마련했다
- [ ] 프롬프트 캐싱 지원 모델은 시스템 프롬프트를 앞단에 고정 배치했다
- [ ] Semantic cache는 임계치(threshold)와 fallback 경로를 함께 정의했다
- [ ] 캐시 적중률, 절감 토큰, miss latency를 메트릭으로 추적했다
