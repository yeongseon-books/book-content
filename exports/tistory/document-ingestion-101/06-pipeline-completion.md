...
문서 수집 파이프라인을 어떻게 완성해야 운영에서도 안정적으로 동작할까요?

이 글은 그 질문에 답하기 위해 파이프라인 완성의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- 로딩, 청킹, 임베딩, FAISS 저장·재로딩을 한 번에 어떻게 연결할까요?
- 전체 파이프라인을 검증할 때 최소한 어떤 출력이 있어야 할까요?
- 데모를 오프라인으로 재현하면서도 검색 흐름을 어떻게 유지할 수 있을까요?

> 완성된 수집 파이프라인은 각 단계를 더 많이 붙이는 구조가 아니라 각 단계의 입출력이 이어지는 구조입니다.

예제 코드: `/root/Github/document-ingestion-101/ko/06-pipeline-completion/main.py`

![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/06/06-01-questions-this-post-answers.ko.png)
...
![전체 수집 단계가 이어지는 파이프라인 흐름](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/06/06-01-end-to-end-ingestion-pipeline.ko.png)
...
![단계별 검증 지표가 남는 체크포인트 흐름](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/06/06-02-stage-verification-checkpoints.ko.png)
...
![모니터링과 장애 복구가 이어지는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/06/06-01-monitoring-and-recovery-path.ko.png)
...
![재시도와 재처리를 나누는 제어 구조](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/06/06-02-retry-and-replay-control.ko.png)
...
- **단계별 idempotency가 기본** — 재시도 안전성이 신뢰의 기반입니다.
- **관측이 모든 단계에** — 어디서 깨졌는지 모르면 개선이 안 됩니다.
- **실패 데이터를 quarantine** — 성공 흐름을 막지 않고 별도 처리합니다.
- **스키마 진화를 처음부터 설계** — 버전 관리 없는 파이프라인은 사고가 됩니다.
