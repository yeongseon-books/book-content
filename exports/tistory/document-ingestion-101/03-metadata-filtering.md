...
메타데이터를 어떻게 설계하고 필터링에 써야 retrieval 정확도가 올라갈까요?

이 글은 그 질문에 답하기 위해 메타데이터 필터링의 핵심 결정과 운영 함정을 살펴봅니다.
...
![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/03/03-01-questions-this-post-answers.ko.png)
...
![검색용 메타데이터 필드가 모이는 스키마](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/03/03-01-metadata-schema-design.ko.png)
...
![필터가 후보군을 줄이는 검색 흐름](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/03/03-02-how-filters-narrow-the-candidate-set.ko.png)
...
![유사도와 필터가 결합되는 처리 흐름](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/03/03-01-how-similarity-and-filters-combine.ko.png)
...
![출처와 분기를 따라가는 추적 경로](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/03/03-02-how-source-tracking-supports-audits.ko.png)
...
- **metadata가 검색의 1차 필터** — 벡터 검색 비용을 크게 줄입니다.
- **스키마를 처음에 결정** — 사후 마이그레이션은 비용이 폭증합니다.
- **값 정규화가 일관성의 기반** — 자유 문자열은 필터를 망칩니다.
- **indexing 비용을 의식** — 모든 필드를 색인하면 비용이 폭증합니다.
- **권한·보안도 metadata로** — user별 접근 제어의 표준 구현입니다.
