...
다중 포맷 문서를 어떻게 처리해야 일관된 다운스트림 형식으로 통합할 수 있을까요?

이 글은 그 질문에 답하기 위해 다중 포맷 파이프라인의 핵심 결정과 운영 함정을 살펴봅니다.
...
![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/05/05-01-questions-this-post-answers.ko.png)
...
![포맷별 로더가 선택되는 라우팅 흐름](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/05/05-01-loader-routing-by-file-format.ko.png)
...
![포맷별 전처리가 갈리는 분기 구조](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/05/05-02-format-specific-preprocessing.ko.png)
...
![공통 Document 계약이 모이는 스키마](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/05/05-01-shared-document-contract-schema.ko.png)

*공통 Document 계약이 모이는 스키마*
...
![포맷 오류를 우회하는 예외 처리 흐름](https://yeongseon-books.github.io/book-public-assets/assets/document-ingestion-101/05/05-02-error-handling-across-file-formats.ko.png)

*포맷 오류를 우회하는 예외 처리 흐름*
...
- **포맷별 어댑터로 분리** — PDF·HTML·Office가 각각 다른 도전입니다.
- **내부 표현은 통일** — 정규화된 중간 형식이 다운스트림을 단순화합니다.
- **실패는 포맷 단위로 격리** — 한 포맷 사고가 전체를 멈추지 않게 합니다.
- **metadata 스키마는 공통** — 포맷이 달라도 검색·필터는 같은 인터페이스입니다.
- **샘플 audit을 정기적으로** — 추출 누락은 자동으로 안 보입니다.
