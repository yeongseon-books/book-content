...
LLM 앱 운영을 어떻게 통합해야 모니터링·비용·평가·보안·배포가 일관되게 동작할까요?

이 글은 그 질문에 답하기 위해 LLM 앱 운영 통합의 핵심 결정과 운영 함정을 살펴봅니다.
...
## 이 코드에서 봐야 할 것
![헬스 상태가 누적 호출과 비용을 보여 주는 구조](https://yeongseon-books.github.io/book-public-assets/assets/llm-apps-ops-101/06/06-02-what-to-notice-in-this-code.ko.png)
...
## 실무에서 헷갈리는 지점
![배포 모니터링 평가 최적화가 다시 배포로 돌아오는 루프](https://yeongseon-books.github.io/book-public-assets/assets/llm-apps-ops-101/06/06-03-where-engineers-get-confused.ko.png)
...
- **관측이 모든 의사결정의 기반** — 데이터 없이는 개선이 불가능합니다.
- **비용·품질·안전이 동등한 축** — 한 가지만 보면 다른 게 무너집니다.
- **회귀 테스트가 진화의 보호막** — 변경은 회귀의 위험을 동반합니다.
- **on-call·runbook을 처음부터** — 사고 시 사람의 행동 지침이 결정적입니다.
- **회고로 운영을 진화시킨다** — 정기적 점검이 마찰을 줄입니다.
