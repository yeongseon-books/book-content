...
**핵심 질문**: 변경에 강하고 확장에 유연한 클래스를 설계하려면 어떤 원칙을 따라야 할까요?

> SOLID는 객체지향 설계의 5가지 핵심 원칙입니다. 단일 책임, 개방-폐쇄, 리스코프 치환, 인터페이스 분리, 의존성 역전 원칙을 Python 코드로 하나씩 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- SOLID 5가지 원칙의 정의와 목적
- 각 원칙의 위반 사례와 개선 코드
- Python에서 SOLID를 적용하는 실용적 방법
- 원칙 간의 관계와 트레이드오프
...
- **SRP 우선** — 단일 책임 위반이 가장 흔한 통증입니다.
- **OCP 현실** — 확장 지점은 실제 변동이 발견된 뒤 만듭니다.
- **LSP** — 하위 타입의 계약 일치 검증을 자동화합니다.
- **ISP** — 넓은 인터페이스는 작게 쪼갭니다.
- **DIP** — 구체보다 추상에 의존합니다.
...
1. SRP를 위반하는 `OrderProcessor` 클래스(주문 처리 + 결제 + 이메일)를 3개의 클래스로 분리하세요.
2. OCP를 적용하여 새 파일 형식(CSV, JSON, XML)을 추가할 때 기존 코드를 수정하지 않는 `FileExporter`를 설계하세요.
3. DIP를 적용하여 `WeatherApp`이 다양한 날씨 API(OpenWeather, WeatherAPI)를 교체 가능하게 만드세요.
