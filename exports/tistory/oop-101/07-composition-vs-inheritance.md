...
**핵심 질문**: 클래스를 확장할 때 상속과 합성 중 어떤 것을 선택해야 할까요?

> "상속보다 합성을 선호하라"는 GoF 디자인 패턴의 핵심 원칙입니다. 상속은 is-a 관계에, 합성은 has-a 관계에 적합합니다. 이 글에서는 두 접근의 구조적 차이와 실무에서의 선택 기준을 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 상속(is-a)과 합성(has-a)의 구조적 차이
- 합성이 상속보다 유연한 이유
- 위임(delegation) 패턴
- 실무에서 상속과 합성을 선택하는 기준
...
- **기본은 합성** — 확신이 없으면 합성으로 시작합니다.
- **의존성 주입** — 합성은 DI와 자연스럽게 맞닿습니다.
- **테스트 가성비** — 합성 구조가 테스트 가성비가 큽니다.
- **리팩터링 유연성** — 합성은 변경 비용이 낮습니다.
- **상속 자격** — is-a가 강하게 성립할 때만 상속을 씁니다.
...
1. `NotificationService`를 합성으로 설계하여 `EmailSender`, `SmsSender`를 교체 가능하게 만드세요.
2. 상속으로 구현된 `AdminUser(User)` 코드를 합성으로 리팩터링하세요.
3. 전략 패턴으로 `Compressor` 클래스를 만들어 gzip, bzip2, lzma 압축 알고리즘을 교체하세요.
