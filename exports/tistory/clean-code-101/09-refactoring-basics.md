...
**핵심 질문**: 리팩토링은 언제 멈춰야 하나요?

> 다음 변경이 더 쉬워졌을 때입니다. 더 깨끗해 보일 때가 아니라.
...
- **Refactoring**: 외부 동작을 바꾸지 않는 내부 개선.
- **Characterization test**: 현재 동작을 잡아두는 테스트.
- **Code smell**: 개선 신호 (긴 함수, 큰 클래스, 데이터 뭉치 등).
- **Two hats**: 기능 추가와 리팩토링을 동시에 하지 않기.
- **Mikado method**: 큰 변경을 작은 단계의 그래프로 분해.
...
- 다음 변경이 쉬워질 때만 리팩토링한다.
- 작은 걸음, 빠른 테스트.
- 두 모자(기능/리팩토링)를 분리한다.
- 레거시는 특성화 테스트로 시작한다.
- 큰 변경은 Mikado 그래프로 쪼갠다.
...
1. 50줄 이상 함수 1개를 특성화 테스트로 잡고, 3단계로 분해해 보세요.
2. 부적절한 위치의 함수 1개를 더 적절한 모듈로 이동시키세요.
3. 리팩토링 전용 PR을 1개 만들어 머지해 보세요.
