...
**핵심 질문**: 중첩된 if는 왜 그렇게 빨리 늘어나는가요?

> 한 함수에 책임이 두 개 이상 들어 있기 때문입니다. 책임을 분리하면 깊이가 줄어듭니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 가드 절(Guard Clause)과 조기 반환(Early Return)
- 부정 조건과 이중 부정을 피하는 법
- 다형성으로 if/else 사슬 제거하기
- 전략 패턴(Strategy)으로 분기 분리하기
- 테이블 주도(Table-driven) 분기
...
- **Guard Clause**: 함수 앞부분에서 예외 케이스를 빠르게 반환.
- **Early Return**: 깊은 중첩 대신 조기 종료.
- **Polymorphism**: 타입별 동작을 클래스로 분리.
- **Strategy Pattern**: 알고리즘을 외부에서 주입.
- **Table-driven**: 분기를 자료구조로 표현.
...
- 깊이 3을 넘으면 설계 문제로 본다.
- if/elif가 5개 넘으면 다형성을 의심한다.
- 분기가 외부 입력에 따라 자주 바뀌면 테이블로 옮긴다.
- 부정 조건을 한 번에 양수로 바꾼다.
- 전략은 무상태로 만든다.
...
1. 본인 코드에서 깊이 4 이상 분기를 찾아 평탄화하세요.
2. if/elif 5개 이상 사슬을 테이블로 바꿔 보세요.
3. isinstance 분기를 다형성으로 바꿔 보세요.
