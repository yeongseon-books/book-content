...
- GPT 모델 클래스는 어떤 부품을 어떤 순서로 호출할까요?
- weight tying은 왜 파라미터를 줄이면서 성능 손해가 없을까요?
- cross-entropy loss를 한 줄로 끝낼 수 있는 이유는 무엇일까요?
- config dataclass로 하이퍼파라미터를 관리하면 어떤 이득이 있을까요?
...
![GPT 순전파 경로의 전체 구조](https://yeongseon-books.github.io/book-public-assets/assets/llm-from-scratch-101/05/05-01-the-forward-pass-at-a-glance.ko.png)
...
- **초기화 일관성** — 각 모듈 초기화를 일관 정책으로 통일합니다.
- **매개변수 수** — 추정 모델 크기를 사전에 계산합니다.
- **저장/로드** — state_dict 키 안정성이 재현성에 중요합니다.
- **Mixed precision** — fp16/bf16 사용 시 NaN 모니터링을 강제합니다.
- **단위 테스트** — decode 일관성·shape 검증을 자동화합니다.

## 체크리스트

- [ ] 80줄 GPT 클래스의 forward pass를 끝까지 따라가 봤다.
- [ ] weight tying 전후의 파라미터 수 차이를 출력했다.
- [ ] 학습 전 sanity check로 forward와 loss를 한 번 돌렸다.
- [ ] GPTConfig dataclass의 모든 필드 의미를 설명할 수 있다.
