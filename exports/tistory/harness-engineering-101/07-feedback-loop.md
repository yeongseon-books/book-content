...
실패를 어떻게 다시 시도하게 해야 Agent가 스스로 고칠 수 있을까요?

이 글은 그 질문에 답하기 위해 Feedback Loop의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **loop는 무한이 아니라 유한** — 최대 시도 횟수를 명시합니다.
- **실패 메시지를 구조화해 전달** — stack trace·diff가 가장 강한 신호입니다.
- **진전 없으면 중단** — 같은 실패의 반복은 사람 개입 신호입니다.
- **partial progress를 보존** — 전체 재시도는 비용을 폭증시킵니다.
- **학습 가능한 실패와 환경 사고를 구분** — 재시도가 의미 있는 경우만 반복합니다.
