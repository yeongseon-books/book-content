...
완료 조건을 테스트로 어떻게 고정해야 Agent가 자기 검증할 수 있을까요?

이 글은 그 질문에 답하기 위해 Test Harness의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **테스트가 success criteria의 단일 출처** — 모호한 '완료'를 코드로 표현합니다.
- **self-verifying이 자율성의 기반** — Agent가 스스로 통과를 확인합니다.
- **작은 테스트가 빠른 피드백** — 큰 통합 테스트는 진단을 어렵게 합니다.
- **테스트는 환경 전체를 검증** — 코드만이 아니라 데이터·연결도 봅니다.
- **실패 메시지가 다음 시도의 입력** — 에러가 학습 신호가 됩니다.
