...
Tool Use로 LLM에 외부 능력을 부여할 때 어떻게 설계해야 안전하고 예측 가능할까요?

이 글은 그 질문에 답하기 위해 AI 에이전트와 Tool Use의 핵심 개념과 실무 고려사항을 단계별로 살펴봅니다.
...
![챗봇과 AI 에이전트의 차이](https://yeongseon-books.github.io/book-public-assets/assets/ai-web-dev-101/05/assistant-vs-agent.ko.png)
...
- **도구 스키마가 곧 계약이다** — 이름·설명·입력 타입이 명확해야 모델이 올바른 도구를 선택합니다.
- **부작용은 승인 단계를 둔다** — 쓰기·결제·외부 호출은 자동 실행 대신 사용자 확인을 거치는 게 안전합니다.
- **루프에 상한을 건다** — 최대 반복 횟수·시간·비용을 제한해 무한 루프와 비용 폭주를 막습니다.
- **도구 실패는 모델에 다시 알려준다** — 에러를 그대로 종료하지 말고 컨텍스트로 돌려줘야 모델이 우회할 수 있습니다.
- **관측을 처음부터 심는다** — 어떤 도구가 언제 왜 호출됐는지 로그로 남겨야 디버깅이 가능합니다.
