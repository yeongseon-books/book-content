...
![AI API 호출의 기본 흐름](https://yeongseon-books.github.io/book-public-assets/assets/ai-web-dev-101/01/api-call-overview.ko.png)
...
- **API 키는 절대 클라이언트에 두지 않는다** — 키가 노출되면 즉시 비용이 발생하므로 백엔드·환경변수·시크릿 매니저에만 둡니다.
- **토큰 단위로 비용을 사고한다** — 요청·응답 모두 과금되므로 max_tokens·프롬프트 길이를 항상 의식합니다.
- **실패는 기본 시나리오로 본다** — rate limit·타임아웃·일시 장애는 일상이므로 재시도와 폴백을 처음부터 넣습니다.
- **로깅은 민감정보를 가린다** — 프롬프트·응답에 PII가 섞이기 쉬우므로 마스킹과 보존 기간을 정해 둡니다.
- **모델 버전을 고정한다** — 모델이 바뀌면 출력도 바뀌므로 버전을 명시해야 회귀를 통제할 수 있습니다.
