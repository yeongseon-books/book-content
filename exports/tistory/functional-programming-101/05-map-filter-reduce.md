...
**핵심 질문**: 반복문 없이 컬렉션을 변환, 필터링, 집계하려면 어떻게 해야 할까요?

> `map`, `filter`, `reduce`는 함수형 프로그래밍의 3대 연산입니다. 반복문의 세부 사항을 숨기고 "무엇을"에 집중하는 선언적 데이터 처리를 가능하게 합니다. 이 글에서는 각 함수의 원리, Python에서의 활용, 리스트 컴프리헨션과의 비교를 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- map, filter, reduce의 동작 원리
- 리스트 컴프리헨션 vs map/filter 비교
- reduce로 복잡한 집계 구현하기
- itertools와의 조합 활용
...
- **컴프리헨션 우선** — map/filter보다 컴프리헨션이 보통 더 명료합니다.
- **Reduce 절제** — reduce는 모나드성·결합성이 명확할 때만 씁니다.
- **지연 평가** — map/filter는 lazy라는 점을 활용합니다.
- **부작용 금지** — 변환 단계에 부수효과를 섞지 않습니다.
- **성능** — 큰 변환은 numpy/pandas가 보통 더 빠릅니다.
...
1. 학생 딕셔너리 리스트에서 map으로 이름만 추출하고 filter로 90점 이상을 선택하세요.
2. reduce로 문자열 리스트를 하나의 CSV 문자열로 결합하세요.
3. 같은 작업을 map/filter/reduce와 리스트 컴프리헨션 두 가지 방식으로 구현하고 비교하세요.
