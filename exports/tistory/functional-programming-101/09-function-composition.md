...
**핵심 질문**: 작은 함수들을 레고처럼 조립하여 복잡한 변환을 만들 수 있을까요?

> 함수 합성은 여러 함수를 결합하여 새로운 함수를 만드는 기법입니다. 파이프라인은 데이터가 함수 체인을 통과하며 변환되는 패턴입니다. 이 글에서는 Python에서 함수 합성과 파이프라인을 구현하는 실용적 방법을 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 함수 합성의 수학적 원리와 Python 구현
- pipe와 compose의 차이
- 타입 안전한 파이프라인 구성
- 실전 데이터 변환 파이프라인 설계
...
- **작은 함수** — 합성을 위해 함수를 단일 책임으로 작게 유지합니다.
- **타입 정합성** — 합성 가능한 타입 시그니처가 핵심입니다.
- **파이프라인 표현** — 데이터 흐름이 위에서 아래로 읽히게 둡니다.
- **디버깅 훅** — 단계 사이에 관측 훅을 넣을 수 있게 둡니다.
- **재사용성** — 합성 가능한 작은 함수가 가장 큰 자산입니다.
...
1. `compose`와 `pipe`를 구현하고 같은 결과를 반환하는지 확인하세요.
2. JSON 데이터를 읽어 필터링, 변환, 정렬, 포맷팅하는 4단계 파이프라인을 작성하세요.
3. 에러 처리가 포함된 파이프라인(`Result` 타입)을 구현하세요.
