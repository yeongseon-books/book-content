...
Prompt Injection을 어떻게 방어해야 사용자 입력의 신뢰 경계가 깨지지 않을까요?

이 글은 그 질문에 답하기 위해 Prompt Injection 방어의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **instruction과 data를 구조적으로 분리** — delimiter·structured input이 표준입니다.
- **indirect injection이 더 위험** — 외부 문서·URL을 통한 공격을 의식합니다.
- **최소 권한 원칙을 도구에** — model이 호출하는 tool의 권한을 좁힙니다.
- **탐지 모델을 함께 사용** — 휴리스틱과 LLM 판정을 결합합니다.
