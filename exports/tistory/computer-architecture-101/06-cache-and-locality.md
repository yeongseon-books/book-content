...
**핵심 질문**: 같은 데이터를 같은 횟수만큼 읽는데, 한쪽 코드는 1초, 다른 쪽 코드는 30초가 걸립니다. 무엇이 그 차이를 만드나요?

> CPU와 메인 메모리 사이의 속도 차이는 100배가 넘습니다. 그 격차를 메우는 것이 캐시이고, 캐시가 효과를 보려면 우리 코드가 시간 지역성과 공간 지역성을 따라야 합니다. 이 글은 캐시의 작동 원리, 지역성의 두 종류, 그리고 캐시 친화적 코드와 적대적 코드의 차이를 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 캐시의 정의와 메모리 계층 안에서의 위치
- 시간 지역성과 공간 지역성
- 캐시 라인과 그것이 만드는 비용
- 캐시 친화적 코드 작성 패턴
...
| 용어 | 설명 |
| --- | --- |
| 캐시 라인 | 캐시의 최소 단위(보통 64바이트) |
| 시간 지역성 | 최근 접근한 데이터를 곧 다시 씀 |
| 공간 지역성 | 인접 주소를 곧 함께 씀 |
| 캐시 미스 | 원하는 데이터가 캐시에 없음 |
| Working set | 현재 자주 접근하는 데이터의 합 |
...
시니어 엔지니어는 핫 코드를 두고 "이 코드의 워킹셋이 L1, L2, L3 중 어디에 들어가는가?"를 추정합니다. L1(보통 32KB)을 넘기면 즉시 비용이 붙고, L3까지 못 들어가면 메인 메모리 접근이 시작됩니다. 데이터 크기를 기준으로 어느 캐시 단계에서 처리되는지 가늠하는 감각이 있어야 측정 결과를 해석할 수 있습니다.

또한 시니어는 "측정하지 않은 캐시 최적화는 미신"이라는 점을 지킵니다. 캐시 동작은 CPU와 데이터 크기에 따라 비선형이라, 머릿속 모델은 항상 측정으로 검증해야 합니다. `perf stat`, `valgrind --tool=cachegrind`, 시간 측정으로 가설을 확인합니다.
...
1. 1024×1024 numpy 배열을 행 우선과 열 우선으로 합산하는 코드를 작성하고 시간을 측정하세요. 차이가 몇 배 나는지, 그 비율이 cache line 크기와 어떤 관계인지 분석합니다.

2. `lru_cache`를 사용한 함수와 사용하지 않은 함수의 시간 지역성을 비교하세요. 동일 입력을 1000번 호출하고 평균 시간을 봅니다.

3. AoS 스타일(Python class)과 SoA 스타일(numpy array)로 입자 1만 개의 위치를 갱신하는 코드를 각각 작성하고 시간을 비교하세요.
