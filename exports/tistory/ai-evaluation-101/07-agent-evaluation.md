...
에이전트는 여러 단계를 거쳐 답을 만듭니다. 최종 응답만이 아니라 '어떤 도구를, 어떤 순서로, 몇 번 호출했는가'를 평가해야 합니다. 이 글은 trajectory-level 평가 방법을 다룹니다.

---
![Agent 평가하기 - 단일 응답이 아닌 trajectory](https://yeongseon-books.github.io/book-public-assets/assets/ai-evaluation-101/07/07-01-evaluating-agents-trajectories-not-singl.ko.png)

*Agent 평가하기 - 단일 응답이 아닌 trajectory*
<!-- a-grade-intro:begin -->
## 핵심 질문

Agent 평가는 왜 단일 응답이 아닌 trajectory를 봐야 할까요?

이 글은 그 질문에 답하기 위해 Agent 평가의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## Agent 평가는 왜 다른가

![Agent 평가의 특수성](https://yeongseon-books.github.io/book-public-assets/assets/ai-evaluation-101/07/07-02-why-agent-evaluation-is-different.ko.png)
...
![평가의 두 층위 - End-to-end vs Step-level](https://yeongseon-books.github.io/book-public-assets/assets/ai-evaluation-101/07/07-03-two-levels-of-evaluation-end-to-end-vs-s.ko.png)
...
![Tool Selection 평가 - Confusion Matrix](https://yeongseon-books.github.io/book-public-assets/assets/ai-evaluation-101/07/07-04-tool-selection-confusion-matrix.ko.png)
...
# send_email         0.85    0.70    0.77   ← agent가 draft_email로 자주 잘못 고름
...
![Trajectory Efficiency - 단계 수 측정](https://yeongseon-books.github.io/book-public-assets/assets/ai-evaluation-101/07/07-05-trajectory-efficiency-step-count.ko.png)
...
# 평균 step overhead: 1.8  ← agent가 거의 2배 단계를 사용함
```
    ...
    for i, s in enumerate(steps):
        if s.get("tool_result", {}).get("error"):
            ...
            if next_step.get("tool") == s["tool"]:
                return "RETRIED"  # 같은 tool 재시도
            if "tool" in next_step:
                return "ALTERNATIVE"  # 다른 tool 시도
            return "GAVE_UP"
    return "NO_FAILURE"
...
# → 16% 케이스에서 agent가 포기. recovery prompt 보강 필요.
```
...
- **성공·실패만으로 부족** — 어떤 경로로 도달했는지가 진단의 핵심입니다.
- **tool call 단위로 평가** — 잘못된 도구 호출이 가장 흔한 실패입니다.
- **step별 정확도와 종단 정확도 분리** — 어디서 무너졌는지 좁힐 수 있습니다.
- **trajectory length가 비용·품질 변수** — 긴 경로는 누적 오류를 키웁니다.
- **real environment 평가가 어려운 도전** — 시뮬레이션이 종종 불가피합니다.
