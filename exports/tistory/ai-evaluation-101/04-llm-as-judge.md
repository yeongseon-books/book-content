...
- **프롬프트가 거의 모든 것을 결정** — rubric·예시·output format이 결과를 좌우합니다.
- **position bias·verbosity bias 의식** — 여러 알려진 편향을 사전에 방어합니다.
- **judge 모델은 평가 대상보다 강해야** — 그렇지 않으면 신호가 약해집니다.
- **샘플 audit이 신뢰의 기반** — 사람 판정과 일치율을 정기적으로 확인합니다.
- **비용을 의식적으로 관리** — 전수 평가는 빠르게 비싸집니다.
...
- **Ep4 LLM-as-Judge — 모델로 모델을 평가하기 (현재 글)**
- Ep5 Rubric 기반 다차원 채점 (예정)
- Ep6 RAG 평가 (예정)
- Ep7 Agent 평가 (예정)
- Ep8 회귀 테스트 (예정)
- Ep9 LLM A/B 테스트 (예정)
