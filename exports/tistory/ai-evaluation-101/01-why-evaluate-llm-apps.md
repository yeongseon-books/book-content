...
LLM 애플리케이션 평가가 왜 일반 소프트웨어 테스트와 본질적으로 다를까요?

이 글은 그 질문에 답하기 위해 LLM 평가의 본질의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **출력이 비결정적** — 단일 정답이 아닌 분포로 평가해야 합니다.
- **자동 메트릭은 부분적** — 사람·LLM 판정과 결합해야 신뢰성이 생깁니다.
- **eval set이 단일 출처** — 작더라도 한 곳에 모입니다.
- **회귀를 막는 게 첫 가치** — 어제 잘 되던 게 오늘 망가지는 것을 잡습니다.
- **프로덕션 신호와 연결** — 오프라인 평가만으로는 부족합니다.
