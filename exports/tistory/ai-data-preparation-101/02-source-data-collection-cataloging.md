...
원본 데이터 수집과 카탈로깅을 어떻게 해야 출처를 잃지 않고 확장할 수 있을까요?

이 글은 그 질문에 답하기 위해 수집과 카탈로깅의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **데이터 카탈로그가 단일 출처** — 출처·라이선스·버전을 한 곳에서 관리합니다.
- **수집 시점에 메타데이터 기록** — 사후 추적은 비용이 폭증합니다.
- **법적·윤리적 검증을 처음에** — robots.txt·ToS·라이선스를 자동 검사합니다.
- **스키마를 처음부터 정의** — ad-hoc 수집은 후속 정제 비용을 키웁니다.
- **증분 수집을 표준으로** — 전체 재수집은 비용·일관성 모두 깨집니다.
