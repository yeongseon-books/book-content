...
![아키텍처 - 6 stage pipeline](https://yeongseon-books.github.io/book-public-assets/assets/ai-data-preparation-101/10/10-01-6-stage-pipeline.ko.png)
...
- **재현성이 1순위** — 코드·데이터·환경을 모두 버전 관리합니다.
- **증분 처리를 표준으로** — 전체 재처리는 비용·일관성 모두 깨집니다.
- **관측·알림이 신뢰의 기반** — 데이터 사고는 모델 품질로만 드러납니다.
- **스키마 진화를 처음부터 설계** — 사후 마이그레이션은 비용이 폭증합니다.
- **거버넌스 메타데이터를 함께 저장** — 출처·라이선스·동의를 잃지 않습니다.
