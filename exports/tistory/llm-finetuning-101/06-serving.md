...
- 파인튜닝된 모델을 FastAPI 엔드포인트로 감싸는 최소 구조를 익힙니다.
- 학습 코드와 서빙 코드의 책임 경계를 구분할 수 있습니다.
- `TestClient`로 uvicorn 없이 엔드포인트를 검증하는 방법을 실습합니다.
- LoRA 어댑터 분리 배포의 이점을 이해합니다.
...
![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/06/06-01-questions-this-post-answers.ko.png)

*이 글에서 답할 질문*

- 파인튜닝된 소형 모델을 FastAPI 엔드포인트로 감싸는 최소 구조는 무엇인가요?
- 서빙 코드에서 학습과 추론 경계를 어디서 끊어야 하나요?
- 브라우저를 열지 않고도 엔드포인트를 어떻게 실행 검증할 수 있나요?
- LoRA 어댑터를 base 모델과 분리해서 배포하면 무엇이 좋아지나요?
...
![모델 준비와 HTTP 계약 분리 구조](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/06/06-02-what-this-demo-isolates-on-purpose.ko.png)
...
![데모 서빙에서 꼭 분리해서 볼 것](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/06/06-01-what-this-demo-isolates-on-purpose.ko.png)
...
![지연 시간과 품질 사이 서빙 선택 기준](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/06/06-04-where-engineers-get-confused.ko.png)
...
- **vLLM·TGI 같은 최적화 서버 활용** — naive serving은 비용·지연 모두 깨집니다.
- **어댑터 hot-swap으로 다중 모델** — 한 base에 여러 어댑터를 운영합니다.
- **quantization이 비용·품질 트레이드오프** — use case에 맞춰 결정합니다.
- **배치·streaming 정책을 결정** — throughput과 latency의 균형입니다.
- **관측·롤백이 운영의 기본** — 모델 회귀를 빠르게 잡습니다.
...
1. `/generate` 엔드포인트에 `temperature`와 `top_p` 파라미터를 추가하고, 같은 prompt에서 출력이 어떻게 달라지는지 확인하세요.
2. `time.perf_counter()`로 단일 요청 latency를 측정하고, `max_new_tokens`을 8/32/128로 바꿔 latency가 어떻게 변하는지 비교하세요.
3. 어댑터 두 개를 준비해 `/generate`의 query parameter `?adapter=A`로 런타임 전환하는 코드를 작성해 보세요.
