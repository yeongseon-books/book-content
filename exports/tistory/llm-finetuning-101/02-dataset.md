...
- instruction / input / output 세 필드를 JSONL로 정리하는 표준 구조를 익힙니다.
- Hugging Face `datasets`로 JSONL 파일을 로드하고 프롬프트 템플릿을 적용하는 방법을 실습합니다.
- 토크나이저 전처리 후 길이 분포를 확인하는 습관을 들입니다.
- 데이터셋의 세 층(원본 → 템플릿 텍스트 → 토큰 텐서)을 분리해서 생각하는 프레임을 갖춥니다.
...
![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/02/02-01-questions-this-post-answers.ko.png)

*이 글에서 답할 질문*

- instruction, input, output 세 필드를 어떤 형태로 정리해야 할까요?
- Hugging Face datasets로 작은 JSONL 파일을 어떻게 바로 읽어 올릴까요?
- 전처리 단계에서 꼭 확인해야 할 최소 검증 포인트는 무엇일까요?
- prompt와 response의 경계를 어디에 둬야 학습이 안정적일까요?

> 좋은 파인튜닝 데이터셋은 문장 모음이 아니라, 모델이 반복해서 따라 배워야 하는 **요청-응답 계약서**입니다.
...
![데이터셋 세 층과 경계 관리 구조](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/02/02-02-the-three-layers-of-dataset-preparation.ko.png)

*데이터셋 세 층과 경계 관리 구조*

파인튜닝 데이터는 보통 세 층으로 나뉩니다. **원본 샘플**, **프롬프트 템플릿을 적용한 텍스트**, **토크나이즈된 텐서**입니다. 이 세 층을 분리해서 생각해야 필터링 문제와 토큰 길이 문제를 따로 잡을 수 있습니다.

![데이터셋에서 먼저 고정할 것](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/02/02-01-the-three-layers-of-dataset-preparation.ko.png)
...
![포맷 검증과 토큰 길이 확인 순서 흐름](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/02/02-03-what-to-notice-in-this-code.ko.png)
...
![중복 제거와 학습 분할 판단 흐름](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/02/02-04-where-engineers-get-confused.ko.png)
...
- **일관된 포맷이 첫 단추** — 혼합된 스타일은 학습을 망칩니다.
- **작아도 고품질이 우선** — 수천 건의 정제된 데이터가 표준입니다.
- **eval set을 학습 시작 전 분리** — contamination을 처음부터 막습니다.
- **품질 검증을 자동화** — 샘플 audit과 자동 검사를 결합합니다.
- **PII·라이선스·동의를 처음에** — 사후 발견은 모델 폐기 비용입니다.
...
1. 위 예제에 5개의 다른 instruction을 추가하고, 토큰 길이의 평균과 95퍼센타일을 출력해 보세요. 어떤 `max_length`가 적절한가요?
2. `input` 필드가 비어 있는 샘플(예: 단답형 질문)을 한 개 추가하고, 템플릿이 깨지지 않도록 `render()` 함수를 보강해 보세요.
3. Llama-3 chat template을 적용해 같은 데이터를 다시 토큰화해 보세요. 같은 텍스트인데 토큰 수가 왜 달라지는지 설명해 보세요.
