...
![인덱싱 단계와 검색 단계의 전체 흐름](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/06/06-01-vector-search-pipeline-from-document-ing.ko.png)

*인덱싱 단계와 검색 단계의 전체 흐름*
---

## 이 글에서 답할 질문

- 수집(ingest), 임베딩, 색인, 검색까지의 파이프라인은 어떤 단계로 분리하는 것이 깨끗한가?
- 재인덱싱(reindex)은 무엇을 트리거로 자동화해야 하는가?
- 벡터 검색만으로 부족할 때 BM25 같은 lexical 검색을 어떻게 결합하는가?
- 검색 결과를 LLM에 넘기기 전 reranker가 필요한 시점은 언제인가?
- 프로덕션에서 검색 품질을 측정하는 지표(recall@k, MRR, nDCG)는 어떻게 계산/추적하는가?

## 파이프라인 구조

![인덱싱 단계와 검색 단계의 전체 흐름](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/06/06-01-pipeline-structure.ko.png)

*인덱싱 단계와 검색 단계의 전체 흐름*
![파이프라인 구성 요소 연결 구조](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/06/06-02-pipeline-structure-2.ko.png)
...
![인덱스 구축 저장 로드 검색 실행 경로](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/06/06-03-complete-pipeline.ko.png)

*인덱스 구축 저장 로드 검색 실행 경로*
...
![벡터 점수와 BM25 점수 결합 구조](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/06/06-04-hybrid-search.ko.png)
...
![인덱스 갱신과 삭제 제약 경로](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/06/06-05-operational-considerations.ko.png)
...
- **ETL 분리** — 수집·청크·임베딩·인덱싱을 단계별로 나눠 재시도 가능하게 둡니다.
- **멱등성** — 같은 입력은 같은 결과로 수렴해야 운영이 단순해집니다.
- **증분 업데이트** — 전체 재인덱싱을 피할 수 있게 변경분 처리 전략을 둡니다.
- **관측성** — 단계별 latency·실패율을 분리 측정해 병목을 찾습니다.
- **롤백** — 임베딩/모델 교체 시 안전한 롤백 경로를 확보합니다.

## 운영 체크리스트

- [ ] ingest -> embed -> index -> search 단계가 독립 배포 가능하도록 분리했다
- [ ] 임베딩 모델/인덱스 버전을 함께 추적해 reindex 트리거를 자동화했다
- [ ] lexical + vector 하이브리드 점수 결합 방식(가중치, RRF 등)을 정의했다
- [ ] 검색 품질 회귀를 막는 평가 셋과 자동 평가 스크립트를 운영했다
- [ ] 검색 latency, recall, 비용을 동일 대시보드로 추적했다
