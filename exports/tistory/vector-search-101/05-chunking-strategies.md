...
![청크 크기와 오버랩의 구조](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/05/05-01-chunking-strategies-how-to-split-long-do.ko.png)
...
- 왜 긴 문서는 통째로 임베딩하면 안 되는가?
- 고정 길이, 문장 단위, 의미 단위 청킹은 각각 어떤 상황에 강점/약점이 있는가?
- 청크 사이의 overlap을 두는 이유와 적절한 비율은 어떻게 정하는가?
- 코드, 표, 마크다운 헤딩이 섞인 문서는 어떻게 청킹해야 검색 품질이 올라가는가?
- 청크에 메타데이터(섹션 제목, 출처)를 같이 저장하면 어떤 이득이 있는가?

## 청크 크기와 오버랩

![청크 크기와 오버랩의 구조](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/05/05-01-chunk-size-and-overlap.ko.png)
...
![고정 크기 청킹 실행 흐름](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/05/05-02-fixed-size-chunking-from-scratch.ko.png)
...
**출력 결과**
    ...
    [0] 100자: 벡터 검색은 텍스트를 수치 벡터로 변환해 의미 기반으로 검색하는 방법입니다. 임베딩 모델은 유사한 의미의 텍...

    [1] 100자: 다. FAISS는 Facebook AI Research에서 개발한 고속 벡터 검색 라이브러리입니다. 청크 전...

    [2] 96자:  수 있는 단위로 나누는 방법입니다. 적절한 청크 크기를 선택하면 검색 정확도를 높일 수 있습니다. 오버랩을...

    [3] 16자: 는 문제를 줄일 수 있습니다....
...
![분리자 우선순위 탐색 경로](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/05/05-03-recursivecharactertextsplitter.ko.png)
...
**출력 결과**
    ...
    [0] 83자:
      벡터 검색은 텍스트를 수치 벡터로 변환해 의미 기반으로 검색하는 방법입니다.
    키워드 검색과 달리 표현이 달라도 의미가 같으면 검색 결과에 포함됩...

    [1] 142자:
      임베딩 모델은 유사한 의미의 텍스트를 벡터 공간에서 가깝게 배치합니다.
    sentence-transformers 라이브러리는 문장 수준 임베딩에 ...

    [2] 153자:
      FAISS는 Facebook AI Research에서 개발한 고속 벡터 검색 라이브러리입니다.
    정확 검색과 근사 검색 모두 지원하며, 수십억 개...
...
![청킹에서 FAISS 검색까지의 실행 경로](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/05/05-04-full-pipeline-chunking-to-faiss.ko.png)
...
**출력 결과**
    ...
    쿼리: '벡터 검색 원리'
      [1] 0.2964 — 벡터 검색은 텍스트를 수치 벡터로 변환해 의미 기반으로 검색하는 방법입니다.
    키워드 검색과 달리 표현이 달라...
      [2] 0.2643 — FAISS는 Facebook AI Research에서 개발한 고속 벡터 검색 라이브러리입니다.
    정확 검색과 ...

    쿼리: 'FAISS 라이브러리 특징'
      [1] 0.2722 — 벡터 검색은 텍스트를 수치 벡터로 변환해 의미 기반으로 검색하는 방법입니다.
    키워드 검색과 달리 표현이 달라...
      [2] 0.2706 — FAISS는 Facebook AI Research에서 개발한 고속 벡터 검색 라이브러리입니다.
    정확 검색과 ...

    쿼리: '청크 크기 설정'
      [1] 0.2764 — 벡터 검색은 텍스트를 수치 벡터로 변환해 의미 기반으로 검색하는 방법입니다.
    키워드 검색과 달리 표현이 달라...
      [2] 0.1859 — FAISS는 Facebook AI Research에서 개발한 고속 벡터 검색 라이브러리입니다.
    정확 검색과 ...
...
![청크 크기에 따른 검색 품질 비교](https://yeongseon-books.github.io/book-public-assets/assets/vector-search-101/05/05-05-how-chunk-size-affects-retrieval.ko.png)
...
- **문서 구조 우선** — 헤딩·문단 구조를 먼저 활용한 뒤 토큰 길이를 적용합니다.
- **Overlap** — 약간의 겹침이 경계 손실을 줄이지만 비용을 늘립니다.
- **청크 크기** — 검색 의도에 따라 작은 청크와 큰 청크를 다르게 운영합니다.
- **메타 보존** — 출처·페이지·헤딩 메타를 청크에 함께 저장합니다.
- **평가로 검증** — 청크 변경은 회귀 셋으로 정량 비교한 뒤 채택합니다.

## 운영 체크리스트

- [ ] 청크 크기와 overlap을 사용 모델의 토큰 한도 기반으로 정했다
- [ ] 구조 정보(헤딩, 페이지)를 청크 메타데이터로 보존했다
- [ ] 코드/표는 일반 텍스트와 다른 청킹 규칙으로 분리했다
- [ ] 동일 문서에서 발생할 수 있는 중복 청크를 dedup 했다
- [ ] 청크별 문서 ID와 위치를 추적해 출처(citation)를 노출 가능하게 했다
