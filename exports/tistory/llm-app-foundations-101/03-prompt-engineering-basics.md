...
아래 다이어그램은 채팅 프롬프트가 역할별 메시지로 조립되는 기본 구조를 보여 줍니다.

![프롬프트 엔지니어링 기초: System·User·Assistant 역할](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/03/03-01-prompt-engineering-basics-system-user-an.ko.png)
...
System·User·Assistant 역할을 어떻게 활용해야 프롬프트의 의도가 안정적으로 전달될까요?

이 글은 그 질문에 답하기 위해 프롬프트 엔지니어링 기초의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- system·user·assistant 세 역할은 모델 입장에서 어떻게 다르게 해석되는가?
- system 메시지 한 줄이 답변을 어떻게 구체적으로 바꾸는지 어떤 실험으로 확인할 수 있는가?
- assistant 메시지를 직접 채워 멀티턴 이력을 만드는 패턴은 무엇이고 언제 유용한가?
- 프롬프트가 길어질 때 system에 둘 것, user에 둘 것, 예시로 둘 것을 어떻게 가르는가?
- 같은 요청을 system 없이/있이 두 번 보내면 출력 품질의 차이가 어디서 가장 크게 나타나는가?
...
![역할별 메시지가 한 배열로 모이는 구조](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/03/03-01-understanding-the-three-roles.ko.png)

*역할별 메시지가 한 배열로 모이는 구조*
...
![system 유무에 따라 달라지는 답변 흐름](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/03/03-02-how-a-system-message-changes-the-answer.ko.png)

*system 유무에 따라 달라지는 답변 흐름*
...
**출력 결과**
    ...
    **딕셔너리와 리스트의 차이**
    ...
    Python의 딕셔너리와 리스트는 모두 데이터를 저장하고 관리하는 데 사용할 수 있는 자료형입니다. 그러나 두 자료형은 사용 방법, 특성, 그리고 용도가 다릅니다.

    ### 1. 리스트 (List)

    리스트는 순서가 있는 데이터의 집합입니다. 각 요소는 인덱스에 의해 식별되며, 인덱스는 0부터 시작합니다.
    ...
    **리스트의 예**
    ...
    딕셔너리는 키-값의 쌍으로 구성된 데이터의 집합입니다. 각 키는 고유하고, 값은 키에 의해 식별됩니다.

    **딕셔너리의 특성**

    - 키-값의 쌍으로 구성된 데이터의 집합
    - 키는 고유하고, 값은 키에 의해 식별
    - 중복된 키가 허용되지 않습니다.
    - 키의 타입이 일치하지 않아도 됩니다.
    ...
    **딕셔너리의 예**
    ...
    - **순서**: 리스트는 순서가 있는 데이터의 집합입니다. 딕셔너리는 순서가 없습니다.
    - **식별**: 리스트는 인덱스에 의해 식별됩니다. 딕셔너리는 키에 의해 식별됩니다.
    - **중복**: 리스트는 중복된 요소가 허용됩니다. 딕셔너리는 중복된 키가 허용되지 않습니다.

    **사용 시기**

    - **리스트**: 리스트를 사용할 때는 데이터의 순서가 중요하고, 중복된 요소가 허용되는 경우에 사용합니다. 예를 들어, 이름 목록이나 숫자 목록에서 리스트를 사용할 수 있습니다.
    - **딕셔너리**: 딕셔너리를 사용할 때는 데이터의 키-값 쌍이 중요하고, 중복된 키가 허용되지 않는 경우에 사용합니다. 예를 들어, 사람의 정보나 상품의 가격 정보에서 딕셔너리를 사용할 수 있습니다.

    **요약**

    리스트와 딕셔너리는 모두 데이터를 저장하고 관리하는 데 사용할 수 있는 자료형입니다. 그러나 두 자료형은 사용 방법, 특성, 그리고 용도가 다릅니다. 리스트는 순서가 있는 데이터의 집합이고, 딕셔너리는 키-값의 쌍으로 구성된 데이터의 집합입니다. 리스트는 중복된 요소가 허용되지만, 딕셔너리는 중복된 키가 허용되지 않습니다. 리스트를 사용할 때는 데이터의 순서가 중요하고, 중복된 요소가 허용되는 경우에 사용합니다. 딕셔너리를 사용할 때는 데이터의 키-값 쌍이 중요하고, 중복된 키가 허용되지 않는 경우에 사용합니다.

    [with system]
    Python의 딕셔너리와 리스트는 모두 데이터를 저장하고 관리하는 데 사용되는 자료형입니다. 하지만 두 자료형은 사용 목적과 특징이 다릅니다.

    딕셔너리는 키-값 쌍으로 데이터를 저장하는 자료형입니다. 예를 들어, 학생의 이름과 학점을 딕셔너리 형태로 저장할 수 있습니다. 딕셔너리는 키를 사용하여 값을 찾을 수 있기 때문에, 데이터를 빠르게 검색하고 수정할 수 있습니다. 또한, 딕셔너리는 키가 중복되지 않도록 보장합니다.
    ... (truncated)
...
![assistant 응답이 다음 턴에 재사용되는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/03/03-03-building-multi-turn-history-with-assista.ko.png)

*assistant 응답이 다음 턴에 재사용되는 흐름*
...
**출력 결과**
    ...
    [assistant turn 1]
    파이썬의 리스트와 튜플은 모두 순서가 있는 컬렉션입니다. 하지만 차이점은 다음과 같습니다. 리스트는 변경 가능하고, 튜플은 변경 불가능합니다. 리스트는 []로 선언하고, 튜플은 ()로 선언합니다. 리스트는 append, insert, remove와 같은 메서드를 사용하여 요소를 추가, 삭제할 수 있지만, 튜플은 이러한 메서드를 사용할 수 없습니다. 튜플은 요소의 변경이 필요하지 않은 경우에 사용하는 것이 좋습니다.
...
![낮은 샘플링과 높은 샘플링 비교](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/03/03-04-temperature-and-top-p-consistency-versus.ko.png)
...
**출력 결과**
    ...
    [temperature=0.9]
    FastAPI는 파이썬으로 작성된 강력하고 빠른 웹 프레임워크입니다.
    FastAPI는 자동으로 Swagger 문서를 생성하고, RESTful API를 위한 강력한 기능들을 제공하여 개발과 문서화에 도움을 드립니다.
    FastAPI는 아스키 아티스트가 개발한 파이썬 프레임워크이며, 유용하고 읽을 수 있는 문서와 함께 사용자가 쉽게 API를 만들수 있습니다.
...
**출력 결과**
    ...
    **데이터 클래스란?**
    ...
    데이터 클래스(dataclass)는 Python 3.7에서 소개된 새로운 문법입니다. 데이터 클래스는 클래스를 정의할 때 자동으로 `__init__`, `__repr__`, `__eq__` 등의 메서드를 생성해 주는 기능을 제공합니다. 이는 데이터를 다루는 클래스를 더 쉽게 만들 수 있도록 도와줍니다.
    ...
    **예제 코드**
    ...
    print(p)  # Person(name='John', age=30, city='New York')
    ```

    ** 언제 쓰면 좋은지?**

    데이터 클래스는 데이터를 다루는 클래스를 만들 때 유용합니다. 예를 들어, 데이터베이스에서 데이터를 읽어와서 처리해야 하는 경우, 데이터 클래스를 사용하면 데이터를 쉽게 다루고 처리할 수 있습니다.
...
**출력 결과**
    ...
    정의: 데코레이터는 함수나 클래스에 기능을 추가하거나 변경할 수 있는 특별한 함수입니다.
    비유: 함수나 클래스에 장식을 붙여주는 것과 비슷합니다.
...
![프롬프트 실수가 출력을 흔드는 관계](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/03/03-05-common-prompt-design-mistakes.ko.png)
...
- **system은 정체성·제약을 명시** — session 전체에 걸친 규칙이 들어갑니다.
- **user·assistant 분리가 컨텍스트 표현** — conversation history의 표준입니다.
- **지시는 구체적으로** — 모호한 지시는 모호한 결과를 만듭니다.
- **출력 형식을 명시** — 구조화 출력이 다운스트림을 단순화합니다.
- **예시가 가장 강한 지시** — few-shot이 instruction보다 효과적입니다.

## 운영 체크리스트

- [ ] `messages`는 항상 `system` → `user` → `assistant` 순서를 지키도록 코드가 보장한다
- [ ] 동일한 user 입력에 대해 system 메시지 유무 두 가지 결과를 비교해 본 적이 있다
- [ ] 하드코딩된 system prompt를 별도 상수나 설정 파일로 분리해 두었다
- [ ] 멀티턴 시나리오에서 assistant 메시지를 손으로 끼워 넣은 테스트 케이스가 있다
- [ ] 출력 형식 요구사항(JSON, 표, 길이 제한 등)을 system 메시지에 명시적으로 적었다
