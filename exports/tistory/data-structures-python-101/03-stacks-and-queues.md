...
**핵심 질문**: 스택과 큐는 왜 list만으로 부족할까요?

> list로 스택은 효율적으로 구현할 수 있지만, 큐를 list로 구현하면 앞에서 꺼낼 때 O(n)이 됩니다. 이 글에서는 스택(LIFO)과 큐(FIFO)의 개념, Python 구현, 그리고 deque를 사용한 효율적 큐 구현을 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 스택(LIFO)과 큐(FIFO)의 동작 원리
- Python list로 스택을 구현하는 방법
- collections.deque로 큐를 구현하는 방법
- 실무에서 스택과 큐가 사용되는 패턴
...
- **list pop/push** — 스택은 list로 충분합니다.
- **deque** — 큐·양방향 큐는 deque가 사실상 표준입니다.
- **스레드 안전성** — 동시성에서는 queue.Queue를 검토합니다.
- **크기 제한** — deque(maxlen=...)로 단순한 슬라이딩 윈도가 가능합니다.
- **회수 패턴** — BFS·undo·괄호검사 등 패턴을 인지합니다.
...
1. 스택 두 개를 사용하여 큐를 구현하세요. (enqueue, dequeue, peek 메서드 포함)
2. 문자열을 뒤집는 함수를 스택을 사용하여 작성하세요.
3. `deque(maxlen=5)`를 만들고 원소를 10개 추가하면서 어떤 일이 일어나는지 확인하세요.
