...
**핵심 질문**: 중복 제거와 멤버십 테스트를 O(1)에 하려면 어떤 자료구조를 써야 할까요?

> Python의 set은 해시 테이블로 구현되어 원소 추가, 삭제, 검색이 O(1)입니다. 또한 합집합, 교집합, 차집합 등 수학적 집합 연산을 기본으로 제공합니다. 이 글에서는 set의 내부 동작과 실무 활용 패턴을 다룹니다.
...
- **멤버십 O(1)** — in 검사는 set이 압도적으로 빠릅니다.
- **frozenset** — 불변 집합으로 dict 키·캐시 키에 활용합니다.
- **연산 표현** — 합·교·차를 연산자로 의도를 드러냅니다.
- **순서 가정 금지** — set은 순서를 보장하지 않는다는 점을 잊지 않습니다.
- **Bloom filter** — 초대규모 멤버십은 확률 자료구조로 대체합니다.
...
1. 두 리스트의 공통 원소를 set 연산으로 구하고, 원래 순서를 유지하여 반환하는 함수를 작성하세요.
2. 여러 학생의 수강 과목을 set으로 관리하고, 모든 학생이 공통으로 수강하는 과목을 구하세요.
3. 두 텍스트 파일에서 각각 단어를 추출하고, 한 파일에만 있는 고유 단어를 대칭 차집합으로 구하세요.
