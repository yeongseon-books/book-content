...
**핵심 질문**: 한 모듈이 너무 많은 일을 한다는 것을 어떻게 알 수 있나요?

> 변경 이유가 둘 이상이면 이미 너무 많은 일을 하고 있는 것입니다.
...
- **Concern**: 시스템이 다루는 한 가지 관심.
- **Coupling**: 모듈 간 의존도. 낮을수록 좋다.
- **Cohesion**: 모듈 내 요소들의 연관도. 높을수록 좋다.
- **Cross-cutting concern**: 로깅/보안 등 여러 관심사를 가로지르는 관심.
- **Boundary**: 관심사 사이의 경계.
...
1. 본인 프로젝트의 한 모듈에서 변경 이유 3가지를 적어 보세요.
2. 한 함수에서 입력/처리/출력을 분리해 보세요.
3. 도메인 코드에서 외부 import를 모두 어댑터로 옮겨 보세요.
