...
**핵심 질문**: 지금까지 배운 9개 layer를 *하나의 프로젝트* 로 어떻게 묶어야 할까요?

> 코드만 잘 짜는 것과 *운영 가능한* 백엔드를 만드는 것은 다른 일입니다. 운영 가능성은 *구조* 에서 시작됩니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 9개 layer를 담는 *프로젝트 디렉터리 구조*
- 환경별(dev/staging/prod) 설정 분리 전략
- 관측(observability) 3대 요소를 한 페이지로
- 성능과 비용의 *기본 baseline*
- 다음에 무엇을 학습할지에 대한 *로드맵*
...
- **Project layout**: 코드의 물리적 위치 규칙.
- **Config layering**: 환경별 설정을 *덮어쓰는* 방식.
- **Secret manager**: 비밀을 *코드 밖* 에서 관리하는 시스템.
- **SLO**: 우리가 *지키기로 약속한* 성능/가용성 목표.
- **Capacity plan**: 트래픽이 N배가 되어도 견디게 하는 계획.
...
- 구조는 *지금 편한 것* 보다 *6개월 뒤 편한 것* 으로 고른다.
- 모든 새 layer는 *이름과 책임이 명확* 해야 한다.
- "이 코드는 *어디에 살아야 하지?*" 가 명확하지 않으면 구조가 잘못된 신호.
- 관측 없는 운영은 *눈 감고 운전* 하는 것과 같다.
- 좋은 구조는 *새 동료의 첫 PR 속도* 로 측정된다.
...
1. 본인의 토이 프로젝트를 위 디렉터리 구조로 *재배치* 하세요.
2. `Settings` 클래스를 만들고 `.env` 하나로 모든 환경 변수가 주입되게 하세요.
3. `/metrics` endpoint를 노출하고 Prometheus 형식의 출력이 보이는지 확인하세요.
