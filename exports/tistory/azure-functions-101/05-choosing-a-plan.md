...
Consumption·Flex·Premium·Dedicated 중 어떤 기준으로 플랜을 골라야 비용과 SLA를 동시에 잡을까요?

이 글은 그 질문에 답하기 위해 Functions 플랜 선택의 핵심 결정과 운영 함정을 살펴봅니다.
...
![플랜별 비용·예열·스케일 방식 비교](https://yeongseon-books.github.io/book-public-assets/assets/azure-functions-101/05/05-01-big-picture.ko.png)
...
- **워크로드 패턴이 출발점** — 트래픽 스파이크·기본 부하 비율이 플랜을 결정합니다.
- **Consumption은 가장 저렴하지만 변수** — 콜드 스타트와 실행 시간 한계를 받아들여야 합니다.
- **Premium은 콜드 스타트의 답** — Pre-warmed 인스턴스가 핵심 가치 제안입니다.
- **Flex는 새 표준이 될 후보** — 스케일·VNet 통합이 균형 잡힌 선택지입니다.
- **Dedicated는 다른 워크로드와 공유 시** — App Service Plan을 이미 쓰는 환경에서만 합리적입니다.
