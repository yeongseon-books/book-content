...
Host와 Worker 분리 모델을 이해하면 어떤 사고를 더 빠르게 진단할 수 있을까요?

이 글은 그 질문에 답하기 위해 Host와 Worker 분리의 핵심 결정과 운영 함정을 살펴봅니다.
...
![Host와 Worker의 역할 분리 구조](https://yeongseon-books.github.io/book-public-assets/assets/azure-functions-101/03/03-01-the-big-picture-two-processes.ko.png)
...
- **Host가 트리거·스케일을 담당** — 런타임 사고의 절반 이상이 호스트 레이어에서 발생합니다.
- **Worker가 함수 코드 실행** — 언어별 워커 격리가 호스트 안정성을 지킵니다.
- **gRPC 채널이 통신의 동맥** — 이벤트 스트림 단절이 가장 흔한 사고 신호입니다.
- **OOM·재시작 패턴을 읽는다** — 워커 크래시가 호스트 메트릭에 어떻게 보이는지 학습합니다.
- **로그는 두 레이어 모두 본다** — Application Insights에서 호스트·워커를 분리해 추적합니다.
