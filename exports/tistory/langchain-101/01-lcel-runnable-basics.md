...
![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-01-questions-this-post-answers.ko.png)
## 최소 실행 예제
...
**출력 결과**
    ...
    Low Emission Capacity Building (LECAB)로 알려진 LCEL은 지속 가능한 개발을 위한 국가와 지역의 능력과 용이성을 강화하는 목적으로 개발된 프로그램입니다. LCEL의 주요 목표는 기후 변화와 환경 문제를 해결하기 위한 기술과 지식의 접근성을 높이고, 국가와 지역의 저탄소 경제 전환에 참여할 수 있는 능력을 증진하는 것입니다.

    LCEL은 전 세계적으로 저탄소 경제 전환에 참여하는 국가와 지역을 지원하기 위해 설계되었습니다. 이 프로그램은 국가와 지역의 저탄소 경제 전환에 필요한 기술, 지식, 및 자원의 접근성을 높이기 위해 국가와 지역의 능력을 강화하는 것을 목표로 합니다. LCEL은 또한 국가와 지역이 저탄소 경제 전환에 참여할 수 있는 능력을 증진하기 위해 국가 및 지역별 저탄소 경제 전환을 위한 전략을 개발하는 역할도 있습니다.
...
![핵심 흐름 한눈에 보기](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-02-the-flow-at-a-glance.ko.png)
...
![반복 연결 코드와 LCEL 추상화 구조](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-01-langchain.ko.png)
...
![invoke batch stream 실행 방식 비교](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-02-runnable.ko.png)
...
**출력 결과**
    ...
    파이썬은 간단하고易 읽을 수 있는 코드를 작성할 수 있는 동적 타입의 프로그래밍 언어로, 개발 비용과 시간을 절약할 수 있습니다. 또한, 파이썬은 다양한 분야에 적용할 수 있는 대화식 shell을 제공하고, 다양한 라이브러리를 통해 개발 속도를 빠르게 할 수 있습니다.
...
![프롬프트 모델 파서 타입 연결 흐름](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-03-lcel.ko.png)
...
**출력 결과**
    ...
    임베딩 벡터는 단어, 문장, 또는 기타 텍스트 데이터를 고維의 벡터 공간으로 매핑하여 유사한 데이터를 가까운 위치에 있도록 함으로써 텍스트 데이터의 구조를 압축하는 기법입니다. 이 벡터는 텍스트 데이터의 의미를 표현할 수 있는 밀도 표현을 가능하게 해주고, 유사한 텍스트 데이터를 찾거나 분류하는 등의 텍스트 분석 작업에서 많이 사용됩니다.
...
**출력 결과**
    ...
    === 2단계: AIMessage ===
      타입: AIMessage
      내용: 임베딩 벡터는 일정한 차원으로 나누어져 있는 벡터를 의미합니다. 이러한 벡터는 특정 단어나 텍스트를 고유한 공간에서 표현하기 위해 사용됩니다....

    === 3단계: 문자열 ===
      임베딩 벡터는 일정한 차원으로 나누어져 있는 벡터를 의미합니다. 이러한 벡터는 특정 단어나 텍스트를 고유한 공간에서 표현하기 위해 사용됩니다.
...
**출력 결과**
    ...
    벡터 검색은 텍스트를 숫자 벡터로 변환하여 의미 기반으로 검색하는 기술입니다.
...
![batch 호출의 입력 분배와 결과 수집 흐름](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-04-batch.ko.png)
...
**출력 결과**
    ...
    [임베딩] 임베딩은 고차원 데이터를 저차원 데이터로 매핑하는 방법입니다. 단어, 이미지, 소리 등 다양한 형태의 데이터를 유의미한 유사도와 차이를 측정할 수 있도록 벡터로 표현하여 같은 의미의 데이터가 가까운 위치에 있도록 만드는 방식입니다.

    [FAISS] FAISS(Falcons Accelerated Infrastructure for Similarity Search)란 밀도 인코딩 기법을 사용하여 고밀도 데이터셋에서 유사성 검색을 빠르게 수행할 수 있는 오픈 소스 라이브러리입니다.

    [RAG] RAG는 Risk, Action, Goal의 약자로, 목표 달성을 위해 위험 분석, 목표 설정, 행동 계획 등을 체계적으로 구분하여 진행하는 프로세스를 의미합니다.
