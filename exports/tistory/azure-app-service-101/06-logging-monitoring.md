...
App Service의 관측을 어떻게 구축해야 사고를 빠르게 진단할 수 있을까요?

이 글은 그 질문에 답하기 위해 로깅과 모니터링의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- App Service의 진단 로그(HTTP, application, container)는 어디에 어떻게 쌓이는가?
- Application Insights와 Log Analytics, Diagnostic Settings는 어떻게 역할이 갈리는가?
- live tail과 log streaming은 언제 사용하고 언제 한계를 만나는가?
- 유용한 알람(메모리, HTTP 5xx, 응답 시간)은 어떤 임계값으로 잡는가?
- 디스크 quota 초과나 외부 의존성 장애는 어떤 신호로 먼저 감지되는가?

## 먼저 큰 그림: App Service에서 로그는 어디로 가는가?

문제를 풀기 전에 로그의 목적지를 먼저 이해해야 합니다. 같은 `logger.info()`라도 **어디에 남느냐에 따라 쓸모가 완전히 달라지기 때문**입니다.

![앱 로그가 파일과 모니터링으로 가는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-101/06/01-log-flow-architecture.ko.png)
...
![로그에서 추적까지 올라가는 관측 단계](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-101/06/02-observability-maturity.ko.png)
...
반대로 **자격 증명, 토큰, 카드번호, 주민등록번호 같은 민감 정보는 로그에 남기면 안 됩니다.** 구조화 로그는 검색이 쉬운 만큼, 잘못 남긴 정보도 더 빨리 퍼집니다.
...
![한 요청에 Correlation ID가 이어지는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-101/06/03-correlation-id-flow.ko.png)
...
그리고 운영에서는 여기서 자연스럽게 이런 질문이 나옵니다.

> “원인을 찾았더니 트래픽이 진짜 늘어난 거라면, 이제 어떻게 확장해야 하지?”

이 질문은 곧 Scale Up과 Scale Out의 경계, 그리고 Autoscale을 언제 어떻게 설계해야 하는지로 이어집니다.

---

## 이 시리즈에서의 위치

이번 글은 App Service 위 애플리케이션을 보이는 시스템으로 바꾸기 위한 로그와 모니터링의 기본 구성을 정리합니다. 관측 데이터가 있어야만 언제 Scale Up과 Scale Out을 선택해야 하는지도 근거 있게 판단할 수 있습니다.

---

## 시니어 엔지니어는 이렇게 생각합니다

- **Application Insights는 출발점이다** — 로그·메트릭·trace를 한 곳에서 본다는 가치가 큽니다.
- **샘플링과 보존을 의식적으로 정한다** — 기본값으로 두면 비용이나 데이터 누락 사고가 생깁니다.
- **KQL을 표준 도구로 익힌다** — 사고 시 쿼리 능력이 진단 시간을 결정합니다.
- **알림은 SLO 기반으로 줄인다** — 알람 피로가 신뢰를 무너뜨립니다.
- **배포 마커를 메트릭에 둔다** — 회귀 원인을 빠르게 좁히는 가장 단순한 단서입니다.

## 운영 체크리스트

- [ ] App Service 진단 로그를 Log Analytics로 통합했다
- [ ] Application Insights를 활성화하고 sampling 정책을 정했다
- [ ] 주요 알람(메모리, 5xx, latency)에 임계값을 설정했다
- [ ] 디스크 quota와 외부 의존성 장애에 알람을 걸었다
- [ ] 장애 대응 런북에 첫 5분 액션을 명시했다
