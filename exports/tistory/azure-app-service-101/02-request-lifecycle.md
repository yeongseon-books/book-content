...
요청이 들어와서 응답이 나가기까지의 경로를 이해하면 어떤 사고를 빠르게 진단할 수 있을까요?

이 글은 그 질문에 답하기 위해 App Service 요청 라이프사이클의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- 사용자 요청은 어떤 Azure 컴포넌트(Front End, ARR, Worker)를 거쳐 우리 앱에 도달하는가?
- ARR Affinity 쿠키는 무엇을 위해 존재하고, 언제 끄는 것이 옳은가?
- Worker가 idle에서 깨어나는 워밍업 단계는 어떤 latency를 만들어내는가?
- Worker 종료/재시작은 어떤 신호로 시작되고, 사용자 요청에 어떤 영향을 주는가?
- 동일 인스턴스에 라우팅되는 패턴이 스케일에 어떤 부작용을 만드는가?
...
![클라이언트부터 앱 응답까지의 요청 경로](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-101/02/01-full-request-lifecycle.ko.png)

*클라이언트부터 앱 응답까지의 요청 경로*
...
![Frontend의 요청 차단과 전달 조건](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-101/02/02-frontend-routing-decision.ko.png)

*Frontend의 요청 차단과 전달 조건*

여기서 봐야 할 점은 **Frontend가 건강한 백엔드를 찾지 못하면 앱 코드가 실행되기 전에 실패한다**는 점입니다.
...
Python/Flask 예시를 들었지만 언어보다 실행 계약이 더 중요합니다.
...
![Health Check 결과에 따른 인스턴스 상태 전이](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-101/02/03-health-check-state-machine.ko.png)

*Health Check 결과에 따른 인스턴스 상태 전이*
...
![상태 코드별 실패 레이어 추적 관계](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-101/02/04-failure-pattern-map.ko.png)
...
- **Front End→Worker 경로를 먼저 그린다** — 사고가 어느 레이어에서 일어났는지 추론의 출발점입니다.
- **상태 코드로 레이어를 역추적한다** — 502·503·504가 가리키는 층이 다르므로 진단 효율이 결정됩니다.
- **타임아웃 누적을 의식한다** — ARR·앱·다운스트림 타임아웃이 정렬되지 않으면 디버깅이 어렵습니다.
- **연결·풀 한도를 본다** — TCP·SNAT 한도가 트래픽 폭주에서 가장 먼저 무너집니다.
- **로그를 레이어별로 본다** — Front End·Worker·앱 로그를 함께 봐야 사고가 보입니다.
