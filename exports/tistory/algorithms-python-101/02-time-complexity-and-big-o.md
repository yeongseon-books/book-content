...
**핵심 질문**: 데이터가 10배로 늘면 실행 시간은 몇 배로 늘어날까요?

> Big-O 표기법은 입력 크기에 따른 알고리즘의 성능 변화를 나타냅니다. O(1), O(n), O(n²) 등으로 알고리즘의 확장성을 객관적으로 비교할 수 있습니다. 이 글에서는 Big-O의 정의, 주요 복잡도 클래스, Python 코드에서의 분석 방법을 다룹니다.
...
- Big-O 표기법의 정의와 의미
- 주요 시간 복잡도 클래스 (O(1)~O(2^n))
- Python 코드에서 시간 복잡도 분석 방법
- 공간 복잡도의 기본 개념
...
- **최악 vs 평균** — 둘을 분리해서 보고하고 의사결정합니다.
- **상수 무시 한계** — 실측에서는 상수가 결정적일 수 있음을 잊지 않습니다.
- **공간 복잡도** — 시간만이 아니라 공간 복잡도도 함께 봅니다.
- **입력 분포** — 최악 가정이 운영 분포와 맞는지 확인합니다.
- **프로파일링** — 복잡도 분석은 프로파일링과 함께 의미를 가집니다.
...
1. 다음 코드의 시간 복잡도를 분석하세요: 이중 루프에서 내부 루프가 `range(10)`인 경우.
2. O(n²) 알고리즘을 O(n)으로 최적화하세요: 리스트에서 합이 K인 쌍을 찾는 문제.
3. n=100, 1000, 10000에서 O(n), O(n log n), O(n²) 알고리즘의 실행 시간을 측정하고 그래프(표)로 비교하세요.
