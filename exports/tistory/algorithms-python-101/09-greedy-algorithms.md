...
- **정당성 증명** — 그리디는 정당성이 증명될 때만 안전합니다.
- **반례 탐색** — 잘 작동해 보이는 그리디도 작은 반례에서 깨질 수 있습니다.
- **정렬 전제** — 대부분의 그리디는 정렬 또는 우선순위 큐 위에서 동작합니다.
- **DP 대안** — 확신이 안 서면 DP로 푸는 편이 안전합니다.
- **간결성의 함정** — 코드가 짧다고 해서 정답이라는 보장은 없습니다.
