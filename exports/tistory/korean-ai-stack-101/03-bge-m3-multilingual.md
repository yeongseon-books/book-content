...
- BGE-M3의 다국어 임베딩이 한국어/영어 혼용 문서에서 왜 효과적인지 이해합니다.
- dense, sparse, multi-vector 세 가지 retrieval 방식을 하나의 모델로 쓰는 방법을 실습합니다.
- KoSimCSE와 BGE-M3를 같은 쿼리로 비교해 한국어 데이터에서의 성능 차이를 확인합니다.
- 다국어 검색 환경에서 임베딩 모델을 선택하는 기준을 정리합니다.

<!-- a-grade-intro:begin -->
## 핵심 질문

BGE-M3 다국어 임베딩을 운영할 때 어떤 결정이 품질과 비용을 좌우할까요?

이 글은 그 질문에 답하기 위해 BGE-M3 다국어의 핵심 결정과 운영 함정을 살펴봅니다.
...
![핵심 흐름](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/03/03-01-core-flow.ko.png)

*핵심 흐름*

## 왜 dense baseline부터 시작할까

![dense baseline을 기준선으로 잡는 측정 흐름](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/03/03-01-dense.ko.png)
...
![최소 실행 예제](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/03/03-02-diagram.ko.png)
...
![이 코드에서 봐야 할 것](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/03/03-03-diagram-2.ko.png)
...
![실무에서 헷갈리는 지점](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/03/03-04-diagram-3.ko.png)
...
- **dense·sparse·multi-vector 조합** — use case가 모드 선택을 결정합니다.
- **긴 문서 처리에 강하지만 비싸다** — 청킹과 결합 전략이 중요합니다.
- **언어 혼합 corpus에 우수** — 코드·문서·다국어 혼합에 적합합니다.
- **GPU 메모리가 운영 변수** — 배치·precision 설정이 비용을 결정합니다.
- **색인 호환성을 처음에 결정** — 벡터 DB 지원 여부가 운영을 좌우합니다.
...
1. 코퍼스를 영어 6건, 한국어 6건으로 늘리고, 한국어 쿼리 5개에 대한 Recall@1을 측정해 보세요. 정답 언어별로 Recall을 분리해 비교하세요.
2. `normalize_embeddings=False`로 바꿔 같은 쿼리를 검색하고, 길이가 긴 영어 문서가 점수를 어떻게 왜곡하는지 관찰하세요.
3. 같은 코퍼스를 KoSimCSE로도 인덱싱해 한국어/영어 정답에 대한 Recall@5를 BGE-M3와 비교하세요. 어떤 패턴이 보이나요?
