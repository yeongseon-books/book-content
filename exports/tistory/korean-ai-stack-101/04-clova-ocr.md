...
- CLOVA OCR API의 호출 방식과 응답 구조를 이해하고 Python으로 호출할 수 있습니다.
- OCR 결과의 후처리(bounding box 병합, 줄 정렬, 노이즈 제거)를 구현합니다.
- 한국어 문서에서 OCR 품질에 영향을 주는 요소(해상도, 글꼴, 레이아웃)를 파악합니다.
- OCR 결과를 RAG 파이프라인에 연결하는 흐름을 설계합니다.
...
![핵심 흐름](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/04/04-01-core-flow.ko.png)

*핵심 흐름*

## 왜 mock 응답부터 다루는가

![mock 응답으로 OCR 후처리를 검증하는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/04/04-01-mock.ko.png)
...
![최소 실행 예제](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/04/04-02-diagram.ko.png)
...
![이 코드에서 봐야 할 것](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/04/04-03-diagram-2.ko.png)
...
![실무에서 헷갈리는 지점](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/04/04-04-diagram-3.ko.png)
...
- **이미지 품질이 정확도의 천장** — 전처리에 시간을 쓰는 게 합리적입니다.
- **템플릿·일반 모드의 트레이드오프** — 구조화 문서에는 템플릿이 압도적입니다.
- **재시도·rate limit을 처음에 설계** — OCR API의 흔한 함정입니다.
- **결과 검증을 자동화** — 정규식·LLM 검증을 결합합니다.
- **PII 처리 정책을 명시** — OCR 결과에도 마스킹이 필요합니다.
