...
- KoSimCSE가 한국어 문장 유사도에 특화된 이유와 contrastive learning 기반 학습 방식을 이해합니다.
- 문장 유사도 검색을 FAISS 인덱스로 구현하는 실습을 합니다.
- 코사인 유사도 점수의 임계값을 운영 환경에 맞게 설정하는 방법을 이해합니다.
- 1화의 범용 모델과 비교해 KoSimCSE가 어떤 상황에서 우위인지 판단할 수 있습니다.
...
![핵심 흐름](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/02/02-01-core-flow.ko.png)

*핵심 흐름*

## 왜 FAQ 질문만 먼저 인덱싱할까

![FAQ 질문만 먼저 인덱싱하는 첫 버전 구조](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/02/02-01-faq.ko.png)
...
![최소 실행 예제](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/02/02-02-diagram.ko.png)
...
![이 코드에서 봐야 할 것](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/02/02-03-diagram-2.ko.png)
...
![실무에서 헷갈리는 지점](https://yeongseon-books.github.io/book-public-assets/assets/korean-ai-stack-101/02/02-04-diagram-3.ko.png)
...
- **토크나이저 일치가 첫 단추** — 잘못된 토크나이저는 임베딩을 망칩니다.
- **normalize가 cosine의 전제** — 정규화 누락이 가장 흔한 사고입니다.
- **배치 처리로 throughput 확보** — 단건 호출은 운영 비용을 폭증시킵니다.
- **도메인 미세조정이 큰 차이** — 범용 모델의 한계를 빠르게 만납니다.
- **벡터 저장 포맷을 처음에 결정** — 사후 변경은 재인덱싱 비용입니다.
...
1. FAQ 코퍼스를 10개로 늘리고, 일부러 의미가 비슷한 항목을 2개 추가하세요. 두 항목의 top-1 점수 차이를 관찰하세요.
2. `normalize_embeddings=False`로 바꿔 같은 쿼리를 검색하고, 결과 순위가 어떻게 달라지는지 비교하세요.
3. KoSimCSE 대신 `jhgan/ko-sroberta-multitask`를 써서 같은 쿼리의 점수 분포를 비교하세요. 어느 쪽이 더 명확한 간격을 보이나요?
