...
**핵심 질문**: 컴파일러가 `2 + 3 * 4`를 보면, 실행 시 매번 계산할까요? 아니면 미리 14로 바꿔 둘까요?

> Optimizer는 IR에서 IR로의 변환입니다. 의미를 보존하면서 더 빠르고 작은 코드로 바꿉니다. 그 출발점이 constant folding과 dead code elimination입니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- "의미 보존(meaning preserving)"이 optimization의 절대 규칙이라는 것
- constant folding: 컴파일 시점에 계산하기
- dead code elimination: 도달할 수 없는 코드 제거
- common subexpression elimination(CSE) 직관
- optimization을 어떻게 단계별로 적용하는가 (pass)
...
- **Pass**: IR을 한 번 훑으며 변환하는 단위.
- **Constant folding**: 상수끼리의 연산을 컴파일 시점에 계산.
- **Dead code elimination(DCE)**: 결과가 어디에도 쓰이지 않는 명령어 제거.
- **Common subexpression elimination(CSE)**: 같은 식 중복 계산 제거.
- **Strength reduction**: `x * 2`를 `x + x` 또는 `x << 1`로 (cheaper로 교체).
...
- 새 pass를 추가하기 전에 "의미 보존"을 가장 먼저 검증합니다.
- pass가 작고 단일 책임을 갖도록 유지합니다.
- 같은 pass를 fixed point까지 돌리는 게 흔한 패턴임을 압니다.
- profile 기반 의사결정을 신뢰합니다 (추측 금지).
- "이 변환이 어느 architecture에서 유리한가?"를 항상 묻습니다.
...
1. 위 fold에 strength reduction(`x * 2 → x + x`)를 추가해 보세요.
2. `fold + dce`를 더 줄어들지 않을 때까지 반복하는 fixed-point 루프를 짜 보세요.
3. side effect가 있는 명령어 (`PRINT`, `STORE`)를 만들고, dce가 그것을 살려 두는지 확인해 보세요.
