...
- ACA의 관측성이 어떤 계층으로 나뉘는지 이해합니다.
- `ContainerAppConsoleLogs_CL`과 `ContainerAppSystemLogs_CL`의 차이를 구분할 수 있습니다.
...
![로그와 추적이 모이는 관측 경로](https://yeongseon-books.github.io/book-public-assets/assets/azure-aca-101/07/07-01-the-observability-map.ko.png)
...
- **로그·메트릭·트레이스 셋을 모두 둔다** — 단일 신호로는 분산 환경의 사고를 진단할 수 없습니다.
- **애플리케이션 로그는 구조화한다** — JSON 로그가 Log Analytics 쿼리 효율을 결정합니다.
- **핵심 SLI를 대시보드에 고정한다** — 지연·에러율·포화도를 한눈에 봐야 회의 비용이 줄어듭니다.
- **알림은 가짜 알람부터 줄인다** — 신뢰를 잃은 알람은 무시되므로 임계값과 윈도를 신중히 정합니다.
- **배포 이벤트를 메트릭과 결합한다** — 회귀의 원인을 빠르게 좁히려면 배포 마커가 필수입니다.
...
1. 새 revision을 배포한 직후 5xx 비율이 급증했습니다. 이전 revision과 새 revision의 에러율을 비교하는 KQL 쿼리를 작성해 보세요.
2. App Insights connection string을 환경변수로 평문 노출하는 것과 secret으로 주입하는 것의 차이는 무엇입니까?
3. Dapr를 쓰지 않는 단순 FastAPI 앱에서 `--dapr-instrumentation-key`를 설정할 필요가 있을까요? 왜 그런가요?
