...
Environment·App·Revision의 3계층을 어떻게 구성해야 운영과 배포가 깔끔해질까요?

이 글은 그 질문에 답하기 위해 ACA의 3계층 구조의 핵심 결정과 운영 함정을 살펴봅니다.
...
![Environment와 Container App와 Revision의 계층 관계](https://yeongseon-books.github.io/book-public-assets/assets/azure-aca-101/02/02-01-start-with-the-hierarchy.ko.png)
...
- **Environment는 네트워크·로깅 경계** — 환경 단위로 VNet·Log Analytics가 묶이므로 prod/staging은 분리합니다.
- **App은 라이프사이클의 단위** — 배포·스케일·이미지가 App 단위로 관리되므로 도메인 경계로 자릅니다.
- **Revision은 불변이다** — 한 번 만든 Revision은 수정하지 않고 새 Revision으로 교체합니다.
- **멀티 Revision 모드는 명확한 이유로만** — 트래픽 분할이 필요할 때만 켜고, 기본은 단일 모드를 유지합니다.
- **리소스 그룹·네이밍을 미리 합의한다** — 환경·앱이 늘어난 뒤 정리 비용이 매우 큽니다.
