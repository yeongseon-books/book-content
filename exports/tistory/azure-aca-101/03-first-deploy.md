...
![로컬 코드가 ACA 배포로 이어지는 경로](https://yeongseon-books.github.io/book-public-assets/assets/azure-aca-101/03/03-01-the-end-to-end-path.ko.png)
...
- **이미지 레지스트리 권한을 먼저 확인한다** — managed identity·ACR 권한 누락이 첫 배포 실패의 가장 흔한 원인입니다.
- **환경 변수와 시크릿을 분리한다** — ACA의 secret 객체에 두고 환경 변수에서 참조하는 방식을 표준화합니다.
- **readiness/liveness probe를 빠뜨리지 않는다** — 헬스체크가 없으면 무한 재시작이나 트래픽 유입 사고가 생깁니다.
- **로그 싱크를 첫 배포에 함께 둔다** — Log Analytics 연결이 없으면 디버깅이 즉시 막힙니다.
- **CLI·Bicep·Terraform 중 하나로 통일한다** — 포털 클릭과 IaC가 섞이면 환경 드리프트가 시작됩니다.
