...
- Azure Container Apps의 스케일링이 어떤 신호를 보고 replica 수를 결정하는지 이해합니다.
- 내장 HTTP·TCP 규칙과 custom KEDA scaler의 차이를 구분할 수 있습니다.
...
KEDA로 이벤트 기반 스케일을 설계할 때 비용·지연·정확도를 어떻게 균형 잡을까요?

이 글은 그 질문에 답하기 위해 KEDA 기반 스케일링의 핵심 결정과 운영 함정을 살펴봅니다.
...
![스케일 신호가 replica 수를 바꾸는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/azure-aca-101/05/05-01-the-scaling-path.ko.png)
...
- **스케일 트리거는 SLA에서 역산한다** — 큐 길이·QPS·CPU 중 SLA에 직결되는 신호를 트리거로 둡니다.
- **최소 인스턴스 0은 신중히 선택한다** — 콜드 스타트가 사용자 체감을 망치는 경우 1 이상을 유지합니다.
- **최대 인스턴스 상한을 반드시 둔다** — 비용 폭주를 막는 가장 단순한 안전장치입니다.
- **폴링 주기와 cooldown을 맞춘다** — 지나치게 짧으면 진동, 지나치게 길면 지연이 생깁니다.
- **스케일 메트릭을 관측 대시보드에 둔다** — 스케일 사고는 사후 추적이 어려우므로 가시성을 먼저 확보합니다.
...
1. 평균 동시 요청이 30이고 peak가 200인 REST API에 대해 `min`, `max`, `http-concurrency` 값을 어떻게 정하겠습니까? 이유와 함께 설명하세요.
2. Service Bus에 메시지가 1000개 쌓였을 때 worker가 10개까지만 늘어났습니다. 가능한 원인 세 가지를 나열하세요.
3. CPU 기반 scaler에 `min-replicas 0`을 설정하면 어떤 일이 발생할까요? 왜 그럴까요?
