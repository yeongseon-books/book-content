...
**핵심 질문**: "계산할 수 있다"는 것은 정확히 무슨 뜻이고, 프로그램은 계산을 어떻게 표현할까요?

> 1936년 앨런 튜링은 "계산 가능한 것"의 정의를 수학적으로 제시했습니다. 테이프 위를 이동하며 기호를 읽고 쓰는 단순한 기계가 모든 계산을 수행할 수 있다는 놀라운 결론이었습니다. 이 이론적 기계가 오늘날 모든 컴퓨터의 기초입니다. 이 글에서는 계산의 정의, 계산 불가능한 문제, 프로그래밍 언어의 발전, 그리고 주요 패러다임을 다룹니다.
...
| 용어 | 설명 |
| --- | --- |
| 튜링 기계 | 계산 가능성을 정의하는 이론적 모델 |
| 정지 문제 | 프로그램이 멈추는지 판별할 수 없는 대표적 비가산 문제 |
| 컴파일러 | 소스 코드를 기계어로 번역하는 프로그램 |
| 인터프리터 | 소스 코드를 한 줄씩 실행하는 프로그램 |
| 패러다임 | 프로그래밍의 사고방식과 코드 조직 원칙 |
...
시니어 엔지니어는 패러다임에 교조적이지 않습니다. 명령형이 명확한 곳에서는 명령형을 쓰고, 함수형이 간결한 곳에서는 함수형을 씁니다. 중요한 것은 패러다임 자체가 아니라 코드가 의도를 명확히 전달하는가입니다.

계산 이론의 한계를 아는 것도 실무에서 중요합니다. "완벽한 정적 분석 도구"나 "모든 버그를 잡는 테스트"가 원리적으로 불가능하다는 것을 알면, 실용적인 대안을 설계하는 데 집중할 수 있습니다.
...
1. 간단한 상태 기계를 구현하세요. 입력 문자열에서 괄호의 균형이 맞는지 판별합니다.

2. 같은 기능을 명령형, 함수형 두 가지 스타일로 작성하세요. 리스트에서 짝수만 골라 제곱의 합을 구합니다.

3. `dis.dis()`를 사용하여 간단한 함수 3개의 바이트코드를 비교하고, 어떤 연산이 수행되는지 분석하세요.
