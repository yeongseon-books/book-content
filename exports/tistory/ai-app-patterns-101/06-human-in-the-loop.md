...
민감한 결정에 사람의 검토를 끼워 넣으면서도 처리량을 유지하려면 어떻게 설계해야 할까요?

이 글은 그 질문에 답하기 위해 휴먼 인 더 루프 패턴의 핵심 설계 결정과 실무 고려사항을 살펴봅니다.
...
![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/06/06-01-questions-this-post-answers.ko.png)
...
![위험도에 따른 사람 개입 범위](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/06/06-01-human-review-by-risk-level.ko.png)
...
![초안 생성과 승인 게이트 흐름](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/06/06-02-draft-generation-with-approval-gate.ko.png)
...
![신뢰도 임계값 라우팅 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/06/06-03-confidence-threshold-routing.ko.png)
...
![검토 결정과 감사 로그 기록 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/06/06-04-review-decisions-with-audit-events.ko.png)
...
![사람 피드백이 정책으로 돌아가는 루프](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/06/06-05-human-feedback-back-into-policy-loop.ko.png)
...
- **검토 지점을 의도적으로 설계한다** — 어디에 사람의 승인이 필요한지를 명시해야 자동화와 통제 사이의 경계가 분명해집니다.
- **검토자에게 충분한 컨텍스트를 준다** — 원본 입력·모델 출력·신뢰도·근거를 함께 보여줘야 빠르고 정확한 판단이 가능합니다.
- **승인 큐는 별도 시스템으로 분리한다** — 메인 처리 흐름과 검토 큐를 분리해야 사람의 응답 지연이 전체 처리량을 막지 않습니다.
- **검토 결과를 학습 신호로 모은다** — 사람의 수정·반려를 데이터로 축적해야 모델 개선과 자동화 임계값 조정이 가능합니다.
- **긴급 우회 경로를 준비한다** — 검토자 부재·SLA 초과 상황을 위한 fallback과 알림 정책이 운영 안정성을 결정합니다.
