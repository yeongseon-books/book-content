...
LLM이 모르는 내부 문서에 대한 질문에 정확히 답하게 하려면, 무엇을 어떻게 찾아서 넘겨야 할까요?

이 글은 그 질문에 답하기 위해 임베딩, 벡터 검색, 그리고 retrieve→rerank→generate 파이프라인을 단계적으로 살펴봅니다.
...
![오프라인 인덱싱 단계 흐름](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/02/02-01-offline-indexing-pipeline.ko.png)
...
![온라인 질의응답 실행 흐름](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/02/02-02-online-question-answering-flow.ko.png)
...
![검색 결과와 출처 반환 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/02/02-03-answer-and-source-return-structure.ko.png)
...
![검색 실패를 줄이는 방어 계층](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/02/02-04-defense-layers-against-retrieval-misses.ko.png)

*검색 실패를 줄이는 방어 계층*
### 문서 없음 상황의 fallback 분기

![문서 없음 상황의 fallback 분기](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/02/02-05-fallback-branch-for-missing-evidence.ko.png)
...
- **검색 품질이 답변 품질의 상한이다.** LLM은 받은 문서로만 답하므로 retrieval recall이 낮으면 모든 prompt engineering이 무용지물입니다.
- **청크 크기와 오버랩은 도메인 의존적이다.** 한 번의 정답 사이즈는 없습니다. 평가 세트를 만들어 측정하면서 조정합니다.
- **임베딩 모델은 교체 가능하게 설계한다.** 모델 갱신 시 전체 재인덱싱이 필요하므로, 인터페이스를 분리해 두면 마이그레이션 비용이 줄어듭니다.
- **출처 표시는 신뢰의 기본이다.** 답변과 함께 인용 청크를 반환하면 사용자가 직접 검증할 수 있고, 환각도 줄어듭니다.
- **첫 버전은 BM25부터.** 의미 검색에 바로 가지 않고 키워드 검색을 베이스라인으로 두면 평가 기준점이 명확해집니다.
