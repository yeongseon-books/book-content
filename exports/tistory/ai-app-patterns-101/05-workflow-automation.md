...
사람의 개입 없이 다단계 작업을 자동으로 수행하려면 워크플로우를 어떻게 분해하고 조정해야 할까요?

이 글은 그 질문에 답하기 위해 워크플로우 자동화 패턴의 핵심 설계 결정과 실무 고려사항을 살펴봅니다.
...
![순차 체인 데이터 전달 흐름](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/05/05-01-sequential-handoff-across-stages.ko.png)

*순차 체인 데이터 전달 흐름*
### DAG 스타일 분기와 병렬 실행 구조

![DAG 스타일 분기와 병렬 실행 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/05/05-02-dag-style-branching-with-parallel-work.ko.png)
...
![분류 기반 라우팅 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/05/05-03-classification-driven-routing.ko.png)

*분류 기반 라우팅 구조*
### 승인 게이트와 재시도 복구 경로

![승인 게이트와 재시도 복구 경로](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/05/05-04-approval-gate-and-retry-recovery.ko.png)
...
![코드 리뷰 워크플로 산출물 계약 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/05/05-05-code-review-artifact-contract.ko.png)
...
- **단계는 명시적으로 분해한다** — 암묵적 체인보다 단계가 명시적으로 정의된 워크플로우가 디버깅·재실행에 강합니다.
- **각 단계는 멱등하게 설계한다** — 재시도해도 결과가 같아야 부분 실패를 안전하게 복구할 수 있습니다.
- **상태는 외부에 저장한다** — 워크플로우 상태를 메모리가 아닌 DB·큐에 두어야 장애 복구와 모니터링이 가능합니다.
- **분기 조건을 데이터로 표현한다** — if/else를 코드에 묻어두기보다 설정·정책으로 분리해야 운영 중 변경할 수 있습니다.
- **관측 단위는 워크플로우 전체와 단계 모두** — 전체 SLA와 단계별 지연·실패율을 함께 추적해야 병목을 정확히 찾습니다.
