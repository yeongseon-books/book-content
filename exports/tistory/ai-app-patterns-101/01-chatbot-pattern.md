...
LLM API가 무상태(stateless)라면, "대화를 기억하는 챗봇"은 도대체 누가 기억하고 있는 걸까요?

이 글은 그 질문에 답하기 위해 단순 누적·윈도·요약·세션 네 가지 이력 관리 전략을 단계적으로 살펴봅니다.
...
![이 글에서 답할 질문](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/01/01-01-questions-this-post-answers.ko.png)
...
![무상태 호출과 이력 재전송 흐름](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/01/01-01-stateless-call-with-replayed-history.ko.png)
...
![최근 N개 메시지 유지 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/01/01-02-sliding-window-message-retention.ko.png)
...
![요약 메모리와 최근 대화 결합 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/01/01-03-summary-memory-with-recent-turns.ko.png)
...
![세션별 대화 상태 분리 구조](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/01/01-04-session-scoped-conversation-state.ko.png)
...
![긴 대화에서 윈도우와 요약으로 넘어가는 분기](https://yeongseon-books.github.io/book-public-assets/assets/ai-app-patterns-101/01/01-05-branching-from-full-history-to-compressi.ko.png)
...
- **"기억"은 앱의 책임이다.** 모델에 귀속시키면 세션 경계·프라이버시·감사 추적이 모두 느슨해집니다.
- **이력은 원본과 압축본을 따로 둔다.** 원본은 감사·디버깅용, 압축본(윈도 또는 요약)은 LLM에 보내는 용도로 구분합니다.
- **세션 저장소는 처음부터 교체 가능하게.** 메모리 dict로 시작해도 Redis/DB로 옮기는 출구를 먼저 뚫어 둡니다.
- **요약은 품질 vs 비용의 트레이드오프이다.** 요약이 아니라 단순 윈도만으로 충분한 도메인이 대부분입니다. 먼저 윈도로 출발합니다.
- **system prompt는 매 호출마다 다시 붙인다.** 이력이 잘려도 페르소나·스타일은 유지되어야 하며, system은 이력과 별도로 관리합니다.
