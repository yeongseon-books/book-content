...
**핵심 질문**: 친구 관계, 도로망, 의존성, 라우팅, 추천 시스템이 공통적으로 사용하는 자료구조는 무엇일까요?

> 그래프는 정점(vertex)들 사이의 임의의 관계를 간선(edge)으로 표현한 자료구조입니다. 트리는 그래프의 특수한 경우(사이클 없는 연결 그래프)이며, 그래프는 거의 모든 관계 모델링의 기본 어휘입니다. 이 글에서는 그래프의 표현 방법, 방향성·가중치·연결성 같은 기본 속성, 그리고 BFS/DFS 두 가지 핵심 탐색 알고리즘을 직접 구현하면서 익힙니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 그래프의 기본 용어(정점, 간선, 차수, 경로)
- 인접 리스트와 인접 행렬의 트레이드오프
- 방향/무방향, 가중/비가중, 연결/비연결 그래프의 차이
- BFS와 DFS의 구현 차이와 적합한 문제 유형
...
시니어 엔지니어는 문제를 보면 가장 먼저 "이게 그래프 문제인가?"를 묻습니다. 정점·간선·관계라는 어휘로 재구성할 수 있다면 BFS·DFS·다익스트라·위상 정렬 같은 기성 알고리즘을 즉시 적용할 수 있기 때문입니다.

또한 시니어는 그래프 라이브러리(NetworkX, igraph)와 그래프 데이터베이스(Neo4j)의 존재를 알고 적절히 사용합니다. 모든 그래프를 직접 구현하지 않습니다. 다만 라이브러리가 내부에서 무엇을 하는지 이해하기 위해 기본 알고리즘은 손에 익혀 둡니다.
...
1. 위 `Graph`에 `bfs_path(start, target)` 메서드를 추가해 보세요. 단순 거리뿐 아니라 실제 경로(정점 리스트)를 반환합니다. 힌트: 이전 정점을 dict로 기록.

2. 유향 그래프에서 위상 정렬을 구현해 보세요. (a) DFS 후행 시간 역순, (b) 진입 차수 0인 정점부터 BFS(Kahn 알고리즘) 두 가지 방법을 비교합니다.

3. 가중 그래프에서 다익스트라 알고리즘을 직접 구현해 보세요. 우선순위 큐(`heapq`)를 사용합니다. 음수 가중치가 있으면 왜 동작하지 않을까요?
