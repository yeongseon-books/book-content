...
**핵심 질문**: 함수 호출은 어떤 자료구조 위에서 동작할까요? 메시지 큐, 작업 큐, 프린터 큐는 왜 모두 "큐"라는 이름을 쓸까요?

> 스택은 마지막에 들어온 것이 먼저 나가는(LIFO) 자료구조이고, 큐는 먼저 들어온 것이 먼저 나가는(FIFO) 자료구조입니다. 두 ADT는 단순하지만 함수 호출, 트리 탐색, 작업 스케줄링, 메시지 전달 등 모든 시스템 소프트웨어의 핵심에 자리잡고 있습니다. 이 글에서는 두 자료구조의 동작 원리를 직접 구현하면서 익히고, 실무에서 어떻게 쓰이는지 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 스택과 큐의 ADT 정의와 핵심 연산
- 배열과 연결 리스트로 두 자료구조 구현하기
- 함수 호출 스택과 BFS 탐색 큐의 동작 원리
- 파이썬에서 어떤 자료구조를 선택해야 하는지
...
| 용어 | 설명 |
| --- | --- |
| 스택(stack) | LIFO(Last In First Out) ADT |
| 큐(queue) | FIFO(First In First Out) ADT |
| push / pop | 스택에 넣기 / 꺼내기 |
| enqueue / dequeue | 큐에 넣기 / 꺼내기 |
| 데크(deque) | 양 끝에서 모두 넣고 빼는 일반화된 구조 |
...
시니어 엔지니어는 재귀 함수를 보면 머릿속으로 스택을 그립니다. 깊은 재귀가 위험할 수 있다는 사실을 알기 때문에 스택 깊이의 상한을 항상 의식합니다. 필요하면 재귀를 명시적 스택 기반 반복으로 변환하는 능력을 갖춥니다.

또한 시니어는 "작업 큐"의 이름을 신중하게 정합니다. 단순 FIFO 큐인지, 우선순위 큐인지, 지연 큐인지, 백프레셔(backpressure)가 있는 큐인지에 따라 시스템 동작이 완전히 달라지기 때문입니다. ADT의 이름이 같다고 동작이 같다고 가정하지 않습니다.
...
1. 스택 두 개로 큐를 구현해 보세요. `enqueue`는 첫 번째 스택에 push하고, `dequeue`는 두 번째 스택에서 pop합니다. 두 번째 스택이 비면 첫 번째 스택을 모두 옮깁니다. 이 큐의 분할 상환 비용은 얼마일까요?

2. 후위 표기법(Postfix) 계산기를 스택으로 구현해 보세요. 예: `"3 4 + 2 *"` → 14.

3. 위 BFS 코드의 deque를 stack(`list`)으로 바꾸면 DFS가 되는지 확인해 보세요. 방문 순서는 어떻게 달라지나요?
