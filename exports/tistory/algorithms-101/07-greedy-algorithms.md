...
**핵심 질문**: "당장 가장 좋은 선택"을 반복하면 정말 전체 최적해가 될까요? 어떤 문제에서는 통하고 어떤 문제에서는 실패하는 이유는 무엇일까요?

> 그리디 알고리즘은 매 단계에서 국소적으로 최선인 선택을 하고, 그 선택을 되돌리지 않는 기법입니다. 정확히 동작하려면 (1) 그리디 선택 속성과 (2) 최적 부분 구조가 모두 성립해야 합니다. 이 둘을 보이는 가장 일반적인 도구가 교환 논증(exchange argument)입니다. 그리디는 짧고 빠르지만 적용 조건을 잘못 보면 미묘하게 잘못된 답을 내므로, 직관과 증명을 함께 다뤄야 합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 그리디 알고리즘이 정확히 동작하는 두 조건
- 교환 논증으로 그리디 정당성을 보이는 방법
- 활동 선택, 동전 거스름돈, Huffman 코드 같은 대표 문제
- 그리디처럼 보이지만 DP가 필요한 함정
...
| 용어 | 설명 |
| --- | --- |
| 그리디 선택 | 매 단계 국소 최적 선택 |
| 그리디 선택 속성 | 그리디 선택을 포함한 최적해 존재 |
| 최적 부분 구조 | 부분 문제도 같은 그리디로 최적 |
| 교환 논증 | 최적해의 한 선택을 그리디 선택으로 바꿔 동치 증명 |
| Huffman 코드 | 빈도 기반 무손실 압축의 대표 그리디 |
...
시니어는 그리디를 쓸 때 "왜 옳은가"를 먼저 한 줄로 적습니다. 교환 논증의 한 문장이 자연스럽게 떠오르지 않으면 DP를 의심합니다. 또한 작은 반례를 적극적으로 만들어 봅니다. 가장 빠른 정답성 검증은 손으로 그려본 케이스입니다.

또한 시니어는 그리디 알고리즘이 통하지 않는 입력 조건을 명시적으로 문서화합니다. "이 알고리즘은 동전 시스템이 정규일 때만 최적"처럼 적어두면 미래의 자신과 동료가 잘못 쓰는 일을 막을 수 있습니다.
...
1. 회의실 한 개에서 가능한 한 많은 회의를 배정하는 함수를 작성하고, 끝나는 시각이 아닌 시작 시각으로 정렬했을 때 왜 최적이 깨지는지 반례를 보이세요.

2. 양수 가중치만 가진 작업들의 평균 완료 시간을 최소화하는 스케줄을 그리디로 구하고, 교환 논증으로 정당성을 증명해 보세요.

3. 동전 [1, 5, 6, 9]로 11을 만들 때 그리디 결과와 DP 결과를 모두 출력하고, 두 결과가 다른 이유를 설명하세요.
