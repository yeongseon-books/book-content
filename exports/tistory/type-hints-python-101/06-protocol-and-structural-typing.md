...
**핵심 질문**: 상속 없이 "이 메서드만 있으면 된다"는 조건을 타입으로 표현할 수 있을까요?

> Python의 "덕 타이핑"을 정적 타입 시스템으로 가져온 것이 `Protocol`입니다. 상속 관계 없이 필요한 메서드나 속성이 있으면 호환되는 structural typing을 지원합니다. 이 글에서는 Protocol의 원리와 ABC와의 차이, 실전 활용법을 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- Protocol의 정의와 structural typing의 원리
- ABC(Abstract Base Class)와 Protocol의 차이
- `@runtime_checkable`로 런타임 검증
- 실전 Protocol 설계 패턴
...
- **덕 타이핑 형식화** — Protocol이 사실상 표준 인터페이스 도구입니다.
- **runtime_checkable** — 런타임 체크가 필요할 때만 활성화합니다.
- **좁은 인터페이스** — 프로토콜은 작은 책임 하나로 정합니다.
- **ABC 비교** — 강한 계약이면 ABC, 느슨하면 Protocol이 적합합니다.
- **테스트** — Protocol 만족을 단위 테스트로 검증합니다.
...
1. `read()`와 `write()` 메서드를 가진 `ReadWrite` Protocol을 정의하고, 이를 충족하는 클래스를 2개 만드세요.
2. `@runtime_checkable` Protocol을 정의하고, isinstance()로 여러 객체를 필터링하세요.
3. ABC로 구현된 인터페이스를 Protocol로 리팩터링하고 차이를 비교하세요.
