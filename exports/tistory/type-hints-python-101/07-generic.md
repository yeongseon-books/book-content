...
**핵심 질문**: 하나의 함수가 여러 타입을 받으면서도 입력과 출력의 타입 관계를 유지할 수 있을까요?

> 리스트에서 첫 번째 요소를 꺼내는 함수를 생각해 봅니다. `list[int]`를 넣으면 `int`가, `list[str]`를 넣으면 `str`가 나와야 합니다. `Any`를 쓰면 타입 정보가 사라지고, 타입별로 함수를 만들면 코드가 중복됩니다. Generic은 "타입을 매개변수로" 받아 이 문제를 해결합니다.
...
- TypeVar의 정의와 사용법
- Generic 클래스 작성 방법
- bound와 constraint로 타입 범위 제한
- Python 3.12의 새로운 타입 매개변수 문법
...
| 용어 | 설명 |
| --- | --- |
| TypeVar | 타입 변수를 선언하는 도구 |
| Generic | 타입 변수를 받는 클래스의 기반 클래스 |
| bound | TypeVar의 상한 타입 제한 |
| constraint | TypeVar가 취할 수 있는 타입 목록 |
| ParamSpec | 함수 시그니처 전체를 캡처하는 타입 변수 |
...
시니어 엔지니어는 Generic을 "필요한 곳에만" 도입합니다. 모든 함수에 TypeVar를 붙이면 코드가 복잡해질 뿐 실익이 없습니다. 입력 타입과 출력 타입 사이에 명확한 관계가 있고, 그 관계를 타입 시스템으로 표현해야 할 때만 Generic을 사용합니다.

실무에서는 라이브러리의 공개 API, 공통 유틸리티, 데이터 래퍼 패턴에서 Generic이 빛을 발합니다. 내부 구현 코드에서는 구체 타입으로 충분한 경우가 대부분입니다. 과도한 추상화보다 명확한 타입이 팀의 생산성을 높입니다.
...
1. `TypeVar`와 `Generic`을 사용하여 키-값 쌍을 저장하는 `Pair[K, V]` 클래스를 작성하세요. `first`와 `second` 속성이 각각 `K`와 `V` 타입을 반환해야 합니다.

2. `bound=Sized`로 제한된 TypeVar를 사용하여 `longest(a: T, b: T) -> T` 함수를 작성하세요. 두 객체 중 `len()`이 더 큰 것을 반환합니다.

3. Python 3.12의 `[T]` 문법으로 Generic 큐(Queue) 클래스를 작성하세요. `enqueue`, `dequeue`, `peek` 메서드를 포함합니다.
