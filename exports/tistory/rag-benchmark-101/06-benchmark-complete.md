...
RAG 벤치마크를 운영 프로세스로 어떻게 정착시키나요?

이 글은 그 질문에 답하기 위해 RAG 벤치마크 운영화의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 시리즈 전체에서 다룬 지표, 도구, 판단 기준을 하나의 벤치마크 파이프라인으로 통합하는 방법을 이해합니다.
- 벤치마크 결과를 기록하고 반복 비교할 수 있는 보고서 형식을 설계합니다.
- 벤치마크를 운영 프로세스에 내재화하는 실습을 합니다.
- 시리즈를 마무리하며 다음 단계로 나아갈 방향을 정리합니다.
...
![검색과 생성과 평가가 한 실행으로 이어지는 파이프라인](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/06/06-01-end-to-end-benchmark-pipeline-in-one-run.ko.png)
...
![Retrieval 리포트와 generation 리포트를 나누는 구조](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/06/06-02-retrieval-and-generation-report-split.ko.png)
...
![낮은 점수를 검색 문제와 생성 문제로 가르는 분기](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/06/06-03-branching-search-failures-from-generatio.ko.png)
...
![기준선 비교부터 최종 의사결정까지 이어지는 벤치마크 루프](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/06/06-04-baseline-to-decision-benchmark-loop.ko.png)
...
벤치마크 파이프라인을 만들었지만 아무도 돌리지 않는 팀이 많습니다. 벤치마크가 살아남으려면 두 가지가 필요합니다. 첫째, 실행이 자동화되어야 합니다. 수동으로 노트북을 열어야 돌릴 수 있는 벤치마크는 오래 못 살아남습니다. 둘째, 결과가 공유되어야 합니다. Slack 채널이든 대시보드든, 팀이 자연스럽게 보는 곳에 결과가 농여야 합니다.

벤치마크를 주기적으로 돌리다 보면, 점수 자체보다 추세가 더 중요해집니다. Recall@5가 0.72에서 0.68로 떨어졌다면 절대값은 나쁘지 않아도 원인을 조사해야 합니다. 새 문서가 추가된 것인지, 임베딩 모델이 바뀐 것인지, 쿼리 패턴이 달라진 것인지를 추적할 수 있도록 메타데이터를 함께 기록해두는 것이 좋습니다.

## 시니어 엔지니어는 이렇게 생각합니다

- **주기와 책임자** — 주간/릴리스 단위로 누가 돌리고 검토하는지 명문화합니다.
- **실험 추적** — 프롬프트·모델·인덱스 버전을 모든 결과와 묶어 저장합니다.
- **보드 표준화** — 지표 정의를 팀 단일화해 비교 가능성을 확보합니다.
- **회귀 알림** — 임계 하락 시 슬랙/PR 코멘트로 자동 알림을 보냅니다.
- **문서화** — 결정과 근거를 ADR로 남겨 모델 교체 시 재학습 비용을 줄입니다.
