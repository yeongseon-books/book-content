...
VectorDB는 어떤 기준으로 선택해야 하나요?

이 글은 그 질문에 답하기 위해 VectorDB 선택의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- VectorDB를 고를 때 성능, 비용, 운영 복잡도를 어떻게 저울질하는지 판단할 수 있습니다.
- FAISS, Chroma, Qdrant, Weaviate 등 주요 옵션의 아키텍처 차이를 이해합니다.
- 내 워크로드 특성에 맞는 VectorDB를 선택하는 의사결정 프레임워크를 적용할 수 있습니다.
- 같은 쿼리로 여러 VectorDB의 retrieval 지표를 비교하는 실습을 합니다.
...
- VectorDB 후보(FAISS, Chroma, pgvector, Qdrant)를 같은 기준 위에서 비교하려면 무엇을 고정해야 하나요?

> VectorDB 선택은 **브랜드 비교가 아닙니다**. 같은 임베딩 벡터를 어떤 인덱스 구조에 넣었을 때 정확도와 속도가 어떻게 바뀌는지 보는 실험입니다.
...
![같은 벡터를 Flat과 IVF에 넣는 인덱스 비교 구조](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/04/04-01-same-vector-flat-and-ivf-comparison-stru.ko.png)
...
![임베딩 계산과 검색 시간을 분리하는 측정 경계](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/04/04-02-boundary-between-embedding-and-search-ti.ko.png)
...
![운영 조건으로 인덱스를 고르는 판단 축](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/04/04-04-index-decision-axes-for-real-workloads.ko.png)
...
- **운영 모델** — managed vs self-hosted 비용·운영 부담을 먼저 정합니다.
- **필터링 성능** — 메타 필터가 잦다면 hybrid·payload 인덱스를 지원하는지 확인합니다.
- **스케일 한계** — 벡터 수·QPS 목표를 사전에 정하고 벤치마킹합니다.
- **일관성/백업** — 스냅샷·리플리카 정책이 운영 SLO와 맞는지 확인합니다.
- **Lock-in 리스크** — 임베딩·인덱스 마이그레이션 비용을 미리 산정합니다.
...
1. corpus 크기를 100, 1,000, 10,000으로 늘려 가며 flat vs IVF의 search latency 비를 그려 보세요. 어디서 격차가 벌어지나요?
2. `IndexHNSWFlat`을 추가해 IVF, flat, HNSW 세 인덱스를 같은 표 위에 올려 보세요.
3. `nprobe=1, 4, 16`에서 recall을 측정하고, recall = 0.95를 만족하는 최소 nprobe를 찾아 보세요.
