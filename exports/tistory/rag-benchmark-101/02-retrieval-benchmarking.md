...
검색 성능을 어떻게 재현 가능하게 벤치마킹하나요?

이 글은 그 질문에 답하기 위해 retrieval 벤치마킹의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- retrieval 벤치마크를 설계할 때 골드셋(ground truth) 만드는 방법과 기준을 이해합니다.
- 쿼리 세트의 규모와 다양성이 벤치마크 신뢰도에 어떻게 영향하는지 판단할 수 있습니다.
- 반복 가능한 벤치마크 파이프라인을 Python으로 구성하는 방법을 실습합니다.
- 벤치마크 결과를 어떻게 기록하고 비교해야 의사결정에 쓸 수 있는지 이해합니다.
...
- FAISS retriever 위에 hit rate와 MRR을 어떻게 붙일까요?
- 검색 지연 시간(latency)은 어디서 재고, 어떤 단위로 기록해야 할까요?
- 작은 골드셋만으로도 의미 있는 검색 벤치마크를 시작할 수 있을까요?
- 같은 측정 루프를 다른 retriever나 임베딩 모델에도 재사용할 수 있도록 설계하려면 무엇을 고정해야 할까요?

> 검색 벤치마크의 핵심은 vector DB나 인덱스 자체가 아니라, **질문·정답 문서·측정 루프를 고정해서 같은 retriever를 반복 관찰**하는 데 있습니다.
...
![질의와 지연 시간이 묶인 검색 벤치마크 루프](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/02/02-01-benchmark-loop-for-queries-and-latency.ko.png)
...
![Hit rate와 MRR을 함께 읽는 검색 품질 축](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/02/02-02-retrieval-quality-axes-with-hit-rate-and.ko.png)
...
![Hit rate는 높지만 순위 품질은 낮은 실패 패턴](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/02/02-03-high-hit-rate-with-weak-ranking.ko.png)
...
- **쿼리 셋 설계** — 도메인·난이도·유형을 균형 있게 섞어 편향을 줄입니다.
- **정답 라벨** — 관련도 라벨을 다단계로 두면 nDCG 신호가 살아납니다.
- **BM25 베이스라인** — 임베딩 결과는 항상 BM25와 비교해 우열을 확인합니다.
- **Latency 동시 측정** — 정확도와 latency를 한 표로 봐야 의사결정이 가능합니다.
- **Top-k 민감도** — k 변화에 따른 곡선을 그려 운영 k를 선택합니다.

## 체크리스트

![질문과 정답 문서를 함께 남기는 벤치마크 기록](https://yeongseon-books.github.io/book-public-assets/assets/rag-benchmark-101/02/02-04-benchmark-record-with-gold-ids-and-logs.ko.png)
...
1. 위 코드를 수정해 `k=1`, `k=3`, `k=5`에서의 hit rate를 한 번에 출력해 보세요. k가 커질수록 hit rate는 어떻게 바뀌나요? MRR은 어떻게 되나요?
2. `time.perf_counter()` 대신 `time.time()`을 쓰면 어떤 상황에서 측정이 왜곡될 수 있을까요? 두 함수의 문서를 찾아 보세요.
3. warm-up 호출을 1회 추가하고, warm-up이 있을 때와 없을 때의 첫 latency 차이를 비교해 보세요.
