...
- 이미지는 기본적으로 public GitHub Pages URL로 재작성된다. `--local-assets`로 상대 경로 유지 가능.
- TOC `<!-- toc:* -->` 마커는 제거되고 TOC 본문은 유지된다.
- 저작권 HTML 주석이 자동 삽입된다 (`series.yaml` meta 기반). 읽는 사람에게는 보이지 않음.
...
| 저작권 주석 | HTML 주석 자동 삽입 | HTML 주석 자동 삽입 | 미삽입 (Medium 자체 약관) | N/A | `mkdocs-ebook` builder |
