...
![챗봇과 AI 에이전트의 차이](../assets/ai-web-dev-101/05/assistant-vs-agent.png)
...
![Function Calling의 동작 흐름](../assets/ai-web-dev-101/05/function-calling-cycle.png)
...
def calculate(expression):
    # 간단한 수식 계산
    try:
        # 주의: 실제 서비스에서는 보안을 위해 eval 대신 전용 라이브러리(예: simpleeval) 사용을 권장합니다.
...
AI는 이 과정을 통해 먼저 환율을 조회하고, 그 결과를 바탕으로 수식을 만들어 계산기를 두드린 뒤 우리에게 최종 금액을 알려줍니다. 여러 도구를 순차적으로 혹은 한꺼번에 호출하며 문제를 해결해 나가는 모습이 정말 대견하지 않나요?

![여러 도구를 순차적으로 사용하는 에이전트](../assets/ai-web-dev-101/05/multi-tool-example-flow.png)
...
![에이전트 루프의 구조](../assets/ai-web-dev-101/05/agent-loop-overview.png)

---

## 에이전트를 만들 때 주의할 점

똑똑한 에이전트를 만들려면 다음 세 가지를 기억하세요.
...
3. **비용 관리**: 에이전트는 한 번의 답변을 위해 여러 번 API를 호출합니다. 루프가 길어질수록 비용과 지연 시간(Latency)이 늘어날 수 있다는 점을 고려해야 합니다.

![도구 사용 권한과 안전 경계](../assets/ai-web-dev-101/05/tool-permission-boundary.png)
...
2. [프롬프트 엔지니어링 101 — 원하는 답변을 얻는 기술](./02-prompt-engineering.md)
3. [이미지 생성 AI 활용하기 — DALL-E와 웹 서비스의 만남 (예정)]
4. [RAG 기초 — 내 데이터를 기억하는 똑똑한 챗봇 만들기 (예정)]
5. **AI 에이전트 첫걸음 — Tool Use로 똑똑한 AI 만들기**
6. 실전! AI 스케줄러 만들기 (예정)
7. 배포와 운영 — 내 AI 서비스를 세상에 내놓기 (예정)

### 참고 자료
- [OpenAI Function Calling Guide](https://platform.openai.com/docs/guides/function-calling)
- [JSON Schema Documentation](https://json-schema.org/)

**Tags:** `AI` `AI Agent` `Function Calling` `Tool Use` `LLM` `Python` `Tutorial`
