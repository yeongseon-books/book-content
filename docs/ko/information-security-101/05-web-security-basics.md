...
**핵심 질문**: 브라우저는 다른 출처로부터 우리를 어떻게 지키나요?

> 브라우저 보안의 핵심은 출처(origin)입니다. 출처를 이해하면 CORS, CSP, 쿠키가 한 줄로 이어집니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- Same-origin Policy의 정확한 정의
- CORS — 무엇을 어떻게 허용하는가
...
- **CORS**: 다른 출처 요청을 서버가 허용하는 메커니즘.
- **CSP**: 어떤 자원을 어디서 로드할지 제한하는 헤더.
- **CSRF**: 사용자의 인증된 세션을 악용한 위조 요청.
- **SameSite cookie**: cross-site 요청에 쿠키 첨부를 제한.
...
- 보안 헤더를 한 곳(미들웨어/엣지)에서 일관되게 관리합니다.
- CSP는 Report-Only로 운영 데이터 모은 뒤 Enforce.
- 쿠키 정책 변경은 점진적 롤아웃과 모니터링.
- CSRF/XSS 방어는 single source of truth로 통합.
- 출처 정책을 문서화하고 변경 로그를 남깁니다.
...
1. `https://app.example.com`과 `https://app.example.com:8443`은 같은 출처인가?
2. CSP `default-src 'self'`만으로 막히지 않는 공격을 한 가지 적어 보세요.
3. SameSite=Strict가 사용성에 미치는 영향을 두 가지 적어 보세요.
