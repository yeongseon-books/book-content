...
**핵심 질문**: 좋은 패턴이 어떻게 *나쁜 코드*가 될까요?

> 문제가 없는데 패턴부터 꺼낼 때입니다. 패턴은 *답*이 아니라 *어휘*입니다.
...
- **YAGNI**: You Aren't Gonna Need It — 지금 필요하지 않다.
- **Premature abstraction**: 너무 이른 추상화.
- **Pattern fever**: 모든 코드를 패턴으로 환원하려는 충동.
- **Cargo cult**: 형식만 흉내 내는 모방.
- **Refactor to pattern**: 단순 코드를 다듬다 보니 패턴이 드러나는 흐름.
...
- 처음에는 *함수*로 시작.
- 변화가 *반복*될 때 추상화.
- 패턴 이름은 *결과* — 시작 시 정하지 않는다.
- 쓰지 않는 추상화는 *지운다*.
- 코드 리뷰에서 "왜 이 패턴인가?"를 묻는다.
...
1. 자기 코드에서 *과한 추상화*를 한 곳 찾아 단순 함수로 접기.
2. 변화 횟수를 세어 "추상화 후보"를 표로 정리.
3. PR 리뷰 체크리스트에 "왜 이 패턴인가?" 질문 추가.
