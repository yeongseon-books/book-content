...
**핵심 질문**: REST API에서 *URL을 잘 짠다* 는 것은 정확히 무엇을 잘 짜는 것일까요?

> *자원의 경계* 와 *관계* 를 잘 표현하는 일입니다. URL은 그 표현의 결과일 뿐입니다.
...
- **Collection**: 같은 종류 자원의 집합 — `/users`.
- **Item**: 컬렉션의 한 원소 — `/users/42`.
- **Sub-resource**: 어느 자원에 *속한* 자원 — `/users/42/orders`.
- **Identifier**: 자원을 가리키는 키 (보통 id 또는 slug).
- **Canonical URL**: 같은 자원을 가리키는 *공식* 경로.
...
- 자원의 *수명* 을 본다 — 한 번 만든 URL은 오래 산다.
- DB 모델과 자원 모델을 *분리* 한다.
- 깊이는 2단계가 기본, 3단계는 예외.
- *액션* 은 자원의 상태 변화로 표현 — 어쩔 수 없으면 `:verb` 같은 명시적 형식.
- 외부에 노출할 id는 *불투명* (UUID, slug)하게.
...
1. 사내 시스템 하나의 자원 모델을 그려 보세요 — 컬렉션 7개, 관계 5개.
2. 위 4단계 예제에 `/users/<uid>/orders` 를 추가하세요.
3. RPC 스타일 endpoint 5개를 REST 스타일로 다시 쓰세요.
