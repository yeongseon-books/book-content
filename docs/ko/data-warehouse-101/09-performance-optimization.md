...
**핵심 질문**: *왜 어떤 쿼리는 1초* 에 끝나고 *어떤 쿼리는 5분* 이 걸릴까요? *비용* 은 어떻게 *줄일 수 있을까요*?

> *Warehouse 의 성능은 *읽은 바이트* 와 *섞인 데이터* 로 결정된다.*
...
- **Bytes Scanned**: 쿼리가 *실제로 읽은 데이터 양*. 비용의 *주축*.
- **Shuffle**: 노드 간 *데이터 이동*. *조인/집계* 시 발생.
- **Spill**: 메모리에 못 담아 *디스크* 로 *흘러내림*.
- **Materialized View**: 자주 쓰는 결과를 *미리 계산* 해 둔 뷰.
- **Approximate Aggregate**: `APPROX_COUNT_DISTINCT` 같은 *근사 집계*.
...
- *읽은 바이트* 를 *제일 먼저* 본다.
- *플랜* 을 읽지 않으면 *튜닝* 이 아니다.
- *Approximate* 를 *겁내지 않는다*.
- *비용 알람* 을 *팀의 채널* 에 둔다.
- *큰 쿼리는 *작게 나눠* 본다.*
...
1. *느린 쿼리 한 개* 를 골라 *컬럼 좁히기* 를 적용해 보세요.
2. *Pruning 이 깨진* 쿼리를 *고쳐* 보세요.
3. *Approximate* 가 *적합한 경우* 와 *부적합한 경우* 를 적어 보세요.
