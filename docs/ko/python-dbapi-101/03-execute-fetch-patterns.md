...
print(cur.rowcount)    # 1 (INSERT 영향받은 row 수)
```
...
```python
cur.arraysize = 500
for row in cur:        # 500 row 단위로 prefetch
    process(row)
```
...
- `execute()`는 single statement, `executemany()`는 같은 statement의 bulk write입니다.
- `fetchone`/`fetchall`/`fetchmany`/`for row in cur`는 결과 크기에 따라 골라 씁니다.
- `for row in cursor`는 streaming의 default이고, 큰 결과의 OOM을 막아줍니다.
- `arraysize`를 키우면 prefetch로 round-trip이 줄어듭니다.
