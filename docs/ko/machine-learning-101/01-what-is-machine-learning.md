...
**핵심 질문**: *머신러닝* 은 *통계의 새 이름* 일까요, *프로그래밍의 새 패러다임* 일까요?

> *머신러닝은 *데이터로부터 함수를 학습* 하여 *처음 보는 입력* 에 *예측* 을 내놓는 기술입니다.*
...
- **학습**: *데이터에서 패턴* 을 *함수* 로 *추정*.
- **일반화**: *처음 보는 데이터* 에서도 *잘 작동* 함.
- **예측**: 학습한 *함수* 를 *새 입력* 에 적용.
- **피처(feature)**: *입력 변수*.
- **레이블(label)**: *예측 대상*.
...
1. *iris* 가 아닌 *직접 만든 데이터* 로 *fit/predict* 를 해보세요.
2. *score* 가 *과대평가* 되는 *조건* 을 설명하세요.
3. *피처 스케일* 이 *결과를 바꾸는* 예를 만드세요.
