...
현재 몇 개 인스턴스가 돌고 있는지 볼 때는 운영자가 먼저 써야 할 경로가 둘입니다.
...
CLI에서 현재 앱 설정 계층을 확인하고 싶다면 `az functionapp show --name $APP --resource-group $RG --query siteConfig`가 지원되는 운영 경로입니다. 일상적인 스케일 관찰은 이 정보와 Application Insights, Azure Monitor 메트릭을 함께 보는 편이 안전합니다.

`/admin/host/scale/status`는 심화 분석이나 진단에서 볼 수는 있지만, 일반 운영용 안정된 공개 표면이라기보다 내부 진단 엔드포인트에 가깝다고 이해하는 편이 맞습니다.
...
상대 시간 범위는 `--start-time -7d`처럼 쓰지 말고 `--offset 7d`로 적는 것이 현재 CLI 형식에 맞습니다. `--start-time`을 쓸 때는 ISO-8601 시각을 명시해야 합니다.
...
내부 구현이 더 궁금하면 [Azure Functions Deep Dive — 스케일링 내부 동작](../azure-functions-deep-dive/05-scaling-internals.md)과 [Azure Functions Deep Dive — 콜드 스타트와 Placeholder Mode](../azure-functions-deep-dive/06-cold-start-placeholder.md)를 이어서 보면 좋습니다. 101 시리즈가 실무 감각을 잡는 데 초점을 맞췄다면, 심화편은 Host 코드와 논문을 바탕으로 내부 동작을 해부합니다.

---

## 운영 체크리스트

- [ ] 함수별 latency/실패율 차트를 기본 대시보드에 추가했다
- [ ] 주요 의존성(DB, 외부 API) 호출의 distributed trace를 활성화했다
- [ ] 비용 알람(execution 수, GB-s)을 설정했다
- [ ] 장애 대응 런북에 첫 5분 액션을 명시했다
- [ ] Live Metrics 접근 권한을 운영팀과 공유했다
