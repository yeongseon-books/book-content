...
여기서는 앞 장에서 정리한 플랜 선택을 운영 관점으로 다시 읽습니다. 트래픽이 갑자기 치솟을 때 Consumption, Flex Consumption, Premium, Dedicated가 각각 어떻게 반응하는지, 콜드 스타트는 어디서 생기는지, 줄이려면 무엇부터 손대야 하는지를 정리합니다.
...
스케일링과 콜드 스타트를 이해하면 어떤 비용·지연 사고를 예방할 수 있을까요?

이 글은 그 질문에 답하기 위해 스케일링과 콜드 스타트의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- Functions의 스케일 컨트롤러는 어떤 신호를 보고 인스턴스를 늘리는가?
- 콜드 스타트는 정확히 어느 단계에서 발생하고, 무엇으로 측정하는가?
- Premium의 always-ready 인스턴스는 콜드 스타트를 얼마나 줄여주는가?
- burst 트래픽에서 인스턴스가 너무 빨리 늘면 어떤 부작용이 생기는가?
- 외부 의존성(DB connection)이 스케일과 충돌하는 패턴은 무엇인가?
...
![인스턴스 수와 동시성의 스케일 구조](../../assets/azure-functions-101/06/06-01-scaling-has-two-axes-instance-count-and.ko.png)
...
![트래픽 급증 시 플랜별 확장 반응](../../assets/azure-functions-101/06/06-02-how-the-plans-react-to-a-traffic-spike.ko.png)
...
![콜드 스타트를 이루는 준비 단계](../../assets/azure-functions-101/06/06-03-what-a-cold-start-actually-includes.ko.png)
...
## 코드가 뜨기 전까지 플랫폼이 이미 해 주는 일

콜드 스타트를 줄이는 일을 전부 애플리케이션이 혼자 떠안는 것은 아닙니다. Azure Functions 플랫폼도 먼저 움직입니다. Functions Host는 **placeholder 모델**을 통해 실행 환경 준비와 Host 부팅 경로 일부를 미리 당겨서 첫 할당 비용을 낮추려 하고, Premium은 **pre-warmed / Always Ready 인스턴스**를 유지해서 요청이 오기 전에 이미 warm capacity를 확보해 둡니다. Flex Consumption도 같은 방향의 제어점을 제공합니다. **alwaysReady 인스턴스 수**를 0으로 두면 scale to zero가 가능하고, 1 이상으로 잡으면 그만큼 warm capacity를 유지하면서 첫 요청 지연을 줄일 수 있습니다.

여기서 경계를 분명히 보면 판단이 쉬워집니다. 플랫폼이 대신해 주는 것은 “인스턴스가 생기고 Host가 기본적으로 준비되는 시간”까지입니다. 반면 큰 import, 무거운 전역 초기화, 첫 요청에서 열리는 느린 DB 연결, 애플리케이션이 스스로 만드는 초기화 비용은 여전히 사용자 책임입니다. 즉 Azure가 인프라 쪽 스타트업 지연을 줄여 줄 수는 있어도, 여러분 코드의 시작 비용까지 없애 주지는 못합니다.
...
![함수 확장과 다운스트림 용량의 차이](../../assets/azure-functions-101/06/06-04-1-downstream-systems-do-not-scale-with-y.ko.png)
...
## 동작을 봤다면 이제 관측으로 넘어갑니다

스케일링과 콜드 스타트는 보이지 않으면 관리할 수 없습니다. 이어지는 모니터링 장에서는 Application Insights, 메트릭, KQL, 알람을 중심으로 “지금 몇 개 인스턴스가 돌고 있는가”, “실패율이 언제 튀는가”, “비용이 왜 늘었는가”를 어떻게 추적하는지 정리합니다.

콜드 스타트와 스케일링 내부 구현이 궁금하면 [Azure Functions Deep Dive — 스케일링 내부 동작](../azure-functions-deep-dive/05-scaling-internals.md)과 [Azure Functions Deep Dive — 콜드 스타트와 Placeholder Mode](../azure-functions-deep-dive/06-cold-start-placeholder.md)를 같이 보면 좋습니다. 101 시리즈가 운영 판단 기준을 잡는 글이라면, 심화편은 그 판단이 코드에서 어떻게 구현됐는지를 따라갑니다.

---

## 시니어 엔지니어는 이렇게 생각합니다

- **Scale Controller가 결정의 중심** — 메트릭 기반 스케일 신호를 이해하면 튜닝이 가능합니다.
- **스케일 단위는 인스턴스가 아니라 워커** — 동시성 설정이 비용·지연 모두에 영향을 줍니다.
- **콜드 스타트는 첫 사용자 비용** — P99 지연이 SLA를 깨는 가장 흔한 원인입니다.
- **Pre-warmed로 콜드 스타트 줄이기** — Premium 플랜 선택의 핵심 이유입니다.
- **최대 인스턴스 상한을 둔다** — 비용 사고를 막는 단순하지만 강력한 장치입니다.

## 운영 체크리스트

- [ ] 스케일 컨트롤러가 보는 신호(큐 길이, HTTP 큐)를 워크로드별로 정리했다
- [ ] 콜드 스타트 메트릭(p50/p95)을 대시보드에 노출했다
- [ ] always-ready 인스턴스 수와 비용의 트레이드오프를 결정했다
- [ ] DB 커넥션 풀 한도와 함수 인스턴스 한도를 같이 계산했다
- [ ] burst 시 throttling 정책(max scale-out limit)을 설정했다
...
- [Azure Functions Deep Dive — 스케일링 내부 동작](../azure-functions-deep-dive/05-scaling-internals.md)
- [Azure Functions Deep Dive — 콜드 스타트와 Placeholder Mode](../azure-functions-deep-dive/06-cold-start-placeholder.md)
