...
- **이름이 명세** — 콜백 이름이 의도를 드러내야 합니다.
- **부분 적용** — functools.partial로 보일러플레이트를 줄입니다.
- **타입 힌트** — Callable 시그니처를 정확히 명시합니다.
- **테스트** — 콜백 주입이 테스트 가성비를 크게 높입니다.
- **람다 절제** — 복잡한 람다는 named function으로 풀어 가독성을 높입니다.
