...
**핵심 질문**: "사이트가 안 돼요"라는 한 줄 보고를 받았을 때, 무엇부터 어떤 순서로 확인하면 가장 빠르게 원인을 좁힐 수 있을까요?

> 네트워크 디버깅의 본질은 추측이 아니라 **계층을 따라 한 칸씩 내려가며 가설을 잘라 내는 일**입니다. 링크 → 라우팅 → DNS → TCP → TLS → HTTP 순서로 보면 1분 안에 80%의 문제가 어느 층의 일인지 결정됩니다. 좋은 도구는 새로운 정보를 주는 것이 아니라, 잘못된 가설을 빨리 죽여 줍니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 네트워크 문제를 계층 순서대로 좁히는 사고 방식
- ping, dig, curl, ss, tcpdump 각각이 알려 주는 것
- 흔한 실패 패턴(타임아웃 vs reset vs DNS 실패)을 출력으로 구분하기
- 시리즈에서 배운 개념이 실제 디버깅에서 어떻게 합쳐지는지
...
- **ICMP**: ping과 traceroute가 사용하는 진단용 IP 프로토콜.
- **RTT (Round Trip Time)**: 패킷이 목적지에 갔다가 돌아오는 데 걸리는 시간.
- **Connection refused**: 호스트는 살아 있지만 그 포트에서 듣고 있는 프로세스가 없을 때.
- **RST**: TCP에서 "이 연결을 즉시 끊는다"는 신호. 종종 방화벽이 보냅니다.
- **Capture filter**: tcpdump에 넘기는 BPF 식. 잡을 패킷을 미리 좁혀 줍니다.
...
- 가설을 적어 두고 도구로 죽입니다. 도구를 먼저 잡고 가설을 만들지 않습니다.
- 같은 문제를 두 시점(클라이언트/서버)에서 동시에 봅니다.
- "재시작으로 고쳐졌다"는 결론을 좋아하지 않습니다. 다음에 또 만날 일이라고 가정합니다.
- 자주 쓰는 5단계 명령은 외워 둡니다. 새벽 3시의 자기 자신이 검색에 시간을 쓰지 않도록.
- 디버깅 후에는 짧은 포스트모템을 남깁니다. 다음 사람의 5분을 절약합니다.
...
1. `nc -vz host port`가 "Connection refused"를 줄 때와 "timeout"을 줄 때, 다음에 시도해야 하는 행동을 각각 한 줄로 적어 보세요.
2. 사용자가 "갑자기 사이트가 안 된다"고 합니다. 위 5단계 명령을 순서대로 적고, 각 명령에서 어떤 결과가 나와야 다음 단계로 넘어가는지 적어 보세요.
3. `tcpdump`로 `api.example.com`의 443번 포트 트래픽만 잡아서 `cap.pcap`에 저장하는 명령을 작성해 보세요.
