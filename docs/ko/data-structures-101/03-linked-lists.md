...
**핵심 질문**: 배열의 중간에 값을 끼워 넣으면 모든 원소가 한 칸씩 밀려납니다. 이것을 피할 자료구조는 없을까요?

> 연결 리스트는 각 노드가 다음 노드의 주소(포인터)를 가지고 있는 자료구조입니다. 메모리상 흩어져 있어도 포인터를 따라가면 순서대로 데이터를 방문할 수 있습니다. 노드를 끼워 넣거나 빼는 일은 주변 두 포인터만 수정하면 끝나므로 O(1)입니다. 하지만 인덱스 접근은 처음부터 따라가야 해서 O(n)입니다. 이 글에서는 단일/이중 연결 리스트의 구조와 트레이드오프를 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 단일 연결 리스트와 이중 연결 리스트의 구조 차이
- 연결 리스트와 배열의 시간/공간 복잡도 비교
- 연결 리스트를 직접 구현하면서 포인터 조작 익히기
- 실무에서 연결 리스트가 적합한 경우와 부적합한 경우
...
| 용어 | 설명 |
| --- | --- |
| 노드(node) | 값과 포인터를 가진 단위 |
| 포인터(pointer) | 다른 노드의 메모리 주소 |
| 헤드(head) | 리스트의 첫 노드 참조 |
| 테일(tail) | 마지막 노드 참조 (이중 연결에서 자주 유지) |
| 센티넬(sentinel) | 경계 처리를 단순화하기 위한 가짜 노드 |
...
시니어 엔지니어는 "교과서적인 단일 연결 리스트"를 잘 쓰지 않습니다. 대부분의 실제 워크로드에서는 캐시 친화성 때문에 배열·deque가 더 빠릅니다. 연결 리스트가 진짜 빛나는 경우는 (1) 양 끝에서만 연산하거나 (2) 다른 자료구조가 노드 참조를 직접 들고 있어 임의 노드를 O(1)에 삭제할 수 있을 때입니다.

또한 시니어는 포인터 조작을 그림으로 그리고 코드를 씁니다. 머릿속에서 포인터를 추적하다 보면 거의 반드시 실수가 생깁니다. 종이에 노드와 화살표를 그려서 단계별로 검증하는 습관이 버그를 가장 효과적으로 막습니다.
...
1. 위 `SinglyLinkedList`에 `reverse()` 메서드를 구현해 보세요. 새 리스트를 만들지 않고 포인터만 뒤집어야 합니다.

2. 단일 연결 리스트에서 사이클(순환)이 있는지 판별하는 알고리즘을 구현해 보세요. 힌트: 빠른 포인터와 느린 포인터를 사용합니다(Floyd's Tortoise and Hare).

3. 두 개의 정렬된 단일 연결 리스트를 하나의 정렬된 리스트로 합치는 함수를 작성해 보세요. 새 노드를 만들지 말고 기존 노드를 재배치합니다.
