...
**핵심 질문**: 요구사항을 들었다는 것과 이해했다는 것은 같을까요?

> 잘못된 문제를 정확히 푸는 것이 가장 큰 낭비입니다.
...
- **기능 요구**: 시스템이 무엇을 하는가.
- **비기능 요구**: 어떻게 잘 해야 하는가 (성능/보안/가용성).
- **사용자 스토리**: "역할로서 결과를 위해 행동"의 1줄 형식.
- **인수 조건(AC)**: 완료를 측정하는 조건.
...
- "왜 필요한가"를 한 줄로 답할 수 없으면 시작하지 않습니다.
- 비기능 요구를 코드 작성 전에 합의합니다.
- 인수 조건을 테스트로 자동화합니다.
- 변경 결정은 RFC/ADR로 남깁니다.
- 모호함은 코드가 아니라 대화로 해결합니다.
...
1. 본인 프로젝트의 한 기능을 사용자 스토리로 다시 적어 보세요.
2. AC가 없는 요구를 골라 AC 5개를 써 보세요.
3. 무시하면 사고가 날 비기능 요구 두 가지를 적어 보세요.
