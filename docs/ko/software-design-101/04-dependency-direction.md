...
**핵심 질문**: 의존성의 방향이 왜 그렇게 중요한가요?

> 화살표가 어디를 향하느냐가 변경의 비용을 결정합니다. 핵심이 세부를 모르게 만들면 자유가 생깁니다.
...
- **Dependency**: A가 B를 import/호출하면 A는 B에 의존한다.
- **Stable module**: 변경 빈도가 낮고, 추상도가 높은 쪽.
- **Volatile module**: DB, HTTP, 외부 SaaS처럼 자주 바뀌는 쪽.
- **DIP (Dependency Inversion Principle)**: 핵심은 추상에 의존하고, 세부는 그 추상을 구현한다.
- **Port / Adapter**: 핵심이 정의한 인터페이스(port)를 외부 어댑터가 구현한다.
...
- 의존성 그래프를 머릿속에 들고 있다.
- 변경이 잦은 쪽에서 안정적인 쪽으로 화살표를 두지 않는다.
- 도메인이 인프라를 import하면 즉시 알아챈다.
- 포트의 수와 모양을 도메인이 결정한다.
- 조립은 한 곳에 모은다.
...
1. 본인 프로젝트에서 도메인이 import하는 외부 모듈 1개를 고르세요. DIP로 뒤집을 가치가 있나요?
2. 결제 모듈의 DB 호출을 포트/어댑터로 분리해 보세요.
3. 도메인 단위 테스트를 가짜 어댑터로 작성해 보세요.
