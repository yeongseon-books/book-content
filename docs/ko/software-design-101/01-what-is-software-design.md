...
**핵심 질문**: 코드를 잘 쓰는 것과 설계를 잘하는 것은 무엇이 다른가요?

> 좋은 코드는 한 줄을 읽기 좋게 만들고, 좋은 설계는 코드 베이스 전체를 변경하기 좋게 만듭니다.
...
- **Software design**: 모듈/책임/의존성에 대한 결정의 집합.
- **Architecture**: 가장 큰 단위의 설계 결정.
- **Coupling**: 모듈 간 의존도.
- **Cohesion**: 모듈 내 응집도.
- **Change cost**: 다음 변경에 드는 시간/위험.
...
1. 본인 프로젝트의 모듈 의존성 그래프를 그려 보세요.
2. 가장 잦은 변경 1건을 골라 손대는 파일 수를 세 보세요.
3. 마지막 큰 결정을 ADR 한 페이지로 적어 보세요.
