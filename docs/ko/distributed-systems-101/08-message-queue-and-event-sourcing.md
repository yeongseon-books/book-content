...
**핵심 질문**: 서비스 둘이 같은 데이터에 의존할 때, 직접 호출하지 않고 일을 주고받는 방법은 무엇일까요?

> message queue와 event sourcing은 시간 축을 분산 시스템의 일급 도구로 끌어올리는 두 가지 방식입니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- message queue가 풀어 주는 결합과 보장 모델
- at-most-once / at-least-once / exactly-once의 의미
- event sourcing의 정의와 CQRS와의 관계
- Kafka가 broker가 아닌 log인 이유
- consumer group, partition, offset의 핵심
...
- **Message queue**: 메시지를 저장하고 분배하는 중간자.
- **Event sourcing**: 상태를 사건들의 누적으로 표현하는 패턴.
- **Offset**: consumer가 어디까지 읽었는지 가리키는 위치.
- **Consumer group**: 같은 그룹 안에서 메시지를 분담해 읽는 단위.
- **Idempotency**: 같은 메시지를 여러 번 처리해도 결과가 같은 성질.
...
- 큐 도입 전에 정말 비동기여도 되는 흐름인지 확인합니다.
- consumer의 idempotency 설계를 큐 선택보다 먼저 합니다.
- offset 관리(누가 commit하는가)를 명세로 적습니다.
- event schema versioning 전략을 처음부터 결정합니다.
- read model 재구성 시간을 측정해 SLO에 둡니다.
...
1. 주문 시스템을 event sourcing으로 설계해 보세요. 이벤트 5개의 schema를 적어 보세요.
2. at-least-once + idempotent consumer로 exactly-once에 가까워지는 이유를 한 문단으로 설명해 보세요.
3. Kafka partition 수를 결정하는 기준 두 가지를 적어 보세요.
