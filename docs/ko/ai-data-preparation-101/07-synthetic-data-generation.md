...
- **Instruction tuning**: Self-Instruct, Alpaca, Evol-Instruct가 GPT-4 등으로 instruction-response 쌍 생성
- **RAG eval**: 문서로부터 question-answer 쌍 자동 생성
- **Data distillation**: 거대 모델의 답변을 작은 모델 학습에 활용
...
1. **License 확인 없이 teacher 사용**: OpenAI 출력으로 경쟁 모델을 학습하면 ToS 위반입니다. open-source teacher 또는 commercial license를 확인합니다.
2. **Verification 단계 생략**: synthetic QA의 evidence가 실제 문서에 없는 경우가 흔합니다. substring 또는 NLI 모델로 검증합니다.
3. **Temperature를 너무 낮게**: 0.0~0.3은 다양성이 죽어 같은 패턴만 반복됩니다. self-instruct는 0.8~1.0이 적정합니다.
4. **Synthetic만으로 fine-tune**: model collapse가 발생합니다. real human data와 1:3~1:5 비율로 섞습니다.
5. **Bias 전이를 무시**: teacher의 편향이 그대로 student에 복제됩니다. demographic distribution을 반드시 측정합니다.
