...
- **여러 데이터 준비 단계를 실제 운영 가능한 하나의 파이프라인으로 묶으려면 어떤 시스템 속성이 필요할까요?**
  - 이 글 기준 운영 가능한 파이프라인은 `Stage`마다 입력·출력·파라미터·manifest가 있고, `rows_in`, `rows_out`, `drop_rate` 같은 통계를 남기는 구조입니다. 단계가 많아도 계약과 관측성이 분명하면 다시 돌려도 같은 결과를 설명할 수 있습니다.
- **DVC와 stage fingerprint는 데이터 버전 관리와 idempotency를 어떻게 함께 해결할까요?**
  - `dvc add data/02_clean.parquet`는 산출물 버전을 고정하고, `fingerprint()`는 입력 파일과 파라미터가 같을 때 stage를 `[skip] cached`로 넘기게 합니다. 그래서 같은 배치를 다시 돌릴 때 “무엇을 썼는가”와 “왜 다시 안 돌렸는가”를 동시에 답할 수 있습니다.
- **Airflow 같은 오케스트레이터는 단순 스케줄링 외에 어떤 운영 가치를 줄까요?**
  - 본문 DAG에서 `t1 >> t2 >> ... >> t6`처럼 흐름을 고정하면 retry, lineage UI, 실패한 stage만 재실행하는 운영 루프가 생깁니다. cron과 달리 어느 단계가 깨졌는지와 어디서 다시 시작해야 하는지가 바로 보인다는 점이 핵심입니다.
...
### 관련 시리즈
- [LLM API 프로덕션 101 — 캐싱 전략 — 비용과 지연 시간 줄이기](../llm-api-production-101/04-caching-strategies.md)
- [AI Evaluation 101 — LLM-as-Judge — 모델로 모델을 평가하기](../ai-evaluation-101/04-llm-as-judge.md)
