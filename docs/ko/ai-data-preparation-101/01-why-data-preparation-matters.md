...
- **모델 아키텍처보다 데이터 준비가 더 큰 차이를 만드는 순간은 언제일까요?**
  - 이 글의 실험처럼 `TfidfVectorizer`와 `LogisticRegression`을 그대로 둔 채 `clean()`만 바꿨는데 `accuracy_score`가 움직인 순간이 바로 그때입니다. 모델보다 입력 데이터의 중복, 잡음, 인코딩 문제를 먼저 바로잡는 편이 더 큰 레버리지를 주는 경우가 많습니다.
- **겉보기 정확도는 높은데 실제 사용자 경험이 나빠지는 대표적인 데이터 실패는 무엇일까요?**
  - `train_test_split(..., stratify=y)`를 빼먹거나 train/eval 사이 중복을 남겨 두면 accuracy는 높게 보여도 실제 분포에서는 쉽게 무너집니다. 글에서 본 duplicate ratio와 클래스 비율 검증이 필요한 이유가 바로 그 착시를 막기 위해서입니다.
- **수집부터 분할까지의 데이터 준비 단계를 왜 하나의 파이프라인으로 봐야 할까요?**
  - `quick_quality_report()`, `load_and_validate()`, `RunManifest`가 한 흐름으로 묶인 이유는 정제, PII 처리, 분할이 서로 결과를 바꾸기 때문입니다. 어느 한 단계만 따로 보면 실험 결과가 아니라 우연한 버전 조합만 남게 됩니다.
