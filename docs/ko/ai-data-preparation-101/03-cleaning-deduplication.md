...
문제는 정제 함수가 길어질수록 무엇이 데이터를 망가뜨렸는지 추적하기 어려워진다는 데 있습니다. 중복 제거도 exact dedup에서 멈추면 웹 크롤링 문서 특유의 near-duplicate를 대부분 놓칩니다.
...
- **정제 함수는 왜 작은 변환들의 합으로 유지해야 할까요?**
  - `clean_text()`를 NFC 정규화, HTML 제거, 제어 문자 정리처럼 잘게 나눠 두면 어느 단계에서 `char_reduction_pct`가 튀었는지 바로 좁힐 수 있습니다. 정제 함수가 한 덩어리면 규칙 하나가 데이터를 얼마나 잘랐는지 설명하기가 어려워집니다.
- **exact dedup만으로는 웹 코퍼스 품질 문제가 왜 충분히 해결되지 않을까요?**
  - `exact_dedup()`은 공백과 대소문자 차이 정도는 잡지만, 광고 한 줄이나 문장부호 하나만 달라진 페이지는 그대로 남깁니다. 그래서 글에서 `MinHashLSH`와 `near_dedup(docs, threshold=0.85)`를 바로 다음 단계로 붙였습니다.
- **MinHash threshold를 너무 낮추거나 높이면 어떤 오류가 생길까요?**
  - threshold가 너무 낮으면 다른 문서까지 같은 것으로 보고 버려서 false positive가 늘고, 너무 높으면 near-duplicate를 놓쳐 false negative가 늘어납니다. 그래서 본문에서도 hand-labeled pair로 precision과 recall을 함께 보고 0.85~0.9 부근에서 시작하자고 정리했습니다.
...
### 관련 시리즈
- [AI Evaluation 101 — 평가 데이터셋 설계하기](../ai-evaluation-101/02-evaluation-dataset-design.md)
- [LLM 파인튜닝 101 — 데이터셋 준비와 전처리](../llm-finetuning-101/02-dataset.md)
