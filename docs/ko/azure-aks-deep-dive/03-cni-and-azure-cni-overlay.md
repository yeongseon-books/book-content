...
- [ ] 예상 Pod 수 대비 subnet 크기와 service CIDR 여유를 계산했다
- [ ] Overlay 사용 여부와 외부 시스템(방화벽, peering)의 호환성을 검증했다
- [ ] NetworkPolicy의 기본 deny 정책 도입 여부와 단계적 롤아웃 계획을 정했다
- [ ] SNAT port 고갈을 모니터링하고 NAT Gateway 사용 여부를 결정했다
- [ ] DNS 트래픽 경로(coredns, upstream)와 캐시 정책을 점검했다
...
- [Azure Functions Deep Dive 1화 — 큰 그림 먼저 보기](../azure-functions-deep-dive/01-host-bootstrap.md)
