...
HPA·CA의 내부 동작을 이해하면 어떤 스케일 사고를 예방할 수 있을까요?

이 글은 그 질문에 답하기 위해 HPA와 Cluster Autoscaler 내부의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- HPA는 어떤 메트릭 소스를 어떤 주기로 읽고, 결정은 어떻게 내려지는가?
- Cluster Autoscaler는 ‘노드를 추가해야 한다’를 어떤 신호로 감지하는가?
- HPA와 CA가 동시에 움직일 때 발생하는 race는 어떻게 보이고, 어떻게 완화되는가?
- scale up은 빠르고 scale down은 느려야 하는 이유는 무엇이며, 그 비율은 어디서 정해지는가?
- VPA(수직 스케일러)와 HPA를 같이 쓰는 것이 안전한 워크로드는 어떤 모양인가?
...
- **HPA는 메트릭 서버에 의존** — 메트릭 파이프라인 장애가 곧 스케일 마비입니다.
- **CA는 Pending Pod를 본다** — request 값이 부정확하면 CA가 헛돕니다.
- **축소 정책이 비용을 결정한다** — 스케일 다운 cooldown과 utilization 임계값을 신중히 정합니다.
- **노드 그룹 형상이 효율을 좌우** — 단일 큰 노드와 여러 작은 노드의 트레이드오프를 의식합니다.
- **스케일 결정은 로깅으로 가시화** — 왜 스케일했는지 추적 못 하면 튜닝이 불가능합니다.
...
- [Azure Functions Deep Dive 5화 — scaling internals](../azure-functions-deep-dive/05-scaling-internals.md)
