...
- **LangGraph는 왜 단순 체인보다 명시적인 상태 기계로 보는 편이 좋을까요?**
  - 체인은 순서를 길게 잇지만 LangGraph는 어떤 상태가 어디서 바뀌고 다음 단계가 어떻게 결정되는지 드러내므로 실패 지점을 좁히기 쉽습니다.
- **노드와 엣지, state는 각각 실행 흐름에서 어떤 책임을 나눌까요?**
  - 노드는 state를 읽고 갱신하는 작업 단위이고, 엣지는 다음 노드를 결정하며, state는 그래프 전체가 공유하는 실행 기록입니다.
- **첫 그래프를 실행한 뒤에는 최종 문장보다 어떤 state 값을 먼저 확인해야 할까요?**
  - 최종 문장만 보지 말고 입력, 중간 필드, 선택된 경로, 누적 메시지처럼 그래프가 갱신한 state를 먼저 확인해야 합니다.

<!-- toc:begin -->
## 시리즈 목차

- **LangGraph 101 (1/6): LangGraph 소개와 그래프 기초 (현재 글)**
- LangGraph 101 (2/6): 상태 관리와 체크포인트 (예정)
- LangGraph 101 (3/6): 조건부 엣지와 분기 흐름 (예정)
...
### 관련 시리즈
- [LangChain 101](../langchain-101/01-lcel-runnable-basics.md) — LangGraph가 노드 안에서 호출하는 Runnable과 LCEL을 다룹니다. 그래프가 아니라 그래프의 한 노드에서 무엇이 실행되는지가 흐릿하면 이 시리즈로 한 단계 내려가 읽기를 권장합니다.
