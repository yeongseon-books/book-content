...
운영 가능한 Agent 작업 환경을 어떻게 통합해야 모든 harness가 일관되게 동작할까요?

이 글은 그 질문에 답하기 위해 Production Harness의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **Task·Context·Tool·Test·Feedback을 통합** — 각 영역의 결정이 서로 정합해야 합니다.
- **배포·롤백이 코드 시스템 수준** — Agent도 일반 서비스와 같은 운영 표준을 따릅니다.
- **리소스 한도가 운영의 안전장치** — 비용 폭주를 처음에 막습니다.
- **승인·관측이 신뢰의 기반** — 사람·시스템의 개입 지점이 명시됩니다.
- **회귀 테스트가 진화의 보호막** — Agent도 회귀할 수 있습니다.
