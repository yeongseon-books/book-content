---
title: "LangChain 101 (1/6): LangChain 소개 — LCEL과 Runnable 기본"
...
![반복되는 글루 코드와 LCEL 추상화 흐름](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-01-the-problem-langchain-addresses.ko.png)
...
![invoke batch stream 실행 모드](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-02-the-runnable-interface.ko.png)
...
![프롬프트 모델 파서 타입 흐름](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-03-lcel-and-the-pipe-operator.ko.png)
...
![여러 입력을 펼쳐 처리한 뒤 모으는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/01/01-04-batch-for-multiple-inputs.ko.png)
...
이 출력에서 중요한 것은 "어디가 느린가"와 "어디서 실패했는가"를 바로 분리할 수 있다는 사실입니다. 프롬프트와 파서는 빠른데 LLM 단계만 느리다면 모델 공급자 또는 네트워크 이슈를 먼저 봐야 하고, parser에서만 실패가 반복되면 출력 형식 제약을 강화해야 합니다.
...
- **LCEL은 프롬프트, 모델, 파서를 어떤 공통 계약으로 묶어 줄까요?**
  LCEL은 각 컴포넌트를 Runnable로 보고 입력과 출력 모양이 맞는 단계들을 같은 실행 계약으로 연결합니다.

- **`prompt | llm | parser` 안에서는 단계마다 어떤 데이터 모양이 오갈까요?**
  프롬프트는 dict를 메시지로 바꾸고, 모델은 메시지를 AIMessage로 바꾸며, 파서는 그 결과를 앱이 쓰기 쉬운 값으로 정리합니다.

- **`invoke()`, `batch()`, `stream()`은 같은 체인에서 각각 언제 써야 할까요?**
  단일 요청은 `invoke()`, 여러 입력의 재사용은 `batch()`, 사용자에게 부분 결과를 보여줘야 할 때는 `stream()`을 먼저 봅니다.

<!-- toc:begin -->
## 시리즈 목차

- **LangChain 101 (1/6): LangChain 소개 — LCEL과 Runnable 기본 (현재 글)**
- LangChain 101 (2/6): Prompt와 LLM Chain — 체인 첫 번째 구성 (예정)
...
- [LangGraph 101](../langgraph-101/01-graph-basics.md) — 이 시리즈에서 익히는 Runnable·LCEL 위에 그래프 기반 상태 머신을 얹어 에이전트를 구성합니다. 단순한 chain을 넘어 멀티 단계·분기·루프가 필요해지는 시점에 다음 단계로 권장합니다.

- [이 글의 예제 코드 (book-examples)](https://github.com/yeongseon-books/book-examples/tree/main/langchain-101/ko/01-lcel-runnable-basics)
