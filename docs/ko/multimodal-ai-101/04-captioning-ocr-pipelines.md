...
- **왜 “이미지에서 텍스트만 추출하면 된다”는 접근이 실제 문서 처리에서는 자주 실패할까요?**
  - OCR은 문자열은 주지만 표 구조, 강조, 시각적 관계 같은 장면 의미를 버리기 때문에 문서와 화면 이해에서 곧 한계가 납니다. 본문이 제시한 hybrid 흐름처럼 `Fast OCR -> layout analysis -> VLM reasoning`을 묶어야 총액, 날짜, 장면 맥락을 함께 안정적으로 복원할 수 있습니다.
- **Captioning 모델과 VLM 기반 설명 생성은 어떤 입력에서 각각 강점과 한계를 보일까요?**
  - BLIP 계열 captioner는 `caption("samples/dog.jpg")` 예제처럼 빠르고 싸게 객체 수준 설명을 대량 생성할 때 유리합니다. 반면 차트 추세나 접근성 alt text처럼 맥락과 추론이 필요한 경우에는 `gpt-4o-mini`를 호출하는 `rich_caption()` 경로가 더 적합하지만 비용과 지연이 커집니다.
- **Tesseract, PaddleOCR, Document AI 계열은 어떤 기준으로 선택해야 할까요?**
  - Tesseract는 빠른 시작과 저비용, PaddleOCR은 한국어와 bbox 기반 layout 처리, 클라우드 Document AI는 표와 key-value 구조 추출에 강점이 있습니다. 글의 기준대로 문자 정확도뿐 아니라 회전 이미지, confidence threshold, PII 마스킹, 페이지당 비용까지 함께 보고 엔진을 고르는 편이 안전합니다.
...
- [문서 수집과 인덱싱 101 - PDF 파싱과 텍스트 추출](../document-ingestion-101/01-pdf-parsing.md)
- [AI 앱 패턴 101 - 문서 어시스턴트](../ai-app-patterns-101/03-document-assistant.md)
- [문서 수집과 인덱싱 101 - 다중 포맷 문서 파이프라인](../document-ingestion-101/05-multi-format-pipeline.md)
