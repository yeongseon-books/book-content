...
- **텍스트 RAG는 왜 이미지, 표, 레이아웃 정보가 중요한 질문에서 곧바로 성능 한계를 드러낼까요?**
  - 텍스트 청크만 인덱싱하면 표 오른쪽 아래 수치, 버튼 위치, 색상과 모양처럼 시각 증거가 필요한 질문을 회수하지 못합니다. 그래서 본문은 CLIP 이미지 인덱스, caption+OCR 텍스트 인덱스, VLM 최종 판독을 분리해 retrieval 단계부터 어떤 근거를 가져오는지 다시 설계해야 한다고 설명합니다.
- **멀티모달 검색을 위해 원본 이미지, caption/OCR, dual index를 쓰는 세 가지 전략은 어떻게 다를까요?**
  - image embedding 단독 인덱스는 CLIP으로 시각 패턴 검색에는 강하지만 숫자 reasoning에는 약하고, caption+OCR 텍스트 인덱스는 `Q3 revenue dropped` 같은 질의에 강하지만 색상과 위치 같은 시각 detail을 잃습니다. hybrid 전략은 본문 `HybridIndex` 코드처럼 두 점수를 `alpha`로 합쳐 질의 의도에 따라 visual 질문과 numeric 질문을 다르게 처리합니다.
- **검색 결과를 최종 답변 단계에서 VLM에 넘길 때 어떤 입력 조합이 가장 실용적일까요?**
  - 검색 top 결과 전체를 무작정 넣기보다 top-3 정도의 핵심 이미지와 필요한 OCR·caption 근거만 선별해 VLM에 보내는 구성이 가장 현실적입니다. 본문 `answer()` 예제처럼 retrieved image를 inline으로 넣고, `citations`와 `retrieval_scores`를 함께 남겨 어떤 근거로 답했는지 추적해야 운영 품질이 안정됩니다.
...
- [Vector Search 101 - 벡터 검색 파이프라인](../vector-search-101/06-vector-search-pipeline.md)
- [RAG Deep Dive - Retriever 설계](../rag-deep-dive/03-retriever-design.md)
- [RAG 평가와 벤치마크 101 - 종단 간 RAG 파이프라인 평가](../rag-benchmark-101/05-e2e-evaluation.md)
