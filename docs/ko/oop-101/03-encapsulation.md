...
**핵심 질문**: 객체의 내부 데이터를 왜, 어떻게 보호해야 할까요?

> 캡슐화는 객체의 내부 상태를 외부에서 직접 변경할 수 없도록 보호하고, 정해진 인터페이스를 통해서만 접근하게 하는 원칙입니다. Python은 강제 접근 제어가 없지만, 관례와 property를 통해 효과적으로 캡슐화를 구현합니다.
...
- **관례 신뢰** — _/__ 접두 관례를 팀 합의로 강제합니다.
- **Property 활용** — 불변식 검증은 property로 단순화합니다.
- **Public API 좁게** — 노출 표면을 최소로 유지합니다.
- **불변 데이터** — 캡슐화의 가장 단순한 형태는 불변성입니다.
- **테스트 경계** — 내부 상태가 아닌 행동을 테스트합니다.
...
1. `Password` 클래스를 만들어 비밀번호 설정 시 최소 8자, 숫자 포함을 검증하세요.
2. `Rectangle` 클래스에서 `width`, `height`는 property로 보호하고 `area`는 읽기 전용 계산 속성으로 구현하세요.
3. `Config` 클래스를 만들어 한번 설정된 값은 변경할 수 없는 freeze 패턴을 구현하세요.
