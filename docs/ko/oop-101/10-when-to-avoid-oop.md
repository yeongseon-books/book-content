...
- **순수 변환** — 데이터 변환 파이프라인은 함수가 더 명료합니다.
- **작은 스크립트** — 단순 작업에는 클래스가 오히려 부담입니다.
- **불필요한 상태** — 상태 없는 곳에 객체를 만들지 않습니다.
- **Type 분기** — if-elif 분기 자체로 충분할 때가 많습니다.
- **FP 결합** — 코어는 함수로, 셸만 객체로 가져갑니다.
