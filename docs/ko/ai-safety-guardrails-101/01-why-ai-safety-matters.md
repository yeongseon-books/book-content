---
title: "AI Safety & Guardrails 101 (1/10): AI Safety가 왜 중요한가"
...
![AI Safety의 핵심: LLM 호출을 비신뢰 경계로 보기](https://yeongseon-books.github.io/book-public-assets/assets/ai-safety-guardrails-101/01/01-01-ai-safety-llm.ko.png)
*AI Safety의 핵심: LLM 호출을 비신뢰 경계로 보기*
> LLM 호출은 안전한 함수 호출이 아니라 비신뢰 데이터 경계입니다.
...
guardrail을 가장 단순하게 표현하면 아래 구조입니다. 입력을 먼저 검사하고, 모델 호출 후에는 출력을 다시 검사합니다. 중요한 점은 모델이 스스로 안전을 보장한다고 가정하지 않는다는 사실입니다.
...
문제는 모델이 이 문장을 운영 정책으로 이해하는 것이 아니라, 다른 텍스트와 함께 해석되는 컨텍스트의 일부로 본다는 데 있습니다. 그래서 뒤에 오는 사용자 메시지가 이 지시를 덮어쓸 수 있습니다.
...
제품 데모에서는 잘 보이지 않지만, 운영에서는 공격과 방어가 같은 요청 안에서 연쇄적으로 나타납니다. 아래는 고객지원 챗봇에서 실제로 자주 보는 형태를 단순화한 시나리오입니다.

| 단계 | 공격자 행동 | 방어 포인트 | 기대 결과 |
| --- | --- | --- | --- |
| 1 | 시스템 지시 무시 문구 입력 | pre-input injection 탐지 | 요청 차단 또는 저신뢰 경로 전환 |
| 2 | 정상 질문으로 위장해 민감 정보 유도 | 정책 judge + 도메인 규칙 | 법무/보안 금지 답변 차단 |
| 3 | 장문 입력으로 비용 압박 | 토큰 기반 rate limiter | 비용 상한 내에서 응답 제한 |
| 4 | 출력에서 이메일/계정번호 재노출 유도 | post-output PII 재검사 | 마스킹 후 전달 또는 차단 |
| 5 | 우회 시도 반복 | audit correlation + 계정/IP 점수 | 단계적 제재 |

핵심은 한 번의 완벽한 차단이 아니라, 여러 지점에서 실패 확률을 낮추는 것입니다. 이 관점이 없으면 공격이 통과한 뒤에야 다음 레이어를 붙이게 됩니다.

### 정책 코드의 최소 골격

아래 예시는 NeMo Guardrails나 Guardrails AI를 도입하기 전에도 바로 적용할 수 있는 정책 계층의 최소 형태입니다.
...
이 구조에서는 정책이 코드에서 명시적으로 보입니다. 정책이 프롬프트 문장 속에 숨어 있으면 운영자가 실제 집행 규칙을 확인하기 어렵습니다.

### 도입 순서 체크리스트

- [ ] 입력 차단, 출력 차단, 감사 기록을 서로 다른 함수와 로그 키로 분리합니다.
- [ ] 차단 사유 코드를 표준화합니다. 예: `prompt_injection_pattern`, `pii_detected`, `cost_budget_exceeded`.
- [ ] 사용자 메시지는 일반화하고 내부 로그에는 정밀 사유를 남깁니다.
- [ ] 첫 릴리스부터 red-team prompt 20개 이상을 회귀 세트로 고정합니다.
- [ ] guardrail 성능 지표(P95 지연, 차단율, FP율)를 대시보드에 올립니다.

## 운영 도입 순서 예시

처음부터 모든 레이어를 붙이면 팀이 운영 포인트를 놓치기 쉽습니다. 보통은 4주 단위로 안전장치를 단계적으로 올립니다.

1주차에는 입력 패턴 차단과 기본 rate limit, 감사 이벤트 스키마를 먼저 고정합니다.
2주차에는 출력 모더레이션과 PII 재검사를 붙여 누출 표면을 줄입니다.
3주차에는 hallucination 검증과 fallback UX를 넣고, 4주차에는 회귀 세트를 CI에 연결해 정책 변경의 부작용을 수치로 확인합니다.

이 순서는 정책을 크게 바꾸지 않고도 위험을 빠르게 낮출 수 있습니다. 또한 각 단계에서 block rate, false positive rate, P95 지연을 같이 관찰하면 팀이 품질과 안전을 동시에 조절할 수 있습니다.
...
- AI Safety & Guardrails 101 (2/10): Prompt Injection 방어 (예정)
...
- [LLM 앱 운영 101 — LLM 앱 모니터링과 로깅](../llm-apps-ops-101/01-monitoring-and-logging.md)
