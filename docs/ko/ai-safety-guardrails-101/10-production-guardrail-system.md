...
## 프레임워크 통합 예시: Guardrails AI + NeMo Guardrails

운영에서는 한 프레임워크로 모든 문제를 해결하기보다, 역할을 분리해 조합하는 편이 현실적입니다.

| 계층 | 도구 | 역할 |
| --- | --- | --- |
| 대화 흐름 | NeMo Guardrails | 입력/대화 상태 정책 |
| 출력 검증 | Guardrails AI | 구조/정책 validator 실행 |
| 공통 관측 | OpenTelemetry + audit sink | request-id 기반 추적 |
...
## 운영 준비도(Production Readiness) 표

| 항목 | Pass 기준 |
| --- | --- |
| 정책 버전 관리 | 모든 guardrail 정책이 Git 태그와 매핑됨 |
| 회귀 테스트 | jailbreak/PII/moderation/grounding 세트 자동 실행 |
| 장애 모드 | fail-open/fail-closed 리허설 완료 |
| 감사 추적 | request-id로 전 단계 재구성 가능 |
| 비용 통제 | 일일/월간 예산 초과 자동 제어 |
...
## 운영 전환 체크포인트: Shadow에서 Full Rollout까지

가드레일 시스템을 바로 100% 적용하면 정상 트래픽까지 함께 막을 수 있습니다. 그래서 단계별 통과 기준을 명시해야 합니다.

| 단계 | 기간 | 통과 기준 |
| --- | --- | --- |
| Shadow | 7일 | 예측 차단율 기록, FP율 추정 완료 |
| Canary 5% | 3일 | SLO 위반 없음, 사용자 불만 급증 없음 |
| Canary 50% | 3일 | guardrail 지연 P95 목표 충족 |
| Full | 7일 집중 관찰 | 인시던트 없음, 회귀 지표 안정 |
...
- 정책 변경이 코드 배포 없이 가능한가
- 장애 시 fail-open/fail-closed 경로가 문서와 일치하는가
- request-id 하나로 전 단계 이벤트를 재구성할 수 있는가
- 비용 상한이 모델 선택 정책과 연결되어 있는가

이 질문에 즉시 답하지 못하면 운영 준비가 끝난 상태가 아닙니다.
...
- [이 글의 예제 코드 (book-examples)](https://github.com/yeongseon-books/book-examples/tree/main/ai-safety-guardrails-101/ko/10-production-guardrail-system)
