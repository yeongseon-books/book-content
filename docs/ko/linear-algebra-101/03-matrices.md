...
**핵심 질문**: *행렬* 은 *숫자의 사각형* 일까요, *변환의 표현* 일까요?

> *행렬은 *벡터를 벡터로 보내는 함수* 의 *압축된 형태* 다.*
...
- **행렬**: `m x n` *수의 배열*.
- **전치**: *행과 열을 바꿈* — `A^T`.
- **항등행렬 I**: *대각이 1*, 나머지 0 — `I v = v`.
- **역행렬**: `A A^-1 = I` — *정방행렬* 에만 가능, 항상 존재 X.
- **행렬 곱**: `(m x k)(k x n) = (m x n)` — *내부 차원 일치 필요*.
...
- *역행렬은 거의 안 쓴다* — *해 풀이는 분해* 사용.
- *형상* 을 항상 본다.
- *비가환성* 을 의식한다.
- *수치 안정성* 을 위해 *전용 함수* 를 쓴다.
- *행렬의 의미* 를 *그림* 으로 본다.
...
1. *2x2 행렬* `A` 의 *전치* 와 *역행렬* 을 손으로 계산하세요.
2. *항등행렬* `I` 를 *임의의 벡터* 와 곱해 *결과가 같은지* 확인하세요.
3. *특이행렬* 의 예를 만들어 *역행렬이 없는 이유* 를 적으세요.
