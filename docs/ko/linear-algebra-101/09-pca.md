...
**핵심 질문**: *고차원 데이터* 의 *가장 중요한 방향* 을 어떻게 찾을까요?

> *PCA는 *분산이 가장 큰 축* 을 찾아 *데이터를 그 축에 투영* 한다.*
...
- **PC (주성분)**: *분산이 큰 순서* 의 *직교 축*.
- **분산 설명률**: 각 PC가 설명하는 *분산의 비율*.
- **공분산 행렬**: `(1/n) X^T X` (centered).
- **SVD 기반 PCA**: `X = U S V^T` — 열 V가 PC.
- **재구성 오차**: 저차원 투영 후 *복원 오차*.
...
1. *iris 데이터* 에 *PCA* 를 적용해 *2D 시각화* 하세요.
2. *분산 설명률 누적 90%* 가 되는 *k* 를 찾으세요.
3. *PCA* 와 *피처 선택* 의 *차이* 를 적으세요.
