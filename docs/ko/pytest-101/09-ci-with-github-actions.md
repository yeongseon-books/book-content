...
**핵심 질문**: PR을 올릴 때마다 수동으로 테스트를 실행해야 하나요?

> GitHub Actions는 push나 PR 이벤트에 반응하여 테스트를 자동 실행합니다. 이 글에서는 pytest + coverage를 GitHub Actions에서 설정하고, 결과를 PR에 표시하는 방법을 배웁니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- GitHub Actions 워크플로우 파일의 구조와 문법
- pytest와 커버리지를 CI에서 실행하는 방법
- 매트릭스 빌드로 여러 Python 버전을 동시에 테스트하는 방법
- 캐싱으로 CI 실행 시간을 줄이는 방법
...
- **Matrix** — OS·Python 버전 매트릭스를 의도적으로 좁힙니다.
- **캐시** — pip/uv 캐시로 시간을 절약합니다.
- **실패 빠르게** — fail-fast 정책으로 비용을 줄입니다.
- **아티팩트** — 리포트·로그를 아티팩트로 보존합니다.
- **보안** — secret은 environment 단위로 분리합니다.
...
1. 워크플로우에 lint job(flake8 또는 ruff)을 추가하고, 테스트 job과 병렬로 실행하세요.
2. `fail-fast: false`를 설정하여 하나의 Python 버전이 실패해도 나머지가 계속 실행되도록 하세요.
3. 워크플로우 상태 배지를 README.md에 추가하세요.
