...
**핵심 질문**: 테스트에서 데이터베이스나 외부 API를 실제로 호출하지 않으려면 어떻게 해야 하나요?

> mock은 외부 의존성을 가짜 객체로 대체하여 테스트를 격리합니다. monkeypatch는 함수, 속성, 환경변수를 테스트 범위 내에서 임시로 바꿉니다. 이 글에서는 두 도구의 차이와 사용 패턴을 배웁니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- mock과 monkeypatch의 차이와 선택 기준
- `unittest.mock.patch`로 함수를 대체하는 방법
- `monkeypatch.setattr`로 속성을 교체하는 방법
- mock의 호출 검증(assert_called_with) 패턴
...
- **경계 모킹** — 외부 경계만 모킹하고 도메인은 실제로 둡니다.
- **monkeypatch** — 전역 상태 변경은 monkeypatch로 자동 복구합니다.
- **Spec 강제** — spec=Class로 잘못된 호출을 잡습니다.
- **Verify** — 호출 인자·횟수까지 검증합니다.
- **과모킹 위험** — 모킹이 늘면 테스트 신뢰도가 떨어집니다.
...
1. `requests.get`을 mock하여 HTTP 200 응답과 404 응답을 각각 테스트하세요.
2. monkeypatch로 `datetime.now()`를 고정된 시간으로 교체하는 테스트를 작성하세요.
3. `spec=True` 옵션을 사용하여 mock 객체에 존재하지 않는 메서드를 호출하면 에러가 발생하는지 확인하세요.
