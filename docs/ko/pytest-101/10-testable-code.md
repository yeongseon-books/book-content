...
**핵심 질문**: 왜 어떤 코드는 테스트하기 쉽고, 어떤 코드는 mock이 10개나 필요한가요?

> 테스트하기 어려운 코드는 대부분 설계 문제입니다. 의존성 주입, 순수 함수, 관심사 분리 원칙을 적용하면 mock 없이도 테스트할 수 있는 코드를 만들 수 있습니다. 이 글에서는 테스트 용이성을 높이는 설계 원칙을 배웁니다.
...
- 테스트하기 어려운 코드의 공통 패턴
- 의존성 주입으로 외부 의존성을 분리하는 방법
- 순수 함수로 비즈니스 로직을 격리하는 방법
- 관심사 분리로 테스트 범위를 좁히는 방법
...
- **순수 함수** — 테스트 가능성의 가장 큰 레버리지입니다.
- **의존성 주입** — 외부 의존을 인자로 받습니다.
- **작은 단위** — 함수·클래스를 작게 유지합니다.
- **Side effect 격리** — I/O는 외곽으로 밀어냅니다.
- **관측 훅** — 로깅·메트릭이 자연스럽게 들어갈 곳을 둡니다.
...
1. DB에 직접 접속하는 함수를 Repository 패턴으로 리팩터링하고, Fake Repository로 테스트하세요.
2. `datetime.now()`에 의존하는 함수를 시간 파라미터를 받도록 변경하고, 고정 시간으로 테스트하세요.
3. 파일 읽기 → 데이터 처리 → 파일 쓰기를 하는 함수를 3개의 순수 함수로 분리하고 각각 테스트하세요.
