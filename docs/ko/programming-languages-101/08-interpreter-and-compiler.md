...
**핵심 질문**: Python은 "인터프리터 언어"라고 흔히 말하는데, 그러면 `.pyc` 파일은 도대체 무엇인가요?

> 인터프리터와 컴파일러는 본질적으로 다른 종(種)이 아니라, **언제 번역하느냐**가 다른 두 전략입니다. 컴파일러는 미리 변환해 두고, 인터프리터는 그때그때 변환합니다. JIT는 그 둘의 가운데에서 가장 자주 도는 코드만 실시간으로 번역합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 인터프리터와 컴파일러를 한 줄로 구분하는 법
- Python의 실행 모델: source → bytecode → VM
- `dis` 모듈로 바이트코드를 읽는 법
- AOT 컴파일과 JIT 컴파일의 차이
- "파이썬은 느리다"의 진짜 의미와 한계
...
- **Compiler**: 원시 코드를 다른 형태(바이트코드, 기계어)로 미리 변환.
- **Interpreter**: 원시 코드(또는 바이트코드)를 실행 시점에 한 줄씩 처리.
- **AOT (Ahead-Of-Time)**: 실행 전에 통째로 컴파일.
- **JIT (Just-In-Time)**: 실행 중 자주 도는 부분만 즉시 컴파일.
- **Bytecode**: 사람이 읽을 수 있지만 기계어보다 추상적인 중간 표현.
...
- 성능 얘기 전에 `dis`로 명령 수부터 확인합니다.
- 인터프리터/JIT/AOT를 도구로 보고, 적합도로 고릅니다.
- "Python이 느리다"가 아니라 "이 함수가 VM에서 N개의 명령으로 돈다"라고 말합니다.
- JIT의 워밍업 비용을 잊지 않습니다.
- 핫 루프를 라이브러리(C 구현)로 떠넘기는 길이 가장 빠른 답일 때가 많다는 걸 압니다.
...
1. 위의 `slow`와 `fast`를 큰 입력에서 측정해 시간 차이를 직접 보고, `dis` 결과와 연결해 한 문단으로 설명하세요.
2. PyPy(또는 Codon, Cython)에서 같은 `slow`를 돌려 시간 차이를 측정해 보세요.
3. 자주 쓰는 모듈 한 개를 골라 `python -m compileall .`로 `.pyc`를 만들고, 임포트 시간이 어떻게 변하는지 측정해 보세요.
