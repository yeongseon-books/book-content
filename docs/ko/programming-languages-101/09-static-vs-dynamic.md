...
**핵심 질문**: 정적 타입은 정말 동적 타입보다 "안전"할까요 — 그리고 그 안전은 어디까지를 말하는 걸까요?

> 정적과 동적은 좋고 나쁨의 문제가 아니라, **검증을 컴파일 시점에 할지 실행 시점에 할지**를 정하는 선택입니다. 둘 다 같은 종류의 오류를 잡지만, 잡는 시점과 비용이 다릅니다. 어느 한쪽도 모든 버그를 막아 주지는 않습니다.
...
- 정적/동적의 정의를 한 줄로 정리
- 같은 함수의 두 버전(타입 힌트 있음/없음)에서 보이는 차이
- mypy/pyright가 잡는 오류와 못 잡는 오류
- 점진적 타이핑(gradual typing)이 무엇을 가능하게 하는지
- "정적은 안전, 동적은 빠른 개발"이라는 신화의 한계
...
- **Static typing**: 변수와 식의 타입이 컴파일 시점에 결정·검증됨.
- **Dynamic typing**: 타입이 값에 묶여 있고, 검증은 실행 시점에 일어남.
- **Strong vs weak**: 묵시적 변환을 얼마나 허용하느냐(예: `1 + "1"`).
- **Gradual typing**: 한 코드베이스에서 정적 영역과 동적 영역을 섞어 쓰는 모델 (Python, TypeScript).
- **Soundness**: 타입 검사기가 통과시킨 코드는 그 타입대로만 동작한다는 보장.
...
- "이 코드의 검증 시점은 어디인가?"를 먼저 묻습니다.
- 외부 입력에는 항상 경계 검증을 둡니다.
- 점진적 타이핑을 활용하되 `Any`의 비용을 의식합니다.
- 타입과 테스트를 보완재로 봅니다.
- 동적 언어가 더 적합한 자리(짧은 스크립트, 노트북)와 정적이 더 적합한 자리(공유 라이브러리, 장기 서비스)를 구분합니다.
...
1. 가장 최근에 짠 함수에 타입 힌트를 달고 mypy를 돌려, 어떤 오류가 잡히는지 보세요. 한 문단으로 정리하세요.
2. JSON을 받는 엔드포인트 한 곳에 `pydantic` 모델을 적용하고, "전"과 "후"의 오류 메시지 차이를 비교하세요.
3. 위 4단계의 `Protocol`을 ABC 상속(`abc.ABC`)으로 바꿔 보고, 어떤 차이가 생기는지 한 문단으로 적으세요.
