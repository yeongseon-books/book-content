...
**핵심 질문**: 같은 변수명이 함수 안과 밖에서 다른 값을 가리키는 일이 어떻게 일관되게 정해질까요?

> 모든 언어는 "이 이름이 지금 이 자리에서 어떤 값을 가리키는가"를 결정해야 합니다. 그 결정 규칙을 **scope**라 부르고, 이름에 값을 묶는 행위를 **binding**이라 부릅니다. 거의 모든 현대 언어가 lexical scope를 택하지만, 그 선택의 의미와 한계는 의외로 자주 잊힙니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- scope와 binding의 정확한 의미
- lexical vs dynamic scope의 차이
- 변수 가림(shadowing)의 의도와 위험
- Python의 LEGB 규칙
- closure를 이해하기 위한 토대(다음 글로 이어짐)
...
- **Binding**: 이름에 값을 연결하는 일.
- **Scope**: 그 binding이 보이는 코드 영역.
- **Lexical scope**: 코드 텍스트 위치로 결정되는 scope.
- **Dynamic scope**: 호출 흐름으로 결정되는 scope(현대 언어에서 드묾).
- **Shadowing**: 안쪽 scope에서 같은 이름이 바깥의 binding을 가림.
...
- "이 이름이 어디서 묶였지?"를 곧바로 답할 수 있어야 좋은 코드라 봅니다.
- 의존을 인자로 끌어내는 리팩터링을 자주 합니다.
- `nonlocal`/`global`을 쓸 때는 그 이유를 짧은 주석으로 남깁니다.
- 내장 이름 가림을 린터가 잡도록 설정합니다.
- 새 언어를 만나면 scope 규칙을 가장 먼저 확인합니다 — 거기서 함수와 클로저의 동작이 다 결정됩니다.
...
1. 위 실습 5단계의 `total` 함수에서 `sum` shadowing을 고치고, 같은 함수 안에서 내장 `sum(...)`도 함께 쓸 수 있게 만들어 보세요.
2. 모듈 전역 변수 한 개에 의존하는 함수를 골라, 그 의존을 인자로 끌어낸 버전과 비교해 보세요. 단위 테스트는 어느 쪽이 더 쉬워졌나요?
3. lexical과 dynamic scope의 차이가 결과를 바꿀 수 있는 가상의 코드 한 조각을 직접 만들어 보세요.
