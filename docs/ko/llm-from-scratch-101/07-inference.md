...
생성 루프를 이렇게 분해해서 보면, 같은 베이스 모델 위에 어떤 디코딩 전략을 얹어도 어디가 결정론적이고 어디가 확률적인지를 일관되게 설명할 수 있습니다.
...
이 방법은 완전한 해결책은 아니지만, 소형 모델의 단조 반복을 완화하는 데 자주 도움이 됩니다. 중요한 점은 패널티 강도를 너무 크게 주면 출력 유창성이 급격히 떨어질 수 있다는 사실입니다.
...
- **생성 루프는 정확히 무엇을 반복할까요?**
  - 생성은 `idx_cond = idx[:, -self.config.block_size :]`로 현재 문맥을 자르고, 마지막 위치 logits를 꺼내 새 토큰 하나를 뽑아 `torch.cat((idx, idx_next), dim=1)`로 다시 붙이는 일을 반복합니다. 즉, 전체 문장을 한 번에 쓰는 것이 아니라 다음 토큰 분포에서 한 조각씩 이어 붙이는 자기회귀 루프입니다.
- **greedy decoding은 왜 자주 지루하고 반복적인 출력을 만들까요?**
  - greedy는 매 step마다 `argmax`에 해당하는 최고 확률 토큰만 고르므로, 모델이 조금만 보수적이어도 같은 패턴을 계속 강화합니다. 그래서 `ROMEO:` 프롬프트에서도 안전한 문자 연쇄는 잘 나오지만, 다양성은 쉽게 사라지고 반복이 늘어납니다.
- **temperature는 logits 분포를 어떻게 바꿀까요?**
  - 코드에서 `logits = logits[:, -1, :] / max(temperature, 1e-5)`로 나눈 뒤 softmax를 취하면, `T < 1`에서는 큰 logits가 더 두드러지고 `T > 1`에서는 분포가 평평해집니다. 즉 temperature는 모델 가중치를 바꾸지 않고도 출력의 보수성·무작위성을 조절하는 손잡이입니다.
...
- [LLM 앱 기초 — 스트리밍 응답 처리](../llm-app-foundations-101/06-streaming-responses.md)
