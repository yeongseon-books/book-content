...
- **학습 루프를 움직이는 핵심 다섯 줄은 무엇일까요?**
  - 이 글의 핵심 루프는 `optimizer.zero_grad(set_to_none=True)`, `model(xb, yb)`, `loss.backward()`, `clip_grad_norm_`, `optimizer.step()` 다섯 줄입니다. 나머지 `estimate_loss()`, lr 스케줄, `torch.save(...)`는 이 반복을 더 안정적이고 재현 가능하게 감싸는 운영 장치였습니다.
- **transformer 학습에서 AdamW는 왜 SGD보다 다루기 쉬울까요?**
  - AdamW는 파라미터별 업데이트 크기를 자동 조정하고 momentum 성격도 함께 가져가므로, 작은 GPT에서도 SGD보다 훨씬 덜 예민하게 학습이 진행됩니다. 본문이 `torch.optim.AdamW(..., lr=3e-4, weight_decay=0.1, betas=(0.9, 0.95))`를 기본값으로 둔 이유가 바로 그 실용성입니다.
- **warmup과 cosine decay는 학습 안정성에 어떤 도움을 줄까요?**
  - `get_lr()`에서 초반 100 step은 선형 warmup으로 보폭을 천천히 키우고, 이후에는 cosine 곡선으로 다시 줄여 초기 흔들림과 후반 진동을 완화했습니다. 그래서 loss가 4점대에서 내려갈 때 과한 첫 스텝이나 마지막 과적합성 흔들림을 덜 만들 수 있습니다.
...
- [LLM API 프로덕션 101 — 재시도와 오류 처리](../llm-api-production-101/05-retry-and-error-handling.md)
- [AI Agent 101 — Agent 평가](../ai-agent-101/07-agent-evaluation.md)
- [LangGraph 101 — 상태와 체크포인트](../langgraph-101/02-state-and-checkpoints.md)
