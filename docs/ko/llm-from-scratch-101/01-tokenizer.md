...
문자 단위 토큰화의 장점은 구현이 짧고 디버깅이 쉽다는 사실입니다. 입력 문자열을 구성하는 문자 집합을 모은 뒤, 각 문자에 정수를 붙이면 됩니다. 다음 코드는 그 핵심을 가장 직접적으로 보여 줍니다.
...
- **모델은 왜 문자열 대신 정수 시퀀스를 입력으로 받아야 할까요?**
  - 신경망은 행렬 곱과 텐서 연산기로 동작하므로 문자열 자체에는 연산을 걸 수 없고, 먼저 `encode()`가 만든 정수 배열이 필요합니다. 이 글의 `stoi`, `itos`, `train.bin`, `val.bin`이 바로 사람이 읽는 텍스트를 모델이 읽을 수 있는 숫자 계약으로 바꾼 결과입니다.
- **문자 단위, 단어 단위, 서브워드 토큰화는 각각 무엇을 얻고 무엇을 잃을까요?**
  - 문자 단위는 vocab이 작고 `decode()`로 추적하기 쉽지만 시퀀스가 길어지고, 단어 단위는 짧지만 OOV와 어휘 폭발이 커집니다. 서브워드는 그 중간에서 길이와 어휘 크기를 절충하므로, 이번 시리즈는 성능보다 투명성을 위해 char-level을 택했습니다.
- **BPE는 실제로 어떤 식으로 어휘를 조금씩 키워 갈까요?**
  - BPE는 처음에 문자 조각으로 시작한 뒤 `l + o`, `lo + w`처럼 자주 붙어 나오는 쌍을 반복적으로 병합해 새 토큰으로 승격합니다. 그래서 `tiktoken.get_encoding("gpt2")` 같은 실전 토크나이저는 자주 등장하는 패턴을 더 짧은 ID 시퀀스로 표현할 수 있습니다.
...
- [LLM 앱 기초 — 토큰 이해하기](../llm-app-foundations-101/02-understanding-tokens.md)
- [Vector Search 101 — 임베딩이 필요한 이유](../vector-search-101/01-what-is-embedding.md)
- [LangChain 101 — Prompt와 LLM Chain](../langchain-101/02-prompt-llm-chain.md)
