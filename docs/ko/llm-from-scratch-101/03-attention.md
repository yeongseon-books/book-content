...
자기회귀 모델에서는 여기에 한 가지 규율이 더 붙습니다. 미래 토큰을 보면 안 된다는 사실입니다. 그래서 causal mask가 필수이고, 이 마스크가 있어야 언어 모델은 다음 토큰 예측이라는 학습 규칙을 지킬 수 있습니다.
...
### 단계 1. 단일 head로 점수 행렬을 먼저 눈으로 확인합니다
...
### 단계 2. mask를 빼 보면 무엇이 잘못되는지 바로 확인할 수 있습니다
...
### 단계 3. multi-head는 여러 관점의 관계를 병렬로 학습하게 합니다
...
### 단계 4. `nn.Linear`와 reshape만으로 시리즈용 어텐션을 완성할 수 있습니다
...
### 단계 5. head별 가중치를 직접 저장해 두면 다음 글 디버깅이 쉬워집니다
...
## 어텐션 디버깅: 숫자가 맞아도 품질이 틀릴 수 있는 이유

어텐션 구현은 shape만 맞으면 끝난다고 느끼기 쉽습니다. 하지만 실제로는 마스킹 위치, 스케일링, 소프트맥스 차원, 드롭아웃 적용 위치가 조금만 어긋나도 모델 출력이 조용히 망가집니다. 에러가 발생하지 않아 더 위험합니다.

### 디버깅 순서

첫째, `B, T, C` 흐름에서 각 연산 직후 tensor shape를 로그로 고정합니다. 둘째, causal mask가 미래 토큰을 완전히 가리는지 단위 테스트로 확인합니다. 셋째, 단일 배치에서 attention score의 분포를 출력해 극단값이 발생하는지 점검합니다. 넷째, 학습 초기에 head별 attention entropy를 비교해 특정 head가 죽는지 확인합니다.

### 실무에서 자주 보는 실패 패턴

- mask를 `float('-inf')` 대신 0으로 채워 미래 토큰이 새어 들어감
- softmax 차원을 마지막이 아닌 다른 축에 적용해 의미가 틀어짐
- scale(`1/sqrt(d_k)`) 누락으로 logits 폭이 과도하게 커짐
- train/eval 모드 전환 누락으로 드롭아웃이 추론에도 적용됨

이 네 가지는 코드 리뷰에서 자주 빠집니다. 그래서 어텐션은 "이론 이해"보다 "검증 루틴 자동화"가 더 중요합니다.
...
- **Q, K, V는 왜 같은 입력에서 나오지만 서로 다른 역할을 가질까요?**
  - 셋 다 `(B, T, C)` 입력에서 선형층으로 나오지만, `query`는 지금 무엇을 찾는지, `key`는 어떤 단서를 제공하는지, `value`는 실제 가져갈 내용을 표현하도록 학습됩니다. 그래서 코드에서는 같은 `x`에서 시작해도 `self.query(x)`, `self.key(x)`, `self.value(x)`가 서로 다른 역할을 맡습니다.
- **어텐션 점수는 왜 `Q · K^T / sqrt(d)` 형태로 계산할까요?**
  - `q @ k.transpose(-2, -1)`는 각 토큰이 어떤 토큰과 잘 맞는지 내적으로 점수화하고, `/ math.sqrt(k.size(-1))`는 차원이 커질 때 softmax가 너무 뾰족해지는 것을 막습니다. 즉, 앞부분은 유사도 계산이고 뒷부분은 학습 안정화를 위한 스케일 조정입니다.
- **causal mask가 없으면 자기회귀 학습에서 정확히 무엇이 망가질까요?**
  - `wei.masked_fill(self.tril[:t, :t] == 0, float("-inf"))`가 없으면 현재 위치가 미래 토큰까지 보고 정답을 맞히게 됩니다. 그러면 학습 loss는 쉽게 내려가도 추론 시점에는 미래 정답이 없어서, 훈련 때만 잘하는 잘못된 언어 모델이 됩니다.
...
- [LangGraph 101 — 상태와 라우팅 설계](../langgraph-101/02-state-and-checkpoints.md)
- [AI Agent 101 — 컨텍스트 엔지니어링](../ai-agent-101/02-context-engineering.md)
- [LLM 앱 기초 — 프롬프트 엔지니어링 기초](../llm-app-foundations-101/03-prompt-engineering-basics.md)
