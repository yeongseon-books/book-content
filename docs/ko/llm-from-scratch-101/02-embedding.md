...
- **`nn.Embedding`은 실제로 어떤 연산을 수행할까요?**
  - `nn.Embedding(vocab_size, n_embd)`는 복잡한 계산기보다 `(vocab_size, n_embd)` 테이블에서 해당 행을 꺼내 오는 학습 가능한 룩업입니다. 본문 `MiniEmbedding`의 `return self.weight[idx]` 한 줄이 바로 그 본질을 보여 줬습니다.
- **토큰 임베딩만으로는 왜 충분하지 않을까요?**
  - 토큰 임베딩만 있으면 같은 문자가 첫 위치에 있든 마지막 위치에 있든 같은 벡터로 보이므로 순서를 잃습니다. 그래서 GPT 입력부는 `tok_emb + pos_emb`를 만들고, 그 덧셈이 있어야 `(B, T, C)` 텐서가 토큰 의미와 위치를 함께 담게 됩니다.
- **위치 정보는 왜 별도 임베딩으로 다루는 편이 실용적일까요?**
  - 토큰 의미는 vocab 전체에서 재사용되고 위치는 `block_size` 범위 안에서만 변하므로, 두 정보를 테이블 두 개로 분리하면 구현과 디버깅이 단순해집니다. 본문 `position_embedding_table = nn.Embedding(config.block_size, config.n_embd)`가 learned positional embedding을 별도 관리한 이유가 바로 그것입니다.
