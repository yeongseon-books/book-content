...
- **FeedForward는 왜 `Linear(C, 4C) -> GELU -> Linear(4C, C)` 형태를 많이 쓸까요?**
  - attention이 토큰 사이 관계를 섞은 뒤에는 각 위치 안에서 표현을 더 풍부하게 가공할 MLP가 필요하기 때문입니다. `Linear(C, 4C) -> GELU -> Linear(4C, C)`는 잠깐 차원을 넓혀 더 많은 조합을 만들고 다시 residual stream 크기 `C`로 돌아오게 하는 전형적 형태입니다.
- **residual connection은 학습을 어떻게 안정화할까요?**
  - `x = x + self.attn(self.ln1(x))`와 `x = x + self.ffn(self.ln2(x))`처럼 원래 입력 경로를 남겨 두면 정보와 gradient가 깊은 층에서도 끊기지 않습니다. 그래서 블록이 여러 개 쌓여도 최소한 이전 표현을 보존한 채 조금씩 수정하는 방향으로 학습이 진행됩니다.
- **pre-norm과 post-norm은 실전에서 어떤 차이를 만들까요?**
  - 이 시리즈가 쓴 pre-norm은 sub-layer 앞에서 `LayerNorm`을 적용해 깊은 GPT에서도 활성값과 gradient를 더 안정적으로 유지하게 합니다. 코드상 한 줄 차이처럼 보여도, `self.ln1(x)`와 `self.ln2(x)`를 먼저 거치게 한 설계가 실제 학습 가능성을 크게 좌우합니다.
...
- [AI Agent 101 — Agent Workflow 설계](../ai-agent-101/04-agent-workflow-design.md)
- [LLM 앱 기초 — 대화 상태 관리](../llm-app-foundations-101/05-conversation-state.md)
