...
- **pre-training, fine-tuning, RLHF는 각각 무엇을 바꾸는 단계일까요?**
  - pre-training은 다음 토큰 예측 능력 자체를 만들고, SFT는 `Q: ...\nA: ...` 형식처럼 출력 습관을 바꾸며, RLHF는 그 위에 사람 선호 신호를 더해 응답 정책을 조정합니다. 이 글은 그중 `ckpt.pt`를 불러와 `ckpt_sft.pt`로 저장하는 SFT 구간에 집중했습니다.
- **instruction 데이터 한 줄은 어떤 필드 구조를 가지면 충분할까요?**
  - 이 시리즈에서는 `{"instruction": ..., "response": ...}` 두 필드만 있으면 충분했습니다. `build_example()`가 이를 `Q: {instruction}\nA: {response}`로 직렬화하고 `encode()`로 숫자화하므로, 복잡한 스키마보다 일관된 템플릿이 더 중요했습니다.
- **작은 데이터셋 50개만으로도 출력 습관이 왜 바뀔 수 있을까요?**
  - 베이스 모델이 이미 문자와 문장 리듬을 알고 있으므로, 50개 예시만 반복해도 `Q:` 뒤 질문, `A:` 뒤 답변이 온다는 패턴은 충분히 강화될 수 있습니다. 본문의 base vs sft 비교에서 셰익스피어풍 continuation이 `A: My lord, I serve thee...` 같은 응답형 출력으로 바뀐 것이 바로 그 효과였습니다.
...
- [LLM API 프로덕션 101 — 구조화 출력](../llm-api-production-101/01-structured-output.md)
