...
- **Tim sort 활용** — 파이썬 sorted/list.sort를 이기려는 시도는 거의 손해입니다.
- **Stable 여부** — 다중 키 정렬에서 stable 속성이 결정적입니다.
- **Key 함수** — 비교 함수보다 key 함수가 빠르고 명료합니다.
- **외부 정렬** — 메모리에 안 들어가면 외부 정렬·청크 전략을 검토합니다.
- **측정** — 이론보다 실측이 정렬 선택의 마지막 결정자입니다.
