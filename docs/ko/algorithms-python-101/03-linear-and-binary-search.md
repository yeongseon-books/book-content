...
**핵심 질문**: 100만 개의 정렬된 데이터에서 값을 찾을 때 최대 몇 번 비교하면 될까요?

> 선형 탐색은 처음부터 끝까지 하나씩 확인하므로 O(n)입니다. 이진 탐색은 정렬된 데이터에서 매번 반을 버려 O(log n)에 찾습니다. 이 글에서는 두 탐색 알고리즘을 구현하고, Python의 bisect 모듈까지 다룹니다.
...
- **정렬 비용** — 이진 탐색의 이득은 정렬 비용을 갚을 만큼 잦은 검색일 때 나옵니다.
- **경계 조건** — off-by-one을 피하려면 [low, high) 같은 일관된 규약을 씁니다.
- **표준 모듈** — bisect 모듈을 우선 활용합니다.
- **불변식** — 루프 불변식을 명시하면 버그가 줄어듭니다.
- **회귀 테스트** — 엣지 케이스를 단위 테스트로 고정합니다.
...
1. 정렬된 리스트에서 target 이상인 값의 개수를 O(log n)에 구하는 함수를 작성하세요.
2. 재귀 방식으로 이진 탐색을 구현하세요.
3. 정수 N의 제곱근을 이진 탐색으로 구하세요 (소수점 이하 6자리까지).
