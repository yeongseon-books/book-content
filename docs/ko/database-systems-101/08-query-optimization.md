...
**핵심 질문**: 똑같은 SQL이 어떤 날은 1ms, 어떤 날은 10초인 이유는 무엇이고, 우리는 무엇을 봐야 할까요?

> 옵티마이저는 통계로 비용을 추정해 가능한 실행 계획 중 가장 싸 보이는 것을 고르는 코드입니다. 통계가 낡거나 데이터 분포가 급변하면 옵티마이저는 잘못된 길을 갑니다. EXPLAIN을 읽는 능력은 곧 그 결정을 사후에 검증하는 능력입니다.
...
- 통계(statistics)가 왜 결정적으로 중요한가
- EXPLAIN/EXPLAIN ANALYZE를 읽는 방법
- 흔한 튜닝 단서 4가지
...
- **Optimizer**: 가능한 계획들 중 가장 싼 것을 고르는 모듈.
- **Statistics**: 컬럼 값 분포, 행 수, 인덱스 선택성 같은 메타 정보.
- **Cardinality 추정**: 어떤 단계에서 몇 행이 나올지 옵티마이저가 추정하는 값.
- **Plan node**: Seq Scan, Index Scan, Hash Join, Nested Loop, Sort, Aggregate 등.
- **EXPLAIN ANALYZE**: 실제 실행한 결과를 함께 보여 주는 명령.
...
- 새 쿼리는 EXPLAIN ANALYZE로 검증하고 머지합니다.
- 추정과 실제 차이가 10배가 넘으면 통계나 데이터 분포를 의심합니다.
- 인덱스 추가 PR에는 "이 쿼리에서 사용된다"는 코멘트를 답니다.
- 옵티마이저 힌트는 마지막 수단입니다. 모델·인덱스·통계가 먼저입니다.
- "지금 빠르다"가 "내일도 빠르다"를 보장하지 않습니다. 모니터링과 알람으로 지킵니다.
...
1. EXPLAIN ANALYZE 결과에서 `rows=10` 추정이지만 `actual rows=10000`입니다. 무엇을 가장 먼저 의심해야 할까요?
2. `SELECT *` 대신 필요한 컬럼만 명시했을 때 옵티마이저가 활용할 수 있는 최적화 한 가지를 적어 보세요.
3. 같은 데이터에서 `WHERE id IN (1,2,3)`과 `WHERE id=1 OR id=2 OR id=3`이 다르게 동작할 수 있는 이유를 한 줄로 설명하세요.
