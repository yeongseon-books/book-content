...
**핵심 질문**: 책 뒷장의 색인이 왜 그렇게 빠른지, 그 직관이 어째서 데이터베이스 인덱스에도 그대로 통할까요?

> 인덱스는 "값 → 행 위치"를 미리 정렬해 둔 별도 자료 구조입니다. 그 덕에 풀스캔이 O(N)인 자리에서 인덱스 조회는 O(log N)이 됩니다. 다만 인덱스도 공짜는 아닙니다. 쓰기 비용과 디스크가 듭니다. 좋은 인덱스 설계는 "어디에 만들지"보다 "어디에 만들지 않을지"를 정하는 일입니다.
...
- B-tree 인덱스의 직관과 한계
- 단일·복합·커버링 인덱스의 차이
- 선택성(selectivity)이 인덱스 효과를 결정한다는 사실
- 인덱스의 비용과 안티패턴
...
- **B-tree index**: 가장 흔한 인덱스. 정렬된 키와 포인터로 균형 잡힌 트리.
- **Selectivity(선택성)**: 한 값이 얼마나 많은 행을 가리키는가. 1/1000이면 좋고, 1/2면 사실상 의미 없음.
- **Composite index(복합 인덱스)**: 여러 컬럼을 묶은 인덱스. 컬럼 순서가 중요합니다.
- **Covering index**: 쿼리가 필요한 컬럼을 인덱스 안에서 모두 줄 수 있을 때. 테이블을 거의 안 봅니다.
- **Index-only scan**: 커버링 인덱스로 답을 만들 수 있는 실행 방식.
...
- 인덱스를 만들기 전에 "이 컬럼의 선택성은 얼마인가?"를 묻습니다.
- 쿼리 패턴 → 인덱스, 인덱스 → 쿼리 패턴 양방향으로 검증합니다.
- 인덱스가 안 타는 쿼리를 보면 통계, 컬럼 순서, 함수 호출(`WHERE lower(email) = ...`) 같은 의심 목록을 갖고 있습니다.
- 인덱스를 추가할 때 "이 인덱스를 누가 쓰는가"를 코멘트로 남깁니다.
- 자주 안 쓰이는 인덱스는 정기적으로 정리합니다.
...
1. `is_paid`(true/false) 컬럼만 있는 단일 인덱스를 만들었습니다. 옵티마이저가 거의 사용하지 않는 이유를 한 줄로 설명하세요.
2. 복합 인덱스 `(country, city, age)`가 있을 때, 다음 세 쿼리 각각에서 인덱스가 효과적으로 쓰이는지 적어 보세요. (a) `WHERE country='KR'`, (b) `WHERE city='Seoul'`, (c) `WHERE country='KR' AND city='Seoul'`.
