...
Pod·Deployment·Service 관계를 어떻게 설계해야 운영과 변경이 안전할까요?

이 글은 그 질문에 답하기 위해 Pod·Deployment·Service의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **Pod는 단명이라는 전제로 설계한다** — 어떤 이유로든 Pod는 죽을 수 있으므로 stateless를 기본으로 합니다.
- **Deployment 전략을 의식적으로 고른다** — RollingUpdate의 maxSurge·maxUnavailable이 무중단 여부를 결정합니다.
- **Service 타입은 보안 경계와 함께** — ClusterIP·LoadBalancer 선택이 외부 노출 위험을 좌우합니다.
- **readiness/liveness/startup probe를 모두 둔다** — 셋의 역할이 다르므로 하나로 대체하지 않습니다.
- **리소스 요청·제한을 합리적으로 잡는다** — request 누락은 노이즈 이웃과 OOM 사고를 만듭니다.
...
### 관련 시리즈
- [Azure App Service 101](../azure-app-service-101/02-request-lifecycle.md) — 요청이 앱 인스턴스로 들어가는 흐름과 비교할 때
- [Azure Functions 101](../azure-functions-101/02-triggers-and-bindings.md) — 함수 단위 실행 모델과 비교할 때
