...
첫 클러스터 배포에서 무엇을 빠뜨리지 말아야 이후 운영이 깔끔해질까요?

이 글은 그 질문에 답하기 위해 첫 클러스터와 배포의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **IaC로 클러스터를 만든다** — 포털 클릭은 재현성과 감사 추적을 잃게 합니다.
- **ACR 통합과 권한을 함께 검증한다** — 이미지 풀 실패가 첫 배포의 가장 흔한 사고입니다.
- **kube-context를 환경별로 분리한다** — 잘못된 컨텍스트의 kubectl이 사고의 원인이 됩니다.
- **RBAC을 처음부터 적용한다** — open 상태로 두면 정리할 시점을 놓치기 쉽습니다.
- **배포 매니페스트를 GitOps로 둔다** — 수기 kubectl apply는 환경 드리프트의 주범입니다.
...
### 관련 시리즈
- [Azure App Service 101](../azure-app-service-101/04-first-deploy.md) — 같은 FastAPI 앱을 더 높은 수준의 PaaS에 올리는 흐름과 비교할 때
- [Azure Functions 101](../azure-functions-101/) — 코드 배포 단위가 컨테이너와 어떻게 다른지 비교할 때
