...
**핵심 질문**: 소셜 네트워크, 지도, 의존성 관계를 코드로 어떻게 표현할까요?

> 그래프는 노드(정점)와 간선으로 관계를 표현하는 자료구조입니다. Python에서는 dict로 인접 리스트를, 2차원 list로 인접 행렬을 구현합니다. 이 글에서는 그래프의 표현 방법과 BFS, DFS 탐색을 다룹니다.
...
- **인접 리스트 기본** — 거의 모든 경우 인접 리스트가 우월합니다.
- **dict[set]** — Python에서 가장 자연스러운 표현입니다.
- **가중치 처리** — (이웃, 가중치) 튜플로 단순화합니다.
- **정규화** — 방향성·자기 루프·중복 간선 정책을 명시합니다.
- **라이브러리** — networkx로 실험·시각화를 가속합니다.
...
1. BFS로 두 노드 사이의 최단 경로(간선 수)를 구하는 함수를 작성하세요.
2. DFS로 그래프의 모든 연결 컴포넌트를 찾는 함수를 작성하세요.
3. 방향 그래프에서 순환이 있는지 감지하는 함수를 작성하세요.
