...
**핵심 질문**: 이 문제에 list를 써야 할까요, dict를 써야 할까요, set을 써야 할까요?

> 자료구조 선택은 "어떤 연산을 가장 자주 하는가?"에 달렸습니다. 검색이 잦으면 dict/set, 순서가 중요하면 list, 우선순위가 필요하면 heap입니다. 이 글에서는 시리즈에서 배운 자료구조를 총정리하고, 상황별 선택 기준을 제시합니다.
...
- **연산 분포 분석** — 어떤 연산이 가장 잦은지 먼저 정의합니다.
- **표준 라이브러리 우선** — 직접 구현은 측정 후에만 검토합니다.
- **메모리 vs 시간** — 트레이드오프를 명시적으로 비교합니다.
- **불변성 검토** — 동시성·테스트가 어렵다면 불변 자료구조로 단순화합니다.
- **측정** — 선택은 가설, 결정은 실측입니다.
...
1. 100만 개의 문자열에서 가장 자주 등장하는 상위 10개를 구하는 코드를 작성하세요. 어떤 자료구조 조합이 가장 효율적인지 설명하세요.
2. LRU 캐시를 dict + 이중 연결 리스트(또는 OrderedDict)로 구현하세요.
3. 실시간 주식 가격 스트림에서 최근 5분간의 최고가, 최저가, 평균가를 효율적으로 계산하는 클래스를 설계하세요. 사용할 자료구조와 그 이유를 설명하세요.
