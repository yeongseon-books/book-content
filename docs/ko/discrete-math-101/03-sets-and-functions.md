...
**핵심 질문**: Python의 `set`, 데이터베이스의 테이블, 해시맵 — 이들의 공통된 수학적 기반은 무엇일까요?

> 집합(set)은 서로 구별되는 원소들의 모임이며, 함수(function)는 한 집합에서 다른 집합으로의 명확한 대응 규칙입니다. 자료구조(`set`, `dict`)부터 데이터베이스의 관계대수, 그리고 함수형 프로그래밍의 매핑까지 — 모두 집합과 함수의 직접적인 응용입니다. 이 글에서는 집합 연산, 곱집합, 함수의 종류, 그리고 합성과 역함수까지 살펴봅니다.
...
| 곱집합 | A × B | (a, b) 쌍의 집합 |
| 함수 | f: A → B | A의 각 원소를 B의 한 원소에 대응 |
| 단사 | injective | 서로 다른 입력은 서로 다른 출력 |
...
시니어는 자료구조를 선택할 때 "이게 집합인가, 다중집합인가, 순서가 있는가?"를 먼저 묻습니다. 또한 "이 함수는 단사인가? 역을 만들 수 있는가?"를 검토합니다. 데이터 변환 파이프라인을 설계할 때, 각 단계가 함수로서 잘 정의되는지 — 같은 입력에 같은 출력을 내는지 — 확인합니다. 이는 멱등성(idempotency)과 직결됩니다.
...
1. 집합 {a, b, c}의 멱집합을 직접 나열하고, 원소 개수가 2³ = 8임을 확인하세요.

2. 함수 f(x) = x²이 정의역 ℝ에서는 단사가 아니지만, 정의역 [0, ∞)에서는 단사임을 보이세요.

3. 자신이 사용하는 데이터베이스 테이블 하나를 골라, 어떤 곱집합의 부분집합으로 표현되는지 설명하세요.
