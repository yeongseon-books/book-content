...
**핵심 질문**: `if x > 0 and y < 10` 같은 코드가 실제로 어떻게 평가될까요? 그 평가의 수학적 기반은 무엇일까요?

> 명제 논리(propositional logic)는 참 또는 거짓이 명확한 문장(명제)들과 그것들을 결합하는 연산자(AND, OR, NOT)를 다룹니다. 모든 프로그래밍 언어의 조건문, SQL의 WHERE 절, 회로 설계의 게이트는 명제 논리의 직접적 응용입니다. 이 글에서는 명제, 진리표, 동치 변환, 그리고 술어 논리(predicate logic)까지 살펴봅니다.
...
| 용어 | 기호 | 설명 |
| --- | --- | --- |
| 부정 | ¬P | P가 참이면 거짓, 거짓이면 참 |
| 논리곱 | P ∧ Q | 둘 다 참일 때만 참 |
| 논리합 | P ∨ Q | 하나라도 참이면 참 |
| 함의 | P → Q | P가 참이고 Q가 거짓일 때만 거짓 |
| 동치 | P ↔ Q | 진리값이 같을 때만 참 |
...
시니어는 복잡한 if 문을 보면 "이걸 단순화할 수 있을까?"를 먼저 묻습니다. 진리표를 머릿속에 그리고, 동치 변환으로 절반 이상의 조건을 제거하는 경우가 많습니다. 또한 "이 조건이 항상 참인가, 어떤 경우에만 참인가"를 양화사로 사고합니다 — 엣지 케이스를 놓치지 않는 비결입니다.
...
1. `(P → Q) ∧ (Q → R) → (P → R)`의 진리표를 작성하고, 이것이 항진명제(tautology)임을 확인하세요.

2. 자신이 작성한 코드에서 중첩 if 문을 하나 골라 드모르간 법칙으로 단순화하세요.

3. "∀x ∃y: x + y = 0"과 "∃y ∀x: x + y = 0"이 정수 집합에서 각각 참인지 거짓인지 판단하고 이유를 설명하세요.
