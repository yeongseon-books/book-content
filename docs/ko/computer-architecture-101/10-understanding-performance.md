...
**핵심 질문**: "느리다"는 말은 정확히 무슨 뜻일까요? 무엇이 어디서 얼마나 느려야 그 시스템은 "최적화가 필요하다"고 말할 수 있을까요?

> 성능 문제는 거의 항상 추측 위에서 시작됩니다. "DB가 느린 것 같다", "Python이 느린 것 같다" 같은 말로요. 하지만 시니어 엔지니어는 측정 없이는 한 줄도 고치지 않습니다. 이 마지막 글은 지금까지 배운 모든 것 — CPU, 메모리, 캐시, I/O, 병렬성 — 을 모아 성능을 이해하는 사고 도구를 정리합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 처리량(throughput)과 지연시간(latency)의 차이
- 병목 식별을 위한 USE 방법론 (Utilization, Saturation, Errors)
- 프로파일링의 두 종류 (sampling, instrumentation)
- 측정의 원칙 — 추측 금지, 작게 시작, 단일 변수 통제
...
| 용어 | 설명 |
| --- | --- |
| 지연시간 | 한 작업의 시작부터 완료까지 걸린 시간 |
| 처리량 | 단위 시간당 완료된 작업 수 |
| p99 | 100건 중 가장 느린 1건의 시간 (꼬리 지연) |
| USE | Utilization / Saturation / Errors |
| 샘플링 프로파일러 | 일정 간격으로 콜스택 채취 |
| Instrumentation | 함수 진입/종료에 측정 코드 삽입 |
...
시니어 엔지니어는 성능 문제를 만나면 가장 먼저 "측정 데이터가 있는가?"를 묻습니다. 데이터가 없으면 토론도 시작하지 않습니다. 토론은 이론으로 흘러가고 끝없는 추측 게임이 되기 때문입니다. 작은 벤치마크라도 만들어 와서 같이 보는 문화가 좋은 팀의 특징입니다.

또한 시니어는 "어디서 느린가"보다 "왜 느린가"를 묻습니다. 같은 함수가 느려도 원인이 CPU, 메모리, 디스크, 락 중 무엇이냐에 따라 처방이 완전히 다릅니다. 이 시리즈에서 지금까지 배운 모든 것 — CPU 사이클, 캐시 계층, I/O 비용, 동시성 모델 — 이 다 이 진단의 도구입니다. 컴퓨터 구조를 아는 엔지니어와 모르는 엔지니어의 차이는 바로 여기서 가장 크게 드러납니다.
...
1. 자신이 자주 쓰는 함수 하나를 골라 cProfile로 프로파일링하세요. 가장 시간을 많이 쓰는 함수가 예상과 일치하는지 확인합니다.

2. 임의의 웹 API에 1000번 요청을 보내고 p50, p95, p99, max를 측정하세요. 평균과 p99의 차이가 얼마나 큰지 봅니다.

3. 같은 알고리즘의 두 구현을 단일 변수 통제 벤치마크로 비교하세요. 5회 반복 중앙값과 표준편차를 함께 보고합니다.
