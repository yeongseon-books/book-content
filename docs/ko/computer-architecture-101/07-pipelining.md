...
**핵심 질문**: 한 명령어가 5단계로 처리되는데, 어떻게 평균적으로 한 사이클에 한 명령어가 완료될 수 있나요?

> CPU 파이프라인은 자동차 조립 라인과 같은 발상입니다. 한 명령어의 처리에 여러 단계가 필요하지만, 단계마다 서로 다른 명령어가 동시에 진행되도록 겹쳐 놓으면 처리량이 단계 수만큼 늘어납니다. 다만 이 마법에는 분기, 데이터 의존, 메모리 지연이라는 적이 있고, 분기 예측은 그 적과의 싸움에서 가장 흥미로운 발명입니다.
...
- 파이프라인의 단계와 처리량 개념
- 데이터 해저드, 제어 해저드, 구조적 해저드
- 분기 예측의 동작과 잘못된 예측의 비용
- 파이프라인 친화적 코드 작성 패턴
...
| 용어 | 설명 |
| --- | --- |
| 파이프라인 | 명령어 처리를 단계별로 겹쳐 진행 |
| 해저드 | 파이프라인을 멈추거나 잘못되게 하는 상황 |
| 데이터 해저드 | 이전 명령어 결과를 다음 명령어가 기다림 |
| 제어 해저드 | 분기로 인한 fetch 불확실성 |
| 분기 예측 | 어디로 분기할지 미리 추측 |
| Stall | 파이프라인이 한 사이클 비는 상태 |
...
시니어 엔지니어는 핫 루프 안의 분기를 한 번씩 짚어 봅니다. 그 분기가 거의 늘 같은 방향으로 가면 예측이 잘 맞아 비용이 거의 없지만, 무작위라면 한 번에 10~20 사이클씩 새 나갑니다. 가능하면 데이터를 정렬해 패턴을 만들거나, 산술로 분기를 대체합니다.

또한 시니어는 "branchless가 항상 빠른 것은 아니다"는 점을 기억합니다. 분기 예측이 잘 맞는 케이스에서는 분기 있는 코드가 더 빠를 수 있고, 짧은 분기는 컴파일러가 cmov로 자동 변환하기도 합니다. 측정 없이 도입한 branchless는 가독성만 잃습니다.
...
1. 위의 `count_positive` 함수를 정렬된 데이터와 무작위 데이터에 대해 측정해 차이를 확인하세요. 차이가 가장 크게 나는 데이터 크기를 찾아봅니다.

2. 자신의 `BranchPredictor` 시뮬레이터에 동일한 분기 시퀀스 1000개(균일 분포 vs 80% True)를 입력하고 적중률 차이를 비교하세요.

3. `abs`, `min`, `max` 같은 함수를 분기 없이(branchless) 구현하고 분기 있는 버전과 시간을 비교하세요. 항상 더 빠른지, 어떤 데이터에서 차이가 나는지 분석합니다.
