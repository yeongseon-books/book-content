...
**핵심 질문**: 두 프로세스가 같은 주소 `0x400000`에 다른 값을 갖고 있을 수 있는데, 둘 다 정상 동작합니다. 어떻게 가능한가요?

> 메모리는 주소가 매겨진 거대한 바이트 배열이지만, 프로세스가 보는 메모리는 그 진짜 RAM이 아닙니다. 운영체제와 CPU의 MMU가 함께 만들어 주는 가상 주소 공간입니다. 그 위에 코드, 데이터, 스택, 힙이 정해진 자리에 놓이고, 모든 변수는 결국 그 안의 어느 주소에 살게 됩니다. 이 글은 그 지도를 그립니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- RAM의 주소 모델과 워드 정렬(alignment)
- 가상 메모리와 페이지 개념
- 프로세스의 메모리 레이아웃(코드, 데이터, 힙, 스택)
- 스택과 힙의 차이, 함수 호출의 메모리 모델
...
| 용어 | 설명 |
| --- | --- |
| 주소 공간 | 프로세스가 볼 수 있는 메모리 범위 |
| 가상 주소 | 프로세스가 사용하는 주소 |
| 물리 주소 | 실제 RAM의 주소 |
| MMU | 가상→물리 변환 회로 |
| 페이지 | 메모리 매핑의 최소 단위(보통 4KB) |
| 정렬(alignment) | 자료형 크기에 맞춰 주소를 배치 |
...
시니어 엔지니어는 프로세스의 메모리 사용량을 RSS(Resident Set Size) 단위로 모니터링합니다. 단순히 "메모리를 적게"가 아니라 "워킹셋(자주 접근하는 페이지의 합)을 작게" 만드는 것이 페이지 폴트와 캐시 미스 모두를 줄이는 길임을 알기 때문입니다.

또한 시니어는 자료구조의 메모리 레이아웃을 의식합니다. 같은 데이터라도 AoS(Array of Structs)와 SoA(Struct of Arrays)는 캐시 동작이 완전히 다르며, 순회 패턴에 맞춰 선택해야 합니다. 메모리 구조는 자료구조 선택의 숨은 차원입니다.
...
1. `ctypes.Structure`로 같은 필드(char, int, char, double)를 가진 두 구조체를 만들되, 한쪽은 큰 타입부터, 다른 쪽은 작은 타입부터 선언하세요. `sizeof`로 차이를 비교합니다.

2. 1MB 크기의 배열을 (a) 스택에, (b) 힙에 할당하는 시도를 해 보세요. (파이썬에서는 `numpy.zeros`와 `ctypes.c_int * N` 사용) 어느 쪽이 실패할 가능성이 큰지 관찰합니다.

3. 자신의 OS에서 `ulimit -s`(macOS/Linux) 또는 `sys.getrecursionlimit()`로 스택 크기 한계를 확인하세요. 그 한계가 어떤 재귀 깊이에 해당하는지 측정합니다.
