...
Worker와 Sandbox 제약을 알면 어떤 코드·런타임 사고를 예방할 수 있을까요?

이 글은 그 질문에 답하기 위해 Worker와 Sandbox의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- Worker 프로세스는 어떤 sandbox 안에서 도는가, 그 sandbox는 무엇을 막는가?
- Sandbox 제약은 file system, network, process spawning에서 각각 어떻게 나타나는가?
- 워커가 죽었을 때 healing은 누가 어떤 신호로 시작하는가?
- Always On은 단순 ‘프로세스 keepalive’ 이상의 어떤 의미를 갖는가?
- Sandbox-friendly 코드를 작성하기 위한 첫 번째 룰은 무엇인가?

## 큰 그림 — worker의 두 얼굴

![Windows 프로세스와 Linux 컨테이너의 실행 경계](../../assets/azure-app-service-deep-dive/03/03-01-the-two-worker-models-that-matter.ko.png)

*Windows 프로세스와 Linux 컨테이너의 실행 경계*
...
![w3wp.exe가 App Service sandbox 안에 놓인 구조](../../assets/azure-app-service-deep-dive/03/03-02-windows-w3wp-exe-under-the-app-service-s.ko.png)
...
![컨테이너 안에서 앱이 도는 Linux 실행 경계](../../assets/azure-app-service-deep-dive/03/03-01-linux-the-container-is-the-execution-bou.ko.png)
...
![공유 home 마운트와 로컬 파일 계층의 차이](../../assets/azure-app-service-deep-dive/03/03-02-when-websites-enable-app-service-storage.ko.png)

*공유 home 마운트와 로컬 파일 계층의 차이*
![공유 home 마운트와 로컬 파일 계층의 차이](../../assets/azure-app-service-deep-dive/03/03-05-when-websites-enable-app-service-storage-2.ko.png)
...
![sandbox 제한이 격리와 자원 공정을 지키는 구조](../../assets/azure-app-service-deep-dive/03/03-06-the-sandbox-is-a-security-feature-and-a.ko.png)
...
- **Sandbox 제약을 사전에 안다** — 권한·네트워크·파일 접근 제한이 설계를 결정합니다.
- **Worker는 단명일 수 있다는 전제** — 재시작·이동에 내성 있는 코드를 씁니다.
- **디스크 I/O는 비용·성능 단서** — 임시 디스크 의존을 줄여야 안정성이 올라갑니다.
- **OOM·CPU 제한을 메트릭으로 본다** — 런타임 종료의 흔한 원인입니다.
- **로컬 캐시 의존을 줄인다** — 스케일·재시작 시 일관성 문제로 이어집니다.

## 운영 체크리스트

- [ ] sandbox에서 금지된 호출(자식 프로세스, raw socket 등)을 코드 리뷰 룰에 넣었다
- [ ] 워커 healing 동작이 SLO에 반영되어 있다
- [ ] Always On 사용 여부의 비용/안정성 trade-off를 결정했다
- [ ] Kudu/SCM 접근 권한을 최소화했다
- [ ] 워커 프로세스 메모리/CPU 메트릭 경보를 분리해서 운영한다
...
### 관련 시리즈
- [Azure App Service 101 — Hosting Models](../azure-app-service-101/03-hosting-models.md)
- [Azure Functions Deep Dive — Worker Process](../azure-functions-deep-dive/02-worker-process.md)
