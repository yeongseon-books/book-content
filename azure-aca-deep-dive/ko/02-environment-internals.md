...
flowchart LR
    ...
    ENV1 --> V1[Virtual network boundary A]
    ENV1 --> L1[Log Analytics workspace A]
    ENV1 --> D1[Dapr components A]

    ENV2 --> V2[Virtual network boundary B]
...
```mermaid
flowchart LR
    INTERNET[공용 인터넷] --> EX[External ingress app]
    EX --> INTA[Internal app A]
    EX --> INTB[Internal app B]

    subgraph ENV[하나의 environment]
        EX
        INTA
        INTB
    end
...
```mermaid
flowchart LR
    ENV[Environment] --> DEST[공용 telemetry destination]
    APP[App config] --> DEST
    REV[Revision 상태] --> DEST
    DAPR[daprd sidecar] --> DEST
    ENVOY[Ingress 경로] --> DEST
```
...
```mermaid
flowchart LR
    CFG[Environment config 변경] --> NET[Network boundary 갱신]
    CFG --> LOG[Logging destination binding]
    CFG --> DAPR[Dapr component registry 갱신]
    NET --> APPS[Environment 안의 apps]
    LOG --> APPS
    DAPR --> APPS
```
