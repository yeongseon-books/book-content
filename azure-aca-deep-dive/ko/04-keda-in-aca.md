...
flowchart LR
    ...
    PLATFORM --> KEDAOBJ[관리되는 KEDA ScaledObject류 의도]
    KEDAOBJ --> HPA[KEDA-managed HPA 동작]
    HPA --> REPS[Revision replica 수]
```
...
flowchart TB
    ...
    MIN --> KEDA[숨은 KEDA scaling object]
    MAX --> KEDA
    RULES --> KEDA
```
...
```mermaid
flowchart LR
    SO[ScaledObject 의도] --> OP[KEDA operator reconcile]
    OP --> HPA[HorizontalPodAutoscaler]
    HPA --> SCALE[Replica count 변경]
```
...
stateDiagram-v2
    [*] --> Zero
    Zero --> Activating: trigger 발생
    Activating --> Running: replica ready
    Running --> ScalingOut: 수요 증가
    ScalingOut --> Running
    Running --> Zero: cooldown 후 수요 없음
```
...
sequenceDiagram
    ...
    User->>ACA: custom scale rule 정의
    ACA->>KEDA: rule metadata와 auth 번역
    KEDA->>KEDA: trigger source polling
    KEDA->>HPA: target metric 제공
    HPA->>Pods: scale out / scale in
```
...
```mermaid
flowchart LR
    HTTP[유입 HTTP 요청] --> CONC[관측된 concurrency window]
...
flowchart LR
    ...
    RULEAUTH --> KEDA[숨은 KEDA trigger auth 의도]
```
...
flowchart LR
    ...
    KEDA --> METRICS[External metric 응답]
...
flowchart TB
    ...
    R2[Rule 2] --> ANY
    R3[Rule 3] --> ANY
    ANY -->|Yes| SCALE[Revision scale out]
    ANY -->|No| HOLD[유지 또는 scale to zero]
```
...
flowchart LR
    ...
    ACA --> KEDA[숨은 KEDA scaler objects]
    KEDA --> ADAPTER[Metrics adapter 동작]
    ADAPTER --> HPA[HPA query와 target]
    HPA --> PODS[Revision pod replica]
```
