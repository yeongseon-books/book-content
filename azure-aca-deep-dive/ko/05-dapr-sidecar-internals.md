...
sequenceDiagram
    ...
    API->>WH: pod용 AdmissionReview
    WH->>WH: SidecarConfig 생성
    WH->>Patch: sidecar patch 생성
    Patch->>Pod: daprd container, env, ports, probes 추가
```
...
flowchart TB
    ...
    CFG --> PROBES[readiness / liveness probe]
...
```mermaid
flowchart LR
    APP[사용자 컨테이너] -->|HTTP| H3500[localhost:3500]
...
```mermaid
flowchart LR
    CODE[애플리케이션 코드] --> API[Dapr localhost API]
    API --> DAPRD[daprd]
    DAPRD --> COMP[구성된 component]
    COMP --> TARGET[원격 backing service]
```
...
flowchart LR
    ...
    APPID --> DAPRD[daprd sidecar가 component 로드]
```
...
```mermaid
flowchart LR
    APPCFG[App Dapr enablement] --> SIDECAR[daprd가 pod에 부착]
...
```mermaid
flowchart LR
    APP[애플리케이션] --> BUILD[State / pubsub / invocation API]
    APP --> OPS[Health / metadata API]
    BUILD --> DAPRD[daprd]
    OPS --> DAPRD
```
...
```mermaid
flowchart LR
    APP[사용자 컨테이너] -->|localhost:3500 / 50001| DAPRD[daprd]
...
```mermaid
flowchart LR
    ENABLE[App에서 Dapr enable] --> INJECT[Injector가 pod mutation]
    INJECT --> POD[사용자 컨테이너 + daprd를 가진 pod]
    POD --> BOOT[daprd bootstrap과 runtime config]
    BOOT --> LOAD[scoped environment component 로드]
    LOAD --> LOCAL[localhost HTTP/gRPC API 노출]
    LOCAL --> RUN[앱 요청과 component 호출 처리]
```
