...
```mermaid
flowchart LR
    CLIENT[외부 클라이언트] --> LB[ACA-managed load balancer]
...
```mermaid
flowchart LR
    CLIENT[클라이언트] --> ENVOY[Ingress proxy]
    ENVOY --> HDRS[Forwarded header 추가]
    HDRS --> APP[사용자 컨테이너 handler]
```
...
flowchart LR
    ...
    WEIGHT --> SVCA[Revision A용 Service]
    WEIGHT --> SVCB[Revision B용 Service]
```
...
flowchart LR
    ...
    WC --> C1[Revision A service용 cluster]
    WC --> C2[Revision B service용 cluster]
```
...
```mermaid
flowchart LR
    ENVOY[Envoy] --> SVC[선택된 revision용 Service]
...
```mermaid
flowchart LR
    REV[선택된 revision] --> SVC[Revision service]
...
sequenceDiagram
    ...
    Client->>Envoy: 첫 요청
    Envoy->>Scale: 수요 발생으로 activation 필요
    Scale->>Rev: replica 생성
    Rev->>Rev: 컨테이너 시작과 readiness 통과
    Envoy->>Rev: upstream ready 이후 요청 전달
```
...
```mermaid
flowchart LR
    CALLER[Environment 내부 caller] --> ENVOY[Ingress / routing layer]
    ENVOY --> SVC[Revision service]
    SVC --> POD[Pod replica]
```
...
flowchart LR
    ...
    WEIGHT --> SVC[선택된 revision service]
    SVC --> READY[Ready pod endpoint]
    READY --> APP[사용자 컨테이너]
```
