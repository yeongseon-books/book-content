...
Microsoft가 관리하는 Kubernetes 계층 위에, Revision, Ingress, Autoscaling, Dapr 통합, 로깅, Environment 경계 같은 제품 기능을 얹은 서버리스 컨테이너 플랫폼입니다.
...
- Ingress와 Replica 사이의 Service성 홉
...
![더 단순한 모델: ACA는 여러 계층 위의 제품 표면입니다](../../assets/azure-aca-deep-dive/01/01-02-a-simpler-model-aca-is-a-product-surface.ko.png)
맨 위는 여러분이 선언하는 층입니다.
...
- Traffic split 하나가 Envoy route와 upstream cluster 가중치로 바뀝니다.
...
![Environment는 무엇을 실제로 경계 짓는가](../../assets/azure-aca-deep-dive/01/01-03-what-the-environment-really-is.ko.png)
...
![Container App은 런타임에서 무엇으로 펼쳐지는가](../../assets/azure-aca-deep-dive/01/01-04-what-a-container-app-becomes-at-runtime.ko.png)
...
![Revision이 운영의 중심인 이유](../../assets/azure-aca-deep-dive/01/01-05-revisions-are-the-operational-center-of.ko.png)
여기서 중요한 미묘함 하나.
...
![ScaledObject를 보지 못해도 KEDA를 알아야 하는 이유](../../assets/azure-aca-deep-dive/01/01-06-why-keda-matters-even-if-you-never-see-a.ko.png)
4화가 이 블랙박스를 엽니다.
...
- 앱에 Dapr를 켜면 Pod에 `daprd` 사이드카가 붙습니다.
- 사용자 컨테이너는 localhost로 그 사이드카에 붙습니다.
- 사이드카는 HTTP 3500, gRPC 50001 포트로 Dapr API를 노출합니다.
- Component는 Environment 수준에서 구성되고, Dapr scope에 따라 로드됩니다.

![ACA의 Dapr는 흉내 낸 API가 아닙니다](../../assets/azure-aca-deep-dive/01/01-07-dapr-in-aca-is-not-a-mock-integration.ko.png)
...
ACA의 revision traffic split은 자연스럽게 weighted upstream cluster 선택 문제로 바뀝니다.

![Envoy는 ingress가 실제 라우팅으로 바뀌는 지점입니다](../../assets/azure-aca-deep-dive/01/01-08-envoy-is-where-ingress-becomes-runtime-r.ko.png)
...
![ACA의 control plane과 data plane을 나누어 보기](../../assets/azure-aca-deep-dive/01/01-09-control-plane-versus-data-plane-in-aca.ko.png)
...
> Azure Container Apps는 보이지 않는 Kubernetes substrate 위에 올라간 관리형 제품 표면입니다. Environment는 격리 경계입니다. Revision은 불변 런타임 스냅샷입니다. KEDA는 스케일 엔진입니다. Dapr는 사이드카 런타임입니다. Envoy는 ingress와 가중치 라우팅 계층입니다.
