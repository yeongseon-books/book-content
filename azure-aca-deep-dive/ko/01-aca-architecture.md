...
```mermaid
flowchart LR
    CLIENT[외부 클라이언트\n브라우저 / API 소비자] --> LB[ACA 관리 Load Balancer]
...
```mermaid
flowchart TB
    U[사용자-facing ACA resource model\nContainer App / Revision / Environment] --> P[플랫폼 동작\nIngress / Traffic split / Scale / Dapr integration]
    P --> OSS[관리되는 OSS 구성 요소\nKEDA / Dapr / Envoy]
...
```mermaid
flowchart LR
    subgraph ENV[하나의 Container Apps environment]
        direction TB
        APPA[App A]
        APPB[App B]
        APPC[App C]
    ...
    ENV --> LOG[공용 Log Analytics 대상]
    ENV --> DAPRCFG[공용 Dapr component 표면]
```
...
flowchart LR
    ...
    REV --> SVC[Environment 내부 Service endpoint]
    SVC --> REP[Pod replica 집합]
    REP --> POD[Pod]
    POD --> USER[사용자 컨테이너]
...
stateDiagram-v2
    [*] --> Provisioning
    Provisioning --> Active: readiness 통과
    Provisioning --> Failed: 검사 실패
    Active --> ScaleToZero: 실행 replica 없음
    ScaleToZero --> Active: 요청 또는 이벤트 발생
    Active --> Inactive: 운영자가 비활성화
    Failed --> Inactive
    Inactive --> [*]
```
...
flowchart LR
    ...
    PLATFORM --> KEDAOBJ[관리되는 KEDA objects]
    KEDAOBJ --> HPA[HPA 동작 / metrics query]
    HPA --> REPLICAS[Revision replica 수]
```
...
```mermaid
flowchart LR
    APP[사용자 컨테이너] -->|localhost:3500 또는 50001| DAPRD[daprd sidecar]
    DAPRD --> COMP[Environment-level Dapr component]
    COMP --> EXT[외부 state / pubsub / secret store]
```
...
```mermaid
flowchart LR
    CLIENT[클라이언트 요청] --> LB[ACA 관리 Load Balancer]
    LB --> ENVOY[Envoy ingress]
    ENVOY -->|80%| C1[Revision A용 upstream cluster]
    ENVOY -->|20%| C2[Revision B용 upstream cluster]
...
flowchart LR
    subgraph CP[Control plane]
        CFG[Environment / app 설정]
        REV[Revision 생성]
        SCALE[Scale reconciliation]
        DAPRCTL[Dapr component 등록]
        TRAFFIC[Traffic weight 갱신]
    end
