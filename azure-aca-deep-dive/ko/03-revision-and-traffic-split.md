...
짧게 답하면 Envoy 라우팅 계층의 weighted upstream selection입니다.
...
![Revision은 불변 런타임 스냅샷입니다](../../assets/azure-aca-deep-dive/03/03-01-a-revision-is-an-immutable-runtime-snaps.ko.png)
...
![Revision-scope 변경과 application-scope 변경](../../assets/azure-aca-deep-dive/03/03-02-revision-scope-versus-application-scope.ko.png)
...
![Single revision mode와 multiple revision mode는 단순 UI 토글이 아닙니다](../../assets/azure-aca-deep-dive/03/03-03-single-revision-mode-and-multiple-revisi.ko.png)
...
![Label과 weight는 서로 다른 라우팅 문제를 풉니다](../../assets/azure-aca-deep-dive/03/03-04-labels-and-weights-solve-different-routi.ko.png)
운영 의미도 분명히 다릅니다.
...
Upstream Envoy route schema는 route configuration API 안에 weighted cluster 개념을 정의합니다.
ACA traffic splitting을 설명할 때 기대야 할 고정점이 바로 이것입니다.

![Revision weight에서 Envoy weight로](../../assets/azure-aca-deep-dive/03/03-05-from-revision-weight-to-envoy-weight.ko.png)
ACA 자체는 closed-source라서, 이 사이를 잇는 제품 어댑터 코드를 직접 볼 수는 없습니다.
그래도 Microsoft의 traffic-splitting 문서와 Envoy의 weighted-cluster 모델을 함께 보면, 메커니즘은 충분히 정확하게 설명할 수 있습니다.
...
![왜 가중치는 앱 안이 아니라 ingress 계층에 있어야 하는가](../../assets/azure-aca-deep-dive/03/03-06-why-the-weight-belongs-at-the-ingress-la.ko.png)
...
![무중단 배포는 Revision 생성만으로 성립하지 않습니다](../../assets/azure-aca-deep-dive/03/03-07-zero-downtime-rollout-depends-on-readine.ko.png)
...
![Multiple revision mode에서는 rollout이 라우팅 수학이 됩니다](../../assets/azure-aca-deep-dive/03/03-08-multiple-revision-mode-turns-rollout-int.ko.png)
...
![`latestRevision: true`는 편하지만, 스위치를 누가 쥐는지가 달라집니다](../../assets/azure-aca-deep-dive/03/03-09-latestrevision-true-is-convenient-but-it.ko.png)
...
![Active Revision과 traffic을 받는 Revision은 같은 개념이 아닙니다](../../assets/azure-aca-deep-dive/03/03-10-a-hidden-but-important-split-active-revi.ko.png)
...
![Revision weight와 scale은 연결되지만 같은 시스템은 아닙니다](../../assets/azure-aca-deep-dive/03/03-11-revision-weight-and-scale-are-related-bu.ko.png)
...
![Envoy의 weighted cluster 모델이 중요한 이유](../../assets/azure-aca-deep-dive/03/03-12-where-envoy-s-weighted-cluster-model-mat.ko.png)
"가중치는 어디에서 오는가"라는 질문에 가장 정확한 대답은 이것입니다.
App-scope ACA ingress 구성이 가중치의 출발점이고, 그 값이 ingress routing 계층에서 Revision upstream에 적용됩니다.
...
![Rollback이 빠른 이유도 Revision 불변성 덕분입니다](../../assets/azure-aca-deep-dive/03/03-13-the-rollback-story-is-better-because-rev.ko.png)
...
> ACA의 Revision은 불변 런타임 스냅샷입니다. Traffic splitting은 application-scope ingress 결정이며, 어떤 active Revision이 app URL에서 노출될지를 정합니다. 퍼센티지는 결국 Envoy routing 계층의 데이터이며, Revision upstream을 weighted destination으로 취급하는 방식으로 적용됩니다.
