...
```mermaid
flowchart LR
    TEMPLATE[Revision-scope template] --> REV[새 revision]
    REV --> PROV[Provisioning 검사]
    PROV --> ACTIVE[Active revision]
    ACTIVE --> TRAFFIC[트래픽 대상 자격]
```
...
```mermaid
flowchart TB
    CHANGE[Container App 변경] --> RS[Revision-scope]
    CHANGE --> AS[Application-scope]
    RS --> NEWREV[새 revision 생성]
    AS --> SAMEAPP[app 표면만 갱신]
```
...
```mermaid
flowchart LR
    SINGLE[Single revision mode] --> S1[새 revision ready]
    S1 --> S2[트래픽이 새 revision으로 전환]
    S2 --> S3[이전 revision deprovision]

    MULTI[Multiple revision mode] --> M1[여러 active revision 공존]
    M1 --> M2[수동 traffic weight]
    M2 --> M3[명시적 activation / deactivation]
```
...
```mermaid
flowchart LR
    MAIN[메인 app URL] -->|80%| REV1[Revision 1]
...
```mermaid
flowchart LR
    ACA[ACA ingress traffic 설정] --> ROUTE[Envoy route config]
    ROUTE --> WC[WeightedCluster entry]
    WC --> C1[Revision A용 upstream cluster]
    WC --> C2[Revision B용 upstream cluster]
```
...
```mermaid
flowchart LR
    CLIENT[클라이언트] --> ENVOY[Ingress Envoy]
    ENVOY -->|weight 80| SVC1[Revision A용 Service]
    ENVOY -->|weight 20| SVC2[Revision B용 Service]
    SVC1 --> POD1[Pod A]
    SVC2 --> POD2[Pod B]
```
...
```mermaid
sequenceDiagram
    participant Deploy as 배포 업데이트
    participant ACA as ACA control plane
    participant Rev as 새 revision
    participant Envoy as Envoy ingress

    Deploy->>ACA: template 갱신
    ACA->>Rev: 불변 revision 생성
    Rev->>Rev: replica provisioning
    Rev->>Rev: startup/readiness 검사 통과
    ACA->>Envoy: ready 이후 traffic target 갱신
```
...
```mermaid
flowchart LR
    AUTO[`latestRevision: true`] --> AUTOSWITCH[latest ready 시 ACA가 traffic 이동]
    EXPL[명시적 revisionName weight] --> MANUAL[운영자가 직접 traffic 이동]
```
...
```mermaid
flowchart LR
    REV[Active revision] --> ZERO[메인 URL traffic 0%]
...
```mermaid
flowchart LR
    WEIGHT[Traffic weight] --> LOAD[관측된 요청 부하]
...
```mermaid
flowchart LR
    ROUTE[app host/path용 route match] --> UPSTREAMS[가중치가 붙은 upstream cluster 집합]
...
```mermaid
flowchart LR
    BAD[문제가 생긴 새 revision] --> ZERO[weight를 0으로]
    GOOD[known good revision] --> FULL[weight를 100으로 복원]
```
