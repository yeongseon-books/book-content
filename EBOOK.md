...
이 문서는 본 저장소(`technical-content`)에서 eBook source bundle을 생성하고, private `mkdocs-ebook` 도구와 연동하는 방식을 정의한다.
...
`technical-content`의 eBook 빌드는 항상 private `mkdocs-ebook` repository의 최신 버전을 기준으로 수행한다.

원칙:

- `mkdocs-ebook`은 `technical-content`의 고정 의존성이 아니다.
