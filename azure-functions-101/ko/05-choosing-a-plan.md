...
4화에서는 함수를 classic Consumption 플랜에 올렸습니다. 그 경로는 데모와 학습에는 여전히 가장 단순합니다. 다만 현재 Microsoft Learn 기준으로 **classic Consumption은 레거시 플랜**이고, **새 서버리스 앱의 기본 선택지는 Flex Consumption**입니다.

그래도 Consumption을 빼고 설명하면 실제 선택을 이해하기 어렵습니다. 이번 글에서는 **Consumption / Flex Consumption / Premium / Dedicated(App Service Plan)** 네 가지를 모두 다루되, 각 플랜의 현재 위치와 제약을 그대로 적겠습니다.
...
| 플랜 | 한 줄 정의 |
|---|---|
| **Consumption** | 가장 단순한 종량제 서버리스. 현재는 레거시 플랜이고, Windows 중심으로 생각하는 편이 맞습니다. |
...
|---|---|---|---|---|
| **현재 위치** | 레거시 | 새 서버리스 기본 선택지 | 고급 서버리스 | 일반 App Service 계열 |
...
| **OS** | Windows 중심 레거시, Linux Consumption은 retired/retiring | Linux only | Windows / Linux | Windows / Linux |
| **VNet 통합** | 없음 | 있음 | 있음 | 있음 |
...
- VNet 통합 없음
- 1.5 GB 고정 메모리
- 레거시 플랜이라는 점 자체가 장기 운영 리스크
- Linux Consumption은 이미 retired 또는 retiring 단계라 새 기준점으로 잡기 어려움

그래서 4화에서는 Consumption으로 배포했지만, 그건 **가장 짧은 시연 경로**였기 때문입니다. 새 서버리스 서비스를 처음 설계한다면 Consumption을 출발점으로 둘 이유는 예전보다 많이 줄었습니다.
...
graph TD
    ...
    Q1 -- 아니오 --> Q2{새 서버리스 앱인가?}
    Q1 -- 예 --> Q3{Linux로 가능한가?}

    Q2 -- 예 --> FLEX0[Flex Consumption<br/>기본 후보]
    Q2 -- 아니오 --> Q4{가장 단순한 데모 경로가 필요한가?}

    Q3 -- 예 --> FLEX1[Flex Consumption<br/>VNet + 종량제]
    Q3 -- 아니오 --> PREM1[Premium<br/>VNet + Windows]

    Q4 -- 예 --> CONS[Consumption<br/>단순 데모 또는 기존 앱]
    Q4 -- 아니오 --> Q5{콜드 스타트가 치명적인가?}

    Q5 -- 예 --> PREM2[Premium<br/>상시 준비 용량]
    Q5 -- 아니오 --> FLEX2[Flex Consumption<br/>현행 기본 선택지]

    DED[Dedicated<br/>기존 App Service Plan 활용 또는 수동 스케일]
```
...
플랜을 골랐다면 다음 질문은 자연스럽게 이어집니다. **실제로 어떤 기준으로 인스턴스가 늘고 줄어드는가, 그리고 첫 호출은 왜 느린가**입니다. 6화에서는 이벤트 기반 스케일, target-based scaling, 콜드 스타트를 한 장으로 묶어 설명합니다.

---

## 시리즈 맥락

이 글은 Azure Functions 101의 5화입니다. 4화에서 실제 배포를 한 번 끝까지 해 봤다면, 이번 글은 그 배포 대상을 어떤 호스팅 플랜에 둘지 결정하는 기준을 정리하는 자리입니다. 다음 글에서는 여기서 고른 플랜이 실제로 어떻게 스케일하고 왜 콜드 스타트 차이를 만드는지 이어서 다룹니다.
