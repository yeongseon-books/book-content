...
`connection: 'StorageConnection'`이라고 쓰면 “환경 변수 `StorageConnection`의 값을 연결 문자열로 사용하라”는 뜻입니다. 코드에 직접 연결 문자열을 박지 않습니다. 운영 환경에서는 이게 Key Vault나 Managed Identity로 바뀝니다(자세한 건 5화 이후 운영 편에서).
