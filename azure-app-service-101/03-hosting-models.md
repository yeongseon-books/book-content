...
핵심은 이겁니다. **티어는 마지막에 고르는 게 맞습니다.** 먼저 Linux/Windows, Code/Container를 정해야 어떤 제약이 생기는지 보입니다.
...
## Bicep 예시: 선택을 코드로 고정하기

포털에서 몇 번 클릭해도 되지만, 팀으로 운영한다면 선택을 IaC로 고정하는 편이 좋습니다. 아래 예시는 **Linux + Standard S1** 기준의 가장 기본적인 형태입니다.
...
이 예시가 좋은 이유는 단순해서가 아니라, **의사결정이 드러나기 때문**입니다.

- Linux를 쓰기로 했는가? → `reserved: true`
- 운영 가능한 티어로 시작할 것인가? → `Standard / S1`
- 앱과 Plan의 관계를 명확히 했는가? → `serverFarmId`

시리즈 04에서 실제 Flask 앱을 배포할 때도, 바로 이런 구조 위에 앱을 올리게 됩니다.
...
| 질문 | 예 / 아니오 |
|---|---|
| 앱이 Windows 전용 의존성을 갖고 있는가? | ☐ |
| 컨테이너 이미지 수준의 제어가 정말 필요한가? | ☐ |
| 고객 트래픽을 받는데도 Free/Shared를 고르고 있진 않은가? | ☐ |
| 배포 슬롯 없이 운영하려는가? 그렇다면 감당 가능한가? | ☐ |
| 이 앱을 다른 앱과 같은 Plan에 넣어도 되는가? | ☐ |
| 사내망 연결, 프라이빗 접근, 강한 격리가 필요한가? | ☐ |
| 비용을 앱 개수가 아니라 Plan 인스턴스 기준으로 보고 있는가? | ☐ |
