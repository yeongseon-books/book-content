...
이 글은 Docker 101 시리즈의 7번째 글입니다. 여기서는 FastAPI 앱을 컨테이너에 넣을 때 PID 1, SIGTERM, healthcheck, non-root가 왜 운영 기본값인지 설명하고, 실제로 어떤 Dockerfile과 검증 절차가 필요한지 살펴봅니다.
## 이 글에서 다룰 문제

- FastAPI와 uvicorn을 어떤 방식으로 컨테이너에 담아야 할까요?
- PID 1과 SIGTERM은 왜 컨테이너 운영에서 중요할까요?
- healthcheck는 어떻게 구성해야 할까요?
- non-root 실행은 왜 기본값이어야 할까요?
- Python 컨테이너에서 자주 하는 실수는 무엇일까요?

> Python 컨테이너화가 실제 운영 수준이 되는 순간은 애플리케이션이 뜨는 시점이 아니라, 종료 신호를 올바르게 처리하고 준비 상태를 정직하게 노출하며 최소 권한으로 실행되는 시점입니다.
...
![Dockerfile과 uvicorn, healthcheck, graceful shutdown이 Python 컨테이너를 구성하는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/docker-101/07/07-01-concept-at-a-glance.ko.png)

*빌드된 Python 이미지가 PID 1, healthcheck, SIGTERM 처리까지 포함해 운영 준비가 되는 흐름*
