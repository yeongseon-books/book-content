...
- 첫 컨테이너를 실제로 실행해 보면 무엇이 보일까요?
- 입문 단계에서 가장 자주 하는 실수는 무엇일까요?

> Docker의 가장 실용적인 멘탈 모델은 이것입니다. 애플리케이션, 의존성, 실행 환경을 하나의 이미지로 묶고, 그 이미지를 어디서나 같은 방식으로 실행하는 도구입니다. 결국 Docker는 "환경을 문서로 설명하는 방식"에서 "환경을 실행 가능한 산출물로 전달하는 방식"으로 팀의 기준을 바꿉니다.
