...
이 글은 Docker 101 시리즈의 5번째 글입니다. 여기서는 Compose를 단순 편의 명령이 아니라 멀티 컨테이너 환경의 선언으로 보고, service·volume·network·healthcheck를 한 파일에 어떻게 담아야 하는지 살펴봅니다.
...
이 그림에서는 Docker Compose를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> Docker Compose의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
