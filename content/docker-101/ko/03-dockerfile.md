...
이 글은 Docker 101 시리즈의 3번째 글입니다. 여기서는 Dockerfile의 핵심 명령이 어떤 역할을 하는지, 레이어 캐시를 살리는 순서가 왜 중요한지, 그리고 `.dockerignore`와 non-root 실행이 왜 운영 품질로 이어지는지 살펴봅니다.
## 이 글에서 다룰 문제

- `FROM`, `RUN`, `COPY`, `CMD`는 각각 어떤 역할을 할까요?
- Dockerfile 명령 순서는 왜 빌드 속도에 큰 영향을 줄까요?
- `.dockerignore`는 성능뿐 아니라 보안에도 왜 중요할까요?
- 왜 기본값이 non-root 실행이어야 할까요?
- 입문 단계에서 Dockerfile로 가장 자주 만드는 실수는 무엇일까요?

> Dockerfile은 단순한 빌드 레시피가 아닙니다. 팀이 이미지를 어떻게 만들고, 어떤 순서로 캐시를 재사용하며, 어떤 런타임 계약을 기대하는지까지 드러내는 운영 문서입니다.
...
![베이스 이미지 위에 의존성과 애플리케이션 코드를 순서대로 쌓는 Dockerfile 레이어 흐름](https://yeongseon-books.github.io/book-public-assets/assets/docker-101/03/03-01-concept-at-a-glance.ko.png)

*변경 빈도가 낮은 단계부터 높은 단계 순서로 쌓아 캐시를 살리는 Dockerfile 레이어 전략*
