...
- **`FROM`, `RUN`, `COPY`, `CMD`는 각각 어떤 역할을 할까요?**
  - 본문 예시처럼 `FROM`은 베이스 이미지 선택, `RUN`은 빌드 시점에 한 번 실행되어 layer를 만드는 명령, `COPY`는 호스트 파일을 이미지로 가져오는 단계, `CMD`는 컨테이너 실행 시 기본으로 돌아갈 프로세스를 정합니다. 빌드 시점과 실행 시점을 가르는 경계가 이 네 명령에 압축돼 있습니다.
- **Dockerfile 명령 순서는 왜 빌드 속도에 큰 영향을 줄까요?**
  - Docker는 명령마다 layer를 만들고 layer 단위로 캐시하기 때문에, 본문에서 강조했듯이 자주 바뀌지 않는 의존성 설치를 앞에 두고 자주 바뀌는 소스 복사는 뒤에 두어야 캐시가 깨지지 않습니다. 예를 들어 `COPY requirements.txt` → `RUN pip install` → `COPY .` 순서가 캐시 적중률을 크게 올립니다.
- **`.dockerignore`는 성능뿐 아니라 보안에도 왜 중요할까요?**
  - 본문에서 본 것처럼 `.dockerignore`가 없으면 `.git`, `.env`, `node_modules` 같은 큰 디렉터리가 빌드 컨텍스트로 전송되어 빌드가 느려질 뿐 아니라, 비밀 키나 로컬 자격증명이 이미지에 들어가 외부로 유출될 위험까지 생깁니다. 그래서 보안과 성능을 동시에 막아 주는 첫 방어선입니다.
