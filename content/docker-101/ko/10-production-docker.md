...
- **프로덕션에서는 어떤 이미지 태그 정책을 가져가야 할까요?**
  - 본문에서 강조했듯이 `latest` 태그는 어떤 시점의 이미지인지 추적할 수 없어 롤백·재현이 어렵습니다. 대신 git commit SHA나 `1.4.2` 같은 immutable 버전 태그를 쓰고, 운영 환경은 그 고정 태그를 가리키게 두어야 같은 태그가 절대 다른 이미지를 가리키지 않게 됩니다. semver + SHA 이중 태깅이 흔한 패턴입니다.
- **레지스트리와 이미지 서명은 왜 공급망 신뢰의 일부일까요?**
  - 본문에서 본 것처럼 레지스트리에서 이미지를 받는 것만으로는 그 이미지가 우리 CI가 만든 것이 맞는지 보장되지 않습니다. cosign·Notation 같은 도구로 이미지에 서명하고, 운영 클러스터의 admission policy가 서명 검증을 강제하면 중간자 공격이나 악성 푸시를 막을 수 있습니다.
- **read-only, capability 제한, non-root는 어떤 식으로 결합해야 할까요?**
  - 본문에서 강조했듯이 컨테이너는 기본적으로 root로 떠서 위험하기 때문에, Dockerfile에서 `USER appuser`로 non-root로 떨어뜨리고 런타임에 `--read-only --cap-drop=ALL --security-opt=no-new-privileges`를 함께 걸어 권한을 최소화해야 합니다. 세 가지가 함께여야 "프로세스가 root가 아니고, 파일도 못 쓰고, 추가 권한도 못 얻는" 상태가 완성됩니다.
