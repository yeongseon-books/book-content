...
- **image와 container는 정확히 무엇이 다를까요?**
  - image는 빌드 결과로 만들어진 읽기 전용 파일 시스템 스냅샷이고, container는 그 image 위에 writable layer를 얹어 실제로 프로세스를 띄운 인스턴스입니다. 본문에서 본 것처럼 같은 image로 여러 container를 띄울 수 있다는 점이 이 분리가 만드는 핵심 이득입니다.
- **layer와 copy-on-write는 왜 중요한 개념일까요?**
  - 이미지가 여러 layer로 쌓이고 컨테이너가 writable layer만 추가하기 때문에, 본문 예시처럼 동일 base 위 100개 컨테이너가 공통 layer를 공유해 디스크·메모리를 절약할 수 있습니다. 변경이 일어날 때만 새 블록을 복사(copy-on-write)하므로 시작 비용도 낮습니다.
- **컨테이너의 수명 주기는 어떤 흐름으로 흘러갈까요?**
  - 본문 다이어그램처럼 `create` → `start`(running) → `stop`(exited) → `rm`이 표준 흐름이고, 그 사이에 `pause/unpause`, `restart`가 끼어듭니다. 그래서 종료된 컨테이너는 디스크에는 남지만 실행 중은 아니라는 점을 알면 `docker ps -a`와 `docker ps`의 결과 차이가 자연스럽게 이해됩니다.
