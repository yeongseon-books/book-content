...
이 글은 Docker 101 시리즈의 2번째 글입니다. 여기서는 image와 container의 수명 주기를 분리해서 보고, layer와 writable layer가 왜 상태 손실과 재현성 문제를 설명하는 핵심인지 실습과 함께 정리합니다.
...
이 그림에서는 Image와 Container를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> Image와 Container의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
