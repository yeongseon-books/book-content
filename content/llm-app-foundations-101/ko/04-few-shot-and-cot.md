...
- 더 나은 응답 유도를 이해하는 가장 좋은 방법: 정답을 직접 강요하는 것이 아니라 따라야 할 패턴과 점검 순서를 보여 주는 것으로 보는 것입니다을 실무에 적용할 때 주의할 점은 무엇일까요?
...
![예시 쌍이 마지막 요청의 답변 형식을 유도하는 구조](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/04/04-01-few-shot-prompting-teaches-by-example-in.ko.png)
...
![zero-shot과 few-shot의 안정성 차이 비교](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/04/04-02-zero-shot-versus-few-shot-on-the-same-re.ko.png)
...
![나쁜 예시와 좋은 예시가 만드는 차이](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/04/04-03-example-quality-can-help-or-hurt.ko.png)
...
![중간 단계를 따라 final_answer로 가는 추론 경로](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/04/04-04-chain-of-thought-helps-the-model-decompo.ko.png)
...
![프롬프트 기법보다 다른 도구가 먼저 필요한 경계](https://yeongseon-books.github.io/book-public-assets/assets/llm-app-foundations-101/04/04-05-where-these-techniques-stop-helping.ko.png)
...
## 분류 작업에서 few-shot 품질을 계량하는 방법

few-shot은 "좋아 보인다"가 아니라 지표로 확인해야 오래 유지됩니다. 예시를 바꾼 뒤 라벨 일관성과 형식 준수율이 얼마나 달라지는지 측정하면, 프롬프트 튜닝이 감각에서 엔지니어링으로 넘어갑니다.
...
이런 간단한 지표만으로도 나쁜 예시를 빠르게 걸러낼 수 있습니다. 예시를 추가했는데 `format_compliance`가 오히려 떨어지면, 예시 수가 아니라 예시 품질이 문제라는 신호입니다.

### CoT를 쓸 때 노출 범위 제어

CoT는 유용하지만 모든 중간 추론을 사용자에게 그대로 노출할 필요는 없습니다. 실무에서는 내부 reasoning과 외부 출력 포맷을 분리하는 패턴을 자주 씁니다.
...
이 방식은 두 가지 장점이 있습니다. 첫째, 사용자 출력이 장황해지는 문제를 줄입니다. 둘째, 후속 시스템이 파싱해야 할 출력 계약을 안정적으로 유지합니다.

### rate limit 구간에서 few-shot 크기 줄이기 전략

트래픽이 몰릴 때는 예시 수를 동적으로 줄이는 편이 안전합니다. 품질을 조금 희생하더라도 시스템을 멈추지 않는 것이 우선인 구간이 있기 때문입니다.

| 상태 | 예시 수 | temperature | 목적 |
|---|---:|---:|---|
| 정상 | 3 | 0.2 | 형식 안정성 극대화 |
| 주의 | 2 | 0.2 | 토큰 절감 시작 |
| 경고(429 증가) | 1 | 0.1 | 생존 모드 |

few-shot과 CoT는 강력하지만 공짜가 아닙니다. 그래서 운영 정책 안에서 "언제 강하게 쓰고 언제 줄일지"를 미리 정해 두는 편이 훨씬 안전합니다.
...
추가로, 이 두 기법은 "정답률"만으로 평가하면 판단이 왜곡될 수 있습니다. 형식 준수율, 재시도 횟수, 평균 토큰 사용량 같은 운영 지표까지 함께 봐야 실제 제품 환경에서 더 나은 선택을 할 수 있습니다.
