...
- 한눈에 보는 아티팩트 흐름에서 가장 흔한 실수는 무엇일까요?
...
이 흐름이 중요한 이유는 빌드와 배포를 같은 러너에 묶지 않아도 된다는 데 있습니다. 저장된 산출물을 다음 잡이 읽기만 하면 되므로, 파이프라인 구조가 훨씬 유연해집니다.
...
- **`upload-artifact`와 `download-artifact`는 각각 언제 쓰일까요?**
  - `upload-artifact`와 `download-artifact`는 각각 언제 쓰일까요 — 본문에서 구체적으로 다룹니다.
- **잡 사이에서 결과물을 넘길 때 아티팩트가 왜 유용할까요?**
  - 잡 사이에서 결과물을 넘길 때 아티팩트가 왜 유용할까요 — 본문에서 구체적으로 다룹니다.
- **`retention-days`는 비용과 어떤 관계가 있을까요?**
  - `retention-days`는 비용과 어떤 관계가 있을까요 — 본문에서 구체적으로 다룹니다.
- **한눈에 보는 아티팩트 흐름에서 가장 흔한 실수는 무엇일까요?**
  - 이 흐름이 중요한 이유는 빌드와 배포를 같은 러너에 묶지 않아도 된다는 데 있습니다. 저장된 산출물을 다음 잡이 읽기만 하면 되므로, 파이프라인 구조가 훨씬 유연해집니다.
- **핵심 용어를 먼저 정리하겠습니다을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 성숙한 팀은 빌드 산출물만 저장하지 않습니다. checksum, SBOM, 테스트 리포트, 커버리지 결과까지 함께 남겨 두고, 필요하면 릴리스 파일에 서명도 붙입니다.
- **자동화 전과 후를 비교해 보겠습니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 아티팩트가 없는 파이프라인에서는 build 잡이 끝나는 순간 결과물이 사실상 사라집니다. 로그에는 성공이라고 남지만, 실제 `dist/*.whl` 파일은 다음 잡이 접근할 수 없습니다.
