...
중요한 점은 `df`를 raw `x`가 아니라 `g(x)`에서 평가한다는 사실입니다. 이 한 줄이 outer/inner 구분의 핵심입니다. 먼저 안쪽 함수가 만들어 낸 중간값에서 바깥 함수의 민감도를 계산하고, 그다음 안쪽 함수의 민감도를 곱합니다.
...
중요한 점은 중간값 `a`, `b`를 재사용하면 계산이 단순해지고 실수도 줄어든다는 사실입니다. 이 재사용 전략이 바로 계산 그래프의 캐시 개념과 같습니다.
...
- **함수가 다른 함수 안에 들어갈 때 전체 미분은 왜 단순 합이 아니라 곱으로 연결될까요?**
  - 공식만 보면 단순해 보이지만, 실제로는 어느 지점에서 어떤 도함수를 평가하는지 자주 헷갈립니다. 먼저 대표 예제를 단계별로 분해해 보겠습니다.
- **바깥 함수와 안쪽 함수를 구분하는 가장 실용적인 방법은 무엇일까요?**
  - 연쇄 법칙을 가장 잘 이해하는 방법은 합성함수를 여러 단계 파이프라인으로 보는 것입니다
- **단계가 여러 개인 합성함수에서 gradient는 어떤 순서로 전달될까요?**
  - 단계가 여러 개인 합성함수에서 gradient는 어떤 순서로 전달될까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 딥러닝 모델은 합성함수입니다. 입력에서 출력까지 가는 길에 수많은 변환이 있고, loss는 그 전체 결과 위에서 정의됩니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 연쇄 법칙을 가장 잘 이해하는 방법은 합성함수를 여러 단계 파이프라인으로 보는 것입니다. 입력은 먼저 안쪽 함수로 들어가 중간 표현을 만들고, 그 결과가 다시 바깥 함수의 입력이 됩니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 연쇄 법칙을 가장 잘 이해하는 방법은 합성함수를 여러 단계 파이프라인으로 보는 것입니다. 입력은 먼저 안쪽 함수로 들어가 중간 표현을 만들고, 그 결과가 다시 바깥 함수의 입력이 됩니다.
...
- **Calculus for ML 101 (5/10): 연쇄 법칙 (현재 글)**
