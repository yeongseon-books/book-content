...
- **계산 그래프 관점에서 순전파와 역전파는 각각 무엇을 남길까요?**
  - 역전파를 직관적으로 이해하려면 같은 샘플에 대해 forward와 backward를 나란히 적는 것이 가장 빠릅니다. 예제로 2층 완전연결 네트워크를 사용하겠습니다.
- **local derivative를 저장한다는 말은 실제로 어떤 의미일까요?**
  - local derivative를 저장한다는 말은 실제로 어떤 의미일까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - PyTorch, TensorFlow, JAX는 모두 gradient를 자동으로 계산해 줍니다. 그래서 역전파를 몰라도 모델은 학습됩니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 역전파를 가장 직관적으로 이해하는 방법은 계산 그래프를 생각하는 것입니다. 각 노드는 값을 계산하고, 자신의 부모 노드에 대한 local derivative를 알고 있습니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 역전파를 가장 직관적으로 이해하는 방법은 계산 그래프를 생각하는 것입니다. 각 노드는 값을 계산하고, 자신의 부모 노드에 대한 local derivative를 알고 있습니다.
