...
> 역전파는 연쇄 법칙을 계산 그래프 위에서 뒤로 적용하는 절차입니다. 값은 앞으로 흘러가고, gradient는 뒤로 흘러가며 각 파라미터의 책임을 빠르게 계산합니다.
...
- **역전파**: 출력 쪽에서 입력 쪽으로 gradient를 전파하는 과정입니다.
...
**After**: 한 번의 backward pass로 모든 gradient를 얻는 구조를 이해합니다.
...
노드는 값과 부모 노드, 그리고 gradient를 저장합니다. 아주 작은 autograd 엔진의 출발점입니다.
...
덧셈의 지역 미분은 각각 1입니다. 역전파는 이런 지역 미분을 따라 gradient를 전달합니다.
...
출력 노드에서 gradient 1.0으로 시작해 부모 방향으로 누적합니다. 이것이 연쇄 법칙의 구현입니다.
...
값을 먼저 계산한 뒤, 뒤로 돌아가며 각 입력이 최종 출력에 얼마나 기여했는지 얻습니다.

## 이 코드에서 주목할 점

- 순전파는 값을 만들고 역전파는 gradient를 만듭니다.
- 각 노드는 지역 미분을 보관해야 합니다.
- shared node에서는 gradient를 더해 누적해야 합니다.

## 자주 하는 실수 5가지

1. gradient가 누적된다는 사실을 잊고 zero_grad를 하지 않습니다.
2. 순전파에서 필요한 값을 저장하지 않아 역전파가 불가능해집니다.
3. shared node에서 gradient 합산을 빠뜨립니다.
...
- [ ] shared node에서 gradient가 누적된다는 점을 이해했습니다.
- [ ] 필요할 때 수치 미분으로 gradient를 검증할 수 있습니다.

## 정리 및 다음 글

역전파는 연쇄 법칙을 효율적으로 실행하는 계산 절차입니다. 값은 앞으로 계산하고, gradient는 뒤로 흘려 보내며 각 파라미터의 책임을 얻습니다. 다음 글에서는 지금까지 배운 미분 개념이 딥러닝 학습 루프 전체에서 어떻게 합쳐지는지 정리하겠습니다.
