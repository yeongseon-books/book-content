...
- 검정 유형을 실무에 적용할 때 주의할 점은 무엇일까요?
- 눈에 보이는 차이와 통계적 차이는 다를 수 있다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **데이터로 “차이가 있다”는 말을 어디까지 할 수 있을까요?**
  - 데이터로 “차이가 있다”는 말을 어디까지 할 수 있을까요 — 본문에서 구체적으로 다룹니다.
- **귀무가설 H0와 대립가설 H1은 무엇을 뜻할까요?**
  - 귀무가설 H0와 대립가설 H1은 무엇을 뜻할까요 — 본문에서 구체적으로 다룹니다.
- **p-value만으로 판단하면 왜 부족할까요?**
  - p-value만으로 판단하면 왜 부족할까요 — 본문에서 구체적으로 다룹니다.
- **멘탈 모델에서 가장 흔한 실수는 무엇일까요?**
  - 가설검정은 먼저 “차이가 없다”는 기본 가정을 세우고, 표본에서 계산한 검정통계량이 그 가정 아래 얼마나 드문지 측정한 뒤, 미리 정한 기준과 비교해 결론을 내리는 절차입니다. 중요한 점은 가설을 데이터 보기 전에 정해야 한다는 사실입니다.
- **검정 유형을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 가설검정은 데이터 유형과 질문에 따라 여러 검정 방법을 씁니다. 아래 표는 실무에서 자주 쓰이는 네 가지 검정과 적용 조건입니다.
