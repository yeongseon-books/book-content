...
- 상관 대비 인과을 실무에 적용할 때 주의할 점은 무엇일까요?
- 함께 움직인다고 바로 원인이라고 말할 수는 없다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
문제는 이 도구들이 너무 익숙해서 오용되기 쉽다는 사실입니다. 상관이 높다고 곧바로 원인이라고 읽거나, R²가 높다고 좋은 모델이라고 단정하거나, 잔차를 보지 않고 선형성을 가정하는 일이 자주 생깁니다. 그래서 기본 개념을 차분하게 구분할 필요가 있습니다.
...
- **상관계수는 무엇을 말하고 무엇은 말하지 못할까요?**
  - 상관계수는 무엇을 말하고 무엇은 말하지 못할까요 — 본문에서 구체적으로 다룹니다.
- **회귀식은 상관계수보다 어떤 정보를 더 줄까요?**
  - 회귀식은 상관계수보다 어떤 정보를 더 줄까요 — 본문에서 구체적으로 다룹니다.
- **R²는 어떤 범위에서 어떻게 읽어야 할까요?**
  - R²는 어떤 범위에서 어떻게 읽어야 할까요 — 본문에서 구체적으로 다룹니다.
- **멘탈 모델에서 가장 흔한 실수는 무엇일까요?**
  - 상관은 두 변수가 같은 방향으로 움직이는지와 그 강도를 보여 줍니다. 회귀는 그 관계를 식으로 적어, x가 바뀔 때 y가 어떻게 달라지는지 예측 가능한 형태로 만듭니다. 마지막으로 R²와 잔차는 그 식이 데이터를 얼마나 설명하는지 점검하게 합니다.
- **상관 대비 인과을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 상관과 인과는 가장 자주 섞이는 개념입니다. 아래 표는 둘의 차이와 판단 기준을 정리합니다.
