...
단순무작위추출은 가장 기본이지만, 실무에서는 세그먼트 분포가 중요하거나 비용 제약이 있을 때 층화추출이나 군집추출을 씁니다.
...
# 오차를 절반으로 줄이면?
n_half = sample_size_mean(sigma=20, margin_error=1, confidence=0.95)
print(f"오차 ±1로 줄일 때 필요한 표본 수: {n_half}")
print(f"표본 수 증가 배율: {n_half/n:.1f}배")
```
...
```text
필요한 표본 수: 385
오차 ±1로 줄일 때 필요한 표본 수: 1537
표본 수 증가 배율: 4.0배
```

만약 오차를 ±1로 줄이려면 표본 수는 4배로 늘어납니다. 정밀도를 2배 높이려면 비용은 4배 늘어나는 구조입니다.
...
# 단순무작위추출
sample_srs = users.sample(n=500, random_state=42)
...
print("모집단 plan 분포:")
print(users["plan"].value_counts(normalize=True).round(3))

print("\n단순무작위 표본 plan 분포:")
print(sample_srs["plan"].value_counts(normalize=True).round(3))

print("\n층화 표본 plan 분포:")
print(stratified["plan"].value_counts(normalize=True).round(3))
```
...
population = pd.DataFrame({
    "user_id": range(10000),
    "satisfaction": rng.normal(3.5, 0.8, 10000).clip(1, 5),
    "plan": rng.choice(["free", "pro"], size=10000, p=[0.8, 0.2])
})

# 비응답 편향 시뮬레이션: pro 사용자는 응답률 높음 (더 만족함)
response_prob = population["plan"].map({"free": 0.15, "pro": 0.60})
responded = rng.random(len(population)) < response_prob.values
responses = population[responded]

print(f"모집단 평균 만족도: {population['satisfaction'].mean():.3f}")
print(f"응답자 평균 만족도: {responses['satisfaction'].mean():.3f}")
print(f"응답률: {responded.mean():.1%}")
print(f"\n모집단 plan 분포:\n{population['plan'].value_counts(normalize=True).round(3)}")
print(f"\n응답자 plan 분포:\n{responses['plan'].value_counts(normalize=True).round(3)}")
```

이 예제에서 응답자 평균 만족도가 모집단보다 높게 나옵니다. 만족도가 높은 pro 사용자의 응답률이 더 높기 때문입니다. 이것이 비응답 편향입니다.
...
- **비응답 편향**: 응답하지 않은 사람들이 응답자와 체계적으로 다를 때 발생합니다.
- **생존자 편향**: 살아남은 데이터만 보고 판단할 때 생기는 편향입니다.
...
| 실수 유형 | 구체적 상황 | 올바른 접근 |
|---|---|---|
| 편의 표본을 대표 표본으로 사용 | 사내 직원들에게만 설문 | 목표 모집단에서 무작위 추출 설계 |
| 비응답자 무시 | 응답자 100명만 분석 | 비응답자 특성 파악 후 보정 여부 결정 |
| N=30 법칙 맹신 | "30명 넘으면 충분하다" | 표본 수보다 설계와 대표성이 먼저 |
| 모집단 정의 없이 추출 | 막연히 "사용자"라고만 지칭 | "지난 30일 활성 사용자"처럼 명확히 정의 |
| 시간순 구간만 사용 | "최근 1주일" 데이터만 분석 | 무작위성 없는 편의 표본이 될 수 있음 |
| 생존자만 분석 | 현재 사용 중인 고객만 조사 | 이탈 고객 포함한 코호트 전체 추적 |
...
```text
모집단: "지난 30일 이내 1회 이상 구매한 웹사이트 고객"
```
...
users = pd.DataFrame({
    ...
    "plan": rng.choice(["free", "pro", "enterprise"], size=5000, p=[0.75, 0.20, 0.05]),
    "region": rng.choice(["KR", "US", "JP"], size=5000, p=[0.6, 0.3, 0.1])
})
print(f"모집단 크기: {len(users)}")
...
print(f"\n표본 크기: {len(sample)}")
print("표본 plan 분포:")
print(sample["plan"].value_counts(normalize=True).round(3))
```
...
response_rate = 0.35  # 35% 응답률
responded = rng2.random(len(sample)) < response_rate
responses = sample[responded]

print(f"\n응답률: {responded.mean():.1%} ({responded.sum()}/{len(sample)})")
print("\n응답자 plan 분포:")
print(responses["plan"].value_counts(normalize=True).round(3))
```
...
```python
print("\n모집단 vs 응답자 plan 분포 비교:")
comparison = pd.DataFrame({
    "모집단": users["plan"].value_counts(normalize=True),
    "응답자": responses["plan"].value_counts(normalize=True)
}).round(3)
print(comparison)
```
...
resp = sample[rng.random(len(sample)) < resp_prob.values]
...
print(f"모집단 실제 평균: {pop['satisfaction'].mean():.3f}")
print(f"단순 평균 (편향 있음): {naive:.3f}")
...
생존자 편향을 피하려면 가입 시점부터 이탈까지 전체 코호트를 추적하고, 이탈자 인터뷰를 포함해야 합니다.
...
- **Statistics 101 (4/10): 표본과 모집단 (현재 글)**
