...
- 유사도 검색이 시작되기 전, 로더 경계 설계는 왜 중요할까요?
- Character, Recursive, Token splitter는 같은 텍스트를 어떻게 다르게 자를까요?
- `chunk_overlap`은 왜 설정한 숫자만큼 정확하게 겹치지 않는 것처럼 느껴질까요?
- 문자 수가 아니라 토큰을 기준으로 나누는 편이 더 안전한 때는 언제일까요?

> 청킹은 텍스트를 잘게 자르는 작업이 아닙니다. 나중에 retrieval이 다시 회수하길 바라는 의미 경계를 지금 얼려 두는 작업입니다.

![이 글에서 답할 질문](../../../assets/rag-deep-dive/01/01-01-questions-this-post-answers.en.png)
...
예제 파일: `/root/Github/rag-deep-dive/en/01-document-loading-and-chunking/main.py`
...
![문서 로더별 메타데이터 전달 흐름](../../../assets/rag-deep-dive/01/01-01-loader-metadata-flow.en.png)
...
![문자 분할 뒤 병합 창 이동 흐름](../../../assets/rag-deep-dive/01/01-02-character-splitter-merge-window.en.png)
...
![재귀 분할의 구분자 우선순위 흐름](../../../assets/rag-deep-dive/01/01-03-recursive-separator-fallback.en.png)
...
![문자 수와 토큰 수의 어긋남 흐름](../../../assets/rag-deep-dive/01/01-04-token-aware-splitting.en.png)
...
![청크 품질 측정과 조정 순환 흐름](../../../assets/rag-deep-dive/01/01-05-chunk-quality-feedback-loop.en.png)
