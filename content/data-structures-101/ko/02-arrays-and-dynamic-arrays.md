...
- **고정 배열은 왜 인덱싱이 O(1)일까요?**
  - 고정 배열은 원소가 연속 메모리에 놓여 있어서 `arr[i] = 시작 주소 + i × 원소 크기` 계산만으로 원하는 칸을 바로 찾습니다. 그래서 `data[0]`, `data[5_000_000]`, `data[-1]`를 읽는 실험에서도 위치와 무관하게 비슷한 시간이 나온 것입니다.
- **동적 배열은 공간이 모자랄 때 어떻게 늘어나고, 왜 append가 평균적으로 빠를까요?**
  - 본문 `DynamicArray` 구현처럼 size가 capacity에 닿으면 더 큰 블록을 새로 만들고 기존 값을 복사한 뒤, 보통 capacity를 두 배로 키웁니다. resize는 가끔 비싸지만 대부분의 `append`는 빈 슬롯에 바로 쓰기만 하므로 분할 상환 기준으로 평균 O(1)을 유지합니다.
- **파이썬의 `list`는 내부적으로 어떤 자료구조에 가까울까요?**
  - 파이썬 `list`는 앞쪽 삽입이 `insert(0, -1)`에서 O(n)이고, 끝 `append`가 빠르며, capacity를 늘려 가는 점에서 전형적인 동적 배열 계열입니다. 또한 연속 메모리를 써서 dict보다 순회가 빠른 캐시 친화성까지 보여 주므로, 본문 설명과 정확히 맞아떨어집니다.
