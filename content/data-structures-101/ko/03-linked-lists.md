...
연결 리스트는 개념상 삽입과 삭제가 빠르지만, 실제 서비스에서는 메모리 지역성이 떨어져 캐시 미스가 늘어날 수 있습니다. 그래서 노드 구조를 단순화하고 불필요한 포인터 필드를 줄이는 설계가 중요합니다. 성능 문제를 볼 때는 이론 복잡도와 함께 CPU 캐시 동작도 반드시 확인해야 합니다.
...
- **단일 연결 리스트와 이중 연결 리스트는 구조적으로 무엇이 다를까요?**
  - 단일 연결 리스트는 각 노드가 `value`와 `next`만 가져 앞으로만 이동할 수 있고, 본문 `Node` 구현도 그 형태입니다. 이중 연결 리스트는 `prev`와 `next`를 모두 둬서 양방향 순회와 제거가 쉬워지며, 예제의 sentinel head·tail 구조가 그 차이를 잘 보여 줍니다.
- **연결 리스트는 왜 양 끝 삽입·삭제에 강하고, 왜 인덱스 접근에는 약할까요?**
  - `push_front`와 `pop_front`는 head 포인터 하나만 바꾸면 끝나므로 O(1)이고, 이중 연결 리스트도 양 끝 포인터만 고치면 같은 장점을 얻습니다. 반대로 중간 원소에 가려면 `cur = cur.next`로 노드를 따라가야 하므로, 배열의 `lst[mid]` 같은 O(1) 인덱스 접근을 기대할 수 없습니다.
- **포인터 조작을 직접 구현할 때 어떤 실수를 가장 조심해야 할까요?**
  - 삭제할 때 `prev.next = cur.next`를 빠뜨리거나, 이중 연결 리스트에서 `node.next.prev = node.prev` 갱신을 놓치면 구조가 바로 깨집니다. 본문이 sentinel 노드를 도입한 이유도 빈 리스트, 첫 노드, 마지막 노드 같은 경계 분기를 줄여 이런 포인터 실수를 예방하기 위해서입니다.
