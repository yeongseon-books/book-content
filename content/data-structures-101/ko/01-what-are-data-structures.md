...
- **자료구조와 추상 자료형(ADT)은 어떻게 다를까요?**
  - 이 글에서 ADT는 `Stack` 추상 클래스처럼 `push`와 `pop`이 무엇을 해야 하는지만 정한 인터페이스입니다. 자료구조는 `ListStack`과 `LinkedStack`처럼 그 인터페이스를 배열 기반으로 구현할지, 연결 기반으로 구현할지를 결정하는 실제 메모리 전략입니다.
- **같은 데이터를 다루는데도 왜 구조 선택에 따라 성능이 크게 달라질까요?**
  - 같은 사용자 데이터를 리스트로 두면 `for user in users` 선형 탐색이 필요하지만, dict로 인덱싱하면 `users.get(target_id)` 한 번으로 바로 접근할 수 있습니다. 이벤트 우선순위 처리에서도 매번 `sort()` 하는 방식과 `heapq.heappush()`를 쓰는 방식이 삽입 비용부터 다르게 갈립니다.
- **입력 크기가 커질수록 자료구조 선택이 어떤 식으로 비용 차이를 만들까요?**
  - 본문 비교표처럼 배열 탐색은 O(n), 균형 트리 탐색은 O(log n), 해시 테이블 조회는 평균 O(1)이라서 입력이 커질수록 격차가 눈에 띄게 벌어집니다. 동적 배열의 append도 가끔 재할당이 비싸지만 분할 상환으로 평균 O(1)을 유지하므로, 반복 연산이 많은 경로에서는 이런 성장률 차이가 그대로 운영 비용 차이로 이어집니다.
