...
자료구조를 비교할 때는 평균 시간 복잡도만으로 결론을 내리면 정확도가 떨어집니다. 실제 시스템에서는 데이터 분포, 갱신 비율, 메모리 제약, 동시성 요구가 동시에 작동하기 때문입니다.
...
구현 단계에서는 연산 정의를 코드 시그니처로 먼저 고정하는 방식이 안전합니다. `insert`, `remove`, `contains`, `iterate`의 사전/사후 조건을 먼저 문서화하고, 그 뒤에 내부 저장 구조를 바꾸면 테스트 재사용성이 크게 올라갑니다.

## 디버깅 시나리오

### 시나리오 1: 리스트 조회가 점점 느려지는 경우

```python
# 증상: 데이터가 늘수록 로그인 API 응답이 느려짐
session_store = []  # 세션 ID 리스트

def check_session(sid):
    return sid in session_store  # O(n) 선형 탐색!

# 진단: 세션 수가 10만 개를 넘을 때부터 응답 시간이 급증
# 해결: set으로 교체 → O(1) 조회

session_store_fast = set()

def check_session_fast(sid):
    return sid in session_store_fast  # O(1) 해시 조회
```

### 시나리오 2: 정렬 비용이 핫패스에 반복될 때
...
def add_log_wrong(priority, msg):
    log_queue.append((priority, msg))
    log_queue.sort()  # 매번 O(n log n)

# 올바른 패턴: 힙 사용
log_heap = []

def add_log_right(priority, msg):
    heapq.heappush(log_heap, (priority, msg))  # O(log n)
...
start = time.perf_counter()
for i in range(N):
    add_log_wrong(N - i, f"msg{i}")
print(f"sort-every-insert: {(time.perf_counter() - start):.2f}s")

start = time.perf_counter()
for i in range(N):
    add_log_right(N - i, f"msg{i}")
print(f"heap-insert: {(time.perf_counter() - start):.2f}s")
```

### 시나리오 3: 잘못된 구조로 중복 제거

```python
# 문제: 1백만 개 중복 제거를 리스트로 시도
data = list(range(500_000)) * 2  # 중복 포함

# 잘못된 방법 O(n^2)
def dedupe_wrong(lst):
    result = []
    for item in lst:
...
# 올바른 방법 O(n)
def dedupe_right(lst):
    return list(dict.fromkeys(lst))  # 삽입 순서 보존
...
| 해시 테이블 | 조회/삽입/삭제 | O(1) | O(n) | 충돌과 부하율 관리 |
...
다음 글에서는 가장 기본적인 구조인 배열을 다룹니다. 고정 배열과 동적 배열이 어떻게 다르고, 파이썬의 `list`가 내부에서 무엇을 하는지 봅니다.

## 처음 질문으로 돌아가기

- **자료구조와 추상 자료형(ADT)은 어떻게 다를까요?**
  - ADT는 "무엇을 할 수 있는가"라는 인터페이스를 정의합니다. 자료구조는 그 인터페이스를 메모리에서 실제로 구현하는 방식입니다. 같은 ADT(Stack)도 배열로 구현할 수 있고 연결 리스트로도 구현할 수 있습니다.
- **같은 데이터를 다루는데도 왜 구조 선택에 따라 성능이 크게 달라질까요?**
  - 리스트는 앞에서부터 끝까지 훑고, 집합은 해시를 통해 곧바로 버킷으로 이동합니다. 연산의 내부 동작 방식이 다르기 때문에 입력이 커질수록 격차가 벌어집니다.
- **입력 크기가 커질수록 자료구조 선택이 어떤 식으로 비용 차이를 만들까요?**
  - O(n)과 O(1)의 차이는 데이터가 100개일 때는 미미하지만 1천만 개가 되면 수백만 배 차이로 드러납니다. 성장률이 다르기 때문입니다.

<!-- toc:begin -->
## 시리즈 목차

- **Data Structures 101 (1/10): 자료구조란 무엇인가? (현재 글)**
