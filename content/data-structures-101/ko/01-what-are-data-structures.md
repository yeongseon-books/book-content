...
- 이 시리즈에서 다룰 아홉 가지 핵심 자료구조는 각각 어떤 역할을 할까요?

같은 데이터, 같은 작업인데 어떤 코드는 1초 만에 끝나고 어떤 코드는 한 시간을 씁니다. 이 차이는 대개 알고리즘 이름보다 먼저, 데이터를 어떤 구조 위에 올려두었는지에서 시작합니다.

> 자료구조는 데이터를 메모리에 어떻게 배치하고 어떤 연산을 효율적으로 지원할지를 정하는 구체 구현입니다. ADT는 무엇을 할 수 있는지를 정의하고, 자료구조는 그것을 어떻게 할지를 결정합니다. 예를 들어 1,000만 명 중 한 사용자를 찾는 문제는 배열을 훑으면 평균 500만 번 비교가 필요하지만, 해시 테이블을 쓰면 평균 한 번에 가깝게 줄어듭니다.
