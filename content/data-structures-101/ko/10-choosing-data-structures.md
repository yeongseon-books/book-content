...
자료구조 선택은 한 번의 정답 찾기가 아니라 지속적인 재평가 과정입니다. 트래픽 패턴과 데이터 분포는 시간이 지나면 바뀌기 때문에, 초기 선택이 영구히 맞는 경우는 드뭅니다. 연산 비율, 지연 시간, 메모리 사용량을 정기적으로 측정해 선택 근거를 갱신해야 합니다.
...
- **새로운 문제를 만났을 때 자료구조 선택은 무엇부터 생각해야 할까요?**
  - 이 글의 출발점은 데이터 모양이 아니라 핫패스에서 어떤 연산이 가장 자주 일어나는지부터 묻는 것입니다. 그래서 우선순위 작업이면 `heapq`, 양끝 조작이면 `deque`, 키 조회가 핵심이면 `dict / set`처럼 후보를 연산 중심으로 좁혀 갑니다.
- **워크로드를 정의하는 다섯 가지 질문은 무엇일까요?**
  - 본문은 가장 빈번한 연산, 입력 크기, 정렬 필요 여부, 메모리 제약, 동시성·영속성 요구라는 다섯 질문을 먼저 적으라고 권합니다. 이 질문에 답하면 `list`, `deque`, `dict`, 균형 트리, 그래프 중 무엇이 맞는지 감이 아니라 근거로 고를 수 있습니다.
- **자료구조 선택을 결정 트리처럼 체계화할 수 있을까요?**
  - 가능합니다. 본문 `recommend_structure()` 함수처럼 `primary_op`가 `key_lookup`이면 `dict / set`, `min_or_max`이면 힙, `graph_traversal`이면 인접 리스트 그래프와 BFS/DFS를 고르는 식으로 선택 기준을 코드화할 수 있습니다. 여기에 `list.pop(0)` 대 `deque.popleft()` 측정처럼 실제 벤치마크를 붙이면, 결정 트리가 이론이 아니라 운영 가능한 판단 기준이 됩니다.
