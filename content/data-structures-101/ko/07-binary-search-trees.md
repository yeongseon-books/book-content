...
- **BST의 핵심 불변식은 무엇이며 검색·삽입·삭제는 어떻게 동작할까요?**
  - BST의 핵심은 항상 `left.key < node.key < right.key`를 유지하는 불변식이며, `insert`와 `search`는 비교 결과에 따라 왼쪽이나 오른쪽으로 내려가면서 동작합니다. 삭제는 특히 자식이 둘일 때가 까다롭고, 본문처럼 오른쪽 서브트리의 최소값인 중위 후속자로 교체해야 정렬 성질이 보존됩니다.
- **평균 시간 복잡도와 최악 시간 복잡도가 왜 달라질까요?**
  - 값이 적절히 퍼져 들어오면 매 단계에서 왼쪽 또는 오른쪽 절반을 버릴 수 있어서 평균적으로 O(log n)에 가깝게 탐색합니다. 하지만 `0, 1, 2, ...`처럼 정렬된 입력을 그대로 넣으면 본문 `skewed_height` 예제처럼 트리 높이가 30까지 치솟아, 탐색 단계 수가 사실상 연결 리스트 수준인 O(n)으로 무너집니다.
- **AVL, Red-Black, B-Tree 같은 균형 트리는 왜 필요할까요?**
  - 균형 트리는 트리가 한쪽으로 기울어지는 것을 회전과 규칙으로 제어해, BST의 정렬 장점과 탐색 효율을 계속 유지하려는 구조입니다. 그래서 데이터베이스 인덱스는 B-Tree/B+Tree를, `TreeMap` 같은 라이브러리는 Red-Black 트리를 택해 최악 성능 붕괴를 막습니다.
