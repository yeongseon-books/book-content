...
- **루트, 리프, 깊이, 높이 같은 트리 용어를 어떻게 정확히 구분할까요?**
  - 본문 그림에서 `A`는 부모가 없는 루트이고, `D`, `E`, `F`처럼 자식이 없는 노드는 리프입니다. 깊이는 루트에서 내려온 간선 수이고, 높이는 어떤 노드에서 가장 먼 리프까지의 간선 수라서 `height(root)`가 2로 계산되는 예제가 그 차이를 보여 줍니다.
- **일반 트리와 이진 트리는 어떤 점이 다를까요?**
  - 일반 트리는 `children` 리스트를 둬서 자식 수에 제한이 없고, 조직도 예제의 `CEO → CTO/CFO` 구조가 그 형태입니다. 이진 트리는 `left`, `right` 두 포인터만 가지므로 표현이 더 제약되지만, 순회와 탐색 알고리즘을 표준화하기 쉬워집니다.
- **전위·중위·후위 순회는 무엇이 다르고 어디에 쓰일까요?**
  - 세 순회는 루트를 언제 방문하느냐가 다르며, 본문 예제에서도 `preorder`는 `1 2 4 5 3`, `inorder`는 `4 2 5 1 3`, `postorder`는 `4 5 2 3 1`로 서로 다른 결과를 냅니다. 전위는 복제나 직렬화, 중위는 BST 정렬 출력, 후위는 계산 트리 평가처럼 목적에 따라 쓰임새가 나뉩니다.
