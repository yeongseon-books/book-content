...
| --- | --- | --- | --- |
| 순서 | LIFO | FIFO | 양방향 |
| 삽입 위치 | top | back | 앞/뒤 모두 |
| 제거 위치 | top | front | 앞/뒤 모두 |
| 파이썬 구현 | list | deque | deque |
| 대표 용도 | 함수 호출, DFS, 파싱 | BFS, 작업 큐, 메시징 | 슬라이딩 윈도우, 양방향 탐색 |
| push/pop | O(1) | enqueue/dequeue O(1) | 양 끝 O(1) |

## 디버깅 시나리오

### 시나리오 1: list.pop(0)로 큐를 구현해 속도가 느려지는 경우
...
### 시나리오 2: BFS에서 visited를 늦게 표시해 중복 방문 발생

```python
from collections import deque

# 잘못된 BFS — dequeue 시점에 visited 표시
def bad_bfs(graph, start):
    ...
    while queue:
        node = queue.popleft()
        if node in visited:  # 이미 큐에 있어도 중복 enqueue됨
            continue
        visited.add(node)
        order.append(node)
        for nb in graph[node]:
            queue.append(nb)  # 중복 enqueue 가능
    return order

# 올바른 BFS — enqueue 시점에 visited 표시
    ...
    while queue:
        ...
        for nb in graph[node]:
            if nb not in visited:
                visited.add(nb)  # enqueue 직전에 표시
                queue.append(nb)
    return order
```

### 시나리오 3: 재귀 스택 오버플로를 명시적 스택으로 해결

```python
import sys

# 재귀 방식 — 깊은 트리에서 RecursionError 위험
...
def dfs_iterative(graph, start):
    ...
    while stack:
        ...
        visited.add(node)
        order.append(node)
        for nb in reversed(graph[node]):  # 순서 일치를 위해 reversed
            if nb not in visited:
                stack.append(nb)
    return order
```
...
또한 "작업 큐"라는 말도 뭉뚱그리지 않습니다. 단순 FIFO, 우선순위 큐, 지연 큐, backpressure가 있는 큐는 시스템 거동이 완전히 다릅니다. 이름이 같다고 같은 구조라고 가정하지 않습니다. 스택과 큐는 기능이 단순하지만 운영 사고에서 자주 드러납니다. 재시도 작업이 계속 쌓일 때는 큐 길이와 처리율을 같이 봐야 하고, 파싱 실패가 증가할 때는 스택 깊이 분포를 확인해야 합니다.
...
- **스택과 큐라는 ADT는 각각 어떤 규칙을 가질까요?**
  - 스택은 LIFO로 마지막에 넣은 것을 먼저 꺼냅니다. 큐는 FIFO로 처음 넣은 것을 먼저 꺼냅니다. 두 구조 모두 양 끝만 만지므로 핵심 연산이 O(1)입니다.
- **파이썬에서는 어떤 자료구조로 스택과 큐를 구현하는 편이 자연스러울까요?**
  - 스택은 list, 큐는 `collections.deque`가 표준입니다. list는 한쪽 끝 연산이 O(1)이고, deque는 양쪽 끝이 모두 O(1)입니다.
- **함수 호출, BFS, 메시지 큐 같은 시스템은 왜 이 두 구조 위에서 이해할 수 있을까요?**
  - 함수 호출은 호출-복귀의 LIFO 패턴이어서 스택으로 모델링됩니다. BFS는 가까운 노드부터 방문하는 FIFO 패턴이어서 큐를 씁니다. 메시지 시스템도 처리 순서가 들어온 순서를 따르는 큐 구조입니다.
...
- **Data Structures 101 (4/10): 스택과 큐 (현재 글)**
