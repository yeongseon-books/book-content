...
- **힙 불변식과 완전 이진 트리는 어떤 관계일까요?**
  - 힙은 최소 힙 기준으로 부모가 항상 자식보다 작거나 같다는 힙 불변식을 유지하고, 동시에 마지막 레벨만 왼쪽부터 채워지는 완전 이진 트리 형태를 가집니다. 이 두 조건이 함께 있어야 루트에서 최솟값을 바로 꺼낼 수 있고, 트리 높이도 O(log n)으로 제어됩니다.
- **힙은 왜 배열 하나만으로 표현할 수 있을까요?**
  - 완전 이진 트리는 중간에 빈칸이 없어서 노드를 레벨 순서대로 배열에 담아도 구조가 보존됩니다. 그래서 본문 `MinHeap` 코드처럼 부모는 `(i - 1) // 2`, 자식은 `2 * i + 1`, `2 * i + 2` 계산만으로 찾아낼 수 있어 별도 포인터가 필요 없습니다.
- **삽입(sift up)과 삭제(sift down)는 어떻게 동작할까요?**
  - 삽입은 새 값을 배열 끝에 붙인 뒤 부모와 비교하며 위로 올리는 `sift up`으로 처리하고, 삭제는 마지막 값을 루트에 올린 뒤 더 작은 자식과 바꾸며 내려보내는 `sift down`으로 처리합니다. 본문 우선순위 큐 예제에서 `heapq.heappush`와 `heappop`이 바로 이 과정을 이용해 항상 다음 작업을 O(log n)에 고릅니다.
