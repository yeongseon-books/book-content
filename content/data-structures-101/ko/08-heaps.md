...
print(order)
# [(0, 'critical alert'), (0, 'page on-call'),
#  (1, 'retry failed payment'), (1, 'retry webhook'),
#  (2, 'nightly batch')]
```

tie-break 카운터가 없으면 동일 우선순위 항목을 비교할 때 오류가 날 수 있습니다. 특히 task가 비교 불가능한 객체일 때 문제가 됩니다.
...
## 힙 vs BST vs 정렬 배열 비교

| 구조 | 최솟값 조회 | 삽입 | 임의 검색 | 정렬 순회 | 적합한 용도 |
| --- | --- | --- | --- | --- | --- |
| 최소 힙 | O(1) | O(log n) | O(n) | 불가 | 우선순위 큐, 다익스트라 |
| BST (균형) | O(log n) | O(log n) | O(log n) | O(n) | 범위 질의, 정렬 유지 |
| 정렬 배열 | O(1) | O(n) | O(log n) | O(n) | 읽기 전용 정렬 데이터 |
| 해시 테이블 | O(n) | O(1)* | O(1)* | O(n log n) | 키 기반 빠른 조회 |

## 디버깅 시나리오

### 시나리오 1: 비교 불가능 객체에서 TypeError

**증상:** 우선순위가 같은 두 항목을 heapq에 넣었을 때 `TypeError: '<' not supported between instances of ...`가 납니다.
...
heapq.heappush(heap, (1, Task("b")))   # 우선순위 같음 → Task 끼리 비교 시도 → TypeError
```

**해결:** 중간에 tie-break 카운터를 넣습니다.
...
### 시나리오 2: 힙에서 임의 원소를 삭제하려는 시도

**증상:** 처리 중 특정 작업이 취소되어 힙에서 제거해야 하는데, 방법을 모릅니다.
...
# 잘못된 접근: heap.remove() 후 heapify는 O(n)
heap.remove((1, "task-a"))
heapq.heapify(heap)
```
...
def remove_task(task):
    ...
    entry[-1] = REMOVED  # 삭제 표시

def pop_task(heap):
    while heap:
        priority, count_val, task = heapq.heappop(heap)
...
### 시나리오 3: 최대 힙이 필요한데 heapq로 잘못 구현

**증상:** "가장 큰 값을 먼저 처리"해야 하는데 heapq를 그냥 쓰면 가장 작은 값이 나옵니다.
...
# 버그: 최소 힙이므로 가장 작은 점수가 먼저 나옴
heap = scores[:]
heapq.heapify(heap)
print(heapq.heappop(heap))   # 71 — 원하는 게 아님!

# 해결: 음수로 변환
max_heap = [-s for s in scores]
heapq.heapify(max_heap)
print(-heapq.heappop(max_heap))   # 96 — 정상
```

**원인:** `heapq`는 최소 힙 전용입니다. 음수 변환이 관용적 해법입니다.
...
- **힙 불변식과 완전 이진 트리는 어떤 관계일까요?**
  - 완전 이진 트리이기 때문에 배열에 빈 칸 없이 담을 수 있고, 부모-자식 관계를 인덱스 계산으로 O(1)에 찾을 수 있습니다. 불변식은 부모 ≤ 자식(최소 힙)으로 유지됩니다.
- **힙은 왜 배열 하나만으로 표현할 수 있을까요?**
  - 완전 이진 트리는 마지막 레벨도 왼쪽부터 채워지므로 구멍이 없습니다. 인덱스 i의 부모는 (i-1)//2, 자식은 2i+1과 2i+2입니다.
- **삽입(sift up)과 삭제(sift down)는 어떻게 동작할까요?**
  - 삽입은 배열 끝에 추가 후 부모와 비교하며 위로 올리고(sift up), 삭제는 루트와 마지막 원소를 교체 후 자식과 비교하며 내려옵니다(sift down). 둘 다 O(log n)입니다.
