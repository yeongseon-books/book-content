...
| 항목 | 체이닝 (Chaining) | 개방 주소법 (Open Addressing) |
| --- | --- | --- |
| 충돌 처리 방식 | 같은 버킷에 연결 리스트 | 다음 빈 슬롯 탐색 |
| 메모리 | 포인터 오버헤드 있음 | 연속 배열, 효율적 |
| 캐시 효율 | 낮음 (포인터 추적) | 높음 (연속 접근) |
| 삭제 처리 | 단순 | DELETED 마커 필요 |
| 부하율 한계 | 1.0 초과 허용 | 0.5~0.7 권장 |
| 적합한 상황 | 충돌이 많거나 삭제 빈번 | 메모리 효율 중요 |

## 디버깅 시나리오

### 시나리오 1: 해시 플러딩 공격 취약점

```python
# 나쁜 해시 함수 — 공격자가 의도적으로 충돌 유발 가능
class VulnerableHash:
    def __init__(self, val):
        self.val = val
    def __hash__(self):
        return self.val % 4  # 모듈로만 쓰면 충돌 예측 가능

# 파이썬 내장 hash()는 PYTHONHASHSEED로 무작위화됨
# 보안 민감한 맥락에서는 hmac 기반 해시 사용 검토
import hmac
import hashlib
...
class BadKey:
    ...
    # __eq__ 미정의 → 기본 id 비교 사용 → 논리적으로 같아도 다른 키로 취급

# 올바른 구현 — hash와 eq 함께 정의
class GoodKey:
    def __init__(self, x, y):
        self.x = x
        self.y = y
    ...
    def __eq__(self, other):
        return isinstance(other, GoodKey) and self.x == other.x and self.y == other.y

d = {}
d[GoodKey(1, 2)] = "first"
print(d.get(GoodKey(1, 2)))  # "first" — 같은 키로 인식
```

### 시나리오 3: dict 순서 의존 버그

```python
# 파이썬 3.7+에서 dict는 삽입 순서 보장
# 하지만 이 순서를 비즈니스 정렬 기준으로 쓰면 위험

config = {"b_feature": True, "a_feature": False, "c_feature": True}

# 잘못된 가정: 알파벳 순서라고 착각
for k in config:
    print(k)  # b_feature, a_feature, c_feature — 삽입 순서

# 올바른 방법: 정렬이 필요하면 명시적으로
for k in sorted(config):
    print(k)  # a_feature, b_feature, c_feature
```
...
또한 파이썬 3.7+의 dict가 삽입 순서를 보장한다는 사실과, 비즈니스 로직이 그 성질에 기대면 위험하다는 사실을 동시에 압니다. 순서가 정말 중요하면 정렬 컬렉션이나 명시적 구조를 선택합니다. 해시 테이블의 핵심은 평균 O(1) 자체보다 충돌 관리 정책입니다. 로드 팩터 임계값을 언제 넘는지, 리사이즈가 어떤 트래픽 구간에서 실행되는지, 키 분포가 편향되어 있는지 확인하지 않으면 성능 예측이 빗나갑니다.
...
- **해시 함수는 어떤 역할을 하며, 좋은 해시 함수는 무엇이 다를까요?**
  - 해시 함수는 임의 크기의 키를 고정 범위 정수로 변환합니다. 좋은 해시 함수는 균일하게 분산하고, 계산이 빠르며, 충돌을 최소화합니다.
- **충돌은 왜 생기고 어떤 방식으로 해결할 수 있을까요?**
  - 다른 키가 같은 인덱스로 매핑될 때 충돌이 생깁니다. 체이닝은 같은 버킷에 연결 리스트를 두고, 개방 주소법은 다음 빈 슬롯을 탐색해 해결합니다.
- **부하율(load factor)과 리해싱은 어떤 관계일까요?**
  - 부하율이 높아질수록 충돌이 잦아지고 성능이 떨어집니다. 임계값(보통 0.75)을 넘으면 더 큰 배열로 리해싱해 부하율을 낮춥니다.
...
- **Data Structures 101 (5/10): 해시 테이블 (현재 글)**
