...
이 구조의 핵심은 `OrderService`가 흐름을 조정하고, `Order`가 도메인 규칙을 책임진다는 사실입니다. 서비스가 모든 계산을 들고 있으면 규칙이 흩어지고, 엔티티가 외부 인프라를 알면 경계가 무너집니다.

## 절차지향 코드에서 객체 경계로 이동하는 리팩터링

아래 코드는 초기에 빠르게 만들기 쉬운 형태입니다. 문제는 주문 금액 계산, 쿠폰 적용, 상태 변경 규칙이 한 함수에 섞여 있다는 사실입니다.
...
- **객체지향은 절차지향과 무엇이 다르고, 왜 등장했을까요?**
  - 이 글에서는 `users` 리스트와 `create_user()`, `deactivate_user()`처럼 데이터와 함수가 흩어진 절차지향 코드가 `User` 클래스 안으로 모이는 과정을 먼저 보여 주었습니다. 이어서 `Order`가 쿠폰 적용, 상태 전이, 총액 계산을 내부 메서드로 책임지도록 바꾸면서, 객체지향이 등장한 이유가 문법 장식이 아니라 변경 규칙을 한 경계에 모으기 위해서라는 점을 확인했습니다.
- **클래스, 인스턴스, 속성, 메서드는 어떤 관계로 이해하면 쉬울까요?**
  - `Dog("Buddy", "Golden Retriever")`, `Rectangle(5, 3)`, `Counter()` 예제는 클래스를 설계도, 인스턴스를 그 설계도로 만든 개별 객체로 이해하게 해 줍니다. `name`, `breed`, `count` 같은 속성은 객체가 가진 상태이고, `bark()`, `area()`, `increment()` 같은 메서드는 그 상태를 읽거나 바꾸는 동작이라는 대응이 본문 전체에서 반복됩니다.
- **작은 스크립트에서는 함수만으로 충분한데, 언제 객체가 더 자연스러울까요?**
  - 단순 계산만 하는 `area(rect)` 수준이라면 함수로도 충분하지만, `Order`처럼 상태가 `draft`에서 `placed`로 바뀌고 쿠폰 검증까지 함께 따라오는 순간 객체가 더 자연스럽습니다. 글 후반의 `PaymentService`와 `Invoice` 예제도 관련 데이터와 정책이 함께 바뀌는 경우에는 객체 경계를 세우는 편이 테스트와 확장에 유리하다는 점을 보여 줍니다.
