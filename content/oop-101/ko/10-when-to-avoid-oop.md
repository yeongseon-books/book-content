...
*클래스를 만들지 말아야 할 때를 먼저 가르면, Python에서는 함수·데이터 구조만으로 끝낼 수 있는 코드가 생각보다 많습니다.*
...
## 너무 단순화했을 때 생기는 실패 모드

| 과한 단순화 | 나중에 나타나는 문제 | 다시 클래스를 고려할 시점 |
|-------------|----------------------|-----------------------------|
| 모든 로직을 함수로만 둠 | 같은 데이터 묶음이 함수 인자 목록으로 반복됩니다 | 상태와 검증 규칙이 항상 함께 움직일 때 |
| 모든 설정을 dict로만 관리 | 키 오타가 런타임까지 숨어 있습니다 | 필드 수가 늘고 기본값/검증이 중요해질 때 |
| 콜백 함수만으로 전략을 표현 | 관련 설정과 후처리 로직이 흩어집니다 | 전략마다 상태나 보조 메서드가 생기기 시작할 때 |
| 파이프라인 단계를 전역 함수로 누적 | 공유 캐시, 재시도 정책, 의존성 주입이 뒤엉킵니다 | 단계별 수명주기와 협력 객체를 관리해야 할 때 |

클래스를 피하는 목적은 "짧아 보이는 코드"가 아니라 "지금 필요한 복잡도만 쓰는 코드"입니다. 그래서 함수로 시작했다가도, 검증 규칙과 상태 전이가 반복되기 시작하면 클래스로 올리는 것이 오히려 단순해질 수 있습니다.
