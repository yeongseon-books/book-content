...
## SOLID를 적용해야 한다는 고장 신호

| 보이는 증상 | 자주 연결되는 원칙 | 먼저 해볼 조치 |
|-------------|--------------------|----------------|
| 기능 하나 추가할 때 파일 여러 개를 같은 이유로 고침 | OCP, DIP | 변하는 규칙을 Protocol/전략 객체로 뽑습니다 |
| 클래스가 저장, 검증, 알림, 포맷팅까지 모두 담당 | SRP | 변경 이유가 다른 메서드를 별도 클래스로 분리합니다 |
| 부모 타입으로 받았는데 특정 자식에서만 예외가 남 | LSP | 상속을 끊고 공통 계약을 다시 정의합니다 |
| 인터페이스 메서드 절반이 `pass`, `raise`, no-op | ISP | 호출자별 작은 인터페이스로 쪼갭니다 |

이 표의 핵심은 "원칙을 외워서 적용"하는 것이 아니라 "고장 나는 모양을 보고 필요한 원칙만 꺼내 쓰는 것"입니다. 예를 들어 테스트 더블을 넣기 어렵고 외부 API 교체 비용이 큰 코드라면, 다섯 원칙을 모두 고민하기보다 DIP 하나만 먼저 적용해도 체감이 큽니다.
