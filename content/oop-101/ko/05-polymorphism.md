...
**핵심 질문**: 서로 다른 타입의 객체를 동일한 인터페이스로 다루려면 어떻게 해야 할까요?

> 다형성은 같은 이름의 메서드가 객체의 타입에 따라 다르게 동작하는 것입니다. Python은 덕 타이핑 덕분에 상속 없이도 다형성을 구현할 수 있습니다. 이 글에서는 메서드 오버라이딩 기반 다형성, 덕 타이핑, 그리고 Python 3.8+의 Protocol까지 다룹니다.
...
- 다형성의 개념과 세 가지 구현 방식
- 덕 타이핑(duck typing)의 원리와 활용
- Python `Protocol`을 사용한 구조적 서브타이핑
- 다형적 코드의 실무 패턴
...
- **Protocol 우선** — Python에서는 Protocol이 사실상 표준입니다.
- **Duck typing 한계** — 런타임 실패를 피하려면 타입 검증을 함께 둡니다.
- **isinstance 절제** — 타입 분기 코드는 다형성으로 풀어냅니다.
- **계약 명시** — 메서드 시그니처를 protocol로 문서화합니다.
- **테스트** — 다형성 경로마다 회귀 테스트를 둡니다.
...
1. `Exportable` Protocol을 정의하고 `CsvExporter`, `JsonExporter`, `XmlExporter`를 구현하세요.
2. `__add__`와 `__mul__`을 구현한 `Matrix` 클래스를 만드세요.
3. `singledispatch`로 다양한 타입의 데이터를 로그 형식으로 출력하는 함수를 만드세요.
