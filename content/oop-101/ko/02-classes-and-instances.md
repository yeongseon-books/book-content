...
**핵심 질문**: 클래스를 어떻게 설계하고, 인스턴스를 어떻게 활용해야 할까요?

> 클래스는 객체의 청사진이고 인스턴스는 그 청사진으로 만든 실체입니다. 이 글에서는 생성자, 인스턴스 메서드, 클래스 메서드, 정적 메서드, 그리고 Python의 특수 메서드까지 클래스의 구성 요소를 상세히 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 생성자(`__init__`)와 인스턴스 초기화 패턴
- 인스턴스 메서드, 클래스 메서드, 정적 메서드의 차이
- `__repr__`, `__str__`, `__eq__` 등 특수 메서드
- 실무에서 자주 사용하는 클래스 설계 패턴
...
- **__init__ 명료성** — 필요한 의존성을 모두 인자로 받습니다.
- **dataclass 활용** — 단순 값 객체는 dataclass로 충분합니다.
- **불변 우선** — frozen=True로 시작해 필요 시에만 가변으로 갑니다.
- **__repr__ 정의** — 디버깅 시간을 가장 크게 줄여줍니다.
- **Type hints** — 필드·메서드 모두 타입 힌트를 강제합니다.
...
1. `Money` 클래스를 만들어 `__add__`, `__sub__`, `__repr__`, `__eq__`를 구현하세요.
2. `Temperature` 클래스를 만들고 `from_fahrenheit()` 클래스 메서드를 추가하세요.
3. `__slots__`를 사용한 `Pixel` 클래스를 만들어 100만 개의 인스턴스를 생성하고 일반 클래스와 메모리 사용량을 비교하세요.
