...
- **생성자(`__init__`)는 어디까지 책임져야 하고, 어디서부터 과해질까요?**
  - `Product`는 가격이 음수인지 바로 검사하고, `InventoryItem`은 `sku`, `quantity`, `unit_price` 불변식을 생성 시점에 고정합니다. 반면 DB 조회나 외부 API 호출처럼 객체의 핵심 상태를 넘는 일까지 생성자에 넣기 시작하면 과해지고, 이 글은 그런 대안으로 `Date.from_string()` 같은 클래스 메서드 생성자를 보여 주었습니다.
- **인스턴스 메서드, 클래스 메서드, 정적 메서드는 어떤 기준으로 나눠야 할까요?**
  - `InventoryItem.increase()`와 `decrease()`처럼 인스턴스 상태를 바꾸면 인스턴스 메서드이고, `Date.from_string()`이나 `today()`처럼 다른 입력 경로로 같은 타입을 만들어 주면 클래스 메서드입니다. `MathUtils.factorial()`처럼 `self`나 `cls`가 전혀 필요 없는 계산은 정적 메서드가 자연스럽고, 이 구분이 흐려지면 클래스가 단순 네임스페이스로 무거워진다는 점을 본문이 짚었습니다.
- **Python의 dunder 메서드는 왜 디버깅과 비교 연산에 중요할까요?**
  - `Point` 예제에서는 `__repr__`가 없을 때 `<__main__.Point object ...>`만 보여 디버깅이 불편했고, `__eq__`가 없을 때 같은 좌표의 두 점도 `False`로 비교되었습니다. 또 `Vector`의 `__add__`, `__mul__`, `__abs__`는 객체가 Python 문법과 자연스럽게 통합되는 모습을 보여 주어, dunder 메서드가 단순 꾸밈이 아니라 사용성과 테스트 정확도를 좌우한다는 점을 드러냈습니다.
