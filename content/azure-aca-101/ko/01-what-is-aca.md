...
App Service가 코드나 zip을 받아 ingress, scaling, slot이 연결된 웹 앱으로 바꿔 주듯, ACA는 컨테이너 이미지를 받아 비슷한 일을 해 줍니다. 차이는 입력이 컨테이너 이미지라는 점이고, 스케일러가 KEDA라서 0까지 내려갈 수 있다는 사실입니다.
...
- **Azure Container Apps(ACA)는 다른 Azure 컨테이너 서비스(App Service, AKS, Functions)와 무엇이 다를까요?**
  - ACA는 `az containerapp up --name myapi --image ... --ingress external --target-port 8000`처럼 컨테이너 이미지를 바로 관리형 런타임으로 연결해 주지만, AKS처럼 클러스터를 직접 운영하게 하지는 않습니다. 반대로 App Service보다 컨테이너 네이티브하고, Functions보다 HTTP API와 worker 조합에 잘 맞는다는 점이 비교표의 핵심이었습니다. 그래서 이 글은 ACA를 "AKS와 App Service 사이의 빈 공간"을 메우는 서비스로 설명했습니다.
- **ACA의 세 가지 핵심 구성 요소인 Environment, Container App, Revision은 각각 어떤 역할을 할까요?**
  - `Environment`는 VNet과 Log Analytics를 공유하는 실행 경계이고, `Container App`은 `orders-api` 같은 논리 서비스 이름이며, `Revision`은 이미지와 설정이 고정된 실행 스냅샷입니다. 본문 Bicep 예시에서도 `Microsoft.App/managedEnvironments@2024-03-01`와 `Microsoft.App/containerApps@2024-03-01`를 분리해 선언하고, 실제 런타임은 `latestRevisionName`과 ingress FQDN으로 확인하도록 구성했습니다. 즉 이 세 단어는 단순 용어가 아니라 네트워크 경계, 서비스 정체성, 배포 단위를 각각 가리킵니다.
- **어떤 워크로드는 ACA에 잘 맞고, 어떤 워크로드는 다른 서비스에 두는 편이 나을까요?**
  - HTTP API, worker, 마이크로서비스처럼 버스트와 유휴가 반복되고 `minReplicas` 조절이나 revision split이 중요한 워크로드는 ACA에 잘 맞습니다. 반대로 GPU, Operator, DaemonSet, custom scheduler처럼 Kubernetes 제어면이 필요한 경우는 AKS가 맞고, zip/code 중심 웹앱은 App Service가 더 단순합니다. 그래서 이 글은 `낮은 평균 + 큰 버스트`, `높은 평균 + 엄격한 지연 시간`, `고급 스케줄러/노드 제어`라는 세 기준으로 플랫폼 선택을 정리했습니다.
