...
Revision 모델의 장점은 배포를 이벤트처럼 다룰 수 있다는 사실입니다. 각 revision은 "무엇이 배포되었는가"를 정확히 가리키는 불변 기록이므로, 장애 분석에서 시간축을 복원하기 쉽습니다. 반대로 이 기록을 관리하지 않으면 revision 목록이 빠르게 오염되어 오히려 추적이 어려워집니다.
