...
Dapr는 마이크로서비스의 반복 작업을 줄여 주지만, 켠다고 해서 아키텍처 복잡성이 자동으로 사라지지는 않습니다. App 수준 설정과 Environment 수준 구성을 분리해서 봐야 어디까지가 플랫폼 책임인지 선명해집니다.

이 글은 Azure Container Apps 101 시리즈의 6번째 글입니다. 여기서는 ACA에서 Dapr 사이드카로 무엇을 얻고 무엇을 새로 책임지는지 살펴보겠습니다.
