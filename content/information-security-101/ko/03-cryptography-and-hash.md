...
- **해시와 HMAC은 어디서 갈릴까요?**
  - 현대 암호화에서 중요한 기준은 “암호화했다”가 아니라 “인증된 암호화까지 했는가”입니다.
- **왜 단순 암호화만으로는 충분하지 않고 AEAD가 필요할까요?**
  - - 키 생성이 안전한 난수(`secrets` 또는 OS CSPRNG)를 사용하는가 - 키/nonce 재사용 가능성이 코드 경로상 없는가 - 복호화 실패를 무시하지 않고 즉시 오류 처리하는가 - 암호화와 무결성 검증을 분리하지 않고 AEAD를 기본으로 사용하는가 - 평문 민감값이 로그/예외에 남지 않는가 체크리스트는 단순
- **전후 비교에서 가장 흔한 실수는 무엇일까요?**
  - 현대 암호화에서 중요한 기준은 “암호화했다”가 아니라 “인증된 암호화까지 했는가”입니다.
- **이 코드와 예제에서 먼저 볼 점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 암호화 자체가 강해도 키 접근 제어가 약하면 전체 보호가 무너집니다. 이때 자주 쓰는 모델이 RBAC와 ABAC입니다.
...
- **Information Security 101 (3/10): 암호화와 해시 (현재 글)**
