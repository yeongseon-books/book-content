...
| 사고 유형 | 첫 10분 목표 | 격리 우선순위 | 필수 증거 |
| --- | --- | --- | --- |
| 계정 탈취 의심 | 세션 강제 만료, MFA 재인증 | 사용자/관리자 계정 | 로그인/권한 변경 로그 |
| 데이터 유출 의심 | 외부 전송 경로 차단 | API 토큰, egress 규칙 | 접근 로그, 다운로드 기록 |
| 랜섬웨어 의심 | 확산 차단, 백업 보호 | 감염 노드 네트워크 분리 | 프로세스 목록, 파일 해시 |

## 사고 커뮤니케이션 30-30-60 규칙

- 30분: 현재 상태와 초기 영향 범위를 내부에 공유합니다.
- 30분: 다음 업데이트 시각과 책임자를 명시합니다.
...
테이블탑은 실제 장애 없이 의사결정 품질을 점검하는 훈련입니다.

| 시간 | 주입 이벤트 | 기대 행동 |
| --- | --- | --- |
| T+0 | 관리자 계정 비정상 로그인 경보 | 지휘관 지정, 채널 생성 |
| T+15 | 외부 데이터 대량 전송 의심 | 격리 조치, 증거 스냅샷 |
| T+30 | 고객 문의 급증 | 커뮤니케이션 템플릿 발송 |
| T+60 | 핵심 서비스 오류 증가 | 복구 우선순위 재조정 |

## 포스트모템 액션 아이템 품질 기준

- 각 항목에 담당자와 마감일이 있어야 합니다.
- "모니터링 강화" 같은 추상 문장을 금지하고 측정 가능한 결과를 씁니다.
- 동일 원인 재발률을 다음 분기 핵심 지표로 추적합니다.
- 미완료 항목은 리더십 리뷰에서 직접 점검합니다.
...
- **사고가 발생하면 첫 1분에 무엇을 해야 할까요?**
  - 1. 타임라인 확정본 작성 및 공유.
- **NIST IR 사이클은 어떤 흐름으로 이어질까요?**
  - NIST IR 사이클은 어떤 흐름으로 이어질까요 — 본문에서 구체적으로 다룹니다.
- **격리와 증거 보존은 어떻게 균형을 잡아야 할까요?**
  - 격리와 증거 보존은 어떻게 균형을 잡아야 할까요 — 본문에서 구체적으로 다룹니다.
- **전후 비교에서 가장 흔한 실수는 무엇일까요?**
  - 준비된 조직만 사고를 통제하고 다음 번을 더 잘 준비할 수 있습니다.
- **보안 인시던트 대응 단계을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 각 단계는 명확한 책임자, 시간 목표, 성공 기준을 가져야 합니다. NIST IR 사이클은 순환 구조입니다. 한 번의 사고에서 얻은 교훈은 다음 준비 단계로 돌아가야 조직이 강해집니다.
