...
이 그림에서는 Cloud Computing이란 무엇인가?를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> Cloud Computing이란 무엇인가?의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **왜 많은 팀이 서버를 직접 사는 방식에서 인터넷으로 자원을 빌려 쓰는 방식으로 이동했을까요?**
  - 본문의 기준은 Cloud Computing이란 무엇인가?를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **클라우드의 핵심 특성은 무엇이고, 온프레미스와 비교하면 무엇이 달라질까요?**
