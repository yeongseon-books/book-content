...
- **호스트-워커 gRPC 스트림에는 어떤 메시지가 어떤 방식으로 실릴까요?**
  - 호스트-워커 gRPC 스트림에는 어떤 메시지가 어떤 방식으로 실릴까요 — 본문에서 구체적으로 다룹니다.
- **스트림이 끊기면 호스트와 워커는 각각 무엇을 가정할까요?**
  - 스트림이 끊기면 호스트와 워커는 각각 무엇을 가정할까요 — 본문에서 구체적으로 다룹니다.
- **큰 페이로드는 이 스트림 위를 어떻게 지나가며, 어디에서 한계가 드러날까요?**
  - 큰 페이로드는 이 스트림 위를 어떻게 지나가며, 어디에서 한계가 드러날까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 운영에서 “워커와 통신이 안 된다”는 말은 매우 자주 쓰이지만, 실제로는 여러 다른 층이 섞여 있습니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - Azure Functions 호스트-워커 통신은 서비스 하나와 RPC 하나로 압축됩니다. 중요한 것은 RPC가 많지 않다는 사실이 아니라, **하나의 양방향 스트림 위에 거의 모든 프로토콜 메시지가 `oneof`로 다중화되어 있다는 점**입니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - Azure Functions 호스트-워커 통신은 서비스 하나와 RPC 하나로 압축됩니다. 중요한 것은 RPC가 많지 않다는 사실이 아니라, **하나의 양방향 스트림 위에 거의 모든 프로토콜 메시지가 `oneof`로 다중화되어 있다는 점**입니다.
