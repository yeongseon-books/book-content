...
- **워커 프로세스 모델은 언어마다 어떻게 다르며, 운영적으로 무엇을 뜻할까요?**
  - 워커 프로세스 모델은 언어마다 어떻게 다르며, 운영적으로 무엇을 뜻할까요 — 본문에서 구체적으로 다룹니다.
- **워커는 상태가 없다고 봐야 할까요, 아니면 인프로세스 상태를 어느 정도 믿어도 될까요?**
  - 워커는 상태가 없다고 봐야 할까요, 아니면 인프로세스 상태를 어느 정도 믿어도 될까요 — 본문에서 구체적으로 다룹니다.
- **워커가 OOM이나 hang에 빠지면 호스트는 어떤 신호로 그것을 감지할까요?**
  - 워커가 OOM이나 hang에 빠지면 호스트는 어떤 신호로 그것을 감지할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - Azure Functions의 out-of-proc 모델은 “호스트와 사용자 코드가 분리된다”는 한 문장으로 자주 설명되지만, 운영에서는 그 한 문장으로 충분하지 않습니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - Functions Host는 언어별 시작 방법을 코드에 직접 하드코딩하지 않습니다. 대신 각 언어 워커 패키지가 제공하는 `worker.config.json`을 읽고, 그 설정을 합쳐 `RpcWorkerConfig` 카탈로그를 만든 뒤, 필요할 때 그 설명에 따라 실제 OS 프로세스를 시작합니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - Functions Host는 언어별 시작 방법을 코드에 직접 하드코딩하지 않습니다. 대신 각 언어 워커 패키지가 제공하는 `worker.config.json`을 읽고, 그 설정을 합쳐 `RpcWorkerConfig` 카탈로그를 만든 뒤, 필요할 때 그 설명에 따라 실제 OS 프로세스를 시작합니다.
...
- [Azure Functions Deep Dive (6/6): 콜드 스타트와 Placeholder Mode — 새 인스턴스가 만들어질 때](./06-cold-start-placeholder.md)
