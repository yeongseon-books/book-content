...
실무에서 중요한 포인트는 "워커 수 증가 = 항상 처리량 증가"가 아니라는 사실입니다. Python처럼 GIL 영향을 받는 워커는 I/O 중심 부하에서 이득이 크지만, CPU 바운드 함수는 워커를 늘려도 메모리 압박만 늘어날 수 있습니다. 따라서 워커 수 실험은 처리량과 함께 RSS, GC pause, OOM 빈도까지 같이 봐야 합니다.
...
| 런타임 | 초기 기동 비용 | 메모리 특성 | 흔한 장애 패턴 | 권장 워커 수 기준 |
|---|---|---|---|---|
| Node.js | 낮음~중간 | 상대적으로 작음 | 이벤트 루프 블로킹, native 모듈 오류 | CPU 코어 수보다 부하 패턴 우선 |
| Python | 중간 | 라이브러리 의존 시 급증 | OOM, import 지연, GIL로 인한 병렬성 착시 | I/O 바운드는 2~4, CPU 바운드는 1 |
| Java | 높음 | 큼 | JVM warm-up 지연, GC pause | GC 메트릭 안정화 후 조정 |
| PowerShell | 중간~높음 | 중간 | 모듈 로드 지연, 시작 시간 편차 | 모듈 캐시 확인 후 결정 |
...
아래 명령으로 현재 인스턴스의 워커 프로세스를 확인하고, 워커 로그를 실시간으로 조회할 수 있습니다.

```bash
# 워커 관련 앱 설정 조회
az functionapp config appsettings list -n my-func -g my-rg \
  --query "[?starts_with(name, 'FUNCTIONS_WORKER')]" -o table

# 라이브 로그 스트림 (워커 stdout/stderr 포함)
az functionapp log tail -n my-func -g my-rg

# 실행 중인 인스턴스와 워커 수 확인 (Consumption 플랜)
az monitor metrics list \
  --resource "/subscriptions/<sub>/resourceGroups/my-rg/providers/Microsoft.Web/sites/my-func" \
  --metric "FunctionExecutionCount" \
  --interval PT1M \
  -o table
```

워커 재시작이 반복될 때는 Application Insights에서 아래 쿼리로 워커 수준 패턴을 확인합니다.

```kusto
traces
| where timestamp > ago(1h)
| where message contains "Worker" and (message contains "restart" or message contains "exited")
| project timestamp, message, cloud_RoleInstance
| order by timestamp desc
```

`cloud_RoleInstance` 컬럼으로 인스턴스를 구분하면 특정 인스턴스에서만 반복되는 워커 장애를 빠르게 좁힐 수 있습니다.

### 워커 장애 증상-진단-대응 매핑

운영에서 시간을 줄이는 방법 중 하나는 증상별 1차 진단 경로를 미리 정해두는 것입니다.

| 관찰 증상 | 1차 확인 위치 | 대표 원인 | 권장 대응 |
|---|---|---|---|
| 함수 응답 없음, 워커 로그 정상 | gRPC 채널 상태 | StartStream 대기 timeout | 워커 재시작, 포트 충돌 확인 |
| OOM Killed 반복 | 워커 RSS 메트릭 | 메모리 누수, 대용량 페이로드 | 메모리 프로파일링, 워커 수 조정 |
| 특정 인스턴스만 재시작 | `cloud_RoleInstance` 필터 | native 모듈 버전 불일치 | 이미지 재배포 또는 인스턴스 교체 |
| 처음 호출은 느리고 이후 정상 | `ProcessStartMs` 지표 | cold start, JVM warm-up | pre-warmed 인스턴스, 항상 실행 설정 |
| `console.log`가 Application Insights에 없음 | stdout 연결 구성 | 워커 패키지 버전 오래됨 | 워커 패키지 업데이트 |

이 표를 런북에 넣어두면 온콜 엔지니어가 처음 본 증상에서도 1~2분 안에 1차 가설을 잡을 수 있습니다.
...
- **워커 수 증가가 항상 처리량 증가를 보장하지 않습니다.** CPU 바운드 부하에서는 메모리 압박만 늘어날 수 있어 프로파일링 없는 설정 변경은 위험합니다.
...
- [ ] 워커 패키지 버전을 이미지 태그와 함께 버전 관리에 포함했습니다.
- [ ] 부하 유형(I/O vs CPU 바운드)에 따른 워커 수 기준을 런타임별로 문서화했습니다.

## 정리

이번 글에서 가장 중요한 사실은 Azure Functions 워커 시작이 추상적 "런타임 선택"이 아니라는 사실입니다. 각 언어 워커 패키지가 제공하는 `worker.config.json`에서 출발해, 호스트가 설정을 모아 `RpcWorkerConfig`를 만들고, 그 설명을 바탕으로 실제 OS 프로세스를 띄웁니다. 이 과정을 보면 언어 지원이 왜 호스트 본체 변경보다 워커 패키지 문제에 가까운지 이해됩니다.
...
- **워커 프로세스 모델은 언어마다 어떻게 다르며, 운영적으로 무엇을 뜻할까요?**
  - 각 워커 패키지의 `worker.config.json`이 실행 파일, 엔트리 포인트, 확장자 책임을 정의합니다. 운영 관점에서는 언어마다 기동 비용, 메모리 특성, 흔한 장애 패턴이 달라 런타임별 경보와 설정을 분리해야 합니다.
- **워커는 상태가 없다고 봐야 할까요, 아니면 인프로세스 상태를 어느 정도 믿어도 될까요?**
  - 워커가 죽고 재생성될 때 인프로세스 상태는 사라집니다. 외부 저장소(스토리지, 코스모스 DB)로 상태를 분리하지 않으면, 워커 장애 시 데이터 손실로 이어집니다.
- **워커가 OOM이나 hang에 빠지면 호스트는 어떤 신호로 그것을 감지할까요?**
  - OOM 종료는 `WorkerProcess.Exited` 이벤트로 감지됩니다. hang은 직접 감지하기 어렵고, 지연 시간 임계치와 `WorkerConcurrencyManager`의 스로틀 신호가 간접 신호가 됩니다. 진행 중인 `InvocationRequest`는 실패로 마킹되고, 새 워커와 채널이 재생성됩니다.
