...
- 안전한 운영을 위해 어떤 단위의 런타임 버전 고정이 필요할까요?
- Host와 Worker는 같은 컨테이너일까요, 별도 프로세스일까요, 아니면 별도 머신일까요?
...
## 호스트 부팅을 이해하는 가장 좋은 방법: `WebJobsScriptHostService` 바깥 껍질과 `ScriptHost` 안쪽 커널을 분리해서 보는 것입니다
...
![호스트와 워커가 함께 있는 인스턴스 구조](https://yeongseon-books.github.io/book-public-assets/assets/azure-functions-deep-dive/01/01-01-the-big-picture-one-azure-functions-host.ko.png)
