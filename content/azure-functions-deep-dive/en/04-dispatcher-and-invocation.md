...
The previous chapter covered **The gRPC Event Stream — What Do the Host and Worker Actually Exchange?**.
After this chapter, the next one moves on to **Scaling Internals — Scale Controller, ScaleMonitor, and What Differs Across Plans**.
<!-- ebook-only:end -->
...
The object that `WorkerFunctionInvoker` carries when it hands the call to the dispatcher is **`ScriptInvocationContext`**. At `5e59423`, the concrete fields break down like this:

- Function metadata (`FunctionMetadata`)
- Execution context (`ExecutionContext`)
- Input and binding data (`Inputs`, `BindingData`)
- Trace context (`Traceparent`, `Tracestate`, `Attributes`)
- Cancellation/logging-related state (`CancellationToken`, `Logger`, `AsyncExecutionContext`, `Properties`)
- A `TaskCompletionSource<ScriptInvocationResult>` that will receive the result (`ResultSource`)

One correction that matters for accuracy: `RetryContext` is not a direct field on `ScriptInvocationContext`. It gets added later when `ToRpcInvocationRequest()` builds the protobuf request. That last item, `ResultSource`, is the decisive one. **The dispatcher does not expect a synchronous response.** It returns a `Task` immediately after pushing the invocation, and only when the worker sends a response does that `Task` complete. A single worker can have many in-flight invocations at once, distinguished by `invocation_id`.
...
2. Register the call's `TaskCompletionSource` in an **in-memory dictionary keyed by invocation_id** so the eventual response can be matched to it
3. Create `new StreamingMessage { InvocationRequest = invocationRequest }` and send it toward the worker-specific `_outbound` writer
4. [`FunctionRpcService`](https://github.com/Azure/azure-functions-host/blob/5e59423ba45491041d18224c3e72c168a4a5b7f7/src/WebJobs.Script.Grpc/Server/FunctionRpcService.cs) grabs the same worker's channel pair via `TryGetGrpcChannels(workerId, out var inbound, out var outbound)`, then drains `outbound.Reader` into the real gRPC stream
...
The key correction here is that `SendInvocationRequest` does **not** mint a new GUID into `request_id` and use that as the correlation key. The host sends the payload as `new StreamingMessage { InvocationRequest = invocationRequest }`, and the correlation key for the round trip is the `invocation_id` already present on the `InvocationRequest`.
...
The `invocation_id ↔ TaskCompletionSource` dictionary from Step 5 is what makes that possible. The host can keep multiple invocations in flight at once, and the in-instance worker-expansion thresholds are controlled by real knobs such as `WorkerConcurrencyOptions`, not by a separate per-worker request-count setting in this source tree.
...
[`WorkerConcurrencyManager.cs`](https://github.com/Azure/azure-functions-host/blob/5e59423ba45491041d18224c3e72c168a4a5b7f7/src/WebJobs.Script.Grpc/WorkerConcurrencyManager.cs) does not work from a simple invocation-count threshold. It evaluates `WorkerStatus.LatencyHistory`, determines whether a worker is overloaded from recent latency samples, and then requests **that an additional worker be spun up**. So a single function app can have multiple workers running side by side — multiple worker processes inside one instance.
...
| Step | Object | Role |
|---|---|---|
| 1 | WebJobs SDK trigger listener | Detects the trigger firing |
| 2 | `WorkerFunctionInvoker` | Is invoked by the SDK when it wants to call user code |
