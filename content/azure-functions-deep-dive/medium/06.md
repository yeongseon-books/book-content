...
**Consumption**
- Cold start frequency: Frequent (scale 0 → 1, or every new instance)
- Mechanism: placeholder → specialization every time

**Flex Consumption (on-demand)**
- Cold start frequency: Frequent (scale 0 → 1)
- Mechanism: placeholder → specialization every time

**Flex Consumption (Always Ready)**
- Cold start frequency: Almost none
- Mechanism: Always-specialized instances kept around

**Premium (pre-warmed)**
- Cold start frequency: Almost none (small on scale-out)
- Mechanism: Pre-warmed instances play a role similar to the placeholder pool

**Dedicated**
- Cold start frequency: Only at instance boot
- Mechanism: App Service always-on, no placeholders involved
...
- Stage affected: Skips specialization entirely
- Rationale: Flex Consumption docs + the code flow above

**Premium pre-warmed**
- Stage affected: Owns a specialization pool directly
- Rationale: Same

**Lighter dependencies**
- Stage affected: User-code load time during `RestartHostAsync`
- Rationale: `FunctionAssemblyLoadContext.ResetSharedContext`

**Smaller deployment package**
- Stage affected: Content injection time
- Rationale: Outside this code; platform layer

**Latest Functions runtime**
- Stage affected: Improved JIT trace
- Rationale: `WarmUpConstants.JitTraceFileName`

**Application Initialization (Premium/Dedicated)**
- Stage affected: Time from host-ready to first request
- Rationale: App Service layer
