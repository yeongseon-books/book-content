...
- **Group**: **Lifecycle** / **Messages**: StartStream, WorkerInitRequest/Response, WorkerTerminate / **Direction**: `StartStream` is worker → host, `WorkerInitRequest` is host → worker, `WorkerInitResponse` is worker → host, `WorkerTerminate` is host → worker
- **Group**: **Health checks** / **Messages**: WorkerStatusRequest/Response / **Direction**: Host → Worker
- **Group**: **Function loading** / **Messages**: FunctionLoadRequest/Response, FunctionsMetadataRequest, FunctionMetadataResponse / **Direction**: Host ↔ Worker
- **Group**: **Invocation** / **Messages**: InvocationRequest/Response, InvocationCancel / **Direction**: Host → Worker (response goes the other way)
- **Group**: **Operations** / **Messages**: RpcLog, FileChangeEventRequest, FunctionEnvironmentReloadRequest/Response, WorkerWarmupRequest/Response / **Direction**: Mixed
...
![All on one screen](https://raw.githubusercontent.com/yeongseon/tech-writing/cb179c5/assets/azure-functions-deep-dive/03/03-01-all-on-one-screen.en.png)
...
- **File**: `GrpcWorkerChannel.cs` / **Role**: The handle that represents one worker. The concrete flow is split between `StartWorkerProcessAsync()`, `SendWorkerInitRequest`, `SendInvocationRequest`, and `StopWorkerProcess()` across this class and the base `WorkerChannel`.
- **File**: [`WorkerChannel.cs`](https://github.com/Azure/azure-functions-host/blob/5e59423ba45491041d18224c3e72c168a4a5b7f7/src/WebJobs.Script.Grpc/Channel/WorkerChannel.cs) / **Role**: The shared base layered on top of gRPC
- **File**: [`GrpcWorkerChannelFactory.cs`](https://github.com/Azure/azure-functions-host/blob/5e59423ba45491041d18224c3e72c168a4a5b7f7/src/WebJobs.Script.Grpc/Channel/GrpcWorkerChannelFactory.cs) / **Role**: The factory that produces `GrpcWorkerChannel` instances
- **File**: [`GrpcCapabilities.cs`](https://github.com/Azure/azure-functions-host/blob/5e59423ba45491041d18224c3e72c168a4a5b7f7/src/WebJobs.Script.Grpc/Channel/GrpcCapabilities.cs) / **Role**: Constants for capability keys
- **File**: [`OrderedInvocationMessageDispatcher.cs`](https://github.com/Azure/azure-functions-host/blob/5e59423ba45491041d18224c3e72c168a4a5b7f7/src/WebJobs.Script.Grpc/Channel/OrderedInvocationMessageDispatcher.cs) / **Role**: Preserves ordering for messages that belong to the same `invocation_id`
...
![The channel layout — closer to per-worker `Channel<T>` pairs than a generic event bus](https://raw.githubusercontent.com/yeongseon/tech-writing/cb179c5/assets/azure-functions-deep-dive/03/03-02-the-channel-layout-closer-to-per-worker.en.png)
...
- [Host and Worker — who actually runs your functions (101 series, Part 3)](https://github.com/yeongseon/tech-writing/blob/cb179c5/content/azure-functions-101/en/03-host-and-worker.md)
