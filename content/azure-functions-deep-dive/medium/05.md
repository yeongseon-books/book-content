...
![Scaling decision points](https://raw.githubusercontent.com/yeongseon/tech-writing/cb179c5/assets/azure-functions-deep-dive/05/05-01-the-big-picture-where-scaling-decisions.en.png)
The key insight is that two different decisions are made in two different places.

- **Decision**: **Instance count** (scale-out) / **Decided by**: Scale Controller (outside the host) / **Signal**: ScaleMonitor / TargetScaler metrics + host health ping / **Result**: Move instances from N → N±k
- **Decision**: **Workers per instance** / **Decided by**: `WorkerConcurrencyManager` (inside the host) / **Signal**: Latency history from worker status responses / **Result**: Add worker processes on the same instance
...
![ScaleMonitor and TargetScaler — the signals triggers measure directly](https://raw.githubusercontent.com/yeongseon/tech-writing/cb179c5/assets/azure-functions-deep-dive/05/05-02-scalemonitor-and-targetscaler-the-signal.en.png)
...
- **Trigger**: Storage Queue / **Setting for target executions per instance**: `extensions.queues.batchSize` / **Default**: 16
- **Trigger**: Service Bus (single dispatch, v5+) / **Setting for target executions per instance**: `extensions.serviceBus.maxConcurrentCalls` / **Default**: 16
- **Trigger**: Service Bus (batch, v5+) / **Setting for target executions per instance**: `extensions.serviceBus.maxMessageBatchSize` / **Default**: 1000
- **Trigger**: Event Hubs (v5+) / **Setting for target executions per instance**: `extensions.eventHubs.maxEventBatchSize` / **Default**: 100
- **Trigger**: Cosmos DB / **Setting for target executions per instance**: `MaxItemsPerInvocation` (function attribute) / **Default**: 100
- **Trigger**: Apache Kafka / **Setting for target executions per instance**: `LagThreshold` (function attribute) / **Default**: 1000
...
- **Aspect**: Decided by / **Instance scale-out**: Scale Controller (external) / **Worker concurrency**: `WorkerConcurrencyManager` (inside the host)
- **Aspect**: Unit / **Instance scale-out**: VM instance / **Worker concurrency**: Worker process within an instance
- **Aspect**: Signal / **Instance scale-out**: ScaleMonitor metrics + health ping / **Worker concurrency**: Proportion of overloaded samples in `LatencyHistory`
- **Aspect**: Impact / **Instance scale-out**: Bill, cold-start time / **Worker concurrency**: Throughput within the instance
- **Aspect**: Scope / **Instance scale-out**: Varies by plan / **Worker concurrency**: Node / PowerShell / Python only; excludes HTTP worker; disabled when static process count is set
...
![Plan-by-plan — same code, different behavior](https://raw.githubusercontent.com/yeongseon/tech-writing/cb179c5/assets/azure-functions-deep-dive/05/05-03-plan-by-plan-same-code-different-behavio.en.png)
...
- **Scale group**: `http` / **Triggers included**: HTTP trigger, SignalR trigger
- **Scale group**: `blob` / **Triggers included**: Blob storage trigger (Event Grid–based)
- **Scale group**: `durable` / **Triggers included**: Orchestration / Activity / Entity trigger
- **Scale group**: `function:<NAMED_FUNCTION>` / **Triggers included**: Every other function (individually)
...
- **Plan**: Consumption / **Scale decided by**: Scale Controller / **Scale to zero**: Yes / **Max instances**: 200 / **Per-function**: No / **Always ready**: No / **VNet**: No
- **Plan**: Flex Consumption / **Scale decided by**: Scale Controller (new) / **Scale to zero**: Yes / **Max instances**: 1000 / **Per-function**: Yes / **Always ready**: Yes / **VNet**: Yes
- **Plan**: Premium / **Scale decided by**: Scale Controller (+ option) / **Scale to zero**: No (min 1) / **Max instances**: Varies by SKU/region / **Per-function**: No / **Always ready**: pre-warmed / **VNet**: Yes
- **Plan**: Dedicated / **Scale decided by**: App Service Auto-Scale / **Scale to zero**: No / **Max instances**: Depends on plan / **Per-function**: No / **Always ready**: Always On available / **VNet**: Yes
...
- [Dispatcher and Invocation — How a Function Call Reaches the Worker](https://github.com/yeongseon/tech-writing/blob/cb179c5/content/azure-functions-deep-dive/en/04-dispatcher-and-invocation.md)
