...
**Storage Queue**
- Setting for target executions per instance: `extensions.queues.batchSize`
- Default: 16

**Service Bus (single dispatch, v5+)**
- Setting for target executions per instance: `extensions.serviceBus.maxConcurrentCalls`
- Default: 16

**Service Bus (batch, v5+)**
- Setting for target executions per instance: `extensions.serviceBus.maxMessageBatchSize`
- Default: 1000

**Event Hubs (v5+)**
- Setting for target executions per instance: `extensions.eventHubs.maxEventBatchSize`
- Default: 100

**Cosmos DB**
- Setting for target executions per instance: `MaxItemsPerInvocation` (function attribute)
- Default: 100

**Apache Kafka**
- Setting for target executions per instance: `LagThreshold` (function attribute)
- Default: 1000
...
**Decided by**
- Instance scale-out: Scale Controller (external)
- Worker concurrency: `WorkerConcurrencyManager` (inside the host)

**Unit**
- Instance scale-out: VM instance
- Worker concurrency: Worker process within an instance

**Signal**
- Instance scale-out: ScaleMonitor metrics + health ping
- Worker concurrency: Proportion of overloaded samples in `LatencyHistory`

**Impact**
- Instance scale-out: Bill, cold-start time
- Worker concurrency: Throughput within the instance

**Scope**
- Instance scale-out: Varies by plan
- Worker concurrency: Node / PowerShell / Python only; excludes HTTP worker; disabled when static process count is set
...
**`http`**
- Triggers included: HTTP trigger, SignalR trigger

**`blob`**
- Triggers included: Blob storage trigger (Event Grid–based)

**`durable`**
- Triggers included: Orchestration / Activity / Entity trigger

**`function:<NAMED_FUNCTION>`**
- Triggers included: Every other function (individually)
