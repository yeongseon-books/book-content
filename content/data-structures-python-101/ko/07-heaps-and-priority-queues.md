...
힙의 핵심 통찰은 완전 이진 트리를 배열(list)로 표현할 수 있다는 사실입니다. 노드 포인터 없이도 부모-자식 관계를 인덱스 계산으로 파악합니다.
...
- **가장 작은 값이나 가장 큰 값을 빠르게 꺼내려면 어떤 구조가 필요할까요?**
  - 가장 작은 값이나 가장 큰 값을 빠르게 꺼내려면 어떤 구조가 필요할까요 — 본문에서 구체적으로 다룹니다.
- **힙은 왜 정렬보다 우선순위 처리에 유리할까요?**
  - `heapq`는 모듈 수준 함수로 list를 힙처럼 다룹니다. 여기서는 이를 감싸서 ADT 인터페이스를 제공하는 우선순위 큐를 구현합니다.
- **Python의 `heapq`는 왜 최소 힙만 제공할까요?**
  - Python의 `heapq`는 왜 최소 힙만 제공할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 현실의 많은 문제는 단순히 데이터를 저장하는 것이 아니라, 우선순위에 따라 처리하는 것입니다. 가장 급한 작업부터 실행해야 하거나, 가장 가까운 노드를 먼저 방문해야 하거나, 가장 이른 이벤트를 먼저 소비해야 하는 상황이 계속 등장합니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 실무에서는 `heapq`를 직접 쓰는 경우도 많지만, 스레드 안전한 `queue.PriorityQueue`나 비동기 환경의 `asyncio.PriorityQueue`를 더 자주 만날 수 있습니다.
- **적용 전후 비교의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 최솟값을 반복해서 꺼내는 문제를 정렬과 힙으로 각각 처리해 보겠습니다.
...
- **Data Structures with Python 101 (7/10): 힙과 우선순위 큐 (현재 글)**
