...
- **Python의 `list`는 배열일까요, 연결 리스트일까요?**
  - Python의 `list`는 배열일까요, 연결 리스트일까요 — 본문에서 구체적으로 다룹니다.
- **`append()`는 list가 계속 커져도 왜 대체로 빠를까요?**
  - `append()`는 list가 계속 커져도 왜 대체로 빠를까요 — 본문에서 구체적으로 다룹니다.
- **`insert(0, x)`와 중간 삭제는 왜 비쌀까요?**
  - `insert(0, x)`와 중간 삭제는 왜 비쌀까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - Python에서 가장 자주 만나는 자료구조는 list입니다. 그런데 자주 쓰는 만큼 내부 동작을 너무 당연하게 여기기 쉽습니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 대부분의 상황에서 list는 충분히 빠릅니다. 중요한 것은 list의 강점 구간 안에서 쓰고 있는지 아는 것입니다. 끝 append/pop, 인덱스 접근, 순차 순회가 많다면 list는 아주 좋은 기본값입니다.
- **적용 전후 비교의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 같은 “앞에 하나 넣기”라도 내부 구조가 다르면 비용이 크게 달라집니다.
...
- **Data Structures with Python 101 (2/10): 배열과 리스트 (현재 글)**
