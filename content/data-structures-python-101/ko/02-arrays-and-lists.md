...
**핵심 질문**: Python의 list는 배열인가요, 연결 리스트인가요?

> Python의 list는 내부적으로 동적 배열(dynamic array)로 구현됩니다. 인덱스 접근은 O(1)이지만, 중간 삽입·삭제는 O(n)입니다. 이 글에서는 배열과 리스트의 차이, Python list의 내부 동작, 그리고 성능 특성을 실습합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 배열(array)과 동적 배열(dynamic array)의 차이
- Python list의 내부 구현 원리
- list 연산별 시간 복잡도
- array 모듈과 list의 차이
...
- **Append 패턴** — 끝 추가·인덱스 접근에 최적화된 자료입니다.
- **Insert 비용** — 중간 삽입은 O(n)이라는 점을 잊지 않습니다.
- **리스트 컴프리헨션** — for-append 패턴을 컴프리헨션으로 단순화합니다.
- **얕은 복사** — 리스트 복사 시 참조 공유 함정을 인지합니다.
- **array.array** — 동질 수치 데이터는 array가 메모리에 유리합니다.
...
1. 10만 개의 원소가 있는 list에서 insert(0, x)를 1,000번 반복하는 시간과, deque.appendleft(x)를 1,000번 반복하는 시간을 측정하세요.
2. 2차원 리스트(5x5)를 올바르게 초기화하고, (2,3) 위치에 값을 넣었을 때 다른 행에 영향이 없는지 확인하세요.
3. list comprehension으로 1~100 중 3의 배수이면서 5의 배수가 아닌 수의 리스트를 만드세요.
