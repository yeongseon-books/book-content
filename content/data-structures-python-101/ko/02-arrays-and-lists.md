...
- `array` 모듈은 언제 list보다 나은 선택이 될까요?

> 멘탈 모델: Python의 `list`는 느슨한 “값 상자”가 아니라, 내부적으로 자동 확장되는 동적 배열입니다. 그래서 인덱스 접근은 빠르지만 앞 삽입은 기존 원소를 밀어야 합니다.
...
![리스트 capacity 증가를 그림으로 보면](https://yeongseon-books.github.io/book-public-assets/assets/data-structures-python-101/02/02-01-capacity.ko.png)

*논리 길이와 예약 capacity가 어떻게 다르고, spare slot이 다 떨어지면 더 큰 연속 메모리 블록으로 재할당이 일어나는지 보여 주는 그림*
...
### Step 2: `sys.getsizeof()`로 overallocation 관찰하기
...
### Step 4: slicing도 결국 복사라는 점 기억하기
...
### Step 5: `array`는 기본값이 아니라 특수 도구로 보기
