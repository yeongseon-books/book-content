...
- **파일 시스템, DOM, 조직도는 왜 트리 구조로 모델링될까요?**
  - 이진 트리의 가장 실용적인 형태인 이진 탐색 트리(BST)를 타입 힌트와 함께 구현합니다. BST는 왼쪽 자식 < 부모 < 오른쪽 자식이라는 불변식을 유지합니다.
- **트리의 루트, 리프, 깊이, 높이는 각각 무엇을 뜻할까요?**
  - 트리의 루트, 리프, 깊이, 높이는 각각 무엇을 뜻할까요 — 본문에서 구체적으로 다룹니다.
- **이진 트리 순회는 왜 재귀와 잘 맞을까요?**
  - 이진 트리의 가장 실용적인 형태인 이진 탐색 트리(BST)를 타입 힌트와 함께 구현합니다. BST는 왼쪽 자식 < 부모 < 오른쪽 자식이라는 불변식을 유지합니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 트리는 컴퓨터 과학에서 가장 널리 쓰이는 구조 중 하나입니다. 파일 시스템, HTML DOM, 데이터베이스 인덱스, AST까지 계층 관계를 표현해야 하는 곳에는 거의 항상 트리가 등장합니다.
- **트리 구조를 그림으로 보면을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 실무에서 BST를 처음부터 직접 구현할 일은 많지 않습니다. 대신 `sorted()`, `bisect`, 데이터베이스 인덱스, 라이브러리 내부 구조가 이미 그런 역할을 대신합니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
...
- **Data Structures with Python 101 (6/10): 트리와 이진 트리 (현재 글)**
