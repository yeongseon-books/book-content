...
- 삽입 순서 유지와 해시 테이블 구조는 어떻게 함께 성립할까요?

> 멘탈 모델: Python의 `dict`는 단순한 키-값 묶음이 아니라, 키를 해시해 sparse table 안의 probe path를 따라 찾는 해시 테이블입니다. 그리고 테이블이 너무 빽빽해지기 전에 resize해서 평균 성능을 지킵니다.
