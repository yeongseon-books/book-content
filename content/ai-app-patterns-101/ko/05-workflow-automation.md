...
> 워크플로 자동화는 모델의 선택권을 없애고, 사람이 정의한 단계와 데이터 계약을 따르는 파이프라인으로 바꾸는 방식입니다.

여러 단계를 거쳐야 하는 업무는 한 번의 프롬프트보다 단계별 계약을 먼저 설계하는 편이 훨씬 안정적입니다. 특히 중간 결과를 다음 단계가 그대로 받아야 한다면, 모델의 자유도보다 흐름의 예측 가능성이 더 중요해집니다.

이 글은 AI 앱 패턴 101 시리즈의 5번째 글입니다. 여기서는 고정 워크플로를 설계할 때 어떤 단계와 데이터 계약이 필요한지 구체적인 체인 예제로 정리합니다.
