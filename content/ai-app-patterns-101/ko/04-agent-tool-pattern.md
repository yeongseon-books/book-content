...
> 에이전트는 모든 단계를 미리 하드코딩하는 대신, 모델이 런타임에 도구 호출 경로를 고르게 하는 제어기입니다.

고정된 체인으로 풀 수 없는 문제를 만나면, 어느 시점에 어떤 도구를 호출할지 실행 중에 결정해야 할 때가 있습니다. 이때 핵심은 에이전트를 마법처럼 보는 것이 아니라, 모델에게 제한된 선택권을 주는 제어 구조로 이해하는 일입니다.

이 글은 AI 앱 패턴 101 시리즈의 4번째 글입니다. 여기서는 Agent + Tool 패턴이 왜 필요한지, 그리고 도구 선택을 어떻게 설계하고 디버깅해야 하는지 설명합니다.

## 이 글에서 다룰 문제

- 언제 고정 체인 대신 `AgentExecutor`를 쓰는 편이 합리적일까요?
- LLM이 계산기와 검색 도구 사이에서 올바르게 고를 수 있게 하려면 도구 설명을 어떻게 써야 할까요?
- 도구를 고르는 에이전트를 디버깅할 때 어떤 실행 추적을 봐야 할까요?
