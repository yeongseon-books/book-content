...
> RAG는 답을 외우는 모델이 아니라, 검색한 문서를 생성 전에 프롬프트에 주입하는 파이프라인입니다.

RAG를 이해할 때 중요한 점은 모델을 더 똑똑하게 만드는 일이 아니라, 필요한 문맥을 제때 붙여 넣는 일이라는 점입니다. 결국 품질은 검색된 문서가 얼마나 정확한지, 그리고 그 문서를 프롬프트에 어떻게 연결하는지에 달려 있습니다.

이 글은 AI 앱 패턴 101 시리즈의 2번째 글입니다. 여기서는 문서 검색과 답변 생성을 결합한 가장 작은 RAG Q&A 파이프라인을 단계별로 살펴봅니다.

## 이 글에서 다룰 문제

- 최소한의 RAG 파이프라인에서는 청킹, 임베딩, 검색, 답변 생성을 어떻게 연결해야 할까요?
- 작은 FAISS 예제로도 답변 텍스트와 근거 스니펫을 함께 반환할 수 있을까요?
- 답이 문서 안에 없을 때 환각을 막기 위한 설계는 어디에서 해야 할까요?
