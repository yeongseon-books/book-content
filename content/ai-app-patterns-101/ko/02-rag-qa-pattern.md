...
- RAG의 두 단계에서 가장 흔한 실수는 무엇일까요?
- 생성보다 먼저 검색을 검증할 때 실무에 적용할 때 주의할 점은 무엇일까요?
- RAG가 실패하는 순간의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
> 멘탈 모델은 검색 품질이 생성 품질의 상한선을 만든다는 사실입니다. 생성 모델은 검색이 가져온 근거 위에서만 안전하게 답할 수 있습니다.
...
RAG를 운영 환경으로 옮길 때 가장 중요한 차이는 "답변 문자열"만 반환하지 않는 데 있습니다. 호출자는 답변과 함께 근거 문서, 점수, 폴백 여부를 받아야 합니다. 아래 예시는 최소한의 계약을 보여 줍니다.
