...
> Human-in-the-loop는 자동화를 포기하는 방식이 아니라, 자동화가 위험한 지점에만 사람의 판단을 넣는 패턴입니다.

자동화 품질이 높아져도 모든 결정을 모델에 맡길 수는 없습니다. 비용이 큰 실수나 규정 준수 문제가 걸린 구간에서는, 사람 검토를 어디에 넣을지 설계하는 일이 모델 선택만큼 중요합니다.

이 글은 AI 앱 패턴 101 시리즈의 마지막 글입니다. 여기서는 Human-in-the-loop 패턴으로 자동화와 사람 판단을 어떤 기준으로 연결해야 하는지 살펴봅니다.
