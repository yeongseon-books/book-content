...
> 문서 어시스턴트는 대화형 시스템이 아니라, 긴 입력을 작업에 맞는 짧은 출력으로 바꾸는 변환기입니다.

문서 처리 작업은 겉으로 보면 각각 달라 보여도, 실제 구현에서는 긴 입력을 안정적으로 잘라서 규칙 있는 출력으로 바꾸는 문제가 반복됩니다. 그래서 요약, 추출, 분류를 따로 보지 말고 하나의 변환 파이프라인으로 이해하면 설계가 훨씬 단순해집니다.

이 글은 AI 앱 패턴 101 시리즈의 3번째 글입니다. 여기서는 문서 어시스턴트 패턴을 기준으로 요약, 정보 추출, 분류 작업을 한 흐름으로 정리합니다.

## 이 글에서 다룰 문제

- 문서가 한 번의 프롬프트에 담기기에는 너무 길 때, 어떤 체인 구조가 요약을 안정적으로 유지할까요?
- 요약 파이프라인에서 청크 단위 처리와 최종 종합은 어떻게 분리해야 할까요?
