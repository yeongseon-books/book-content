...
- 긴 문서를 한 번에 넣지 않고도 안정적으로 요약하려면 어떤 체인 구조가 필요할까요?
- 요약 체인에서 chunk별 처리와 최종 통합은 어떻게 분리하는 것이 좋을까요?
- 문서 어시스턴트 패턴이 챗봇보다 배치 처리에 잘 맞는 이유는 무엇일까요?
...
출력 결과
=== 경영진 요약 ===
파이썬은 2024년 설문조사에서 5년 연속으로 가장 인기 있는 프로그래밍 언어로 선정되었습니다. 이 중 67%의 응답자가 파이썬을 주 언어로 사용하며, 데이터 과학과 머신러닝을 지원하는 경우가 45%로 가장 많았습니다. 파이썬의 성능 향상과 지원이 강화되면서 파이썬 3.x 버전을 사용하는 유저가 89%로 압도적입니다.

=== 개발자 요약 ===
다음은 문서 요약:

* 2024년 파이썬 개발자 설문조사 결과에 따르면, 파이썬이 5년 연속으로 가장 인기 있는 프로그래밍 언어로 선정되었습니다.
* 파이썬 사용자는 67%이며, 그 중 45%는 데이터 과학과 머신러닝 업무에 파이썬을 사용합니다.
* 파이썬 3.12 버전은 전년 대비 성능이 25% 향상되었으며, 타입 힌트 지원이 강화되었습니다.
* 가장 많이 쓰는 프레임워크는 FastAPI(52%), Django(38%), Flask(34%) 순이었으며, 데이터 과학 분야에서는 pandas(78%), numpy(72%), scikit-learn(65%)이 상위를 차지했습니다.
* 파이썬 3.x를 사용하는 개발자 비율은 89%이며, 파이썬 2.x를 사용하는 개발자 비율은 2%에 불과합니다.
~~~
...
=== 최종 요약 ===
인공지능(AI)은 인간의 지적 능력을 컴퓨터로 구현하는 기술로, 1950년대 앨런 튜링이 시작한 분야에서 발전해 왔습니다. 머신러닝은 컴퓨터가 데이터에서 스스로 규칙을 학습하도록 하는 기술로, 대표적인 알고리즘에는 의사결정 트리, 랜덤 포레스트, 서포트 벡터 머신 등이 있습니다.

딥러닝은 머신러닝의 한 분야로, 인간 뇌의 신경망을 모방한 인공신경망을 사용하여 이미지 인식, 음성 인식, 자연어 처리 분야에서 혁신을 이끌고 있습니다. 대표적인 대규모 언어 모델(LLM)은 GPT, BERT, LLaMA 등으로, 이 모델들은 텍스트 생성, 요약, 번역, 코드 작성 등에 활용되며, 2023년 ChatGPT의 등장으로 일반 대중에게도 알려졌습니다.

하지만 AI의 미래에는 사회적으로 해결해야 할 도전과제도 있습니다. 예를 들어 설명 가능성, 편향성, 프라이버시, 에너지 소비, 일자리 대체 등이 있습니다. 하지만 동시에 의료, 기후 변화, 교육 등 중요한 문제를 해결하는 데 AI가 핵심 역할을 할 것으로 기대됩니다. AI는 앞으로 다양한 분야에서 혁신을 이끌며, 사회적으로도 새로운 기회와 도전을 불러올 것입니다.
~~~
...
~~~
출력 결과
텍스트: 파이썬 3.12에서 타입 힌트 성능이 크게 개선되었다. 특히 제네릭 타입 처리 속도가 25...
  카테고리: 기술/IT, 신뢰도: 1.00
  이유: 파이썬 3.12와 타입 힌트 성능 개선에 대한 언급

텍스트: 3분기 영업이익이 전년 동기 대비 15% 증가했다. 해외 시장 확대가 주요 요인이다....
  카테고리: 비즈니스/경영, 신뢰도: 1.00
  이유: 영업이익 증가와 해외 시장 확대에 대한 언급

텍스트: 규칙적인 유산소 운동이 심혈관 질환 위험을 30% 낮춘다는 연구 결과가 발표됐다....
  카테고리: 의료/건강, 신뢰도: 1.00
  이유: 텍스트에서 심혈관 질환과 유산소 운동이 언급되어 건강과 관련된 내용임을 알 수 있습니다.

텍스트: 챔피언스리그 결승에서 레알 마드리드가 맨체스터 시티를 2-1로 꺾고 우승했다....
  카테고리: 스포츠, 신뢰도: 1.00
  이유: 텍스트에서 챔피언스리그 결승, 레알 마드리드, 맨체스터 시티, 우승이라는 단어들이 포함되어 있어 스포츠 카테고리에 분류할 수 있다.
~~~
...
- `main.py`는 map 단계와 reduce 단계를 명시적으로 분리해 긴 문서를 여러 번 호출로 처리합니다.
- 각 청크 요약을 먼저 출력하므로 어디서 정보가 빠졌는지 확인하기 쉽습니다.
- 같은 패턴을 요약뿐 아니라 추출과 분류 배치 작업에도 그대로 확장할 수 있습니다.
...
- [ ] 긴 문서가 여러 청크로 분할된다
- [ ] 청크마다 개별 요약이 생성된다
- [ ] 개별 요약을 다시 합쳐 최종 요약을 만든다
- [ ] 최종 요약 프롬프트가 중복 제거와 흐름 유지 역할을 맡는다
