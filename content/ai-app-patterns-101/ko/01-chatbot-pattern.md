...
> 챗봇은 모델이 기억하는 시스템이 아니라, 애플리케이션이 누적된 메시지 목록을 계속 다시 보내는 루프입니다.

챗봇을 처음 만들면 모델이 이전 대화를 알아서 기억할 것처럼 느끼기 쉽습니다. 하지만 실제로는 애플리케이션이 매 요청마다 어떤 문맥을 다시 보낼지 결정해야 하고, 그 선택이 비용과 품질을 함께 좌우합니다.

이 글은 AI 앱 패턴 101 시리즈의 첫 번째 글입니다. 여기서는 가장 단순한 멀티턴 챗봇을 출발점으로 삼아, 대화 이력과 상태를 어떻게 관리해야 하는지 설명합니다.

## 이 글에서 다룰 문제

- 챗봇 애플리케이션은 왜 대화 이력을 직접 들고 다녀야 할까요?
- 누적된 메시지를 사용해 멀티턴 챗봇을 만드는 가장 작은 동작 패턴은 무엇일까요?
- 세션 이력을 언제 메모리에 두고, 언제 외부 저장소로 옮겨야 할까요?
