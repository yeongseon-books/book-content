...
```python
# 3_lsp.py
# inheritance hierarchy를 의심해 보세요.
```
...
- **설계 원칙은 외워야 하는 규칙일까요, 진단 도구일까요?**
  - 이 글의 결론은 설계 원칙이 암기 목록보다 코드 냄새를 읽는 진단 도구에 가깝다는 사실입니다. 거대한 클래스, 분기 체인, 하위 타입 예외, 거대한 인터페이스, 도메인의 DB import 같은 증상을 봤을 때 어떤 질문을 꺼낼지 정해 주기 때문입니다.
- **SRP, OCP, LSP, ISP, DIP는 각각 어떤 냄새에 반응할까요?**
  - `validation + storage + email + analytics`가 한 클래스에 몰리면 SRP, `if-elif` 체인이 늘어나면 OCP, 하위 타입이 예외를 던지면 LSP, 읽기 전용 구현이 write를 억지로 품으면 ISP, 도메인이 DB를 직접 알면 DIP를 먼저 떠올리면 됩니다. 글의 표처럼 냄새와 원칙을 한 줄로 연결할 수 있어야 실제 리뷰에서 바로 쓸 수 있습니다.
- **KISS와 YAGNI는 언제 구조를 단순하게 붙잡아 줄까요?**
  - 작은 스크립트에 과한 추상화와 계층을 미리 넣고 싶어질 때 KISS와 YAGNI가 제동 장치 역할을 합니다. DRY도 비슷해 보여서 섣불리 합치기보다, 정말 변화 이유가 같은 코드인지 먼저 확인하라는 경고로 읽는 편이 맞습니다.
...
- **Software Design 101 (9/10): 설계 원칙 모음 (현재 글)**
