...
- **좋은 코딩과 좋은 설계는 무엇이 다를까요?**
  - 좋은 코딩이 함수 이름, 포매팅, 지역적인 가독성에 더 가깝다면 좋은 설계는 다음 변경의 비용을 낮추는 구조적 판단에 가깝습니다. 이 글에서 본 `files_touched`, 의존성 그래프, 모듈 책임, 고립 테스트 가능성 같은 신호는 코드가 아니라 구조를 평가하는 기준입니다.
- **설계 품질은 어떤 신호로 판단할 수 있을까요?**
  - 글에서는 변경 시뮬레이션으로 수정 파일 수를 세고, `A <-> B` 같은 순환 의존이 있는지 보고, 모듈을 한 문장으로 설명할 수 있는지 확인하자고 제안했습니다. 여기에 `can_test_alone()`처럼 IO 없이 도메인을 검증할 수 있는지, 새 팀원이 30분 안에 구조를 읽을 수 있는지도 함께 봐야 합니다.
- **설계 실패는 코드베이스에서 어떤 증상으로 드러날까요?**
  - 결제 수단 하나를 추가하는데 여러 폴더를 모두 열어야 하거나, 작은 수정인데 통합 테스트 전체에 기대야 하거나, 새 팀원이 모듈 설명을 오래 못 한다면 설계 실패 신호입니다. 이런 증상은 보통 책임 경계가 흐리고 의존성 방향이 뒤엉켜 있다는 뜻입니다.

<!-- toc:begin -->
## 시리즈 목차

- **Software Design 101 (1/10): 소프트웨어 설계란 무엇인가? (현재 글)**
