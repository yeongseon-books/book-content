...
- **계층을 왜 나누고, 무엇을 기준으로 나눌까요?**
  - 계층은 UI, 유스케이스, 도메인 규칙, 인프라가 서로 다른 속도와 이유로 바뀌기 때문에 나눕니다. 라우터 안에 검증, 업무 규칙, DB 호출이 함께 있던 코드를 `charge_view()`와 `charge_use_case()`로 분리한 예시가 그 기준을 잘 보여 줍니다.
- **각 계층은 어떤 책임을 가져야 할까요?**
  - 표현 계층은 HTTP 입력과 응답, 애플리케이션 계층은 유스케이스 조율, 도메인 계층은 `Wallet.debit()` 같은 핵심 규칙, 인프라 계층은 `SqlWalletRepo` 같은 저장 세부를 맡아야 합니다. 외부 SaaS 응답은 `to_domain_user()` 같은 부패 방지 계층에서 번역한 뒤 도메인으로 들여보내는 편이 안전합니다.
- **의존성은 어떤 방향으로만 흘러야 할까요?**
  - 바깥의 표현·인프라가 안쪽의 도메인을 향해 의존하고, 도메인은 웹 프레임워크나 ORM 세부를 모르는 방향이 바람직합니다. 도메인 모델에 ORM 데코레이터가 늘어나거나 라우터가 업무 규칙을 품기 시작하면 그 방향이 무너지고 있다는 신호입니다.
...
- **Software Design 101 (6/10): 계층 아키텍처 (현재 글)**
