...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 의존성 방향은 왜 변경 비용을 크게 좌우할, 둘째 안정적인 모듈과 변동이 큰 모듈은 어떻게 구분할, 셋째 도메인이 세부 구현을 모르게 만드는 방법은 무엇일입니다. 전체 그림에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **의존성 방향은 왜 변경 비용을 크게 좌우할까요?**
  - 의존성 방향은 왜 변경 비용을 크게 좌우할까요 — 본문에서 단계별로 설명합니다.
- **안정적인 모듈과 변동이 큰 모듈은 어떻게 구분할까요?**
  - 안정적인 모듈과 변동이 큰 모듈은 어떻게 구분할까요 — 본문에서 단계별로 설명합니다.
- **도메인이 세부 구현을 모르게 만드는 방법은 무엇일까요?**
  - 도메인이 세부 구현을 모르게 만드는 방법은 무엇일까요 — 본문에서 단계별로 설명합니다.
- **전체 그림에서 가장 흔한 실수는 무엇일까요?**
  - 핵심 아이디어는 간단합니다. 세부 구현이 코어를 향하게 하고, 코어는 자신이 필요한 모양만 추상으로 선언합니다. 그러면 구현 교체가 코어를 직접 흔들지 않습니다.
- **기본 용어을 실무에 적용할 때 주의할 점은 무엇일까요?**
