...
- **작은 프로젝트는 어디서부터 설계를 시작해야 할까요?**
  - 이 글의 URL 단축기 예제처럼 작은 프로젝트도 먼저 도메인과 유스케이스 중심으로 흐름을 스케치하는 편이 좋습니다. `shorten_view()`를 얇게 두고, 입력은 유스케이스로 넘기고, 저장과 키 생성은 바깥으로 미루는 순간 구조가 한 번에 정리됩니다.
- **왜 도메인부터 쓰는 편이 좋을까요?**
  - `ShortLink.create()` 안의 URL 형식 검증처럼 오래 남을 규칙을 먼저 고정하면 웹 프레임워크나 저장소가 바뀌어도 핵심 판단은 그대로 유지됩니다. 그래서 URL 검증 테스트에 Flask가 필요하다면 이미 규칙이 도메인 밖으로 새어 나간 상태라고 볼 수 있습니다.
- **저장소와 키 생성 같은 인프라 세부를 어떻게 막아 둘 수 있을까요?**
  - `LinkRepo`, `KeyGen` 포트를 유스케이스 쪽에서 정의하고, `InMemoryLinkRepo`와 `md5_key` 같은 구현은 어댑터로 붙이면 됩니다. 실제 조립은 composition root에서 한 번만 하고, 이후에는 SQL 저장소나 다른 키 생성 전략으로 바꿔도 `shorten_use_case()`는 거의 손대지 않게 만드는 것이 목표입니다.
