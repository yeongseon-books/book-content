...
```python
# 2_domain_infra.py
# Domain은 IO를 전혀 알지 못합니다.
def calculate_total(items, member): ...
# Infra가 domain을 사용합니다.
...
```python
# 5_seam.py
# 분리된 concern이 만나는 지점(seam)을 점검하세요.
def app(req):
...
- **관심사란 정확히 무엇일까요?**
  - 이 글에서 관심사는 시스템이 신경 써야 하는 하나의 주제이며, 주문 처리 예시에서는 입력 파싱, 도메인 규칙, 저장, 알림, 응답 변환이 각각 다른 관심사였습니다. 같은 함수 안에 있더라도 바뀌는 이유가 다르면 분리 후보로 봐야 합니다.
- **한 모듈이 너무 많은 일을 하는지 어떻게 알아낼 수 있을까요?**
  - 가격 정책 변경, DB 스키마 변경, 알림 채널 변경처럼 변경 이유를 적어 봤을 때 한 모듈에 세 가지 이상이 함께 나오면 이미 책임이 섞였을 가능성이 큽니다. `parse(req)`, `handle(cmd)`, `render(res)`처럼 입력·처리·출력을 분해해 보면 어디가 과하게 비대해졌는지 바로 드러납니다.
- **결합도와 응집도는 관심사 분리와 어떤 관계가 있을까요?**
  - 관심사 분리가 잘되면 같은 목적을 향하는 코드가 한곳에 모여 응집도는 높아지고, 서로 다른 이유로 바뀌는 코드가 분리되어 결합도는 낮아집니다. 반대로 로깅 정책 변경이 도메인 규칙까지 흔들린다면 횡단 관심사가 안으로 새어 들어온 상태라고 볼 수 있습니다.

<!-- toc:begin -->
## 시리즈 목차

- [Software Design 101 (1/10): 소프트웨어 설계란 무엇인가?](./01-what-is-software-design.md)
- **Software Design 101 (2/10): 관심사 분리 (현재 글)**
