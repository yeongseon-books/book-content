...
관심사 분리는 파일을 많이 쪼개는 일이 아니라, '다른 이유로 바뀌는 책임'을 다른 경계로 나누는 일입니다 — 한 함수 안에서 같이 바뀌어야 응집이고, 함께 바뀔 이유가 없는데 묶여 있으면 결합입니다. 이 글에서는 전체 그림부터 현업 적용 관점에서 다시 정리까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
