...
이 그림에서는 데이터 흐름 설계를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> 데이터 흐름 설계의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **데이터 흐름을 설계한다는 말은 구체적으로 무엇일까요?**
  - 본문의 기준은 데이터 흐름 설계를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **입력과 출력 사이를 왜 한 방향으로만 흐르게 해야 할까요?**
