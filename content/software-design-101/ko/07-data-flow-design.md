...
**핵심 질문**: 데이터의 흐름을 설계한다는 게 무슨 뜻인가요?

> 데이터가 어디서 와서, 어떻게 변환되고, 어디로 가는지를 한 방향으로 흐르게 만드는 것입니다.
...
- **Pipeline**: 작은 변환 함수의 체인.
- **Pure function**: 같은 입력이면 같은 출력, 부수효과 없음.
- **Immutability**: 한 번 만들어진 값을 바꾸지 않는다.
- **Push model**: 생산자가 소비자에게 밀어넣는다.
- **Pull model**: 소비자가 필요한 만큼 당겨 간다.
...
1. 본인 코드의 한 함수에서 부수효과와 변환을 분리해 보세요.
2. dict로 다루던 입력을 dataclass로 바꿔 보세요.
3. 흐름이 거꾸로 가는 지점을 찾아 메모해 두세요.
