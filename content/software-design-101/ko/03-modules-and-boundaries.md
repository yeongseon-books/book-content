...
- **좋은 모듈 경계는 어떤 조건을 갖춰야 할까요?**
  - 좋은 경계는 공개 표면은 작고 내부 책임은 깊으며, 변하기 쉬운 결정을 안쪽에 숨겨 외부 변경을 차단해야 합니다. `__all__ = ["read_file"]`처럼 진입점을 줄이고, DTO로 내부 모델 노출을 막고, 의존성을 한 방향으로 두는 조건이 함께 맞아야 합니다.
- **깊은 모듈과 얕은 모듈은 무엇이 다를까요?**
  - 얕은 모듈은 `open_file`, `read_chunk`, `close_file`처럼 내부 절차를 호출자에게 그대로 떠넘기고, 깊은 모듈은 `read_file(path)` 하나로 그 복잡도를 안에서 흡수합니다. 호출자가 적게 알고도 많은 일을 하게 만드는 쪽이 깊은 모듈입니다.
- **공개 API는 어디까지 드러내야 할까요?**
  - 공개 API는 외부 호출자가 실제로 알아야 하는 최소 약속까지만 드러내야 합니다. 내부 dict 구조나 캐시 구현 선택까지 밖으로 새기 시작하면 DTO와 캡슐화가 무너져 작은 내부 변경도 호출자 수정으로 번집니다.
