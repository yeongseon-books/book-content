...
좋은 모듈은 기능을 숨기는 것이 아니라 복잡도를 안으로 가두고 바깥에는 작은 약속만 드러냅니다 — 깊은 모듈일수록 인터페이스는 좁고 구현은 풍부하며, 얕은 모듈일수록 인터페이스가 구현을 그대로 노출해 경계가 사라집니다. 이 글에서는 전체 그림부터 현업 적용 관점에서 다시 정리까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
