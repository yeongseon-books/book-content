...
- **무상태 함수가 상태 있는 비즈니스를 어떻게 처리할까요?**
  - 무상태 함수는 상태를 없애는 것이 아니라, 세션과 처리 이력을 함수 바깥 저장소에서 읽고 갱신하는 방식으로 비즈니스를 처리합니다. `with_session()`이 외부 캐시에서 상태를 꺼내고 다시 저장하며, `use_token()`이 이미 처리한 요청을 막는 흐름이 그 핵심입니다.
- **세션, 캐시, 데이터 저장소, 워크플로 상태는 어디에 둬야 할까요?**
  - 글에서는 짧은 세션성 데이터는 Redis 같은 캐시, 영속 데이터는 데이터베이스, 여러 단계 진행 상태는 상태 머신이나 워크플로 엔진으로 분리하라고 정리했습니다. 같은 저장소 하나에 전부 몰아넣기보다 접근 패턴과 수명, 정합성 요구에 맞춰 나누는 편이 운영에 유리합니다.
- **TTL과 멱등 토큰은 왜 상태 관리의 핵심일까요?**
  - TTL은 캐시뿐 아니라 멱등 키 테이블의 `expiresAt`처럼 중복 방지 상태의 수명을 제어해 비용과 정합성을 함께 관리하게 해 줍니다. 멱등 토큰이 없으면 재시도 때 중복 반영이 생기고, TTL이 없으면 오래된 토큰과 상태가 쌓여 저장 비용과 충돌 판단 품질이 함께 나빠집니다.
