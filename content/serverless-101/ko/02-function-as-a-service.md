...
- 오늘 사용할 예제 계약의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **FaaS의 실행 계약은 핸들러, 런타임, 패키지 사이에서 어떻게 만들어질까요?**
  - 이 글의 실행 계약은 `app.handler` 같은 핸들러 진입점, Python 3.12 런타임, 그리고 실제로 ZIP 안에 들어가는 파일 집합이 함께 맞아야 성립합니다. 핸들러 코드가 맞아도 런타임 버전이나 패키지 내용이 어긋나면 호출 자체가 달라지므로, FaaS는 코드만이 아니라 배포 단위를 통째로 다루는 모델입니다.
- **실제 배포 전에는 어떤 파일 구조와 명령 순서로 검증하는 편이 좋을까요?**
  - `app.py`, `smoke_test.py`, `requirements.txt`를 분리한 뒤 `pip install -r requirements.txt -t package`, `cp app.py package/`, `zip -r ../function.zip .`, `python3 smoke_test.py` 순서로 확인하는 흐름이 본문 핵심입니다. 이 순서를 따르면 같은 이벤트로 로컬 성공 여부를 먼저 고정하고, 이후 차이는 패키지나 런타임 환경 문제로 좁혀 볼 수 있습니다.
- **패키지 크기와 의존성 수는 왜 실행 시간과 콜드 스타트에 직접 영향을 줄까요?**
  - `function.zip` 크기와 내부 파일 목록을 확인하라고 한 이유는 초기화 전에 로드해야 할 코드와 라이브러리가 많아질수록 지연이 커지기 때문입니다. 그래서 메모리 값을 올리기 전에 먼저 불필요한 의존성을 줄이고, `requests`처럼 꼭 필요한 패키지만 남긴 뒤 다시 측정하는 루프가 더 효과적입니다.

<!-- toc:begin -->
## 시리즈 목차

- [Serverless 101 (1/10): 서버리스란 무엇인가?](./01-what-is-serverless.md)
- **Serverless 101 (2/10): 함수형 서비스(FaaS)란 무엇인가? (현재 글)**
