...
- **함수의 첫 호출은 왜 느릴까요?**
  - 첫 호출이 느린 이유는 새 실행 환경을 만들고 런타임을 초기화하고 코드와 의존성을 로드하는 비용이 한 번에 붙기 때문입니다. 본문의 `INIT_MS` 측정 코드와 `BOOT_TIME`, `load_model_once()` 예시는 이 지연이 비즈니스 로직 전에 이미 시작된다는 점을 보여 줍니다.
- **콜드 스타트는 어떤 단계들의 합으로 생길까요?**
  - 글에서는 실행 환경 생성, 런타임 초기화, 패키지와 import 로딩, 핸들러 밖 초기화 코드를 합친 값으로 설명했습니다. 그래서 대형 의존성 제거, 전역 클라이언트 재사용, 프로비저닝 동시성 선택 같은 대응도 각 단계의 병목에 맞춰 달라져야 합니다.
- **평균이 아니라 p99를 봐야 하는 이유는 무엇일까요?**
  - 웜 호출이 대부분이면 평균은 좋아 보여도, 몇 번의 콜드 스타트가 로그인이나 결제 같은 민감 경로의 체감 지연을 크게 올릴 수 있습니다. 그래서 본문은 `p50`, `p95`, `cold p95` 비교표를 두고, 특히 꼬리 지연과 프로비저닝 비용을 함께 보라고 강조했습니다.
