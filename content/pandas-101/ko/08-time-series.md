...
- **날짜 열을 인덱스로 두면 무엇이 달라질까요?**
  - 날짜를 `DatetimeIndex`로 올리면 시간 구간 선택, 리샘플링, 이동 계산이 모두 Pandas의 기본 문법으로 연결됩니다. `ts.loc["2026-01-03":"2026-01-06"]`처럼 문자열 슬라이싱만으로 기간을 자를 수 있는 점이 가장 큰 변화입니다.
- **리샘플링은 단순 집계와 어떤 차이가 있을까요?**
  - 리샘플링은 단순히 값을 합치는 것이 아니라 시간 축의 단위를 바꿔 다시 묶는 연산입니다. `ts.resample("3D").sum()`이나 `df.resample("W")["value"].sum()`처럼 일별 데이터를 3일 단위나 주간 단위로 재구성하는 것이 일반 집계와 다릅니다.
- **이동 평균 같은 창 기반 계산은 어떻게 할까요?**
  - `rolling(window=3).mean()`처럼 이동 창을 만든 뒤 그 창마다 통계를 계산하면 됩니다. 본문에서 7일 이동 평균과 경계 `NaN`, 그리고 `shift()` 이후 값 해석까지 함께 언급한 것은 창 기반 계산이 추세를 부드럽게 보게 해 주지만 경계 조건을 반드시 같이 봐야 하기 때문입니다.
