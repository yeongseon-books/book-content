...
**핵심 질문**: *결측치* 는 *제거할 대상* 일까요, *모델링할 정보* 일까요?

> *결측치는 *데이터의 메시지* 입니다. *왜 비었는지* 를 모르면 *어떻게 채울지* 도 모릅니다.*
...
- **NaN**: *Not a Number* — *float* 결측 표시.
- **NA / pd.NA**: *Pandas의 통합* 결측 마커 (nullable dtypes).
- **dropna**: 결측치가 있는 *행/열 제거*.
- **fillna**: *고정값/평균/직전값* 으로 *채움*.
- **interpolate**: *보간* — 시계열에 적합.
...
1. *결측 비율* 을 *열별로* 출력하세요.
2. *dropna* 전후 *행 수 변화* 를 출력하세요.
3. *시계열 결측* 을 *ffill* 과 *interpolate* 로 처리하고 *결과 차이* 를 보이세요.
