...
분석 속도는 수십~수백 배까지 차이날 수 있습니다. 벡터화는 Pandas의 본질이고, `apply` 남용은 가장 흔한 안티패턴입니다.
...
3. **`eval/numexpr` 없이 거대한 표현식을 직접 작성.**
4. ***map* 의 *NaN 발생* 을 무시.**
5. ***dtype 불일치* 로 *벡터화 실패* 후 object dtype.**

## 실무에서는 이렇게 쓰입니다

ETL 변환, 피처 엔지니어링, 대규모 리포트에서 벡터화는 비용과 시간을 직접 줄여 줍니다. 클라우드 비용을 줄이는 가장 쉬운 레버이기도 합니다.
