...
- 다양한 DataFrame 생성법의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **시리즈는 내부적으로 어떤 구조일까요?**
  - 시리즈는 값 배열만 있는 것이 아니라 `values`, `index`, `name`을 함께 가진 1차원 구조입니다. 본문에서 `pd.Series([1.0, 2.0, 3.0], index=["a", "b", "c"], name="x")`를 만들고 이 속성을 직접 출력한 이유가 바로 이 구조를 눈으로 확인하기 위해서입니다.
- **데이터프레임을 열 중심으로 본다는 말은 무엇을 뜻할까요?**
  - 데이터프레임의 각 열이 독립적인 시리즈라는 뜻이므로, `df["x"]`를 꺼내면 곧바로 시리즈 연산 규칙으로 넘어갑니다. 그래서 `df["x"]`는 Series를 반환하고, 열별 자료형 관리나 벡터화 계산도 열 단위로 자연스럽게 이어집니다.
- **인덱스는 왜 단순한 행 번호가 아닐까요?**
  - Pandas는 위치가 아니라 레이블을 먼저 맞춘 뒤 계산하므로 인덱스는 계산 기준 자체입니다. `s1 + s2` 예제에서 `a`, `b`, `c`, `d` 레이블을 정렬한 뒤 겹치지 않는 곳에 `NaN`이 생기는 결과가 그 점을 분명하게 보여 줍니다.
