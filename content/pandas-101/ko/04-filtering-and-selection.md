...
- 인덱싱 방법 비교의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **`loc`와 `iloc`는 언제 구분해서 써야 할까요?**
  - 이름이 기준이면 `loc`, 숫자 위치가 기준이면 `iloc`를 써야 합니다. 본문에서도 `df.loc[["a", "c"], "x"]`는 레이블을 읽기 좋게 드러내고, `df.iloc[0:2, 0]`은 순수한 위치 슬라이싱이라는 점을 대비해 보여 줍니다.
- **조건 마스크는 어떤 상황에서 가장 자연스러울까요?**
  - 행 값 자체가 조건을 만족하는지 골라야 할 때 조건 마스크가 가장 직접적입니다. `df[df["x"] > 1]`나 `(df["x"] > 1) & (df["y"] < 30)`처럼 불리언 Series를 만들면 어떤 행이 남는지 코드에서 바로 읽힙니다.
- **표현식이 길어질수록 `query`가 왜 읽기 쉬워질까요?**
  - 조건이 길어질수록 대괄호와 괄호가 많이 겹치는데, `df.query("x > 1 and y < 30")`는 이를 문장처럼 펼쳐 줍니다. 여기에 `isin([1, 3])`까지 함께 쓰면 긴 OR 체인보다 의도가 더 선명하게 남습니다.
