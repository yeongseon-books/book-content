...
**Before**: `df[조건]`만 사용해서 체이닝 인덱싱 경고가 납니다.

**After**: 의도에 맞는 도구를 골라 `loc`/`iloc`/`query`로 명확히 분리합니다.
...
- `query`는 표현식이 길어질수록 읽기 쉽습니다.

## 자주 하는 실수 5가지

1. **`and/or` 사용 → 오류.** `&/|`와 괄호가 필요합니다.
...
KPI 대시보드, 이상치 탐지, A/B 테스트 분리처럼 조건 기반 선택은 분석 함수의 핵심입니다. 팀 코드 표준으로 `loc` 사용을 강제하기도 합니다.
