...
- **Pandas는 정확히 어떤 문제를 해결하는 라이브러리일까요?**
  - Pandas는 CSV나 Excel처럼 표 형태로 들어온 데이터를 메모리 안의 DataFrame으로 올려 읽고, 점검하고, 변형하는 기본 작업을 짧은 코드로 처리하게 해 주는 도구입니다. 본문에서 `pd.read_csv`, `shape`, `dtypes`, `describe()`를 바로 이어 쓰는 흐름이 그 역할을 보여 줍니다.
- **Series와 DataFrame은 어떤 관계로 이해해야 할까요?**
  - Series는 인덱스를 가진 1차원 구조이고, DataFrame은 그런 열 여러 개를 한 표로 묶은 2차원 구조입니다. 예제에서도 `pd.Series([10, 20, 30], index=["a", "b", "c"])`로 시리즈를 만든 뒤 `name`, `age` 열을 가진 DataFrame으로 확장하면서 이 관계를 보여 줍니다.
- **왜 많은 분석 작업이 Pandas에서 시작될까요?**
  - 대부분의 실무 데이터가 결국 표 형태로 도착하므로, 먼저 크기와 자료형을 확인하고 조건 필터링까지 한 자리에서 처리할 수 있는 Pandas가 자연스러운 출발점입니다. `df[df["age"] > 40]`, `sales.groupby("product")["quantity"].mean()` 같은 예제가 이후 집계와 전처리까지 같은 문법으로 이어진다는 점도 이유입니다.
