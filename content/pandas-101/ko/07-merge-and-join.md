...
- **왜 Pandas에는 `merge`와 `join`이 둘 다 있을까요?**
  - `merge`는 열을 기준으로 관계를 명시해 합칠 때 중심이 되고, `join`은 인덱스를 기준으로 붙일 때 더 간단한 문법을 제공합니다. 이 글이 `users.merge(orders, on="uid")`처럼 키 열을 분명히 드러내는 예제로 설명한 것도, 초반에는 어떤 열이 연결 기준인지 명시하는 편이 안전하기 때문입니다.
- **안쪽, 왼쪽, 오른쪽, 바깥쪽, 교차 조인은 어떻게 다를까요?**
  - 차이는 어떤 키를 결과에 남길지에 있습니다. 본문에서 `how="left"`는 `users`의 모든 행을 보존했고, `how="outer", indicator=True`는 양쪽 키를 모두 남기면서 `_merge` 열로 `both`와 `left_only`를 구분해 보여 주었습니다.
- **중복 키가 있을 때 왜 행 수가 갑자기 늘어날까요?**
  - 조인은 같은 키의 가능한 조합을 모두 만들기 때문에 `orders`처럼 `uid=1`이 두 번 있으면 `users`의 한 행이 여러 행으로 복제됩니다. 그래서 본문은 `validate="one_to_one"`로 예외를 내 보이고, 병합 전후 행 수와 키 중복을 반드시 점검하라고 강조했습니다.
