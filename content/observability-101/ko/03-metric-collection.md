...
- 카디널리티 폭발 방지에서 가장 흔한 실수는 무엇일까요?
- 한눈에 보는 구조을 실무에 적용할 때 주의할 점은 무엇일까요?
- Pull vs Push 방식 비교의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
문제는 메트릭을 잘못 수집하거나 잘못 그리면 오히려 오해가 커진다는 사실입니다. 카운터를 그대로 선 그래프로 그리거나, 라벨을 과하게 붙이거나, 1초마다 무리하게 수집하면 숫자는 많아져도 판단은 더 느려집니다. 그래서 수집과 시각화는 함께 배워야 합니다.
...
- **메트릭은 어떻게 수집되고 그래프로 바뀔까요?**
  - 본문에서 본 것처럼 애플리케이션이 카운터·게이지·히스토그램 값을 메모리에 누적하면, 수집 에이전트나 시계열 DB가 일정 주기로 그 값을 끌어가 시점·라벨과 함께 저장합니다. 저장된 시점-값-라벨 묶음을 PromQL 같은 질의로 집계하면 비로소 대시보드 위 그래프가 됩니다.
- **pull 방식과 push 방식은 무엇이 다를까요?**
  - 본문 비교 예시처럼 pull은 Prometheus가 `/metrics` 엔드포인트를 주기적으로 긁어 가는 방식이라 수집 주기와 서비스 디스커버리를 수집 측이 통제하고, push는 애플리케이션이 StatsD·OTel Collector·Pushgateway로 값을 밀어 보내는 방식이라 단기 실행 작업이나 외부망 너머 워크로드에 적합합니다. 그래서 둘은 "장수명 서비스 vs 단기 작업" 같은 워크로드 성격에 맞춰 선택합니다.
- **`/metrics` 엔드포인트는 어떤 역할을 할까요?**
  - 본문에서 강조했듯이 `/metrics`는 애플리케이션이 현재 카운터·게이지 값을 Prometheus exposition 형식의 텍스트로 노출하는 표준 접점입니다. 이 엔드포인트가 있어야 수집기가 별도 SDK 통합 없이 HTTP GET 한 번으로 메트릭을 가져갈 수 있고, 그래서 사이드카·라이브러리 어느 쪽으로 구현하든 동일한 수집 파이프라인을 그대로 쓸 수 있습니다.
...
- **Observability 101 (3/10): 메트릭 수집과 시각화 (현재 글)**
