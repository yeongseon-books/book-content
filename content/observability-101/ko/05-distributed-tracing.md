...
- 샘플링은 왜 비용 통제의 핵심일까요?
- 트레이싱 용어 정리에서 가장 흔한 실수는 무엇일까요?
- OpenTelemetry로 HTTP 문맥 전파할 때 실무에 적용할 때 주의할 점은 무엇일까요?
...
- **요청이 여러 서비스를 지날 때 문맥 전파는 왜 중요할까요?**
  - 분산 트레이싱의 핵심은 서비스 사이에서 trace_id를 끊지 않고 전달하는 것입니다. 아래는 OpenTelemetry로 HTTP 헤더를 통해 문맥을 전파하는 예제입니다.
- **샘플링은 왜 비용 통제의 핵심일까요?**
  - 샘플링은 왜 비용 통제의 핵심일까요 — 본문에서 구체적으로 다룹니다.
- **트레이싱 용어 정리에서 가장 흔한 실수는 무엇일까요?**
  - 분산 트레이싱에는 몇 가지 핵심 용어가 있습니다. 이 용어를 정확히 이해하면 트레이스 화면을 읽는 속도가 빨라집니다.
- **OpenTelemetry로 HTTP 문맥 전파할 때 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 분산 트레이싱의 핵심은 서비스 사이에서 trace_id를 끊지 않고 전달하는 것입니다. 아래는 OpenTelemetry로 HTTP 헤더를 통해 문맥을 전파하는 예제입니다.
...
- **Observability 101 (5/10): 분산 트레이싱 기초 (현재 글)**
