...
- Project Summary의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **GitHub 저장소, README, 실행 방법, 데모 링크는 왜 모두 중요할까요?**
  - 1. python -m venv .venv
- **코드만 올려 두는 것과 설명 가능한 결과물을 공개하는 것은 무엇이 다를까요?**
  - 아래 원칙은 전공 주제가 달라도 공통으로 적용됩니다. 첫째, 매주 하나의 "관찰 가능한 결과"를 남겨야 합니다.
- **이 글에서 배울 것에서 가장 흔한 실수는 무엇일까요?**
  - 지원 단계에서는 눈에 보이는 결과가 있어야 대화가 시작됩니다. 이력서 한 줄보다 저장소와 README, 데모 링크가 훨씬 더 많은 정보를 담고, 문제 해결 방식과 협업 태도까지 보여 줍니다.
- **바뀌는 지점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - - 포트폴리오의 정의 - GitHub 활용 - README 작성 - 문서화 패턴 - 공개의 의미 지원 단계에서는 눈에 보이는 결과가 있어야 대화가 시작됩니다
- **Project Summary의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 대학생 시간표 충돌을 찾아 주는 Flask 기반 웹 도구입니다.
