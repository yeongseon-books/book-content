...
이 그림에서는 컴퓨터학과에서는 무엇을 배우는가를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> 컴퓨터학과에서는 무엇을 배우는가의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **컴퓨터학과 4년 과정은 어떤 큰 축으로 이해하면 좋을까요?**
  - 본문의 기준은 컴퓨터학과에서는 무엇을 배우는가를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **수학, 프로그래밍, 시스템, 데이터, AI, 프로젝트는 왜 따로 떨어진 과목이 아니라 이어진 흐름일까요?**
