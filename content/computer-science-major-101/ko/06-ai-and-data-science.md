...
이 그림에서는 AI와 데이터사이언스를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> AI와 데이터사이언스의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **AI와 데이터사이언스는 전공 안에서 어떻게 나뉘고 어떻게 연결될까요?**
  - 본문의 기준은 AI와 데이터사이언스를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **통계, 머신러닝, 딥러닝, 데이터 분석은 어떤 흐름으로 이어질까요?**
