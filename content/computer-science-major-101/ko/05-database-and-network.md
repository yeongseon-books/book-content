...
인덱스 추가 전후를 수치로 비교하는 습관은 학습 초기부터 들이는 것이 좋습니다. 아래는 실제 튜닝 작업을 기록하는 간단한 템플릿입니다.

| 항목 | 인덱스 추가 전 | 인덱스 추가 후 |
|---|---|---|
| 실행 계획 | Full Table Scan | Index Range Scan |
| 100만 건 기준 평균 응답 | 4.8초 | 0.03초 |
| p95 응답 시간 | 7.2초 | 0.08초 |
| 쓰기 비용 변화 | 기준 | INSERT 약 5% 증가 |

이 표를 코드 리뷰나 설계 문서에 포함하면 "인덱스가 필요한가?"라는 질문을 느낌이 아닌 숫자로 결정할 수 있습니다.
