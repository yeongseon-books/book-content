...
- **데이터베이스와 네트워크는 왜 거의 모든 서비스의 바닥에 놓일까요?**
  - 서비스 관점에서 데이터베이스와 네트워크는 분리된 과목이 아니라 한 요청의 앞뒤 구간입니다. 클라이언트 요청이 네트워크를 통해 들어오고, 애플리케이션이 비즈니스 규칙을 적용한 뒤 데이터베이스에서 상태를 읽거나 갱신하고, 다시 응답이 네트워크를 통해 반환됩니다.
- **SQL, 테이블, 인덱스는 실제 성능과 어떻게 연결될까요?**
  - 작은 메모리 데이터베이스를 열고 테이블을 만듭니다
- **TCP/IP와 HTTP는 어떤 층에서 역할을 나눌까요?**
  - - 관계형 데이터베이스의 의미 - SQL의 역할 - TCP/IP 모델 - HTTP의 위치 - 두 과목이 만나는 지점 백엔드 코드의 상당수는 결국 데이터베이스와 네트워크를 다룹니다
- **이 글에서 배울 것에서 가장 흔한 실수는 무엇일까요?**
  - 백엔드 코드의 상당수는 결국 데이터베이스와 네트워크를 다룹니다. 많은 장애와 지연도 저장 계층과 통신 계층에서 함께 시작되기 때문에, 두 과목을 따로 배우더라도 머릿속에서는 한 흐름으로 묶어 이해하는 편이 좋습니다.
- **바뀌는 지점을 실무에 적용할 때 주의할 점은 무엇일까요?**
