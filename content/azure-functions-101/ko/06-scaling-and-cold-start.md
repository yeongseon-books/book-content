...
이 그림에서는 스케일링과 콜드 스타트 — 서버리스가 빨라지는 순간과 느려지는 순간를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.
...
- **Functions scale controller는 어떤 신호를 보고 인스턴스를 추가할까요?**
  - 본문의 기준은 스케일링과 콜드 스타트 — 서버리스가 빨라지는 순간과 느려지는 순간를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **콜드 스타트는 정확히 어느 단계에서 발생하고, 무엇을 측정해야 볼 수 있을까요?**
