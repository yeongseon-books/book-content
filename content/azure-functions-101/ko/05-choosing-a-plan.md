...
- 플랜별 스케일 상한은 어느 지점에서 실제 피크 트래픽과 부딪힐까요?
- 월간 비용 시뮬레이션을 현실적으로 하려면 어떤 가정을 먼저 고정해야 할까요?
...
## 플랜 선택을 이해하는 가장 좋은 방법: 비용, warm capacity, 운영 모델의 조합으로 보는 것입니다
...
| 플랜 | 한 줄 정의 |
|---|---|
| **Consumption** | 가장 단순한 종량제 서버리스 플랜입니다. 역사적으로 오래된 경로이고 Windows 중심으로 보는 편이 안전합니다. |
...
|---|---|---|---|---|
| **현재 위치** | 오래된 경로 | 새 서버리스 기본 후보 | 고급 서버리스 | 일반 App Service 계열 |
...
- VNet 통합 부재
- 1.5 GB 고정 메모리
- Linux 가용성 제약과 장기적 권장 방향 변화

즉 Consumption은 여전히 유효하지만, “아무 제약 없는 새 서버리스 앱의 당연한 기본값”은 아닙니다. 현재 기준에서는 먼저 Flex를 보고, 필요 시 Consumption으로 내려오는 흐름이 더 자연스럽습니다.
...
3. **Consumption은 단순한 데모나 기존 자산 유지에 더 가깝습니다.**
