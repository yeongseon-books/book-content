...
### Host/Worker 계층별 장애 분류와 로그 탐색 순서

Host와 Worker를 구분해서 로그를 보는 습관은 현장에서 즉시 효과가 납니다. 아래 표는 증상별 1차 탐색 순서를 정리한 것입니다.

| 관찰 증상 | 1차 탐색 층 | 확인 내용 |
|---|---|---|
| 트리거 감지 자체가 안 됨 | Host | 바인딩 확장 로드 오류, 연결 문자열 오류 |
| 트리거는 감지되나 코드가 안 돌아감 | Worker | import 오류, 언어 런타임 버전 불일치 |
| 함수가 실행되나 결과가 이상함 | 사용자 코드 | 로직 오류, 직렬화 문제 |
| Host 재시작 반복 | Host 헬스 체크 | 메모리 초과, 구성 오류 |
| Worker 재시작 반복 | Worker 프로세스 | OOM, 무한 루프, unhandled exception |

이 표를 런북에 넣어두면 "함수가 안 된다"는 신고를 받은 후 30초 안에 어느 층 로그를 먼저 볼지 결정할 수 있습니다.
...
- **Host 로그와 Worker 로그는 다른 계층입니다.** 문제 층을 먼저 좁혀야 탐색 시간이 줄어듭니다.
...
- [ ] 계층별 장애 분류 표를 런북에 포함했습니다.
- [ ] 로컬에서 Host/Worker 두 프로세스 분리를 `ps` 명령으로 한 번 확인했습니다.
...
- **Functions Host와 언어 Worker는 왜 같은 프로세스가 아니라 분리된 프로세스일까요?**
  - Host가 V8, CPython, JVM 같은 언어 런타임을 모두 직접 품으면 충돌과 유지 비용이 커집니다. 분리된 구조에서는 Host가 언어 중립 프로토콜(gRPC+Protobuf)만 유지하면 되고, 새 언어 지원은 호스트 변경 없이 Worker 패키지 추가로 가능합니다. 동시에 언어 런타임 장애가 Host 전체를 무너뜨리지 않도록 격리도 됩니다.
- **Host와 Worker 사이의 gRPC 채널에서는 어떤 메시지 흐름이 오갈까요?**
  - Host가 트리거 이벤트를 감지하면 `InvocationRequest`를 만들어 Worker에 보냅니다. Worker는 사용자 함수를 실행하고 결과를 `InvocationResponse`로 돌려줍니다. 양방향 스트림 위에서 `invocation_id`로 요청과 응답을 상관 지어 여러 호출이 동시에 in-flight 상태로 존재할 수 있습니다.
- **한 Worker 프로세스는 동시에 몇 개의 함수 호출을 처리할 수 있을까요?**
  - 언어와 설정에 따라 다릅니다. Python 동기 함수는 thread pool에서 병렬 실행 가능하고, async 함수는 단일 event loop를 공유합니다. `FUNCTIONS_WORKER_PROCESS_COUNT`를 높이면 Worker 프로세스 자체를 여러 개 띄울 수 있습니다. `maxConcurrentRequests`, `batchSize` 같은 설정도 호출 병렬도에 영향을 줍니다.
