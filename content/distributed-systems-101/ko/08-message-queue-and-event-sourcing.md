...
- 운영 시나리오: consumer lag과 재생 복구의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **메시지 큐는 어떤 결합 해소와 전달 보장을 제공할까요?**
  - 앞선 실습이 큐와 이벤트 소싱의 핵심 감각을 잡는 단계였다면, 이 절은 실제 서비스 설계 문서에 들어가는 판단 기준을 다룹니다.
- **at-most-once, at-least-once, exactly-once는 각각 무엇을 뜻할까요?**
  - at-most-once, at-least-once, exactly-once는 각각 무엇을 뜻할까요 — 본문에서 구체적으로 다룹니다.
- **이벤트 소싱은 무엇이며 CQRS와는 어떤 관계일까요?**
  - 가장 단순한 메모리 큐입니다. 이후 단계는 이 아이디어를 분산 환경으로 확장합니다.
- **적용 전후 비교에서 가장 흔한 실수는 무엇일까요?**
  - 각 소비자는 자기 속도로 일하고, 잠시 죽어도 큐가 메시지를 붙들고 있습니다.
...
- **Distributed Systems 101 (8/10): 메시지 큐와 이벤트 소싱 (현재 글)**
