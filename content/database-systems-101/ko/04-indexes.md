...
- 인덱스가 읽기를 빠르게 만드는 대신 무엇을 희생할까요?

> **멘탈 모델**: 인덱스는 “값 → 행 위치”를 미리 정렬해 둔 별도 자료 구조입니다. 그래서 풀스캔은 O(N)이고 인덱스 조회는 대체로 O(log N)입니다. 다만 이 구조는 공짜가 아닙니다. 모든 쓰기마다 인덱스도 함께 갱신해야 하고, 선택성이 낮으면 옵티마이저는 일부러 무시합니다.
