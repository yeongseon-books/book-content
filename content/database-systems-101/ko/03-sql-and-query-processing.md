...
SELECT * FROM orders o WHERE EXISTS (
    SELECT 1 FROM users u WHERE u.id = o.user_id AND u.name = 'Alice'
);
```

세 쿼리는 보통 같은 결과를 냅니다. 다만 옵티마이저가 셋을 같은 계획으로 바꿀 수도 있고 아닐 수도 있습니다. EXPLAIN으로 직접 확인하는 것이 유일한 확실한 방법입니다.
...
아래 네 가지 신호를 먼저 확인하는 습관만으로도 쿼리 장애 대응 속도가 빨라집니다.
...
-- 경고 신호 1: 예상 행 수 오차
Bitmap Heap Scan on orders
  (rows=5 estimated, rows=12000 actual) -- 통계 갱신 필요

-- 경고 신호 2: 풀스캔 반복
Seq Scan on orders (actual time=0.030..842.112 rows=120000)

-- 경고 신호 3: 디스크 스필
Sort  (actual time=423.1..489.3 rows=200000)
  Sort Method: external merge  Disk: 14352kB  -- 메모리 부족

-- 경고 신호 4: Nested Loop + 대량 스캔
Nested Loop (actual rows=12000 loops=500) -- 조인 순서 재검토 필요
```
...
| 실수 | 증상 | 올바른 접근 |
|------|------|-------------|
| EXPLAIN도 보지 않고 쿼리가 느리다고 단정 | 증거 없는 튜닝, 반복 실패 | 항상 EXPLAIN ANALYZE부터 실행 |
| 인덱스를 만들고 안심한다 | 옵티마이저가 인덱스를 무시 | 인덱스 추가 후 EXPLAIN으로 채택 여부 확인 |
| `SELECT *`를 습관처럼 쓴다 | 네트워크·메모리·캐시 비용 누적 | 필요한 컬럼만 명시 |
| 애플리케이션 루프 안에서 SQL 반복 호출 | N+1 쿼리, 응답 시간 폭증 | JOIN이나 IN절로 한 번에 조회 |
| DDL과 DML을 한 트랜잭션에 섞는다 | 엔진별 동작 차이, 운영 위험 | DDL은 별도 트랜잭션으로 분리 |

## 핵심 요약

- 같은 SQL도 **데이터 양과 통계**에 따라 전혀 다른 계획으로 실행될 수 있습니다.
- 인덱스를 만들었다고 끝이 아닙니다. 통계가 낡거나 선택성이 낮으면 옵티마이저는 인덱스를 무시할 수 있습니다.
- `EXPLAIN`은 추정치 중심이고, `EXPLAIN ANALYZE`는 실제 실행 수치를 함께 보여줍니다.
...
- **Database Systems 101 (3/10): SQL과 쿼리 처리 (현재 글)**
