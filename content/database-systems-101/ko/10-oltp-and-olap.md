...
- **OLTP와 OLAP 워크로드의 근본 차이는 무엇일까요?**
  - OLTP는 `SELECT * FROM orders WHERE user_id=7`처럼 짧은 인덱스 점프와 빠른 갱신이 핵심인 운영 처리입니다. OLAP는 `GROUP BY country` 같은 대규모 스캔과 집계가 중심이어서, 같은 주문 데이터라도 요구하는 저장 방식과 실행 환경이 전혀 다릅니다.
- **행 저장과 컬럼 저장은 어떤 트레이드오프를 가질까요?**
  - 행 저장은 주문 한 건처럼 한 레코드를 통째로 읽고 쓰는 작업에 유리해서 운영 DB 기본값으로 잘 맞습니다. 컬럼 저장은 DuckDB와 Parquet 예시처럼 `country`, `status`, `total`처럼 필요한 컬럼만 읽어 집계 속도와 압축 효율을 크게 높이지만, 단일 행 UPDATE에는 불리합니다.
- **데이터 웨어하우스와 ETL/ELT는 왜 필요한가요?**
  - 운영 DB에서 직접 분석 쿼리를 돌리면 긴 집계가 캐시와 리소스를 잡아먹어 주문·결제 같은 트랜잭션을 방해합니다. 그래서 ETL/ELT로 운영 데이터를 별도 웨어하우스로 옮기고, 거기서 스타 스키마나 컬럼 저장 구조로 분석에 맞게 다시 모델링하는 것입니다.
