...
동기 복제와 비동기 복제는 "어느 쪽이 더 좋은가"보다 "어떤 상황에 맞는가"로 선택해야 합니다.

| 항목 | 동기 복제 | 비동기 복제 |
|---|---|---|
| COMMIT 완료 시점 | Replica 확인 후 | Primary 기록 후 즉시 |
| 데이터 손실 위험 | 거의 없음 | Replica 지연만큼 손실 가능 |
| 쓰기 지연 | 네트워크 왕복 추가 | 없음 |
| Replica 장애 영향 | 전체 쓰기 지연 | 없음 |
| 적합한 워크로드 | 금융, 트랜잭션 중심 | 분석, 읽기 복제, 지역 분산 |

실무에서는 동기 Replica 1개 + 비동기 Replica 1개를 혼합하는 패턴이 많습니다. 동기 Replica는 데이터 보호를 담당하고, 비동기 Replica는 읽기 부하 분산을 담당합니다.
