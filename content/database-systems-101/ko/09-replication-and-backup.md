...
중요한 것은 장애 대응을 즉흥적으로 하지 않는다는 사실입니다. 페일오버 훈련과 백업 복원 훈련은 정기 일정으로 운영되어야 합니다. “백업이 있다”는 말은 “최근에 실제로 복원했다”는 사실이 동반될 때만 믿을 수 있습니다.
...
- **Primary-Replica 복제는 어떻게 동작하고 각 노드는 무슨 역할을 할까요?**
  - Primary는 쓰기를 받고 WAL을 생성하는 원본 노드이고, Replica는 `pg_basebackup`으로 만든 베이스 상태 위에서 그 WAL 스트림을 계속 따라갑니다. 그래서 레플리카는 읽기 분산과 장애 전환 후보가 되며, `pg_last_xact_replay_timestamp()` 같은 지표로 복제 지연을 계속 감시해야 합니다.
- **동기 복제와 비동기 복제는 무엇을 주고받을까요?**
  - 동기 복제에서는 `synchronous_commit = on`처럼 COMMIT이 레플리카의 WAL 수신 확인을 기다리므로 데이터 손실 위험이 줄어듭니다. 비동기 복제는 Primary가 먼저 커밋을 끝내므로 지연은 짧지만, 장애 순간에는 아직 전달되지 않은 마지막 변경을 잃을 수 있습니다.
- **전체 백업, 증분 백업, WAL 기반 PITR은 어떻게 다를까요?**
  - 전체 백업은 특정 시점 데이터 전체를 한 번에 저장하는 기준 스냅샷이고, 증분 백업은 그 이후 바뀐 부분만 따로 보관해 저장 비용과 시간을 줄이는 방식입니다. WAL 기반 PITR은 이런 기준 백업에 WAL 아카이브를 이어 붙여 `recovery_target_time`까지 재생함으로써, 잘못된 `DELETE` 직전 같은 임의 시점으로 되돌리는 복구 경로를 제공합니다.
