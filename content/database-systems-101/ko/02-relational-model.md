...
- **테이블에서 말하는 관계, 튜플, 속성은 각각 무엇을 가리킬까요?**
  - 이 글에서는 `users`와 `orders` 같은 테이블 하나를 관계, 그 안의 한 행을 튜플, `email`이나 `price` 같은 컬럼을 속성으로 설명했습니다. 즉 표 모양 자체보다 같은 속성 집합을 가진 사실들의 집합으로 읽는 관점이 관계형 모델의 출발점입니다.
- **기본키와 외래키는 정확히 무엇을 보장할까요?**
  - 기본키는 `users.id`처럼 각 행이 유일하게 식별되도록 보장하고, `UNIQUE`가 붙은 `email`은 중복 사용자 생성을 막습니다. 외래키는 `orders.user_id REFERENCES users(id)`처럼 존재하지 않는 사용자를 가리키는 주문을 거절하고, 참조 중인 사용자를 삭제하려 할 때도 실패시켜 참조 무결성을 지킵니다.
- **NULL과 무결성 제약은 어떤 의미를 가질까요?**
  - NULL은 값이 비어 있다는 뜻이 아니라 그 속성의 값이 없거나 아직 정해지지 않았다는 의미이므로, 습관적으로 허용하면 모델 의미가 흐려집니다. 그래서 `NOT NULL`, `CHECK (price >= 0)`, `UNIQUE`, 외래키 같은 제약으로 데이터베이스가 애플리케이션보다 먼저 잘못된 상태를 막아야 합니다.
