...
이 글은 Data Science 101 시리즈의 5번째 글입니다.
...
- 평가 지표 비교을 실무에 적용할 때 주의할 점은 무엇일까요?
- 파이썬 classification_report 예제의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
EDA는 데이터를 설득하기보다 먼저 듣는 시간입니다. 이 단계를 통해 데이터의 모양을 읽어야 이후 모델링과 해석도 제대로 서게 됩니다. 다음 글에서는 이렇게 읽은 내용을 다른 사람도 빠르게 이해할 수 있도록 시각화하는 방법을 봅니다.
...
- **모델을 만들기 전에 데이터의 모양을 어떻게 빠르게 읽을 수 있을까요?**
  - 이 글의 EDA 루프는 `df.shape`, `df.dtypes`, `df.head()`로 전체 윤곽을 보고, `describe()`와 히스토그램으로 분포를 확인한 뒤, 범주형 고유값과 산점도, 결측률과 상관행렬까지 차례로 읽는 방식입니다. 즉, 한 번에 깊게 파지 않고 데이터 크기·분포·관계·결측을 빠르게 훑는 절차가 핵심입니다.
- **평균 하나만 보면 왜 자주 오해하게 될까요?**
  - 평균은 긴 꼬리, 치우침, 이상치를 숨기기 쉬워서 전형값처럼 보이지만 실제 분포를 잘못 대표할 수 있습니다. 그래서 본문은 `df["amount"].describe()`만 보지 말고 histogram과 분위수, boxplot 감각을 함께 보라고 강조했습니다.
- **분포, 결측 패턴, 상관관계는 어떤 순서로 보는 편이 좋을까요?**
  - 먼저 1차원 분포로 값의 모양을 읽고, 그다음 `df.isna().mean()`으로 비어 있는 부분을 확인한 뒤, 마지막에 `df.select_dtypes("number").corr().round(2)`로 변수 관계를 보는 편이 안전합니다. 분포도 모른 채 상관계수부터 보면 이상치나 결측 패턴이 만든 가짜 관계를 곧바로 오해하기 쉽습니다.
...
- **Data Science 101 (5/10): 탐색적 데이터 분석 (현재 글)**
