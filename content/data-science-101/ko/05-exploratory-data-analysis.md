...
이 순서를 추천하는 이유는 단순합니다. 먼저 크기와 타입을 보고, 다음에 단일 변수 분포를 읽고, 그다음 변수 간 관계를 보고, 마지막에 결측과 상관을 함께 점검해야 전체 그림이 그려지기 때문입니다.
...
EDA는 이 개념들을 실제 데이터에 붙여 보는 시간입니다. 예를 들어 cardinality가 매우 큰 범주형 컬럼은 이후 원-핫 인코딩에서 바로 비용 문제로 이어질 수 있습니다.
...
1. **평균만 보고 결론을 냅니다.** 분포의 모양을 놓치기 쉽습니다.
2. **상관관계를 인과로 읽습니다.** 가장 흔한 해석 오류 중 하나입니다.
3. **전체 데이터를 그대로 시각화합니다.** 시간과 메모리를 불필요하게 씁니다.
4. **cardinality를 무시합니다.** 이후 인코딩 비용이 폭발합니다.
5. **결측이 MCAR라고 가정합니다.** 근거 없는 가정이 편향으로 이어질 수 있습니다.
