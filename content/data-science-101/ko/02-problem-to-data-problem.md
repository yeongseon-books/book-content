...
문제를 데이터 문제로 바꾸기를 운영 시스템 속에서 올바르게 배치하려면 어떤 경계에서 무엇을 입력받고, 어디에서 검증하며, 어떤 신호를 남겨야 하는지 먼저 봐야 합니다.
...
- **“왜 매출이 떨어졌지?” 같은 질문은 왜 그대로는 답하기 어려울까요?**
  - 본문의 기준은 문제를 데이터 문제로 바꾸기를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **지표, 기간, 대상 집단은 왜 문제 정의의 핵심일까요?**
