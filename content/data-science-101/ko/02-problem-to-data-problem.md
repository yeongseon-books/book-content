...
실무에서 데이터 작업이 느려지는 가장 흔한 이유는 도구 부족이 아닙니다. 질문이 흐릿한 상태로 출발했기 때문입니다. “매출이 떨어진 것 같아요”, “이탈이 늘었어요”, “이번 캠페인이 효과 있었나요?” 같은 질문은 모두 중요하지만, 그대로는 데이터가 답하기 어렵습니다. 무엇을 측정할지, 어느 기간을 볼지, 누구를 대상으로 할지 빠져 있기 때문입니다.

그래서 데이터 분석의 절반은 분석이 아니라 프레이밍입니다. 질문을 다시 써서 데이터가 답할 수 있는 형태로 바꾸는 일입니다. 이 글에서는 막연한 비즈니스 질문을 측정 가능한 데이터 질문으로 바꾸는 5단계 틀을 정리하겠습니다.

## 왜 중요한가

질문이 흐릿하면 어떤 데이터를 가져와야 할지도 정할 수 없습니다. 팀마다 다른 기간을 보고, 다른 대상 집단을 보고, 다른 지표를 보게 되면 같은 문제를 놓고도 전혀 다른 결론이 나옵니다. 반대로 문제를 한 문장으로 정확히 쓰면 쿼리도 빨라지고, 분석 범위도 줄고, 팀 간 오해도 크게 줄어듭니다.
...
이 흐름은 생각보다 강력합니다. 질문이 막연할수록 바로 데이터를 파기 쉽지만, 실제로는 먼저 지표를 고르고, 비교할 기간을 정하고, 어떤 집단을 볼지 한정해야 합니다. 마지막에는 “맞거나 틀릴 수 있는 문장”으로 다시 써야 데이터가 답하기 시작합니다.
...
특히 가설은 반증 가능해야 합니다. “우리는 성장하고 있다”는 문장은 방향은 있지만 데이터 질문으로는 약합니다. 반면 “최근 30일 유료 구독자의 월 매출이 이전 30일보다 5% 이상 감소했다”는 문장은 측정도 가능하고 틀렸는지도 확인할 수 있습니다.

## Before / After

**Before**: “왜 매출이 떨어졌지?”라는 질문을 받고 어디서부터 봐야 할지 막막합니다. 사람마다 떠올리는 지표도 다르고, 비교 구간도 다릅니다.
...
1. **지표를 맨 마지막에 고릅니다.** 그러면 분석이 계속 흔들립니다.
2. **팀마다 다른 기간을 씁니다.** 같은 문제를 놓고도 불공정한 비교가 됩니다.
3. **대상 집단이 중간에 바뀝니다.** 추세가 섞여 원인을 읽기 어려워집니다.
4. **반증 불가능한 가설을 씁니다.** 데이터가 답할 수 없는 질문이 됩니다.
5. **여러 질문을 한 번에 묻습니다.** 답이 섞여 무엇이 원인인지 흐려집니다.
