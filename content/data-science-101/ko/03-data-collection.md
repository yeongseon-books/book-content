...
데이터 분석이라고 하면 모델링이나 시각화부터 떠올리기 쉽습니다. 하지만 실제로는 수집 단계에서 이미 결과의 절반이 갈립니다. 누가 언제 어떤 경로로 데이터를 가져왔는지 모르면 같은 분석을 다시 할 수 없고, 결과를 검증하기도 어렵습니다. 팀원이 보내 준 엑셀 파일 하나로 시작한 분석이 나중에 왜 신뢰를 잃는지도 대개 여기서 시작됩니다.
...
실무에서는 원본에서 바로 분석하지 않고, 대개 스테이징 단계를 둡니다. 원본은 그대로 보존하고 분석이나 정제는 복사본에서 진행합니다. 이 분리가 있어야 원복이 쉽고 반복 실행도 단순해집니다.
...
이 다섯 가지를 구분해 두면 수집 단계가 훨씬 선명해집니다. 특히 provenance를 남기는 습관은 수집 작업을 일회성 스크립트가 아니라, 나중에 다시 검증할 수 있는 과정으로 바꿔 줍니다.
