...
- 이상치는 왜 바로 삭제하지 말고 먼저 표시해야 할까요?
- 정제 과정을 문서화하지 않으면 무엇이 문제일까요?

> Data Science 101 시리즈 (4/10)

많은 사람이 데이터 작업의 핵심을 모델링이라고 생각하지만, 실제 시간을 가장 많이 잡아먹는 단계는 대개 정제입니다. 수집한 데이터를 그대로 쓰는 일은 거의 없습니다. 문자열이어야 할 값이 숫자로 들어오기도 하고, 날짜가 문자로 저장되기도 하고, 동일한 사용자가 여러 번 들어오거나, 결측치가 조용히 섞여 들어오기도 합니다.

정제는 지루한 사전 작업처럼 보일 수 있지만, 사실상 분석의 보험입니다. 입력이 불안정하면 그 위에 쌓은 시각화, 지표, 모델도 모두 흔들립니다. 이 글에서는 실무에서 가장 자주 만나는 네 가지 품질 문제와, 이를 점검하는 기본 순서를 정리하겠습니다.
...
정제 순서를 두는 이유가 있습니다. 타입이 틀리면 결측치도 제대로 보이지 않고, 중복이 섞이면 이상치 판단도 왜곡됩니다. 그래서 보통은 타입 정리부터 시작해 중복, 결측, 이상치 순으로 점검합니다.
...
이 용어들은 단순한 이론이 아니라 정제 의사결정의 기본 단위입니다. 예를 들어 결측치는 무조건 채우는 것이 아니라, 왜 비었는지 보고 버릴지 채울지 결정해야 합니다.
...
중복은 단순히 지우는 것으로 끝내면 아쉬울 때가 많습니다. 왜 중복이 생겼는지 알아야 다음 수집 단계에서 같은 문제가 반복되지 않기 때문입니다. 정제는 임시 수리이고, 원인 추적은 별도 과제라는 관점이 필요합니다.
...
이상치는 무조건 제거 대상이 아닙니다. 데이터 오류일 수도 있지만, 오히려 중요한 신호일 수도 있습니다. 그래서 보통은 먼저 표시하고, 그다음 도메인 맥락에서 검토합니다.
...
정제가 끝났다면 무엇이 얼마나 바뀌었는지 남겨야 합니다. 행 수가 얼마나 줄었는지, 어떤 컬럼의 결측이 얼마나 남았는지, 이상치 비율이 얼마인지 적어 두면 다음 실행과 비교하기가 쉬워집니다.
...
1. **결측치를 무조건 `0`으로 채웁니다.** 평균과 분포가 쉽게 왜곡됩니다.
2. **중복을 조용히 지웁니다.** 원인을 배우지 못해 같은 문제가 반복됩니다.
3. **이상치를 즉시 삭제합니다.** 실제 중요한 신호를 버릴 수도 있습니다.
4. **타입 변환 실패를 무시합니다.** 실패 지점을 드러내지 않으면 오류가 숨어 버립니다.
5. **정제 과정을 기록하지 않습니다.** 재현 가능한 분석이 아니게 됩니다.
