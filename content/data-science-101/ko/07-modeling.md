...
이 구조를 머릿속에 두면 모델링이 훨씬 단순해집니다. 먼저 데이터를 나누고, 같은 조건에서 기준선과 실제 모델을 각각 평가해 비교하면 됩니다.
...
이 다섯 개를 이해하면 입문 수준의 모델링에서 생기는 큰 실수 대부분을 피할 수 있습니다. 특히 Pipeline은 전처리 누수를 막는 데 매우 중요합니다.
...
이 단계는 단순하지만 매우 중요합니다. “가장 흔한 답만 찍는 모델”보다 나은지조차 확인하지 않고 복잡한 모델로 가면, 성능 해석이 쉽게 틀어집니다.
...
1. **베이스라인 없이 시작합니다.** 무엇이 개선인지 판단할 기준이 없습니다.
2. **테스트 데이터로 전처리 통계를 학습합니다.** 대표적인 데이터 누수입니다.
3. **accuracy만 봅니다.** 클래스 불균형 상황에서는 특히 위험합니다.
4. **`random_state`를 고정하지 않습니다.** 재현성이 무너집니다.
5. **한 번의 분할 결과로 결론 냅니다.** 운을 실력으로 착각하기 쉽습니다.
