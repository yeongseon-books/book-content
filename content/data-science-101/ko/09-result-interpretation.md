...
- **숫자 결과를 어떻게 의사결정 문장으로 바꿀 수 있을까요?**
  - 이 글의 5단계처럼 원 수치, 신뢰구간, 세그먼트 맥락, 마지막 행동 제안을 한 문장으로 묶으면 됩니다. 예를 들어 `conversion 3.2% vs 3.6%`, `delta = +0.4pp`, `paid users`, `roll out`을 한 문단에 적는 순간 숫자가 결정 문장으로 바뀝니다.
- **왜 숫자와 맥락은 항상 함께 적어야 할까요?**
  - 같은 2%p 상승도 누구에게, 언제, 어떤 환경에서 나온 수치인지 모르면 과대일반화가 쉽게 생깁니다. 그래서 본문은 `campaign window: 2 weeks; segment: paid users; device: desktop only`처럼 숫자 옆에 기간·세그먼트·환경을 함께 적도록 했습니다.
- **효과 크기와 불확실성은 왜 동시에 보고해야 할까요?**
  - `relative lift = +12.5%`만 보면 커 보일 수 있지만, `95% CI: +0.2pp ~ +0.6pp`를 같이 봐야 그 차이가 얼마나 안정적인지 알 수 있습니다. 효과 크기는 행동 가치의 크기를, 신뢰구간은 그 판단의 위험도를 보여 주므로 둘 중 하나만 있으면 결정이 과장되거나 지나치게 약해집니다.
