...
- **문제 정의, 수집, 정제, EDA, 모델링, 평가, 해석은 각각 어떤 산출물을 남길까요?**
  - 1. **도구를 먼저 고르는 실수**: 프로젝트는 문제에서 시작해야 합니다
- **이탈 예측 같은 실전 예제에서는 무엇이 결정 시점이 될까요?**
  - - 이탈 예측 프로젝트를 처음부터 끝까지 따라가는 방식 - 앞선 아홉 단계가 하나의 흐름으로 이어지는 모습 - 각 단계의 산출물과 결정 지점 - 5단계 미니 프로젝트 실습 - 끝까지 연결해 볼 때 자주 드러나는 실수 다섯 가지 단계를 따로따로 배우면 각각은 이해해도 전체 설계 감각은 잘 생기지 않습니다
