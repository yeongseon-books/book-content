...
이 글은 Data Science 101 시리즈의 6번째 글입니다.
...
- **어떤 메시지에 어떤 차트를 써야 할까요?**
  - 메시지가 분포면 히스토그램, 비교면 `plot.barh`, 추세면 선 차트, 관계면 `sns.relplot`이나 산점도가 기본 선택입니다. 이 글은 차트를 데이터 종류가 아니라 전달하려는 질문에 맞춰 고르는 습관을 만들기 위해 구성됐습니다.
- **같은 데이터를 두고도 왜 어떤 그래프는 이해를 돕고, 어떤 그래프는 오해를 만들까요?**
  - 같은 비율 데이터도 3D 파이 차트로 그리면 면적과 원근 때문에 비교가 흐려지고, 수평 막대 차트로 바꾸면 길이 차이가 바로 읽힙니다. 즉, 차트 자체가 데이터 해석 규칙을 바꾸므로 잘못된 encoding은 사실을 숨기지 않아도 오해를 만듭니다.
- **축, 색상, 라벨은 왜 사소한 장식이 아니라 핵심 설계 요소일까요?**
  - `ax.axvline(pd.Timestamp("2026-04-01"), ...)`처럼 축과 주석이 있어야 캠페인 전후 맥락이 읽히고, colorblind-safe 팔레트와 라벨이 있어야 범주를 안정적으로 구분할 수 있습니다. 차트의 메시지는 데이터 점만이 아니라 축 스케일, 범례, 텍스트 주석까지 합쳐서 완성됩니다.
...
- **Data Science 101 (6/10): 시각화 (현재 글)**
