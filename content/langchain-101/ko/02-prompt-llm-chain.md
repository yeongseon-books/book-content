---
title: "LangChain 101 (2/6): Prompt와 LLM Chain — 체인 첫 번째 구성"
...
이 글은 LangChain 101 시리즈의 두 번째 글입니다.
...
![전체 흐름 한눈에 보기](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/02/02-02-the-flow-at-a-glance.ko.png)

*전체 흐름 한눈에 보기*

이 그림에서는 사용자 입력이 프롬프트 템플릿을 거쳐 메시지로 바뀌고, 모델과 parser를 지나 최종 값으로 정리되는 흐름을 봅니다. 첫 체인의 목표는 코드를 줄이는 것이 아니라 각 단계의 입출력 경계를 선명하게 만드는 것입니다.
...
![system human ai 메시지 역할](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/02/02-01-chatprompttemplate-structure.ko.png)
...
![여러 변수가 하나의 프롬프트로 들어가는 흐름](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/02/02-02-prompts-with-multiple-variables.ko.png)
...
![문자열 파서와 JSON 파서 출력 비교](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/02/02-03-stroutputparser-vs-jsonoutputparser.ko.png)
...
![기본 경로 실패 시 대체 경로로 전환](https://yeongseon-books.github.io/book-public-assets/assets/langchain-101/02/02-04-adding-a-fallback-to-a-chain.ko.png)
...
- **ChatPromptTemplate는 단순 문자열 포맷팅과 무엇이 다를까요?**
  ChatPromptTemplate는 문자열 하나가 아니라 역할이 있는 메시지 목록을 만들고, 변수 검증과 조합을 체인 안에서 다룰 수 있게 합니다.

- **여러 입력 변수와 parser를 붙이면 체인의 입출력 모양은 어떻게 바뀔까요?**
  입력은 여러 키를 가진 dict가 되고, parser를 붙이면 모델 응답 객체가 문자열이나 JSON 같은 애플리케이션 값으로 바뀝니다.

- **fallback은 어떤 실패를 숨기고 어떤 실패는 여전히 드러내야 할까요?**
  일시적 provider 실패는 fallback 후보가 될 수 있지만, 잘못된 입력 계약이나 파싱 실패처럼 설계를 고쳐야 하는 문제는 로그에 남아야 합니다.
