...
질문: FAISS는 무엇인가요?
답변: FAISS는 (Facebook AI Similarity Search) 라는 이름의 벡터 임베딩을 위한 검색 알고리즘 라이브러리를 말합니다. FAISS는 Facebook에 의해 개발된 오픈 소스 라이브러리로, 벡터 임베딩을 효율적으로 검색하고 유사성을 측정하는 데 사용됩니다. FAISS는 코사인 유사도, 유사도 계산 및 벡터 임베딩 검색을 위한 다양한 알고리즘과 데이터 구조를 제공합니다.

질문: RAG 패턴은 어떻게 동작하나요?
답변: RAG 패턴은 검색된 문서를 LLM(Large Language Model) 프롬프트에 결합하는 패턴입니다. 

RAG 패턴은 다음 단계로 동작합니다.

1. LLM 프롬프트: 사용자가 입력하는 프롬프트가 생성됩니다. 이 프롬프트는 문맥을 제공하고, RAG 패턴이 문서를 어떻게 활용할지 설정합니다.
2. 문서 검색: RAG 패턴은 검색 알고리즘을 사용하여 관련 문서를 찾습니다. 이 과정에서 벡터 검색이 사용되어 문서와 프롬프트에 있는 키워드의 의미적 유사성을 측정합니다.
3. 문서-프롬프트 결합: 찾은 문서는 LLM 프롬프트에 결합됩니다. 이 결합은 문서의 정보가 프롬프트에 있는 문맥과 조합되기 때문에 유용한 정보를 생성할 수 있습니다.
4. LLM 모델로 전달: 결합된 문서-프롬프트 쌍은 LLM 모델로 전달됩니다. LLM 모델은 이 정보를 기반으로 답변을 생성합니다.

RAG 패턴은 문서와 프롬프트를 효과적으로 결합하여 답변을 생성하는 데 도움을 줄 수 있습니다.

질문: 임베딩 모델은 무엇을 하나요?
답변: 임베딩 모델은 자연어 처리에서 벡터 또는 수치 표현으로 텍스트를 변환하는 모델입니다. 임베딩 모델은 텍스트 데이터를 고차원 벡터공간으로 매핑하여, 벡터의 유사성 분석이나 거리 계산을 통해 텍스트의 의미를 측정할 수 있습니다. 임베딩 모델은 다양한 어휘 분석, 문서 분류, 문맥 추론 및 자연어 번역과 같은 자연어 처리 태스크에 적용됩니다. 임베딩 모델의 예로는 Word2Vec, GloVe, BERT 등이 있습니다.
~~~
