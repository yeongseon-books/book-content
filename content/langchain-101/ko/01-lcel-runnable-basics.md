...
- `ChatPromptTemplate`은 dict를 받아 채워진 채팅 메시지로 바꿉니다.
- `ChatGroq`는 메시지를 받아 `AIMessage`를 반환합니다.
- `StrOutputParser`는 최종 결과를 문자열로 정규화합니다.
- 파이프가 동작하는 이유는 세 객체가 모두 *Runnable*을 구현하기 때문입니다.

## 엔지니어가 여기서 자주 헷갈리는 지점

- LCEL은 모델 기능이 아니라 **컴포넌트 조합 문법**입니다.
- `invoke()`와 `stream()`은 체인 정의를 바꾸지 않고 **실행 방식만** 바꿉니다.
- `RunnableLambda`는 가벼운 변환 단계로는 좋지만, 애플리케이션 구조 자체를 대체하는 도구는 아닙니다.

## 체크리스트

- [ ] `prompt | llm | parser` 각 단계의 입력과 출력을 설명할 수 있다
- [ ] `invoke()`, `batch()`, `stream()`을 언제 선택할지 안다
- [ ] Groq를 이용한 최소 LCEL 체인을 끝까지 실행해 봤다
...
- LCEL은 왜 컴포넌트를 `|` 연산자로 연결할까요?
- 공통 *Runnable* 인터페이스는 어떤 메서드들로 이루어질까요?
- 프롬프트, 모델, 파서의 출력은 체인 안에서 어떻게 맞물릴까요?
- `invoke()` 대신 `batch()`를 고르는 기준은 무엇일까요?

> LCEL은 프롬프트, 모델, 파서를 모두 같은 *Runnable* 인터페이스로 맞춘 뒤, 하나의 실행 가능한 데이터 흐름으로 묶는 LangChain의 방식입니다.
...
![반복되는 글루 코드와 LCEL 추상화 흐름](../../../assets/langchain-101/01/01-01-the-problem-langchain-addresses.en.png)
...
![invoke batch stream 실행 모드](../../../assets/langchain-101/01/01-02-the-runnable-interface.en.png)
...
![프롬프트 모델 파서 타입 흐름](../../../assets/langchain-101/01/01-03-lcel-and-the-pipe-operator.en.png)
