...
- LangChain이 해결하려는 문제을 실무에 적용할 때 주의할 점은 무엇일까요?
- Runnable 인터페이스의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **LCEL은 프롬프트, 모델, 파서를 어떤 공통 계약으로 묶어 줄까요?**
  - LCEL은 프롬프트, 모델, 파서를 어떤 공통 계약으로 묶어 줄까요 — 본문에서 구체적으로 다룹니다.
- **`prompt | llm | parser` 안에서는 단계마다 어떤 데이터 모양이 오갈까요?**
  - `prompt | llm | parser` 안에서는 단계마다 어떤 데이터 모양이 오갈까요 — 본문에서 구체적으로 다룹니다.
- **`invoke()`, `batch()`, `stream()`은 같은 체인에서 각각 언제 써야 할까요?**
  - `invoke()`, `batch()`, `stream()`은 같은 체인에서 각각 언제 써야 할까요 — 본문에서 구체적으로 다룹니다.
- **최소 실행 예제에서 가장 흔한 실수는 무엇일까요?**
  - 이 짧은 예제만 봐도 LangChain의 핵심 구조가 드러납니다. 프롬프트는 입력 dict를 메시지로 바꾸고, 모델은 그 메시지를 받아 응답을 만들고, 파서는 응답 객체를 애플리케이션이 다루기 쉬운 문자열로 바꿉니다.
- **LangChain이 해결하려는 문제을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - LLM 애플리케이션 코드는 금방 비슷한 패턴으로 굳어집니다. 프롬프트 문자열을 만들고, 모델 API를 호출하고, 응답에서 텍스트를 꺼내고, 그 결과를 다음 단계 입력으로 다시 조립합니다. 한 번은 단순하지만 단계가 늘어나면 glue code가 빠르게 쌓입니다.
- **Runnable 인터페이스의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - LangChain의 거의 모든 핵심 컴포넌트는 *Runnable* 인터페이스를 구현합니다. 그래서 프롬프트도, 모델도, 파서도, 심지어 간단한 사용자 정의 변환 단계도 같은 방식으로 실행할 수 있습니다.
