...
1. 분류 모델(NLU): minority class를 늘려 class imbalance를 완화합니다.
2. robustness: typo, paraphrase, code-switching에 강한 모델을 학습합니다.
...
```python
# 패키지 설치: pip install transformers torch
...
# -> "The model achieves the best results to date." 예시 출력
```
...
1. Validation set에 augment를 적용합니다: data leakage입니다. augmentation은 train에만 적용합니다.
2. aug_p를 0.3 이상으로 높입니다: 문장 의미가 바뀌어 label이 더 이상 맞지 않을 수 있습니다.
3. Back-translation을 fact-critical domain에 적용합니다: medical, legal text에서 숫자나 entity가 변할 수 있습니다.
4. EDA를 한국어에 그대로 적용합니다: random word swap은 한국어 어순과 조사 체계를 무시합니다. 형태소 단위 augmentation이 필요합니다.
5. Paraphrase 후 dedup을 하지 않습니다: 원본과 거의 같은 paraphrase가 dataset의 절반을 차지할 수 있습니다. semantic dedup으로 0.95 이상 유사한 sample은 제거합니다.
