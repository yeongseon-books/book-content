...
Kaggle에서 데이터를 받아 `pandas.read_csv()`로 읽고 바로 모델에 넣으면 끝날 것처럼 보일 때가 많습니다. 하지만 실무에서 성능 문제를 추적해 보면 모델보다 데이터 준비 단계에서 원인이 드러나는 경우가 훨씬 많습니다.

이 글은 AI Data Preparation 101 시리즈의 첫 번째 글입니다. 여기서는 왜 데이터 준비가 모델 품질을 좌우하는지, 그리고 입문자가 초반에 놓치기 쉬운 함정을 어떤 관점으로 봐야 하는지 다룹니다.
