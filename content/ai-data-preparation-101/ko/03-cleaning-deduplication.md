...
대규모 코퍼스를 다뤄 본 팀일수록 품질 향상에 가장 큰 영향을 주는 단계로 중복 제거를 먼저 꼽습니다. 모델을 바꾸지 않아도 정제와 dedup만으로 지표가 달라지는 이유가 여기에 있습니다.

이 글은 AI Data Preparation 101 시리즈의 3번째 글입니다. 여기서는 raw text를 다듬는 기본 정제 규칙과, 학습 품질을 갉아먹는 중복을 어떤 방식으로 제거하는지 다룹니다.

---

## "중복 제거가 그렇게 중요한가요?"

The Pile, C4, RedPajama 같은 대규모 코퍼스 작업자들이 입을 모아 강조하는 한 가지가 있습니다. 중복 제거 단계가 가장 큰 품질 향상을 가져온다는 점입니다. Lee et al.(2021)의 "Deduplicating Training Data Makes Language Models Better" 논문은 학습 데이터의 1% 중복 제거만으로도 perplexity가 의미 있게 떨어진다는 사실을 보여줬습니다.
...
각 단계는 측정 가능한 문제(인코딩 깨짐, HTML 잔재, 제어 문자, 공백 노이즈)를 정확히 하나씩 잡습니다. 임의 변환을 덧붙이지 않는 편이 좋습니다. 정제 함수가 길어질수록 디버깅이 어려워지고, 의도치 않은 정보 손실이 생깁니다.
...
1. 완전 중복 제거(sha256): 완전히 같은 문서입니다. O(N) hashmap입니다.
2. 준중복 제거(MinHash + LSH): 90% 이상 유사한 문서입니다. O(N) 근사입니다.
3. 의미 중복 제거(embedding): 의미가 같은 paraphrase입니다. O(N²) 또는 ANN으로 O(N log N)입니다.
...
여기서 중요한 점은 해시 전에 정규화를 거쳐야 의미 없는 공백 차이가 다른 문서로 인식되지 않는다는 사실입니다.
...
```python
# 패키지 설치: pip install sentence-transformers faiss-cpu
...
eval 셋은 절대 건드리지 않습니다. 항상 train에서만 제거해야 평가 메트릭의 정의가 일관됩니다.
...
1. Cleaning 함수에 너무 많은 변환을 넣습니다: 한 함수에 10개 이상의 정규식이 들어가면 디버깅이 거의 불가능합니다. 함수를 작게 쪼개고 단계별 로그를 남깁니다.
2. Exact dedup만 하고 끝냅니다: 웹 크롤 데이터는 광고 한 줄, 타임스탬프 한 줄 차이로 거의 같은 문서가 수천 개 나옵니다. MinHash 단계를 반드시 추가합니다.
3. Cross-dedup을 빼먹습니다: train/eval 사이 leakage가 메트릭을 5~10%포인트 부풀립니다. 모든 평가 셋에 대해 cross-dedup을 실행합니다.
4. MinHash threshold를 default 0.5로 둡니다: datasketch 기본값은 너무 관대합니다. 0.85~0.9가 일반적인 텍스트 dedup의 안전한 시작점입니다.
5. Cleaning 메트릭을 보지 않습니다: 새 정규식을 추가했더니 데이터의 30%가 사라지는 사고가 흔합니다. char_reduction_pct를 매번 체크합니다.
