...
- **Tokenizer 선택이 모델과 묶인다** — 사후 변경은 재학습 수준의 비용입니다.
- **Chunk 경계가 의미를 깬다** — semantic chunking이 품질을 크게 올립니다.
- **overlap이 컨텍스트 누락을 줄인다** — RAG에서 특히 중요합니다.
- **길이는 use case가 결정** — 고정 사이즈는 거의 항상 차선입니다.
- **metadata와 함께 chunk 저장** — 복원·debug에서 결정적 차이를 만듭니다.
