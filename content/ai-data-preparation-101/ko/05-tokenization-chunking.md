...
이 그림에서는 Tokenization과 Chunking 전략를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.
...
- **왜 같은 의미의 한국어 문장이 영어보다 더 많은 토큰을 쓰는 경우가 많을까요?**
  - 본문의 기준은 Tokenization과 Chunking 전략를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **BPE, WordPiece, SentencePiece는 어떤 기준으로 선택해야 할까요?**
