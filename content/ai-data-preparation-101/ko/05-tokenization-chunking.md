...
1. Tokenization: text를 token id로 변환하는 subword 알고리즘입니다.
2. Chunking: 긴 문서를 모델 입력 길이에 맞게 분할하는 과정입니다.
...
1. English BPE를 한국어에 그대로 적용합니다: 한국어 한 글자가 3~4 token으로 폭증합니다. domain에서 한국어 비중이 크면 ko-aware tokenizer를 학습하거나 SentencePiece multilingual 모델을 씁니다.
2. Fixed-size chunking을 production에 바로 사용합니다: sentence boundary를 무시해 의미 손실이 생깁니다. 최소한 sentence-aware 방식으로 가는 편이 좋습니다.
3. Overlap을 0으로 둡니다: chunk 경계에서 entity가 잘리면 retrieval이 망가집니다. 10~20% overlap이 baseline입니다.
4. Token count를 char count로 추정합니다: 한국어와 영어 비율, 코드 비율에 따라 ratio가 달라집니다. 항상 실제 tokenizer로 측정합니다.
5. vocab size를 무작정 크게 잡습니다: embedding 행렬이 커져 메모리가 크게 늘어납니다. 7B 미만은 32k면 충분합니다.
