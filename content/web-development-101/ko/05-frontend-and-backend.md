...
- SPA와 SSR은 무엇이 다를까요?
- 왜 이 경계가 중요한가에서 가장 흔한 실수는 무엇일까요?
...
- **Frontend와 Backend의 일은 어디서 갈릴까요?**
  - Frontend는 `fetch("http://localhost:8000/api/items")`로 JSON을 받아 `<li>`를 그리는 쪽이고, Backend는 `/api/items`에서 `[{"id": 1, "name": "apple"}]` 같은 데이터를 만들어 돌려주는 쪽입니다. 즉 화면 표현과 사용자 상호작용은 브라우저에, 데이터 처리와 저장 책임은 서버에 두는 것이 글의 기본 경계입니다.
- **데이터의 진실은 어느 쪽이 가져야 할까요?**
  - 데이터의 진실과 민감한 판단은 Backend가 가져야 합니다. 글에서 비밀번호 비교를 프론트엔드 코드에서 하지 말고 `check_password(user, password)`처럼 서버에서 수행해야 한다고 강조한 이유도, 브라우저 코드는 누구나 읽고 우회할 수 있기 때문입니다.
- **SPA와 SSR은 무엇이 다를까요?**
  - SPA는 HTML 골격과 JavaScript가 먼저 내려온 뒤 브라우저가 데이터를 가져와 DOM을 채우는 방식이고, SSR은 `render_template_string(...)`처럼 서버가 매 요청마다 완성된 HTML을 만들어 돌려주는 방식입니다. 그래서 SSR은 첫 화면과 SEO에 유리할 수 있고, SPA는 이후 상호작용을 더 유연하게 설계하기 쉽습니다.
