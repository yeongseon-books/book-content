...
작은 회사의 한 사람이 둘 다 짜더라도, 역할의 경계를 흐리면 코드가 빠르게 부패합니다. 경계는 물리적인 줄이 아니라 책임에 대한 약속입니다.
...
- API 계약(`/api/items` 의 응답 모양)은 양쪽이 합의한 것이다.
- CORS는 브라우저의 보안 정책이지 서버의 것이 아니다.
- SSR은 첫 페인트가 빠르고, SPA는 이후 인터랙션이 빠르다.
...
5. **CORS를 모든 출처로 연다.** 보안 구멍이다.

## 실무에서는 이렇게 쓰입니다

스타트업은 SPA + REST API로 시작하는 경우가 많습니다. 콘텐츠 사이트는 SSR(Next.js, Remix)을 선호합니다. Full-stack 엔지니어는 양쪽을 짜더라도 경계에 대한 약속을 분명히 둡니다.
...
경계는 책임에 대한 약속입니다. 다음 글에서는 그 경계 위에 인증과 세션을 어떻게 얹는지 봅니다.
