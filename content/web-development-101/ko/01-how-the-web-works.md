...
**핵심 질문**: `https://example.com` 을 주소창에 치고 Enter를 누르면 *정확히* 무슨 일이 일어나나요?

> DNS 조회 → TCP/TLS 연결 → HTTP 요청 → 서버 응답 → 브라우저 렌더링. 다섯 단계의 *합주*입니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- URL을 입력한 순간부터 화면이 그려지기까지의 흐름
- DNS, HTTP, 브라우저 렌더러의 역할
- 클라이언트와 서버의 경계
- 네트워크 요청을 직접 관찰하는 방법
- 시리즈 전체의 지도
...
- **URL**: 자원의 주소 (scheme + host + path).
- **DNS**: 도메인을 IP로 바꿔주는 전화번호부.
- **HTTP**: 요청과 응답을 주고받는 프로토콜.
- **Server**: 요청을 받아 응답을 만드는 프로그램.
- **Browser**: 응답을 받아 화면으로 그리는 프로그램.
...
- 단계마다 *시간 예산* 을 둔다 (DNS 50ms, TLS 100ms 등).
- 캐시할 수 있는 것은 *모두* 캐시한다.
- 브라우저와 서버 중 *어디서* 할지 항상 묻는다.
- DevTools Network 탭을 *디버거* 로 쓴다.
- 추측 대신 *측정* 한다.
...
1. 좋아하는 웹사이트를 열고 DevTools Network 탭에서 *가장 큰* 요청을 찾으세요.
2. `dig` 또는 `nslookup` 명령으로 세 도메인의 IP를 조회하세요.
3. `requests`로 같은 URL을 100번 호출하고 평균 응답 시간을 출력하세요.
