...
- **PyPI와 TestPyPI는 무엇이 다를까요?**
  - TestPyPI에 올린 패키지의 의존성(`httpx`, `pydantic` 등)은 TestPyPI에 없으므로 `--extra-index-url`로 실제 PyPI를 추가해야 합니다.
- **`twine`은 정확히 어떤 역할을 할까요?**
  - - PyPI/TestPyPI 계정을 만들고 API 토큰을 발급하는 방법 - `twine`으로 패키지를 업로드하는 방법 - TestPyPI에서 먼저 검증한 뒤 PyPI에 배포하는 흐름 - 업로드 실패를 다루는 기본 원칙 패키지를 만들고 빌드해도, 다른 사람이 `pip install`로 설치할 수 없으면 사실상 배포한 것이
- **API 토큰은 어떻게 만들고 관리할까요?**
  - - PyPI/TestPyPI 계정을 만들고 API 토큰을 발급하는 방법 - `twine`으로 패키지를 업로드하는 방법 - TestPyPI에서 먼저 검증한 뒤 PyPI에 배포하는 흐름 - 업로드 실패를 다루는 기본 원칙 패키지를 만들고 빌드해도, 다른 사람이 `pip install`로 설치할 수 없으면 사실상 배포한 것이
