...
- **SemVer의 MAJOR.MINOR.PATCH는 언제 올려야 할까요?**
  - - SemVer 규칙과 각 버전을 올리는 기준 - `pyproject.toml`과 `__version__`을 동기화하는 방법 - Git 태그로 릴리스를 표시하고 관리하는 방법 - CHANGELOG를 쓰고 자동화하는 방법 패키지를 업데이트할 때 사용자는 가장 먼저 “이 업데이트가 안전한가?”를 판단합니다
- **버전은 코드의 어디에 기록해야 할까요?**
  - - SemVer 규칙과 각 버전을 올리는 기준 - `pyproject.toml`과 `__version__`을 동기화하는 방법 - Git 태그로 릴리스를 표시하고 관리하는 방법 - CHANGELOG를 쓰고 자동화하는 방법 패키지를 업데이트할 때 사용자는 가장 먼저 “이 업데이트가 안전한가?”를 판단합니다
- **Git 태그와 릴리스는 어떤 관계일까요?**
  - 패키지를 릴리스할 때 의존성의 버전 범위를 어떻게 설정하느냐가 사용자 경험에 직접 영향을 줍니다.
