...
- 테스트를 분류하는 기준: 피라미드가 먼저입니다에서 가장 흔한 실수는 무엇일까요?
- Unit 테스트: 서비스 판단을 고립해서 검증합니다을 실무에 적용할 때 주의할 점은 무엇일까요?
- Integration 테스트: 실제 HTTP 계약을 검증합니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 단위 테스트와 통합 테스트의 차이를 설명할 수 있습니다.
- [ ] 테스트 더블이 필요한 이유를 말할 수 있습니다.
- [ ] 백엔드 테스트 전략을 설명할 수 있습니다.

## 정리

이 글에서 다룬 핵심은 세 가지입니다. 첫째 unit, integration, E2E 테스트는 각각 무엇을 검증할, 둘째 pytest로 service를 어떻게 테스트할 수 있을, 셋째 FastAPI `TestClient`는 endpoint를 어떻게 검증하게 해 줄입니다. 테스트를 분류하는 기준: 피라미드가 먼저입니다에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **unit, integration, E2E 테스트는 각각 무엇을 검증할까요?**
  - 테스트 설계의 성숙도는 "무엇을 뺄지"에서 드러납니다. 모든 라인을 검증하겠다는 접근은 대부분 유지 불가능합니다.
- **pytest로 service를 어떻게 테스트할 수 있을까요?**
  - pytest로 service를 어떻게 테스트할 수 있을까요 — 본문에서 구체적으로 다룹니다.
- **FastAPI `TestClient`는 endpoint를 어떻게 검증하게 해 줄까요?**
  - FastAPI `TestClient`는 endpoint를 어떻게 검증하게 해 줄까요 — 본문에서 구체적으로 다룹니다.
- **테스트를 분류하는 기준: 피라미드가 먼저입니다에서 가장 흔한 실수는 무엇일까요?**
  - 테스트를 단순히 "많이" 쓰면 품질이 올라간다고 생각하기 쉽습니다. 실제로는 비율이 더 중요합니다.
- **Unit 테스트: 서비스 판단을 고립해서 검증합니다을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - unit 테스트의 목적은 함수 호출 자체가 아니라 "판단"을 검증하는 것입니다. repository, 외부 API, 메시지 브로커 같은 의존성은 테스트 대상이 아니라 협력자이므로 mock이나 fake로 대체하고, 서비스 계층의 규칙만 확인합니다.
- **Integration 테스트: 실제 HTTP 계약을 검증합니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - integration 테스트에서는 FastAPI `TestClient`로 라우터, 의존성 주입, 검증기, 직렬화까지 실제 요청 경로를 통과시킵니다.
