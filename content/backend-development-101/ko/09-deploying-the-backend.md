...
- 배포를 재현성 문제로 보는 이유에서 가장 흔한 실수는 무엇일까요?
- Dockerfile을 운영 문서로 다루기을 실무에 적용할 때 주의할 점은 무엇일까요?
- 환경 변수: 12-factor config와 secret 분리의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 패턴의 장점은 누락된 필수 값이 애플리케이션 시작 시점에 바로 실패한다는 사실입니다. 배포 후 트래픽을 받은 뒤에야 오류를 발견하는 상황을 줄입니다.
...
- [ ] 배포 파이프라인의 기본 단계를 나열할 수 있습니다.
- [ ] 컨테이너 기반 배포의 장점을 말할 수 있습니다.
- [ ] 무중단 배포가 왜 필요한지 설명할 수 있습니다.

## 정리

이 글에서 다룬 핵심은 세 가지입니다. 첫째 배포 환경은 어떤 요소들로 이루어질, 둘째 Dockerfile은 왜 재현 가능한 실행 환경을 만드는 핵심일, 셋째 환경 변수와 secret은 어떻게 분리해야 할입니다. 배포를 재현성 문제로 보는 이유에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **Dockerfile은 왜 재현 가능한 실행 환경을 만드는 핵심일까요?**
  - 배포를 업로드 작업으로 보면 결과를 사람 감각으로 확인하게 됩니다. "이번에는 잘 떴나"라는 확인이 반복되고, 담당자가 바뀌면 품질이 흔들립니다. 반대로 배포를 재현성 문제로 보면 핵심 질문이 바뀝니다.
- **환경 변수와 secret은 어떻게 분리해야 할까요?**
  - 운영 환경 차이는 코드 분기가 아니라 설정 주입으로 관리해야 합니다. 데이터베이스 주소, 외부 API endpoint, feature flag는 config입니다.
- **배포를 재현성 문제로 보는 이유에서 가장 흔한 실수는 무엇일까요?**
  - 배포를 업로드 작업으로 보면 결과를 사람 감각으로 확인하게 됩니다. "이번에는 잘 떴나"라는 확인이 반복되고, 담당자가 바뀌면 품질이 흔들립니다. 반대로 배포를 재현성 문제로 보면 핵심 질문이 바뀝니다.
- **Dockerfile을 운영 문서로 다루기을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - Dockerfile은 단순 빌드 스크립트가 아니라 런타임 계약서입니다. Python 버전, 시스템 패키지, 의존성 잠금, 실행 엔트리포인트를 명시합니다. 운영 장애의 절반 이상은 코드보다 이 계약서의 빈칸에서 시작합니다.
