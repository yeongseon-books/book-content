...
프론트엔드만 다루면 화면에 보이는 부분만 만들 수 있습니다. 백엔드를 알아야 오래 운영할 수 있는 시스템을 만들 수 있습니다. 데이터, 인증, 정합성, 운영은 모두 백엔드의 책임이며, 사용자가 보지 않는 곳에서 결정됩니다.

> 백엔드는 눈에 보이지 않지만 시스템의 핵심을 결정합니다.
...
비밀번호 검증은 서버의 일입니다. 클라이언트는 결과만 받습니다.
...
- 서버는 주소(path)와 함수를 연결하는 매핑입니다.
- 입력은 검증을 거친 뒤 함수에 전달됩니다.
- 응답은 데이터(JSON)이며 화면이 아닙니다.

## 자주 하는 실수 5가지

1. **백엔드를 DB 호출 코드로만 본다.** 라우팅, 인증, 검증, 로깅까지 모두 포함합니다.
...
스타트업이든 대기업이든 백엔드의 구조는 크게 다르지 않습니다. Router, Service, Repository, 그리고 그 위의 Middleware가 기본입니다. 처음부터 이런 레이어 분리를 익혀 두면 어느 팀에 가도 빠르게 적응할 수 있습니다. 그렇지 않으면 매번 코드 구조부터 다시 배워야 합니다.
...
백엔드는 책임의 집합입니다. 다음 글에서는 가장 아래에 있는 HTTP 서버가 어떻게 동작하는지 직접 만들어 봅니다.
