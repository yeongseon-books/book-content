...
**핵심 질문**: 사용자는 화면만 보는데, 그 뒤에서 *백엔드* 는 무엇을 하나요?

> 요청을 받고, 규칙을 적용하고, 데이터를 다루고, 응답을 돌려보냅니다. 백엔드는 *책임의 집합* 입니다.
...
- **HTTP server**: 요청을 받는 입구.
- **Router**: 어떤 함수가 응답을 만들지 결정.
- **Service**: 비즈니스 규칙을 담는 layer.
- **Repository**: DB와의 대화 창구.
- **Middleware**: 모든 요청에 *공통으로* 적용되는 처리.
...
- 비즈니스 규칙을 *라우트가 아닌 service* 에 둔다.
- 입력은 *반드시* 검증한다.
- 모든 의존성을 *주입* 가능하게 만든다.
- 로그와 메트릭을 *코드와 함께* 설계한다.
- "동작한다"가 아니라 "운영 가능한가"를 기준으로 본다.
...
1. `/health` 라우트를 추가해 `{"status": "ok"}` 를 반환하세요.
2. `/users/{user_id}` 라우트를 만들고 path 파라미터를 받아 echo 하세요.
3. `POST /login` 라우트에서 잘못된 비밀번호일 때 401을 반환하세요.
