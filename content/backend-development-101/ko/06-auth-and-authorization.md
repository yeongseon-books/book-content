...
멘탈 모델: 인증은 "신원 확인 경계", 권한은 "행동 허용 경계"입니다. 두 경계가 코드에서 분리되어 있지 않으면 보안 사고는 기능 버그 형태로 숨어 들어옵니다. 이 글에서는 인증과 권한을 분리해서 모델링해야 하는 이유부터 시니어가 보는 흔한 실수와 이유까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
