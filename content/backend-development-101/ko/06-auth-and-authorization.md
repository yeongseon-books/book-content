...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/228"
...
- 인증과 권한을 분리해서 모델링해야 하는 이유에서 가장 흔한 실수는 무엇일까요?
- JWT를 토큰 문자열이 아니라 프로토콜 구성요소로 보기을 실무에 적용할 때 주의할 점은 무엇일까요?
- 세션 기반 vs 토큰 기반: 무엇이 더 안전한가가 아니라 어떤 위험을 관리할 것인가의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
중요한 포인트는 비밀번호가 Client 앱을 통과하지 않는다는 사실입니다. OAuth2/OIDC를 쓰는 순간 애플리케이션은 계정 저장소를 직접 다루는 부담을 줄이고, 인증 강도(MFA, 위험 기반 정책)를 IdP 역량에 위임할 수 있습니다. 그래서 조직 규모가 커질수록 인증 자체보다 "권한 모델 설계"에 더 집중할 수 있습니다.
...
- [ ] authentication과 authorization은 무엇이 다를까요
- [ ] 비밀번호 저장에서 최소한으로 지켜야 할 안전 기준은 무엇인지 이해했다
- [ ] session과 JWT는 각각 언제 더 자연스러울까요
- [ ] 인증과 권한을 분리해서 모델링해야 하는 이유에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] JWT를 토큰 문자열이 아니라 프로토콜 구성요소로 보기을 실무에 적용할 때 주의할 점은 무엇인지 이해했다

---

## 정리

이 글에서 다룬 핵심은 세 가지입니다. 첫째 authentication과 authorization은 무엇이 다를, 둘째 비밀번호 저장에서 최소한으로 지켜야 할 안전 기준은 무엇일, 셋째 session과 JWT는 각각 언제 더 자연스러울입니다. 인증과 권한을 분리해서 모델링해야 하는 이유에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **session과 JWT는 각각 언제 더 자연스러울까요?**
  - JWT는 단순 문자열이 아니라 세 부분의 합입니다.
- **인증과 권한을 분리해서 모델링해야 하는 이유에서 가장 흔한 실수는 무엇일까요?**
  - 보안 사고 보고서를 읽어 보면, 치명적인 문제 상당수가 대단한 암호학 실패가 아니라 경계 혼동에서 시작됩니다.
- **JWT를 토큰 문자열이 아니라 프로토콜 구성요소로 보기을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - JWT는 단순 문자열이 아니라 세 부분의 합입니다.
- **세션 기반 vs 토큰 기반: 무엇이 더 안전한가가 아니라 어떤 위험을 관리할 것인가의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - "세션이 더 안전하다", "JWT가 확장성이 좋다" 같은 문장은 반쯤만 맞습니다. 설계는 위협 모델과 운영 제약을 같이 봐야 합니다.
...
- **Backend Development 101 (6/10): 인증과 권한 (현재 글)**
