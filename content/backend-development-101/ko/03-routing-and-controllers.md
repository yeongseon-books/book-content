...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/225"
...
- 라우팅은 결국 디스패치 문제입니다에서 가장 흔한 실수는 무엇일까요?
- Router와 Controller의 경계을 실무에 적용할 때 주의할 점은 무엇일까요?
- Path, Query, Body 파라미터를 나누는 기준의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] router와 controller는 각각 무엇을 책임져야 할 수 있다
- [ ] path, query, body parameter는 언제 어떻게 나눠 써야 할 수 있다
- [ ] REST 스타일 엔드포인트는 어떤 기준으로 설계해야 할 수 있다
- [ ] 라우팅은 결국 디스패치 문제입니다에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] Router와 Controller의 경계을 실무에 적용할 때 주의할 점은 무엇인지 이해했다

---

## 정리

이 글에서 다룬 핵심은 세 가지입니다. 첫째 router와 controller는 각각 무엇을 책임져야 할, 둘째 path, query, body parameter는 언제 어떻게 나눠 써야 할, 셋째 REST 스타일 엔드포인트는 어떤 기준으로 설계해야 할입니다. 라우팅은 결국 디스패치 문제입니다에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **router와 controller는 각각 무엇을 책임져야 할까요?**
  - router와 controller는 각각 무엇을 책임져야 할까요 — 본문에서 구체적으로 다룹니다.
- **path, query, body parameter는 언제 어떻게 나눠 써야 할까요?**
  - path, query, body parameter는 언제 어떻게 나눠 써야 할까요 — 본문에서 구체적으로 다룹니다.
- **REST 스타일 엔드포인트는 어떤 기준으로 설계해야 할까요?**
  - REST에서 가장 먼저 지켜야 할 규칙은 URL을 명사로 설계하는 것입니다. 동작은 HTTP 메서드가 표현합니다.
- **라우팅은 결국 디스패치 문제입니다에서 가장 흔한 실수는 무엇일까요?**
  - > 정신 모델: router는 "요청 주소를 책임 단위로 배치하는 교환기"이고, controller는 "검증된 입력을 서비스 호출로 연결하는 지휘자"입니다.
- **Router와 Controller의 경계을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - Router와 Controller를 분리하는 목적은 계층을 늘리기 위해서가 아닙니다. 변경 이유가 다른 코드를 물리적으로 분리하기 위해서입니다.
- **Path, Query, Body 파라미터를 나누는 기준의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 파라미터 구분은 취향이 아니라 의미 계약입니다. 같은 데이터를 어디에 두느냐에 따라 캐시 동작, 관측 가능성, 보안 리스크가 달라집니다.
