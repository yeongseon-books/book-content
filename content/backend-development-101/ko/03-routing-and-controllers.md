...
정신 모델: router는 "요청 주소를 책임 단위로 배치하는 교환기"이고, controller는 "검증된 입력을 서비스 호출로 연결하는 지휘자"입니다. 교환기와 지휘자를 분리하면 경로 충돌, 인증 누락, 타입 불일치를 각각 다른 레이어에서 빠르게 추적할 수 있습니다. 이 글에서는 라우팅은 결국 디스패치 문제입니다부터 점검 체크리스트까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
