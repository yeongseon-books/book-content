...
- 관측 가능한 백엔드는 어떻게 다른가에서 가장 흔한 실수는 무엇일까요?
- Structured Logging: 왜 JSON인가을 실무에 적용할 때 주의할 점은 무엇일까요?
- 로그 레벨은 심각도가 아니라 운영 계약입니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 왜 `print` 대신 logger를 써야 할 수 있다
- [ ] 구조화 로그는 어떤 모양이어야 운영에서 쓸모가 있을까요
- [ ] 글로벌 예외 처리는 왜 응답 일관성을 지켜 주는지 알고 있다
- [ ] 관측 가능한 백엔드는 어떻게 다른가에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] Structured Logging: 왜 JSON인가을 실무에 적용할 때 주의할 점은 무엇인지 이해했다

---

## 정리

이 글에서 다룬 핵심은 세 가지입니다. 첫째 왜 `print` 대신 logger를 써야 할, 둘째 구조화 로그는 어떤 모양이어야 운영에서 쓸모가 있을, 셋째 글로벌 예외 처리는 왜 응답 일관성을 지켜 줄입니다. 관측 가능한 백엔드는 어떻게 다른가에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **왜 `print` 대신 logger를 써야 할까요?**
  - 왜 `print` 대신 logger를 써야 할까요 — 본문에서 구체적으로 다룹니다.
- **구조화 로그는 어떤 모양이어야 운영에서 쓸모가 있을까요?**
  - 많은 팀이 로그 레벨을 감정적으로 씁니다. "문제가 커 보이니 ERROR" 같은 방식입니다. 운영에서는 레벨을 감정이 아니라 계약으로 다뤄야 합니다. 레벨마다 "무엇이 벌어지고", "누가 반응하며", "어떤 액션이 필요한지"가 명확해야 합니다.
- **글로벌 예외 처리는 왜 응답 일관성을 지켜 줄까요?**
  - 프로세스가 예외 하나로 즉시 종료되면 웹 서버는 신뢰 가능한 서비스가 아닙니다. ASGI 서버(Uvicorn/Gunicorn) + FastAPI 조합에서는 미처리 예외가 발생해도 프로세스 전체가 매번 죽는 구조가 아니라, 요청 단위 실패를 500 응답으로 반환하는 흐름을 갖습니다.
- **관측 가능한 백엔드는 어떻게 다른가에서 가장 흔한 실수는 무엇일까요?**
  - 운영 가능한 백엔드는 "실패해도 멈추지 않는 시스템"이 아니라 "실패했을 때 원인을 잃어버리지 않는 시스템"입니다.
- **Structured Logging: 왜 JSON인가을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 문자열 로그는 사람이 콘솔에서 읽기에는 편합니다. 운영 시스템이 대량 로그를 파싱하고 검색하고 집계하는 데는 매우 불리합니다.
- **로그 레벨은 심각도가 아니라 운영 계약입니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 많은 팀이 로그 레벨을 감정적으로 씁니다. "문제가 커 보이니 ERROR" 같은 방식입니다. 운영에서는 레벨을 감정이 아니라 계약으로 다뤄야 합니다. 레벨마다 "무엇이 벌어지고", "누가 반응하며", "어떤 액션이 필요한지"가 명확해야 합니다.
