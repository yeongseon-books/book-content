...
DB layer의 목적은 'SQL을 잘 쓰는 것'이 아니라 '도메인 객체와 행(row) 사이의 경계를 한 곳에 모아 두는 것'입니다 — 이 경계가 위 계층으로 새는 순간 비즈니스 로직 안에서 SQL이 자라고, 테스트가 불가능해집니다. 이 글에서는 Repository Pattern을 먼저 잡아야 하는 이유부터 마무리까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
