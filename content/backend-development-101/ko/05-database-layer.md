...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/227"
...
- Repository Pattern을 먼저 잡아야 하는 이유에서 가장 흔한 실수는 무엇일까요?
- SQLAlchemy 구성요소를 역할로 이해할 때 실무에 적용할 때 주의할 점은 무엇일까요?
- Alembic Migration: 스키마 변경을 코드처럼 다뤄야 하는 이유의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 왜 service가 SQL을 직접 작성하지 않는 편이 좋을까요
- [ ] repository pattern은 어떤 경계를 만들어 주는지 알고 있다
- [ ] ORM은 왜 편리하면서도 함정을 함께 가져올까요
- [ ] Repository Pattern을 먼저 잡아야 하는 이유에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] SQLAlchemy 구성요소를 역할로 이해할 때 실무에 적용할 때 주의할 점은 무엇인지 이해했다

---

## 정리

이 글에서 다룬 핵심은 세 가지입니다. 첫째 왜 service가 SQL을 직접 작성하지 않는 편이 좋을, 둘째 repository pattern은 어떤 경계를 만들어 줄, 셋째 ORM은 왜 편리하면서도 함정을 함께 가져올입니다. Repository Pattern을 먼저 잡아야 하는 이유에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **왜 service가 SQL을 직접 작성하지 않는 편이 좋을까요?**
  - ORM 논쟁은 도구 선호 문제처럼 보이지만, 실제로는 "어떤 종류의 복잡성을 어디에 둘 것인가"의 선택입니다.
- **repository pattern은 어떤 경계를 만들어 줄까요?**
  - repository pattern은 어떤 경계를 만들어 줄까요 — 본문에서 구체적으로 다룹니다.
- **ORM은 왜 편리하면서도 함정을 함께 가져올까요?**
  - ORM은 왜 편리하면서도 함정을 함께 가져올까요 — 본문에서 구체적으로 다룹니다.
- **Repository Pattern을 먼저 잡아야 하는 이유에서 가장 흔한 실수는 무엇일까요?**
  - repository는 "DB에 접근하는 코드 묶음"이 아닙니다. 서비스 계층이 도메인 언어로 말하도록 만들고, 데이터 접근 구현을 교체 가능한 어댑터로 제한하는 경계입니다. 이 경계가 있으면 성능 개선과 인프라 변경이 서비스 로직을 흔들지 않습니다.
- **SQLAlchemy 구성요소를 역할로 이해할 때 실무에 적용할 때 주의할 점은 무엇일까요?**
  - SQLAlchemy를 처음 도입할 때 흔한 실수는 `engine/session/model`을 한 덩어리로 보는 것입니다. 세 요소는 역할이 다르고, 문제도 다른 층에서 발생합니다.
