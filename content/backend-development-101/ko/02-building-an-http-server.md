...
- [ ] HTTP 요청-응답 흐름을 설명할 수 있습니다.
- [ ] 라우팅이 무엇인지 말할 수 있습니다.
- [ ] 간단한 HTTP 서버를 직접 만들 수 있습니다.

## 정리

HTTP 서버는 마법이 아니라 'TCP 위에서 요청을 받아 라우터로 보내고, 응답을 돌려주는 무한 루프'입니다 — 이 모델이 잡혀야 FastAPI·Flask·Express가 같은 일을 다르게 포장한 것이라는 게 보입니다. 이 글에서는 HTTP는 텍스트 프로토콜입니다부터 작은 실험: 같은 본문, 다른 헤더가 만드는 차이까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- **Backend Development 101 (2/10): HTTP 서버 만들기 (현재 글)**
