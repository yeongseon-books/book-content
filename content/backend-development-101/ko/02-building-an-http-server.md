...
- 상태 코드는 계약입니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 HTTP 요청과 응답은 실제로 어떤 모양의 텍스트일, 둘째 HTTP는 TCP 위에서 어떻게 동작할, 셋째 status code와 header는 왜 단순 장식이 아니라 계약일입니다. HTTP는 텍스트 프로토콜입니다에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
