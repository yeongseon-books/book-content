...
이 그림에서는 GitHub Actions에서 테스트 자동화하기를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> GitHub Actions에서 테스트 자동화하기의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
### Step 7: 실패한 테스트에서도 아티팩트는 남기고, job 자체는 실패시킨다
