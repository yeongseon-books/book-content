...
**핵심 질문**: 테스트를 작성하면 개발 속도가 느려지는 것 아닌가요?

> 테스트는 개발 속도를 늦추는 것이 아니라, 코드 변경에 대한 두려움을 제거하여 결과적으로 속도를 높입니다. 이 글에서는 테스트가 왜 필요한지, 어떤 종류가 있는지, pytest를 선택하는 이유를 설명합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 테스트가 개발 생산성에 미치는 구체적 영향
- 테스트 피라미드와 테스트 종류의 차이
- 수동 테스트와 자동화 테스트의 비교
- pytest가 Python 테스트 도구로 선택되는 이유
...
- **회귀 자산** — 테스트는 변경 자유를 보장하는 자산입니다.
- **우선순위** — 비즈니스 가치 큰 경로부터 커버합니다.
- **테스트 피라미드** — 단위 다수, 통합 소수, e2e 소량이 기본입니다.
- **실패 명확성** — 실패 메시지가 곧 디버깅 비용입니다.
- **CI 강제** — CI 통과 없이는 머지 불가 정책을 둡니다.
...
1. `multiply(a, b)` 함수를 작성하고, 양수, 음수, 0 입력에 대한 테스트 3개를 작성하세요.
2. `is_even(n)` 함수를 작성하고, 짝수/홀수/음수에 대한 테스트를 작성하세요.
3. `parse_int(s)` 함수를 작성하고, 유효하지 않은 문자열 입력 시 `ValueError`를 발생시키는 테스트를 작성하세요.
