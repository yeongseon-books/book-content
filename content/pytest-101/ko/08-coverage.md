...
- **코드 커버리지는 정확히 무엇을 측정할까요?**
  - 커버리지는 테스트가 실제로 실행한 코드 라인과 전체 라인의 비율을 보여 주는 지표입니다. `validator.py`나 `score.py` 예시에서 보였듯, 숫자 자체보다 어떤 조건문과 예외 경로가 한 번도 실행되지 않았는지를 드러내는 데 가장 큰 가치가 있습니다.
- **라인 커버리지와 브랜치 커버리지는 어떻게 다를까요?**
  - 라인 커버리지는 코드 줄을 한 번이라도 지나갔는지를 보고, 브랜치 커버리지는 `if/else`의 각 갈래까지 실제로 탔는지를 봅니다. 그래서 `grade(score)`처럼 A/B/C/D/F와 범위 오류가 갈리는 함수는 라인 커버리지보다 브랜치 커버리지를 함께 봐야 빈 분기를 놓치지 않습니다.
- **`pytest-cov`로 누락 라인을 어떻게 확인할 수 있을까요?**
  - `pytest --cov=src/myapp --cov-report=term-missing --cov-branch`를 실행하면 터미널에서 `Missing` 라인과 분기 누락을 바로 확인할 수 있습니다. 여기에 `coverage html`과 `--cov-fail-under=80`을 더하면, 누락 구간을 브라우저로 보고 PR 품질 게이트까지 같은 흐름으로 연결할 수 있습니다.
...
- **pytest 101 (8/10): coverage와 테스트 품질 보기 (현재 글)**
