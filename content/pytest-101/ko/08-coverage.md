...
**핵심 질문**: 내 테스트가 코드의 몇 퍼센트를 실행하고 있나요?

> 코드 커버리지는 테스트가 실행한 코드 라인의 비율을 측정합니다. pytest-cov 플러그인으로 커버리지를 측정하고, 누락된 라인을 찾아 테스트를 보완하는 방법을 배웁니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- pytest-cov를 설치하고 커버리지를 측정하는 방법
- 라인 커버리지와 브랜치 커버리지의 차이
- HTML 리포트로 누락 라인을 시각적으로 확인하는 방법
- 커버리지 임계값을 설정하여 CI에서 품질을 강제하는 방법
...
- **측정 도구** — coverage.py + pytest-cov가 표준입니다.
- **Branch coverage** — 라인보다 분기 커버리지가 더 의미 있습니다.
- **임계값 게이트** — CI에 임계값 게이트를 둡니다.
- **Uncovered 분석** — 수치 자체보다 미커버 영역의 위험을 봅니다.
- **과도한 목표 경계** — 100% 추구는 비용 대비 효과가 낮을 수 있습니다.
...
1. `validate_email` 함수에 `--cov-branch`를 추가하여 브랜치 커버리지를 측정하고, 라인 커버리지와의 차이를 확인하세요.
2. `pyproject.toml`에 커버리지 설정을 추가하고, `--cov-fail-under=90`을 설정하여 임계값 미달 시 실패를 확인하세요.
3. HTML 리포트를 생성하고 브라우저에서 열어 누락 라인을 시각적으로 확인하세요.
