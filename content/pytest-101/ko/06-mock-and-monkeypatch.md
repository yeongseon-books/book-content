...
## 적용 전과 후: try/except 기반 수동 검증 제거
...
이 구조의 장점은 HTTP 세부사항을 직접 검증하지 않아도 서비스 계약을 안정적으로 고정할 수 있다는 사실입니다.
...
- **mock과 monkeypatch는 무엇이 다를까요?**
  - 테스트 종료 후 `SERVICE_FEE`는 자동 원복됩니다.
- **`patch()`는 어디를 기준으로 적용해야 할까요?**
  - `patch()`는 어디를 기준으로 적용해야 할까요 — 본문에서 구체적으로 다룹니다.
- **외부 호출이 실패하는 상황은 어떻게 재현할 수 있을까요?**
  - 외부 호출이 실패하는 상황은 어떻게 재현할 수 있을까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 단위 테스트의 핵심은 대상 함수를 고립된 상태에서 검증하는 것입니다. 그런데 그 함수가 DB, 네트워크, 파일 시스템에 직접 의존하면 테스트는 환경 영향을 크게 받습니다.
- **핵심 개념 잡기을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - > mock = 실제 객체 대신 가짜 객체를 넣어 테스트를 고립시키는 기법
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - > mock = 실제 객체 대신 가짜 객체를 넣어 테스트를 고립시키는 기법
...
- **pytest 101 (6/10): mock과 monkeypatch (현재 글)**
