...
**핵심 질문**: 파일을 읽고 쓰거나 현재 시간에 의존하는 코드를 어떻게 안정적으로 테스트하나요?

> pytest의 `tmp_path` fixture로 임시 파일을 관리하고, `monkeypatch`로 환경변수를 격리하며, `freezegun`으로 시간을 고정합니다. 이 글에서는 시스템 리소스에 의존하는 코드의 테스트 패턴을 배웁니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- `tmp_path`와 `tmp_path_factory`로 임시 파일을 관리하는 방법
- `monkeypatch.setenv`로 환경변수를 테스트별로 격리하는 방법
- `freezegun`으로 현재 시간을 고정하여 시간 의존 로직을 테스트하는 방법
- 파일 시스템 테스트의 안전한 패턴
...
- **tmp_path** — 임시 파일은 tmp_path로 격리합니다.
- **환경변수** — monkeypatch.setenv로 안전하게 다룹니다.
- **시간 제어** — freezegun 등으로 시간을 인자로 만듭니다.
- **랜덤** — seed 고정으로 결정성을 확보합니다.
- **외부 호출** — 네트워크는 단위 테스트에서 차단합니다.
...
1. YAML 설정 파일을 `tmp_path`에 생성하고 파싱하는 테스트를 작성하세요.
2. `LOG_FORMAT` 환경변수에 따라 JSON 또는 텍스트 로그를 출력하는 함수를 테스트하세요.
3. `freeze_time`을 parametrize와 결합하여 매월 1일, 15일, 말일에 대한 테스트를 작성하세요.
