...
```python
# 이전 방식: 의존성이 내부에 결합되어 있어 mock 없이 테스트할 수 없습니다
import requests
...
```python
# 개선 방식: 의존성 주입 — mock 없이 테스트할 수 있습니다
...
```python
# pricing.py 파일 — 순수 함수: 입력 → 출력, 부작용이 없습니다
...
```python
# test_pricing.py 파일 — mock이 필요 없습니다
...
```python
# report.py 파일 — 데이터 처리와 출력을 분리합니다
...
```python
# test_report.py 파일 — 각 관심사를 독립적으로 테스트합니다
...
```python
# 리팩터링 전: mock이 5개 필요합니다
# 예시 테스트 함수: def test_create_order():
#     with patch("module.db") as mock_db, \  # DB patch
#          patch("module.requests") as mock_req, \  # requests patch
#          patch("module.send_email") as mock_email, \  # 이메일 patch
#          patch("module.datetime") as mock_dt, \  # 시간 patch
#          patch("module.logger") as mock_log:  # 로거 patch
#         ...  # 설정 코드 30줄
