...
트랜잭션과 isolation level은 어떻게 다뤄야 하나요?

이 글은 그 질문에 답하기 위해 트랜잭션과 isolation의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **기본 isolation 확인** — 드라이버 기본값이 무엇인지 항상 확인합니다.
- **Autocommit 인지** — autocommit on/off가 트랜잭션 경계를 바꿉니다.
- **Savepoint** — 복잡한 단위 작업은 savepoint로 부분 롤백을 활용합니다.
- **락 경합** — 장기 트랜잭션이 락을 키운다는 점을 잊지 않습니다.
- **일관성 vs 처리량** — isolation 강화는 처리량 비용이 든다는 점을 명시합니다.
