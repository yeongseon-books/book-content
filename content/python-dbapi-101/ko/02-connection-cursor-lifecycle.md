...
- **with 문 강제** — context manager로 누수를 구조적으로 차단합니다.
- **Cursor 재사용** — 장기 보관보다 짧게 만들고 닫는 편이 안전합니다.
- **트랜잭션 경계** — commit/rollback 시점이 명확해야 데이터 일관성이 유지됩니다.
- **자원 누수 모니터링** — 오픈된 connection 수를 메트릭으로 본다.
- **예외 시 롤백** — 예외 경로에서 반드시 rollback이 호출되도록 보장합니다.
