...
SQLite connection은 어떻게 관리하고 풀링은 어떻게 적용하나요?

이 글은 그 질문에 답하기 위해 Connection 관리와 풀링의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- SQLite의 세 가지 thread-safety 모드(single, multi, serialized)는 무엇이고 어떻게 확인하는가?
- `sqlite3.connect()`의 `check_same_thread=True`는 무엇을 막아주고, False로 바꾸면 무엇이 위험해지는가?
- per-thread connection과 shared connection은 각각 언제 적합한가?
- 왜 SQLite에는 PostgreSQL 같은 정교한 connection pool이 잘 어울리지 않는가?
- FastAPI에서 SQLite connection을 어떻게 dependency로 쥐어야 하는가?
...
- **Thread-safety** — check_same_thread 의미를 정확히 이해합니다.
- **WAL 모드** — 동시성 향상을 위해 WAL 모드를 우선 검토합니다.
- **Busy timeout** — 락 경합을 견디기 위해 적정 timeout을 설정합니다.
- **풀 크기** — 프로세스/스레드 모델에 맞춰 풀 크기를 정합니다.
- **종료 절차** — graceful shutdown에서 connection 정리를 보장합니다.
...
1. **모드 확인.** 사용 중인 Python 환경에서 `sqlite3.threadsafety`, `sqlite3.sqlite_version`, `journal_mode` 기본값을 출력하세요.
2. **재현.** `check_same_thread=True`인 connection을 다른 스레드에서 사용해 `ProgrammingError`를 발생시키세요. 그 후 `False`로 바꾸고 두 스레드가 동시에 트랜잭션을 시작했을 때 어떤 일이 벌어지는지 관찰하세요.
3. **측정.** WAL 모드와 DELETE 모드(`PRAGMA journal_mode=DELETE`)에서 200개의 동시 쓰기 작업을 돌려 BUSY 발생 횟수를 비교하세요.
4. **설계.** "사용자 요청은 read만, 백오피스 배치만 write"하는 시스템이라면 connection 전략을 어떻게 잡겠는지 다이어그램으로 그려 보세요.
