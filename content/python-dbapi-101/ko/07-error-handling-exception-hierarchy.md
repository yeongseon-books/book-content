...
PEP 249 예외 계층을 활용해 에러를 어떻게 다뤄야 하나요?

이 글은 그 질문에 답하기 위해 예외 계층과 에러 처리의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- PEP 249가 정의하는 8개의 예외는 어떤 의미를 가지며 어떻게 상속되는가?
- `sqlite3`는 SQLite의 에러 코드(SQLITE_BUSY, SQLITE_CONSTRAINT 등)를 어떤 PEP 249 예외로 매핑하는가?
- `OperationalError`, `IntegrityError`, `ProgrammingError` 중 어느 것이 retry 대상이고 어느 것이 코드 버그인가?
- BUSY와 LOCKED는 무엇이 다르며, 각각 어떻게 대응해야 하는가?
- Python 3.11+의 `sqlite3.Error.sqlite_errorcode`와 `sqlite_errorname`은 무엇을 해결해 주는가?
...
- **계층 활용** — OperationalError·IntegrityError 등 상위 계층을 우선 잡습니다.
- **재시도 정책** — transient 에러만 재시도하고 횟수를 제한합니다.
- **로깅** — 에러 코드·SQL·파라미터 메타를 함께 기록합니다.
- **사용자 메시지** — 내부 메시지를 그대로 노출하지 않습니다.
- **관측성** — 에러 유형별 카운터를 메트릭으로 둡니다.
...
1. **분류 연습.** 다음 예외 각각을 retry / 4xx / 5xx 중 하나로 분류하세요. (a) `IntegrityError(SQLITE_CONSTRAINT_FOREIGNKEY)`, (b) `OperationalError(SQLITE_BUSY)`, (c) `OperationalError(SQLITE_CORRUPT)`, (d) `ProgrammingError(SQLITE_MISUSE)`, (e) `DataError`.
2. **재현.** 두 개의 connection을 열어 한쪽에서 `BEGIN IMMEDIATE`를 걸고, 다른 쪽에서 짧은 `busy_timeout`(예: 100ms)으로 INSERT를 시도해 `SQLITE_BUSY`를 재현하세요. 그 후 `retry_on_transient` 데코레이터를 적용해 성공시키세요.
3. **버그 만들기.** 위 retry 데코레이터에서 `is_retryable`이 `IntegrityError`도 True를 돌려주도록 바꾸어 UNIQUE 위반을 retry하면 어떤 일이 벌어지는지 측정하세요(횟수, 총 지연 시간).
4. **확장.** `is_retryable`을 외부 함수로 받도록 데코레이터를 수정해 PostgreSQL용 분류기를 주입할 수 있도록 만드세요.
