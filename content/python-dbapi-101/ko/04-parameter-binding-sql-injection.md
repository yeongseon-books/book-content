...
파라미터 바인딩으로 SQL injection을 어떻게 구조적으로 막나요?

이 글은 그 질문에 답하기 위해 파라미터 바인딩과 보안의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- f-string으로 SQL을 만들면 왜 위험한가요?
- `?`와 `:name` placeholder는 언제 어느 것을 쓰나요?
- `executemany`로 bulk insert할 때 binding은 어떻게 동작하나요?
- table 이름이나 column 이름은 왜 binding 대상이 되지 않나요?
- `paramstyle`이 driver마다 다르다는 건 무슨 뜻인가요?

> SQL injection은 query string과 사용자 입력이 섞이는 순간 발생합니다. PEP 249의 parameter binding은 이 둘을 끝까지 분리해 주는 표준 도구입니다.

> Python DB-API 101 시리즈 (4/10)

---

## 이 글에서 배울 것

이 글에서는 sqlite3를 기준으로 PEP 249의 parameter binding을 깊이 다룹니다. 구체적으로:

1. **두 가지 placeholder** — `?` (qmark)와 `:name` (named) 사용법, sqlite3가 둘 다 지원하는 이유.
2. **executemany와 binding** — list of tuples, list of dicts 형태 입력의 차이.
3. **driver별 paramstyle** — `qmark`, `format`, `named`, `pyformat`, `numeric` 5종을 PEP 249가 정의한 이유.
4. **binding이 안 되는 자리** — table name, column name, ORDER BY 방향. 안전하게 동적으로 만드는 패턴.
5. **공격 시나리오 재현** — 의도적으로 취약한 코드를 작성해 SQL injection이 실제로 동작하는 모습을 보고, parameter binding으로 막는 과정.
...
- **문자열 결합 금지** — 어떤 경우에도 사용자 입력을 SQL에 직접 결합하지 않습니다.
- **Paramstyle 통일** — 프로젝트에서 한 가지 paramstyle로 통일합니다.
- **식별자 처리** — 테이블·컬럼명은 화이트리스트로만 허용합니다.
- **로깅 주의** — 쿼리 로그에 비밀값이 새지 않도록 마스킹합니다.
- **정적 분석** — 위험 패턴을 린트로 잡아 회귀를 막습니다.
...
1. **공격 재현** — 위의 `find_user_BAD`를 그대로 실행해 모든 row가 반환되는 것을 직접 확인하고, parameter binding 버전이 빈 결과를 돌려주는 것을 비교하세요.
2. **`UPDATE` injection** — `UPDATE users SET name = '{new_name}' WHERE id = {id}` 형태로 `new_name = "x', secret='leaked"` 같은 입력을 시도해 보세요. 그리고 `?`로 바꾼 뒤 다시 시도하세요.
3. **dict-style executemany** — CSV 파일을 dict로 읽어 `:name` placeholder로 1000건을 insert하고, `tuple` 버전과 시간을 비교하세요.
4. **whitelist sort** — `list_products` 함수에 `direction='DROP TABLE products; --'`를 넣어 호출하고 `ValueError`가 나오는 것을 확인하세요.
5. **paramstyle 확인** — `psycopg2`(또는 `psycopg`)와 `mysql-connector-python`을 설치해 각 module의 `paramstyle` 값을 출력해 보세요.
