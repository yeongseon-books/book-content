...
- **agent 신뢰성을 볼 때 실패를 없애는 대신 무엇을 제어해야 할까요?**
  - agent 신뢰성을 볼 때 실패를 없애는 대신 무엇을 제어해야 할까요 — 본문에서 구체적으로 다룹니다.
- **Retry, fallback, circuit breaker는 각각 어떤 종류의 실패에 맞을까요?**
  - Retry, fallback, circuit breaker는 각각 어떤 종류의 실패에 맞을까요 — 본문에서 구체적으로 다룹니다.
- **tool 실행을 안전하게 만들려면 실패 전후에 어떤 guard가 필요할까요?**
  - tool 실행을 안전하게 만들려면 실패 전후에 어떤 guard가 필요할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - agent는 성공 경로만 설계해서는 배포할 수 없습니다. 외부 검색 API 하나만 붙어도 timeout, rate limit, malformed output, auth failure 같은 문제가 생기고, 여기에 LLM의 비결정성과 tool schema 오류까지 더해집니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 운영에서 중요한 것은 실패를 0으로 만드는 일이 아닙니다. 실제로는 불가능합니다. 더 중요한 것은 실패가 났을 때 시스템이 예측 가능한 형태로 멈추거나, 줄어든 기능으로라도 응답하거나, 안전하게 재시도하도록 만드는 것입니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 운영에서 중요한 것은 실패를 0으로 만드는 일이 아닙니다. 실제로는 불가능합니다. 더 중요한 것은 실패가 났을 때 시스템이 예측 가능한 형태로 멈추거나, 줄어든 기능으로라도 응답하거나, 안전하게 재시도하도록 만드는 것입니다.
