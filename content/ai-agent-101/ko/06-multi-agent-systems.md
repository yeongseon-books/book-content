...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/100"
...
이 글은 AI Agent 101 시리즈의 6번째 글입니다.
...
multi-agent를 "여러 agent가 함께 일한다"고만 설명하면 너무 추상적입니다. 더 실용적인 설명은 누가 어떤 하위 작업을 맡고 어떤 결과만 남기는지 정의된 위임 그래프라는 사실입니다. 이 관점이 있어야 supervisor와 worker, reviewer와 writer, manager와 child agent의 차이가 선명해집니다.
...
핵심은 supervisor가 재시도 예산과 종료 조건을 가진다는 사실입니다. 각 에이전트가 스스로 무한 루프를 돌지 않도록 상위 제어기가 필요합니다.
...
- **single agent로 충분한 업무와 multi-agent가 필요한 업무는 어디서 갈라질까요?**
  - single agent로 충분한 업무와 multi-agent가 필요한 업무는 어디서 갈라질까요 — 본문에서 구체적으로 다룹니다.
- **supervisor, worker, handoff 계약은 각각 어떤 책임을 가져야 할까요?**
  - supervisor, worker, handoff 계약은 각각 어떤 책임을 가져야 할까요 — 본문에서 구체적으로 다룹니다.
- **agent를 여러 개로 나눌수록 비용과 실패 지점은 어떻게 늘어날까요?**
  - agent를 여러 개로 나눌수록 비용과 실패 지점은 어떻게 늘어날까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - multi-agent는 종종 성능 향상 도구로 소개되지만, 실제 현장에서는 구조화된 위임을 위한 도구에 더 가깝습니다. 어떤 agent가 어떤 종류의 결정을 하고 어떤 산출물만 남겨야 하는지 분리되면, 시스템이 커져도 읽기가 쉬워집니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - multi-agent를 "여러 agent가 함께 일한다"고만 설명하면 너무 추상적입니다. 더 실용적인 설명은 누가 어떤 하위 작업을 맡고 어떤 결과만 남기는지 정의된 위임 그래프라는 사실입니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - multi-agent를 "여러 agent가 함께 일한다"고만 설명하면 너무 추상적입니다. 더 실용적인 설명은 누가 어떤 하위 작업을 맡고 어떤 결과만 남기는지 정의된 위임 그래프라는 사실입니다.
