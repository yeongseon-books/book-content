...
Agent가 여러 단계를 거쳐 작업을 수행하려면 이전 단계에서 무엇을 했는지 기억해야 합니다. 이것이 Memory입니다. 단기 메모리는 현재 대화 세션 동안 유지되고, 장기 메모리는 세션이 끝나도 유지됩니다. 이 글은 AI Agent 101 시리즈의 5번째 글입니다.
...
- [ ] 단기·장기 메모리 구분 기준을 한 문장으로 설명할 수 있다.
- [ ] Sliding window·summarization 두 가지 truncation 전략을 비교했다.
- [ ] 토큰 한도에 가까워졌을 때 자동 요약하는 코드를 작성했다.
- [ ] Vector store와 KV store의 적절한 사용처를 한 표로 적었다.
