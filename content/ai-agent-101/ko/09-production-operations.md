...
이 글은 AI Agent 101 시리즈의 9번째 글입니다.
...
### 배포 단위를 runbook과 함께 버전 관리합니다

프로덕션 운영에서는 코드 배포만으로 충분하지 않습니다. 프롬프트 버전, 도구 스키마 버전, 알람 정책, 롤백 절차를 같은 릴리스 단위로 묶어야 합니다. 그래야 장애 시점을 정확히 역추적할 수 있습니다.

### FastAPI 기반 serving 골격
...
서비스 경계가 HTTP API로 명확해지면 인증, 관측성, 비용 제어를 공통 미들웨어로 관리할 수 있습니다.

### OpenTelemetry 추적 필드

```text
trace_id, span_id, run_id, user_id_hash, model, prompt_version,
tool_name, tool_latency_ms, token_input, token_output, stop_reason
```

이 필드 조합이 있으면 "느리다"는 증상을 모델 지연, 도구 지연, 재시도 폭증 중 어디서 발생했는지 즉시 분리할 수 있습니다.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: api
          image: ghcr.io/example/ai-agent-api:1.0.0
          ports:
            - containerPort: 8000
          env:
            - name: OPENAI_API_KEY
...
운영 환경에서는 모델 키와 내부 토큰을 Secret으로 분리하고, 최소 권한 원칙으로 접근을 제한해야 합니다.

### 운영 지표와 알람 기준

| 지표 | 경보 |
| --- | --- |
| p95 end-to-end latency | 10초 초과 5분 지속 |
| error_rate | 2% 초과 10분 지속 |
| cost_per_1k_runs | 주간 예산 120% 초과 |
| policy_violation_rate | 즉시 페이지 |

운영 체계는 장애를 막는 도구이면서 동시에 비용을 통제하는 장치입니다. 관측성 없는 자동화는 장기적으로 유지되지 않습니다.
...
- **운영 중 agent가 왜 그런 답을 냈는지 설명하려면 어떤 trace가 먼저 필요할까요?**
  - 실전에서는 실패를 "모델이 틀렸다" 한 문장으로 닫지 않습니다. 다음 템플릿처럼 실패 축을 분리하면 개선 우선순위가 명확해집니다.
- **비용과 latency는 어떤 단위로 측정해야 실제 병목이 보일까요?**
  - 비용과 latency는 어떤 단위로 측정해야 실제 병목이 보일까요 — 본문에서 구체적으로 다룹니다.
- **agent 배포와 rollback을 안전하게 만들려면 어떤 관측 지표가 gate가 되어야 할까요?**
  - agent 배포와 rollback을 안전하게 만들려면 어떤 관측 지표가 gate가 되어야 할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 운영 체계가 없으면 agent는 금방 블랙박스가 됩니다. 잘될 때는 멋져 보이지만, 느려지거나 틀리거나 비싸질 때 왜 그런지 알 수 없습니다. production 시스템으로서는 매우 위험한 상태입니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - agent 운영은 모델 응답을 모니터링하는 일이 아닙니다. 요청 하나가 내부에서 어떤 단계들을 거쳤는지 추적하고, 그 단계별 비용과 시간을 읽는 일에 가깝습니다. 이 관점이 있어야 병목과 낭비를 분리할 수 있습니다.
...
- [이 글의 예제 코드 (book-examples)](https://github.com/yeongseon-books/book-examples/tree/main/ai-agent-101/ko/09-production-operations)
