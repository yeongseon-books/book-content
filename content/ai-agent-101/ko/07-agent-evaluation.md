...
Agent를 만드는 것보다 어려운 것이 "이 Agent가 제대로 작동하는지" 평가하는 것입니다. 단순 LLM은 응답 품질만 평가하면 되지만, Agent는 도구 호출 정확도, 작업 완료율, 불필요한 단계 수, 비용 효율성 등 여러 지표를 봐야 합니다. 이 글은 AI Agent 101 시리즈의 7번째 글입니다.
