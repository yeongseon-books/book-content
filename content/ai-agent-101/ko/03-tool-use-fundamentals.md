...
- **스키마가 계약** — JSON schema가 곧 모델·앱 사이 계약입니다.
- **최소 권한** — 각 툴은 가장 좁은 권한·스코프로 노출합니다.
- **멱등성** — 재시도 가능한 멱등 설계로 모델의 중복 호출을 흡수합니다.
- **에러 메시지** — 에러 본문이 모델 다음 행동을 결정합니다 — 명확하게 설계합니다.
- **관측** — 툴 호출은 input/output을 모두 trace로 남깁니다.
