...
## Diátaxis 4구역과 튜토리얼의 자리

Diátaxis는 문서를 목적에 따라 4구역으로 나눕니다. 튜토리얼이 어디에 위치하는지 이해하면 어떤 내용을 담아야 하는지 더 선명해집니다.

| 구역 | 지향점 | 독자 질문 | 특징 |
| --- | --- | --- | --- |
| Tutorial | 학습 | "처음 어떻게 시작하나요?" | 손을 움직이며 배우는 경험 제공 |
| How-to | 문제 해결 | "이걸 어떻게 하나요?" | 이미 알고 있는 독자를 안내 |
| Reference | 정보 조회 | "이 파라미터는 무엇인가요?" | 완전하고 정확한 사양 제공 |
| Explanation | 이해 | "왜 이렇게 동작하나요?" | 개념과 맥락 설명 |

튜토리얼은 Tutorial 구역에 속합니다. 여기서 중요한 것은 독자가 배경 이해 없이도 따라 하며 첫 성공을 경험하도록 만드는 것입니다.
...
튜토리얼은 라이브러리 업데이트와 함께 낡아집니다. 처음부터 유지보수를 고려해 설계해야 합니다.

| 문제 | 발생 시점 | 예방 방법 |
| --- | --- | --- |
| 명령이 달라짐 | 라이브러리 메이저 버전 업 | 버전을 명시하고 변경 시 업데이트 |
| 오류 메시지가 달라짐 | 라이브러리 마이너 버전 업 | 복구 힌트를 일반적으로 작성 |
| 기대 출력이 달라짐 | 기능 변경 | 출력의 핵심 부분만 검증 |
| 링크가 깨짐 | 외부 문서 이동 | 아카이브 링크 또는 버전 고정 링크 사용 |

튜토리얼을 처음 작성할 때 "이 튜토리얼은 언제 깨질 수 있는가"를 한 번 점검해 두면 유지보수가 훨씬 쉬워집니다.
