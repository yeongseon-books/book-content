---
title: "LangGraph 101 (1/6): LangGraph 소개와 그래프 기초"
...
## 상태 전이 다이어그램으로 먼저 그려보기

코드를 작성하기 전에 상태 전이 흐름을 텍스트로 먼저 그려 두면 노드 책임이 훨씬 선명해집니다. 아래는 이 글의 예제를 상태 전이 관점에서 표현한 것입니다.
...
[build_outline]
  ...
  v
[write_answer]
  - 읽는 필드: user_request, topic, outline
  - 쓰는 필드: answer
  |
  v
END
...
이 다이어그램에서 주목할 것은 화살표 방향이 아닙니다. **각 노드가 읽는 필드와 쓰는 필드가 분리되어 있다**는 사실입니다. `choose_topic`은 `user_request`를 읽어서 `topic`만 씁니다. `build_outline`은 `topic`을 읽어서 `outline`만 씁니다. 이 분리가 명확할수록 "어느 단계에서 이 필드가 잘못됐는가"라는 디버깅 질문에 빠르게 답할 수 있습니다.

팀 차원에서는 이 다이어그램을 코드 PR에 함께 넣는 관행을 들이면 리뷰 품질이 올라갑니다. 코드를 보지 않아도 "이 노드는 왜 이 필드를 읽는가"를 질문할 수 있게 되기 때문입니다.

---

## 상태 스키마를 먼저 설계하는 이유

LangGraph 개발에서 가장 먼저 해야 할 일은 `StateGraph`를 만드는 것이 아닙니다. **상태 스키마를 설계하는 것**입니다. 상태는 노드들이 공유하는 데이터 계약이기 때문에, 스키마가 불분명하면 어떤 노드가 어떤 데이터를 믿을 수 있는지가 모호해집니다.

좋은 상태 스키마 설계의 기준은 세 가지입니다.

첫째, **필드마다 소유 노드가 있어야 합니다.** `topic`은 `choose_topic`이 쓰고, `outline`은 `build_outline`이 씁니다. 여러 노드가 같은 필드를 쓰기 시작하면 나중에 "이 값을 누가 바꿨는가"를 추적하기 어려워집니다.

둘째, **필드 이름이 용도를 설명해야 합니다.** `data`, `result`, `output` 같은 범용 이름보다 `topic`, `outline`, `answer`처럼 구체적인 이름이 낫습니다. 코드 리뷰에서 "이 `result`는 어느 단계의 결과인가?"를 묻지 않아도 되기 때문입니다.

셋째, **불필요한 필드를 넣지 않아야 합니다.** 나중에 쓸 것 같아서 미리 추가한 필드는 대부분 혼란만 가중시킵니다. 필요해지면 그때 추가하는 편이 훨씬 낫습니다.

아래는 잘 설계된 상태와 그렇지 않은 상태를 비교한 예시입니다.

```python
# 권장하지 않는 방식: 범용 필드로 가득 찬 상태
class VagueState(TypedDict):
    input: str       # 입력인지 중간 결과인지 불분명
    result: str      # 어느 노드의 결과인지 불분명
    data: list       # 무엇을 담는지 불분명
    output: str      # answer와 다른가?

# 권장하는 방식: 필드마다 역할이 명확한 상태
class ArticleState(TypedDict):
    user_request: str    # 사용자 원본 입력 (변경 없음)
    topic: str           # choose_topic이 설정
    outline: list[str]   # build_outline이 설정
    answer: str          # write_answer가 설정
```

이 차이는 작아 보이지만, 그래프가 커질수록 영향이 커집니다. 특히 체크포인트나 조건부 엣지를 붙일 때 상태 스키마가 불분명하면 "이 필드가 지금 어떤 상태인가"를 확인하는 데 시간이 많이 걸립니다.
...
def choose_topic(state: ArticleState) -> dict:
    """사용자 요청에서 핵심 주제를 추출합니다."""
    request = state["user_request"].lower()
    if "checkpoint" in request:
        topic = "checkpoints"
    elif "tool" in request:
        topic = "tool calling"
    elif "conditional" in request or "edge" in request:
...
def build_outline(state: ArticleState) -> dict:
    """선택된 주제로 교육용 개요를 생성합니다."""
    outline = [
        f"1. {state['topic']} 개념 정의",
        f"2. {state['topic']}을 그래프에서 사용하는 방법",
        f"3. invoke()로 실행하고 상태 검증하기",
    ]
    return {"outline": outline}


def write_answer(state: ArticleState) -> dict:
    """개요를 기반으로 최종 답변을 조립합니다."""
    bullet_lines = "\n".join(f"  - {item}" for item in state["outline"])
    answer = (
        f"요청: {state['user_request']}\n"
        f"선택된 주제: {state['topic']}\n"
        f"학습 개요:\n{bullet_lines}"
    )
...
if __name__ == "__main__":
    ...
    result = app.invoke(
        {
            "user_request": "LangGraph StateGraph가 어떻게 동작하는지 설명해 주세요.",
            "topic": "",
            "outline": [],
            "answer": "",
        }
...
=== 최종 상태 ===
주제: graph basics
개요: ['1. graph basics 개념 정의', '2. graph basics을 그래프에서 사용하는 방법', '3. invoke()로 실행하고 상태 검증하기']

답변:
요청: LangGraph StateGraph가 어떻게 동작하는지 설명해 주세요.
선택된 주제: graph basics
학습 개요:
  - 1. graph basics 개념 정의
  - 2. graph basics을 그래프에서 사용하는 방법
  - 3. invoke()로 실행하고 상태 검증하기
```
...
예제를 보면 각 노드 함수가 비슷한 패턴을 따른다는 것을 알 수 있습니다. 이 패턴은 우연이 아닙니다. LangGraph 노드 함수를 안정적으로 작성하는 데는 세 가지 규칙이 있습니다.

**규칙 1: 전체 상태를 입력으로 받고, 변경할 필드만 반환한다**

```python
# 좋은 예: 변경할 필드만 반환
def choose_topic(state: ArticleState) -> dict:
    return {"topic": "graph basics"}  # topic만 반환

# 나쁜 예: 전체 상태를 다시 조립해서 반환
def choose_topic(state: ArticleState) -> ArticleState:
    return {
        "user_request": state["user_request"],  # 불필요한 복사
        "topic": "graph basics",
        "outline": state["outline"],             # 불필요한 복사
        "answer": state["answer"],               # 불필요한 복사
    }
```

전체 상태를 다시 조립하면 실수로 기존 값을 빈 값으로 덮어쓰는 문제가 생깁니다. LangGraph는 노드가 반환한 딕셔너리를 현재 상태에 병합(merge)하기 때문에, 변경할 필드만 반환하는 것이 안전하고 의도도 명확합니다.

**규칙 2: 노드 함수는 부작용 없이 상태만 바꿔야 한다**

노드 함수 안에서 외부 API를 호출하거나, 파일을 쓰거나, 전역 변수를 바꾸는 것은 가능합니다. 하지만 그런 부작용이 있다면 노드 이름에 명시하는 편이 좋습니다. `call_external_api`, `write_to_file`처럼 이름에 드러나면 그래프 구조만 봐도 어디서 부작용이 발생하는지 알 수 있습니다.

**규칙 3: 노드 이름은 동사형으로 짓는다**

`choose_topic`, `build_outline`, `write_answer`처럼 동사형 이름을 쓰면 그래프를 읽을 때 "무엇을 하는 단계인가"가 바로 드러납니다. `topic_node`, `outline_module`처럼 명사형 이름을 쓰면 역할이 모호해질 수 있습니다.
...
# 중간 상태 필드부터 검증
assert result["topic"] == "graph basics", f"예상: graph basics, 실제: {result['topic']}"
assert len(result["outline"]) == 3, f"개요는 3개여야 함, 실제: {len(result['outline'])}"
assert "graph basics" in result["outline"][0], "개요 첫 항목에 주제가 포함되어야 함"
assert "선택된 주제: graph basics" in result["answer"], "답변에 주제가 포함되어야 함"
...
```python
# 나쁜 예: 초기값 없이 호출
result = app.invoke({"user_request": "LangGraph 설명해 주세요."})
# build_outline에서 state["outline"]이 없어서 KeyError 발생 가능

# 좋은 예: 모든 필드 초기화
result = app.invoke({
    "user_request": "LangGraph 설명해 주세요.",
    "topic": "",
    "outline": [],
    "answer": "",
})
...
`answer`만 보면 `write_answer()`를 먼저 의심하기 쉽습니다. 하지만 실제 원인은 `choose_topic()`에서 잘못된 route를 골랐거나 `build_outline()`이 비어 있는 리스트를 만들었기 때문일 수 있습니다. 항상 상태 필드를 단계 순서대로 확인하는 습관을 들이세요.
...
![요청이 상태 필드로 매핑되는 구조](https://yeongseon-books.github.io/book-public-assets/assets/langgraph-101/01/01-02-what-to-notice-in-this-code.ko.png)
*요청이 상태 필드로 매핑되는 구조*
...
![정의에서 invoke까지 이어지는 실행 흐름](https://yeongseon-books.github.io/book-public-assets/assets/langgraph-101/01/01-03-where-engineers-get-confused.ko.png)
*정의에서 invoke까지 이어지는 실행 흐름*

**실수 1: 노드가 상태 전체를 다시 만들어 반환해야 한다고 생각하기**

노드는 바뀐 필드만 돌려주면 충분합니다. 전체를 재조립하면 기존 값이 빈 값으로 초기화되는 실수가 생깁니다. 특히 `list` 타입 필드를 `[]`로 덮어쓰면 이전에 쌓인 데이터가 모두 사라집니다.

**실수 2: StateGraph를 단순 DAG 파이프라인으로만 생각하기**

같은 추상화 위에 루프와 분기를 올릴 수 있습니다. 2편에서 다루는 체크포인트, 3편의 조건부 엣지, 4편의 도구 호출 루프가 모두 이 위에 올라갑니다. 처음부터 "이것도 할 수 있다"는 감각을 가지고 있으면 다음 글들이 훨씬 자연스럽게 읽힙니다.

**실수 3: invoke()를 마지막 노드의 출력만 주는 함수로 이해하기**

`invoke()`는 마지막 노드의 출력만 주는 함수가 아니라, 실행이 끝난 뒤의 최종 상태를 반환합니다. `topic`, `outline`, `answer`가 모두 남아있는 것을 확인하면 이 차이가 분명해집니다. 이 이해가 나중에 체크포인트, 멀티턴 대화, 도구 호출 루프를 붙일 때 바로 생산성 차이로 이어집니다.
...
여기서 첫 번째 안티패턴은 **Implicit State 안티패턴**입니다. 상태가 어디서 어떻게 바뀌는지 명확히 읽지 않고, 마지막 출력만 보고 수정하는 방식입니다. 이 안티패턴에 빠지면 문제는 앞에서 생겼는데 수리는 뒤에서 하게 됩니다.

두 번째 안티패턴은 **Full Rewrite State 안티패턴**입니다. 노드가 안전하려면 상태 전체를 항상 다시 만들어야 한다고 믿는 경우입니다. 그래서 작은 업데이트 하나를 위해 모든 필드를 매번 재조립하고, 그 과정에서 기존 값을 빈 문자열로 덮어쓰거나 의도치 않게 초기화해 버립니다.

---

## 디버깅 전술: 상태 추적 로그 남기기

운영에서 그래프 실행을 디버깅할 때 가장 유용한 방법은 각 노드 실행 후 상태 변화를 로그로 남기는 것입니다. LangGraph는 `stream()` 메서드를 통해 노드별 중간 상태를 실시간으로 받을 수 있습니다.

```python
app = build_graph()

# stream()으로 노드별 상태 변화 추적
...
[build_outline] 업데이트:
  outline: ['1. graph basics 개념 정의', '2. graph basics을 그래프에서 사용하는 방법', '3. invoke()로 실행하고 상태 검증하기']

[write_answer] 업데이트:
  answer: 요청: LangGraph StateGraph가 어떻게 동작하는지 설명해 주세요.
선택된 주제: graph basics
...
```

이 로그는 "어느 노드가 어떤 필드를 바꿨는가"를 한눈에 보여 줍니다. 문제가 생겼을 때 이 로그를 보면 어느 단계에서 값이 의도와 달라졌는지 바로 확인할 수 있습니다.

---

## 실무에서는 이렇게 생각한다

입문 글에서 너무 이른 이야기처럼 들릴 수 있지만, 저는 첫 번째 그래프를 만들 때부터 운영 질문을 같이 붙여 보는 편이 좋다고 생각합니다. 예를 들어 이런 질문입니다.

- "이 노드가 나중에 외부 API 호출로 바뀌면 어디에서 재시도 정책을 붙일까?"
- "이 상태 필드가 체크포인트에 저장되면 다음 턴에서도 의미가 유지될까?"
- "이 분기가 나중에 조건부 엣지로 커져도 이름이 그대로 버틸까?"

이 질문을 지금 던지는 이유는 겁을 주려는 게 아닙니다. 오히려 반대입니다. LangGraph의 장점은 나중에 커질 구조를 지금부터 작은 형태로 드러낼 수 있다는 사실입니다. 노드, 엣지, 상태를 분리해 생각하는 습관만 잡히면, 이후에 메모리와 라우팅을 붙이는 일은 "새로운 세계"가 아니라 "같은 구조의 확장"으로 느껴집니다.

현업에서 저는 LangGraph를 잘 쓰는 팀일수록 LLM 호출보다 상태 계약을 먼저 리뷰하는 걸 자주 봤습니다. 이유는 단순합니다. 모델은 바꿀 수 있어도, 흐름이 숨어 있는 구조는 바꾸기 어렵기 때문입니다. 첫 번째 글에서 꼭 가져가야 할 운영 감각도 여기 있습니다. **LangGraph의 기본 단위는 답변 생성이 아니라 상태 전이 설계**라는 사실입니다.
...
- [ ] 노드 함수가 변경할 필드만 반환하고 있는가 (전체 재조립 금지)
- [ ] 마지막 답변이 아니라 중간 상태 필드를 검증하는 습관을 팀 규칙으로 정했다
- [ ] `stream()`으로 노드별 상태 변화를 추적하는 디버깅 루틴을 갖추었다
...
- **LangGraph는 왜 단순 체인보다 명시적인 상태 기계로 보는 편이 좋을까요?**
  - 체인 코드에서는 실행 순서를 머릿속으로 따라가야 할 때가 많습니다. 그래프에서는 그 순서가 코드 구조에 박혀 있습니다. 운영하면서 "왜 이 단계가 먼저 돌았지?"를 따질 수 있다는 것은, 단순히 예쁘게 그릴 수 있다는 의미보다 훨씬 큽니다. 상태 기계로 보면 어느 노드가 어떤 필드를 바꿨는지 추적할 수 있고, 그 추적 가능성이 유지보수 비용을 줄입니다.

- **노드와 엣지, state는 각각 실행 흐름에서 어떤 책임을 나눌까요?**
  - State는 노드들이 공유하는 데이터 계약입니다. Node는 상태를 받아 자신이 책임지는 필드만 갱신하는 작업 단위입니다. Edge는 다음 노드로 이동하는 규칙입니다. 이 세 가지를 분리해서 보면 "어느 단계가 어떤 필드를 읽고 썼는가"라는 질문에 바로 답할 수 있습니다.

- **첫 그래프를 실행한 뒤에는 최종 문장보다 어떤 state 값을 먼저 확인해야 할까요?**
  - `invoke()` 결과에서 중간 단계 필드인 `topic`과 `outline`을 먼저 확인해야 합니다. 최종 답변인 `answer`가 이상할 때 원인은 대부분 앞 단계 필드에서 생깁니다. 상태 필드를 실행 순서대로 확인하는 습관이 디버깅 시간을 크게 줄여 줍니다.

<!-- toc:begin -->
## 시리즈 목차

- **LangGraph 101 (1/6): LangGraph 소개와 그래프 기초 (현재 글)**
