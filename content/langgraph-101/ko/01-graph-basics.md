...
LangChain 기반 에이전트를 처음 붙일 때 많은 팀이 비슷한 벽을 만납니다. 프롬프트를 바꾸면 답변은 달라지는데, 왜 달라졌는지는 설명하기 어렵고, 한 단계에서 생긴 문제가 다음 단계의 문장 안으로 숨어 버립니다. 겉으로는 “에이전트가 똑똑하지 않다”는 문제처럼 보이지만, 실제로는 흐름이 코드 구조에 드러나지 않아서 디버깅이 어려운 경우가 많습니다.

운영에 들어가면 더 직접적으로 드러납니다. 어떤 요청은 잘 끝나는데 어떤 요청은 같은 도구를 두 번 호출하고, 어떤 세션은 중간 상태가 남지 않아 재현이 안 되고, 어떤 팀은 마지막 답변만 비교하다가 앞단 라우팅 오류를 한참 뒤에 발견합니다. 현업 팀들이 “LLM은 맞는 것 같은데 체인이 자꾸 설명 불가능하게 흔들린다”고 말할 때, 저는 대개 흐름이 암묵적으로 묶여 있는지부터 봅니다.

이 글은 LangGraph 101 시리즈의 첫 번째 글입니다. 여기서는 LangGraph를 단순한 에이전트 유틸리티가 아니라, **상태가 명시적으로 이동하는 그래프 런타임**으로 이해하는 출발점을 잡겠습니다. 한 가지 관점이 중요합니다. **LangGraph는 프롬프트를 잘 쓰는 도구라기보다, 상태 전이를 숨기지 않는 워크플로 구조**라는 점입니다.

이 차이를 한 번 잡아 두면 다음 글들이 훨씬 쉬워집니다. 체크포인트는 “대화를 기억하는 기능”이 아니라 상태 스냅샷 저장소로 읽히고, 조건부 엣지는 “if 문 대체재”가 아니라 런타임 분기 규칙으로 보이기 시작합니다. 도구 호출 루프도 마찬가지입니다. 노드, 엣지, 상태를 따로 본 사람은 실패를 단계별로 추적하고, 하나의 체인으로만 본 사람은 마지막 결과 문자열부터 붙잡는 경우가 많습니다.

제가 본 기준으로 LangGraph 입문에서 가장 큰 차이는 문법 암기가 아니라 멘탈 모델입니다. `StateGraph`, `add_node()`, `add_edge()`, `invoke()`를 외우는 것만으로는 운영 감각이 생기지 않습니다. 반대로 “상태가 어디서 바뀌고, 다음 단계가 왜 선택됐는가”를 읽는 눈이 생기면 작은 예제도 곧바로 실무 구조의 축소판으로 보이기 시작합니다.
