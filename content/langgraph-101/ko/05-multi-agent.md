...
- Multi-agent를 책임 분리로 읽기을 실무에 적용할 때 주의할 점은 무엇일까요?
...
- **멀티 에이전트는 여러 모델을 붙이는 일이 아니라 어떤 책임 분리 구조일까요?**
  - 멀티 에이전트에서 가장 먼저 잡아야 할 문장은 이것입니다. **Multi-agent는 책임 분리된 그래프 노드 클러스터**입니다.
- **각 에이전트 노드가 공유 state를 읽고 쓸 때 무엇을 제한해야 할까요?**
  - 각 에이전트 노드가 공유 state를 읽고 쓸 때 무엇을 제한해야 할까요 — 본문에서 구체적으로 다룹니다.
- **handoff나 supervisor 경계가 흐리면 어떤 운영 문제가 생길까요?**
  - 멀티 에이전트를 붙이는 순간부터 아래 항목은 단순한 구조 리뷰가 아니라 위임 안정성 점검 항목이 됩니다.
- **왜 이 구조가 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 멀티 에이전트를 배우는 이유를 “역할을 나눠서 더 똑똑하게 만들 수 있으니까”라고만 설명하면 너무 약합니다.
- **Multi-agent를 책임 분리로 읽기을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 멀티 에이전트에서 가장 먼저 잡아야 할 문장은 이것입니다. **Multi-agent는 책임 분리된 그래프 노드 클러스터**입니다.
- **최소 실행 예제의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 가장 작은 supervisor-worker 예제로 보겠습니다. supervisor가 요청을 읽고 `research` 또는 `code` worker 중 하나를 선택하고, worker는 자신의 전용 결과를 남기며, 마지막에 finalizer가 출력 형식을 정리합니다.
