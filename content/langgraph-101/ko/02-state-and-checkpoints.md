...
이 그림에서는 그래프 실행 중 state가 갱신되고 checkpoint가 호출 사이의 상태를 저장하는 흐름을 봅니다. state와 checkpoint를 분리해서 봐야 멀티턴 실행과 복구 경계를 설명할 수 있습니다.
...
- **LangGraph에서 state를 단일 진실 공급원으로 두면 어떤 버그를 줄일 수 있을까요?**
  - state를 한 곳에 모으면 노드마다 별도 변수를 추측하지 않아도 되어 메시지 누락, 필드 불일치, 중간 결과 유실을 줄일 수 있습니다.
- **checkpoint는 메모리 저장과 무엇이 다르고, 언제 복구 경계가 될까요?**
  - checkpoint는 실행 중 메모리가 아니라 호출 사이에 state를 다시 불러올 수 있게 남기는 저장 지점입니다. 실패 후 재개와 멀티턴 대화의 기준이 됩니다.
- **MemorySaver 예제를 운영 코드로 착각하면 어떤 한계에 부딪힐까요?**
  - MemorySaver는 프로세스 안 예제라 재시작, 다중 인스턴스, 장기 보존을 보장하지 않습니다. 운영에서는 외부 저장소 기반 saver가 필요합니다.
