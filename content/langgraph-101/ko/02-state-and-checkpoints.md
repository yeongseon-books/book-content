...
에이전트가 한 번의 요청으로 끝날 때는 상태를 눈감고 지나갈 수 있습니다. 하지만 대화가 두 턴만 넘어가도 상황이 달라집니다. 첫 번째 턴에서 사용자가 무엇을 말했는지, 어떤 도구 호출 결과가 남아 있는지, 지금 답변이 몇 번째 응답인지가 모두 중요해집니다. 이때 상태를 저장하지 않으면, 두 번째 턴부터는 “이전 맥락을 기억하는 척하는 단발 호출”이 되기 쉽습니다.

운영에서는 이 문제가 더 거칠게 드러납니다. 어떤 세션은 잘 이어지는데 프로세스가 한 번 재시작되자 바로 맥락이 끊기고, 어떤 요청은 중간 실패 후 다시 돌렸더니 같은 작업을 중복 실행하며 비용만 늘어납니다. 체크포인트가 없는 long-running agent는 실패 자체보다 **실패 뒤 복구 불가능성**이 더 비싼 경우가 많습니다.

특히 도구 호출과 멀티턴 대화가 붙기 시작하면, “마지막 사용자 메시지만 다시 보내면 되지 않을까?”라는 생각이 얼마나 위험한지 금방 드러납니다. 이전 메시지 누적 규칙, turn counter, 요약 상태, 외부 도구 응답이 모두 빠져 버리기 때문입니다. 현업에서 저는 팀들이 이 지점을 단순한 편의 기능으로 보고 넘겼다가, 재현이 안 되는 버그와 불안정한 재시도로 시간을 크게 쓰는 장면을 자주 봤습니다.

이 글은 LangGraph 101 시리즈의 두 번째 글입니다. 여기서는 체크포인트를 “대화를 기억하는 기능”이 아니라, **상태를 저장하고 다시 이어 실행하게 만드는 런타임 계층**으로 이해해 보겠습니다. 한 가지 관점이 중요합니다. **State는 그래프의 단일 진실 공급원이고, Checkpoint는 그 진실을 호출 사이에 보존하는 장치**라는 점입니다.

이 관점을 잡아 두면 다음 글도 훨씬 쉬워집니다. 조건부 엣지는 저장된 상태를 보고 다음 경로를 고르는 규칙이 되고, 도구 호출 루프는 같은 상태가 여러 번 변형되는 구조로 읽힙니다. 반대로 상태를 메모리 비슷한 무언가로만 이해하면, 왜 `thread_id`가 필요한지, 왜 병합 규칙을 필드마다 다르게 설계해야 하는지가 계속 흐릿하게 남습니다.
