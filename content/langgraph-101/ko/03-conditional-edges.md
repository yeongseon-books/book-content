...
에이전트가 항상 한 경로만 따라간다면 그래프는 생각보다 단순합니다. 그런데 실제 운영에서는 거의 그렇지 않습니다. 어떤 요청은 코드 생성으로 가야 하고, 어떤 요청은 개념 설명으로, 어떤 요청은 오류 분석으로 가야 합니다. 이 분기를 코드 안쪽 `if/else`에 숨겨 두면 실행은 되지만, 왜 그 길을 탔는지는 바로 흐려집니다.

운영에서 더 까다로운 순간은 분기 실패가 조용히 시작될 때입니다. 분류 라벨 하나가 비어 있거나, 기대하지 않은 문자열이 들어오거나, default 경로가 없어서 그래프가 dead-end에 걸리면 겉으로는 “가끔 이상한 요청에서만 실패하는 시스템”처럼 보입니다. 그런데 실제로는 라우팅 계약이 약해서 생긴 구조 문제인 경우가 많습니다.

여기에 루프까지 붙으면 비용은 더 커집니다. 분기 하나가 잘못되면 같은 노드들 사이를 계속 왕복하거나, 종료해야 할 흐름이 계속 다음 분기로 흘러가기도 합니다. 현업에서 저는 이 상황을 “모델이 멍청하다”는 말로 포장하는 팀을 자주 봤습니다. 하지만 실제 원인은 LLM 품질보다 **분기 규칙이 코드 구조에 얼마나 명시적으로 드러나는가**에 있는 경우가 많습니다.

이 글은 LangGraph 101 시리즈의 세 번째 글입니다. 여기서는 조건부 엣지를 단순한 편의 문법이 아니라, **그래프가 다음 노드를 결정하는 의사결정 지점**으로 이해해 보겠습니다. 한 가지 관점이 중요합니다. **Conditional Edge는 상태를 읽고, 그 상태를 다음 경로 선택으로 번역하는 그래프의 라우팅 경계**라는 점입니다.

이 관점을 잡아 두면 다음 글의 도구 호출 에이전트도 훨씬 쉬워집니다. 도구 호출 루프는 결국 “지금 어떤 상태인가?”를 보고 다음 행동을 고르는 구조이기 때문입니다. 반대로 분기를 단지 런타임의 작은 `if` 정도로만 보면, 나중에 loop termination, fallback route, observability 설계가 왜 중요한지 계속 감이 안 잡히게 됩니다.
