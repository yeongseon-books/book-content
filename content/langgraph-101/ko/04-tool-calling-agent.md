...
도구를 쓰는 에이전트는 데모에서는 늘 똑똑해 보입니다. 계산이 필요하면 계산기를 부르고, 검색이 필요하면 검색기를 부르고, 그 결과를 근거처럼 섞어 답하니까 그럴듯합니다. 그런데 운영에서는 질문이 달라집니다. “왜 이 요청에서만 도구를 세 번 불렀지?” “왜 없는 도구를 호출하려고 했지?” “실패한 도구 결과를 읽고도 왜 또 같은 도구를 다시 불렀지?”

문제는 보통 모델이 도구를 ‘쓸 수 있다’는 사실보다, 그 호출 루프가 얼마나 명시적이고 통제 가능한가에 있습니다. 도구 호출을 모델 내부 판단에만 맡겨 두면, 실패한 도구 재시도와 성공한 도구 후속 응답이 한 덩어리로 섞입니다. 그러면 재현도 어렵고, 로깅 경계도 흐려지고, 어느 지점에서 비용이 폭발했는지도 읽기 힘들어집니다.

특히 side effect가 있는 도구가 붙는 순간 위험은 더 커집니다. 읽기 전용 계산기나 카운터는 비교적 안전합니다. 하지만 외부 API를 호출하거나 파일을 수정하거나 티켓을 생성하는 도구라면, 한 번의 잘못된 루프가 중복 실행과 잘못된 상태 변경으로 이어질 수 있습니다. 현업에서 저는 이 지점을 “도구를 붙였더니 agent가 강해졌다”는 기대만으로 넘겼다가, 나중에 통제 불가능한 재시도와 실패 복구 비용을 크게 치르는 팀을 자주 봤습니다.

이 글은 LangGraph 101 시리즈의 네 번째 글입니다. 여기서는 도구 호출 에이전트를 “모델이 알아서 도구를 쓰는 구조”가 아니라, **LLM 판단과 실제 도구 실행을 분리한 안전한 실행 envelope**로 이해해 보겠습니다. 한 가지 관점이 중요합니다. **Tool-calling agent는 LLM + ToolNode + 종료 규칙이 결합된 제어 루프**라는 점입니다.
