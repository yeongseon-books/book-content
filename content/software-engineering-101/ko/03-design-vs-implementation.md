...
이 글은 Software Engineering 101 시리즈의 3번째 글입니다.
...
## 프로젝트 관리 예시: 설계 리뷰를 스프린트에 넣는 방법

설계와 구현을 명확히 나누면 스프린트 진행도 달라집니다. 아래는 "주문 취소 API 추가" 태스크를 설계/구현으로 분리한 예시입니다.

```markdown
[스프린트 계획]
태스크 1 (설계 단계 - 1일):
- 주문 취소 API의 책임 경계 정의
- 취소 가능 상태 목록과 실패 케이스 정의
- 데이터 정합성 규칙 명시 (부분 취소 허용 여부)
- 인터페이스 스케치 + ADR 초안 작성
- 설계 리뷰 30분 (팀원 2인 이상)

태스크 2 (구현 단계 - 3일):
- 태스크 1 리뷰 완료 후 착수
- 인터페이스 기반 구현 (Fake로 단위 테스트 병행)
- 통합 테스트 작성
- PR 작성 (설계 ADR 링크 포함)

완료 기준: ADR 승인 + PR 머지 + 배포 확인
```

이렇게 구분하면 PR 리뷰 시점에 "설계 자체가 맞는가"를 논쟁하는 일이 줄어듭니다. 설계 리뷰와 구현 리뷰를 분리하면 각 단계의 초점이 선명해집니다.
...
|---|---|---|---|
| 설계 없이 구현 시작 | "일단 짜고 나중에 리팩터링" | 설계 판단이 코드 깊숙이 숨어 변경 비용 증가 | 인터페이스 스케치 + ADR 초안을 구현 전에 먼저 작성 |
| 과도한 선행 추상화 | 채널이 하나인데 팩토리+레지스트리 도입 | 단순함이 사라지고 유지비만 늘어남 | YAGNI: 실제 변경 압력이 생길 때까지 추상화 미룸 |
| ADR 미작성 | "코드를 보면 이해할 수 있다"로 넘어감 | 6개월 뒤 같은 논의 반복 | 변경 비용이 큰 결정에 한해 ADR 한 장 의무화 |
| 관측성 후순위 | 배포 후 로그가 없어 장애 원인 파악 불가 | 장애 복구 시간 증가 | 설계 문서에 "어떤 이벤트를 로깅할지" 항목 포함 |
| 설계 리뷰 생략 | 구현 PR에서 설계 논쟁이 시작됨 | 리뷰 비용 급증, 머지 지연 | 큰 기능은 설계 검토 PR과 구현 PR을 분리 |
...
설계는 문서 한 장으로 끝나는 활동이 아니라, 구현 전에 팀이 같은 경계를 공유하는 합의 과정입니다.
...
- **잘 작성된 코드와 잘 설계된 시스템은 무엇이 다를까요?**
  - 잘 작성된 코드는 개별 함수나 클래스가 읽기 좋고 오류를 잘 처리합니다. 잘 설계된 시스템은 각 컴포넌트의 경계가 명확하고, 어떤 변경이 어떤 범위에 영향을 주는지 예측 가능합니다. 코드가 아무리 깔끔해도 경계 설계가 잘못되면 기능 추가마다 예상치 못한 곳이 깨집니다.
- **설계는 "무엇을", 구현은 "어떻게"라는 말은 실제로 무슨 뜻일까요?**
  - 설계는 "어떤 컴포넌트가 어떤 책임을 가지고, 어떤 인터페이스로 통신하며, 어떤 실패 케이스를 어디서 처리하는가"를 정합니다. 구현은 그 결정에 따라 실제 코드를 작성합니다. 설계가 "무엇을 분리할지"를 정하면 구현은 "어떤 언어와 패턴으로 실현할지"를 결정합니다.
- **ADR은 언제 쓰고, 어떤 수준으로 남겨야 할까요?**
  - "이 결정을 되돌리거나 바꿀 때 팀이 왜 이렇게 만들었는지 알아야 하는가?"라는 질문에 "예"이면 ADR을 작성합니다. 기술 스택 선택, 서비스 경계 설정, 데이터 저장 전략 같은 되돌리기 어려운 결정이 대상입니다. 형식은 길 필요가 없습니다. 배경·결정·대안·결과를 각각 두세 줄씩만 써도 충분합니다.
...
- **Software Engineering 101 (3/10): 설계와 구현의 차이 (현재 글)**
