...
- 단계별로 작은 릴리스 파이프라인 만들기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
릴리스 프로세스에서 가장 중요한 원칙은 되돌릴 수 있는 변경만 배포한다는 사실입니다. 데이터베이스 마이그레이션처럼 되돌리기 어려운 변경은 별도 단계로 분리하고, 애플리케이션 코드 배포와 시간차를 두어야 합니다. 이렇게 하면 코드 롤백 시 스키마 불일치로 인한 장애를 예방할 수 있습니다.
...
이 예시의 핵심은 롤백 기준이 수치로 정의되어 있다는 사실입니다. "문제가 커 보이면 되돌린다" 같은 주관적 기준은 운영자마다 판단이 달라집니다.
...
- **브랜치 전략은 언제 trunk-based가 맞고, 언제 Git Flow가 맞을까요?**
  - 이 글은 짧은 브랜치를 자주 main에 합치고 카나리로 바로 확인하는 흐름이 가능할 때 trunk-based가 잘 맞는다고 설명했습니다. 반대로 release나 hotfix 단계가 길게 분리되는 조직이라면 Git Flow가 도움이 될 수 있지만, 핵심은 브랜치 이름보다 테스트 통과·리뷰 승인·변경 로그 작성 같은 병합 기준을 명확히 두는 일입니다.
- **버전 1.4.2 같은 숫자는 사용자에게 무엇을 약속할까요?**
  - 본문의 SemVer 규칙처럼 `feat -> MINOR`, `fix -> PATCH`, `BREAKING CHANGE -> MAJOR`가 지켜질 때 버전 번호는 호환성 약속이 됩니다. 사용자는 PATCH에서 큰 동작 변화가 없기를 기대하고, MAJOR에서는 인터페이스나 사용 방식 변화 가능성을 예상할 수 있습니다.
- **체인지로그는 어떻게 자동화할 수 있을까요?**
  - 자동화의 출발점은 `feat(auth): ...`, `fix(billing): ...` 같은 Conventional Commits를 꾸준히 남기는 일입니다. 여기에 `release-please-action`과 태그 기반 배포를 붙이면 릴리스 PR, 사용자 관점의 변경 기록, 카나리 이후 승격 절차까지 하나의 반복 가능한 파이프라인으로 묶을 수 있습니다.
