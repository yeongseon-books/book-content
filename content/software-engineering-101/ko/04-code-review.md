...
## 프로젝트 관리 예시: 팀 리뷰 SLA 설정하기

코드 리뷰가 팀 속도를 막는 병목이 되는 경우는 대부분 리뷰 리드타임이 너무 길기 때문입니다. 아래는 리뷰 SLA를 팀 기준으로 고정한 예시입니다.
...
재리뷰: 작성자 반영 후 2시간 이내
승인 기한: 리뷰 요청 후 1영업일 이내

예외 상황:
- 긴급 핫픽스: 1시간 이내 첫 리뷰
- 리뷰어 부재: Slack 공지 후 대체 지정
...
목표: PR 생성에서 머지까지 24시간 이내 (핫픽스 제외)
```

이 기준이 팀에 문서화되면 "리뷰가 느리다"는 불만이 구체적인 개선 행동으로 바뀝니다. 데이터를 주간 회고에서 공유하면 프로세스 병목을 조기에 감지할 수 있습니다.
...
|---|---|---|---|
| 거대 PR 제출 | 500줄 이상 diff로 리뷰 요청 | 설계·경계 판단보다 표면 지적만 남음 | PR을 머지 가능한 최소 단위로 분할 |
| 자동화할 것을 사람이 검토 | 포매팅·타입 오류를 리뷰 코멘트로 남김 | 리뷰어 집중력이 사소한 항목에 낭비됨 | CI에 linter·타입체커·테스트 필수화 |
| 의도 없는 제목 | "fix bug", "update code"처럼 모호한 PR 제목 | 히스토리 검색 시 해당 변경을 찾기 어려움 | Conventional Commits 형식으로 제목 작성 |
| 공격적 코멘트 | "이게 왜 이래요?"처럼 코드보다 작성자를 향한 문장 | 학습 분위기 저하, 리뷰 회피 심화 | 관찰·영향·제안·근거 순서로 코멘트 구조화 |
| 기술부채 코멘트 방치 | "나중에 고쳐요"만 적고 이슈 미연결 | 부채가 추적 불가 상태로 쌓임 | `debt:` 접두사 + 이슈 번호 필수화 |
...
- **코드 리뷰는 결함 탐지와 지식 공유 중 무엇이 더 중요할까요?**
  - 두 가지는 분리할 수 없습니다. 결함 탐지는 즉각적인 품질 보호이고, 지식 공유는 장기적인 팀 역량 강화입니다. 결함 탐지만 집중하면 특정 영역에 대한 지식이 소수에게 몰립니다. 지식 공유만 강조하면 중요한 오류를 놓칩니다. 리뷰가 두 목적을 동시에 달성하려면 PR 크기가 작아야 하고, 리뷰어가 충분히 문맥을 이해할 수 있어야 합니다.
- **리뷰하기 쉬운 PR은 어떻게 만들 수 있을까요?**
  - 첫째, 하나의 의도만 담습니다. 버그 수정과 리팩터링을 같은 PR에 넣지 않습니다. 둘째, 제목 한 줄로 변경의 핵심을 설명합니다. 셋째, 본문에 "왜 이 변경이 필요한가"를 먼저 적습니다. 넷째, 검증 방법과 예외 케이스를 명시합니다. 다섯째, 300줄 이하를 목표로 합니다.
- **리뷰어는 실제로 어떤 항목을 먼저 볼까요?**
  - 경험 있는 리뷰어는 대개 다음 순서로 봅니다. 첫째 요구사항과 구현의 일치 여부, 둘째 실패 케이스와 경계 조건 처리, 셋째 운영 가시성(로그·메트릭·알람), 넷째 설계 경계 위반 여부, 다섯째 이름과 가독성입니다. CI가 포매팅과 타입을 처리해 준다면 리뷰어의 첫 눈길은 의도와 경계 판단으로 바로 향할 수 있습니다.
