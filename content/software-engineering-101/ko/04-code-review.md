...
- 한눈에 보는 흐름에서 가장 흔한 실수는 무엇일까요?
- 전후 비교을 실무에 적용할 때 주의할 점은 무엇일까요?
- 단계별로 리뷰 가능한 변경 묶음 만들기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
다음 글에서는 결함 탐지를 리뷰에만 기대지 않고, 테스트 전략으로 어떻게 앞당길지 다룹니다. 어떤 테스트를 어느 층에 놓아야 변경 비용이 내려가는지 이어서 봅니다.

## 처음 질문으로 돌아가기

- **코드 리뷰는 결함 탐지와 지식 공유 중 무엇이 더 중요할까요?**
  - 이 글은 코드 리뷰를 결함 탐지와 지식 분산을 동시에 수행하는 장치로 설명했습니다. `ExpiredTokenError`를 401로 매핑하는 변경처럼 버그를 잡는 일도 중요하지만, PR 제목·본문·코멘트 태그를 통해 왜 그렇게 바꿨는지 팀이 함께 이해하는 효과가 장기적으로 더 큽니다.
- **리뷰하기 쉬운 PR은 어떻게 만들 수 있을까요?**
  - 리뷰하기 쉬운 PR은 `fix(auth): handle expired refresh token without 500`처럼 제목만으로 의도가 보이고, 본문에 What, Why, How, Test가 모두 들어 있어야 합니다. 여기에 데이터 모델, 서비스 로직, 라우팅을 별도 PR로 나누면 리뷰어가 구조 판단과 운영 위험에 집중하기 쉬워집니다.
- **리뷰어는 실제로 어떤 항목을 먼저 볼까요?**
  - 본문 체크리스트대로라면 리뷰어는 스타일보다 요구사항 일치 여부, 예외 경로 테스트, 로그와 모니터링 포인트, 후속 부채 처리 여부를 먼저 봐야 합니다. 포매팅과 타입 검사는 `ruff`, `pytest`, frontmatter 검사처럼 CI가 맡고, 사람은 변경이 시스템 경계와 운영성에 미치는 영향을 판단하는 편이 맞습니다.
...
- **Software Engineering 101 (4/10): 코드 리뷰 (현재 글)**
