...
결제 팀 사례를 보면 부채 관리가 스프린트 운영에서 어떻게 작동하는지 구체적으로 드러납니다.

**상황**: 레거시 결제 서비스에 재시도 로직이 3개 모듈에 중복 존재. 장애 발생 시 각각 다르게 동작해 디버깅 비용이 높음.
...
Sprint 23 계획
- 기능 개발 용량: 80% (4명 × 2주)
- 부채 상환 용량: 20% (0.8명 × 2주 = 개발자 1명이 집중)

부채 상환 태스크 (D-12 결제 재시도 중복):
  Day 1-2: 공통 RetryPolicy 인터페이스 설계 + ADR 작성
  Day 3-5: 첫 번째 모듈 교체 + 회귀 테스트 추가
  Day 6-8: 두 번째, 세 번째 모듈 교체
  Day 9-10: 통합 테스트 + 부채 인덱스 갱신
```

**완료 기준 (DoD)**:
- 세 모듈 모두 공통 RetryPolicy 사용
- 재시도 관련 테스트 커버리지 +15%
- D-12 부채 항목에 "closed" 표시 및 상환 효과 지표 기록
- 다음 온콜 핸드오프 문서 업데이트

**결과 측정**:
```text
상환 전: 장애 시 디버깅 평균 45분, 관련 코드 이해 시간 60분
상환 후: 장애 시 디버깅 평균 12분, 관련 코드 이해 시간 15분
```

이 예시가 보여주는 핵심은 부채 상환을 "언젠가 해야 할 일"이 아니라 "이번 스프린트에 누가 언제 무엇을 측정하며 한다"는 태스크로 변환하는 것입니다.
...
|-----------|------------|------------|----------|
| 빅뱅 리팩터링 계획 | "다음 분기에 전면 개편" 선언 후 범위 폭발 | 변경 영향도 파악 전에 일정 약속 → 장애 리스크 집중 | Strangler Fig로 모듈 하나씩 교체, 기능 플래그로 롤백 보장 |
| 부채를 개인 탓으로 귀인 | "저 사람이 짠 코드라 어쩔 수 없어" | 구조적 원인 탐색 차단, 재발 방지 불가 | 부채 발생 맥락(일정 압박, 정보 부족)을 문서화하고 시스템 개선 |
| deprecation 즉시 제거 | 경고 없이 구 API 삭제 → 타팀 장애 | 호출자 준비 시간 미보장, 신뢰 손상 | 경고 → 마이그레이션 기간(최소 1분기) → 제거 3단계 의무화 |
| 부채 항목에 소유자 없음 | 인덱스가 있지만 아무도 책임 안 짐 | 분기 회고 때 같은 항목이 반복 등장 | 등록 시 소유자 + 목표 분기 필수 필드로 강제 |
| 스프린트에 부채 용량 미확보 | 매 스프린트 기능 100% → 부채 자동 이월 | 리드 타임 서서히 증가, 팀 피로 누적 | 스프린트 용량의 10-20%를 부채 상환 예산으로 고정 |
...
- **기술부채는 언제 도구이고, 언제 사고의 씨앗이 될까요?**
  - 소유자, 만기, 상환 계획이 있는 부채는 속도를 위한 도구입니다. 세 가지 중 하나라도 빠지면 사고의 씨앗이 됩니다. "의도적이고 신중한 부채"는 다음 분기 계획에 이미 들어가 있고, "비의도적이고 무모한 부채"는 누구도 인식하지 못한 채 장애 당일 발견됩니다. 차이는 기록과 추적 여부입니다.
- **Martin Fowler의 네 가지 기술부채 분류는 실무에 어떤 도움을 줄까요?**
  - 의도성(intentional/inadvertent)과 신중함(prudent/reckless)의 2×2 매트릭스는 부채 발생 원인을 진단하는 언어를 제공합니다. "이 부채가 왜 생겼는가"를 설명할 수 있어야 상환 전략도 다르게 세울 수 있습니다. 비의도적으로 생긴 부채는 팀 학습과 페어 리뷰로 예방하고, 의도적으로 진 부채는 명시적 마감일과 소유자 배정으로 관리합니다.
- **부채 상환 우선순위는 어떻게 정해야 할까요?**
  - 상환 비용과 미루는 비용을 함께 봐야 합니다. 상환 비용만 보면 항상 나중으로 밀립니다. 미루는 비용에는 장애 복구 시간 증가, 배포 리드타임 악화, 신규 인력 온보딩 지연이 포함됩니다. 이 세 가지 중 하나라도 측정 가능한 항목이라면 우선순위 논의가 데이터 기반으로 바뀝니다.
