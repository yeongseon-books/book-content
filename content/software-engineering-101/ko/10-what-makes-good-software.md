...
- 단계별로 작은 품질 키트 만들기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **좋은 코드와 좋은 소프트웨어는 왜 같은 말이 아닐까요?**
  - 좋은 코드는 클래스와 함수 수준에서 읽기 쉽고 책임이 분리된 상태를 뜻하지만, 좋은 소프트웨어는 그 코드가 시간이 지나도 낮은 리드타임과 짧은 MTTR, 빠른 온보딩으로 이어지는지까지 포함해 판단해야 합니다. 그래서 이 글은 `It works` 대신 복잡도, 배포 리드타임, 신규 입사자의 첫 PR 시간 같은 외부 신호를 함께 보라고 강조했습니다.
- **품질 속성은 어떤 축으로 나뉘고, 왜 균형이 중요할까요?**
  - 본문은 기능 적합성, 신뢰성, 유지보수성, 성능 효율성 같은 축을 따로 보되, 품질 기준표에서는 변경 용이성, 운영 안정성, 협업 투명성, 학습 가능성까지 함께 묶었습니다. 한 축만 밀면 다른 축이 무너질 수 있으므로, 가용성 99.9%, p95 300ms, MTTR 30분 이하처럼 서로 다른 품질 목표를 동시에 관리해야 균형이 잡힙니다.
- **SOLID 원칙은 실무에서 어떤 기준으로 읽어야 할까요?**
  - SRP와 DIP 예시는 SOLID를 암기 과목이 아니라 변경 비용을 줄이는 판단 도구로 읽어야 한다는 점을 보여 줍니다. 클래스 하나에 변경 이유가 여러 개 몰리거나 구체 구현에 직접 묶이는 순간 유지보수성이 떨어지므로, 원칙은 추상화를 늘리기보다 어떤 경계가 바뀌기 쉬운지 읽는 기준으로 써야 합니다.
