...
- 단계별로 작은 문서 세트 만들기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **좋은 README는 최소한 어떤 블록을 가져야 할까요?**
  - 이 글은 좋은 README의 최소 단위로 무엇을 바꾸는가, 왜 바꾸는가, 60초 안에 실행하는 빠른 시작, 설정, 더 읽을 거리를 제시했습니다. 최근 확장 예시에서는 로컬 실행·테스트, 배포 경로, 자주 묻는 운영 질문까지 더해 첫 5분 안에 프로젝트의 가치와 진입 경로가 보이게 해야 한다고 설명했습니다.
- **ADR은 무엇을 남기고, 코드 주석과 어떻게 다를까요?**
  - ADR은 `Context, Decision, Alternatives, Consequences`처럼 왜 그런 구조를 택했는지와 어떤 대안을 버렸는지를 남기는 문서입니다. 코드 주석이나 docstring이 함수와 클래스의 사용 계약을 설명한다면, ADR은 그 코드 바깥에서 의사결정의 배경과 영향 범위를 보존한다는 점이 다릅니다.
- **docstring과 타입 힌트는 문서화에서 어떤 역할을 할까요?**
  - `compute_invoice(amount: int, tax_rate: float)` 예시처럼 타입 힌트는 함수가 어떤 입력과 출력을 기대하는지 가장 가까운 계약을 드러냅니다. 여기에 docstring이 음수 입력에서 `ValueError`가 난다는 점까지 적어 주면 사용자는 코드를 열자마자 정상 경로와 실패 경로를 함께 이해할 수 있습니다.
