...
- 단계별로 보는 책임 확장의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **코드를 작성하는 일과 소프트웨어 엔지니어링은 무엇이 다를까요?**
  - 이 글은 `need -> design -> code -> test -> operate -> change -> reflect -> repeat` 흐름으로, 한 번 실행하고 끝나는 코딩과 오래 운영할 시스템을 다루는 엔지니어링을 구분했습니다. `sum_to` 예제가 입력 검증, 로깅, 테스트, README까지 단계적으로 늘어나는 이유도 코드 자체보다 변경 가능성과 운영 가능성을 함께 책임져야 하기 때문입니다.
- **엔지니어는 코드 외에 어떤 영역까지 책임져야 할까요?**
  - 본문은 요구사항 문서 템플릿, 설계 리뷰 체크리스트, Git 워크플로, 기술부채 추적 항목, 릴리스 후 점검 루틴까지를 모두 엔지니어의 책임 범위로 묶었습니다. 특히 로그·메트릭·트레이스를 설계 단계에서 넣고, 배포 후 10분·1시간·24시간 단위로 지표를 확인해야 시스템이 실제로 운영 가능한 상태가 됩니다.
- **“맞는 일을 고르는 것”과 “일을 제대로 만드는 것”은 어떻게 다를까요?**
  - 맞는 일을 고르는 일은 기능 실행 카드와 요구사항 문서처럼 문제, 성공 조건, 제외 범위, 롤백 기준을 먼저 정하는 일입니다. 일을 제대로 만드는 일은 그 선택을 테스트, ADR, 대시보드, 배포 노트 같은 재현 가능한 장치로 연결해 시간이 지나도 같은 품질을 유지하게 만드는 일입니다.
