...
이 그림에서는 오픈소스 포트폴리오를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> 오픈소스 포트폴리오의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **오픈소스 기여가 어떻게 포트폴리오가 될 수 있을까요?**
  - 본문의 기준은 오픈소스 포트폴리오를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **프로필 소개 문서와 고정 저장소는 각각 무엇을 보여 줄까요?**
