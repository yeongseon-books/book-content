...
이 그림에서는 이슈 읽기를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> 이슈 읽기의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **제목만 보고 판단하면 왜 기여 방향이 자주 빗나갈까요?**
  - 본문의 기준은 이슈 읽기를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **라벨, 재현 절차, 담당자, 댓글은 각각 어떤 역할을 할까요?**
