...
이 표가 말해주는 것은, "완벽한 자동화"가 원리적으로 불가능한 영역이 있다는 사실입니다. 그래서 엔지니어는 완벽함 대신 "충분히 좋은" 근사와 안전장치를 조합합니다.
...
- **정지 문제처럼 원리적으로 풀 수 없는 문제는 무엇을 의미할까요?**
  - 명령형, 함수형, 객체지향 패러다임을 같은 문제에 적용해 보겠습니다. 문제는 "단어 목록에서 길이가 4 이상인 단어만 골라 대문자로 바꾸기"입니다.
- **이 글에서 배울 것에서 가장 흔한 실수는 무엇일까요?**
  - "모든 문제를 프로그램으로 풀 수 있을까?"라는 질문의 답은 "아니오"입니다. 계산 이론은 풀 수 있는 문제와 풀 수 없는 문제의 경계를 알려줍니다.
- **적용 전후 비교을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 명령형, 함수형, 객체지향 패러다임을 같은 문제에 적용해 보겠습니다. 문제는 "단어 목록에서 길이가 4 이상인 단어만 골라 대문자로 바꾸기"입니다.
- **단계별로 따라하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 튜링 기계의 핵심 아이디어입니다. 현재 상태와 현재 기호를 보고 다음 행동을 결정합니다.
