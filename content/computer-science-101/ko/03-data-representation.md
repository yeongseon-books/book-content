...
- **컴퓨터는 0과 1만으로 숫자, 문자, 이미지를 어떻게 저장할까요?**
  - 실무에서 가장 흔한 인코딩 문제는 "깨진 글자"입니다
- **ASCII와 UTF-8은 무엇이 다르고 왜 바이트 수가 달라질까요?**
  - 같은 정수라도 메모리에 저장하는 바이트 순서가 다를 수 있습니다. 이 차이가 네트워크 프로토콜과 파일 포맷에서 문제를 일으킵니다.
- **음수는 왜 2의 보수 방식으로 표현할까요?**
  - - 이진수와 십진수 변환 원리 - ASCII와 UTF-8 인코딩의 차이 - 부호 있는 정수와 2의 보수 표현 - 부동소수점이 근사값이라는 사실과 그 영향 문자 깨짐, 부동소수점 오차, 정수 오버플로우는 모두 데이터 표현을 모르면 해결할 수 없는 문제입니다
- **이 글에서 배울 것에서 가장 흔한 실수는 무엇일까요?**
  - 문자 깨짐, 부동소수점 오차, 정수 오버플로우는 모두 데이터 표현을 모르면 해결할 수 없는 문제입니다. `0.1 + 0.2 != 0.3`이 되는 이유를 알아야 금융 시스템에서 올바른 설계를 할 수 있습니다.
- **적용 전후 비교을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - - 웹 API에서 Content-Type 헤더의 charset=utf-8 설정 - 금융 시스템에서 Decimal 또는 정수 기반 금액 처리 - 데이터베이스 컬럼 타입 선택 시 정수 크기 결정 (INT vs BIGINT) - 파일 처리 시 BOM(Byte Order Mark)과 인코딩 감지 - 네트워크 프로토콜에서 바이트
- **단계별로 따라하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - - 같은 비트열도 해석 방식에 따라 숫자, 문자, 색상이 됩니다 - UTF-8은 문자마다 바이트 수가 다르므로 `len(문자열) != len(바이트열)`입니다 - 부동소수점은 근사 표현이므로 직접 비교하면 안 됩니다 - Python 정수는 오버플로우가 없지만, 다른 언어에서는 주의해야 합니다
...
- **Computer Science 101 (3/10): 데이터 표현 (현재 글)**
