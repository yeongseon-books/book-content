...
- **같은 문제를 푸는 두 코드 중 무엇이 더 빠를지 어떻게 판단할까요?**
  - - 같은 문제를 푸는 알고리즘도 자료구조 선택에 따라 복잡도가 달라집니다 - `list`의 `in`은 O(n), `set`과 `dict`의 `in`은 평균 O(1)입니다 - 정렬은 가능하면 표준 라이브러리(`sorted`, `list.
- **시간 복잡도와 공간 복잡도는 무엇을 각각 뜻할까요?**
  - - 좋은 알고리즘을 판단하는 기준 - 시간 복잡도와 공간 복잡도의 의미 - Big-O로 성능 증가율을 읽는 방법 - 자료구조 선택이 복잡도를 바꾸는 방식 코드가 100개 데이터에서 잘 돌아가도 100만 개에서는 멈출 수 있습니다
- **Big-O 표기법은 왜 코드를 실행하지 않고도 성능을 가늠하게 해 줄까요?**
  - - 좋은 알고리즘을 판단하는 기준 - 시간 복잡도와 공간 복잡도의 의미 - Big-O로 성능 증가율을 읽는 방법 - 자료구조 선택이 복잡도를 바꾸는 방식 코드가 100개 데이터에서 잘 돌아가도 100만 개에서는 멈출 수 있습니다
- **이 글에서 배울 것에서 가장 흔한 실수는 무엇일까요?**
  - 코드가 100개 데이터에서 잘 돌아가도 100만 개에서는 멈출 수 있습니다. 데이터 크기에 따른 성능 변화를 예측하지 못하면 출시 후에 장애로 이어집니다. Big-O는 코드를 실행해 보지 않고도 성능 한계를 미리 판단하게 해 줍니다.
- **적용 전후 비교을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - - API 응답 시간 분석에서 데이터 증가에 따른 회귀 측정 - 데이터베이스 쿼리 EXPLAIN 결과 해석 시 인덱스 유무에 따른 복잡도 차이 - 캐시 자료구조 선택 (LRU에는 `OrderedDict`, 빠른 멤버십 검사는 `set`) - 대용량 로그 처리에서 스트리밍 알고리즘으로 공간 복잡도 절감 - 검색·추천
- **단계별로 따라하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - - 같은 문제를 푸는 알고리즘도 자료구조 선택에 따라 복잡도가 달라집니다 - `list`의 `in`은 O(n), `set`과 `dict`의 `in`은 평균 O(1)입니다 - 정렬은 가능하면 표준 라이브러리(`sorted`, `list.
...
- **Computer Science 101 (4/10): 알고리즘과 복잡도 (현재 글)**
