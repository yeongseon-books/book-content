...
| 역량 수준 | 질문: "일별 DAU를 계산하세요" | 면접관 판단 |
| --- | --- | --- |
| 기초 | `SELECT date, COUNT(*) FROM events GROUP BY date` | 로그인 이벤트와 전체 이벤트를 구분하지 못함 |
| 중급 | `SELECT date, COUNT(DISTINCT user_id) FROM events WHERE event_name='login' GROUP BY date` | 고유 사용자 기준 이해, 하지만 필터 조건 설명 없음 |
| 실무 수준 | 위 쿼리 + "DAU는 로그인 이벤트 기준인지 앱 실행 기준인지 먼저 확인하겠습니다. 이중 로그인도 제거 대상인지 여쭤봐도 될까요?" | 지표 정의 먼저 합의하는 실무 감각 |
...
| 상황 | 권장 JOIN | 이유 |
| --- | --- | --- |
| 구매 이력 없는 사용자도 포함해야 함 | LEFT JOIN | 오른쪽 테이블 매칭 없어도 왼쪽 유지 |
| 양쪽 모두 일치하는 행만 필요 | INNER JOIN | 두 테이블 모두 매칭되는 행만 반환 |
| 모든 행 + 매칭 정보 함께 필요 | FULL OUTER JOIN | 양쪽 모두 유지하며 NULL로 누락 표시 |
| 두 집합의 차이를 구해야 함 | LEFT JOIN + WHERE IS NULL | 오른쪽에 없는 왼쪽 행만 필터 |
...
| 함수 | 용도 | 동률 처리 |
| --- | --- | --- |
| `ROW_NUMBER()` | 중복 없는 순번 부여 | 동률도 서로 다른 번호 |
| `RANK()` | 동률 허용 순위 | 동률이면 같은 번호, 다음 번호는 건너뜀 |
| `DENSE_RANK()` | 동률 허용 연속 순위 | 동률이면 같은 번호, 다음 번호는 연속 |
| `LAG(col, n)` | n행 이전 값 참조 | - |
| `LEAD(col, n)` | n행 이후 값 참조 | - |
...
**시나리오 예시:** 면접관이 "재구매율이 15%가 나왔습니다. 이 수치를 어떻게 해석하겠습니까?"라고 추가 질문을 했을 때:

약한 답변: "낮은 편인 것 같습니다."

강한 답변: "산업 평균과 비교하거나 전분기 수치와 비교해야 판단할 수 있습니다. 또 유입 채널별로 재구매율을 분해하면 어떤 채널이 충성 사용자를 더 많이 데려오는지 파악할 수 있습니다. 신규 프로모션 기간에 가입한 코호트와 그렇지 않은 코호트의 재구매율 차이도 확인하면 프로모션 효과를 측정할 수 있습니다."
...
```sql
-- 1단계: 데이터 파이프라인 이상 여부 확인
SELECT date, COUNT(*) AS total_events
FROM events
...
-- 2단계: 플랫폼별 DAU 분해
SELECT date, platform, COUNT(DISTINCT user_id) AS dau
FROM events
...
-- 3단계: 유입 채널별 신규 사용자 분해
SELECT date, acquisition_channel, COUNT(DISTINCT user_id) AS new_users
FROM users
...
| JOIN 방향 설명 못함 | LEFT JOIN을 쓰면서 이유를 모름 | JOIN 종류와 사용 이유를 항상 함께 설명 |
...
| 시기 | 준비 항목 | 목표 |
| --- | --- | --- |
| 인터뷰 1주 전 | 패턴 5가지를 손으로 직접 작성 | 문법이 아니라 구조가 손에 익도록 |
| 인터뷰 2-3일 전 | 시간 재며 90초 답변 구조 연습 | 속도와 논리 흐름 점검 |
| 인터뷰 전날 | 지원 회사 도메인에서 나올 법한 지표 정의 정리 | DAU, MAU, 전환율, LTV, CAC 등 |
| 인터뷰 당일 | 첫 질문에서 지표 정의 확인하는 습관 상기 | 쿼리보다 지표 먼저 |
...
- [ ] DAU 하락 진단 시나리오를 처음부터 끝까지 연습했다
- [ ] 지원 회사 도메인에서 쓸 법한 지표 5개를 정의해 봤다

## 정리

SQL과 분석 인터뷰는 쿼리 시험이면서 동시에 사고력 시험입니다. 질문을 지표 언어로 번역하고, 가정을 소리 내어 말하고, CTE로 구조를 잡고, 결과를 해석하는 흐름이 반복되어야 면접 답변이 완성됩니다. 쿼리 정확도보다 이 과정의 일관성이 면접 결과를 더 많이 좌우합니다. 인터뷰 직전 준비 루틴을 따라 패턴을 손으로 익히고 지표 언어를 먼저 정의하는 습관을 만들어 두면 낯선 도메인에서도 안정적으로 답변할 수 있습니다.
