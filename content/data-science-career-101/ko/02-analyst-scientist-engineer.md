...
이 그림에서는 분석가 vs 사이언티스트 vs 엔지니어를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> 분석가 vs 사이언티스트 vs 엔지니어의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **분석가, 사이언티스트, 엔지니어가 각각 어떤 목적을 중심에 두는지 비교합니다?**
  - 본문의 기준은 분석가 vs 사이언티스트 vs 엔지니어를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **세 역할이 만드는 대표 산출물을 정리합니다?**
