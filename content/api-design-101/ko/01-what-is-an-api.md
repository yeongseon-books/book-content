...
API의 정의와 역할, 좋은 API의 조건을 먼저 잡아 두면 이후 설계 원칙이 훨씬 잘 보입니다.

이 글은 API Design 101 시리즈의 첫 번째 글입니다.

## 이 글에서 다룰 문제

API는 시스템의 얼굴입니다. 안쪽 구현이 바뀌어도 API가 안정적이면 외부는 영향받지 않습니다. 반대로 API가 흔들리면 내부 변경이 외부 폭풍이 됩니다.

> API는 변하지 않을 약속에 가까워야 합니다.
...
HTTP는 가장 흔한 전송 방식입니다.

### 3단계 — 약속 살펴보기

```python
# 예제 3: 계약 확인
# GitHub 저장소 조회 API의 약속
# - 입력: 저장소 식별 경로
# - 출력: 200 OK와 JSON 응답
```
...
- 응답의 상태 코드와 본문이 약속의 양 축입니다.

## 자주 하는 실수 5가지

1. **약속 문서 없이 시작합니다.** 클라이언트는 구현을 추측하게 됩니다.
2. **상태 코드를 무시합니다.** 200으로 모든 것을 돌려보냅니다.
3. **에러 본문이 자유 형식입니다.** 클라이언트가 파싱하지 못합니다.
4. **버전 없이 배포합니다.** 변경이 외부 폭풍이 됩니다.
5. **클라이언트 없이 설계합니다.** 사용자 입장이 빠집니다.

## 실무에서는 이렇게 쓰입니다

GitHub REST API, Stripe API, Google Maps API는 모두 문서화된 약속입니다. 사내에서는 OpenAPI(Swagger) 문서가 그 역할을 합니다. 좋은 회사일수록 내부 API도 외부 API처럼 다룹니다.
...
API는 약속입니다. 다음 글에서는 그 약속의 가장 흔한 형태인 REST 기본을 봅니다.
