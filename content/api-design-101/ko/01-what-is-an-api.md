...
# 계약: GET /users → 200 + 사용자 접근의 JSON 배열
...
웹 API에서는 HTTP가 전송 수단이고, JSON이 데이터 형식입니다. 여기서 중요한 점은 GitHub이 이 endpoint의 응답 형태를 바꾸지 않는 한, 전 세계의 GitHub CLI, IDE 플러그인, CI 스크립트가 계속 동작한다는 사실입니다. 수만 개의 클라이언트가 하나의 계약에 의존합니다.

### 단계 3 — 계약을 읽는다

```python
# 3_contract.py
# GitHub REST API 문서에서
# GET /repos/{소유자}/{repo}
#
# 경로 매개변수:
# 소유자(문자열, 필수) — 포인터 위치
...
#
# 성공 응답: 200 OK
# 본문: { "full_name": str, "stargazers_count": int, ... }
#
# 실패 응답: 404 Not Found
# 본문: { "message": "찾을 수 없음" }
```
...
가장 작은 API 서버입니다. 이미 두 개의 계약이 정의되어 있습니다. `/health`는 시스템 상태를, `/users`는 사용자 목록을 돌려줍니다. 여기서 핵심은 구현이 아니라 **계약이 먼저** 정해졌다는 사실입니다.
...
- **API Design 101 (1/10): API란 무엇인가? (현재 글)**
