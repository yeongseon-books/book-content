...
- 이 코드에서 봐야 할 점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **JSON과 content type은 어떤 식으로 계약에 들어갈까요?**
  - JSON과 content type은 어떤 식으로 계약에 들어갈까요 — 본문에서 구체적으로 다룹니다.
- **필드 이름 규칙은 어떻게 정해야 할까요?**
  - 문서에서 맞아 보이던 선택이 운영에서 실패하는 이유는 대부분 관측 지점이 없기 때문입니다. 그래서 설계 단계에서 아래 세 가지를 같이 준비해야 합니다.
- **validation은 어디에서, 어떤 방식으로 해야 할까요?**
  - validation은 어디에서, 어떤 방식으로 해야 할까요 — 본문에서 구체적으로 다룹니다.
- **전후 비교에서 가장 흔한 실수는 무엇일까요?**
  - 문제점:
...
- **API Design 101 (5/10): Request와 response schema (현재 글)**
