...
API 변경이 발생하면 가장 먼저 "이것이 breaking인가?"를 판단합니다. non-breaking이면 현재 버전에 additive하게 추가하고, breaking이면 새 major 버전을 만듭니다. 새 버전 배포 시점부터 이전 버전에 `Deprecation` header를 달고, 충분한 유예 기간 후 sunset합니다. 이 흐름이 문서화되어 있어야 클라이언트가 "언제, 무엇이, 왜" 바뀌는지 예측할 수 있습니다.
