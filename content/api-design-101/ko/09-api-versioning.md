...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/201"
...
> API 버저닝은 '/v1을 붙이는 일'이 아니라 '깨지는 변경을 언제, 누구에게, 어떻게 옮길 것인가'를 결정하는 정책입니다 — URI·헤더·미디어 타입 중 어느 축으로 가르든, 비호환 변경이 클라이언트에 도달하는 경로가 명시되어 있어야 합니다.
...
이 방식의 장점은 breaking change마다 major 번호가 올라가는 것이 아니라, 시간 순서로 정렬된 변경 이력이 자연스럽게 만들어진다는 사실입니다.
...
- **API Design 101 (9/10): Versioning (현재 글)**
