...
![API Design 101 2장 흐름 개요](https://yeongseon-books.github.io/book-public-assets/assets/api-design-101/02/02-01-concept-at-a-glance.ko.png)

*API Design 101 2장 흐름 개요*

이 그림에서는 REST 기본를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> REST 기본의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
클라이언트가 `GET /users/42`를 읽는 순간 “사용자 42를 조회한다”는 뜻이 거의 자동으로 해석되어야 합니다. 이 예측 가능성이 무너지면 REST처럼 보여도 실무에서는 매번 문서를 다시 펴야 하는 API가 됩니다.

## 핵심 용어

- **Resource**: API가 외부에 드러내는 명사입니다. 예를 들면 users, orders, posts입니다.
- **Representation**: 리소스를 어떤 형식으로 표현해 돌려주는지입니다. 보통 JSON이나 XML입니다.
- **Stateless**: 각 요청이 스스로 필요한 정보를 모두 담고, 서버는 호출 사이의 클라이언트 상태를 붙들고 있지 않는 구조입니다.
- **Uniform Interface**: 리소스와 메서드, 헤더가 비슷한 상황에서 비슷한 의미로 반복되는 일관된 호출 규칙입니다.
...
- **REST는 어디서 나왔고 무엇을 뜻할까요?**
  - 본문의 기준은 REST 기본를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **REST를 이루는 여섯 가지 아키텍처 제약은 무엇일까요?**
