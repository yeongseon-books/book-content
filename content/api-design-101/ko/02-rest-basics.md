...
REST는 단순히 "URL을 예쁘게 만드는 것"이 아닙니다. 클라이언트와 서버 사이에 캐시, 로드 밸런서, CDN 같은 중간 계층을 자유롭게 넣을 수 있도록 **제약을 정의한 아키텍처 스타일**입니다. 그림에서 보듯 클라이언트는 바로 앞의 계층만 알면 되고, 그 뒤에 무엇이 있는지는 투명하게 처리됩니다.
