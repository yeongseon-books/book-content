...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/196"
...
- 이 설계 원칙을 무시하면 API 소비자에게 어떤 문제가 생길까요?
...
- **GET, POST, PUT, PATCH, DELETE는 각각 무엇을 의미할까요?**
  - 문제점: - 모든 응답이 200이라 클라이언트는 body를 파싱해야 성공/실패를 판단합니다
- **safe와 idempotent는 어떻게 다를까요?**
  - 문서에서 맞아 보이던 선택이 운영에서 실패하는 이유는 대부분 관측 지점이 없기 때문입니다. 그래서 설계 단계에서 아래 세 가지를 같이 준비해야 합니다.
- **2xx, 3xx, 4xx, 5xx 계열은 어떻게 읽어야 할까요?**
  - 문서에서 맞아 보이던 선택이 운영에서 실패하는 이유는 대부분 관측 지점이 없기 때문입니다. 그래서 설계 단계에서 아래 세 가지를 같이 준비해야 합니다.
...
- **API Design 101 (4/10): HTTP method와 status code (현재 글)**
