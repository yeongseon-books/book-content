...
- 예제는 왜 문서의 중심이어야 할까요?
- 왜 문서가 채택률을 결정하는가에서 가장 흔한 실수는 무엇일까요?
- 다섯 축 상세을 실무에 적용할 때 주의할 점은 무엇일까요?
- 예제 엔드포인트: POST /v1/customers의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 좋은 API 문서의 필수 구성 요소를 나열할 수 있습니다.
- [ ] 예제 중심 문서가 왜 중요한지 설명할 수 있습니다.
- [ ] API 문서를 코드와 동기화하는 방법을 말할 수 있습니다.
...
- **API 문서는 어떤 다섯 축으로 구성하면 좋을까요?**
  - Stripe가 2016년에 공개한 개발자 설문에서 "API 선택 기준 1위"는 문서 품질이었습니다. 기능이 동일하면 문서가 좋은 쪽을 씁니다. 내부 API도 마찬가지입니다. 문서가 없으면 Slack에 질문이 쌓이고, 온보딩 비용이 사람 수에 비례해서 커집니다.
- **사용자가 첫 호출까지 5분 안에 도달하게 하려면 무엇이 필요할까요?**
  - 사용자가 첫 호출까지 5분 안에 도달하게 하려면 무엇이 필요할까요 — 본문에서 구체적으로 다룹니다.
- **예제는 왜 문서의 중심이어야 할까요?**
  - Creates a new customer.
- **왜 문서가 채택률을 결정하는가에서 가장 흔한 실수는 무엇일까요?**
  - Stripe가 2016년에 공개한 개발자 설문에서 "API 선택 기준 1위"는 문서 품질이었습니다. 기능이 동일하면 문서가 좋은 쪽을 씁니다. 내부 API도 마찬가지입니다. 문서가 없으면 Slack에 질문이 쌓이고, 온보딩 비용이 사람 수에 비례해서 커집니다.
