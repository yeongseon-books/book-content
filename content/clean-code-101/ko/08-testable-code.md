...
- **순수 로직과 부수 효과는 어떻게 분리해야 할까요?**
  - 이 글은 계산 중심 코어를 먼저 순수 함수로 떼어 내고, 시간·HTTP·DB 같은 부수 효과는 얇은 어댑터 바깥으로 밀어내는 구조를 권했습니다. `total(items)`, `compute_invoice_total` 같은 함수는 코어에 남기고, `fetch_user(uid, http)`나 어댑터 클래스는 경계에 두는 방식입니다.
- **의존성 주입은 어떻게 테스트용 이음새를 만들까요?**
