...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/170"
...
> 테스트하기 쉬운 코드는 의존성이 명확하고 부수효과가 격리된 코드입니다.
...
- 실전 적용: 테스트 가능성을 높이는 다섯 단계에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 테스트하기 어려운 코드는 왜 설계 품질의 낮음을 드러내는 신호가 될까요?
- 구조 변경 PR과 기능 추가 PR을 분리해야 하는 이유는 무엇일까요?
...
점수를 수치로 남기면 리뷰 대화가 감각에서 근거 중심으로 이동합니다.
  - `is_business_hour(now)`, `issue_coupon(user_id, clock)`, `fetch_user(uid, http)`처럼 외부 의존을 인자로 받으면 실제 구현 대신 `FakeClock`이나 가짜 HTTP 클라이언트를 끼울 수 있습니다. 이 주입 지점이 바로 seam이 되어, 같은 로직을 운영 환경과 테스트 환경에서 다르게 연결할 수 있습니다.
- **Fake와 Spy는 각각 언제 쓰는 편이 좋을까요?**
  - Fake는 `FakeRepo`, `FakePaymentGateway`처럼 실제 저장이나 호출을 대신하면서 결과를 만들어 주는 테스트 대체물에 적합하고, Spy는 `EmailSpy`, `SpyNotifier`처럼 몇 번 어떤 값으로 호출됐는지 기록을 검증할 때 적합합니다. 즉 상태와 결과를 단순화하고 싶을 때는 Fake를, 상호작용 계약을 보고 싶을 때는 Spy를 고르는 편이 맞습니다.

## 처음 질문으로 돌아가기

- **순수 로직과 부수 효과는 어떻게 분리해야 할까요?**
  - `total(items)`처럼 입출력 없이 계산만 하는 부분을 먼저 순수 함수로 뽑아내고, 저장·알림·HTTP 호출은 별도 함수나 어댑터로 경계를 만듭니다. 이 구조가 되면 핵심 계산은 단위 테스트 대상으로, IO 경계는 통합 테스트 대상으로 자연스럽게 나뉩니다.
- **의존성 주입은 어떻게 테스트용 이음새를 만들까요?**
  - `is_business_hour(now)`, `issue_coupon(user_id, clock)`, `fetch_user(uid, http)`처럼 외부 의존을 인자로 받으면 실제 구현 대신 `FakeClock`이나 가짜 HTTP 클라이언트를 끼울 수 있습니다. 이 주입 지점이 seam이 되어 같은 로직을 운영과 테스트 환경에서 다르게 연결합니다.
- **Fake와 Spy는 각각 언제 쓰는 편이 좋을까요?**
  - Fake는 `FakeRepo`, `FakePaymentGateway`처럼 실제 저장이나 호출을 대신하면서 결과를 만들어 주는 경우에 적합하고, Spy는 `EmailSpy`, `SpyNotifier`처럼 몇 번 어떤 값으로 호출됐는지 기록을 검증할 때 적합합니다. 상태와 결과를 단순화하려면 Fake를, 상호작용 계약을 보려면 Spy를 고릅니다.
- **실전 적용: 테스트 가능성을 높이는 다섯 단계에서 가장 흔한 실수는 무엇일까요?**
  - `datetime.now()`와 난수가 핵심 로직 안에 남아 있거나, mock 수는 많아졌는데 함수 책임이 그대로 큰 경우가 가장 흔합니다. mock 라이브러리에만 의존하면 숨은 결합은 그대로 남고, 테스트를 위해 공개 메서드를 늘리면 캡슐화가 깨집니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 순수 함수 테스트는 매우 빠르게 끝나야 하고, 어댑터 테스트만 외부 의존성과 통합되어야 합니다. `classify_test_layer`처럼 테스트 레이어를 분류해 단위 테스트가 부족한지 통합 테스트가 과도한지 추세를 보는 것이 중요합니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 핵심 로직은 IO를 몰라야 하고, 시간과 난수는 항상 주입하는 편이 좋으며, Fake를 쓰면 테스트가 빠르고 안정적으로 돌아갑니다.
- **테스트하기 어려운 코드는 왜 설계 품질의 낮음을 드러내는 신호가 될까요?**
  - 테스트 준비가 무겁고 느린 이유는 대개 도메인 로직과 IO가 분리되어 있지 않기 때문입니다. 외부 의존이 함수 내부에 깊게 붙어 있으면 작은 규칙 하나를 검증하는 일조차 데이터베이스나 네트워크 연결을 요구합니다. 테스트 비용이 곧 설계 결합도를 측정하는 지표입니다.
- **구조 변경 PR과 기능 추가 PR을 분리해야 하는 이유는 무엇일까요?**
  - 순수 함수 추출, 의존성 주입 적용, 테스트 추가를 각각 별도 PR로 나누면 리뷰어가 구조 변경이 동작을 바꾸지 않았음을 확인하기 쉬워집니다. 구조 변경과 기능 추가를 한 PR에 섞으면 회귀가 어디서 왔는지 구분하기 어려워 디버깅 비용이 높아집니다.
...
- **Clean Code 101 (8/10): 테스트 가능한 코드 (현재 글)**
