---
series: clean-code-101
episode: 8
title: 테스트 가능한 코드
status: content-ready
targets:
  tistory: true
  medium: true
  hashnode: true
  mkdocs: true
  ebook: true
language: ko
tags:
  - Computer Science
  - CleanCode
  - Testability
  - Testing
  - DependencyInjection
  - Refactoring
seo_description: 의존성 주입, 순수 함수, 테스트 이음새, 페이크로 테스트 가능한 코드를 만드는 법.
last_reviewed: '2026-05-04'
---

# 테스트 가능한 코드

> Clean Code 101 시리즈 (8/10)

<!-- a-grade-intro:begin -->

**핵심 질문**: 왜 어떤 코드는 테스트가 한 줄이고, 어떤 코드는 테스트 자체가 불가능한가요?

> 부수효과와 의존성을 어떻게 다루느냐가 결정합니다. 둘을 분리하면 테스트가 쉬워집니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 순수 함수와 부수효과의 분리
- 의존성 주입(DI)으로 이음새 만들기
- 테스트용 페이크(Fake)와 스파이(Spy)
- 시간/난수처럼 비결정적 의존성 다루기
- 테스트 가능성을 높이는 리팩토링

## 왜 중요한가

테스트가 어렵다는 것은 코드 구조가 어렵다는 신호입니다. 테스트 가능성은 설계 품질의 척도입니다.

> 테스트 가능성은 결과가 아니라 설계의 결과다.

## 개념 한눈에 보기

```mermaid
flowchart LR
    L["로직"] --> P["순수 함수"]
    L --> S["부수효과 어댑터"]
    P --> T1["단위 테스트"]
    S --> T2["통합 테스트"]
```

순수 핵심 + 얇은 어댑터.

## 핵심 용어 정리

- **Pure function**: 같은 입력 → 같은 출력, 부수효과 없음.
- **Dependency Injection**: 외부 의존성을 인자로 받기.
- **Seam**: 동작을 갈아 끼울 수 있는 경계.
- **Fake**: 단순화된 가짜 구현.
- **Spy**: 호출 정보를 기록하는 테스트 더블.

## Before/After

**Before**

```python
import datetime, requests
def is_business_hour():
    now = datetime.datetime.now()
    return 9 <= now.hour < 18

def fetch_user(uid):
    return requests.get(f"https://api/users/{uid}").json()
```

**After**

```python
def is_business_hour(now):
    return 9 <= now.hour < 18

def fetch_user(uid, http):
    return http.get(f"/users/{uid}").json()
```

시간과 HTTP가 바깥에서 들어옵니다.

## 실습: 테스트 가능성 5단계

### 1단계 — 순수 추출

```python
# 1_pure.py
def total(items):
    return sum(it.price * it.qty for it in items)
```

IO 없는 계산은 항상 순수 함수로.

### 2단계 — 시간 의존성 주입

```python
# 2_clock.py
from datetime import datetime
def is_overdue(due, now=None):
    now = now or datetime.now()
    return now > due
```

테스트에서 `now`를 고정합니다.

### 3단계 — Fake 객체

```python
# 3_fake.py
class FakeRepo:
    def __init__(self): self.users = {}
    def save(self, u): self.users[u.id] = u
    def get(self, uid): return self.users.get(uid)

def register(repo, user):
    repo.save(user); return user
```

DB 없이 도메인 로직을 테스트합니다.

### 4단계 — 호출 기록(Spy)

```python
# 4_spy.py
class EmailSpy:
    def __init__(self): self.sent = []
    def send(self, to, body): self.sent.append((to, body))

def notify(email, user):
    email.send(user.email, "welcome")
```

테스트에서 호출 횟수와 인자를 검증.

### 5단계 — 외부 호출 격리

```python
# 5_adapter.py
class HttpClient:
    def get(self, path): ...

def fetch_user(uid, http: HttpClient):
    return http.get(f"/users/{uid}").json()
```

외부 시스템 호출을 어댑터 한 클래스로 모읍니다.

## 이 코드에서 주목할 점

- 핵심 로직은 IO를 모릅니다.
- 시간/난수는 항상 주입됩니다.
- 테스트는 가짜 구현으로 빠르게 돕니다.

## 자주 하는 실수 5가지

1. **`datetime.now()`를 함수 안에서 직접 호출.** 시간이 흐르면 테스트도 깨집니다.
2. **DB/네트워크와 도메인 로직 결합.** 단위 테스트가 사라집니다.
3. **mock 라이브러리에만 의존.** 단단한 결합이 숨겨집니다.
4. **테스트만을 위한 public 메서드.** 캡슐화 파괴.
5. **글로벌 싱글톤 의존.** 격리가 어렵습니다.

## 실무에서는 이렇게 쓰입니다

좋은 팀은 헥사고날/포트-어댑터 구조로 도메인 핵심을 IO로부터 분리합니다. 단위 테스트가 수천 개 있어도 1초 안에 돕니다.

## 시니어 엔지니어는 이렇게 생각합니다

- 핵심은 항상 순수 함수로 시작한다.
- 의존성은 인자로 받는다.
- mock보다 fake를 선호한다.
- 시간/난수/IO는 경계로 밀어낸다.
- 테스트가 느리면 설계 문제로 본다.

## 체크리스트

- [ ] 핵심 로직이 순수한가?
- [ ] 외부 의존성이 인자로 들어오나?
- [ ] 시간/난수가 주입되나?
- [ ] Fake로 IO 없이 테스트가 가능한가?
- [ ] 단위 테스트가 1초 내 도는가?

## 연습 문제

1. 본인 코드의 `datetime.now()` 호출을 인자 주입으로 바꾸세요.
2. DB에 의존하는 함수 1개를 Fake로 단위 테스트하세요.
3. 외부 HTTP 호출을 어댑터 클래스로 추출하세요.

## 정리 및 다음 단계

테스트 가능성은 설계의 거울입니다. 다음 글에서는 안전하게 코드를 변경하는 기술 — 리팩토링 기초 — 를 다룹니다.

<!-- toc:begin -->
- [Clean Code란 무엇인가?](./01-what-is-clean-code.md)
- [이름 짓기](./02-naming.md)
- [함수 작게 만들기](./03-small-functions.md)
- [조건문 줄이기](./04-simplifying-conditionals.md)
- [중복 제거](./05-removing-duplication.md)
- [오류 처리](./06-error-handling.md)
- [주석과 문서화](./07-comments-and-docs.md)
- **테스트 가능한 코드 (현재 글)**
- 리팩토링 기초 (예정)
- 좋은 코드 리뷰 기준 (예정)
<!-- toc:end -->

## 참고 자료

- [Working Effectively with Legacy Code (M. Feathers)](https://www.oreilly.com/library/view/working-effectively-with/0131177052/)
- [Hexagonal Architecture (Alistair Cockburn)](https://alistair.cockburn.us/hexagonal-architecture/)
- [Mocks Aren't Stubs (Martin Fowler)](https://martinfowler.com/articles/mocksArentStubs.html)
- [Pytest — Fixtures](https://docs.pytest.org/en/stable/how-to/fixtures.html)

Tags: Computer Science, CleanCode, Testability, Testing, DependencyInjection, Refactoring
