...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/172"
...
> 좋은 코드 리뷰는 결함을 찾는 것보다 팀 전체의 코드 품질 기준을 맞추는 데 더 큰 가치가 있습니다.
...
- 실전 적용: 탄탄한 리뷰 프로세스 다섯 단계에서 가장 흔한 실수는 무엇일까요?
- 무엇을 바꾸는가을 실무에 적용할 때 주의할 점은 무엇일까요?
- 왜 바꾸는가의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 반복되는 리뷰 코멘트가 자동화로 넘어가지 않으면 팀에 어떤 비용이 쌓일까요?
- 리뷰 프로세스의 성숙도를 수치로 측정하면 어떤 지표가 가장 유용할까요?
...
점수를 수치로 남기면 리뷰 대화가 감각에서 근거 중심으로 이동합니다.
  - 체크리스트는 이름과 함수 경계가 명확한지, 예외와 실패 경계가 분명한지, 테스트와 CI가 초록인지, PR 설명에 What/Why/How/Risk가 들어 있는지까지 포함합니다. 즉 코드 한 줄만 보는 것이 아니라 정확성, 가독성, 안정성, 확장성, 운영성 다섯 관점에서 근거를 확인하는 방식입니다.
- **좋은 리뷰 코멘트는 어떤 형태를 가져야 할까요?**
  - 좋은 코멘트는 `MUST`, `SUGG`, `NIT`, `QUESTION`처럼 우선순위를 드러내고, 관찰·위험·제안·검증 요청까지 함께 담아야 합니다. `order_total is 60 lines` 예시와 `ReviewAction` 템플릿처럼 작성자가 바로 행동할 수 있는 다음 선택지를 주는 코멘트가 좋은 리뷰입니다.

## 처음 질문으로 돌아가기

- **리뷰 가능한 PR 크기는 어느 정도일까요?**
  - 400줄 diff 이하에 한 가지 책임만 담긴 PR이 이상적입니다. 한 책임만 담긴 PR이어야 리뷰어가 기준을 선명하게 적용할 수 있고, 기능 PR 전에 리팩토링 PR을 먼저 병합하면 기능 PR의 크기가 더 줄어듭니다.
- **Clean Code 관점의 리뷰 체크리스트는 무엇일까요?**
  - 이름과 함수 경계가 명확한지, 예외와 실패 경계가 분명한지, 테스트와 CI가 초록인지, PR 설명에 What/Why/How/Risk가 들어 있는지를 정확성·가독성·안정성·확장성·운영성 다섯 관점에서 근거로 확인합니다.
- **좋은 리뷰 코멘트는 어떤 형태를 가져야 할까요?**
  - `MUST`, `SUGG`, `NIT`, `QUESTION` 우선순위 라벨이 붙고, 관찰·위험·제안·검증 요청까지 함께 담아야 합니다. `ReviewAction` 템플릿처럼 작성자가 바로 행동할 수 있는 다음 선택지를 주는 코멘트가 좋은 리뷰입니다.
- **실전 적용: 탄탄한 리뷰 프로세스 다섯 단계에서 가장 흔한 실수는 무엇일까요?**
  - 리뷰 코멘트가 취향과 필수를 구분하지 못하거나, 반복 지적이 lint나 템플릿으로 옮겨가지 않는 것이 가장 흔합니다. MUST를 남용하면 신뢰가 빠르게 떨어지고, 자동화 가능한 일을 사람이 반복하면 리뷰 질이 바로 떨어집니다.
- **무엇을 바꾸는가을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - PR 설명의 What 항목은 변경 범위와 영향 파일을 명확히 적어야 하고, 자동화 실패율이 10%를 넘으면 CI 게이트를 강화하는 쪽으로 먼저 대응해야 합니다. 변경 내용이 맥락 없이 오면 리뷰어는 코드 diff를 이해하기 위해 별도로 조사해야 합니다.
- **왜 바꾸는가의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 자동화는 잡무를 처리하고, 사람은 의도와 구조를 봐야 리뷰가 가치 있는 시간이 됩니다.
- **반복되는 리뷰 코멘트가 자동화로 넘어가지 않으면 팀에 어떤 비용이 쌓일까요?**
  - 같은 지적을 매번 사람이 반복하면 리뷰어의 인지 자원이 설계 판단이 아닌 스타일 검사에 소모됩니다. 반복 코멘트가 lint 규칙이나 PR 템플릿으로 전환되지 않으면 신규 멤버마다 같은 지적을 받는 온보딩 비용도 계속 쌓입니다.
- **리뷰 프로세스의 성숙도를 수치로 측정하면 어떤 지표가 가장 유용할까요?**
  - 첫 리뷰 응답 시간, 승인까지 왕복 횟수, 린터·테스트 실패율, 병합 후 7일 내 회귀 결함률이 핵심 지표입니다. `review_health_score` 예시처럼 지표를 조합하면 개인 평가 대신 프로세스 개선 기준으로 사용할 수 있습니다.
