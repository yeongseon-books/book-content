...
- **리팩토링은 정확히 무엇이고, 재작성과 무엇이 다를까요?**
  - 이 글은 리팩토링을 외부 동작을 바꾸지 않은 채 내부 구조를 개선하는 일로 정의했고, 재작성과의 차이는 초록 테스트 사이를 작은 단계로 이동한다는 점에 두었습니다. `subtotal`, `with_coupon`, `with_member`로 쪼개는 예시처럼 결과는 유지한 채 읽기와 변경 구조만 더 낫게 만드는 것이 핵심입니다.
- **Fowler 카탈로그의 핵심 기법은 무엇일까요?**
