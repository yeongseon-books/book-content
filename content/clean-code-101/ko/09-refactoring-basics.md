...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/171"
...
> 리팩토링은 동작을 바꾸지 않으면서 코드의 구조를 개선하는 작업입니다.
...
- 실전 적용: 안전한 리팩토링 다섯 단계에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 리팩토링 PR과 기능 PR을 같은 커밋에 섞으면 어떤 실질적인 문제가 생길까요?
- 레거시 코드에서 한 번에 전체를 교체하는 것보다 점진적 개선이 더 안전한 이유는 무엇일까요?
...
점수를 수치로 남기면 리뷰 대화가 감각에서 근거 중심으로 이동합니다.
  - 글에서 반복해서 쓰인 핵심 기법은 Extract Method, Rename, Move Field/Function, Introduce Parameter Object, Inline Function입니다. `Order.total_with_tax()`, `items_subtotal`, `next_refactor_step` 같은 예시는 긴 함수 분해, 책임 이동, 과도한 추상화 되돌리기를 언제 쓰는지 구체적으로 보여 줍니다.
- **특성화 테스트는 왜 레거시 코드에서 중요할까요?**
  - 레거시 코드는 현재 동작을 완전히 이해하기 전에 건드려야 하는 경우가 많아서, `test_legacy_total()` 같은 특성화 테스트가 먼저 안전망이 됩니다. 지금 동작을 고정해 두어야 이후 추출, 이름 변경, 이동이 실제로 구조 개선인지 기능 변경인지 분리해서 판단할 수 있습니다.

## 처음 질문으로 돌아가기

- **리팩토링은 정확히 무엇이고, 재작성과 무엇이 다를까요?**
  - 리팩토링은 외부 동작을 바꾸지 않으면서 내부 구조를 개선하는 작업입니다. 재작성은 새로운 동작까지 함께 도입하므로 리뷰가 어렵고 원인 파악이 느려집니다. 리팩토링은 초록 테스트와 초록 테스트 사이의 작은 이동을 반복하는 기술입니다.
- **Fowler 카탈로그의 핵심 기법은 무엇일까요?**
  - 핵심 기법은 Extract Method(긴 함수 분해), Rename(이름 명확화), Move Field/Function(잘못된 책임 이동), Introduce Parameter Object(인자 다발 정리), Inline Function(과도 추상화 축소)입니다. `Order.total_with_tax()`, `items_subtotal` 예시처럼 언제 어떤 기법을 쓰는지 팀 공통 언어로 만들면 리뷰 대화가 빨라집니다.
- **특성화 테스트는 왜 레거시 코드에서 중요할까요?**
  - 레거시 코드는 현재 동작을 완전히 이해하기 전에 건드려야 하는 경우가 많아서, `test_legacy_total()` 같은 특성화 테스트가 먼저 안전망이 됩니다. 현재 동작을 고정해야 이후 추출, 이름 변경, 이동이 구조 개선인지 기능 변경인지 분리해서 판단할 수 있습니다.
- **실전 적용: 안전한 리팩토링 다섯 단계에서 가장 흔한 실수는 무엇일까요?**
  - 기능 변경이 리팩토링 커밋 안에 함께 섞이거나, 이름만 바꾸고 잘못 놓인 책임을 그대로 두는 경우가 가장 흔합니다. 테스트 없이 시작하면 회귀가 우연이 되고, 한 번에 너무 크게 바꾸면 되돌릴 방법이 사라집니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 현재 동작을 고정한 테스트가 먼저 초록이어야 하고, 추출·이름 변경·이동 뒤에도 테스트 결과가 같아야 합니다. 리팩토링 검증 로그를 PR 본문에 남겨야 추적 가능성이 올라갑니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 매 단계 뒤에 테스트가 계속 초록 상태를 유지해야 하고, 각 변경은 작고 되돌리기 쉬워야 하며, 이름이 의도를 더 잘 드러내는 방향으로 바뀌어야 합니다.
- **리팩토링 PR과 기능 PR을 같은 커밋에 섞으면 어떤 실질적인 문제가 생길까요?**
  - 리뷰어가 "이 변경이 구조 개선인지 새 동작인지" 판단하기 어려워져 리뷰 시간이 늘고, 버그 발생 시 원인을 코드 변경에서 찾기 힘들어집니다. 또한 롤백이 필요할 때 구조 개선과 기능 추가를 같이 되돌려야 하는 문제가 생깁니다.
- **레거시 코드에서 한 번에 전체를 교체하는 것보다 점진적 개선이 더 안전한 이유는 무엇일까요?**
  - 큰 교체는 현재 동작의 모든 경계 케이스를 사전에 파악해야 하지만 레거시 코드에서는 이것이 거의 불가능합니다. `migrate_refactor_step` 예시처럼 단계별로 검증 로그를 남기면 각 단계마다 신뢰를 쌓고 다음 변경의 출발 비용을 줄일 수 있습니다.
