...
- 실전 적용: 이름을 개선하는 다섯 단계에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 함수 이름이 모호할 때 코드 리뷰에서 어떤 문제가 반복될까요?
- 대규모 저장소에서 이름 마이그레이션을 안전하게 진행하려면 어떤 순서가 필요할까요?
...
- **좋은 이름을 판단할 때 어떤 신호를 봐야 할까요?**
  - 의도가 드러나는지(Intention-revealing), grep으로 정확히 찾을 수 있는지(Searchable), 말할 수 있는지(Pronounceable), 도메인 용어를 쓰는지(Domain term)가 핵심 신호입니다. `ruff check app/ --select N`으로 명명 규칙 위반을 자동으로 확인할 수 있습니다.
- **변수 이름과 함수 이름, 클래스 이름은 무엇이 다를까요?**
  - 변수는 `invoice_total_cents`처럼 값의 의미와 단위를 드러내야 하고, 함수는 `calculate_order_total`처럼 동사와 목적을 함께 담아야 하며, 클래스는 `InvoiceRepository`처럼 역할 중심 명사여야 합니다. 호출자에게 무엇을 약속하는지에 따라 기준이 달라집니다.
- **도메인 용어를 코드 안으로 어떻게 자연스럽게 가져올 수 있을까요?**
  - 사용자 표현과 코드 용어를 1:1로 맞추는 도메인 용어 테이블을 만들고, `order_total_cents`, `payment_confirmed`, `discount_coupon` 같은 이름을 테스트와 API 스키마에도 같은 단어로 쓰는 방식이 효과적입니다. 용어가 레이어마다 일치하면 장애 보고 문장도 짧아집니다.
- **실전 적용: 이름을 개선하는 다섯 단계에서 가장 흔한 실수는 무엇일까요?**
  - 타입 이름만 바꾸고 도메인 용어는 그대로 놓치는 경우가 가장 흔합니다. 또한 리네임 후 호출 지점 전체를 함께 검증하지 않으면 숨은 참조가 남아 다른 이름이 코드베이스에 혼재하게 됩니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 축약어와 일관성 없는 명명 규칙이 먼저 드러나도록 lint를 돌리고, 이름을 바꾼 뒤에도 테스트가 그대로 초록이어야 합니다. 자동화 치환은 문맥을 이해하지 못하므로 변경 후 의미 검증 리뷰를 반드시 추가해야 합니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 이름은 호출 지점에서 의미를 만들고, 검색 가능성이 미래의 분석 비용을 줄이며, 도메인 용어는 사용자와 개발자 사이의 번역 비용을 없애 줍니다.
- **함수 이름이 모호할 때 코드 리뷰에서 어떤 문제가 반복될까요?**
  - `do_stuff`, `handle_data`, `result`처럼 모호한 이름은 리뷰어가 함수 본문을 모두 읽어야 의도를 파악하게 만들어 리뷰 시간을 늘립니다. 같은 개념을 파일마다 다른 이름으로 부르면 검색성이 떨어지고 협업 속도가 급격히 저하됩니다.
- **대규모 저장소에서 이름 마이그레이션을 안전하게 진행하려면 어떤 순서가 필요할까요?**
  - 후보 이름의 도메인 의미를 먼저 정의하고, 참조 검색으로 영향 파일을 확인한 뒤, 공개 API와 내부 구현을 구분해 우선순위를 나눕니다. 테스트를 먼저 초록으로 고정한 후 한 번에 한 개념씩 리네임하고 즉시 재검증하는 방식이 안전합니다.
