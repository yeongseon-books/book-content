...
- **예외를 던질지 값을 반환할지 어떻게 판단할까요?**
  - 호출자가 그 자리에서 분기해 처리할 수 있는 실패라면 `Result(ok, value, error)`처럼 값으로 돌려주고, 복구가 어려워 상위 계층 판단이 필요한 실패라면 예외로 올리는 것이 이 글의 기준입니다. `parse_int`와 `FetchError`, `ValidationError`, `InfraError` 예시가 바로 그 경계를 보여 줍니다.
- **Fail Fast는 어떤 상황에서 특히 중요할까요?**
