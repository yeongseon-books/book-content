...
- 실전 적용: 견고한 오류 처리 다섯 단계에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 예외 계층을 명확하게 설계하면 장애 대응과 로그 집계에 어떤 이점이 생길까요?
- 재시도 전략을 잘못 붙이면 어떤 종류의 장애를 오히려 악화시킬 수 있을까요?
...
- **Fail Fast는 어떤 상황에서 특히 중요할까요?**
  - `transfer(amount)`에서 음수 금액을 바로 막고 `handle_http_request`에서 `amount` 누락과 0 이하 값을 입구에서 검증하듯이, 잘못된 입력이 더 깊은 계층으로 퍼지기 전에 끊어야 할 때 Fail Fast가 특히 중요합니다. 문제를 초반에 드러내야 원인 보존도 쉽고 재시도 같은 후속 정책도 잘못 붙지 않습니다.
- **"값으로서의 오류" 패턴은 언제 유용할까요?**
  - 파싱, 검증, 사용자 입력 처리처럼 실패가 예상 가능하고 호출자가 정상 흐름 안에서 선택지를 나눠야 하는 경우에 값으로서의 오류 패턴이 유용합니다. 이 방식은 테스트에서 성공·실패 케이스를 나란히 고정하기 쉽고, 예외를 흐름 제어로 남용하지 않게 도와줍니다.

## 처음 질문으로 돌아가기

- **예외를 던질지 값을 반환할지 어떻게 판단할까요?**
  - 호출자가 그 자리에서 복구하기 어려운 상황이면 예외를 던지고, 파싱이나 검증처럼 실패가 예상 가능하고 호출자가 정상 흐름 안에서 분기해야 하면 `Result` 패턴으로 값을 돌려줍니다. 복구 가능성과 경계 위치가 핵심 판단 기준입니다.
- **Fail Fast는 어떤 상황에서 특히 중요할까요?**
  - `transfer(amount)`에서 음수 금액을 바로 막거나 `handle_http_request`에서 누락 필드를 입구에서 검증하듯, 잘못된 입력이 더 깊은 계층으로 퍼지기 전에 끊어야 할 때 특히 중요합니다. 문제를 초반에 드러내야 원인 보존도 쉽고 재시도 같은 후속 정책도 잘못 붙지 않습니다.
- **"값으로서의 오류" 패턴은 언제 유용할까요?**
  - 파싱, 검증, 사용자 입력 처리처럼 실패가 예상 가능하고 호출자가 성공·실패 둘 다 처리해야 할 때 유용합니다. 이 방식은 테스트에서 성공·실패 케이스를 나란히 고정하기 쉽고, 예외를 흐름 제어로 남용하지 않게 도와줍니다.
- **실전 적용: 견고한 오류 처리 다섯 단계에서 가장 흔한 실수는 무엇일까요?**
  - `except Exception`이 내부 로직 깊숙한 곳에 남아 있거나, 재시도가 중복 결제처럼 멱등하지 않은 작업에도 붙어 있는 경우가 가장 흔합니다. 빈 except 블록으로 정보를 모두 삼키는 것도 디버깅을 거의 불가능하게 만드는 실수입니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 예외 타입과 HTTP 경계 매핑을 테스트로 고정해야 하고, 재시도는 멱등한 호출에서만 통과해야 합니다. 예외 계층과 로그 이벤트 이름을 함께 표준화하면 장애 대응 시간(MTTR)을 줄일 수 있습니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 검증과 처리 책임이 분리되어 있어야 하고, 도메인 예외 타입이 실패의 의미를 운반하며, 재시도는 멱등한 작업에서만 안전합니다.
- **예외 계층을 명확하게 설계하면 장애 대응과 로그 집계에 어떤 이점이 생길까요?**
  - `ValidationError`는 400, `InfraError`는 503처럼 일관된 HTTP 매핑이 가능해지고, 로그 집계에서 도메인 오류와 인프라 오류를 따로 관찰할 수 있습니다. 경보 라우팅도 `ValidationError`는 개발팀, `ExternalDependencyError`는 SRE처럼 채널을 분리해 대응 속도를 높일 수 있습니다.
- **재시도 전략을 잘못 붙이면 어떤 종류의 장애를 오히려 악화시킬 수 있을까요?**
  - 데이터 무결성 오류나 중복 결제처럼 멱등하지 않은 작업에 재시도를 붙이면 동일한 잘못된 상태가 반복 생성되어 장애가 더 커집니다. 재시도는 일시적 네트워크 장애처럼 같은 작업을 다시 해도 안전한 경우에만 적용해야 합니다.
