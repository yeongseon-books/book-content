...
**핵심 질문**: 예외를 던질까, 값을 돌려줄까?

> 호출자가 결정을 내려야 하는 상황은 값으로, 호출자가 처리할 수 없는 상황은 예외로 표현합니다.
...
- **Fail Fast**: 잘못된 상태를 즉시 알린다.
- **Result/Either**: 성공/실패를 값으로 표현.
- **Exception**: 호출자가 회복할 수 없는 상황.
- **Retry**: 일시적 실패를 다시 시도.
- **Backoff**: 재시도 간격을 점진적으로 늘림.
...
1. 본인 코드에서 빈 except 1건을 의미 있는 처리로 바꾸세요.
2. parse 류 함수 1건을 Result 패턴으로 리팩토링하세요.
3. 외부 호출 1건에 백오프 재시도를 추가해 보세요.
