...
> 좋은 주석은 코드가 "왜" 이렇게 작성되었는지를 설명하고, 좋은 문서는 코드를 사용하는 방법을 안내합니다.
...
- 실전 적용: 유용한 문서화 다섯 단계에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 코드가 수정될 때 주석을 함께 갱신하지 않으면 어떤 위험이 생길까요?
- 익명 TODO가 팀에 누적되면 코드베이스 품질에 어떤 영향을 미칠까요?
...
점수를 수치로 남기면 리뷰 대화가 감각에서 근거 중심으로 이동합니다.
  - 의도 주석은 결제 게이트웨이가 HTTP 200이어도 본문 상태를 봐야 하는 이유처럼 왜 이런 구현을 택했는지 설명하고, 경고 주석은 `트랜잭션 내부에서 호출하지 마세요`처럼 호출자가 다칠 수 있는 위험을 바로 알려 줍니다. 둘 다 코드 반복 설명이 아니라, 코드만으로는 전달하기 어려운 배경과 위험을 남기는 역할입니다.
- **Python docstring은 어떤 규칙으로 쓰는 편이 좋을까요?**
  - 이 글은 docstring을 구현 설명이 아니라 계약 문서로 다루며, `Args`, `Returns`, `Raises`를 통해 입력 단위와 예외 조건을 분명히 적는 방식을 권했습니다. `discount`와 `calculate_refund_amount` 예시처럼 호출자가 무엇을 넣고 무엇을 받는지 먼저 알 수 있어야 테스트와 문서가 같은 계약을 공유합니다.

## 처음 질문으로 돌아가기

- **언제 주석을 쓰지 않는 편이 더 좋을까요?**
  - 코드 동작을 그대로 반복하는 주석, 이름과 구조를 고치면 사라질 주석은 쓰지 않는 편이 좋습니다. `p(a, b)`에 주석을 다는 대신 `can_purchase(is_active_user, stock_quantity)`로 바꾸는 것처럼 코드 개선이 주석보다 더 영구적인 해결입니다.
- **의도 주석과 경고 주석은 어떤 차이가 있을까요?**
  - 의도 주석은 결제 게이트웨이가 HTTP 200이어도 본문 상태를 봐야 하는 이유처럼 왜 이런 구현을 택했는지 설명하고, 경고 주석은 "트랜잭션 내부에서 호출하지 마세요"처럼 호출자가 다칠 수 있는 위험을 바로 알려 줍니다. 둘 다 코드만으로는 전달하기 어려운 배경과 위험을 남기는 역할입니다.
- **Python docstring은 어떤 규칙으로 쓰는 편이 좋을까요?**
  - docstring을 구현 설명이 아니라 계약 문서로 다루며, `Args`, `Returns`, `Raises`를 통해 입력 단위와 예외 조건을 분명히 적는 방식을 권합니다. `discount`와 `calculate_refund_amount` 예시처럼 호출자가 무엇을 넣고 무엇을 받는지 먼저 알 수 있어야 테스트와 문서가 같은 계약을 공유합니다.
- **실전 적용: 유용한 문서화 다섯 단계에서 가장 흔한 실수는 무엇일까요?**
  - 주석이 코드를 그대로 반복하거나, TODO에 담당자와 추적 링크가 빠져 영구 부채가 되는 경우가 가장 흔합니다. 모든 함수에 형식적인 docstring을 붙여도 정보량이 0이면 소음만 늘어납니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 주석 없이도 이름과 구조가 핵심 흐름을 설명해야 하고, 공개 함수의 계약이 테스트와 문서 둘 다에서 맞아야 합니다. `should_keep_comment` 기준을 리뷰 체크리스트에 넣으면 소음 주석이 줄고 핵심 문서화 품질이 올라갑니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 코드는 "무엇"을, 주석은 "왜"를 설명해야 하고, docstring은 사용 계약을 분명하게 만들며, TODO는 담당자와 기한이 있어야 추적 가능합니다.
- **코드가 수정될 때 주석을 함께 갱신하지 않으면 어떤 위험이 생길까요?**
  - 코드와 주석이 어긋나면 읽는 사람이 둘 중 무엇을 믿어야 할지 모르게 됩니다. 낡은 주석은 코드와 다른 진실을 만들어 오해성 버그를 유발하고, 새 기여자가 잘못된 기대를 갖고 수정하는 위험으로 이어집니다.
- **익명 TODO가 팀에 누적되면 코드베이스 품질에 어떤 영향을 미칠까요?**
  - 소유자 없는 TODO는 누가 언제 처리할지 알 수 없어 거의 항상 영구 부채가 됩니다. 기술 부채 목록에도 올라가지 않아 백로그 밖으로 빠져나가고, 코드베이스 곳곳에 "언젠가 할 일" 표식만 쌓입니다. 주간 TODO 정리 루틴과 만료 후 자동 이슈 전환 규칙이 이를 막습니다.
...
- **Clean Code 101 (7/10): 주석과 문서화 (현재 글)**
