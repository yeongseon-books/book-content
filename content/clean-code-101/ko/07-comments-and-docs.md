...
- **언제 주석을 쓰지 않는 편이 더 좋을까요?**
  - 코드가 이름과 구조만으로 의도를 드러낼 수 있을 때는 주석보다 리네임과 구조 개선이 먼저입니다. `# i를 1씩 증가` 같은 설명 대신 `get_active_users()`나 `can_purchase(is_active_user, stock_quantity)`처럼 코드 자체가 읽히게 만드는 편이 오래 갑니다.
- **의도 주석과 경고 주석은 어떤 차이가 있을까요?**
