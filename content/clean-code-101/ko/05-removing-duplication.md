...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/167"
...
- 실전 적용: 안전하게 중복 없애기에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 잘못된 추상화가 만들어진 것을 어떤 신호로 알 수 있고, 어떻게 되돌려야 할까요?
- 데이터 중복이 코드 중복보다 더 위험할 수 있는 이유는 무엇일까요?
...
점수를 수치로 남기면 리뷰 대화가 감각에서 근거 중심으로 이동합니다.
  - 본질적인 중복은 같은 변경 이슈, 같은 도메인 용어, 같은 실패 영향, 같은 배포 타이밍을 공유하는 경우이고, 글에서는 이를 `is_essential_duplication` 예시로 정리했습니다. 반대로 겉모양만 비슷한 코드를 섣불리 합치면 인자가 늘고 예외 플래그가 붙는 잘못된 추상화가 됩니다.
- **추출과 매개변수화는 어떤 순서로 적용해야 할까요?**
  - 글의 순서는 세 번째 반복까지 기다린 뒤 공통 로직을 먼저 추출하고, 차이가 정말 값 하나로 표현될 때만 매개변수화하는 방식입니다. `with_tax(price, rate)`나 `greet(name, lang)`처럼 공통과 차이가 분명할 때만 인자를 올려야 호출부가 단순해지고 미니 프레임워크 같은 실패한 추상화를 피할 수 있습니다.

## 처음 질문으로 돌아가기

- **DRY의 진짜 의미는 무엇일까요?**
  - DRY는 코드 줄 수를 줄이라는 조언이 아니라 지식의 출처를 하나로 유지하라는 원칙입니다. 같은 비즈니스 규칙이 두 곳에 있으면 한 곳을 고치고 다른 곳을 놓치는 순간 시스템에 서로 다른 진실이 생깁니다.
- **우연히 닮은 중복과 본질적인 중복은 어떻게 구분할까요?**
  - `is_essential_duplication` 예시처럼 변경 이슈가 같이 열리는지, 도메인 용어가 동일한지, 실패 영향과 배포 타이밍이 같은지를 기준으로 판단합니다. 이 네 조건 중 셋 이상이 맞으면 본질적 중복으로 보고 추출을 검토합니다.
- **추출과 매개변수화는 어떤 순서로 적용해야 할까요?**
  - 세 번째 반복까지 기다린 뒤 공통 로직을 먼저 추출하고, 차이가 정말 값 하나로 표현될 때만 매개변수화합니다. `with_tax(price, rate)`나 `greet(name, lang)`처럼 공통과 차이가 분명할 때만 인자를 올려야 호출부가 단순해집니다.
- **실전 적용: 안전하게 중복 없애기에서 가장 흔한 실수는 무엇일까요?**
  - 공통 함수가 인자 여섯 개짜리 미니 프레임워크가 되는 것과, 우연한 유사성을 억지로 합쳐 서로 다른 변경 이유가 묶이는 경우가 가장 흔합니다. 첫 번째 중복에서 바로 추출하는 것도 우연한 유사성일 가능성이 크므로 세 번째가 나올 때까지 기다리는 편이 안전합니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 추출 전후 호출 결과가 그대로 유지되는지 테스트로 확인해야 합니다. 데이터 구조로 옮긴 정책은 새 케이스 추가가 기존보다 단순해야 하며, 중복 제거의 성패는 충돌 감소와 리뷰 시간 단축 같은 운영 지표로 판단합니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 달라지는 부분만 인자로 올려야 하고, 데이터 구조가 분기와 중복을 흡수할 수 있으며, 추상화는 필요가 분명할 때만 생겨야 합니다.
- **잘못된 추상화가 만들어진 것을 어떤 신호로 알 수 있고, 어떻게 되돌려야 할까요?**
  - 공통 함수 인자가 5개 이상으로 증가하거나, 호출자 대부분이 더미 값을 넘기거나, 새 요구사항마다 예외 플래그가 늘어나면 잘못된 추상화 신호입니다. `process_order_payment`와 `process_subscription_payment`처럼 두 개의 단순한 함수로 다시 인라인하는 것이 올바른 되돌림입니다.
- **데이터 중복이 코드 중복보다 더 위험할 수 있는 이유는 무엇일까요?**
  - 상수나 정책 값이 여러 함수에 흩어져 있으면 하나를 바꿀 때 나머지를 놓치기 쉽고, 코드 중복과 달리 lint나 정적 분석으로 잡기 어렵습니다. `PRICING_RULES`, `PLANS` 같은 테이블 구조로 출처를 단일화하면 수정 위치가 하나로 고정됩니다.
...
- **Clean Code 101 (5/10): 중복 제거 (현재 글)**
