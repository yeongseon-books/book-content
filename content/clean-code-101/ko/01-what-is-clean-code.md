...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/163"
...
- **Clean Code를 판단할 때 어떤 신호를 먼저 봐야 할까요?**
  - 함수 길이, 인자 개수, 들여쓰기 깊이, 이름의 정직성, 순환 복잡도가 핵심 신호입니다. `radon cc app/ -a -s`와 `ruff check app/`처럼 측정 가능한 도구를 붙여야 감상이 아닌 근거로 품질을 판단할 수 있습니다.
- **동작하는 코드와 읽기 쉬운 코드의 차이는 무엇일까요?**
  - `f(d, t)`를 `total_with_tax(amount, tax_rate)`로 바꾼 예시처럼, 읽기 쉬운 코드는 호출 지점에서 의도와 단위가 바로 드러납니다. 같은 동작을 하더라도 이름과 구조가 분명하면 다음 사람이 어디를 바꿔야 하는지 훨씬 빨리 판단할 수 있습니다.
- **작은 원칙이 실제 유지보수 비용에 왜 큰 차이를 만들까요?**
  - `QualityGate`, `change_impact_score`, 코드 품질 대시보드 예시가 보여 주듯 작은 원칙은 리뷰 시간, 버그 수, 변경 실패율로 이어집니다. 이름 하나와 분기 하나를 바로잡는 습관이 쌓이면 다음 수정의 탐색 비용과 장애 위험이 함께 내려갑니다.
- **실전 적용: 지저분함을 먼저 측정하기에서 가장 흔한 실수는 무엇일까요?**
  - 복잡도 수치만 보고 이름과 책임 분리를 놓치는 것이 가장 흔한 실수입니다. lint 경고가 많아서 진짜 설계 냄새가 묻히는 경우도 빈번합니다. 측정 도구는 대화의 시작점이지 절대 기준이 아닙니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 복잡도 등급이 높은 함수를 찾아내더라도, 12줄짜리 함수라도 이름이 거짓말하고 부수 효과가 숨어 있으면 더 위험할 수 있습니다. 지표를 맹신하지 않고 설계 의도를 함께 검증하는 습관이 중요합니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 이름은 가장 먼저 의도를 보여 주는 인터페이스이고, 함수 길이·분기 깊이·인자 수는 감상이 아니라 측정 가능한 신호이므로, 작은 규칙을 누적하면 코드베이스 전체의 변경 비용이 달라집니다.
- **기술 부채를 "나중에 고치면 된다"고 두면 실제로 비용이 어떻게 변할까요?**
  - `estimate_total_cost` 예시처럼 지연할수록 수정 시간이 복리로 증가합니다. 지금 18시간이면 6개월 뒤 30시간 이상으로 커지고, 장애 위험 비용까지 더해지면 리팩토링이 취향이 아니라 재무적 선택임이 분명해집니다.
- **코드 품질 지표를 팀이 매주 공유해야 하는 이유는 무엇일까요?**
  - 복잡도 상위 함수, 테스트 미보강 파일, 중복 감지 목록, 반복 리뷰 키워드를 주간 대시보드로 공유하면 "어디부터 고칠지"가 바로 보이고, 다음 행동을 즉시 고르게 됩니다. 측정하지 않으면 개선 우선순위도 흐려집니다.

<!-- toc:begin -->
## 시리즈 목차

- **Clean Code 101 (1/10): Clean Code란 무엇인가? (현재 글)**
