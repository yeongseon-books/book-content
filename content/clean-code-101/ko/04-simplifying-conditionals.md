...
- **가드 절과 조기 반환은 어떤 상황에서 가장 효과적일까요?**
  - 가드 절과 조기 반환은 비정상 입력이나 예외 케이스를 먼저 걷어 내고 정상 흐름을 한눈에 남겨야 할 때 가장 효과적입니다. `approve_refund`와 `price(user, item)` 예시처럼 `None`, 비활성 사용자, 품절 상품을 초반에 반환하면 핵심 정책이 마지막 줄에 또렷하게 남습니다.
- **부정형 조건과 이중 부정은 왜 읽기 어렵게 만들까요?**
