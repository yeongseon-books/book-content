...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/166"
...
- 실전 적용: 분기를 줄이는 다섯 단계에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 새 배송 정책이 추가될 때마다 기존 if/elif 코드를 수정해야 한다면 어떤 구조로 바꿔야 할까요?
- 정책 분기를 데이터 구조로 옮기면 테스트 전략이 어떻게 달라질까요?
...
점수를 수치로 남기면 리뷰 대화가 감각에서 근거 중심으로 이동합니다.
  - `if not is_not_empty(x)` 같은 표현은 독자가 조건을 머릿속에서 한 번 더 뒤집게 만들어 인지 비용을 올립니다. 그래서 이 글은 `user.is_inactive가 아닌 경우`보다 `if not user.is_active: return False`처럼 긍정형 이름과 단순한 가드 절로 읽는 방향을 권했습니다.
- **if/else 체인은 언제 다형성으로 바꾸는 편이 좋을까요?**
  - 타입 분기가 반복되거나 사용자 등급·국가별 정책처럼 규칙 추가가 잦은 영역에서는 다형성, 전략 객체, 정책 테이블로 옮기는 편이 좋습니다. `Shape.area()`, `POLICIES`, `FREE_SHIPPING_POLICY` 예시처럼 새 규칙을 추가할 때 기존 `if/elif` 본문을 계속 뜯지 않아도 되는 구조가 목표입니다.

## 처음 질문으로 돌아가기

- **가드 절과 조기 반환은 어떤 상황에서 가장 효과적일까요?**
  - 비정상 입력이나 예외 케이스를 초반에 걸러야 할 때 가장 효과적입니다. `approve_refund` 예시처럼 중첩 깊이 4에서 1로 줄면 정상 흐름이 마지막 한 줄로 모여 리뷰어가 정책을 더 빠르게 검증할 수 있습니다.
- **부정형 조건과 이중 부정은 왜 읽기 어렵게 만들까요?**
  - `if not is_not_empty(x)` 같은 표현은 독자가 조건을 머릿속에서 한 번 더 뒤집게 만들어 인지 비용을 올립니다. 부정형 이름과 부정형 조건이 겹치면 이중 부정이 되어 거의 항상 더 나쁜 이름이나 구조의 신호입니다.
- **if/else 체인은 언제 다형성으로 바꾸는 편이 좋을까요?**
  - 타입 분기가 반복되거나 국가별 배송 정책처럼 규칙 추가가 잦은 영역에서 다형성, 전략 객체, 정책 테이블로 옮기는 편이 좋습니다. `FREE_SHIPPING_POLICY`나 `POLICIES` 예시처럼 새 규칙을 추가할 때 기존 if/elif 본문을 건드리지 않아도 되는 구조가 목표입니다.
- **실전 적용: 분기를 줄이는 다섯 단계에서 가장 흔한 실수는 무엇일까요?**
  - 가드 절로 바꾸면서 예외 처리 순서가 달라지는 경우와, 타입 분기를 숨기고 이름만 예쁘게 바꾸는 경우가 가장 흔합니다. 순서 의존적인 테이블을 만들면 우선순위가 깨지기 쉬우므로 주의가 필요합니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 중첩을 줄인 뒤 복잡도와 테스트 안정성을 함께 확인해야 합니다. `max_indentation_depth` 같은 측정으로 깊이가 실제로 감소했는지 수치를 비교하고, 분기 정책을 테이블로 옮겨도 기존 테스트 결과가 동일해야 합니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 가드 절은 들여쓰기를 줄이고, 다형성은 조건문 자체를 없애 주며, 테이블은 정책을 데이터로 표현하게 만들어 줍니다.
- **새 배송 정책이 추가될 때마다 기존 if/elif 코드를 수정해야 한다면 어떤 구조로 바꿔야 할까요?**
  - `FREE_SHIPPING_POLICY` 딕셔너리처럼 정책을 데이터 구조로 올리거나, `DiscountRule` 전략 객체를 추가하는 방식으로 바꿔야 합니다. 기존 함수 본문을 수정하지 않고 규칙만 추가할 수 있어야 개방-폐쇄 원칙을 만족합니다.
- **정책 분기를 데이터 구조로 옮기면 테스트 전략이 어떻게 달라질까요?**
  - 분기 조건별 테스트가 데이터 항목별 테스트로 바뀌어 테스트 케이스 수가 줄고 추가도 쉬워집니다. `shipping_fee`를 테이블로 바꾸면 국가-임계값-요금 쌍을 데이터로 표현해 파라미터화 테스트로 일괄 검증할 수 있습니다.
...
- **Clean Code 101 (4/10): 조건문 줄이기 (현재 글)**
