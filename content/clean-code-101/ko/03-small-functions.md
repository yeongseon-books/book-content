...
- 실전 적용: 안전하게 추출하기에서 가장 흔한 실수는 무엇일까요?
- 검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
- 리팩터링 없이 기능만 추가하면 왜 변경 비용이 계속 올라갈까요?
- 함수 분해 시점을 팀이 명확하게 합의하려면 어떤 기준을 코드로 표현해야 할까요?

작은 함수의 장점은 단순히 줄 수가 적다는 데 있지 않습니다. 핵심은 “이 함수가 한 가지 일만 한다”는 사실이 이름과 본문에서 동시에 드러난다는 사실입니다. 그 상태가 되면 함수 본문은 설명문이 아니라 목차처럼 읽히기 시작합니다.
...
- **Extract Function은 어떤 순서로 적용해야 안전할까요?**
  - 이 글은 반복문 같은 의미 단위를 먼저 추출하고, 그다음 의도를 드러내는 이름을 붙이며, 이어서 Command/Query 분리와 매개변수 객체 도입까지 가는 순서를 제안했습니다. `line_total`, `Range`, `should_split_function` 예시처럼 한 번에 크게 바꾸지 않고 책임이 분명한 덩어리부터 떼어 내는 편이 안전합니다.
- **부수 효과를 줄이는 대표 패턴은 무엇일까요?**
  - 가장 대표적인 패턴은 Query에서 상태 변경을 제거하는 Command-Query Separation과, 계산을 순수 함수로 떼어 내는 방식입니다. `discount(price, rate)`처럼 같은 입력에 같은 출력을 돌려주는 함수가 늘수록 단위 테스트는 빨라지고 외부 의존성은 경계로 밀려납니다.

## 처음 질문으로 돌아가기

- **작은 함수가 주는 효과는 무엇일까요?**
  - 함수 본문이 목차처럼 읽히고, 이름이 주석을 대신하며, 단위 테스트가 쉬워집니다. `checkout` 예시처럼 60줄을 4줄의 흐름 함수로 바꾸면 가독성뿐 아니라 각 단계를 독립적으로 테스트할 수 있게 됩니다.
- **Extract Function은 어떤 순서로 적용해야 안전할까요?**
  - 반복문 같은 의미 단위를 먼저 추출하고, 의도를 드러내는 이름을 붙이며, Command/Query를 분리하고, 인자가 늘어나면 매개변수 객체를 도입하는 순서가 안전합니다. 한 번에 크게 바꾸지 않고 책임이 분명한 덩어리부터 떼어 냅니다.
- **부수 효과를 줄이는 대표 패턴은 무엇일까요?**
  - Query 함수에서 상태 변경을 제거하는 Command-Query Separation과, `discount(price, rate)`처럼 계산을 순수 함수로 추출하는 방식이 대표적입니다. 순수 함수가 늘수록 단위 테스트는 빨라지고 외부 의존성은 경계로 밀려납니다.
- **실전 적용: 안전하게 추출하기에서 가장 흔한 실수는 무엇일까요?**
  - 추출 뒤 인자 수가 급격히 늘어나는 것과 질문 함수가 여전히 상태를 바꾸는 것이 가장 흔한 실수입니다. 인자가 4개 이상으로 늘어나면 매개변수 객체 도입 시점인지 다시 점검해야 합니다.
- **검증 방법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 추출 전후 복잡도와 테스트 안정성을 함께 확인해야 합니다. 함수 본문이 목차처럼 읽히는지 바로 확인하고, 복잡도 등급이 낮아졌는지 `radon cc`로 수치를 비교하는 것이 중요합니다.
- **실패하기 쉬운 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 함수 본문은 목차처럼 읽혀야 하고, 이름이 주석을 대신해야 하며, Command/Query를 분리하면 버그 추적이 쉬워집니다.
- **리팩터링 없이 기능만 추가하면 왜 변경 비용이 계속 올라갈까요?**
  - 긴 함수 안에 새 정책이 중첩될수록 들여쓰기 깊이와 순환 복잡도가 함께 올라갑니다. `FunctionSplitDecision` 예시처럼 책임이 여러 개인 함수는 테스트 조합이 폭증하고, 버그 수정이 의도치 않게 다른 정책에 영향을 미칩니다.
- **함수 분해 시점을 팀이 명확하게 합의하려면 어떤 기준을 코드로 표현해야 할까요?**
  - `should_split_function` 예시처럼 복수 정책 존재 여부, 부수 효과 혼합 여부, 인자 수 기준을 코드로 선언해 두면 리뷰 대화가 "감각"이 아닌 "기준"으로 이동합니다. 함수 길이 5~20줄, 인자 0~3개, 중첩 깊이 0~2단을 경고 기준으로 lint에 고정하는 것도 같은 효과를 냅니다.
...
- **Clean Code 101 (3/10): 함수 작게 만들기 (현재 글)**
