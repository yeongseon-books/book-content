...
이 글은 Incident Response 101 시리즈의 4번째 글입니다.

incident 대응에서 기술 복구만 잘하면 끝이라고 생각하기 쉽습니다. 하지만 실제로는 반대 장면을 더 자주 봅니다. 복구가 조금 늦는 것보다, 공지가 없거나 잘못된 사람에게 잘못된 메시지가 가는 편이 신뢰를 더 빨리 무너뜨립니다. 그래서 communication은 부가 작업이 아니라 대응 그 자체의 일부입니다.
...
```mermaid
flowchart LR
    Incident["incident"] --> Internal["내부"]
    Incident --> External["외부"]
    Incident --> Exec["경영진"]
    Internal --> Slack["채팅"]
    External --> Status["상태 페이지"]
    Exec --> Email["이메일"]
```
