...
- **incident가 끝난 뒤 무엇이 언제 일어났는지 어떻게 복원할까요?**
  - 본문에서 본 것처럼 단일 화자의 기억에 의존하지 말고 모니터링 알람, 채팅(Slack/Teams) 타임스탬프, 배포 파이프라인 기록, 명령 로그, 티켓 상태 변화 같은 객체별 기계 시각을 모아 한 줄로 정렬해야 합니다. 사람이 본 시간이 아니라 시스템이 찍은 시간으로 줄을 세워야 사건 사이 인과 관계가 흐트러지지 않습니다.
- **왜 사건 중간에 바로 기록해야 할까요?**
  - 본문에서 강조했듯이 incident 한가운데서는 결정이 1-2분 단위로 일어나기 때문에, 그 자리에서 채팅에 짧게라도 "왜 이걸 시도하기로 했는지"를 남기지 않으면 사후에 의도를 복원할 수가 없습니다. 결과만 남고 판단 근거는 사라지면 postmortem에서 같은 실수의 패턴을 찾을 수 없게 됩니다.
- **한 채널이 아니라 여러 채널을 함께 스크랩해야 하는 이유는 무엇일까요?**
  - 본문 예시처럼 알람은 모니터링 시스템에, 의사결정은 채팅에, 실제 명령 실행은 셸/배포 도구에, 외부 공지는 status page에 흩어져 남기 때문입니다. 한 채널만 보면 "감지-결정-실행-소통" 중 한 단면만 보여 인과 사슬이 끊기고, 모든 채널을 합쳐야 비로소 사건의 전체 그림이 복원됩니다.
...
- **Incident Response 101 (5/10): Timeline 작성 (현재 글)**
