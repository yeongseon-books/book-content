...
이 그림의 핵심은 모든 이벤트가 incident는 아니라는 사실입니다. 먼저 고객 영향이 있는지 묻고, 그 영향이 팀이 정한 기준을 넘는지 본 뒤에야 incident라는 이름을 붙입니다. 이름이 바뀌면 대응 경로도 함께 바뀝니다.
...
- **어떤 문제를 incident라고 불러야 할까요?**
  - 어떤 문제를 incident라고 불러야 할까요 — 본문에서 구체적으로 다룹니다.
- **alert와 incident는 무엇이 다를까요?**
  - 먼저 사건을 판단하는 데 필요한 최소 정보를 한 구조로 묶습니다. 여기서는 영향을 받은 사용자 수와 지속 시간을 넣습니다.
- **고객 영향은 어떤 식으로 수치화해야 할까요?**
  - 고객 영향은 어떤 식으로 수치화해야 할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 주제가 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - incident 정의가 없으면 대응은 두 방향으로 무너집니다. 하나는 과잉 대응입니다. 작은 경고에도 사람을 모두 깨우고, 결국 팀은 알림 자체를 덜 믿게 됩니다.
- **한눈에 보는 구조을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 이 그림의 핵심은 모든 이벤트가 incident는 아니라는 사실입니다. 먼저 고객 영향이 있는지 묻고, 그 영향이 팀이 정한 기준을 넘는지 본 뒤에야 incident라는 이름을 붙입니다. 이름이 바뀌면 대응 경로도 함께 바뀝니다.
- **전후 비교의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 이전: 모든 alert를 incident처럼 취급합니다.
