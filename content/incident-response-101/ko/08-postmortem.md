...
많은 팀이 사후 분석 문서를 쓰지만, 실제로는 두 극단으로 흔들리기 쉽습니다. 하나는 비난 문서가 되는 경우입니다. 누가 실수했는지 찾는 데 집중하면 사람은 방어적으로 바뀌고, 시스템 원인은 가려집니다. 다른 하나는 예쁘게 정리된 기록만 남고 후속 조치가 사라지는 경우입니다. 문서는 남았지만 행동은 바뀌지 않습니다.

이 글에서는 postmortem의 목적을 먼저 짚고, blameless 원칙, 템플릿을 고정해야 하는 이유, action item을 어떻게 추적할지, 그리고 실무에서 이 문서를 어떻게 조직 전체 학습으로 연결하는지 정리하겠습니다.

> 좋은 postmortem은 누군가를 탓하는 문서가 아니라, 사실·영향·원인·후속 조치를 남기는 학습 문서입니다.

## 이 글에서 다룰 문제

- postmortem은 왜 사건 종료 뒤에 꼭 필요할까요?
- 비난 없는 원칙은 “책임 없음”과 어떻게 다를까요?
...
같은 incident가 반복된다는 말은 대개 시스템이 아니라 학습 루프가 끊겼다는 뜻입니다. 사건은 끝났는데 원인이 구조로 정리되지 않았고, 후속 조치가 추적되지 않았으며, 다음 사람에게 전달되지 않았기 때문입니다. 그래서 사후 분석 문서는 회고 문서이면서 동시에 예방 장치입니다.

또 하나 중요한 이유는 기억의 왜곡입니다. 사건이 끝난 직후에는 모두가 생생하게 기억한다고 생각하지만, 며칠만 지나도 순서와 판단 근거가 흐려집니다. 사실을 남기지 않으면 조직은 가장 큰 목소리나 마지막 기억에 의존하게 됩니다. 사후 분석 문서는 그 왜곡을 줄이기 위한 기록 장치입니다.

## 한눈에 보는 흐름

```mermaid
flowchart LR
    Incident["incident"] --> Draft["초안 작성"]
    Draft --> Review["리뷰"]
    Review --> Publish["공유"]
    Publish --> Action["액션 아이템"]
    Action --> Track["추적"]
```

핵심은 publish 뒤에 끝나지 않는다는 점입니다. 공유가 끝이 아니라 액션 아이템 등록과 추적이 이어져야 학습이 행동으로 바뀝니다.

## 핵심 용어

- **blameless**: 개인 비난 없이 시스템과 맥락을 중심으로 원인을 살피는 원칙입니다.
- **summary**: 사건을 짧게 이해할 수 있도록 정리한 세 문장 안팎의 요약입니다.
- **impact**: 고객이 실제로 겪은 영향입니다. 가능하면 수치로 남겨야 합니다.
- **action item**: 재발 방지나 대응 개선을 위한 검증 가능한 후속 작업입니다.
- **owner**: 그 후속 작업의 책임자입니다.

여기서 비난 없는 원칙은 책임을 지우지 않는다는 뜻이 아닙니다. 개인을 원인으로 끝내지 않는다는 뜻입니다. “누가 잘못했다”에서 멈추면 다음 incident에서도 똑같은 조건이 남습니다. 어떤 경보가 없었는지, 어떤 리뷰가 부족했는지, 어떤 기본값이 위험했는지를 함께 봐야 합니다.

## Before / After

**Before**: 내부에서만 돌고, 비난과 방어가 섞인 문서를 남깁니다.

**After**: 조직 전체가 참고할 수 있는 비난 없는 문서와 후속 조치 목록을 남깁니다.

after 상태에서는 문서의 용도가 바뀝니다. 읽는 사람은 누구를 탓해야 하는지가 아니라, 다음에 무엇을 바꿔야 하는지를 찾습니다. 그래서 좋은 사후 분석 문서는 감정보다 구조가 앞에 옵니다.
...
영향을 “컸다”라고만 쓰면 다음 비교가 어렵습니다. 영향을 받은 사용자 수와 지속 시간처럼 비교 가능한 단위를 남기는 편이 좋습니다.
...
추적은 복잡할 필요가 없습니다. 오늘 날짜와 due date를 비교하는 것만으로도 방치된 액션을 찾을 수 있습니다.
...
즉, 좋은 사후 분석 문서는 글을 잘 쓰는 재능보다 구조를 잘 고정하는 습관에서 나옵니다. 양식이 일정해야 읽는 사람도 빨리 훑을 수 있고, 사건 사이 비교도 쉬워집니다.

## 자주 하는 실수 5가지

1. 개인 이름을 그대로 원인으로 적고 시스템 원인을 더 보지 않습니다.
2. 후속 조치를 적어 놓고 담당자를 붙이지 않습니다.
3. due date가 없어 추적이 끊깁니다.
4. 문서를 대응팀 내부에서만 보고 조직에 공유하지 않습니다.
5. 사건마다 템플릿을 새로 만들어 비교 가능성을 잃습니다.

가장 위험한 실수는 후속 조치 없는 postmortem입니다. 문서는 근사하지만 무엇도 바뀌지 않습니다. 그 문서는 기록일 뿐 학습 장치가 아닙니다.
...
시니어 엔지니어는 blameless를 문화라고 봅니다. 사고의 원인을 개인에게만 묶지 않고, 시스템과 프로세스의 빈틈으로 확장해서 봅니다. 동시에 문서만 남겨서는 아무 의미가 없다는 점도 잘 압니다. 후속 조치, 책임자, 기한, 분기 리뷰까지 있어야 사후 분석이 닫힙니다.
...
postmortem은 incident 뒤에 남기는 회고 메모가 아니라, 조직 학습을 실제 행동으로 바꾸는 문서입니다. 비난 없는 원칙으로 사실과 맥락을 정리하고, 영향을 수치로 남기고, 후속 조치에 담당자와 기한을 붙여야 같은 문제가 반복되는 일을 줄일 수 있습니다. 문서 작성만으로 끝나지 않고, 추적과 리뷰까지 이어질 때 비로소 의미가 생깁니다.
