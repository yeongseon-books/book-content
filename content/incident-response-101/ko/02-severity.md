...
여기서 중요한 사실은 severity가 기술 규칙이면서 동시에 조직 규칙이라는 사실입니다. 숫자는 코드에 있지만, 그 숫자를 정하는 기준은 서비스 특성과 비즈니스 맥락에서 나옵니다. 결제 서비스의 SEV1과 내부 도구의 SEV1이 같을 필요는 없습니다.
...
실서비스에서는 결제 실패가 기본적으로 SEV1로 올라가고, 검색 결과 정렬 오류는 기본적으로 SEV3로 내려가는 식의 기본 규칙을 둡니다. 중요한 점은 사건이 발생한 뒤 감으로 토론하는 일이 아니라, 발생하기 전에 이미 표와 예시를 만들어 두는 일입니다.
...
여기서 중요한 점은 "외부 공지 없음"도 정책으로 명시해야 한다는 사실입니다. 명시되지 않으면 incident마다 임의 해석이 생깁니다.
...
severity 체계를 운영에 붙이려면 정의 문장만으로는 부족합니다. 사건이 열렸을 때 바로 읽고 점수화할 수 있는 매트릭스가 필요합니다. 아래 표는 사용자 영향, 핵심 경로, 금전 영향, 복구 예상 시간을 함께 보도록 설계한 예시입니다.

| 판단 축 | SEV1 | SEV2 | SEV3 |
| --- | --- | --- | --- |
| 사용자 영향 | 광범위(전사/다수 리전) | 주요 고객군 영향 | 제한적 사용자 영향 |
| 핵심 경로 | 결제/로그인 중단 | 핵심 기능 저하 | 보조 기능 이상 |
| 금전/계약 영향 | 즉시 손실/위약 리스크 | 부분 손실 가능 | 경미 또는 없음 |
| 복구 예상 시간 | 60분 이상 불확실 | 15~60분 | 15분 이내 |

점수화 규칙도 함께 고정하면 경계 사례 토론 시간이 줄어듭니다. 예를 들어 SEV1 조건 두 개 이상이면 즉시 상향, SEV2 조건 두 개면 SEV2 유지 같은 단순 규칙을 둘 수 있습니다. 중요한 것은 규칙의 복잡도가 아니라 일관성입니다.

## 보고 체계와 승격(상향) 템플릿

severity는 정적인 라벨이 아니라 사건 중 바뀔 수 있는 상태입니다. 따라서 상향/하향 시점의 근거를 남기는 템플릿이 필요합니다.
...
이 템플릿을 timeline과 함께 쓰면 "왜 그때 SEV2였는가"를 나중에 설명할 수 있습니다. severity 품질은 선언 순간보다 변경 이력 관리에서 더 잘 드러납니다.

## 운영 부록: Severity 점수 카드

팀마다 숫자는 다를 수 있지만, 점수화 기준을 고정해 두면 경계 사례 토론 시간이 크게 줄어듭니다.

| 항목 | 점수 0 | 점수 1 | 점수 2 |
| --- | --- | --- | --- |
| 고객 수 | 100 미만 | 100~5000 | 5000 초과 |
| 핵심 경로 | 영향 없음 | 일부 실패 | 결제/로그인 직접 실패 |
| 지속 시간 | 5분 미만 | 5~30분 | 30분 초과 |
| 금전 영향 | 없음 | 추정 가능 | 즉시 손실 관측 |

점수 합계가 0~2면 SEV3, 3~5면 SEV2, 6 이상이면 SEV1처럼 기본 규칙을 둘 수 있습니다.

## 운영 부록: 상향/하향 로그

```text
[severity-change]
incident_id: INC-2026-05-21-004
changed_at_utc: 2026-05-21T02:11:00Z
from: SEV3
to: SEV2
reason:
- impacted_users exceeded 1200
- checkout failure rate reached 6.2%
owner: incident-commander
next_review_utc: 2026-05-21T02:25:00Z
```

severity 변경 이력을 남기면 postmortem에서 "그때 왜 그렇게 판단했는가"를 설명할 수 있습니다.

## 운영 부록: 분기 리뷰 지표

- SEV1/2/3 분포가 특정 서비스에 편중되는가?
- 상향 횟수 대비 하향 횟수는 어떤가?
- 평균 상향 시점이 탐지 후 몇 분인가?
- 상향 직후 커뮤니케이션 지연이 반복되는가?
- 등급 정의 문서 개정이 실제 분류 편차를 줄였는가?
...
또한 severity 임계값은 분기마다 트래픽 변동과 사용자 기대 수준을 반영해 재조정해야 합니다. 특히 사용자 수가 급증하는 이벤트 기간에는 임시로 임계값을 낮추는 것이 안전합니다.
...
- **severity는 무엇을 위한 언어일까요?**
  - 영향을 한 숫자로만 보면 놓치는 부분이 많습니다. 사용자 수, 지역 범위, 금전 손실처럼 사건을 여러 축으로 나눠 봐야 경계가 분명해집니다.
- **SEV1, SEV2, SEV3는 어떤 기준으로 갈라야 할까요?**
  - severity 체계가 없으면 incident 대응은 매번 즉석 토론으로 시작합니다
- **사용자 수, 범위, 금전 손실 같은 영향 축은 왜 필요할까요?**
  - 영향을 한 숫자로만 보면 놓치는 부분이 많습니다
  - 여기서 핵심은 매핑입니다. 영향이 먼저 있고, severity는 그 영향을 운영 언어로 번역한 결과입니다. 그리고 각 등급은 누가 움직이고 얼마나 자주 공유할지를 결정합니다.
...
- **Incident Response 101 (2/10): Severity 분류 (현재 글)**
