...
- 자주 하는 실수 다섯 가지을 실무에 적용할 때 주의할 점은 무엇일까요?
- 실무에서는 이렇게 나타납니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **AOT와 JIT는 각각 어떻게 정의할 수 있을까요?**
  - AOT는 배포 전에 미리 컴파일해 바이너리를 만들어 두는 방식이고, JIT는 실행 중에 필요한 코드를 기계어로 바꾸는 방식입니다. 글의 흐름도처럼 AOT는 `compile time -> binary -> fast startup`으로 이어지고, JIT는 먼저 해석하거나 baseline으로 돌린 뒤 hot path를 컴파일합니다.
- **warmup은 왜 생기고 어떻게 측정해야 할까요?**
  - warmup은 JIT가 아직 프로파일을 모으지 못했거나 최적화된 코드를 만들기 전이라서, 첫 실행이 인터프리트나 초기 컴파일 비용을 함께 떠안기 때문에 생깁니다. `numba` 예제에서 첫 호출 시간과 두 번째 호출 시간을 따로 재는 방식이 warmup 비용과 warm 성능을 분리해 보는 가장 직접적인 측정입니다.
- **각 실행 모델은 어떤 최적화 기회를 열어 줄까요?**
  - JIT는 호출 빈도와 분기 방향 같은 런타임 정보를 이용해 hot path에만 공격적인 최적화를 적용할 수 있고, tiered compilation으로 그 강점을 더 키웁니다. AOT는 실행 중 정보는 없지만 `gcc -fprofile-generate`와 `-fprofile-use`처럼 PGO를 통해 실제 워크로드 데이터를 일부 반영해 안정적인 배포물에 최적화를 녹여 넣을 수 있습니다.
