...
- 변경 전후에서 가장 흔한 실수는 무엇일까요?

- 자주 하는 실수 다섯 가지의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
```python
# 2_interference.py
# 동시에 살아 있는 두 temporary는 같은 register를 공유할 수 없습니다.
# → 그래프에서 edge 하나로 표현
...
```python
# 3_color.py
# K개의 color(register)가 주어졌을 때 인접 노드가 다른 색이 되도록 칠합니다.
...
```python
# 4_spill.py
# register가 부족하면 stack에 저장했다가 다시 불러옵니다.
def spill(code, var):
    new = []
...
```python
# 5_call.py
# x86-64 System V: 앞의 6개 integer 인자는 rdi, rsi, rdx, rcx, r8, r9에 전달
# return 값은 rax에 둡니다.
...
- **코드 생성이 해결해야 하는 두 핵심 문제는 무엇일까요?**
  - 코드 생성의 두 핵심 문제는 IR 연산을 실제 CPU 명령으로 고르는 instruction selection과, 무한해 보이는 가상 레지스터를 제한된 물리 레지스터에 배치하는 register allocation입니다. `t1`, `t2`, `t3`가 있는 IR이 `mov rax, [a]`, `add rax, [b]`, `ret` 같은 실제 명령으로 접히는 예제가 두 문제를 한 번에 보여 줍니다.
- **instruction selection은 어떤 직관으로 동작할까요?**
  - instruction selection은 IR 패턴마다 대응하는 기계 명령 시퀀스를 붙이는 패턴 매칭으로 이해하면 됩니다. 본문의 `select(inst)`는 `LOAD`를 `mov`, `+`를 `mov` 다음 `add`, `RET`를 `mov rax, ...`와 `ret`으로 바꾸며 가장 단순한 1:1 직관을 보여 줍니다.
- **register allocation은 왜 그래프 색칠 문제로 보일까요?**
  - 동시에 살아 있는 temporary끼리는 같은 레지스터를 쓸 수 없으므로, 이 관계를 간선으로 그리면 간섭 그래프가 됩니다. `greedy_color(nodes, edges, k)` 예제처럼 인접 노드가 다른 색을 가져야 하고, 색이 부족하면 `SPILL`로 떨어지기 때문에 register allocation을 그래프 색칠 문제로 보는 것입니다.
