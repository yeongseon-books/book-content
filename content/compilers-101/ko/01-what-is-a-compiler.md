...
- **컴파일러를 한 줄로 어떻게 정의할 수 있을까요?**
  - 이 글에서 컴파일러는 소스 코드를 토큰, AST, 타입 정보, IR을 거쳐 최종 타깃 코드로 번역하는 단계적 변환 시스템입니다. `2 + 3 * 4`가 `lex → parse → check → IR → optimize → codegen`을 통과하는 예제가 그 정의를 그대로 보여 줍니다.
- **표준적인 컴파일러 파이프라인은 어떤 단계로 나뉠까요?**
  - 본문은 lexer, parser, semantic analyzer, intermediate representation, optimizer, code generator의 여섯 단계를 한 흐름으로 정리했습니다. `Token` 목록, `Num/BinOp` AST, `check(node)`의 타입 검사, `emit(node)`의 가짜 어셈블리 출력이 각 단계의 입력과 출력을 구체적으로 보여 줍니다.
- **인터프리터와 트랜스파일러는 이 파이프라인을 어디까지 공유할까요?**
  - 인터프리터는 같은 프런트엔드를 지나 AST를 `evaluate(node)`로 바로 실행하고, 컴파일러는 그 AST를 계속 내려 코드로 방출한다는 점에서 앞단을 넓게 공유합니다. 트랜스파일러도 토큰화와 파싱, 의미 확인 같은 구조를 공유하되 결과물을 기계어 대신 다른 고수준 언어로 내보낸다는 차이가 있습니다.
