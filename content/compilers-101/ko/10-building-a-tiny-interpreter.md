...
- **렉서, 파서, 평가기를 한 파일로 어떻게 조합할 수 있을까요?**
  - 이 글은 `tokenize(src) -> Parser(tokens).parse() -> evaluate(ast) -> print(result)` 순서로 세 단계를 한 파일에 직렬로 연결했습니다. `run(src)`가 그 연결점을 한 줄로 묶고, REPL 루프가 그 함수를 반복 호출하면서 전체 흐름을 보여 줍니다.
- **재귀 하강 파서의 최소 구현은 어떤 모습일까요?**
  - 최소 구현은 `peek`, `eat`, `expr`, `term`, `factor` 다섯 축만 있어도 충분합니다. `expr -> term`, `term -> factor` 구조와 `eat("EOF")` 검사가 들어가면 우선순위와 입력 종료 검증까지 한 번에 해결됩니다.
- **인터프리터는 AST를 어떻게 걸어 값을 만들까요?**
  - `evaluate(node)`는 `("Num", value)`면 값을 그대로 돌려주고, `("BinOp", op, left, right)`면 자식 노드를 재귀적으로 계산한 뒤 `+`, `-`, `*`, `/`를 적용합니다. 그래서 `mini> 1 + 2 * 3`이 `7.0`으로, `(1 + 2) * 3`이 `9.0`으로 나오는 결과가 AST 순회의 의미를 그대로 보여 줍니다.
