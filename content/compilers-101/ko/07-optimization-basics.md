...
- **최적화에서 가장 절대적인 규칙은 무엇일까요?**
  - 최적화의 절대 규칙은 프로그램 의미를 바꾸지 않는 범위에서만 더 빠르거나 더 작게 만드는 것입니다. `t1 = 2 * 3`, `t2 = 1 + t1`, `return t2`를 `return 7`로 줄이는 예제도 결과값이 같다는 전제가 유지될 때만 정당합니다.
- **constant folding은 어떤 식으로 동작할까요?**
  - `fold(code)`는 `consts` 환경에 상수 값을 기억해 두고, `+ - * /`의 양쪽 피연산자가 모두 상수로 해석되면 그 자리에서 계산해 `LOAD`로 바꿉니다. 그래서 `("*", "t3", "t1", "t2")`가 두 입력이 상수일 때 `("LOAD", "t3", 6, None)`으로 접히게 됩니다.
- **dead code elimination은 어떤 정보를 기반으로 할까요?**
  - DCE는 아래에서 위로 모은 사용 정보, 즉 어떤 값이 실제로 읽히는지에 대한 liveness 비슷한 정보를 바탕으로 동작합니다. 본문의 `dce(code)`는 `RET`에서 시작해 `used` 집합을 채우고, 결과가 한 번도 참조되지 않는 명령어를 두 번째 순회에서 제거합니다.
