...
```python
# x: int (3번째 줄에서 선언됨)
# Bin.+ 는 int + int를 요구하지만 int + str이 들어와 TypeError가 발생합니다.
```
...
- **문법적으로 맞다는 것과 의미적으로 맞다는 것은 어떻게 다를까요?**
  - 문법적으로 맞다는 것은 파서가 문장 구조를 받아들였다는 뜻이고, 의미적으로 맞다는 것은 그 구조가 실제 언어 규칙까지 만족한다는 뜻입니다. 본문의 `Bin("+", Var("x"), Str("hello"))`와 `x = y + 1` 예시는 파싱은 되더라도 선언되지 않은 이름이나 `int + str` 같은 조합은 시맨틱 단계에서 거부된다는 점을 보여 줍니다.
- **이름 해석은 무엇이며, 식별자는 어디를 가리킬까요?**
  - 이름 해석은 `Env`의 `declare()`와 `lookup()`을 통해 식별자가 어느 선언 엔트리를 가리키는지 결정하는 과정입니다. `resolve(node)`가 `Var`를 만났을 때 환경에 이름이 없으면 바로 `NameError`를 내는 코드가 그 연결 방식을 가장 단순하게 보여 줍니다.
- **타입 검사는 어떤 규칙으로 동작할까요?**
  - 타입 검사는 AST를 아래에서 위로 올라가며 각 식의 타입을 계산하고, `BIN` 노드에서는 좌우 피연산자의 타입이 규칙에 맞는지 확인하는 방식으로 동작합니다. `type_of()`와 `annotate()` 예제에서 `l != r`이면 `TypeError`를 내고, 맞으면 노드 끝에 타입을 붙여 annotated AST를 만드는 흐름이 핵심입니다.
