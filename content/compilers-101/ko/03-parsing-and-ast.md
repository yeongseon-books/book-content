...
- **AST는 무엇이며, 왜 꼭 트리여야 할까요?**
  - AST는 괄호 같은 표면 문법을 걷어 내고 `Bin("+", Num(1), Bin("*", Num(2), Num(3)))`처럼 의미 구조만 남긴 트리입니다. `1 + 2 * 3`이 `(1 + (2 * 3))`로 묶이는 이유가 트리 깊이로 표현되기 때문에, 평가기와 코드 생성기가 같은 구조를 그대로 순회할 수 있습니다.
- **재귀 하강 파서의 기본 형태는 어떻게 생겼을까요?**
  - 재귀 하강 파서는 문법 규칙마다 함수 하나를 두고 `expr()`, `term()`, `factor()`가 서로 호출하는 형태입니다. 여기에 `peek / advance / expect` 커서를 붙이면 토큰 스트림을 한 칸씩 읽으면서 필요한 지점에서 `SyntaxError`를 내는 최소 구조가 완성됩니다.
- **우선순위와 결합성은 코드 안에서 어떻게 표현할까요?**
  - 우선순위는 `expr → term → factor`처럼 함수 호출 깊이로 표현되고, `*`와 `/`를 `term()` 안에 넣는 순간 `+`보다 강하게 묶입니다. 결합성은 `while` 루프가 누적되는 방향으로 정해져서 `1 - 2 - 3`이 좌결합으로 읽히게 됩니다.
