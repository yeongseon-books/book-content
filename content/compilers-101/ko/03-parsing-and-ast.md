...
**핵심 질문**: `1 + 2 * 3`을 컴파일러는 왜 `(1 + (2 * 3))`으로 묶고, `((1 + 2) * 3)`으로 묶지 않습니까?

> Parser는 token stream을 입력으로 받아, 의미 구조를 가진 **트리(AST)**를 만듭니다. 연산자 우선순위, 결합성, 그리고 errror 보고가 모두 이 단계에서 결정됩니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- AST(abstract syntax tree)가 무엇이고, 왜 트리인가
- 재귀 하강(recursive descent) 파서의 기본 구조
- 연산자 우선순위와 결합성을 코드로 표현하는 법
- AST를 보기 좋게 출력하는 법
- 파서가 만드는 좋은 오류 메시지의 형태
...
- **AST**: 프로그램의 구조를 표현하는 트리. 괄호 같은 표면적 구문은 사라지고, 의미만 남습니다.
- **Recursive descent**: 문법의 각 규칙을 함수 하나로 옮긴 파서.
- **Precedence**: 어느 연산자가 더 강하게 묶이는가 (`*`가 `+`보다 강함).
- **Associativity**: 같은 우선순위에서 좌/우 어느 쪽으로 묶이는가 (`-`는 좌결합).
- **Lookahead**: 다음 토큰을 한 개 (또는 k개) 미리 보는 것.
...
- 새 문법을 보면 "어느 함수에 들어가야 하는가"를 먼저 정합니다.
- AST 노드 종류는 가능한 한 적게 유지합니다 (확장성보다 가독성).
- 파서가 만든 AST를 그림으로 출력하는 디버깅 도구를 항상 갖추고 있습니다.
- "이 표현식이 왜 이렇게 묶였지?"라는 질문은 함수 호출 깊이로 답합니다.
- 모호한 문법은 문법을 고쳐 해결하지, 파서로 우회하지 않습니다.
...
1. 위 파서에 단항 음수(`-1`, `-(1+2)`)를 추가해 보세요. 어느 함수에 들어가야 하는가?
2. `**` (지수, 우결합)를 추가해 보세요. 좌결합인 `+`와 다른 부분이 코드의 어디인가?
3. 잘못된 입력 `1 + * 2`에 대해 사용자에게 보여 줄 오류 메시지를 직접 설계해 보세요.
