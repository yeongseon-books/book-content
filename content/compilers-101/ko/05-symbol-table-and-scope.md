...
- **심볼 테이블은 정확히 무엇이며 왜 컴파일러의 핵심 자료구조일까요?**
  - 심볼 테이블은 이름을 `(kind, type, location)` 같은 선언 정보와 연결해 주는 컴파일러의 메모리입니다. `Scope.define("x", "int")`와 `Symbol(name, kind, ty, line, col)` 예제처럼 선언 정보와 위치를 함께 저장해야 이름 해석뿐 아니라 go-to-definition까지 같은 구조로 처리할 수 있습니다.
- **스코프는 스택이나 연결 딕셔너리로 어떻게 표현할 수 있을까요?**
  - 본문은 `parent` 포인터를 가진 `Scope` 하나로 모듈, 함수, 블록 스코프를 모두 표현했습니다. `Analyzer`의 `enter()`와 `exit()`가 새 `Scope(self.scopes[-1])`를 밀고 빼는 방식이라서, AST를 걷는 동안 현재 스코프와 바깥 스코프를 자연스럽게 오갈 수 있습니다.
- **shadowing과 lookup은 왜 자연스럽게 따라올까요?**
  - `resolve()`가 현재 테이블을 먼저 보고 없으면 `parent`로 올라가는 규칙을 쓰기 때문에 shadowing과 lookup은 별도 특수 규칙 없이 자동으로 생깁니다. `g.define("x", "int(global)")` 뒤에 `f.define("x", "int(local)")`를 호출하면 `f.resolve("x")`는 로컬을, `g.resolve("x")`는 글로벌을 돌려주는 예제가 바로 그 결과입니다.
