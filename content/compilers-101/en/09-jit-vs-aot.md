...
- How do you define AOT and JIT?
- Why does warmup happen and how do you measure it?
- What optimization opportunities does each execution model unlock?
## Why It Matters
...
- **How do you define AOT and JIT?**
  - AOT compiles before deployment and produces a binary; JIT compiles during execution and keeps compiled code in memory. As the flowchart shows, AOT follows `compile time → binary → fast startup`, while JIT interprets or baseline-compiles first, then optimizes hot paths.
- **Why does warmup happen and how do you measure it?**
  - Warmup happens because the JIT has not yet collected profiles or generated optimized code, so the first run pays interpretation or initial compilation cost. The numba example measures it directly: compare first-call time against second-call time to separate warmup cost from warm performance.
- **What optimization opportunities does each execution model unlock?**
  - JIT can use runtime information (call frequencies, branch directions) to aggressively optimize hot paths, amplified by tiered compilation. AOT lacks runtime data but can borrow some via PGO (`gcc -fprofile-generate` / `-fprofile-use`), baking observed workload data into a stable deployment binary.
<!-- toc:begin -->
## In this series
