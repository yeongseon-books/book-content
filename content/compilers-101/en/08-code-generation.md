...
- What are the two core problems code generation solves?
- What intuition drives instruction selection?
- Why does register allocation look like a graph coloring problem?
## Why It Matters
...
- **What are the two core problems code generation solves?**
  - Instruction selection (picking CPU instructions for each IR node) and register allocation (mapping unlimited virtual registers onto a handful of physical ones). The example where `t1, t2, t3` collapse into `mov rax, [a]; add rax, [b]; ret` shows both problems at once.
- **What intuition drives instruction selection?**
  - Pattern matching: each IR pattern gets a corresponding machine-instruction sequence. The `select(inst)` function maps `LOAD` to `mov`, `+` to `mov`+`add`, `RET` to `mov rax, ...`+`ret` — the simplest 1:1 intuition.
- **Why does register allocation look like a graph coloring problem?**
  - Temporaries alive at the same time cannot share a register, so that relationship forms edges in an interference graph. `greedy_color(nodes, edges, k)` assigns colors (registers) so adjacent nodes differ; when colors run out, the node spills to memory.
<!-- toc:begin -->
## In this series
