...
- What is an AST, and why does it have to be a tree?
- What does the basic shape of a recursive descent parser look like?
- How are precedence and associativity expressed in code?
## Why It Matters
...
- **What is an AST, and why does it have to be a tree?**
  - An AST strips away surface syntax like parentheses and keeps only meaning: `Bin("+", Num(1), Bin("*", Num(2), Num(3)))`. The reason `1 + 2 * 3` groups as `(1 + (2 * 3))` is expressed by tree depth, so evaluators and code generators can walk the same structure directly.
- **What does the basic shape of a recursive descent parser look like?**
  - One function per grammar rule — `expr()`, `term()`, `factor()` — calling each other, with a `peek/advance/expect` cursor that reads tokens one at a time and raises `SyntaxError` at the right point.
- **How are precedence and associativity expressed in code?**
  - Precedence is function call depth (`expr → term → factor`); putting `*` and `/` inside `term()` makes them bind tighter than `+`. Associativity comes from the `while` loop direction — left-associative operators accumulate leftward.
<!-- toc:begin -->
## In this series
