...
이 그림에서는 이력서와 포트폴리오를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> 이력서와 포트폴리오의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **채용 담당자가 짧은 시간 안에 나를 이해하려면 이력서는 어떤 구조여야 할까요?**
  - 본문의 기준은 이력서와 포트폴리오를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **담당 업무 나열이 아니라 결과와 영향 중심으로 쓰려면 무엇이 달라져야 할까요?**
