...
고정소수점의 장점은 **연산이 정수 ALU만으로 가능**하다는 사실입니다. FPU가 없는 마이크로컨트롤러에서 오디오 필터, PID 제어를 구현할 때 필수적인 기법입니다.
...
- **비트, 바이트, 워드는 각각 무엇일까요?**
  - 비트, 바이트, 워드는 각각 무엇일까요 — 본문에서 구체적으로 다룹니다.
- **음수는 왜 2의 보수로 저장할까요?**
  - 음수는 왜 2의 보수로 저장할까요 — 본문에서 구체적으로 다룹니다.
- **IEEE 754 부동소수점은 어떤 구조를 가질까요?**
  - IEEE 754 부동소수점은 어떤 구조를 가질까요 — 본문에서 구체적으로 다룹니다.
- **단계별로 따라가기에서 가장 흔한 실수는 무엇일까요?**
  - 음수의 비트 패턴은 양수와 전혀 다르게 보입니다. 특히 `-1`이 모든 비트가 1이라는 점은 이후 비트 연산을 이해할 때 자주 중요해집니다.
