...
문제는 "최적화"라는 말이 종종 잘못된 직관과 결합한다는 사실입니다. 진짜 병목이 SQL인데 Python을 의심하고, 진짜 병목이 I/O인데 CPU 튜닝부터 시작하는 식입니다. 그래서 측정이 먼저입니다.
...
- **지연시간과 처리량은 어떻게 다를까요?**
  - 지연시간과 처리량은 어떻게 다를까요 — 본문에서 구체적으로 다룹니다.
- **USE 방법론은 병목을 어떻게 찾을까요?**
  - USE 방법론은 병목을 어떻게 찾을까요 — 본문에서 구체적으로 다룹니다.
- **샘플링 프로파일링과 계측은 무엇이 다를까요?**
  - 샘플링 프로파일링과 계측은 무엇이 다를까요 — 본문에서 구체적으로 다룹니다.
- **단계별로 따라가기에서 가장 흔한 실수는 무엇일까요?**
  - 어떤 시스템은 지연시간이 중요하고, 어떤 시스템은 처리량이 더 중요합니다. 먼저 무엇을 개선할지 정해야 합니다.
