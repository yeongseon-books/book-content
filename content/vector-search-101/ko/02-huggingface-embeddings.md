...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/54"
...
- 첫 번째 임베딩을 실무에 적용할 때 주의할 점은 무엇일까요?
- 배치 임베딩의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 sentence-transformers로 만든 벡터가 정말 검색에 쓸 수 있는 형태인지 어디서 확인할, 둘째 한 문장씩 인코딩하는 코드와 배치 인코딩 코드는 운영에서 어떤 차이를 만들, 셋째 벡터를 저장했다가 다시 불러올 때 무엇을 함께 기록해야 나중에 재현할 수 있을입니다. 설치에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **sentence-transformers로 만든 벡터가 정말 검색에 쓸 수 있는 형태인지 어디서 확인할까요?**
  - sentence-transformers로 만든 벡터가 정말 검색에 쓸 수 있는 형태인지 어디서 확인할까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **한 문장씩 인코딩하는 코드와 배치 인코딩 코드는 운영에서 어떤 차이를 만들까요?**
  - 문서가 여러 개라면 `embed_query()`를 반복 호출하는 것보다 `embed_documents()` 한 번이 더 효율적입니다. 모델이 내부적으로 배치 처리하고, 반복 초기화 오버헤드도 줄일 수 있기 때문입니다.
- **벡터를 저장했다가 다시 불러올 때 무엇을 함께 기록해야 나중에 재현할 수 있을까요?**
  - 같은 문서에 대해 실행할 때마다 임베딩을 다시 계산하면 시간이 낭비됩니다. 한 번 계산한 행렬을 저장하고 재사용하는 편이 낫습니다.
