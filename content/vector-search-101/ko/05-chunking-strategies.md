...
- 청크 크기와 오버랩에서 가장 흔한 실수는 무엇일까요?
- 고정 크기 청킹 직접 구현할 때 실무에 적용할 때 주의할 점은 무엇일까요?
- RecursiveCharacterTextSplitter의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 긴 문서를 그대로 임베딩하지 않고 왜 청크로 나눠야 할, 둘째 chunk_size와 overlap은 검색 품질과 비용 사이에서 어떤 균형을 만들, 셋째 청크에 메타데이터를 붙이지 않으면 답변 경로에서 무엇이 막힐입니다. 청크 크기와 오버랩에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **긴 문서를 그대로 임베딩하지 않고 왜 청크로 나눠야 할까요?**
  - 긴 문서를 그대로 임베딩하지 않고 왜 청크로 나눠야 할까요 — 본문에서 구체적으로 다룹니다.
- **chunk_size와 overlap은 검색 품질과 비용 사이에서 어떤 균형을 만들까요?**
  - 청크가 너무 작으면 쿼리와 정확히 매칭될 만큼의 문맥이 없습니다. 반대로 청크가 너무 크면 관련 없는 내용이 섞여 의미 신호가 희석됩니다.
- **청크에 메타데이터를 붙이지 않으면 답변 경로에서 무엇이 막힐까요?**
  - 검색 결과를 사용자에게 보여 주려면 텍스트 조각만 있어서는 부족한 경우가 많습니다. 어느 문서에서 왔는지, 몇 번째 섹션인지, 원문 안에서 어느 위치인지 같은 정보가 함께 있어야 인용과 디버깅이 쉬워집니다.
...
- **고정 크기 청킹 직접 구현할 때 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 먼저 가장 단순한 구현으로 개념을 고정해 보겠습니다.
- **RecursiveCharacterTextSplitter의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - RecursiveCharacterTextSplitter의 핵심 원리를 한 문장으로 설명하면 무엇일까요 — 본문에서 구체적으로 다룹니다.
