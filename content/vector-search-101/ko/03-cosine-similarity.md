...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/55"
...
- 세 가지 거리 척도에서 가장 흔한 실수는 무엇일까요?
- 정규화가 왜 중요한가을 실무에 적용할 때 주의할 점은 무엇일까요?
- 브루트 포스 최근접 이웃 검색의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
다음 글에서는 FAISS를 소개합니다. 인덱스 유형, 인덱스 구축과 저장, 그리고 근사 검색이 작은 정확도 손실로 큰 속도 이득을 만드는 방식을 봅니다.
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 벡터가 있으면 왜 바로 검색이 끝나는 게 아니라 거리 척도를 골라야 할, 둘째 코사인 유사도와 내적, L2 거리는 결과를 어떻게 다르게 만들, 셋째 정규화 여부가 검색 순위와 FAISS 인덱스 선택에 왜 영향을 줄입니다. 세 가지 거리 척도에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
