...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/56"
...
- 설치에서 가장 흔한 실수는 무엇일까요?
- IndexFlatIP로 정확 검색 구현할 때 실무에 적용할 때 주의할 점은 무엇일까요?
- 쿼리 실행하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 벡터가 많아질수록 단순 반복 검색은 어디서 한계가 날, 둘째 IndexFlatIP와 IndexFlatL2는 어떤 전제에서 선택해야 할, 셋째 인덱스를 저장하고 다시 불러올 때 벡터와 메타데이터를 어떻게 맞춰야 할입니다. 설치에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **벡터가 많아질수록 단순 반복 검색은 어디서 한계가 날까요?**
  - 벡터가 많아질수록 단순 반복 검색은 어디서 한계가 날까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **IndexFlatIP와 IndexFlatL2는 어떤 전제에서 선택해야 할까요?**
  - 두 인덱스 모두 올바른 순위를 반환합니다. 텍스트 검색에서는 정규화된 벡터와 함께 `IndexFlatIP`를 쓰는 것이 표준적인 선택입니다.
- **인덱스를 저장하고 다시 불러올 때 벡터와 메타데이터를 어떻게 맞춰야 할까요?**
  - 인덱스를 영속화하면 애플리케이션이 시작될 때마다 문서를 다시 임베딩하지 않아도 됩니다.
- **설치에서 가장 흔한 실수는 무엇일까요?**
  - 설치에서 가장 흔한 실수는 무엇일까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **IndexFlatIP로 정확 검색 구현할 때 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 텍스트 검색에서 가장 표준적인 패턴은 정규화된 벡터와 내적 인덱스를 조합하는 방식입니다.
- **쿼리 실행하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 쿼리 실행하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요에 대해 본문에서 실무 예시와 함께 답합니다.
