...
- FAISS의 IndexFlat, IVF, HNSW는 각각 어떤 상황에서 선택하는 것이 맞을까요?
- 정확 검색(exact search)과 근사 최근접 이웃 검색(ANN) 사이의 정확도-지연 시간 트레이드오프(trade-off)는 어떻게 이해해야 할까요?
- 어떤 인덱스 유형(index type)은 학습(training)이 필요하고, 그 학습은 어떻게 진행할까요?
- FAISS 인덱스를 저장했다가 다시 불러올 때 어떤 함정(gotcha)을 조심해야 할까요?
- 어떤 워크로드(workload)에서는 GPU FAISS가 CPU FAISS보다 유리하고, 또 어떤 경우에는 반대일까요?
