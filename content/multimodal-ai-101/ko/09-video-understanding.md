...
비디오를 LLM/VLM에 어떻게 입력하고, frame sampling을 어떻게 정하나요?

이 글은 그 질문에 답하기 위해 video 이해와 frame sampling의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **Sampling 전략** — uniform vs scene-change vs adaptive를 시나리오에 맞게 선택합니다.
- **토큰 폭발** — 초당 N프레임이 토큰을 폭발시키므로 예산을 먼저 정합니다.
- **시간 정합성** — 타임스탬프와 caption을 함께 보존해 답변 근거로 씁니다.
- **오디오 결합** — STT 결과와 시각 정보를 결합하면 정확도가 크게 오릅니다.
- **Video-LLaVA류** — 전용 모델은 짧은 클립에 강하고, 긴 영상은 chunk 전략이 필요합니다.
