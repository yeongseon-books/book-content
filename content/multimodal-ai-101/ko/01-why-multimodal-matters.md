...
- 첫 번째 실전 예제로 GPT-4o Vision과 CLIP을 보면 어떤 가치가 바로 보일까요?
- 비용, 해상도, 평가, 정책, 개인정보 관점에서 도입 전에 반드시 확인해야 할 함정은 무엇일까요?
...
## Multimodal AI를 이해하는 가장 좋은 방법: 입력 채널이 아니라 추론 경계를 바꾸는 기술로 보는 것입니다
...
![Multimodal AI의 핵심: 입력 채널이 아닌 추론 경계의 변화](https://yeongseon-books.github.io/book-public-assets/assets/multimodal-ai-101/01/01-01-mental-model-multimodal-expands-the-reas.ko.png)
*멀티모달 파이프라인의 핵심은 입력을 너무 일찍 텍스트로 압축하지 않고, 시각 증거를 검색과 추론 단계까지 보존하는 데 있습니다.*
...
핵심은 비전 인코더가 출력한 시각 토큰을 LLM이 추가 텍스트 토큰처럼 처리한다는 점입니다. 그래서 LLaVA류 모델은 GPT-4V에 가까운 추론 능력을 보여 주면서도, 기반 LLM 학습은 거의 그대로 유지할 수 있습니다.
...
초반에 꼭 가져가야 할 감각은 단 하나입니다. 멀티모달을 붙이는 순간 비용, 평가, 정책, 개인정보 처리까지 함께 설계해야 한다는 점입니다. 그 현실을 먼저 받아들이면 기술 선택이 훨씬 안정적이 됩니다.
