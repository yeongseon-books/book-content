...
- adapter 구조가 단순할수록 좋은 경우와, token compression이 반드시 필요한 경우는 언제일까요?
- fine-tuning, multilingual, benchmark 선택에서 자주 과소평가되는 리스크는 무엇일까요?
...
## VLM 아키텍처를 이해하는 가장 좋은 방법: LLM에 새로운 감각을 더하는 것이 아니라 토큰 계약을 다시 쓰는 일로 보는 것입니다
