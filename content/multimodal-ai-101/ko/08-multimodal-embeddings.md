...
Cross-modal 검색을 위한 multimodal embedding은 어떻게 설계하나요?

이 글은 그 질문에 답하기 위해 multimodal embedding과 cross-modal 검색의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **같은 공간 가정** — 텍스트·이미지가 동일 공간에 있어야 cross-modal 검색이 의미를 가집니다.
- **정규화/거리 함수** — cosine 가정과 normalize 누락이 가장 흔한 버그입니다.
- **멀티벡터 옵션** — 단일 임베딩이 부족하면 멀티벡터/late interaction을 검토합니다.
- **도메인 미세조정** — 범용 모델로 부족하면 contrastive 미세조정이 가장 가성비입니다.
- **평가 셋** — cross-modal recall@k를 도메인 셋으로 따로 측정합니다.
