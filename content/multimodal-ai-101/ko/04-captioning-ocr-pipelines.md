...
캡셔닝과 OCR 파이프라인은 어떻게 조합해야 안정적으로 운영되나요?

이 글은 그 질문에 답하기 위해 captioning과 OCR 파이프라인의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **OCR 먼저 vs VLM 먼저** — 정형 문서는 OCR 선처리, 자연 이미지는 VLM 직접 호출이 보통 유리합니다.
- **후처리 정합성** — 좌표/언어/회전 메타를 같이 저장해야 다운스트림이 신뢰할 수 있습니다.
- **언어 혼합 처리** — 한국어/영어 혼합 문서를 한 엔진으로 강제하지 않습니다.
- **환각 검증** — VLM 캡션은 OCR 결과·룰과 교차 검증해 사실성을 확인합니다.
- **배치 비용** — OCR은 배치 처리, VLM은 sampling 전략으로 비용을 통제합니다.
