---
title: "Multimodal AI 101 (2/10): Image Encoder: CLIP과 ViT"
...
![Multimodal AI 101 2장 흐름 개요](https://yeongseon-books.github.io/book-public-assets/assets/multimodal-ai-101/02/02-01-big-picture.ko.png)
...
ViT와 CLIP을 볼 때 가장 먼저 가져가야 할 관점은 “이미지를 읽는다”가 아닙니다. 더 정확한 표현은 이미지를 토큰 또는 임베딩으로 재표현한다는 사실입니다. 이 재표현 덕분에 트랜스포머는 이미지를 텍스트와 비슷한 계산 틀 안에서 다룰 수 있습니다.
...
CLIP의 강점은 학습 없이 새 클래스를 추가할 수 있다는 사실입니다. text encoder에 prompt를 넣어 class vector를 만들고, image vector와 cosine similarity를 계산합니다.
...
## 실전 벤치마크 설계: CLIP 인코더를 운영 기준으로 검증하기

모델 카드의 리더보드 점수만으로는 팀의 실제 입력 분포를 설명하기 어렵습니다. 따라서 인코더 교체 전에는 최소한 세 가지 축으로 벤치마크를 분리해야 합니다. 첫째는 제품 카탈로그나 문서 스크린샷 같은 도메인 데이터에서의 Recall@K입니다. 둘째는 전처리 변형(해상도, crop, 압축률) 내구성입니다. 셋째는 latency와 GPU 메모리 사용량입니다. 이 세 값을 함께 보면 같은 정확도라도 운영에서 어떤 모델이 더 안전한지 판단할 수 있습니다.
...
def evaluate_rankings(rankings: list[list[int]], gold: list[int]) -> RetrievalMetrics:
    r1 = sum(1 for r, g in zip(rankings, gold) if r and r[0] == g) / len(gold)
    r5 = sum(1 for r, g in zip(rankings, gold) if g in r[:5]) / len(gold)
    rr = []
    for r, g in zip(rankings, gold):
        rr.append(1.0 / (r.index(g) + 1) if g in r else 0.0)
    return RetrievalMetrics(recall_at_1=r1, recall_at_5=r5, mrr=float(np.mean(rr)))
```

실제 운영에서는 이미지 인코더를 바꾸면 분류 모델뿐 아니라 검색, 추천, 중복 탐지 파이프라인이 동시에 영향을 받습니다. 그래서 `encoder_version`을 요청 로그에 남기고, 같은 질의셋을 두 버전에 동시에 태워 A/B 비교하는 절차가 필요합니다. 이 기준이 없으면 품질 하락이 생겨도 원인을 빠르게 좁히기 어렵습니다.

## Vision API와 로컬 인코더를 함께 쓰는 패턴

CLIP은 빠른 후보 검색에, GPT-4V나 Claude는 고난도 판별에 배치하는 구조가 비용 대비 효율이 좋습니다. 예를 들어 상품 이미지 검색에서 먼저 CLIP으로 top-20을 뽑고, 그다음 Vision API로 상위 3개만 rerank하면 품질은 높이고 호출 비용은 제한할 수 있습니다.
...
def rerank_with_vlms(image_url: str, candidates: list[str]) -> dict:
    prompt = (
        "Given the query image and candidate product names, "
        "rank top 3 by visual match and explain mismatches briefly."
    )
...
".join(candidates)},
    ...
    return {
        "gpt4v": oai.choices[0].message.content,
        "claude": claude.content[0].text,
    }
```

이 방식의 핵심은 역할 분리입니다. CLIP은 넓은 후보군을 빠르게 좁히는 도구이고, VLM API는 최종 판정기를 맡습니다. 두 계층을 섞지 않으면 트래픽이 늘어도 비용 구조를 예측하기 쉽습니다.

## 도메인 전이에서 자주 발생하는 실패 사례

CLIP 인코더는 일반 웹 이미지에 강하지만, 산업 도메인에서는 같은 레이블도 시각적 분포가 크게 다릅니다. 예를 들어 제조 라인의 불량 이미지, 의료 스캔, 위성 이미지처럼 색상과 질감이 일반 사진과 다르면 zero-shot 성능이 급격히 내려갈 수 있습니다. 이런 경우에는 backbone 전체를 무리하게 미세조정하기보다, 소량 라벨로 linear probe를 먼저 올리고 실패 케이스를 수집하는 순서가 안전합니다.
...
또한 프로덕션에서는 "모델 교체"보다 "데이터 드리프트 감지"가 먼저 필요합니다. 임베딩 평균 벡터나 상위 검색 score 분포가 일정 범위를 벗어나면 경고를 띄우는 방식이 실용적입니다.

## 실전 운영 시나리오: 검색 장애를 줄이는 롤백 전략

인코더를 교체한 직후 검색 품질이 떨어지면 가장 먼저 해야 할 일은 모델 자체를 의심하는 것이 아니라, 인덱스와 전처리 계약이 같은 버전으로 배포되었는지 확인하는 것입니다. 실제 장애의 상당수는 모델보다 배포 순서 문제에서 시작합니다. 예를 들어 추론 서버는 새 인코더로 올라갔는데 인덱스는 이전 버전을 유지하면, 질의 벡터와 문서 벡터가 다른 공간에 있어 점수가 무의미해집니다.

운영에서는 이를 막기 위해 encoder-version gate를 둡니다. 요청 시점에 질의 인코더 버전과 인덱스 버전을 비교해 다르면 즉시 차단하거나 구버전 경로로 우회하는 방식입니다. 이 가드레일 하나로 대규모 품질 사고를 대부분 예방할 수 있습니다.

```python
def validate_embedding_contract(query_encoder: str, index_encoder: str) -> None:
    if query_encoder != index_encoder:
        raise RuntimeError(
            f"embedding contract mismatch: query={query_encoder}, index={index_encoder}"
        )
```

또한 롤백은 코드 롤백보다 인덱스 롤백이 더 오래 걸리는 경우가 많습니다. 따라서 새 인덱스를 완전 전환하기 전에 최소 24시간 shadow 비교를 유지하고, 결과가 기준 이하일 때는 라우팅만 즉시 되돌릴 수 있게 준비해야 합니다.
...
- **왜 멀티모달 입문에서 image encoder부터 이해하는 편이 전체 구조를 가장 빠르게 잡게 해 줄까요?**
  - 왜 멀티모달 입문에서 image encoder부터 이해하는 편이 전체 구조를 가장 빠르게 잡게 해 줄까요 — 본문에서 구체적으로 다룹니다.
- **ViT는 이미지를 어떤 방식으로 token sequence로 바꾸고, CNN과 무엇이 다를까요?**
  - ViT는 이미지를 어떤 방식으로 token sequence로 바꾸고, CNN과 무엇이 다를까요 — 본문에서 구체적으로 다룹니다.
- **CLIP은 어떻게 텍스트와 이미지를 같은 embedding space에 맞추고 zero-shot을 가능하게 할까요?**
  - CLIP은 어떻게 텍스트와 이미지를 같은 embedding space에 맞추고 zero-shot을 가능하게 할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 이미지 encoder를 이해하면 멀티모달 시스템의 절반이 정리됩니다. 이미지가 어떤 형태의 벡터가 되는지 알아야 그 벡터를 검색에 쓸지, 분류에 쓸지, VLM adapter의 입력으로 넣을지 설계할 수 있기 때문입니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - ViT와 CLIP을 볼 때 가장 먼저 가져가야 할 관점은 “이미지를 읽는다”가 아닙니다. 더 정확한 표현은 이미지를 토큰 또는 임베딩으로 재표현한다는 사실입니다. 이 재표현 덕분에 트랜스포머는 이미지를 텍스트와 비슷한 계산 틀 안에서 다룰 수 있습니다.
...
- **Multimodal AI 101 (2/10): Image Encoder: CLIP과 ViT (현재 글)**
