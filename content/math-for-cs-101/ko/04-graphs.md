...
- 한 장으로 보는 그래프 탐색을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다섯 단계로 보는 그래프 기초의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
그래프를 처음 배울 때 가장 도움이 되는 문장은 이것입니다. **그래프는 값을 저장하는 구조가 아니라 관계를 드러내는 구조**라는 사실입니다. 정점은 대상이고, 간선은 대상 사이의 연결입니다. 방향이 있느냐 없느냐에 따라 의미도 크게 달라집니다.
...
- **관계가 있는 데이터를 왜 그래프로 표현할까요?**
  - 그래프는 값 목록으로는 숨겨지는 연결 구조를 정점과 간선으로 드러내기 때문에 관계 중심 데이터를 가장 자연스럽게 표현합니다. 소셜 관계, 서비스 호출, 빌드 의존성을 `G = {"A": ["B", "C"] ...}` 같은 인접 리스트로 바꾸면 BFS, DFS, 최단 경로, 위상 정렬처럼 바로 적용할 수 있는 해법이 열립니다.
- **정점과 간선은 실무 모델에서 각각 무엇에 해당할까요?**
  - 이 글의 응용 표처럼 정점은 사용자, 서비스, 작업, 위치 같은 대상이고 간선은 친구 관계, 호출 의존성, 선행 조건, 도로 연결 같은 관계입니다. 그래서 `topological_sort(build_deps)`는 작업 간 선행 관계를, `dijkstra(network, "gateway")`는 서비스 간 비용 경로를 계산하는 식으로 같은 문법이 여러 도메인에 재사용됩니다.
- **방향 그래프와 무방향 그래프는 어떻게 다를까요?**
  - 방향이 있으면 `api -> auth`나 `app -> lib`처럼 관계의 순서와 의존성이 의미를 가지므로 위상 정렬과 사이클 탐지가 가능해집니다. 반대로 친구 관계나 양방향 도로처럼 대칭 연결이면 무방향 그래프로 읽는 편이 자연스럽고, 이 구분을 놓치면 BFS 거리, DAG 판정, 장애 전파 해석이 모두 달라집니다.
