...
- 한 장으로 보는 정보이론을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다섯 단계로 보는 정보이론 기초의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **정보의 양은 무엇으로 측정할까요?**
  - 이 글은 사건 하나의 정보량을 `-log2(p)`로 정의해 비트 단위로 측정했습니다. 그래서 `info(p)`에서 확률이 작은 사건일수록 값이 커지고, 드문 사건이 더 놀랍다는 직관을 숫자로 바로 표현할 수 있습니다.
- **엔트로피는 왜 평균 정보량이라고 부를까요?**
  - 엔트로피는 각 사건의 정보량에 그 사건이 일어날 확률을 곱해 더한 값이라서 분포 전체의 평균 놀라움을 나타냅니다. 공정한 동전과 편향 동전의 `entropy([0.5, 0.5])`, `entropy([0.9, 0.1])` 비교와 허프만·zlib 압축 실험은 엔트로피가 곧 평균 부호 길이의 하한과 연결된다는 점을 보여 줍니다.
- **교차 엔트로피는 왜 손실 함수로 자주 쓰일까요?**
  - 교차 엔트로피는 실제 분포 `p`를 모델 분포 `q`로 설명할 때 추가로 드는 평균 비용이라서 예측이 틀릴수록 손실이 커집니다. 본문에서 정리한 `H(p, q) = H(p) + KL(p||q)` 관계와 `target`, `model_good`, `model_bad` 예시는 교차 엔트로피 최소화가 결국 KL 발산을 줄여 모델 분포를 실제 분포에 가깝게 만드는 과정임을 설명합니다.
