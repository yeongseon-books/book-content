...
- 한 장으로 보는 경우의 수 구조을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다섯 단계로 보는 셈의 기초의 핵심 원리를 한 문장으로 설명하면 무엇일까요?

알고리즘이 왜 느려지는지 설명하려면 가능한 경우의 수가 얼마나 빨리 늘어나는지 알아야 합니다. 해시 충돌 가능성을 가늠할 때도, 테스트 케이스 수를 정할 때도, 샘플링 전략을 세울 때도 결국 셈이 필요합니다.

조합론의 장점은 모든 경우를 손으로 나열하지 않아도 된다는 사실입니다. 순서가 중요한지, 중복을 허용하는지, 선택이 서로 배타적인지 같은 구조만 파악하면 공식과 원리로 훨씬 빠르게 판단할 수 있습니다. 그래서 조합론은 계산 기술이면서 동시에 모델링 기술이기도 합니다.
...
- **모든 경우를 직접 나열하지 않고도 왜 정확히 셀 수 있을까요?**
  - 조합론은 결과를 하나씩 적는 대신 선택 구조를 분해해 셉니다. `multiplication_rule`, `addition_rule`, `npr`, `ncr`로 독립 선택·배타 선택·순서 중요 여부를 분리하고, 작은 입력에서는 `itertools.permutations`와 `itertools.combinations`로 공식이 열거 결과와 맞는지 다시 검증했습니다.
- **곱의 법칙과 합의 법칙은 언제 각각 쓰일까요?**
  - 각 자리가 독립인 비밀번호처럼 연속 선택이 누적되면 곱의 법칙을 씁니다. 반대로 로그인 실패 원인이 비밀번호 오류 또는 계정 잠금처럼 동시에 성립하지 않는 범주라면 합의 법칙으로 더해야 하며, 이 차이를 먼저 잡아야 경우의 수 모델이 흔들리지 않습니다.
- **순열과 조합은 무엇이 다를까요?**
  - 순열은 `nPr(n, r)`처럼 순서가 바뀌면 다른 경우로 세고, 조합은 `nCr(n, r)`처럼 고르는 집합만 같으면 같은 경우로 셉니다. 팀 선발, feature 선택, 생일 문제, 토큰 강도 계산에서 무엇을 배열로 볼지 선택으로 볼지 먼저 정해야 올바른 공식을 고를 수 있습니다.
