...
- 머릿속에 먼저 둘 관점에서 가장 흔한 실수는 무엇일까요?
- 한 장으로 보는 선형대수의 기본 구조을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다섯 단계로 보는 선형대수 기초의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
선형대수를 처음 배울 때 가장 도움이 되는 문장은 이것입니다. **벡터는 데이터이고, 행렬은 그 데이터를 움직이는 변환**이라는 사실입니다. 이 관점만 잡아도 많은 식이 훨씬 덜 위협적으로 보입니다.
...
PCA의 핵심은 공분산 행렬의 고유벡터가 분산 최대 방향을 나타낸다는 사실입니다. 고유값이 크면 그 축이 정보를 많이 담고 있다는 뜻입니다. 임베딩 시각화, 노이즈 제거, 특징 선택 등에 널리 쓰입니다. 실무에서는 `sklearn.decomposition.PCA`를 쓰지만, 내부 원리를 이해하면 `n_components` 선택과 설명력(설명된 분산 비율) 해석을 더 잘할 수 있습니다.

### 코사인 유사도와 임베딩 검색

벡터 간 유사도 측정은 추천 시스템, 검색 엔진, RAG 파이프라인의 기본입니다.
...
- **벡터와 행렬은 데이터를 어떻게 표현할까요?**
  - 이 글은 벡터를 하나의 데이터 객체, 행렬을 그 데이터를 움직이는 변환으로 설명했습니다. `A @ v`, 회전 행렬 `rotation(theta) @ p`, PCA의 `X_centered @ W`, 희소 행렬 `A_sparse @ x` 예시는 숫자 표가 아니라 데이터와 변환의 계약으로 읽어야 의미가 또렷해진다는 점을 보여 줍니다.
- **내적은 왜 유사도 계산의 핵심 연산일까요?**
  - 내적은 두 벡터가 같은 방향을 얼마나 보고 있는지 요약하므로 추천 점수와 임베딩 비교의 중심 연산이 됩니다. 특히 `cosine_similarity`와 `batch_cosine_similarity`처럼 norm으로 정규화하면 크기 차이를 걷어 내고 방향만 비교할 수 있어 검색과 추천에 바로 쓰기 좋습니다.
- **행렬곱은 단순 반복 계산과 무엇이 다를까요?**
  - 행렬곱은 반복문을 줄인 계산 요령이 아니라 변환을 합성하는 수학적 연산입니다. `matmul(A, B)`와 `A @ B`는 한 변환 뒤에 다른 변환을 붙이는 의미를 가지므로, `B @ A`와 다를 수 있고 차원·축 해석·전치 여부를 함께 확인해야 올바른 모델이 됩니다.
