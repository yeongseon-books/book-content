...
- 한 장으로 보는 확률의 흐름을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다섯 단계로 보는 확률 기초의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
확률을 이해할 때 가장 먼저 잡아야 할 문장은 이것입니다. **확률은 숫자 하나가 아니라, 가능한 결과와 조건을 함께 다루는 구조**라는 사실입니다. 표본공간은 가능한 모든 결과의 집합이고, 사건은 그 안에서 우리가 관심 있는 부분집합입니다.
...
여기서 핵심은 양성 판정 정확도가 높아도 유병률(사전 확률)이 낮으면 사후 확률이 생각보다 낮을 수 있다는 사실입니다. 숫자 해석에서 가장 흔한 오해를 바로잡는 지점입니다.
...
- **불확실성을 감으로 넘기지 않고 어떻게 수치로 다룰까요?**
  - 이 글은 기본 확률 `prob`, 조건부 확률 `cond`, 베이즈 갱신 `bayes`, 그리고 `expect`와 `variance`로 평균과 흔들림을 분리해 불확실성을 숫자로 바꾸었습니다. 여기에 `decide_action(p_risk, threshold)`처럼 임계값 정책까지 붙이면 막연한 감이 아니라 비교 가능한 운영 판단으로 연결됩니다.
- **표본공간과 사건은 무엇이 다를까요?**
  - 표본공간은 가능한 모든 결과의 집합이고, 사건은 그중에서 우리가 관심 있는 부분집합입니다. 질병 검사 예시에서는 전체 가능한 상태가 표본공간이고, "양성이면서 실제 질병" 같은 조합이 사건이므로 무엇을 분모로 잡는지에 따라 같은 숫자도 전혀 다른 뜻을 갖습니다.
- **조건부 확률은 왜 문맥을 붙인 확률이라고 할 수 있을까요?**
  - 조건부 확률은 이미 어떤 정보가 주어진 뒤 남은 세계만 다시 보는 계산이기 때문에 문맥이 붙은 확률입니다. `false_positive_scenario()`와 베이즈 진단 예제에서 알 수 있듯이, 알람이 울렸다는 조건이 붙는 순간 분모가 바뀌고 `P(장애 | 알람)`은 단순 장애율과 전혀 다른 값이 됩니다.
