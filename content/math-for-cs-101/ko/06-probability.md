...
- **불확실성을 감으로 넘기지 않고 어떻게 수치로 다룰까요?**
  - 불확실성을 감으로 넘기지 않고 어떻게 수치로 다룰까요 — 본문에서 구체적으로 다룹니다.
- **표본공간과 사건은 무엇이 다를까요?**
  - 표본공간과 사건은 무엇이 다를까요 — 본문에서 구체적으로 다룹니다.
- **조건부 확률은 왜 문맥을 붙인 확률이라고 할 수 있을까요?**
  - 조건부 확률은 왜 문맥을 붙인 확률이라고 할 수 있을까요 — 본문에서 구체적으로 다룹니다.
- **머릿속에 먼저 둘 관점에서 가장 흔한 실수는 무엇일까요?**
  - 확률을 이해할 때 가장 먼저 잡아야 할 문장은 이것입니다. **확률은 숫자 하나가 아니라, 가능한 결과와 조건을 함께 다루는 구조**라는 사실입니다. 표본공간은 가능한 모든 결과의 집합이고, 사건은 그 안에서 우리가 관심 있는 부분집합입니다.
- **한 장으로 보는 확률의 흐름을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 스팸 필터는 베이즈 관점으로 점수를 갱신하고, 추천 시스템은 클릭 확률을 추정합니다. SRE 영역에서는 장애 발생 확률과 SLA 위반 가능성을 봅니다.
- **다섯 단계로 보는 확률 기초의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 가장 기본 형태입니다. 관심 있는 경우 수를 전체 경우 수로 나누는 사고는 이후 모든 확률 계산의 출발점이 됩니다. 단순해 보여도 어떤 경우를 전체로 볼지 먼저 정해야 한다는 점이 중요합니다.
