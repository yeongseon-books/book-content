...
이 그림에서는 HPA와 Cluster Autoscaler 내부 — 두 컨트롤 루프를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.
...
- **HPA는 어떤 메트릭을 어떤 주기로 읽고 desired replica를 계산할까요?**
  - 본문의 기준은 HPA와 Cluster Autoscaler 내부 — 두 컨트롤 루프를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **Cluster Autoscaler는 어떤 신호를 보고 “새 노드가 필요하다”고 판단할까요?**
