...
정밀도와 재현율을 임계값 메모로 읽고 나면, 팀은 곧바로 “그래서 하나의 숫자로 비교하면 무엇을 쓰면 되나요?”라고 묻습니다. F1은 바로 그 요구에서 자주 등장합니다. 문제는 기존 글의 이진 분류 예제가 `ko/05-f1-score.md:109-118`에서 **학습한 같은 데이터 위에서 임계값을 훑어 F1을 최대화하는 낙관적 패턴**을 가르쳤다는 데 있습니다.
...
- 테스트셋에서 F1만 보면 0.20이 좋아 보이는데, 왜 운영팀은 0.50도 함께 검토해야 하나요?
- 이 글이 답하는 질문에서 가장 흔한 실수는 무엇일까요?
- 한눈에 보는 멘탈 모델을 실무에 적용할 때 주의할 점은 무엇일까요?
- 1부 — 평균 방식이 다르면 같은 예측도 다른 점수가 됩니다의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 한눈에 보는 멘탈 모델을 실무에 적용할 때 주의할 점은 무엇인지 이해했다
...
- **`micro 0.927`, `macro 0.881`, `weighted 0.925`가 같은 예측인데 왜 서로 다른 결론을 주나요?**
  - 세 점수는 같은 예측을 서로 다른 질문으로 요약하기 때문입니다. 본문처럼 micro와 weighted는 다수 클래스의 비중을 더 많이 반영해 높게 나오고, macro는 소수 클래스도 동등하게 세므로 세 번째 클래스의 약점 `per class [0.952, 0.923, 0.768]`이 더 강하게 드러납니다.
- **검증셋에서 최고 F1을 준 임계값 0.20을 테스트셋에 잠가 보는 절차가 왜 중요한가요?**
  - 이 절차가 있어야 임계값 선택과 최종 평가가 섞이지 않고, 검증셋에서 고른 정책이 새 데이터에서도 유지되는지 확인할 수 있습니다. 실제 예제에서도 검증셋 최고점 0.20을 테스트셋에 그대로 적용해 `F1 0.627`, `precision 0.527`, `recall 0.775`를 확인함으로써 낙관적 패턴을 피했습니다.
- **테스트셋에서 F1만 보면 0.20이 좋아 보이는데, 왜 운영팀은 0.50도 함께 검토해야 하나요?**
  - 0.20은 재현율 0.775를 주지만 정밀도가 0.527이라 경보의 거의 절반이 오탐이 될 수 있습니다. 반면 0.50은 `F1 0.490`으로 낮아도 정밀도 0.717을 제공하므로, 리뷰 팀의 false positive 예산이 빡빡한 운영에서는 더 현실적인 후보가 됩니다.
