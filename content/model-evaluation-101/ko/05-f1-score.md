...
- **F1 점수를 운영 관점에서 볼 때 먼저 어떤 경계를 확인해야 할까요?**
  - 먼저 볼 경계는 어떤 평균 방식으로 F1을 계산했는지와, 임계값을 어느 데이터에서 골랐는지입니다. 본문도 micro 0.927, macro 0.881, weighted 0.925가 서로 다른 질문에 답하고, 임계값 선택은 반드시 train·validation·test를 분리해야 한다고 정리했습니다.
- **F1 점수에서 예제나 다이어그램으로 검증해야 할 핵심 신호는 무엇일까요?**
  - 다중 분류 예제에서는 클래스별 F1이 `[0.952, 0.923, 0.768]`로 갈라져 세 번째 클래스가 약점임을 보여 줍니다. 이진 분류 예제에서는 validation sweep으로 0.20이 최고 F1을 주지만, 테스트에 잠갔을 때의 정밀도·재현율까지 함께 봐야 실제 의미가 드러납니다.
- **F1 점수를 실제 시스템에 적용할 때 어떤 실패를 먼저 막아야 할까요?**
  - 학습에 쓴 같은 데이터에서 임계값까지 최적화해 낙관적인 F1을 얻는 실패를 먼저 막아야 합니다. 또한 최고 F1 임계값이 곧 운영 최적점이라고 단정하지 말고, false positive 예산과 놓침 비용을 별도로 비교해야 합니다.
