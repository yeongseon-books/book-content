...
리더보드 한 줄에는 F1이 자주 등장합니다. 하지만 어떤 F1인지 밝히지 않으면 서로 비교할 수 없습니다.
...
1. **평균 종류를 명시하지 않습니다.**
2. **Macro F1을 불균형 데이터에서 오해합니다.**
3. ***F1* 만 보고 *Precision/Recall* 분리 무시.**
4. **F-beta의 β를 임의로 선택합니다.**
5. **임계값별 F1을 그래프로 보지 않습니다.**

## 실무에서는 이렇게 쓰입니다

경진대회와 리더보드에서는 단일 숫자 비교에 F1이 자주 등장합니다. 하지만 프로덕션에서는 클래스별 결과가 더 중요합니다.
...
F1은 요약일 뿐 진단 도구는 아닙니다. 다음 글은 ROC와 AUC로 임계값 무관 평가를 다룹니다.
