...
- 오류 분석을 읽는 방식의 전환의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 전체 점수가 비슷한 두 모델은 어디에서 다르게 실패할 수 있다
- [ ] 슬라이스 분석은 어떤 약점을 드러내 주는지 알고 있다
- [ ] false positive와 false negative를 왜 나눠 봐야 할 수 있다
...
- **전체 점수가 비슷한 두 모델은 어디에서 다르게 실패할까요?**
  - 전체 평균이 비슷해도 특정 슬라이스, 특정 오류 유형, 특정 확신도 구간에서 완전히 다르게 무너질 수 있습니다. 그래서 본문은 점수 하나로 끝내지 않고 `Xte[:, 0] > 0` 같은 슬라이스와 FP·FN 분리를 통해 약점을 드러냈습니다.
- **슬라이스 분석은 어떤 약점을 드러내 줄까요?**
  - 슬라이스 분석은 전체 F1이 가리는 구간별 성능 저하를 보여 줍니다. 본문 예제처럼 `slice +`와 `slice -` 점수를 나란히 놓으면 어느 조건에서 모델이 유난히 약한지 보이고, 그 결과가 데이터 추가 수집이나 특성 개선의 우선순위가 됩니다.
- **false positive와 false negative를 왜 나눠 봐야 할까요?**
  - 두 오류는 운영 비용과 처방이 다르기 때문입니다. 본문도 FP는 임계값 조정이나 규칙 강화와 연결되고, FN은 놓침 비용, 데이터 부족, 라벨 문제를 의심하게 하므로 한 숫자로 합치면 다음 실험 방향이 흐려진다고 설명했습니다.
