...
- 리포트를 읽는 방식의 전환의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **모델 배포 전에 어떤 평가 정보를 한곳에 모아야 할까요?**
  - 본문 기준으로는 전체 지표, 슬라이스 점수, 운영 임계값, 재현성 메타데이터, 알려진 리스크가 함께 있어야 합니다. 예제 코드도 `metrics`, `slices`, `meta`, `risks`를 한 JSON에 모은 뒤 마크다운으로 렌더링해 리포트의 최소 골격을 만들었습니다.
- **평가 리포트와 Model Card는 무엇이 다를까요?**
  - Model Card는 모델의 의도와 한계를 폭넓게 설명하는 문서이고, 평가 리포트는 특정 실험과 배포 판단을 뒷받침하는 좁고 단단한 운영 문서입니다. 그래서 이 글은 점수 설명에 그치지 않고 데이터 해시, sklearn 버전, 임계값 0.5 같은 재현성 정보까지 함께 남기라고 요구합니다.
- **임계값, 슬라이스, 재현성 정보는 왜 빠지면 안 될까요?**
  - 같은 `F1 0.81`이라도 어느 임계값에서 나왔는지 모르면 숫자의 의미가 달라지고, 슬라이스 정보가 없으면 약한 구간이 숨습니다. 또한 `data_hash`, Python·scikit-learn 버전 같은 메타데이터가 빠지면 나중에 같은 결과를 다시 만들 수 없어 배포 판단의 근거가 약해집니다.
