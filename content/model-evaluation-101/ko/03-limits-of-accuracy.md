...
- **정확도의 한계를 운영 관점에서 볼 때 먼저 어떤 경계를 확인해야 할까요?**
  - 이 글은 정확도를 바로 읽지 말고 베이스레이트, 더미 기준선, 소수 클래스 재현율, 균형 정확도 순서로 확인하라고 제안했습니다. 실제 예제에서도 `accuracy 0.9608`보다 `base rate 0.0468`과 `dummy accuracy 0.9536`이 먼저 해석 기준을 만들어 줍니다.
- **정확도의 한계에서 예제나 다이어그램으로 검증해야 할 핵심 신호는 무엇일까요?**
  - 핵심 신호는 더미 대비 정확도 이득이 얼마나 작은지, 그리고 그 대신 소수 클래스를 얼마나 놓치는지입니다. 본문 수치처럼 양성 58개 중 47개를 놓쳐 `minority recall 0.1897`, `balanced accuracy 0.5940`이 나온다면 높은 정확도는 착시에 가깝습니다.
- **정확도의 한계를 실제 시스템에 적용할 때 어떤 실패를 먼저 막아야 할까요?**
  - 다수 클래스를 편하게 맞춘 결과를 성능 향상으로 오해하는 실패를 먼저 막아야 합니다. 그래서 보고서 첫 줄에도 정확도만 쓰지 말고 더미 대비 개선 폭, 소수 클래스 재현율, 균형 정확도를 같은 문단에 함께 남겨야 합니다.
