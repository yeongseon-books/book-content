...
- 이번 글의 판단 순서을 실무에 적용할 때 주의할 점은 무엇일까요?
...
- [ ] 더미 기준선 95.36%와 모델 정확도 96.08%가 거의 비슷한데, 왜 이 모델을 바로 좋다고 말하면 안 될까요
- [ ] 혼동 행렬에서 양성 58개 중 47개를 놓쳤다는 사실이 정확도 96.08%보다 더 중요한 이유는 무엇인지 이해했다
- [ ] `minority recall 0.1897`과 `balanced accuracy 0.5940`를 함께 보면 보고서 첫 줄에 어떤 숫자를 올려야 할 수 있다
- [ ] 이 글이 답하는 질문에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] 이번 글의 판단 순서을 실무에 적용할 때 주의할 점은 무엇인지 이해했다
...
- **`minority recall 0.1897`과 `balanced accuracy 0.5940`를 함께 보면 보고서 첫 줄에 어떤 숫자를 올려야 할까요?**
  - 양성 비율이 4.68%라는 뜻은, 아무 생각 없이 대부분을 음성으로 찍어도 높은 정확도를 얻기 쉽다는 의미입니다. 즉 이 문제에서 정확도는 모델 실력보다 데이터 분포의 도움을 많이 받습니다.
- **이 글이 답하는 질문에서 가장 흔한 실수는 무엇일까요?**
  - - 더미 기준선보다 약간 높은 정확도는 언제 의미가 약할까요? - 소수 클래스 재현율과 균형 정확도는 정확도의 착시를 어떻게 벗겨 낼까요? - 최종 보고서에서 정확도를 대표 숫자로 써도 되는지 어떻게 판단할까요?
- **이번 글의 판단 순서을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 정확도 해석은 다음 다섯 단계로 고정해 두는 편이 안전합니다.
- **한눈에 보는 멘탈 모델의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 핵심은 간단합니다. 클래스 비율이 크게 기울어져 있으면 정확도는 바로 읽는 숫자가 아니라, 다른 점검을 마친 뒤에만 참고할 수 있는 숫자입니다.
