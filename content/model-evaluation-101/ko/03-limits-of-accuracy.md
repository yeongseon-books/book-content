...
- **`minority recall 0.1897`과 `balanced accuracy 0.5940`를 함께 보면 보고서 첫 줄에 어떤 숫자를 올려야 할까요?**
  - 이 글의 결론은 `accuracy=0.9608`보다 `minority recall=0.1897`과 `balanced accuracy=0.5940`를 먼저 올려야 한다는 점입니다. 정확도는 더미 대비 개선 폭 0.72%포인트와 함께 보조 숫자로 남기고, 핵심 판단은 양성 놓침 위험이 큰 모델이라는 문장으로 적는 편이 맞습니다.
