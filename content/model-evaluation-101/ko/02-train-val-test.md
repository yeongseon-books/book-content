...
- 분할을 잘못 볼 때와 제대로 볼 때의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] train, validation, test는 각각 무엇을 맡아야 할 수 있다
- [ ] 왜 validation과 test를 같은 용도로 쓰면 안 될까요
- [ ] 데이터 누수는 어떤 경로로 가장 자주 들어올까요
...
- **train, validation, test는 각각 무엇을 맡아야 할까요?**
  - train은 `LogisticRegression`이 파라미터를 학습하는 구간이고, validation은 하이퍼파라미터와 임계값을 고르는 구간이며, test는 마지막에 한 번만 성능을 확인하는 구간입니다. 본문이 `Xtr`, `Xva`, `Xte`를 따로 만든 이유도 각 세트의 책임이 섞이면 점수 해석이 무너지기 때문입니다.
- **왜 validation과 test를 같은 용도로 쓰면 안 될까요?**
  - test를 반복해서 들여다보며 모델을 고르면 그 세트는 더 이상 최종 확인용이 아니라 튜닝용 데이터가 됩니다. 본문은 validation으로 선택을 끝내고 test는 잠가 둬야, 마지막 점수가 처음 보는 데이터에 대한 일반화 성능으로 남는다고 정리했습니다.
- **데이터 누수는 어떤 경로로 가장 자주 들어올까요?**
  - 예제에서는 `StandardScaler().fit(X)`처럼 전체 데이터에 먼저 전처리를 맞추는 경우가 대표적인 누수로 나왔고, 시계열 무작위 분할과 같은 사용자 그룹의 중복 등장도 함께 짚었습니다. 그래서 `TimeSeriesSplit`, `GroupKFold`처럼 데이터 구조에 맞는 분할 방식을 먼저 정하는 것이 핵심입니다.
