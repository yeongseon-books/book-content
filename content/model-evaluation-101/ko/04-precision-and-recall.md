...
- `AP 0.745`가 나왔는데도 배포 임계값을 따로 메모해야 하는 이유는 무엇일까요?
- 이 글이 답하는 질문에서 가장 흔한 실수는 무엇일까요?
- 운영 시나리오 먼저 정하겠습니다을 실무에 적용할 때 주의할 점은 무엇일까요?
...
- [ ] 임계값을 0.20에서 0.70으로 올리면 리뷰 큐 118건과 42건 사이에서 무엇을 맞바꾸게 될까요
- [ ] 왜 이 글은 0.35를 운영 기본값 후보로 두고 0.20과 0.50을 함께 비교할 수 있다
- [ ] `AP 0.745`가 나왔는데도 배포 임계값을 따로 메모해야 하는 이유는 무엇인지 이해했다
- [ ] 이 글이 답하는 질문에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] 운영 시나리오 먼저 정하겠습니다을 실무에 적용할 때 주의할 점은 무엇인지 이해했다
...
- **임계값을 0.20에서 0.70으로 올리면 리뷰 큐 118건과 42건 사이에서 무엇을 맞바꾸게 될까요?**
  - 임계값을 올리면 경보는 더 깨끗해지지만 실제 이상 거래를 더 많이 놓치게 됩니다. 본문 수치에서도 0.20은 `precision 0.610`, `recall 0.735`, 큐 118건이고, 0.70은 `precision 0.952`, `recall 0.408`, 큐 42건이라서 리뷰 부담과 놓침 위험을 서로 맞바꾸는 선택임이 드러납니다.
- **왜 이 글은 0.35를 운영 기본값 후보로 두고 0.20과 0.50을 함께 비교할까요?**
  - 0.35는 0.20보다 정밀도를 크게 끌어올리면서도 0.50보다 재현율을 덜 희생하는 지점이기 때문입니다. 실제로 `precision 0.795`, `recall 0.633`, 리뷰 큐 78건이 나와, 경보 품질과 운영 부담을 함께 설명하기 좋은 기본값 후보가 됩니다.
- **`AP 0.745`가 나왔는데도 배포 임계값을 따로 메모해야 하는 이유는 무엇일까요?**
  - AP 0.745는 모델의 전반적인 정밀도-재현율 곡선 품질을 요약할 뿐, 어느 임계값에서 몇 건을 경보로 보내는지는 말해 주지 않습니다. 그래서 배포 판단은 여전히 0.20, 0.35, 0.50, 0.70 같은 후보별 혼동 행렬과 `flagged` 건수를 붙인 운영 메모로 내려야 합니다.
