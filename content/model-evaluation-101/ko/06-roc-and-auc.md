...
- **`ROC-AUC 0.819`와 `PR-AUC 0.463`이 함께 나오면 무엇을 다르게 읽어야 할까요?**
  - 이 데이터의 ROC-AUC는 **0.819**라서 꽤 괜찮아 보입니다. 하지만 PR-AUC는 **0.463**입니다.
- **`FPR <= 0.05` 제약 아래에서 임계값 0.141를 고르면 혼동 행렬과 비용은 어떻게 바뀌나요?**
  - 운영 정책을 `FPR <= 0.05`로 둔다고 가정하면, 이 예제에서 선택되는 임계값은 **0.141**입니다. 이 지점의 결과는 다음과 같습니다.
- **비용만 보면 임계값 0.10이 더 낮을 수도 있는데, 왜 0.141나 다른 정책 선택을 다시 논의해야 하나요?**
  - 예상 결과는 다음과 같습니다.
- **이 글이 답하는 질문에서 가장 흔한 실수는 무엇일까요?**
  - - ROC-AUC와 PR-AUC가 왜 같은 데이터에서 다른 온도로 들릴까요? - `FPR <= 0.05` 같은 정책 제약은 실제 임계값 선택으로 어떻게 연결될까요? - AUC가 괜찮아 보여도 어떤 조건에서 “아직 배포 준비가 안 됨” 결론이 나올까요?
- **이 장의 최종 질문을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 이번 장의 질문은 단순합니다.
- **한눈에 보는 멘탈 모델의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - ROC와 PR은 둘 다 **점수 순위**에서 출발합니다. 하지만 배포는 결국 하나의 임계값에서 일어나므로, 곡선은 마지막에 다시 혼동 행렬과 비용으로 내려와야 합니다.
