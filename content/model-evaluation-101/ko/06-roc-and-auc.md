...
- **ROC와 AUC 이해하기를 운영 관점에서 볼 때 먼저 어떤 경계를 확인해야 할까요?**
  - 먼저 확인할 경계는 곡선 요약에서 멈출 것인지, 아니면 실제 운영 제약 아래의 임계값까지 내려올 것인지입니다. 이 글은 ROC-AUC 0.819와 PR-AUC 0.463을 본 뒤에도 바로 배포 결론을 내리지 않고 `FPR <= 0.05` 조건으로 다시 판단했습니다.
- **ROC와 AUC 이해하기에서 예제나 다이어그램으로 검증해야 할 핵심 신호는 무엇일까요?**
  - 핵심 신호는 선택한 임계값에서의 정밀도, 재현율, 혼동 행렬, 비용입니다. 본문 예제에서는 임계값 0.141에서 `FPR 0.049`, `precision 0.352`, `recall 0.507`, 비용 440이 나와 곡선 한 점이 실제 운영 손해로 번역됐습니다.
- **ROC와 AUC 이해하기를 실제 시스템에 적용할 때 어떤 실패를 먼저 막아야 할까요?**
  - AUC가 괜찮아 보인다는 이유만으로 모델이 배포 준비를 끝냈다고 오해하는 실패를 막아야 합니다. 정책상 `FPR <= 0.05`를 지켜도 재현율이 0.507에 머물면, 본문 결론처럼 모델 개선이나 다른 정책 설계가 먼저일 수 있습니다.
