...
- 비용만 보면 임계값 0.10이 더 낮을 수도 있는데, 왜 0.141나 다른 정책 선택을 다시 논의해야 하나요?
- 이 글이 답하는 질문에서 가장 흔한 실수는 무엇일까요?
- 이 장의 최종 질문을 실무에 적용할 때 주의할 점은 무엇일까요?
- 한눈에 보는 멘탈 모델의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
흥미로운 점은 임계값 0.10이 총비용 **426**으로 더 낮아 보일 수 있다는 사실입니다. 하지만 그 임계값은 `FPR=0.081`이라 운영 정책을 위반합니다. 즉 **비용 최소화와 정책 제약은 별개**입니다. 배포 기준은 보통 두 가지를 동시에 만족해야 합니다.
...
- [ ] `ROC-AUC 0.819`와 `PR-AUC 0.463`이 함께 나오면 무엇을 다르게 읽어야 할 수 있다
- [ ] `FPR <= 0.05` 제약 아래에서 임계값 0.141를 고르면 혼동 행렬과 비용은 어떻게 바뀌나요
- [ ] 비용만 보면 임계값 0.10이 더 낮을 수도 있는데, 왜 0.141나 다른 정책 선택을 다시 논의해야 하나요
- [ ] 이 글이 답하는 질문에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] 이 장의 최종 질문을 실무에 적용할 때 주의할 점은 무엇인지 이해했다
...
- **`ROC-AUC 0.819`와 `PR-AUC 0.463`이 함께 나오면 무엇을 다르게 읽어야 할까요?**
  - ROC-AUC 0.819는 모델이 양성과 음성을 순위로 어느 정도 잘 가른다는 뜻이지만, PR-AUC 0.463은 낮은 베이스레이트 환경에서 양성을 믿을 만하게 끌어올리는 힘이 그보다 약하다는 신호입니다. 그래서 이 글은 ROC 곡선의 인상만 보지 않고, PR-AUC까지 함께 읽은 뒤 실제 임계값 선택으로 내려왔습니다.
- **`FPR <= 0.05` 제약 아래에서 임계값 0.141를 고르면 혼동 행렬과 비용은 어떻게 바뀌나요?**
  - 본문 예제에서 0.141를 고르면 `FPR 0.049`, `precision 0.352`, `recall 0.507`, 혼동 행렬 `[[1355, 70], [37, 38]]`가 됩니다. 여기에 `FP=1`, `FN=10` 비용 가정을 붙이면 false positive 70건과 false negative 37건이 합쳐져 총비용 440으로 번역됩니다.
- **비용만 보면 임계값 0.10이 더 낮을 수도 있는데, 왜 0.141나 다른 정책 선택을 다시 논의해야 하나요?**
  - 임계값 0.10은 총비용이 426으로 더 낮아 보일 수 있어도 `FPR 0.081`이라서 `FPR <= 0.05` 정책을 위반합니다. 이 글이 강조한 핵심은 비용 최소화와 정책 제약을 동시에 만족해야 한다는 점이며, 재현율 목표가 60%라면 0.141조차 부족하므로 모델 개선이나 정책 재설계가 추가로 필요합니다.
