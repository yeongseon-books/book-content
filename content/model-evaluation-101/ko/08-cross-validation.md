...
- 교차 검증을 읽는 방식의 전환의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 테스트 세트 점수 하나만으로 모델을 고르면 왜 불안정할 수 있다
- [ ] K-Fold는 어떤 아이디어 위에서 동작할 수 있다
- [ ] 왜 분류 문제에서는 stratified가 기본 선택이 될까요
...
- **테스트 세트 점수 하나만으로 모델을 고르면 왜 불안정할까요?**
  - 한 번의 분할에서 나온 숫자는 데이터가 조금만 바뀌어도 쉽게 뒤집히기 때문입니다. 그래서 이 글은 단일 점수 대신 `cross_val_score`로 여러 폴드의 평균과 표준편차를 함께 보고, 차이가 작은 모델 비교일수록 흔들림까지 읽어야 한다고 설명했습니다.
- **K-Fold는 어떤 아이디어 위에서 동작할까요?**
  - 핵심 아이디어는 데이터를 k개로 나눠 검증 조각을 바꿔 가며 같은 모델을 여러 번 시험하는 것입니다. 본문 코드에서 `StratifiedKFold(n_splits=5, shuffle=True, random_state=0)`를 쓰는 이유도 점수 하나가 아니라 점수 묶음을 얻기 위해서입니다.
- **왜 분류 문제에서는 stratified가 기본 선택이 될까요?**
  - 분류에서는 각 폴드의 클래스 비율이 크게 흔들리면 점수 분산도 불필요하게 커집니다. 그래서 stratified가 원본 비율을 유지해 평균과 표준편차를 더 안정적으로 읽게 해 주고, 그룹 데이터나 시계열 데이터라면 다시 `GroupKFold`, `TimeSeriesSplit`으로 바꿔야 합니다.
