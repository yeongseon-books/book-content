...
- "데이터 마이그레이션"이 schema 마이그레이션과 어떻게 다른가
- `op.execute`와 SQLAlchemy core의 두 가지 작성 스타일
- 큰 데이터셋을 batch 단위로 나누는 패턴
...
- **스키마와 데이터 변경은 별도 리비전** — 한 리비전에 섞으면 롤백 단위가 모호해집니다.
- **ORM 대신 op.execute로 직접 SQL을 쓴다** — ORM이 변하면 과거 마이그레이션이 깨질 수 있습니다.
- **배치로 잘라서 실행한다** — 큰 테이블을 한 번에 UPDATE하면 락과 복제 지연이 큽니다.
- **백필은 기본값과 함께 단계적으로** — NOT NULL 추가는 기본값 설정·배치 백필·제약 추가 3단계로 나눕니다.
- **dry-run 통계를 먼저 본다** — 영향 행 수를 사전에 확인해야 사고를 예방합니다.
...
1. `users` 테이블에 `display_name` 컬럼을 추가하고 `name`을 backfill한 뒤, 별도 revision으로 `name`을 drop하는 3단계 마이그레이션을 작성하세요.
2. 100만 row 가짜 데이터로 batch loop의 동작 시간을 측정하세요.
3. backfill assertion이 실패했을 때 트랜잭션이 rollback되는지 직접 확인하세요.
