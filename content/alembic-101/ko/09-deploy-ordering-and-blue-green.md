...
- **스키마는 항상 앱보다 먼저 배포한다** — 구 버전 앱이 새 스키마에서도 동작하도록 호환성을 유지합니다.
- **expand-contract 패턴을 표준화한다** — 추가→이중 쓰기→소비 전환→축소 4단계로 무중단을 유지합니다.
- **마이그레이션은 배포 파이프라인의 1급 시민이다** — 별도 수기 절차가 아니라 자동 단계로 통합합니다.
- **실행 권한은 배포 사용자에게만 준다** — 운영 DB에 직접 alembic을 돌리는 사람과 권한을 명확히 분리합니다.
- **마이그레이션 락을 잡는다** — 동시 실행을 막아야 head 충돌과 부분 적용을 예방합니다.
