...
운영 환경에서 downgrade는 정말 가능한가? 어떤 변경은 되돌릴 수 없고 어떻게 대비해야 할까요?

이 글은 그 질문에 답하기 위해 다운그레이드 전략의 핵심 결정과 실무 함정을 살펴봅니다.
...
- **downgrade가 항상 안전하다는 가정을 버린다** — DROP COLUMN은 데이터 유실이므로 되돌림이 사실상 불가능합니다.
- **롤백은 forward fix가 기본이다** — 되돌리기보다 새 리비전으로 수정하는 편이 안전한 경우가 많습니다.
- **운영용 downgrade는 별도로 검증한다** — 스테이징 데이터로 실제 다운그레이드를 미리 시연해야 합니다.
- **백업은 마이그레이션 전후로 둔다** — 복구 가능 시점을 명확히 해야 사고 시 의사결정이 빠릅니다.
- **breaking change는 expand-contract로** — 추가·이중 쓰기·이전·축소 단계로 나눠 되돌림 여지를 항상 남깁니다.
