...
- alembic의 online/offline 두 가지 실행 모드
- `--sql` 옵션으로 실제 SQL을 미리 보는 방법
- DBA 검토용 SQL 스크립트를 만드는 워크플로우
...
- **온라인 모드는 운영 중 변경에 쓴다** — 대부분의 운영 환경에서는 트랜잭션 안에서 적용 가능한 변경만 허용합니다.
- **오프라인 모드로 SQL을 미리 만든다** — DBA가 검토하거나 변경 관리 절차가 있는 환경에서는 SQL 출력이 유용합니다.
- **SQLite·일부 변경은 batch가 필수다** — DB가 ALTER를 제한하는 경우 batch_alter_table로 안전하게 우회합니다.
- **락 시간을 의식한다** — MySQL·PostgreSQL의 ALTER 동작 차이를 이해하고 락 보유 시간을 점검합니다.
- **실패 시 재실행 시나리오를 정한다** — 중간 실패가 났을 때 어디서 다시 시작할지 사전에 합의합니다.
...
1. 임의의 schema 변경을 만들고 `alembic upgrade head --sql > out.sql`을 실행해 출력 구조를 확인하세요.
2. SQLite에서 batch 없이 `op.alter_column`으로 type을 바꾸려 시도해 에러 메시지를 직접 보세요.
3. `<prev>:<head>` 구문으로 마지막 한 단계 SQL만 추출해 보세요.
