...
- SQLite의 `ALTER TABLE` 제약과 `batch_alter_table`의 내부 동작은 무엇일까요?
- offline mode에서 피해야 할 패턴은 무엇일까요?
...
핵심은 `<from>:<to>` 문법입니다. `head`만 쓰면 `None:head`로 해석되어 처음부터 head까지의 SQL이 모두 출력됩니다. 이미 적용된 구간을 빼고 싶다면 `<current>:<target>`을 명시해야 합니다.
