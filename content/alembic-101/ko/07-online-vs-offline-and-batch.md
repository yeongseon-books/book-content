...
production 배포 전에는 마이그레이션이 실제로 어떤 SQL을 낼지 미리 보고 싶을 때가 많습니다. 여기에 SQLite의 제한까지 겹치면 offline 모드와 batch 모드를 함께 이해해야 합니다.

이 글은 Alembic 101 시리즈의 7번째 글입니다. 여기서는 online·offline 실행 흐름과 SQLite batch 모드를 한 번에 정리합니다.
