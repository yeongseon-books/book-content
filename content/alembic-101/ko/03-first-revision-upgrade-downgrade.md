...
- `alembic revision`이 만들어 주는 파일의 구조
- `op.create_table`, `op.add_column`, `op.drop_column`, `op.execute` 핵심 연산
- `upgrade()`와 `downgrade()`를 대칭으로 짜는 규칙
...
- **downgrade는 처음부터 작성한다** — 나중에 추가하기 어려우므로 작성 시점에 함께 검증합니다.
- **리비전은 작게 자른다** — 한 PR에 여러 변경을 묶지 말고 단일 의도로 분리해야 롤백이 쉽습니다.
- **upgrade는 멱등하게 설계한다** — 재실행해도 안전하도록 IF NOT EXISTS·존재 확인을 적극 사용합니다.
- **파괴적 변경은 별도 단계로 분리한다** — DROP·RENAME은 데이터 유실 위험이 있으므로 단독 리비전으로 둡니다.
- **CI에서 up/down 라운드 트립을 검증한다** — downgrade가 실제로 동작하는지 자동 검증해야 신뢰할 수 있습니다.
...
1. `users` 테이블에 `tier`, `last_login_at` 두 컬럼을 추가하는 revision을 직접 작성하고 `downgrade`까지 검증하세요.
2. `orders` 테이블을 만든 뒤 `downgrade -1 && upgrade head` 사이클을 5회 반복해 데이터 정합성이 유지되는지 확인하세요.
3. SQLite에서 `op.alter_column`으로 nullable을 변경해 보세요. batch 모드를 켜고 끄는 양쪽을 모두 시도하세요.
