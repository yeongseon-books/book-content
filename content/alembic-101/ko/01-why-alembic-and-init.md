...
- **스키마는 코드처럼 버전 관리한다** — 수기 SQL은 환경 간 차이를 만들기 쉬우므로 마이그레이션 파일을 PR 단위로 관리합니다.
- **디렉터리 구조는 처음에 정한다** — 프로젝트가 커진 뒤 alembic 디렉터리를 옮기는 비용이 크므로 초기에 위치를 합의합니다.
- **환경 분리는 alembic.ini에 의존하지 않는다** — DB URL을 코드에서 주입해야 dev·staging·prod 분기가 깔끔해집니다.
- **최초 베이스라인을 명확히 둔다** — 기존 DB가 있다면 stamp로 시작점을 고정해야 첫 업그레이드가 깨지지 않습니다.
- **리비전 네이밍 규칙을 합의한다** — 타임스탬프·이슈 번호·간단한 설명을 슬러그에 포함하면 히스토리가 읽힙니다.
