...
- 한 PR에 한 revision 원칙과 그 이유
- alembic 기반 PR 템플릿과 CI 체크 항목 구성
- dev=SQLite, staging+prod=PostgreSQL 다중 환경 관리
...
> migration은 **"가장 비가역적인 코드 변경"**입니다. 일반 코드는 revert 한 번으로 되돌리지만, schema 변경은 데이터를 동반하기 때문에 되돌리기가 매우 어렵습니다. 그래서 PR 단계에서 일반 코드보다 더 엄격하게 다뤄야 합니다.
...
2. alembic check                                        # model과 schema drift
3. alembic upgrade head && alembic downgrade -1         # downgrade 검증
4. alembic upgrade head --sql                           # SQL preview 생성, PR artifact로 첨부
5. python3 scripts/check_alembic_heads.py               # head가 1개인지 확인
6. python3 scripts/migrate_smoke_test.py                # fresh DB 기준 마이그레이션 적용
```
...
- **revision 메시지는 명령형 한 줄**: "add users phone column" (not "added", not "users phone added")
- **PR 제목에 expand-contract 단계 태그**: `[expand]`, `[migrate]`, `[contract]`
- **revision 단위로 PR 분리**: 한 PR에 한 revision
- **down_revision 충돌 시 즉시 merge**: multi-head 상태가 24시간 이상 유지되지 않게
...
```text
# Before: 한 PR에 6개 revision, downgrade 미검증
...
```text
# After: 한 PR = 한 revision, CI에서 downgrade 검증 자동화
...
- **한 PR에 여러 revision.** 리뷰가 어렵고 부분 revert가 불가능합니다.
- **downgrade를 CI에서 검증하지 않는다.** prod에서 사고가 나야 깨진 것을 알게 됩니다.
- **dev에서 PostgreSQL을 강제.** 환경 구축 비용이 증가하고 한 명짜리 빠른 실험이 어려워집니다.
- **dev SQLite로만 검증하고 prod로 직행.** SQLite와 PostgreSQL의 차이로 staging에서 사고가 납니다.
- **monitoring에 alembic_version이 없다.** 부분 적용 상태를 감지할 방법이 없습니다.

## 실무에서 쓰는 패턴

- **PR template과 CI 강제 조합.** 사람이 잊어도 시스템이 강제.
- **dev=SQLite, staging+prod=PostgreSQL.** 빠른 실험과 정확성을 동시에.
- **forward-fix를 기본 사고 대응으로.** downgrade는 정말 필요한 경우만.
- **revision 메시지에 GitHub issue 번호 포함.** "add users phone column (#1234)".
- **schema 변경은 영업시간에.** 새벽 배포는 오히려 사고 시 대응 인력이 부족합니다.
- **사후 분석에 schema 변경의 검증 흐름 항상 포함.** 무엇이 빠져 있었는지 회고.

## 시니어 엔지니어는 이렇게 생각합니다

- **마이그레이션 PR 체크리스트를 만든다** — downgrade·idempotency·락 영향·데이터 영향 항목을 표준화합니다.
- **DBA·SRE 리뷰 경로를 만든다** — 고위험 변경은 자동 라벨링으로 추가 리뷰어를 강제합니다.
- **CI에서 적용·되돌림을 모두 돈다** — up/down 라운드 트립을 자동 검증하면 사고를 크게 줄입니다.
- **히스토리 정리는 함부로 하지 않는다** — 운영에 적용된 리비전을 squash하면 환경 간 일관성이 깨집니다.
- **배포 알림에 마이그레이션 정보를 포함한다** — 어떤 리비전이 언제 적용됐는지 가시성이 있어야 사고 대응이 빠릅니다.
...
1. 자신의 프로젝트에 위 PR template과 CI workflow를 추가하고, 임의의 깨진 downgrade를 만들어 CI가 차단하는지 확인하세요.
2. `/health` endpoint에 alembic_version을 반환하도록 추가하고, drift 시 alarm이 발생하는 monitoring을 한 줄짜리 스크립트로 작성하세요.
3. 가상의 incident scenario (예: revision 적용 후 일부 row가 깨짐)를 정해 forward-fix revision을 작성해 보세요.
