...
- **target_metadata는 단일 진실 공급원이다** — 모델이 분산돼 있어도 한 곳에서 import해야 누락 없는 자동 감지가 됩니다.
- **URL은 환경 변수에서 읽는다** — alembic.ini에 하드코딩하지 말고 os.environ을 거쳐야 보안과 환경 분리가 됩니다.
- **include_object로 잡음을 거른다** — 임시 테이블·뷰·외부 스키마를 제외해야 의도치 않은 diff가 줄어듭니다.
- **compare_type을 의식적으로 켠다** — 타입 변경을 감지하려면 명시적으로 활성화해야 합니다.
- **멀티 DB는 alembic의 multi-db 템플릿을 쓴다** — 수기로 분기하기보다 공식 패턴을 따르는 편이 유지보수에 유리합니다.
