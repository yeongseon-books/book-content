...
- **PID 안전** — kill 대상 검증은 pgrep로 좁힙니다.
- **SIGTERM 우선** — SIGKILL은 마지막 수단으로만 씁니다.
- **Zombie 인지** — 부모 프로세스가 자원을 회수하지 않으면 누수가 생깁니다.
- **nohup vs systemd** — 장기 실행은 systemd unit으로 관리합니다.
- **관측** — ps + top + iotop으로 다축 관측합니다.
