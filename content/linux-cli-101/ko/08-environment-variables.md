...
- **환경변수는 어떤 방식으로 프로세스에 전달될까요?**
  - 환경변수는 `key=value` 형태로 현재 셸에 잡히고, `export`된 값만 자식 프로세스로 상속됩니다. 글의 `MY_VAR="hello"`와 `bash -c 'echo $MY_VAR'` 예시처럼 export 전후 결과가 달라지는 이유가 바로 이 전달 규칙입니다.
- **`export`와 로컬 Shell 변수는 무엇이 다를까요?**
  - 로컬 셸 변수는 현재 셸 안에서만 보이고, `export DB_HOST="localhost"`처럼 내보낸 변수만 Python, Docker, 하위 Bash 같은 자식 프로세스가 읽을 수 있습니다. 그래서 `.env`를 `set -a`와 `source`로 불러와 자동 export 하거나, systemd `EnvironmentFile`로 주입하는 패턴이 실무에서 자주 쓰입니다.
- **`PATH`는 명령 실행에서 어떤 검색 순서를 만들까요?**
  - PATH는 `:`로 구분된 디렉터리를 왼쪽에서 오른쪽 순서로 검사해 실행 파일을 찾는 목록입니다. `echo $PATH | tr ':' '\n'`, `which http`, `export PATH="$HOME/.local/bin:$PATH"` 예시처럼 누락된 경로를 앞에 추가하면 "command not found" 문제를 바로 해결할 수 있습니다.
