...
- Linux 파일 시스템의 시작점은 어디이고 왜 하나뿐일까요?
- 절대 경로와 상대 경로는 무엇이 다를까요?
- 파일과 디렉터리를 만들고, 복사하고, 옮기고, 삭제하는 명령어는 무엇일까요?
- `rm -rf`가 왜 위험한지 설명할 수 있을까요?

> Linux 파일 시스템은 하나의 뿌리(/)에서 시작하는 거대한 나무입니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- Linux 파일 시스템 계층 구조(`/`, `/home`, `/etc`, `/var`)
- `pwd`, `cd`, `ls`로 현재 위치 확인과 이동
- `mkdir`, `touch`, `cp`, `mv`, `rm`으로 파일/디렉터리 조작
- 절대 경로와 상대 경로의 차이와 사용 기준
...
- **절대 vs 상대** — 스크립트는 절대 경로를 기본으로 합니다.
- **rm -rf 위험** — 변수 미정의 시 재앙을 막기 위해 :? 패턴을 씁니다.
- **Hidden 파일** — ls -la로 숨김 파일을 항상 확인합니다.
- **inode 한계** — 디렉터리당 파일 수가 비대해지지 않게 분리합니다.
- **백업 습관** — 변경 전 cp로 빠른 스냅샷을 만듭니다.
...
1. 홈 디렉터리에 `project/src/`, `project/tests/`, `project/docs/` 구조를 `mkdir -p` 한 줄로 만들어보세요.
2. `touch`로 `project/src/main.py`, `project/tests/test_main.py`를 만들고, `project/src/main.py`를 `project/src/app.py`로 이름을 바꿔보세요.
3. `project/` 전체를 `project-backup/`으로 복사한 뒤, 원본 `project/`를 삭제해보세요.
