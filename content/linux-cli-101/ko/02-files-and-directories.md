...
- 전과 후의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **절대 경로와 상대 경로는 언제 다르게 느껴질까요?**
  - 절대 경로는 `/opt/my-app/releases/prod-20260521`처럼 어디서 실행해도 같은 대상을 가리키고, 상대 경로는 현재 위치에 따라 대상이 달라집니다. 그래서 배포 경로나 백업 경로처럼 실수 비용이 큰 작업은 절대 경로를 선호하고, 현재 작업 디렉터리 안에서만 움직일 때는 `./src`, `../config.yaml` 같은 상대 경로가 편합니다.
- **`pwd`, `cd`, `ls`만으로 현재 위치를 어떻게 읽어야 할까요?**
  - `pwd`로 현재 기준점을 확인하고, `cd ~/practice/linux-cli`처럼 이동한 뒤 `ls -la`로 그 위치에 실제로 무엇이 있는지 확인하면 됩니다. 실무 시나리오에서도 `/opt/my-app` 아래 `releases`를 보기 전에 먼저 `pwd`와 `ls -la ./releases`를 실행해 잘못된 경로에서 작업하는 사고를 막았습니다.
- **`cp`, `mv`, `rm`은 각각 어떤 상황에서 쓰면 안전할까요?**
  - `cp`는 원본을 남긴 채 복제할 때, `mv`는 위치를 옮기거나 이름을 바꿀 때, `rm`은 정말 삭제가 맞을 때만 써야 안전합니다. 글에서처럼 삭제는 `find ... -print`로 후보를 먼저 보고, 실제 실행은 `-print0 | xargs -0 rm -rf`처럼 대상을 검증한 뒤 진행하는 흐름이 중요합니다.

<!-- toc:begin -->
## 시리즈 목차

- [Linux CLI 101 (1/10): CLI와 Shell이란 무엇인가?](./01-what-is-cli-and-shell.md)
- **Linux CLI 101 (2/10): 파일과 디렉터리 다루기 (현재 글)**
