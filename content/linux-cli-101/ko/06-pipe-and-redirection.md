...
- `|`(pipe)는 어떻게 두 명령어를 연결할까요?
- `>`, `>>`, `<`의 차이는 무엇일까요?
- stdout과 stderr가 분리된 이유는 무엇일까요?
- 에러 메시지만 파일에 저장하려면 어떻게 할까요?

> pipe는 명령어를 수도관으로 연결하는 것이고, redirection은 물줄기의 방향을 파일로 바꾸는 것입니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- pipe(`|`)로 명령어의 출력을 다음 명령어의 입력으로 전달하는 법
- `>`(덮어쓰기)와 `>>`(이어쓰기)로 출력을 파일에 저장하는 법
- stdin(0), stdout(1), stderr(2) 파일 디스크립터의 의미
- `2>&1`로 에러와 일반 출력을 합치거나 분리하는 법
...
- **PIPESTATUS** — 파이프 중간 실패를 잡으려면 PIPESTATUS를 봅니다.
- **2>&1 순서** — 리다이렉션 순서는 결과를 바꿉니다.
- **Process substitution** — 임시 파일 대신 <(...)으로 단순화합니다.
- **Buffering** — 파이프 버퍼링이 실시간 출력을 막을 수 있습니다.
- **set -o pipefail** — 파이프 실패가 종료 코드에 반영되도록 합니다.
...
1. `/etc/passwd`에서 `/bin/bash`를 사용하는 사용자 수를 pipe 한 줄로 세어보세요.
2. `ls -la /etc`의 출력에서 파일 크기(5번째 열)만 추출하여 크기 순으로 정렬하고, 상위 5개를 출력해보세요.
3. `find / -name "*.conf" 2>/dev/null | head -10`을 실행해보세요. `2>/dev/null`이 왜 필요한지 설명해보세요.
