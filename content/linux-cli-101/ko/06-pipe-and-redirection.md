...
- **표준 입력, 표준 출력, 표준 오류는 왜 분리되어 있을까요?**
  - 표준 입력, 표준 출력, 표준 오류는 왜 분리되어 있을까요 — 본문에서 구체적으로 다룹니다.
- **`|`, `>`, `>>`, `2>`는 각각 어떤 흐름을 만들까요?**
  - 파이프와 리다이렉션은 CLI 자동화의 중심입니다. 핵심은 출력 포맷을 예쁘게 만드는 것이 아니라, **다음 단계가 소비할 수 있는 형태로 변환**하는 것입니다.
- **중간 파일 없이 명령을 이어 붙이면 무엇이 좋아질까요?**
  - 중간 파일 없이 명령을 이어 붙이면 무엇이 좋아질까요 — 본문에서 구체적으로 다룹니다.
- **머릿속에 먼저 그릴 그림에서 가장 흔한 실수는 무엇일까요?**
  - > 명령어는 수도꼭지이고, pipe(`|`)는 수도관입니다. 물(데이터)은 왼쪽에서 오른쪽으로 흐릅니다. redirection(`>`)은 물줄기를 수도관 대신 물통(파일)으로 보내는 것입니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - pipe는 Unix 철학의 핵심입니다. 작은 도구를 조합하면 전용 프로그램을 만들지 않아도 대부분의 텍스트 처리를 해결할 수 있습니다.
- **전과 후의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
