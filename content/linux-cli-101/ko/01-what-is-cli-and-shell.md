...
- **GUI 없이 컴퓨터를 다룬다는 말은 실제로 무엇을 뜻할까요?**
  - GUI 없이 컴퓨터를 다룬다는 말은 실제로 무엇을 뜻할까요 — 본문에서 구체적으로 다룹니다.
- **Terminal, Shell, CLI는 무엇이 다르고 어디서 헷갈릴까요?**
  - 처음 CLI를 배우는 단계에서는 `ls`, `cd`, `echo`처럼 작은 명령이 보통의 학습 단위입니다.
- **개발자가 실무에서 CLI를 꼭 익혀야 하는 이유는 무엇일까요?**
  - "GUI가 있는데 왜 굳이 CLI를 쓰냐"는 질문을 받으면, 대답은 **재현성과 자동화**입니다.
- **머릿속에 먼저 그릴 그림에서 가장 흔한 실수는 무엇일까요?**
  - > CLI는 키보드만으로 컴퓨터를 조종하는 리모컨이고, Shell은 그 리모컨의 신호를 해석하는 통역사입니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - "GUI가 있는데 왜 굳이 CLI를 쓰냐"는 질문을 받으면, 대답은 **재현성과 자동화**입니다.
...
- **Linux CLI 101 (1/10): CLI와 Shell이란 무엇인가? (현재 글)**
