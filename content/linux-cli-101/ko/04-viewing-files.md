...
- **파일을 통째로 볼 때와 일부만 볼 때는 어떤 명령을 골라야 할까요?**
  - 수십 줄짜리 `data.csv`처럼 짧은 파일은 `cat`이나 `cat -n`으로 한 번에 보고, 큰 로그 파일은 `less`, 앞부분은 `head -n 40`, 최근 구간은 `tail -n 80`처럼 목적에 맞게 잘라 보는 편이 맞습니다. 글에서 142MB 로그를 보기 전에 `ls -lh`와 `stat`로 크기와 수정 시각을 먼저 확인한 흐름도 같은 판단 기준입니다.
- **`less`가 단순 출력보다 더 안전한 이유는 무엇일까요?**
  - `less`는 파일을 메모리에 전부 올리지 않고 페이지 단위로 탐색하므로 큰 파일을 열어도 터미널 버퍼를 무너뜨리지 않습니다. `/ERROR`, `n`, `g`, `G`, `q`로 이동과 검색을 제어할 수 있어 `cat access.log`처럼 한꺼번에 쏟아내는 방식보다 분석용 도구로 훨씬 안전합니다.
- **`head`와 `tail`은 로그 확인에서 어떻게 다르게 쓰일까요?**
  - `head`는 파일 포맷, 헤더, 초기화 로그처럼 시작 구간을 확인할 때 쓰고, `tail`은 최근 에러와 현재 상태를 볼 때 씁니다. 특히 `tail -f /var/log/my-app/app.log`는 요청을 재현하면서 새 로그를 즉시 확인할 수 있어 장애 대응의 기본 루틴이 됩니다.
...
- **Linux CLI 101 (4/10): cat, less, head, tail — 파일 내용 보기 (현재 글)**
