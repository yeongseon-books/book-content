...
- SSH는 무엇이고 왜 텔넷 대신 쓸까요?
- 비밀번호 인증과 키 기반 인증의 차이는 무엇일까요?
- `~/.ssh/config`는 어떻게 접속을 편리하게 만들까요?
- `scp`와 `rsync`로 파일을 원격 전송하는 법은 무엇일까요?

> SSH는 네트워크를 통해 다른 컴퓨터의 터미널을 원격으로 여는 암호화된 통로입니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- SSH 접속의 기본 흐름과 키 기반 인증 설정
- `ssh-keygen`으로 키 쌍을 생성하고 서버에 등록하는 법
- `~/.ssh/config`로 접속 별칭을 만드는 법
- `scp`와 `rsync`로 파일을 원격으로 복사/동기화하는 법
...
- **Key 기반** — 비밀번호 인증은 비활성화합니다.
- **ssh-agent** — 키는 agent로 관리하고 디스크 암호화도 함께 둡니다.
- **Config** — ~/.ssh/config로 호스트별 설정을 분리합니다.
- **ProxyJump** — 베스천 호스트는 ProxyJump로 단순화합니다.
- **Audit** — 원격 명령은 감사 로그에 남도록 셸을 구성합니다.
...
1. `ssh-keygen -t ed25519`로 키 쌍을 생성하고, 공개 키(`~/.ssh/id_ed25519.pub`)의 내용을 확인해보세요.
2. `~/.ssh/config`에 가상의 서버 별칭을 작성해보세요 (실제 접속하지 않아도 됩니다). `Host`, `HostName`, `User`, `Port`, `IdentityFile` 5개 항목을 포함합니다.
3. `scp`로 로컬 파일을 `/tmp/`에 복사하는 명령어를 작성해보세요 (localhost를 원격 호스트로 사용).
