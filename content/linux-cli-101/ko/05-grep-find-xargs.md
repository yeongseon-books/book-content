...
- 수천 개 파일에서 특정 문자열을 포함한 줄을 어떻게 찾을까요?
- 파일 내용 검색(grep)과 파일 이름 검색(find)은 무엇이 다를까요?
- xargs는 왜 필요하고, pipe와 어떻게 다를까요?
- 정규표현식 없이도 grep을 효과적으로 쓸 수 있을까요?

> grep은 파일 내용에서 텍스트를 찾는 탐정이고, find는 파일 이름과 속성으로 파일을 찾는 수색대입니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- `grep`으로 파일 내용에서 문자열을 검색하는 법
- `find`로 이름, 크기, 시간 기준으로 파일을 찾는 법
- `xargs`로 검색 결과를 다른 명령어의 인자로 전달하는 법
- 세 명령어를 조합하여 실무 작업을 자동화하는 법
...
- **ripgrep 권장** — 신규 코드 검색은 rg가 사실상 표준입니다.
- **find -print0** — 공백 안전을 위해 -print0 + xargs -0를 씁니다.
- **정규식 절제** — 고정 문자열은 grep -F가 안전·빠릅니다.
- **성능 함정** — 디렉터리 깊이가 깊으면 prune으로 가지치기합니다.
- **동시성** — xargs -P로 병렬화는 안전 검증 후에만 씁니다.
...
1. 홈 디렉터리에서 `.py` 확장자를 가진 모든 파일을 찾아보세요.
2. `grep -rn "import" project/`를 실행하고, 결과에서 파일별 import 수를 `grep -rc`로 확인해보세요.
3. 7일 이내에 수정된 파일 중 `TODO`가 포함된 파일만 찾는 명령어를 `find`와 `grep`을 조합하여 만들어보세요.
