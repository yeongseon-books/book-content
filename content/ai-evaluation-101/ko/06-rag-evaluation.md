...
- **retrieval과 generation을 분리 평가** — 어디서 깨지는지 모르면 개선이 안 됩니다.
- **recall@k·precision@k가 retrieval 기본** — 결정적이고 자동화 가능합니다.
- **faithfulness가 generation 핵심** — 환각을 가장 빠르게 잡습니다.
- **context relevance도 별도 지표** — 관련 없는 context가 들어오면 답이 나빠집니다.
- **end-to-end 평가는 별도** — 구성 평가만으로는 부족합니다.
