...
두 prompt 중 어느 쪽이 더 나은지 어떻게 결정할까요?

이 글은 AI Evaluation 101 시리즈의 9번째 글입니다. 여기서는 paired comparison, win rate, statistical significance, sample size 계산까지 LLM A/B 테스팅의 실무를 다룹니다.

---
![LLM A/B 테스팅 - prompt 비교](../../../assets/ai-evaluation-101/09/09-01-a-b-testing-llms-which-prompt-is-better.ko.png)

LLM A/B 테스팅 - prompt 비교

## "더 나아 보인다"는 증거가 아닙니다

!["더 나아 보인다"는 증거가 아닙니다](../../../assets/ai-evaluation-101/09/09-01-looks-better-is-not-evidence.ko.png)
...
![Pairwise Comparison으로 Win Rate 측정](../../../assets/ai-evaluation-101/09/09-03-pairwise-comparison-and-win-rate.ko.png)
...
def ab_test(questions: list[str], model_a: str, model_b: str) -> dict:
    ...
    for q in questions:
        ...
        # 위치 편향 통제: 순서를 바꿔 한 번 더 평가 (Ep4 참조)
...
![표본 크기 - 필요 샘플 수](../../../assets/ai-evaluation-101/09/09-04-sample-size-how-many-to-evaluate.ko.png)
...
![통계적 유의성 검증 - Two-Proportion Z-Test](../../../assets/ai-evaluation-101/09/09-05-statistical-significance-two-proportion.ko.png)
...
def assign_variant(user_id: str, experiment: str) -> str:
    """user_id를 해시해 안정적으로 A 또는 B에 할당합니다."""
...
def handle_request(user_id: str, question: str) -> str:
    ...
    log_metric(user_id, variant, response)  # 나중에 분석할 때 사용
    return response
```
