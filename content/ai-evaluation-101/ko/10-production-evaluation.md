...
이 그림에서는 배포 후 production trace를 샘플링하고, online metric과 drift를 감시하며, 실패를 다시 regression set으로 넣는 지속 평가 루프를 봅니다. 운영 평가는 배포 전 검사를 실제 서비스 신호와 연결해 품질 관리를 닫는 마지막 단계입니다.
