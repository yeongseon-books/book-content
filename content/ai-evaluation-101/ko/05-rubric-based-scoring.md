...
Rubric 기반 채점을 어떻게 설계해야 평가의 일관성과 해석 가능성을 동시에 잡을까요?

이 글은 그 질문에 답하기 위해 Rubric 채점 설계의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **rubric이 평가의 단일 출처** — 사람·LLM 판정 모두 같은 기준을 씁니다.
- **기준은 행동 기반으로** — 추상적 형용사는 평가자 간 일치율을 떨어뜨립니다.
- **scale은 짧게** — 5점 이상은 일관성이 떨어집니다.
- **예시가 rubric의 절반** — good·bad 예시가 일치율을 결정적으로 올립니다.
- **rubric도 반복 개선** — 사용하면서 모호한 부분을 보완합니다.
