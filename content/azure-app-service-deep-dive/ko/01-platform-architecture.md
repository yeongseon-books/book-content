...
운영적으로 가장 큰 수확은 문제를 박스별로 자를 수 있게 된다는 사실입니다. 일부 사용자만 계속 문제를 겪는다면 Front-End와 affinity를 먼저 보고, 배포 직후 앱이 안 뜬다면 Kudu와 startup readiness를 분리해서 보고, 여러 인스턴스에서 파일이 어긋나게 보인다면 shared storage semantics를 먼저 확인할 수 있습니다. deep dive의 가치는 이처럼 증상을 물리적 경계에 붙여 읽게 만드는 데 있습니다.
