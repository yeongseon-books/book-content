...
이 그림에서는 App Service 플랫폼 아키텍처 — Front-End·Worker·File Server를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.
...
- **App Service의 "플랫폼"은 실제로 어떤 박스들로 나눠서 이해해야 할까요?**
  - 본문의 기준은 App Service 플랫폼 아키텍처 — Front-End·Worker·File Server를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **App Service Plan은 단순한 과금 단위가 아니라 어떤 격리와 용량의 의미를 가질까요?**
