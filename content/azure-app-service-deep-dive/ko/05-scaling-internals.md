---
title: "Azure App Service Deep Dive (5/6): 스케일링 내부 동작 — Scale Out 결정과 워커 추가 경로"
...
![Azure App Service Deep Dive 5장 흐름 개요](https://yeongseon-books.github.io/book-public-assets/assets/azure-app-service-deep-dive/05/05-01-the-control-path-in-one-diagram.ko.png)

*Azure App Service Deep Dive 5장 흐름 개요*

이 그림에서는 스케일링 내부 동작 — Scale Out 결정과 워커 추가 경로를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.
...
- **scale-up과 scale-out은 App Service에서 실제로 무엇을 바꿀까요?**
  - 본문의 기준은 스케일링 내부 동작 — Scale Out 결정과 워커 추가 경로를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **autoscale rule은 앱이 아니라 왜 App Service Plan에 붙는다고 봐야 할까요?**
...
- **Azure App Service Deep Dive (5/6): 스케일링 내부 동작 — Scale Out 결정과 워커 추가 경로 (현재 글)**
