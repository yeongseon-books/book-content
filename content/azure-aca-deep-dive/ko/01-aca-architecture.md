...
Azure Container Apps의 공개 설명은 일부러 단순합니다. 컨테이너 이미지를 올리고, Ingress나 Dapr, 스케일 규칙을 켜면 플랫폼 운영은 Microsoft가 맡는다고 말합니다.

이 단순화는 분명 편하지만, 내부 구조를 한 덩어리로 오해하게 만들기도 합니다. Container Apps는 raw Kubernetes 서비스도 아니고, "AKS에 편의 기능만 얹은 것"으로 끝나는 제품도 아닙니다.

이 글은 Azure Container Apps Deep Dive 시리즈의 첫 번째 글입니다. 여기서는 Azure Container Apps가 Microsoft 관리 Kubernetes 위에 어떤 계층을 얹어 제품으로 만드는지 전체 지도를 먼저 잡습니다.
