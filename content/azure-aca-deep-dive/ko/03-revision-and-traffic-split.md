...
Azure Container Apps는 rollout을 실제보다 훨씬 부드럽게 보이게 만듭니다. 이미지를 바꾸면 ACA가 새 Revision을 만들고, 트래픽 비율을 옮겨도 서비스는 계속 응답하는 것처럼 보입니다.

표면이 이렇게 단순한 이유는 그 아래 단계가 감춰져 있기 때문입니다. 불변 Revision이 먼저 만들어지고 active 상태가 되어야 하며, 마지막 요청 경로 어딘가에서 가중치가 실제로 적용돼야 합니다.

이 글은 Azure Container Apps Deep Dive 시리즈의 3번째 글입니다. 여기서는 Revision 불변성과 트래픽 분할이 Envoy 라우팅 계층에서 어떻게 의미를 갖는지 추적합니다.
