...
Azure Container Apps의 ingress 공개 설명은 짧습니다. Ingress를 켜고 FQDN을 받아 HTTPS 트래픽을 받으며, 필요하면 Revision 사이에 traffic을 나누면 된다고 설명합니다.

서비스를 올리는 데는 이 설명으로도 충분하지만, 첫 요청의 경로를 설명하기에는 부족합니다. 실제 경로를 이해하려면 공개된 동작과 Envoy·Kubernetes 패턴에 기대는 추론의 경계를 먼저 나눠서 봐야 합니다.

이 글은 Azure Container Apps Deep Dive 시리즈의 마지막 글입니다. 여기서는 ingress의 첫 요청이 Envoy 계층을 지나 사용자 컨테이너에 닿기까지의 경로를 살펴봅니다.
