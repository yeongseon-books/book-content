...
### Envoy filter chain 관점으로 보면 header 동작이 정리됩니다

ACA 내부 Envoy 구성을 직접 덤프할 수는 없지만, 공개 Envoy 모델 기준으로 보면 요청은 대체로 listener/filter chain/route 선택 순서를 따릅니다. 아래는 개념을 설명하기 위한 축약 예시입니다.
...
첫 요청 지연이나 간헐 5xx를 분석할 때는 어느 계층에서 지연이 시작됐는지 분리해야 합니다. 그래서 ingress 계층에서 내려온 trace 정보와 앱 로그의 trace 키를 같은 형식으로 남기는 것이 중요합니다.

```text
# ingress access log (개념 예시)
2026-05-20T08:21:11Z trace_id=ab12 route=rev-green upstream_time_ms=187 status=200 path=/checkout

# app log
2026-05-20T08:21:11Z trace_id=ab12 revision=orders-api--green handler=/checkout app_time_ms=132 status=200
```

`upstream_time_ms`와 `app_time_ms`를 비교하면 지연이 ingress 이전/이후 어디에서 커졌는지 빠르게 분리할 수 있습니다.

### forwarded header 복구 코드는 프레임워크별로 명시해야 합니다
...
이 설정이 없으면 앱은 항상 `http`로 들어왔다고 판단해 `https` 강제 리다이렉트를 반복할 수 있습니다. Ingress는 정상인데 사용자 체감은 장애처럼 보이는 전형적인 사례입니다.

### 디버깅 시나리오: 502가 나는데 앱 로그가 비어 있을 때

이 케이스는 매우 흔합니다. 보통은 앱 코드 실행 전 단계에서 실패했기 때문입니다. 아래 순서를 고정하면 대응이 빨라집니다.

1. `hostname list`로 도메인 바인딩과 ingress posture를 확인합니다.
2. `ingress show`로 target port와 transport 값을 확인합니다.
3. `traffic show`로 요청이 향하는 revision 집합을 확인합니다.
4. 선택된 revision의 ready replica 수와 최근 재시작 여부를 확인합니다.
5. 필요한 경우 label URL로 우회 호출해 라우팅 층과 앱 층을 분리합니다.

앱 로그가 비어 있다는 사실 자체가 힌트입니다. 요청이 컨테이너까지 도달하지 못했을 가능성이 높다는 뜻이기 때문입니다.

### 디버깅 시나리오: canary 10%인데 특정 고객군만 실패할 때

가중치가 정상인데 일부 고객군만 실패하는 경우, 종종 쿠키 기반 세션 고정이나 장수 연결 특성이 관찰값을 왜곡합니다. 이때는 단순 비율 평균이 아니라 client segment별로 요청 분포를 다시 봐야 합니다.

```text
segment=mobile-app traffic_share=11% error_rate=4.8%
segment=web-browser traffic_share=9% error_rate=0.6%
```

이런 분해를 하면 "가중치가 틀렸다"보다 "특정 클라이언트 패턴에서 green revision의 취약점이 드러난다"는 결론에 더 빨리 도달합니다. ingress 운영은 전체 평균뿐 아니라 세그먼트 분포까지 봐야 안정적입니다.
