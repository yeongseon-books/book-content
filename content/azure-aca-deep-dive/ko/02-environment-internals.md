...
- Dapr component는 왜 앱 리소스가 아니라 Environment 리소스일까요?
- 언제 하나의 Environment를 공유하고, 언제 분리하는 편이 더 안전할까요?
...
## Environment를 이해하는 가장 좋은 방법: 플랫폼의 격리 단위로 보는 것입니다
...
![Environment-scoped isolation and shared planes](../../../assets/azure-aca-deep-dive/02/02-01-the-environment-is-the-platform-s-isolat.ko.png)

*Environment-scoped isolation and shared planes*

운영상 의미는 단순합니다. 두 앱이 같은 blast radius와 observability plane에 있어도 된다면 하나의 Environment가 합리적일 수 있습니다. 반대로 네트워크, 로그, Dapr 구성 경계를 분리해야 한다면 Environment부터 나누는 편이 맞습니다.
...
![Per-app ingress choices inside one environment](../../../assets/azure-aca-deep-dive/02/02-02-network-scope-begins-at-the-environment.ko.png)
...
![External and internal ingress within one environment](../../../assets/azure-aca-deep-dive/02/02-03-external-versus-internal-ingress-still-s.ko.png)

*External and internal ingress within one environment*

하지만 이 편의는 동시에 제약입니다. 같은 Environment 안에 넣는 순간 네트워크 경계의 가장 중요한 단위는 개별 앱보다 Environment가 됩니다. 즉 “앱 하나만 internal이면 충분하다”가 아니라 “이 앱들이 같은 Environment에 있어도 되는가”를 먼저 물어야 합니다.
...
![Environment-level shared observability boundary](../../../assets/azure-aca-deep-dive/02/02-04-observability-is-centralized-at-the-envi.ko.png)
...
![Shared workspace and per-runtime signal sources](../../../assets/azure-aca-deep-dive/02/02-05-shared-logs-do-not-mean-all-signals-are.ko.png)
...
![Environment-scoped components and app-level consumers](../../../assets/azure-aca-deep-dive/02/02-06-dapr-components-are-environment-resource.ko.png)
...
![Dapr app ID to component scope mapping](../../../assets/azure-aca-deep-dive/02/02-07-scope-means-dapr-app-id-not-container-ap.ko.png)
...
![Cross-app Dapr calls inside one environment](../../../assets/azure-aca-deep-dive/02/02-08-cross-app-dapr-communication-only-makes.ko.png)

*Cross-app Dapr calls inside one environment*

핵심은 workaround 가능 여부가 아닙니다. 제품이 어떤 trust boundary와 networking shape를 기본값으로 여기는가를 읽는 것입니다. ACA는 이 통신을 Environment 내부의 네이티브 패턴으로 취급합니다.
...
![Shared control loops terminating at environment boundary](../../../assets/azure-aca-deep-dive/02/02-09-control-loops-that-terminate-at-the-envi.ko.png)
