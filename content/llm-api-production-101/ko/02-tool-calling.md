...
| 실수 | 이유 | 올바른 접근 |
|------|------|------------|
| `tool_calls` 반환을 모델이 코드를 실행한다고 이해 | 모델은 실행 요청을 제안할 뿐이고 실제 실행은 애플리케이션이 담당 | 애플리케이션이 allowlist 확인 후 직접 함수를 실행 |
| 툴 정의 설명이 모호해 모델이 잘못 선택 | 설명이 불명확하면 모델이 부적절한 도구를 고를 수 있음 | 각 도구의 description에 "언제 사용하는지" 명확히 서술 |
| `json.loads()` 뒤에 바로 `**arguments` 전달 | 인자 검증 없이 실행하면 잘못된 타입이나 범위의 값이 함수에 도달 | Pydantic 모델로 먼저 검증 후 `model_dump()` 결과를 전달 |
| 조회 도구와 상태 변경 도구를 동일하게 처리 | 상태 변경은 재시도 시 중복 부작용, 권한 검사 강화 등 다른 정책 필요 | TOOL_REGISTRY에 `side_effect` 플래그를 붙여 정책 분리 |
| 툴 결과를 첫 번째 응답으로 오해 | 첫 응답은 실행 요청이고 실제 사용자 응답은 툴 결과 재주입 이후에 완성 | 두 번째 모델 호출에서 `role: tool` 메시지를 포함해야 최종 답 생성 |
...
- **툴 호출은 모델에게 실행 권한을 넘기는 기능일까요, 애플리케이션이 만든 실행 경계일까요?**
  - 애플리케이션이 만든 실행 경계입니다. `tools` 파라미터로 허용된 함수 집합을 공개하고, 모델은 그 안에서 선택만 합니다. 실제 실행, 인자 검증, 권한 통제는 모두 애플리케이션이 담당합니다.
- **`tools` 정의와 `tool_calls` 응답에서 각각 무엇을 검증해야 할까요?**
  - `tools` 정의에서는 이름, 설명, 인자 스키마(`additionalProperties=False` 포함)를 명확히 해야 합니다. `tool_calls` 응답에서는 함수 이름이 allowlist에 있는지, 인자를 Pydantic으로 검증한 뒤 실행해야 합니다.
- **함수 실행 루프를 운영 환경에서 안전하게 끝내려면 어떤 방어선이 필요할까요?**
  - allowlist 확인, 인자 검증, 최대 호출 횟수 제한, 상태 변경 도구의 멱등성 키 적용, 오류 응답 표준화가 필요합니다. 상태 변경 도구와 조회 도구는 서로 다른 신뢰 수준으로 다뤄야 합니다.
