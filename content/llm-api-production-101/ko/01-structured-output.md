...
JSON 모드와 응답 스키마를 어떻게 써야 구조화 출력의 신뢰성을 확보할 수 있을까요?

이 글은 그 질문에 답하기 위해 구조화 출력의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- 왜 자유 형식 텍스트 응답이 프로덕션에서 깨지기 쉬운가?
- JSON Schema와 Pydantic 중 어떤 도구가 어떤 상황에 적합한가?
- OpenAI의 `response_format`과 함수 호출(tool calling) 기반 구조화 출력은 어떻게 다른가?
- 스키마를 어겼을 때 재시도 vs. 검증 후 fallback 중 어떤 전략이 나은가?
- 스키마가 길어질수록 토큰 비용과 정확도는 어떻게 변하는가?
...
- **JSON 모드는 형식만 보장** — 내용 검증은 별도로 합니다.
- **function calling이 스키마 강제에 강하다** — free-form JSON보다 안전합니다.
- **스키마는 작고 명확하게** — 복잡한 스키마는 정확도를 떨어뜨립니다.
- **validate·retry 흐름을 표준화** — 한 번에 통과하지 못하는 게 정상입니다.
- **실패 사례를 corpus로 보관** — 프롬프트 개선의 가장 빠른 자료입니다.
