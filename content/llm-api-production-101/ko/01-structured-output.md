...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/47"
...
이 글은 LLM API 프로덕션 101 시리즈의 첫 번째 글입니다.
...
| 실수 | 이유 | 올바른 접근 |
|------|------|------------|
| JSON 모드를 켰으니 스키마까지 보장된다고 가정 | JSON 모드는 문법만 보장하고 필드 이름·타입·범위는 보장하지 않음 | JSON 모드 이후에 Pydantic 검증을 별도 계층으로 추가 |
| `json.loads()` 성공을 스키마 검증 통과로 간주 | 파싱과 의미 검증은 서로 다른 계층의 책임 | `model_validate()` 호출을 항상 `json.loads()` 다음에 배치 |
| 프롬프트 문구만 길게 써서 구조 문제를 해결하려는 시도 | 자연어 지시는 코드 계약보다 일관성이 낮음 | enum·필드·범위를 코드 검증 계층에 먼저 정의 |
| 검증 실패를 "모델 품질 문제"로만 분류 | 실패가 파싱인지 스키마인지 요청인지 구분 안 되면 복구 정책 설계 불가 | `json.JSONDecodeError`와 `ValidationError`를 각각 다른 except 블록으로 처리 |
| 스키마 버전 없이 필드를 추가 | 새 코드와 캐시된 응답이 충돌하면 조용한 오류 발생 | `schema_version` 필드를 모델에 포함하고 변경 시 버전 증가 |
...
- **자유 형식 텍스트 파싱은 운영 환경에서 왜 금방 깨질까요?**
  - 모델 출력 형식은 프롬프트 길이, 컨텍스트 내용, 모델 버전에 따라 달라질 수 있습니다. 필드 이름, 값 타입, 누락 데이터 처리 등의 규칙이 코드 밖에 흩어져 있으면 예상치 못한 형식 변화에 바로 깨집니다.
- **JSON 모드는 무엇을 보장하고, 스키마 검증은 무엇을 따로 보장할까요?**
  - JSON 모드는 응답이 파싱 가능한 JSON 구문임을 보장합니다. 스키마 검증은 그 JSON이 애플리케이션이 기대하는 필드·타입·범위를 갖추고 있음을 추가로 보장합니다. 두 계층은 책임이 다릅니다.
- **구조화 출력 계약이 깨졌을 때 어디서 멈추고 무엇을 기록해야 할까요?**
  - 파싱 계층에서 `json.JSONDecodeError`, 검증 계층에서 `ValidationError`를 별도로 잡아야 합니다. 각 실패에는 원문 응답과 오류 유형을 함께 기록해 재현과 분석이 가능하도록 해야 합니다.
