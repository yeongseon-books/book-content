...
재시도와 오류 처리를 어떻게 해야 안정적인 LLM API 호출을 만들 수 있을까요?

이 글은 그 질문에 답하기 위해 재시도와 오류 처리의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **idempotency-key가 표준 도구** — 중복 과금·중복 효과를 막습니다.
- **exponential backoff with jitter** — 동시 재시도가 사고를 키우지 않게 합니다.
- **재시도 가능 오류와 영구 오류 분리** — 4xx와 5xx의 처리가 다릅니다.
- **circuit breaker로 cascade 방지** — 장애가 시스템 전체로 번지지 않게 합니다.
- **타임아웃은 항상 명시** — 기본 무한 대기는 사고 원인입니다.
