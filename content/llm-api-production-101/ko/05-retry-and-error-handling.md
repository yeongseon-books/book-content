...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/51"
...
이 글은 LLM API 프로덕션 101 시리즈의 5번째 글입니다.
...
- **왜 모든 API 실패를 같은 재시도 정책으로 다루면 안 될까요?**
  - 왜 모든 API 실패를 같은 재시도 정책으로 다루면 안 될까요 — 본문에서 구체적으로 다룹니다.
- **어떤 오류는 재시도하고, 어떤 오류는 바로 실패로 분류해야 할까요?**
  - 어떤 오류는 재시도하고, 어떤 오류는 바로 실패로 분류해야 할까요 — 본문에서 구체적으로 다룹니다.
- **최종 실패 뒤 사용자 메시지와 내부 로그는 어떻게 나눠야 할까요?**
  - 최종 실패 뒤 사용자 메시지와 내부 로그는 어떻게 나눠야 할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 재시도는 단순한 안정성 옵션이 아닙니다. 실패를 어떤 종류로 보고 어느 지점까지 복구를 시도할지 결정하는 운영 정책입니다. 이 정책이 없으면 네트워크 일시 장애와 영구 설정 오류가 같은 흐름으로 섞이고, 결과적으로 시스템은 더 느려지고 더 시끄러워집니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 재시도가 유효한 경우는 실패가 잠시 후 사라질 가능성이 있을 때입니다. 짧은 네트워크 흔들림, transport timeout, 일부 5xx 응답, 때로는 429가 여기에 들어갑니다.
- **흔히 헷갈리는 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 재시도가 유효한 경우는 실패가 잠시 후 사라질 가능성이 있을 때입니다. 짧은 네트워크 흔들림, transport timeout, 일부 5xx 응답, 때로는 429가 여기에 들어갑니다.
