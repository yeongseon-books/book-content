...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/49"
...
이 글은 LLM API 프로덕션 101 시리즈의 3번째 글입니다.
...
| 실수 | 이유 | 올바른 접근 |
|------|------|------------|
| 스트리밍을 "빠른 일반 응답"으로만 이해 | 부분 응답 상태, 연결 끊김, 종료 신호 누락 같은 스트리밍 전용 실패를 놓침 | 각 청크를 세션 상태의 일부로 추적하고 부분 결과를 항상 보존 |
| 빈 delta를 오류 신호로 취급 | 빈 청크는 역할 정보, 종료 메타데이터 등 프로토콜의 정상 부분 | `delta is not None and delta != ""`로 빈 청크를 정상 이벤트로 처리 |
| 동기 루프 내부에서 타임아웃 시계 확인 | 블로킹 이터레이터는 청크 사이 무응답을 루프 내부에서 감지할 수 없음 | 비동기 `asyncio.wait_for(anext(stream))` 또는 transport timeout 사용 |
| 스트림 실패 시 누적 텍스트 폐기 | 복구, 디버깅, UI 처리를 위해 부분 결과가 필요 | 예외 발생 시에도 `parts` 리스트를 결과 객체에 포함해 반환 |
| 스트리밍 JSON을 청크 단위로 `json.loads()` 시도 | 불완전한 JSON은 파싱 오류 발생 | `finish_reason` 신호를 받은 이후에만 전체 문자열을 파싱 |
...
- **스트리밍은 최종 문자열 하나가 아니라 왜 부분 상태를 가진 세션으로 봐야 할까요?**
  - 하나의 스트리밍 요청 안에서 청크 도착, 무응답 구간, 연결 끊김이 혼재할 수 있습니다. 각 청크는 독립적인 상태 전이이고, 최종 문자열만 바라보면 어디까지 성공했는지 알 수 없어 재시도와 UI 처리가 모두 어려워집니다.
- **텍스트가 없는 chunk와 중간 실패는 어떤 상태로 다뤄야 할까요?**
  - 빈 delta는 프로토콜 정상 이벤트로 다루고, 중간 실패는 `status=failed`와 함께 누적 텍스트를 보존한 결과 객체로 반환해야 합니다. 실패를 예외로만 처리하면 부분 결과를 잃습니다.
- **스트리밍 실패 뒤 재시도할 때 무엇을 보존하고 무엇을 다시 만들어야 할까요?**
  - 이미 사용자에게 노출된 부분 텍스트는 보존합니다. 새 스트리밍 요청은 원래 프롬프트로 처음부터 다시 시작하거나, UI에서 이어 붙이는 형태로 노출합니다. 재시도 정책은 UI 표시 여부에 따라 달라져야 합니다.
