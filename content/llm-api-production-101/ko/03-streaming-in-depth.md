...
스트리밍 응답의 청크 처리와 오류 복구를 어떻게 해야 사용자 경험을 지킬 수 있을까요?

이 글은 그 질문에 답하기 위해 스트리밍 심화의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- 스트리밍 응답은 일반 응답과 어떤 HTTP 메커니즘 수준에서 다른가?
- Server-Sent Events(SSE)와 chunked transfer는 어떤 차이가 있고 LLM API는 무엇을 쓰는가?
- 스트림 도중 연결이 끊기면 어떻게 부분 응답을 안전하게 누적/저장하는가?
- 토큰 단위 스트림을 단어 단위로 버퍼링해야 할 때는 언제인가?
- 스트리밍 응답의 사용량(usage) 정보는 어디서 받고 어떻게 누적하는가?
...
![스트리밍 세션이 부분 상태를 남기는 흐름](../../../assets/llm-api-production-101/03/03-01-what-changes-when-the-response-is-a-stre.ko.png)
...
![기본 청크 소비 루프의 실행 경로](../../../assets/llm-api-production-101/03/03-02-the-baseline-chunk-loop.ko.png)
...
![동기 루프와 비동기 timeout 적용 비교](../../../assets/llm-api-production-101/03/03-03-enforcing-timeouts-outside-the-loop.ko.png)
...
![스트리밍 결과 객체에 남기는 상태 구조](../../../assets/llm-api-production-101/03/03-04-keeping-partial-output-on-failure.ko.png)
...
![스트리밍 중단 뒤 재시도 판단 경로](../../../assets/llm-api-production-101/03/03-05-retrying-after-a-streaming-failure.ko.png)
...
- **연결 끊김에 대한 재시작 전략** — 처음부터가 아니라 이어가기를 설계합니다.
...
- [ ] SSE/chunked 응답을 비동기로 소비하는 클라이언트 코드를 검증했다
- [ ] 스트림 도중 연결 종료 시 부분 응답을 보존하고 재개 전략을 정의했다
- [ ] 토큰 단위 출력의 줄바꿈/공백 처리를 UI에서 안정적으로 보여줬다
- [ ] 스트림 종료 시점에 usage 정보를 수집해 비용 메트릭에 반영했다
- [ ] 스트리밍 중 도구 호출이나 에러 청크를 별도로 분기 처리했다
