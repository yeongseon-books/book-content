...
- 스트리밍 응답은 일반 완료 응답과 무엇이 다를까요?
- 청크에 텍스트가 없을 때는 어떻게 처리해야 할까요?
- 청크 간 무응답은 왜 루프 안이 아니라 읽기 바깥에서 감시해야 할까요?
- 스트림이 끊겼을 때 부분 결과를 어떻게 보존해야 할까요?
- 사용자에게 이미 일부 응답을 보여 준 뒤 재시도는 어떻게 판단해야 할까요?
...
## 스트리밍을 이해하는 가장 좋은 방법: 최종 문자열이 아니라 부분 상태를 가진 세션으로 보는 것입니다

스트리밍 요청은 한 번의 함수 호출처럼 보이지만, 실제로는 작은 상태 기계에 가깝습니다. 청크가 올 때마다 누적 문자열이 갱신되고, 종료 신호가 오면 정상 완료가 되며, 일정 시간 아무 진전이 없으면 타임아웃으로 분류할 수 있습니다. 즉, 응답은 한 번에 “도착”하는 것이 아니라 시간에 따라 “진행”됩니다.

이 관점이 있어야 구현 우선순위도 맞게 잡힙니다. 가장 먼저 필요한 것은 예쁜 실시간 출력이 아니라, 누적 상태를 잃지 않는 소비 루프입니다. 사용자에게 보이는 문자열과 내부 저장용 문자열이 동시에 유지되어야 하고, 부분 실패에서도 둘 다 버려지지 않아야 합니다.
