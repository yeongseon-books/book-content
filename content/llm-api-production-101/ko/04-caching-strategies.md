...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/50"
...
- **LLM 캐시는 응답 저장소가 아니라 왜 요청 동일성 계약으로 봐야 할까요?**
  - LLM 캐시는 응답 저장소가 아니라 왜 요청 동일성 계약으로 봐야 할까요 — 본문에서 구체적으로 다룹니다.
- **캐시 키에는 프롬프트 외에 어떤 값이 들어가야 할까요?**
  - 캐시 키에는 프롬프트 외에 어떤 값이 들어가야 할까요 — 본문에서 구체적으로 다룹니다.
- **어떤 경로는 비용이 커도 캐시하지 않는 편이 안전할까요?**
  - 어떤 경로는 비용이 커도 캐시하지 않는 편이 안전할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 캐시는 비용 절감 수단이면서 동시에 정확성 경계입니다. 같은 작업을 다시 계산하지 않으면 지연 시간과 토큰 사용량을 동시에 줄일 수 있습니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 운영 로그를 보면 반복은 생각보다 많습니다. FAQ형 챗봇, 비슷한 문장 교정을 반복하는 내부 도구, 같은 리포트를 여러 사용자가 보는 대시보드, 같은 질문을 조금씩 바꿔 재질문하는 대화 세션이 모두 그렇습니다.
- **흔히 헷갈리는 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 운영 로그를 보면 반복은 생각보다 많습니다. FAQ형 챗봇, 비슷한 문장 교정을 반복하는 내부 도구, 같은 리포트를 여러 사용자가 보는 대시보드, 같은 질문을 조금씩 바꿔 재질문하는 대화 세션이 모두 그렇습니다.
