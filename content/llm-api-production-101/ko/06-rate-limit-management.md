...
~~~
출력 결과
Python의 list와 tuple은 모두 순서가 있는 데이터 구조입니다. 하지만, 두 데이터 구조는 다음과 같은 차이점이 있습니다.

### 1. Mutable vs Immutable

- **List**: Mutable, 즉 변경이 가능합니다. 새로운 요소를 추가하거나, 기존 요소를 삭제하거나, 기존 요소를 수정할 수 있습니다.
- **Tuple**: Immutable, 즉 변경이 불가능합니다. 새로운 요소를 추가하거나, 기존 요소를 삭제하거나, 기존 요소를 수정할 수 없습니다.
...
- **List**: List는 동적 배열을 사용하기 때문에, 요소를 추가하거나 삭제할 때 시간이 걸립니다.
- **Tuple**: Tuple은 정적 배열을 사용하기 때문에, 요소를 추가하거나 삭제할 때 시간이 걸리지 않습니다.
...
### 4. 사용 예시
    ...
    try:
        ...
        print("Error: Tuple은 요소를 추가할 수 없습니다.")
    
    # 요소를 삭제할 수 없습니다.
    try:
        my_tuple.remove(2)
    except AttributeError:
        print("Error: Tuple은 요소를 삭제할 수 없습니다.")
    
    # 요소를 수정할 수 없습니다.
    try:
        my_tuple[0] = 10
    except TypeError:
        print("Error: Tuple은 요소를 수정할 수 없습니다.")
    ```
