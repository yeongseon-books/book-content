...
Rate Limit을 어떻게 관리해야 비용과 가용성을 동시에 지킬 수 있을까요?

이 글은 그 질문에 답하기 위해 Rate Limit 관리의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- RPM, TPM, 동시성 제한은 각각 무엇을 의미하고 어떻게 충돌하는가?
- 429 응답의 `Retry-After` 헤더와 자체 백오프는 어떻게 조합해야 하는가?
- 여러 모델/계정을 풀로 묶어 분산하는 전략은 언제 효과적인가?
- 토큰 버킷(token bucket) vs. leaky bucket 중 LLM API에 적합한 알고리즘은?
- 프로덕션에서 한도를 미리 감지하고 큐잉하려면 어떤 메트릭을 봐야 하는가?
...
- **token-level 한도를 의식** — 요청 수보다 token이 비용 신호입니다.
- **client-side throttling이 첫 방어** — provider 제한 도달 전에 우리가 조절합니다.
- **queue·priority가 안정성의 기반** — 공정성과 SLA를 동시에 만족합니다.
- **multi-key·multi-region 분산** — 단일 한도의 천장을 우회합니다.
- **관측·알림으로 조기 감지** — 한도 도달이 사용자에게 보이지 않게 합니다.

## 운영 체크리스트

- [ ] RPM/TPM/동시성 한도를 각 모델별로 표에 명시했다
- [ ] 429 응답 시 `Retry-After`를 우선 존중하는 클라이언트를 구현했다
- [ ] 선제적 rate limiter(token bucket)로 호출 전 한도를 차단했다
- [ ] 여러 키/계정 풀링 시 라우팅 규칙과 장애 격리를 정의했다
- [ ] 토큰 사용량과 한도 임박을 알리는 알람 임계치를 설정했다
