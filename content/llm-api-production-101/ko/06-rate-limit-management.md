...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/52"
...
이 글은 LLM API 프로덕션 101 시리즈의 마지막 글입니다.
...
여기서는 429가 오기 전에 요청 흐름을 제어하는 rate limit 관리 패턴을 봅니다.
...
- 흔히 헷갈리는 지점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
중요한 것은 제한기가 공급자 호출보다 먼저 실행된다는 사실입니다.
...
또한 429 처리도 여전히 중요합니다. 다만 429 처리는 proactive control을 대체하는 수단이 아니라, 로컬 제한기를 통과했음에도 외부 정책과 충돌했을 때의 reactive recovery여야 합니다. 이 두 층이 분리되어야 트래픽이 몰릴 때도 동작이 예측 가능합니다.
...
- **rate limit 대응은 429 뒤에 처리하는 일일까요, 429 전에 흐름을 조절하는 일일까요?**
  - rate limit 대응은 429 뒤에 처리하는 일일까요, 429 전에 흐름을 조절하는 일일까요 — 본문에서 구체적으로 다룹니다.
- **토큰 버킷과 슬라이딩 윈도우는 각각 어떤 트래픽에 맞을까요?**
  - 토큰 버킷과 슬라이딩 윈도우는 각각 어떤 트래픽에 맞을까요 — 본문에서 구체적으로 다룹니다.
- **provider 429를 받은 뒤에도 애플리케이션은 무엇을 해야 할까요?**
  - provider 429를 받은 뒤에도 애플리케이션은 무엇을 해야 할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 재시도와 백오프만으로는 rate limit 문제를 풀 수 없습니다. 그 방식은 이미 공급자 한도를 넘어선 뒤에야 반응합니다. 반면 로컬 제한기는 초과 가능성을 공급자 앞에서 흡수합니다. 즉, 429를 외부의 놀라운 예외가 아니라 내부 정책의 일부로 바꿔 줍니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 로컬 제한기가 유용한 이유는 세 가지입니다. 순간적인 트래픽 스파이크를 공급자 앞에서 흡수할 수 있고, 여러 내부 경로에서 합쳐지는 요청 흐름을 한 정책으로 제어할 수 있으며, rate limit을 원격 놀람이 아니라 애플리케이션 정책으로 다룰 수 있습니다.
