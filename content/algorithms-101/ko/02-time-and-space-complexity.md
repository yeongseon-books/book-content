...
**핵심 질문**: 알고리즘이 빠른지 느린지 어떻게 객관적으로 말할 수 있을까요? 컴퓨터마다, 입력마다 결과가 다른데 말입니다.

> 알고리즘의 비용은 입력 크기에 따른 증가율로 표현하는 것이 표준입니다. 빅오 표기법은 입력이 n일 때 연산 횟수가 어떻게 증가하는지 그 상한을 표시합니다. 시간 복잡도와 공간 복잡도를 함께 보고, 평균·최악·분할 상환을 구분하며, 이론과 실측을 함께 사용하는 것이 실전에서의 분석법입니다.
...
시니어 엔지니어는 빅오를 도구로 사용하지 종교로 사용하지 않습니다. n=10일 때는 어떤 알고리즘이든 같습니다. n이 충분히 커야 빅오의 차이가 실측에 드러납니다. 그래서 시니어는 "현재의 n 범위"와 "미래의 n 범위"를 함께 묻습니다.

또한 시니어는 빅오만으로 결정하지 않고 캐시 친화성, 분기 예측, 메모리 할당 패턴 같은 상수 요인도 함께 봅니다. 같은 O(n log n) 정렬이라도 mergesort보다 quicksort가 보통 빠른 이유입니다.
...
1. 다음 함수의 시간 복잡도를 분석하세요. (a) 이중 루프인데 안쪽이 i까지만 도는 경우, (b) `while n > 0: n -= 1` 후 다시 `for i in range(n): pass`. 정확히 빅오로 표기합니다.

2. Python `list.append`가 분할 상환 O(1)인 이유를 직접 측정으로 보이세요. 1, 10, 100, ..., 10^6번 append할 때 평균 시간을 출력하고 거의 일정함을 확인합니다.

3. 메모이제이션을 적용한 피보나치 함수의 공간 복잡도를 분석하세요. 시간은 O(n)이지만 공간도 O(n)입니다. 왜 그런지 설명합니다.
