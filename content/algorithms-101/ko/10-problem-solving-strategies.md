...
**핵심 질문**: 알고리즘을 잘한다는 것은 외운 풀이가 많다는 뜻일까요, 아니면 새 문제를 분해하는 사고 절차를 가졌다는 뜻일까요?

> 알고리즘 문제 풀이는 패턴 인식과 도구 선택의 게임입니다. 시리즈에서 다룬 정렬·탐색·재귀·DP·그리디·그래프·문자열은 도구함이며, 새 문제를 만나면 입력 크기·구조·제약을 통해 어떤 도구가 적합한지를 좁혀 갑니다. 시니어 엔지니어는 외운 풀이의 양보다 사고 절차의 일관성으로 다른 사람과 구분됩니다. 이 글은 시리즈를 정리하면서 그 사고 절차를 명시화합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 알고리즘 문제를 만났을 때의 표준 사고 절차
- 입력 크기로부터 시간 복잡도 상한 추정하기
- 패턴 인식: 어떤 신호가 어떤 도구를 부르는가
- 면접·실무에서 알고리즘을 잘 다룬다는 것의 의미
...
| 용어 | 설명 |
| --- | --- |
| 문제 진술 | 자신의 언어로 문제를 다시 적기 |
| 허용 복잡도 | 시간 제한과 입력 크기로 추정 |
| 패턴 인식 | 입력 구조에서 도구를 떠올림 |
| 작은 예제 검증 | 코드 전 손으로 단계 추적 |
| Sanity test | 경계·빈 입력·최대 입력 점검 |
...
시니어는 새 문제를 만났을 때 즉시 코드를 쓰지 않습니다. 입력 크기와 구조를 보고, 도구함에서 후보를 좁히고, 작은 예제로 검증한 뒤에야 키보드를 잡습니다. 이 절차는 면접뿐 아니라 운영 환경의 알고리즘 결정에도 그대로 적용됩니다.

또한 시니어는 알고리즘을 외운 양으로 자신을 평가하지 않습니다. 모르는 문제를 만나도 절차에 따라 안정적으로 분해할 수 있다는 자신감이 곧 실력의 정의입니다. 도구함은 늘어날수록 좋지만, 사고 절차의 일관성이 본질입니다.
...
1. 이 시리즈에서 다룬 알고리즘 도구를 한 표로 정리하세요. 각 도구의 평균/최악 복잡도, 적용 신호, 한계를 한 줄씩 적습니다. 이 표 자체가 강력한 학습 산출물입니다.

2. 외부 알고리즘 문제 한 개를 골라 다섯 단계 절차를 글로 적은 뒤 풀이를 작성하세요. 절차의 어느 단계가 가장 어려웠는지를 기록합니다.

3. 동료(또는 자신)에게 "어떤 신호를 보면 그래프 알고리즘을 떠올리는가?"를 5분간 설명하세요. 설명할 수 있다면 그 도구를 자기 것으로 만든 것입니다.
