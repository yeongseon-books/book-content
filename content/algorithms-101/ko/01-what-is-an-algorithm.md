...
- **알고리즘이 반드시 만족해야 하는 세 가지 조건은 무엇일까요?**
  - 이 글의 답은 정확성, 유한성, 효율성입니다. `gcd(48, 36)` 예시처럼 올바른 값을 내야 하고, `b`가 줄어들며 반드시 끝나야 하며, 입력 크기 `n`이 커질 때 비용을 설명할 수 있어야 합니다.
- **알고리즘과 프로그램은 어떻게 다를까요?**
  - 알고리즘은 `Problem → Algorithm → Program → Execution` 도식에서 보듯 문제를 푸는 추상적 절차이고, 프로그램은 그 절차를 Python 같은 언어로 옮긴 구현입니다. 같은 `find` 문제도 선형 탐색과 `bisect.bisect_left` 구현이 서로 다른 알고리즘을 담을 수 있다는 점이 차이를 잘 보여 줍니다.
- **왜 알고리즘 설계에서는 의사코드가 중요한 작업 언어가 될까요?**
  - `find smallest in array` 의사코드처럼 절차를 먼저 적으면 논리와 문법을 분리할 수 있습니다. 그래서 본문도 코드보다 먼저 “무엇을 어떤 순서로 할지”를 명확히 잡는 습관을 강조했습니다.
