...
**핵심 질문**: 재귀는 왜 어렵게 느껴질까요? 그리고 mergesort 같은 알고리즘이 빠른 진짜 이유는 무엇일까요?

> 재귀는 문제를 자기 자신과 같은 형태의 더 작은 문제로 표현하는 기법입니다. 정확히 동작하려면 베이스 케이스, 진행성, 상태 공유 회피 세 가지를 지켜야 합니다. 분할 정복은 재귀의 대표적 사고 패턴으로, 입력을 나누고(divide), 부분을 풀고(conquer), 결과를 결합(combine)합니다. 비용은 점화식으로 분석하며, 같은 부분 문제가 반복되면 메모이제이션으로 동적 계획법으로 진화합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 재귀가 정확히 동작하기 위한 세 가지 조건
- 호출 스택과 RecursionError의 정체
- 분할 정복의 일반 형태와 점화식 분석
- 같은 부분 문제가 반복될 때 메모이제이션으로 가는 길
...
| 용어 | 설명 |
| --- | --- |
| 베이스 케이스 | 더 이상 재귀 호출하지 않는 종료 조건 |
| 호출 스택 | 함수 호출의 컨텍스트가 쌓이는 영역 |
| 분할 정복 | divide → conquer → combine의 패턴 |
| 점화식 | 재귀 비용을 수식으로 표현 |
| 메모이제이션 | 같은 부분 문제 결과 캐싱 |
...
시니어는 "재귀로 자연스러운가, 반복으로 자연스러운가"를 함수의 의도로 판단합니다. 트리 구조는 거의 항상 재귀가 깔끔하지만, 깊이가 매우 깊거나 성능 민감 경로라면 반복문 + 명시적 스택으로 옮깁니다.

또한 시니어는 분할 정복을 만나면 점화식을 머릿속에서 즉시 그립니다. T(n) = 2T(n/2) + O(n)은 O(n log n), T(n) = T(n/2) + O(1)은 O(log n) — 이 두 형태만 외워두어도 대부분의 분석이 끝납니다.
...
1. 정수 배열의 합을 분할 정복으로 구하는 함수를 작성하고, 점화식과 복잡도를 분석하세요. 단순 반복합과의 차이도 함께 정리합니다.

2. 두 정렬된 리스트의 교집합을 재귀 또는 반복으로 구현하세요. 입력 크기가 다를 때 더 효율적인 접근(이진 탐색 결합)도 함께 작성해 봅니다.

3. 하노이의 탑 문제를 푸는 함수를 작성하고, 호출 횟수가 2^n - 1임을 점화식으로 증명하세요. 분할 정복의 고전 사례입니다.
