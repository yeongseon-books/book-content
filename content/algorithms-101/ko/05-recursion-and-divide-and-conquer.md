...
재귀는 처음엔 낯설지만, 복잡한 문제를 작은 문제로 바꾸는 가장 강력한 사고 도구 가운데 하나입니다. 호출 스택과 종료 조건을 함께 이해하면 재귀는 갑자기 훨씬 선명해집니다.

이 글은 Algorithms 101 시리즈의 5번째 글입니다. 여기서는 재귀의 정의와 호출 스택, 분할 정복의 사고 패턴, 그리고 점화식으로 비용을 읽는 법을 정리합니다.
