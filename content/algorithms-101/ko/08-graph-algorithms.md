...
tags:
  ...
  - 다익스트라
  - 최소 신장 트리
seo_description: 그래프 표현, BFS와 DFS의 사용처, 다익스트라의 최단 경로, 그리고 크루스칼·프림의 최소 신장 트리를 실전 코드와 함께 정리합니다.
last_reviewed: '2026-05-11'
---

# 그래프 알고리즘

그래프는 추상 수학 주제가 아니라 실무 시스템을 설명하는 공용 언어에 가깝습니다. 서비스 의존성, 추천, 라우팅처럼 관계가 핵심인 문제는 결국 그래프 관점에서 풀리기 시작합니다.

이 글은 Algorithms 101 시리즈의 8번째 글입니다. 여기서는 그래프 표현, BFS와 DFS의 사용처, 다익스트라의 최단 경로, 그리고 최소 신장 트리를 함께 정리합니다.

## 이 글에서 다룰 문제

대부분의 실무 문제는 결국 그래프로 환원됩니다. 마이크로서비스의 호출 의존성, 빌드 도구의 태스크 그래프, 추천 시스템의 사용자-아이템 이분 그래프, 라우팅 시스템의 도로망까지 모두 같은 관점에서 볼 수 있습니다. 그래프 알고리즘을 모르면 이런 시스템의 핵심 구조를 다루기 어렵습니다.

> 그래프는 시스템 사고를 설명하는 공용 언어입니다.
...
Before — 희소 그래프를 인접 행렬로 표현해 메모리를 낭비합니다.

```python
# V=10000, E=30000인 희소 그래프를 행렬로 표현
...
BFS는 가중치가 없는 그래프에서 최단 거리를 자연스럽게 구합니다. 큐는 `deque`를 쓰면 양끝에서 O(1)로 처리할 수 있습니다.
...
def topological_sort(n, edges):
    ...
    while q:
        ...
        for v in adj[u]:
            ...
            if indeg[v] == 0:
                q.append(v)
    return order if len(order) == n else None   # None을 반환하면 사이클이 있다는 뜻

# A → B, A → C, B → D, C → D
print(topological_sort(4, [(0, 1), (0, 2), (1, 3), (2, 3)]))   # [0, 1, 2, 3]
```

빌드 시스템의 실행 순서나 학습 과목의 선수 관계 같은 문제는 위상 정렬로 자연스럽게 풀립니다.
...
간선 가중치가 음수가 아니라면 다익스트라가 표준 선택입니다. 음의 가중치가 섞이면 Bellman-Ford나 Johnson 같은 다른 알고리즘을 써야 합니다.
...
크루스칼은 가장 가벼운 간선부터 보면서 사이클을 만들지 않을 때만 선택합니다. 그리디와 Union-Find를 결합한 고전적인 패턴입니다.
...
MST는 통신망 설계, 클러스터 거리 분석, 회로 배선 같은 실무 문제에 거의 그대로 적용됩니다.
...
그래프 알고리즘은 노드와 간선이라는 단순한 어휘로 다양한 시스템 문제를 표현하게 해줍니다. 표현, 탐색, 최단 경로, MST의 네 가지 도구만 익혀도 실무의 많은 문제를 풀 수 있습니다. 더 깊이 들어가면 흐름 네트워크, 강한 연결 요소, 매칭 같은 고급 주제로 자연스럽게 확장됩니다.

다음 글에서는 문자열 알고리즘 기초를 살펴봅니다. 단순 매칭의 비용, KMP의 실패 함수, Z 함수, 그리고 실무에서 자주 쓰이는 정규식과 트라이까지 함께 다루겠습니다.
