...
- **비교 기반 정렬은 왜 O(n log n)보다 더 좋아질 수 없을까요?**
  - 본문이 적은 `log(n!) ≈ n log n`이 바로 그 이유입니다. 비교 결과만으로 순서를 결정하는 한 가능한 경우의 수를 구분해야 하므로, 비교 기반 정렬은 그 하한 아래로 내려갈 수 없습니다.
- **mergesort, quicksort, heapsort는 각각 무엇을 주고 무엇을 얻을까요?**
  - `mergesort()`는 안정성과 보장된 O(n log n)을 주는 대신 O(n) 추가 메모리를 요구하고, `quicksort_inplace()`는 평균적으로 빠르지만 pivot이 나쁘면 O(n^2)까지 무너질 수 있으며, `heapsort()`는 안정 정렬은 아니지만 최악 시간 보장을 줍니다. 그래서 본문도 세 알고리즘을 성능 한 줄이 아니라 trade-off 표로 비교했습니다.
- **안정 정렬과 비안정 정렬의 차이는 왜 중요할까요?**
  - `people.sort(key=lambda p: p[0])` 후에 다시 `people.sort(key=lambda p: p[1])`를 해도 상대 순서가 유지되는 것은 Python의 Timsort가 안정 정렬이기 때문입니다. 다중 키 정렬을 간단히 쓰려면 이 성질이 반드시 필요합니다.
