...
**핵심 질문**: 텍스트 안에서 패턴을 찾는 일은 단순해 보이지만 왜 이렇게 많은 알고리즘이 존재할까요?

> 문자열 매칭은 거의 모든 소프트웨어가 다루는 작업이지만, 단순 비교는 최악 O(nm)으로 느릴 수 있습니다. KMP는 실패 함수로 같은 비교를 반복하지 않게 만들어 O(n+m)으로 줄입니다. 트라이는 다중 패턴 검색과 자동완성의 표준 자료구조이며, 정규식은 표현력은 높지만 백트래킹 비용이 폭발할 수 있다는 점을 알아야 합니다. 문자열 알고리즘은 직관과 비용 감각을 같이 다뤄야 합니다.
...
| --- | --- |
| 패턴 | 텍스트 안에서 찾으려는 문자열 |
| 실패 함수(failure function) | KMP의 prefix-suffix 일치 길이 |
| 트라이 | prefix를 공유하는 트리 자료구조 |
| Aho-Corasick | 트라이 + 실패 링크의 다중 패턴 매칭 |
| ReDoS | 백트래킹 정규식의 지수 시간 공격 |
...
시니어는 문자열 작업에서 "패턴이 짧고 일회성인가, 길거나 반복적인가"를 먼저 봅니다. 짧고 일회성이면 표준 라이브러리로 충분합니다. 패턴이 많거나 반복되면 트라이·KMP·Aho-Corasick의 적용을 고민합니다.

또한 시니어는 정규식 입력의 출처를 항상 의식합니다. 사용자 입력이나 외부 데이터에 정규식을 적용할 때는 패턴을 단순화하고, 가능하다면 선형 시간 엔진을 사용하며, 항상 timeout을 둡니다. ReDoS는 실제 운영 사고로 자주 등장합니다.
...
1. KMP의 실패 함수만 활용해 패턴이 텍스트에 등장하는 모든 위치를 반환하는 함수를 작성하세요. 첫 번째 매치만 찾는 코드를 확장하면 됩니다.

2. 트라이를 활용해 단어 사전과 prefix가 주어졌을 때 사전순으로 정렬된 prefix 자동완성 결과를 반환하는 함수를 작성하세요. 결과가 많으면 상위 k개만 반환하도록 확장합니다.

3. 임의의 정규식 패턴이 ReDoS 위험이 있는지 정적으로 점검하는 간단한 휴리스틱을 설계하세요. "중첩 양화사 존재" 같은 규칙으로 시작합니다.
...
Tags: Computer Science, 알고리즘, 문자열, KMP, 트라이, 정규식
