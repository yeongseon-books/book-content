...
문자열 처리는 단순해 보여도 입력이 커지는 순간 비용이 급격히 드러납니다. 검색, 로그 분석, 자동완성, 정규식 검증 같은 작업은 적절한 알고리즘이 없으면 금방 병목이 됩니다.

이 글은 Algorithms 101 시리즈의 9번째 글입니다. 여기서는 단순 매칭의 비용, KMP의 실패 함수, 트라이 자료구조, 그리고 정규식 비용 감각을 함께 정리합니다.
