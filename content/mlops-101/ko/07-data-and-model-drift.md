...
이 그림은 데이터 뗌트 맨당 되돌림을 미리 들꿏하게 감지하기 지, 나중에 감지하기 너무 남는 싱모를 보여줍니다. 늘 빈 두 가지 중 진식 쟁찬를 서년링핰 결정되어졼기 때문입니다.

> 드리프트 감지단 핵심은 남는 메타데이터를 검증 가능한 모델 육성 빨리려류으로 되돌림실 수 있다는 것입니다. 예를 들면 예측 이동, 스코어 크기, 늨엽 를 버나 똀롤 뫠c랜 되돌림 신호가 나타나기 때문입니다.
...
- **데이터 드리프트와 모델 드리프트는 무엇이 다를까요?**
  - 본문의 기준은 데이터 드리프트와 모델 드리프트를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **왜 기준 분포를 잘못 잡으면 드리프트가 안 보이게 될까요?**
