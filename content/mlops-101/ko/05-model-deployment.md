...
이 패턴의 핵심은 배포 코드가 모델 파일 경로를 직접 알 필요가 없다는 사실입니다. 레지스트리가 "Production 스테이지에 해당하는 최신 버전"을 관리하므로, 배포 코드는 스테이지 이름만 참조합니다.
...
- **학습된 모델 파일을 어떻게 사용자 요청에 연결할 수 있을까요?**
  - 본문 예제처럼 `model.pkl`을 시작 시 한 번 로드하고, FastAPI의 `/predict` 엔드포인트에서 Pydantic 요청 스키마를 거쳐 `model.predict()`를 호출하면 모델이 HTTP 요청 경로에 연결됩니다. 이렇게 해야 입력 검증, 버전 응답, 예측 로그를 모델 파일 주위에 함께 둘 수 있습니다.
- **온라인 추론, 배치 추론, 스트리밍 추론은 어떤 차이로 이해하면 좋을까요?**
  - 온라인 추론은 `/predict`처럼 요청마다 즉시 응답해야 하는 경로이고, 배치 추론은 Airflow나 Cron으로 대량 데이터를 한 번에 처리하는 방식이며, 스트리밍 추론은 Kafka 같은 이벤트 흐름을 계속 읽는 형태입니다. 본문 표가 보여 준 것처럼 세 방식은 지연 시간과 처리량 요구가 달라서, 같은 모델이라도 운영 형태가 달라질 수 있습니다.
- **FastAPI와 Docker는 모델 배포에서 어떤 역할을 할까요?**
  - FastAPI는 모델을 사용자 요청이 부를 수 있는 API 계약으로 바꾸고, Docker는 그 API와 라이브러리 환경을 `model-api:1.0.0` 같은 재현 가능한 배포 단위로 묶습니다. 그래서 헬스 체크, 이미지 태그, 카나리, 롤백 같은 운영 절차가 같은 버전 기준 위에서 돌아갈 수 있습니다.
