...
- **앞선 아홉 개 구성 요소는 실제 시스템에서 어떻게 연결될까요?**
  - 이 글은 데이터 버전 관리와 피처 스토어가 입력 일관성을 만들고, DAG 기반 학습과 모델 레지스트리가 아티팩트를 관리하며, 배포·모니터링·드리프트 감지·재학습이 다시 학습 루프로 되돌아가는 닫힌 구조를 정리했습니다. 즉 각 장의 도구는 따로 존재하는 것이 아니라, 다음 단계의 입력과 판단 근거를 만들어 주는 연결 부품입니다.
- **왜 도구를 각각 아는 것만으로는 운영 체계가 되지 않을까요?**
  - MLflow, Feast, Prometheus를 각각 써도 승격 기준, 롤백 규칙, 점검표, 통합 테스트가 없으면 시스템은 사람 기억에 의존합니다. 본문에서 `scorecard`, `missing()`, `next_step()` 같은 코드를 둔 이유도, 도구 목록을 운영 체크리스트와 우선순위로 바꾸기 위해서입니다.
- **런북, 온콜, SLI/SLO는 기술 요소와 어떻게 맞물릴까요?**
  - 런북과 온콜은 Prometheus 경고나 드리프트 알림이 울렸을 때 누가 어떤 순서로 대응할지 정해 주는 운영 계층이고, SLI/SLO는 그 경고가 왜 중요한지 기준을 제공합니다. 그래서 운영 가능한 ML 시스템은 모델 서빙 코드만이 아니라, 5분 내 롤백 가능성·대시보드·포스트모템까지 함께 갖춰야 완성됩니다.
