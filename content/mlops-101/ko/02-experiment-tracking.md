...
이 코드에서 중요한 점은 데이터 버전, 파라미터, 모델, 메트릭, 시각화가 모두 하나의 run 안에 묶여 있다는 사실입니다. 나중에 이 run을 열면 모든 결과를 함께 볼 수 있습니다.
...
- **실험 관리가 없으면 왜 같은 모델도 다시 만들기 어려울까요?**
  - 같은 로지스틱 회귀라도 `C`, `max_iter`, `data_version`, `git_sha` 같은 실행 조건이 빠지면 어떤 run이 좋은 결과를 만들었는지 다시 복원할 수 없습니다. 실험 관리가 없으면 `model.pkl`과 숫자 하나만 남고, 왜 그 메트릭이 나왔는지 설명할 축이 사라집니다.
- **파라미터, 메트릭, 아티팩트, 환경 중 무엇을 반드시 남겨야 할까요?**
  - 본문은 최소 단위로 param, metric, artifact를 모두 같은 run 안에 묶고, 여기에 `data_version`과 `git_sha`까지 붙여야 한다고 설명했습니다. MLflow 예제에서도 `model.pkl`, `acc.png`, `val_acc`를 함께 남겼기 때문에 나중에 상위 run 비교와 승격 판단이 가능해집니다.
- **MLflow에서 experiment와 run은 어떤 관계로 이해하면 좋을까요?**
  - experiment는 `fraud-detection-baseline`처럼 관련 실험을 모아 두는 상자이고, run은 그 안에서 실제 학습 한 번을 뜻합니다. 그래서 부모 run 아래에 nested run으로 스윕을 두거나, 여러 run을 `metrics.val_auc DESC`로 정렬해 비교하는 흐름이 자연스럽게 이어집니다.

<!-- toc:begin -->
## 시리즈 목차

- [MLOps 101 (1/10): MLOps란 무엇인가?](./01-what-is-mlops.md)
- **MLOps 101 (2/10): 실험 관리 (현재 글)**
