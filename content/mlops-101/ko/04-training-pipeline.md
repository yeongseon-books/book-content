...
수집 단계는 raw 입력을 만들어 다음 단계에 넘깁니다. 중요한 것은 이 함수가 어떤 파일을 만들었는지 명확히 드러난다는 사실입니다.
...
이 구조의 핵심은 각 단계가 실패 원인을 분리할 수 있도록 역할이 분명하다는 사실입니다.
...
- **학습 스크립트 하나를 여러 단계 파이프라인으로 나누는 이유는 무엇일까요?**
  - `ingest`, `preprocess`, `train`, `evaluate`를 분리하면 실패 지점을 바로 좁히고, 바뀐 입력이 있는 구간만 다시 실행할 수 있습니다. 특히 평가 단계를 학습과 분리해야 성능 기준 미달 모델을 레지스트리 등록 전에 차단하는 운영 경계가 생깁니다.
- **DAG는 단순 실행 순서와 어떻게 다를까요?**
  - DAG는 단순히 "위에서 아래로 실행"하는 목록이 아니라, 단계 간 의존 관계와 재실행 범위를 명시하는 구조입니다. 그래서 Airflow 예제의 `validate_schema >> train >> evaluate >> register` 같은 연결은 순서만 아니라, 어디서 멈추고 어디부터 재시작할지까지 드러냅니다.
- **Airflow, Prefect, Kubeflow 같은 오케스트레이터는 어디에 들어갈까요?**
  - 이 도구들은 단계 경계를 대신 설계해 주는 것이 아니라, 이미 분리된 단계를 스케줄링하고 재시도하고 알림을 붙이는 실행 계층입니다. 본문에서도 핵심은 오케스트레이터 이름보다 작은 stage, 멱등성, 캐싱, 실패 로그를 먼저 갖추는 데 있다고 정리했습니다.
...
- **MLOps 101 (4/10): 모델 학습 파이프라인 (현재 글)**
