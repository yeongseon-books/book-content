...
- 재시도와 캐싱은 왜 학습 파이프라인에서 중요할까요?
- 어떤 설계 실수가 파이프라인 전체를 불필요하게 다시 돌리게 만들까요?

> 멘탈 모델: 학습 파이프라인은 하나의 큰 학습 스크립트를 자동 실행하는 장치가 아니라, 입력과 출력이 분리된 작은 단계들을 DAG로 연결해 재실행 범위를 줄이는 구조입니다.
