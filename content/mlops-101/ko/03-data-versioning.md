...
여기서는 아주 작은 CSV를 만듭니다. 실제 현업 데이터는 훨씬 크겠지만, 버전 관리 원리는 같습니다. 중요한 것은 파일 크기가 아니라 상태를 식별할 수 있어야 한다는 사실입니다.
...
이 구조의 장점은 협업자가 "같은 코드 + 같은 데이터" 상태를 기계적으로 복원할 수 있다는 사실입니다.
...
핵심은 이 파일이 데이터 자체가 아니라 "데이터 상태를 가리키는 포인터"라는 사실입니다.
...
- **왜 코드 버전만으로는 학습 결과를 재현할 수 없을까요?**
  - 같은 `train.py`라도 `data.csv` 내용이 달라지면 해시가 바뀌고, 그 순간 다른 모델이 만들어집니다. 본문이 강조한 재현성은 코드 커밋만이 아니라 `.dvc` 포인터나 `md5` 같은 데이터 상태까지 함께 고정될 때 비로소 성립합니다.
- **DVC와 git-LFS는 어떤 차이로 이해하면 좋을까요?**
  - git-LFS는 큰 파일을 저장소 바깥에 두는 데는 유용하지만, DVC처럼 `deps`, `outs`, `dvc repro`로 파이프라인 재현까지 연결해 주지는 않습니다. 그래서 수백 MB 바이너리 보관이면 git-LFS도 가능하지만, GB 단위 학습 데이터와 단계 재실행이 필요하면 DVC가 더 잘 맞습니다.
- **큰 데이터 파일은 git 바깥에 두면서도 어떻게 버전 일관성을 유지할 수 있을까요?**
  - 본문 예제처럼 git에는 `data.csv.dvc` 같은 포인터와 파이프라인 정의만 커밋하고, 실제 파일은 S3 같은 원격 저장소에 `dvc push`로 올리면 됩니다. 다른 환경은 `git pull && dvc pull`로 같은 상태를 복원하므로, 코드와 데이터가 같은 릴리스 단위로 움직이게 됩니다.
