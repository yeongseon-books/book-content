...
**핵심 질문**: *언제* *재학습 해야 할지* 와 *재학습한 모델이 정말 더 나은지* 를 *어떻게 결정* 할까요?

> *재학습 은 *트리거* 에 따라 *학습 → 평가 → 챔피언 비교 → 승급* 을 *자동* 으로 수행합니다.*
...
- **Champion**: *현재 운영* 모델.
- **Challenger**: *새로 학습* 한 모델.
- **Shadow**: *예측만* 하고 *반영 안 함*.
- **Promotion**: *Challenger* → *Champion*.
- **Hysteresis**: *떨림 방지* 임계 폭.
...
1. *떨림 방지* 를 위한 *최소 안정 기간* 을 추가하세요.
2. *섀도우 결과* 가 나쁘면 어떻게 학습 데이터를 보정할까요?
3. *모델이 동률* 일 때 *챔피언 유지* 가 합리적인 이유는?
