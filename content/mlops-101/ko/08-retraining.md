...
핵심은 `assign_variant`가 deterministic이라는 사실입니다. 같은 사용자는 실험 기간 동안 항상 같은 그룹에 속하므로, 결과가 일관됩니다.
...
- **언제 재학습해야 하는지를 어떤 신호로 정할 수 있을까요?**
  - 본문은 `psi`, `accuracy`, `days_since_train`, `new_rows` 같은 신호를 함께 보고 `drift`, `performance`, `schedule`, `data_volume` 이유를 코드로 반환하게 했습니다. 즉 재학습 시점은 사람 감각이 아니라, 문서화된 운영 규칙과 메트릭 스냅샷으로 정하는 것이 핵심입니다.
- **일정 기반, 드리프트 기반, 성능 기반 트리거는 어떻게 다를까요?**
  - 일정 기반은 30일처럼 시간을 기준으로 루틴하게 돌리고, 드리프트 기반은 PSI나 입력 변화가 임계값을 넘을 때 반응하며, 성능 기반은 AUC나 정확도가 기준 아래로 떨어질 때 작동합니다. 글의 요지는 세 신호를 섞되 우선순위를 정해, 잦은 재학습과 늦은 재학습 사이에서 균형을 잡아야 한다는 사실입니다.
- **챔피언과 챌린저를 비교할 때 왜 마진이 필요할까요?**
  - `compare(challenger_acc, champion_acc, margin=0.01)` 예제처럼 최소 개선 폭을 두지 않으면 우연한 0.001 차이로도 모델이 계속 교체됩니다. 마진은 재학습 자동화를 느리게 만드는 장치가 아니라, 섀도우 평가와 함께 운영 안정성을 지키는 완충 장치입니다.
