...
- **일정 기반, 드리프트 기반, 성능 기반 트리거는 어떻게 다를까요?**
  - 일정 기반은 30일처럼 시간을 기준으로 루틴하게 돌리고, 드리프트 기반은 PSI나 입력 변화가 임계값을 넘을 때 반응하며, 성능 기반은 AUC나 정확도가 기준 아래로 떨어질 때 작동합니다. 글의 요지는 세 신호를 섞되 우선순위를 정해, 잦은 재학습과 늦은 재학습 사이에서 균형을 잡아야 한다는 점입니다.
- **챔피언과 챌린저를 비교할 때 왜 마진이 필요할까요?**
