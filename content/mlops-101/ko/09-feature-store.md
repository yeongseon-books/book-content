...
- **학습-서빙 불일치는 왜 자꾸 반복될까요?**
  - 학습 노트북은 pandas 집계를 쓰고 서빙 코드는 다른 SQL이나 실시간 계산식을 쓰면, 같은 `age`나 `purchase_cnt_7d`라는 이름도 실제 값은 달라지기 쉽습니다. 본문이 피처 스토어를 저장소가 아니라 정의 공유 계층이라고 본 이유가 바로 이 반복되는 계산 경로 차이 때문입니다.
- **온라인 저장소와 오프라인 저장소는 어떤 역할 차이가 있을까요?**
  - 오프라인 저장소는 `get_historical_features()`처럼 학습용 대량 추출과 시점 일치 조인을 담당하고, 온라인 저장소는 `get_online_features()`처럼 실시간 추론에 필요한 최신 피처를 밀리초 단위로 돌려줍니다. 그래서 본문은 두 저장소가 같은 정의를 공유하되, 지연 시간과 품질 기준은 다르게 관리해야 한다고 설명했습니다.
- **Feast에서 entity와 feature view는 어떻게 이해하면 좋을까요?**
  - entity는 `user_id`나 `customer_id`처럼 피처를 붙일 조인 키이고, feature view는 그 키에 대해 어떤 소스에서 어떤 피처 묶음을 읽을지 선언한 계약입니다. `user_behavior_v1` 예제처럼 TTL, schema, source가 한곳에 모여 있어야 학습과 서빙이 같은 피처 이름을 같은 의미로 사용하게 됩니다.
