...
한국어 검색이나 RAG가 텍스트 문서만 다룬다고 가정하면 실제 업무 데이터를 절반만 보는 셈입니다. 영수증, 세금계산서, 스캔 문서처럼 이미지로 들어오는 자료는 OCR 한 번만 어긋나도 이후 임베딩과 검색 품질이 함께 무너집니다.

이 글은 한국어 AI 스택 101 시리즈의 4번째 글입니다. 여기서는 CLOVA OCR API 응답을 줄 단위 텍스트로 다시 조립하고, 그 결과를 검색 코퍼스로 넘기기 전에 무엇을 검증해야 하는지 다룹니다.

> 처음으로 쓸 만한 OCR 결과물은 평문이 아니라, 의미 있는 줄로 다시 조립해야 하는 구조화된 추출 payload입니다.
