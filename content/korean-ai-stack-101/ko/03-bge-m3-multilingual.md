...
한국 회사의 검색 시스템은 쿼리는 한국어인데 문서는 영어가 절반인 경우가 흔합니다. 이때 단일 언어 모델 기준으로는 검색이 멀쩡해 보여도, 실제 운영에서는 영어 runbook이나 매뉴얼이 빠지는 문제가 바로 드러납니다.

이 글은 한국어 AI 스택 101 시리즈의 3번째 글입니다. 여기서는 BGE-M3를 사용해 한국어·영어 혼합 코퍼스를 같은 벡터 공간으로 다루는 방법과 dense baseline을 측정하는 기준을 설명합니다.

> 다국어 검색을 시작할 때 가장 먼저 할 일은 화려한 multi-vector 신호를 합치는 것이 아니라, dense baseline을 깔끔하게 측정하는 일입니다.
...
# top-1 결과: 'Kubernetes rollback playbook for failed deploys' (score 0.78, en)
