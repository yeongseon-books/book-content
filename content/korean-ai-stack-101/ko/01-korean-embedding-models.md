---
title: "Korean AI Stack 101 (1/6): 한국어 임베딩 모델 비교 — KoSimCSE, BGE-M3, Solar"
...
- 핵심 흐름에서 가장 흔한 실수는 무엇일까요?
- 왜 재현 가능한 비교부터 시작할까을 실무에 적용할 때 주의할 점은 무엇일까요?
- 비교 프레임을 먼저 고정하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **영어 중심 임베딩 모델은 한국어 비중이 높은 데이터에서 어디서 자주 무너질까요?**
  - 한국어 짧은 문장 유사도와 무관 문장 분리에서 먼저 흔들리기 쉽습니다. 특히 띄어쓰기 변형, 고객 문의처럼 짧은 표현, 한국어 의미는 비슷하지만 표면형이 다른 문장에서 gap이 좁아지면 검색 임계값을 잡기 어려워집니다. 그래서 이 글에서는 한국어 유사 쌍과 무관 쌍을 먼저 나눠 비교했습니다.
- **코사인 점수 하나보다 유사 쌍과 무관 쌍 사이의 간격이 왜 더 쓸모 있을까요?**
  - 운영에서는 높은 점수 하나보다 유사 쌍 평균과 무관 쌍 평균이 얼마나 벌어지는지가 더 중요합니다. 이 간격이 넓어야 threshold를 정할 수 있고, 무관한 문장을 상위 후보로 올리는 오탐도 줄일 수 있습니다. 그래서 표에서도 `gap(sim-unrel)`과 최솟값 분포를 함께 봤습니다.
- **한국어 텍스트에 영어 기술 용어가 자주 섞일 때는 무엇부터 시험해야 할까요?**
  - mixed 쌍을 따로 만들어 한국어 질의와 영어 용어가 섞인 문장이 얼마나 안정적으로 붙는지 먼저 확인해야 합니다. 이 실험에서 KoSimCSE는 한국어 분리력이 강하고, BGE-M3와 Solar는 혼합 언어 내구성이 더 안정적으로 보였습니다. 즉 모델 이름보다 우리 코퍼스가 한국어 중심인지, 한영 혼합인지부터 시험 순서를 정하는 편이 맞습니다.

<!-- toc:begin -->
## 시리즈 목차

- **Korean AI Stack 101 (1/6): 한국어 임베딩 모델 비교 — KoSimCSE, BGE-M3, Solar (현재 글)**
