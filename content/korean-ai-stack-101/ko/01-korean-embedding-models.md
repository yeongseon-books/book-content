...
한국어 임베딩 모델을 고를 때 가장 어려운 지점은 벤치마크 순위를 읽는 일이 아니라, 우리 데이터에서 어떤 모델이 더 덜 흔들리는지 판단하는 일입니다. 한국어 FAQ, 한영 혼용 문서, 임계값 기반 검색은 서로 다른 실패 패턴을 만들기 때문에, 첫 글에서는 비교 기준부터 단단하게 세워 둘 필요가 있습니다.

이 글은 한국어 AI 스택 101 시리즈의 첫 번째 글입니다. 여기서는 한국어 임베딩 모델을 비교할 때 무엇을 기준으로 봐야 하는지, 그리고 재현 가능한 비교 프레임을 어떻게 잡아야 하는지 다룹니다.
