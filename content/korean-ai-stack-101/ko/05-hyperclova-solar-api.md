...
- 왜 이 단계가 중요한가에서 가장 흔한 실수는 무엇일까요?
...
- **프롬프트 튜닝보다 먼저 고정해야 할 API 계약은 무엇일까요?**
  - 호출 계약, 메시지 계약, 샘플링 계약, 응답 계약을 먼저 고정해야 합니다. 다시 말해 인증과 endpoint, system/user 메시지 구조, `temperature`와 토큰 상한, JSON 검증과 마스킹까지 한 세트로 묶어 두어야 합니다. 이 틀이 고정돼야 프롬프트 문구를 바꿨을 때도 무엇이 품질을 바꿨는지 읽을 수 있습니다.
- **HyperCLOVA X나 Solar 같은 한국어 생성 API를 도입할 때는 무엇부터 검증해야 할까요?**
  - 한국어 유창성보다 먼저 인증, timeout, retry, 출력 형식, refusal/PII 마스킹이 안정적인지 확인해야 합니다. 이 글에서도 JSON 강제, 길이 검증, 금지 문구 검사, 상태 코드별 재시도 정책을 먼저 다뤘습니다. 운영에서 더 자주 터지는 문제는 표현력 부족보다 호출 계약 불안정 쪽이기 때문입니다.
- **실행 예제가 왜 Groq `llama-3.1-8b-instant`를 대체 모델로 쓰는 걸까요?**
  - 독자가 HyperCLOVA X나 Solar 키가 없어도 system 메시지, 샘플링, JSON 응답 검증 같은 오래 가는 패턴을 바로 실습할 수 있기 때문입니다. Groq 예제에서 익힌 구조는 Solar에서는 `base_url` 변경으로 거의 그대로 옮겨 가고, HyperCLOVA X에서도 헤더와 endpoint만 바꾸면 같은 사고방식을 유지할 수 있습니다. 즉 대체 모델은 개념을 희석하는 우회가 아니라 공급자 독립적인 기준선을 만드는 장치입니다.
