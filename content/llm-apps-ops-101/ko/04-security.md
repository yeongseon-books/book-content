...
LLM 앱의 보안 문제는 모델 응답이 나온 뒤에야 드러나면 이미 늦는 경우가 많습니다. 위험한 입력을 모델 앞에서 끊고, 민감한 출력은 사용자 앞에서 한 번 더 거르는 식으로 실패 지점을 앞당겨야 운영 비용도 함께 줄어듭니다.

이 글은 LLM 앱 운영 101 시리즈의 4번째 글입니다. 여기서는 입력 가드, 민감 정보 마스킹, 출력 필터를 묶어 기본 보안 레이어를 세우는 방법을 설명하겠습니다.
