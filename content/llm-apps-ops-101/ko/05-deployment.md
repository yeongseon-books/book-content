...
- Health 엔드포인트: 살아 있음과 준비됨은 다릅니다에서 가장 흔한 실수는 무엇일까요?
- Self-test를 배포 게이트로 만들기을 실무에 적용할 때 주의할 점은 무엇일까요?
- 블루/그린: 버전 세트를 한 번에 전환하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **일반 웹 앱과 달리 LLM 앱 배포에서 버전 불일치가 특히 위험한 이유는 무엇일까요?**
  일반 웹 앱은 결정적 시스템이라 구버전과 신버전이 잠깐 공존해도 같은 입력에 같은 출력이 나옵니다. LLM 앱은 프롬프트 한 줄, 모델 버전 하나만 달라도 출력이 완전히 바뀝니다. 보안 규칙이 불일치하면 같은 공격이 어떤 인스턴스에서는 차단되고 다른 인스턴스에서는 통과합니다. 그래서 "부분 전환" 자체가 위험합니다.

- **health 엔드포인트가 200을 반환해도 서비스가 준비되지 않은 상태는 어떤 경우일까요?**
  프로세스는 살아 있지만 모델 클라이언트 초기화가 실패한 상태입니다. API 키 누락, 네트워크 정책 차단, SDK 초기화 예외 등으로 `/health`는 200이지만 실제 `/chat`은 전부 500인 경우가 이에 해당합니다. liveness와 readiness를 분리하지 않으면 이 상태를 감지할 수 없습니다.

- **배포 실패 시 코드, 모델, 프롬프트를 각각 따로 롤백해야 하는 이유는 무엇일까요?**
  프롬프트만 문제인데 인프라 전체를 롤백하면 불필요한 다운타임이 생깁니다. 반대로 코드만 롤백했는데 프롬프트가 원인이면 문제가 계속됩니다. 각 레이어를 독립적으로 롤백할 수 있으려면, 애초에 각 레이어가 환경변수나 설정 파일로 분리되어 독립적으로 제어 가능해야 합니다.
