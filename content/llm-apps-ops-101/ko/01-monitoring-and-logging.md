...
LLM 앱을 운영하기 시작하면 가장 먼저 부딪히는 문제는 장애 자체보다도, 요청 한 건에서 무슨 일이 일어났는지 다시 설명할 수 없는 상태입니다. 로그 한 줄이 호출 맥락, 비용, 지연 시간을 함께 붙잡고 있어야 나중에 같은 요청을 끝까지 추적할 수 있습니다.

이 글은 LLM 앱 운영 101 시리즈의 첫 번째 글입니다. 여기서는 운영의 출발점이 되는 모니터링과 로깅을 어떤 필드와 흐름으로 설계해야 하는지 살펴보겠습니다.
