...
- 왜 전통 API 로그로는 부족한가에서 가장 흔한 실수는 무엇일까요?
- 최소 실행 예제 — 호출 한 건을 설명할 수 있는 로그을 실무에 적용할 때 주의할 점은 무엇일까요?
- 메트릭과 로그의 역할 분담의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글은 llm-apps-ops-101 시리즈의 한 단계로, 핵심 개념을 실무 맥락에서 정리했습니다. 여기서 다룬 원칙들은 독립적으로도 유용하지만, 시리즈 전체와 연결될 때 더 큰 그림이 보입니다.

## 처음 질문으로 돌아가기

- **같은 200 응답인데 비용과 품질이 다를 때, 로그에 어떤 필드가 있어야 하는가?**
  `request_id`, `model`, `prompt_tokens`, `completion_tokens`, `latency_ms`, `prompt_preview`, `response_preview`, `prompt_version`이 최소 필드입니다. 이 필드들이 한 레코드에 있으면 "왜 비쌌지?", "왜 느렸지?", "뭘 답했지?"를 같은 출처에서 설명할 수 있습니다.
- **"어제까지 잘 되던 응답"의 원인을 프롬프트 변경인지 모델 변경인지 분리하는 방법은?**
  `prompt_version`과 `model`이 요청마다 기록되어 있으면, 문제 시작 시점을 기준으로 두 필드의 값 변화를 대조할 수 있습니다. 변경 시점이 겹치면 A/B 실험 데이터로 분리하고, 하나만 바뀌었으면 바로 원인을 특정합니다.
- **메트릭이 "이상하다"고 알려주고 로그가 "왜"를 설명하는 구조는?**
  메트릭(P95, 오류율, 토큰 추이)이 시계열로 이상 징후를 탐지하고, 로그가 해당 시간대의 개별 요청을 필터링해 원인을 설명합니다. 대시보드는 지연·비용·품질 세 축을 같은 시간축에 겹쳐서, 상관관계를 읽을 수 있도록 구성합니다.
