...
- 왜 사람 리뷰만으로는 안 되는가에서 가장 흔한 실수는 무엇일까요?
- 최소 실행 예제 — 실패 이유를 설명하는 평가을 실무에 적용할 때 주의할 점은 무엇일까요?
- JSON Schema로 형식 계약을 명확하게의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **형식 통과와 품질 통과는 왜 다른 검사여야 할까요?**
  - 규칙 기반 검사의 다음 단계는 스키마 검증입니다. 키 존재 여부만 보는 것은 "answer가 있나?"까지만 확인합니다.
- **평가 결과가 "실패했다"만 알려주면 왜 운영에 쓸 수 없을까요?**
  - 이 코드에서 라이브러리 사용법은 중요하지 않습니다. 중요한 것은 설계 결정 세 가지입니다.
- **배포 전 평가와 배포 후 평가는 무엇이 달라야 할까요?**
  - 평가가 운영 도구가 되려면, 결과가 배포 결정에 실제로 영향을 미쳐야 합니다. "평가 돌렸는데 실패가 많네요, 그래도 배포할게요"가 반복되면 평가 시스템은 무시되기 시작합니다.
- **왜 사람 리뷰만으로는 안 되는가에서 가장 흔한 실수는 무엇일까요?**
  - 하루에 LLM 호출이 100건이면 사람이 전부 읽을 수 있습니다. 1,000건이면 샘플링을 해야 합니다.
- **최소 실행 예제 — 실패 이유를 설명하는 평가을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 이 코드에서 라이브러리 사용법은 중요하지 않습니다. 중요한 것은 설계 결정 세 가지입니다.
- **JSON Schema로 형식 계약을 명확하게의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 규칙 기반 검사의 다음 단계는 스키마 검증입니다. 키 존재 여부만 보는 것은 "answer가 있나?"까지만 확인합니다.
