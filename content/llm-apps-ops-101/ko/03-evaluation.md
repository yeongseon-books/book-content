...
이 글은 LLM 앱 운영 101 시리즈의 3번째 글입니다.
...
이 코드가 보여 주는 핵심은 평가를 한 번에 뭉뚱그리지 않는다는 사실입니다. 먼저 JSON 파싱이 되는지 보고, 그다음 길이를 보고, 마지막으로 핵심 용어가 빠졌는지를 봅니다. 이렇게 나누어 두면 실패했을 때도 “그냥 품질이 낮다”가 아니라 “형식 오류인지, 길이 문제인지, 키워드 누락인지”를 바로 분류할 수 있습니다.
...
계층형 접근의 장점은 운영 비용을 통제하기 쉽다는 사실입니다. 형식 검사와 도메인 규칙은 계산 비용이 거의 없고 실패 원인을 설명하기 쉽습니다. 반대로 의미 평가는 비싸고 변동성이 있으므로, 앞단을 통과한 응답에 선택적으로 적용하는 것이 안전합니다.
