...
이 그림에서는 LLM 출력이 형식·길이·키워드 같은 빠른 규칙 검사를 지나 더 깊은 품질 평가로 이어지는 흐름을 봅니다. 평가는 배포 전 행사가 아니라 운영 중 품질 변화를 읽는 계층입니다.
...
## 운영 데이터셋을 건강하게 유지하는 방법

평가 품질은 모델보다 데이터셋 건강도에 크게 좌우됩니다. 오래된 데이터셋만 고집하면 실제 사용자 질문 분포를 놓치고, 너무 최신 데이터만 쓰면 회귀 비교가 어려워집니다. 따라서 고정 회귀 세트와 최신 샘플 세트를 병행하는 이중 구조가 필요합니다.

고정 회귀 세트는 장기 비교를 위한 기준선입니다. 최신 샘플 세트는 최근 트래픽 변화와 신규 기능을 반영합니다. 두 세트를 함께 운영하면 안정성과 민감도를 동시에 확보할 수 있습니다.

데이터셋 관리에서 중요한 또 한 가지는 라벨 품질입니다. 판정 근거를 짧게라도 기록해 두면, 이후 모델이 바뀌어도 같은 기준으로 재평가할 수 있습니다. 라벨 기준이 문서화되지 않으면 평가 점수는 있어도 해석이 불가능해집니다.



## 실전 평가 시나리오: 고객 지원 봇

고객 지원 봇을 예로 들면 평가 기준은 더 구체적이어야 합니다. 사용자는 짧고 정확한 답을 기대하지만, 모델은 종종 과도하게 장문을 생성하거나 핵심 절차를 누락합니다. 따라서 지원 봇 평가는 정확성뿐 아니라 실행 가능성을 함께 봐야 합니다.

첫째 기준은 절차 완결성입니다. 환불, 계정 잠금, 배송 변경처럼 중요한 업무는 필수 단계가 빠지면 실패로 간주합니다. 둘째 기준은 정책 준수입니다. 개인정보 요청 방식이나 인증 절차를 우회하면 즉시 실패로 판정합니다. 셋째 기준은 응답 효율성입니다. 같은 정보를 불필요하게 반복해 토큰 비용을 늘리는 패턴을 감점합니다.

이 시나리오에서 운영팀이 자주 쓰는 방법은 실패 유형별 라벨링입니다. "절차 누락", "정책 위반", "근거 부족", "장문 과다"처럼 분류하면 개선 작업이 명확해집니다. 절차 누락이 많으면 프롬프트 구조를 고치고, 근거 부족이 많으면 검색 컨텍스트 품질을 먼저 개선할 수 있습니다.

또한 고객 지원 봇은 계절성과 캠페인 영향을 받습니다. 특정 기간에 문의 패턴이 바뀌면 기존 평가 세트가 현실을 반영하지 못할 수 있습니다. 그래서 월 1회는 최신 문의 샘플을 평가 세트에 반영하는 루틴이 필요합니다. 고정 세트만 유지하면 회귀는 잡아도 신규 실패를 놓칠 수 있습니다.

마지막으로 평가 결과를 상담 품질 지표와 연결하면 효과가 큽니다. 예를 들어 자동 응답 후 재문의율, 상담원 이관율과 평가 실패율을 함께 보면, 모델 품질이 실제 운영 비용에 어떻게 영향을 주는지 선명하게 드러납니다. 이 연결 고리가 있어야 평가 시스템이 연구 도구가 아니라 비즈니스 운영 도구가 됩니다.


운영 평가 체계는 결국 반복 가능한 루틴이어야 합니다. 같은 입력 세트로 같은 기준을 적용하고, 결과를 같은 포맷으로 저장해야만 추세를 읽을 수 있습니다. 도구 선택보다 더 중요한 것은 루틴의 일관성입니다.
