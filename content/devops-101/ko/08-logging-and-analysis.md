...
- 로깅을 위한 5단계을 실무에 적용할 때 주의할 점은 무엇일까요?
- 로그 레벨 기준의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **구조화 로그와 비구조화 로그는 실무에서 무엇이 다를까요?**
  - 비구조화 로그는 `print("user logged in", user_id)`처럼 문자열 검색에 의존하지만, 구조화 로그는 `structlog`와 JSON 필드로 `user_id`, `request_id`, `level`을 바로 조건 검색할 수 있습니다. 그래서 Grafana나 Kibana에서 특정 요청과 특정 사용자 흐름을 훨씬 짧은 시간에 좁힐 수 있습니다.
- **여러 서버의 로그를 한곳에 모아야 하는 이유는 무엇일까요?**
  - 이 글은 분산 시스템에서는 요청이 여러 인스턴스를 지나므로 한 서버에 SSH로 들어가 `grep`하는 방식이 금방 한계에 닿는다고 설명합니다. 애플리케이션이 stdout으로 내보낸 로그를 Promtail이 수집해 Loki나 ELK로 보내야 서비스 전체 사건을 한 화면에서 재구성할 수 있습니다.
- **Loki와 ELK는 어떤 관점에서 비교하면 좋을까요?**
  - 본문 비교표 기준으로 ELK는 전문 텍스트 인덱싱이 강해 복잡한 검색과 집계에 유리하지만, 운영 복잡도와 비용이 더 큽니다. Loki는 라벨 기반 인덱싱으로 비용 효율이 좋고 Grafana와 자연스럽게 붙으므로, 메트릭과 로그를 함께 보는 운영에 특히 잘 맞습니다.
