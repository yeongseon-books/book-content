...
- 장애 대응을 구성하는 5단계을 실무에 적용할 때 주의할 점은 무엇일까요?
- 증상의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **새벽 3시에 알림이 울리면 누가 무엇을 해야 할까요?**
  - 이 글은 primary on-call이 먼저 알림을 받고 런북을 열어 초기 완화 조치를 수행해야 한다고 설명합니다. 첫 15분 순서처럼 SEV 판정, incident 채널 개설, 최근 배포 확인, 필요 시 `kubectl rollout undo deploy/api` 같은 조치를 진행하고, 상황이 커지면 IC가 조율을 맡습니다.
- **장애 심각도(SEV)는 어떤 기준으로 나눠야 팀이 같은 언어로 움직일까요?**
  - 본문은 SEV1을 전사 핵심 기능 중단, SEV2를 핵심 기능 저하, SEV3를 부분 저하, SEV4를 경미한 버그로 나누고 기대 응답 시간도 함께 제시합니다. 이렇게 영향 범위와 복구 기대 시간을 함께 정의해야 Slack에서 의견이 갈리지 않고 같은 기준으로 에스컬레이션할 수 있습니다.
- **on-call 로테이션은 왜 일정표가 아니라 운영 설계일까요?**
  - 이 글의 로테이션은 `primary`, `secondary`, `escalation_timeout`, 주간 handoff처럼 백업 경로와 인수인계까지 포함한 체계입니다. 여기에 주간 페이지 수와 야간 호출 비율 같은 피로도 지표를 함께 봐야 지속 가능한 대응 품질을 유지할 수 있습니다.
