...
- 파이썬 풀루미 예시에서 가장 흔한 실수는 무엇일까요?
- 전환 전후을 실무에 적용할 때 주의할 점은 무엇일까요?
- 테라폼으로 시작하는 다섯 단계의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **콘솔에서 클릭해 만든 인프라는 왜 다른 환경에서 재현하기 어려울까요?**
  - 콘솔 작업은 누가 언제 무엇을 바꿨는지 코드와 이력으로 남기기 어렵고, 같은 구성을 다른 리전이나 계정에 다시 만들 때 값이 조금씩 달라지기 쉽습니다. 이 글이 드리프트와 state를 강조하는 이유도 재현성이 깨지는 지점이 바로 여기에 있기 때문입니다.
- **IaC는 운영팀의 편의가 아니라 팀 전체의 변경 품질과 어떻게 연결될까요?**
  - IaC가 되면 인프라 변경도 애플리케이션 코드처럼 PR, plan 리뷰, 정책 검사, 승인 흐름 안에서 다룰 수 있습니다. 본문의 `terraform validate`, OPA 태그 정책, 원격 state와 locking 예시는 인프라 변경을 개인 노하우가 아니라 팀 품질 관리 대상으로 바꾸는 장치입니다.
- **Terraform의 기본 흐름은 plan과 apply를 중심으로 어떻게 이해하면 좋을까요?**
  - 이 글의 기본 흐름은 `init -> plan -> apply`이며, 핵심은 apply 전에 변경 diff를 눈으로 검토하는 습관입니다. 여기에 S3 + DynamoDB 원격 백엔드, `plan -detailed-exitcode`, 모듈 재사용을 붙이면 한 번의 실행이 아니라 지속적으로 검증되는 운영 루프가 됩니다.
