...
- 전환 전후에서 가장 흔한 실수는 무엇일까요?
- 실전으로 보는 도커파일 다섯 단계을 실무에 적용할 때 주의할 점은 무엇일까요?
- 파이썬 FastAPI 멀티스테이지 도커파일 예시의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **컨테이너는 VM과 무엇이 다르고 왜 배포 재현성에 유리할까요?**
  - 이 글에서 컨테이너는 전체 운영체제를 따로 올리는 VM보다 가볍게 애플리케이션과 의존성을 이미지로 묶는 방식입니다. `FROM python:3.12-slim`으로 시작하는 Dockerfile처럼 실행 환경 자체가 코드로 남기 때문에, 서버마다 설치 상태가 달라서 생기던 재현성 문제가 크게 줄어듭니다.
- **Dockerfile에서 꼭 이해해야 할 기본 명령은 무엇일까요?**
  - 본문은 `FROM`, `WORKDIR`, `COPY`, `RUN`, `CMD`를 기본 축으로 설명하고, 운영 품질을 위해 `USER`와 `HEALTHCHECK`도 함께 봐야 한다고 짚습니다. 특히 `requirements.txt`를 먼저 `COPY`하는 순서와 non-root 사용자 전환은 빌드 캐시와 보안을 동시에 좌우하는 핵심 포인트입니다.
- **multi-stage build는 이미지 크기와 보안에 어떤 차이를 만들까요?**
  - builder 단계에서 의존성을 설치하고 runtime 단계에는 `/root/.local` 같은 실행 결과만 복사하면 최종 이미지에서 불필요한 빌드 도구를 제거할 수 있습니다. 본문 표처럼 이미지 크기를 50~70% 줄이기 쉽고, 함께 따라오던 패키지와 공격 표면도 줄어 보안 스캔 결과가 더 깔끔해집니다.
