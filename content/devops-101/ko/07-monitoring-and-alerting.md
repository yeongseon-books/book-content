...
- 모니터링을 위한 5단계을 실무에 적용할 때 주의할 점은 무엇일까요?
- 파이썬 프로메테우스 클라이언트 예시의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **모니터링의 세 신호인 로그, 메트릭, 트레이스는 어떻게 역할이 다를까요?**
  - 메트릭은 `http_requests_total`, p95 latency 같은 수치로 이상 징후를 빠르게 탐지하고, 로그는 에러 메시지와 문맥으로 원인을 더 좁혀 줍니다. 트레이스는 하나의 요청이 여러 서비스와 의존성을 어떻게 지났는지 보여 주는 흐름 신호라서, 세 가지를 함께 봐야 증상과 원인을 같은 시간축에서 연결할 수 있습니다.
- **Prometheus와 Grafana는 어떤 흐름으로 함께 동작할까요?**
  - 이 글의 흐름은 FastAPI가 `/metrics`를 노출하고, Prometheus가 `scrape_configs`에 따라 이를 수집한 뒤, Grafana가 요청률·에러율·p95 패널로 시각화하는 구조입니다. 여기에 Alertmanager 규칙과 `for: 5m` 조건을 붙여 대시보드에서 본 이상 신호가 실제 대응 알림으로 이어지게 합니다.
- **RED와 USE 같은 메트릭 패턴은 왜 운영에서 자주 언급될까요?**
  - RED는 요청률, 에러율, 지연시간으로 사용자 관점의 서비스 상태를 빠르게 읽게 해 주고, USE는 자원 사용률과 포화도로 시스템 병목을 설명해 줍니다. 본문이 두 패턴을 함께 보라고 한 이유는 "문제가 있다"와 "왜 느린가"를 같은 대화 안에서 이어 주기 때문입니다.
