...
데이터베이스의 행은 곱집합의 원소이고, JOIN은 곱집합 위에서 술어를 평가하는 연산입니다. 해시맵은 키 집합에서 값 집합으로 가는 부분 함수입니다. 함수형 프로그래밍의 `map`도 결국 집합 사이에 함수를 적용하는 방식으로 이해할 수 있습니다. 집합과 함수를 모르면 이런 도구의 본질을 끝까지 이해하기 어렵습니다.
...
마지막 등식은 포함-배제 원리(inclusion-exclusion)의 가장 단순한 형태입니다. 7장에서 이 원리를 더 자세히 다시 다룹니다.
...
함수 합성은 함수형 프로그래밍의 핵심 도구이며, 역함수는 암호학과 데이터 변환에서 빠지지 않는 개념입니다.
