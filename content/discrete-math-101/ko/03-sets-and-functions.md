...
다음 글에서는 집합 위에 정의되는 가장 중요한 구조인 관계와 동치관계를 봅니다.
...
- **집합 표기와 기본 연산은 어떻게 읽어야 할까요?**
  - `A = {1,2,3,4,5}`, 조건 제시법 `B`, 그리고 `power_set` 예시를 통해 집합은 원소와 포함관계로 읽고, 멱집합 크기는 `2^n`으로 계산한다는 흐름을 잡았습니다. 이어서 `A | B`, `A & B`, `A - B`, `U - A` 같은 코드가 합집합, 교집합, 차집합, 여집합을 바로 대응시켜 줍니다.
- **합집합, 교집합, 차집합, 곱집합은 실무에서 어디에 쓰일까요?**
  - 본문은 `permissions = set(product(users, roles))`로 데이터베이스 행과 권한 조합을 곱집합으로 설명했고, `JOIN`과 `CROSS JOIN`이 왜 집합 연산 언어로 읽히는지 연결했습니다. 또 중복 제거를 `list(set(items))`로 바꾸는 전후 비교는 집합 사고가 실무 코드 단순화로 바로 이어진다는 점을 보여 줍니다.
- **함수의 정의역, 공역, 치역은 무엇이 다를까요?**
  - 정의역은 입력 집합, 공역은 함수가 향하도록 선언한 전체 출력 집합, 치역은 실제로 나온 값들의 집합입니다. 그래서 `is_surjective(f1, codomain)` 예시처럼 전사 여부는 단순히 값이 몇 개 나왔는지가 아니라 실제 출력 집합이 공역 전체를 덮는지로 판단해야 합니다.
...
- **Discrete Math 101 (3/10): 집합과 함수 (현재 글)**
