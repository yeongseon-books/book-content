...
```python
# 동치관계: reflexive + symmetric + transitive
# 예시: modulo 3 정수
...
```python
# 부분순서: reflexive + antisymmetric + transitive
# 예시: 부분집합 관계 ⊆
A = [{1}, {2}, {1, 2}, {1, 2, 3}]
...
- **이항관계는 어떻게 정의하고 표현할까요?**
  - 이항관계는 `A × A`의 부분집합이며, 본문에서는 `friends` 같은 순서쌍 집합과 `Adjacency matrix` 예시로 같은 관계를 두 방식으로 표현했습니다. 그래서 관계는 추상 정의에 머무르지 않고 그래프, 행렬, 인접 리스트로 바로 구현 가능한 구조임을 알 수 있습니다.
- **반사, 대칭, 추이, 반대칭 성질은 어떻게 구분할까요?**
  - `is_reflexive`, `is_symmetric`, `is_transitive`, `is_antisymmetric` 함수가 각 성질을 판정하는 기준을 정확히 보여 주었습니다. 핵심은 대칭은 `(a,b)`가 있으면 `(b,a)`도 있어야 하고, 반대칭은 그런 쌍이 동시에 있으면 결국 `a=b`여야 한다는 점이라서 둘을 감으로 섞으면 안 됩니다.
- **동치관계와 동치류, 분할은 왜 서로 연결될까요?**
  - `mod_equivalent` 예시에서 나머지가 같은 수들을 묶으면 `[0]`, `[1]`, `[2]` 같은 동치류가 생기고, 이 동치류들이 전체 집합을 겹치지 않게 나눕니다. 그래서 동치관계를 정하는 일은 결국 어떤 기준으로 데이터를 같은 것으로 분류할지, 즉 분할 규칙을 정하는 일과 같습니다.
...
- **Discrete Math 101 (4/10): 관계와 동치관계 (현재 글)**
