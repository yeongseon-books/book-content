...
- **트리의 정의와 성질은 무엇일까요?**
  - 트리는 연결되어 있으면서 사이클이 없는 무방향 그래프이고, 그래서 정점이 `n`개면 간선은 정확히 `n-1`개입니다. 본문의 `is_tree` 함수는 바로 이 간선 수 조건과 연결성 검사를 함께 써서 트리 여부를 판정했습니다.
- **BFS는 왜 최단 경로와 연결될까요?**
  - BFS는 큐를 사용해 거리 0, 1, 2 순서로 레벨별 확장을 하므로, 가중치가 모두 같은 그래프에서는 첫 방문 시점의 거리가 곧 최단 거리입니다. `distances from 1` 예시에서 `2`, `3`이 거리 1이고 `4`, `5`, `6`이 거리 2로 계산된 이유가 바로 이 성질입니다.
- **DFS는 어떤 문제에서 강할까요?**
  - DFS는 한 갈래를 끝까지 내려갔다가 돌아오는 방식이라 구조를 깊게 파악해야 하는 문제에 적합합니다. 본문에서도 재귀·반복 DFS 방문 순서를 비교했고, 이 감각이 사이클 검출, 위상 정렬, 서브트리 크기 계산 같은 문제로 자연스럽게 이어진다고 설명했습니다.
