...
다음 글에서는 논리와 함께 이산수학의 또 다른 기초인 집합과 함수를 봅니다.
...
- **명제와 비명제는 어떻게 구분할까요?**
  - 이 글에서는 `propositions` 딕셔너리처럼 참·거짓이 분명히 정해지는 문장만 명제로 다뤘고, 질문이나 명령처럼 진리값이 없는 문장은 비명제로 구분했습니다. 이후의 연산자, 진리표, 증명은 모두 이 구분이 정확해야만 흔들리지 않습니다.
- **다섯 가지 기본 논리 연산자는 어떻게 동작할까요?**
  - `NOT`, `AND`, `OR`, `IMPLIES`, `IFF` 함수 예시가 각 연산의 의미를 그대로 보여 주었고, 특히 함의는 `(not p) or q`로 구현해 `P`가 거짓이면 참이 된다는 점을 드러냈습니다. 드모르간 법칙 검증 코드까지 보면 이 연산자들은 단순 암기가 아니라 조건문을 안전하게 바꾸는 규칙입니다.
- **진리표로 논리적 동치를 어떻게 확인할까요?**
  - `truth_table`과 `equivalent` 함수는 가능한 입력 조합을 전부 열거해 두 식이 모든 행에서 같은 결과를 내는지 확인합니다. 그래서 `¬(P ∧ Q)`와 `¬P ∨ ¬Q`처럼 코드 리팩터링에 자주 쓰는 변형도 감이 아니라 전수 검증으로 판단할 수 있습니다.

<!-- toc:begin -->
## 시리즈 목차

- [Discrete Math 101 (1/10): 이산수학이란 무엇인가?](./01-what-is-discrete-math.md)
- **Discrete Math 101 (2/10): 명제와 논리 (현재 글)**
