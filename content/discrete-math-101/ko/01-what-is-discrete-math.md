...
다음 글에서는 이산수학의 가장 작은 단위인 명제와, 모든 컴퓨터 추론을 움직이는 논리 연산자를 봅니다.
...
- **이산수학과 연속 수학은 무엇이 다를까요?**
  - 이 글에서는 `continuous_sample`과 `discrete_set` 예시로 연속 대상은 근사 샘플로만 다루고, 이산 대상은 원소를 정확히 셀 수 있음을 보여 주었습니다. 컴퓨터 메모리와 비트는 본질적으로 이산적이므로, 실수보다 명제·집합·그래프 같은 구조가 컴퓨터공학의 기본 언어가 됩니다.
- **이산수학의 다섯 축은 어떻게 연결될까요?**
  - 본문 ASCII 다이어그램처럼 논리에서 시작해 집합·함수·관계로 내려오고, 다시 조합론·수열·그래프를 거쳐 알고리즘과 자료구조로 이어지는 구조입니다. `binary_search`의 점화식, 집합 연산 코드, 그래프 이웃 조회 예시가 이 다섯 축이 실제로 분리된 장이 아니라 연결된 도구 상자임을 보여 줍니다.
- **컴퓨터공학 커리큘럼에서 왜 이산수학이 필수일까요?**
  - 알고리즘 분석에는 `T(n)=T(n/2)+1` 같은 점화식이 필요하고, 데이터베이스는 `A ∪ B`, `A ∩ B` 같은 집합 연산으로 읽히며, 네트워크와 의존성 관리는 그래프로 모델링됩니다. 그래서 이산수학은 별도 교양 과목이 아니라 알고리즘, DB, 네트워크를 이해하기 위한 공통 기초입니다.

<!-- toc:begin -->
## 시리즈 목차

- **Discrete Math 101 (1/10): 이산수학이란 무엇인가? (현재 글)**
