...
재귀는 트리 탐색, 그래프 탐색, 정렬, 동적 계획법 등 거의 모든 고급 알고리즘의 기반입니다. 재귀를 이해하지 못하면 이후 알고리즘을 배우기가 어려워집니다.

분할 정복은 병합 정렬, 퀵 정렬, 이진 탐색의 공통 패턴입니다. 문제를 쪼개는 관점을 익혀 두면 새로운 문제도 훨씬 체계적으로 풀 수 있습니다.

이 글은 Algorithms with Python 101 시리즈의 5번째 글입니다. 여기서는 재귀 호출이 어떻게 동작하는지부터 시작해, 분할 정복이라는 문제 해결 패턴으로 시야를 넓혀 보겠습니다.
