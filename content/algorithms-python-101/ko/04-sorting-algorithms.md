...
정렬은 알고리즘의 기본 중 기본입니다. 이진 탐색, 중복 제거, 순위 계산 등 수많은 연산이 정렬된 데이터를 전제로 합니다. 데이터 1만 개만 넘어도 O(n²)와 O(n log n)의 차이는 금방 체감됩니다.

실무에서는 대부분 내장 `sorted()`를 사용하지만, 정렬 알고리즘의 원리를 이해하면 `key` 함수 설계, 안정 정렬 활용, 성능 병목 분석에 큰 도움이 됩니다.

이 글은 Algorithms with Python 101 시리즈의 4번째 글입니다. 여기서는 기본 정렬 알고리즘들을 Python으로 구현하고, 각 방식이 어떤 데이터와 상황에서 차이를 만드는지 비교해 보겠습니다.
