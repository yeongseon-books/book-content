...
- **그리디 알고리즘이 최적해를 만드는 조건은 무엇일까요?**
  - 그리디가 최적해를 만들려면 지역 최적 선택이 전체 최적해의 일부가 되는 그리디 선택 속성과, 부분 문제 최적해로 전체 답을 만들 수 있는 최적 부분 구조가 함께 필요합니다. 활동 선택에서 종료 시간이 빠른 활동을 먼저 고르는 규칙이 성립하는 이유가 바로 그 조건을 만족하기 때문입니다.
- **동전 거스름돈, 활동 선택, 분할 가능한 배낭 문제는 어떻게 풀까요?**
  - 거스름돈은 큰 단위 동전부터, 활동 선택은 종료 시간이 빠른 순서부터, 분할 가능한 배낭은 가치/무게 비율이 큰 순서부터 고르면 됩니다. 본문의 `coin_change_greedy()`, `activity_selection()`, `fractional_knapsack()` 예제는 정렬 기준 하나가 선택 규칙을 결정한다는 점을 공통으로 보여 줍니다.
- **그리디는 동적 계획법과 어떤 차이가 있을까요?**
  - 그리디는 매 단계에서 바로 선택을 확정하지만, DP는 여러 가능성을 상태로 남겨 전체 최적을 계산합니다. `coins = [1, 3, 4], amount = 6` 반례에서 그리디는 3개 동전을 고르지만 `coin_change_dp()`는 3+3으로 2개를 찾아내므로, 두 접근의 차이가 분명하게 드러납니다.
