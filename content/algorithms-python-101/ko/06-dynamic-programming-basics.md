...
- **DP가 적용되는 두 조건은 무엇일까요?**
  - DP는 최적 부분 구조와 중복 부분 문제가 함께 있을 때 적용합니다. 피보나치와 계단 오르기 예제는 현재 답이 이전 상태들로 구성되면서 같은 계산이 반복되기 때문에, 저장을 붙이면 지수 시간이 선형 시간으로 줄어든다는 점을 보여 줍니다.
- **메모이제이션을 쓰는 top-down 구현은 어떻게 할까요?**
  - top-down 구현은 먼저 재귀 점화식을 세우고, 이미 계산한 값은 캐시에서 꺼내는 방식으로 작성하면 됩니다. `@lru_cache`를 붙인 `fib_top_down()`과 `memo` 딕셔너리를 쓰는 `fib_memo()`가 같은 아이디어를 두 가지 스타일로 보여 줍니다.
- **테이블을 채우는 bottom-up 구현은 어떻게 할까요?**
  - bottom-up 구현은 가장 작은 상태부터 `dp` 배열이나 표를 채우면서 현재 값을 계산하는 방식입니다. `fib_bottom_up()`의 `dp[i] = dp[i - 1] + dp[i - 2]`, `coin_change()`의 최소 동전 수 갱신, `knapsack()`의 2차원 표가 그 전형적인 예시입니다.
