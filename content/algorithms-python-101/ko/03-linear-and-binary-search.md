...
- **선형 탐색은 어떻게 동작하고, 한계는 무엇일까요?**
  - 선형 탐색은 `linear_search()`처럼 리스트 처음부터 끝까지 값을 하나씩 비교하며 찾고, 정렬이 없어도 바로 쓸 수 있다는 장점이 있습니다. 대신 최악의 경우 끝까지 모두 확인해야 해서 큰 입력에서는 `O(n)` 비용이 그대로 부담이 됩니다.
- **이진 탐색은 어떤 원리로 동작하며 어떻게 구현할까요?**
  - 이진 탐색은 정렬된 데이터에서 `mid`를 잡아 왼쪽 또는 오른쪽 절반만 남기며 범위를 줄이는 방식으로 동작합니다. `binary_search()`의 `left`, `right`, `mid` 갱신과 `while left <= right` 조건이 그 핵심이며, 그래서 시간 복잡도는 `O(log n)`입니다.
- **Python의 `bisect` 모듈은 언제 유용할까요?**
  - `bisect`는 직접 이진 탐색을 다시 쓰지 않고 삽입 위치, 첫 출현 위치, 마지막 다음 위치를 안전하게 구하고 싶을 때 특히 유용합니다. 본문에서 `bisect_left`, `bisect_right`, `insort`로 중복 개수 계산과 정렬 리스트 삽입까지 처리한 예제가 그 활용 범위를 보여 줍니다.
