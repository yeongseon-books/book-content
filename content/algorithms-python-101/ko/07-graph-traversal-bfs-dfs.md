...
네트워크, 지도, 의존성 트리, 추천 시스템은 모두 결국 노드와 연결 관계의 문제로 환원됩니다. 배열과 리스트를 지나면 그래프 사고가 자주 등장하는 이유가 여기에 있습니다. 이 글은 Algorithms with Python 101 시리즈의 일곱 번째 글입니다. 여기서는 Python으로 그래프를 표현하고, BFS와 DFS를 실용적인 관점에서 구현해 보겠습니다.
