...
- **그래프의 기본 개념과 Python에서의 표현 방식은 무엇일까요?**
  - 그래프는 노드와 간선의 집합이며, Python에서는 보통 `{"A": ["B", "C"]}` 같은 인접 리스트 딕셔너리로 표현합니다. 이 글은 무방향 그래프, 방향 그래프, 가중 그래프를 각각 예시로 보여 주면서 이웃 목록 구조가 탐색 구현의 기본이라는 점을 정리했습니다.
- **BFS는 어떤 원리로 동작하고 언제 써야 할까요?**
  - BFS는 `deque` 큐에 시작점을 넣고 가까운 노드부터 층별로 넓혀 가며 방문합니다. 그래서 `bfs_shortest_path()`처럼 가중치가 없는 그래프의 최단 경로와 연결 요소 탐색에 특히 잘 맞습니다.
- **DFS는 어떤 원리로 동작하고 언제 써야 할까요?**
  - DFS는 재귀나 명시적 스택으로 한 경로를 끝까지 내려간 뒤 되돌아오는 방식으로 동작합니다. 본문에서는 `dfs_recursive()`와 `dfs_iterative()`를 모두 구현했고, `has_cycle()`처럼 구조를 깊게 확인해야 하는 문제에 더 자연스럽다는 점을 보여 주었습니다.
