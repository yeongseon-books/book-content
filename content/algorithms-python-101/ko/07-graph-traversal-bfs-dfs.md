...
소셜 네트워크, 지도, 웹 페이지 링크, 의존성 관리처럼 현실 세계의 많은 관계는 그래프로 모델링할 수 있습니다. BFS와 DFS는 이런 그래프를 탐색할 때 가장 먼저 배우는 기본 알고리즘입니다.

BFS는 최단 경로 문제에서 강하고, DFS는 경로 존재 여부 확인, 사이클 탐지, 위상 정렬 같은 문제에서 자주 등장합니다. 둘의 차이를 감으로 구분할 수 있어야 다음 단계로 넘어가기 쉽습니다.

이 글은 Algorithms with Python 101 시리즈의 7번째 글입니다. 여기서는 그래프 표현 방식부터 시작해 BFS와 DFS를 각각 구현하고, 두 탐색이 문제 유형에 따라 어떻게 갈리는지 정리하겠습니다.
...
```python
# 이전 방식: 모든 경로를 무작위로 탐색 — 비효율적
def is_connected(graph, start, end):
    # 탐색 순서가 체계적이지 않아 무한 루프에 빠질 수 있음
    pass
```

```python
# 개선 방식: BFS로 체계적으로 탐색 — O(V+E)
...
print(has_cycle(graph, "A"))  # True 출력: A-B-A 또는 A-C-A 형태의 사이클이 존재함
```

## 이 코드에서 주목할 점

- BFS는 큐(deque)를 사용하고, DFS는 스택 또는 재귀를 사용합니다
- BFS는 최단 경로를 보장하지만 DFS는 보장하지 않습니다
- visited 집합으로 이미 방문한 노드를 추적해 무한 루프를 방지합니다
...
BFS와 DFS는 그래프 알고리즘의 기본 토대입니다. 이 두 알고리즘을 제대로 이해하면 다익스트라, 위상 정렬, 최소 신장 트리 같은 고급 알고리즘도 훨씬 수월하게 받아들일 수 있습니다.

실무에서는 NetworkX 같은 라이브러리를 자주 사용합니다. 그래도 BFS와 DFS의 원리를 알고 있어야 어떤 문제에 어떤 탐색 전략이 맞는지 판단할 수 있습니다.
...
BFS는 가까운 노드부터 탐색하며 가중치가 없는 그래프에서는 최단 경로를 보장합니다. DFS는 한 경로를 깊게 따라가며 사이클 탐지와 위상 정렬에 적합합니다. 다음 글에서는 가중 그래프에서 최단 경로를 구하는 다익스트라 알고리즘을 다룹니다.
