...
> Algorithms with Python 101 시리즈 (1/10)

<!-- a-grade-intro:begin -->

**핵심 질문**: 같은 문제를 푸는데 왜 어떤 코드는 1초에 끝나고 어떤 코드는 10분이 걸릴까요?

> 알고리즘은 문제를 해결하는 단계적 절차입니다. 좋은 알고리즘은 정확하고, 효율적이며, 명확합니다. 이 글에서는 알고리즘의 정의, 특성, 표현 방법을 다루고, Python으로 첫 알고리즘을 작성합니다.
...
- 알고리즘의 정의와 핵심 특성
- 의사 코드와 플로차트로 알고리즘 표현
- Python으로 간단한 알고리즘 구현
- 알고리즘의 정확성과 효율성 개념

## 왜 중요한가

프로그래밍은 결국 문제 해결입니다. 같은 문제라도 어떤 알고리즘을 선택하느냐에 따라 실행 시간이 수천 배 차이 날 수 있습니다. 알고리즘을 이해하면 더 빠르고 정확한 코드를 작성할 수 있습니다.

> 알고리즘 = 입력을 받아 원하는 출력을 만드는 명확한 단계의 집합

코딩 면접, 성능 최적화, 시스템 설계 등 개발자 커리어 전반에서 알고리즘 사고력은 핵심 역량입니다.
...
```python
# before: 매번 정렬하여 최댓값 추출 — O(n log n)
data = [3, 7, 2, 9, 4]
...
- 같은 문제도 알고리즘에 따라 성능이 크게 달라집니다 (O(n²) vs O(n))
- 알고리즘은 입력, 출력, 명확성, 유한성, 효과성 5가지 특성을 가집니다
- Python 내장 함수(max, sorted)도 알고리즘입니다 — 직접 구현해보면 원리를 이해할 수 있습니다
...
대부분의 일상 코딩에서 알고리즘을 직접 구현할 일은 드뭅니다. 라이브러리와 프레임워크가 최적화된 알고리즘을 제공합니다. 하지만 알고리즘적 사고력은 문제를 분석하고 최적의 도구를 선택하는 데 필수입니다.

"이 코드가 왜 느린가?", "더 나은 방법은 없는가?"라는 질문에 답하려면 알고리즘의 기본을 알아야 합니다.

## 시니어 엔지니어는 이렇게 생각합니다

- **문제 정의 우선** — 알고리즘 선택 전에 입력·출력·제약을 형식화합니다.
- **복잡도 직관** — Big-O는 의사결정 도구로 활용하고 정확한 측정과 분리합니다.
- **검증** — 엣지 케이스로 정합성을 먼저 보장합니다.
- **표준 라이브러리** — 직접 구현 전에 표준 자료구조·알고리즘을 우선 검토합니다.
- **벤치마크** — 추정 대신 실제 데이터로 측정합니다.
...
1. 리스트에서 두 번째로 큰 값을 한 번의 순회로 찾는 알고리즘을 작성하세요.
2. 주어진 문자열에서 가장 많이 등장하는 문자를 찾는 알고리즘을 작성하세요.
3. 1부터 N까지의 합을 구하는 방법을 세 가지(반복문, 수학 공식, 재귀)로 구현하고 성능을 비교하세요.

## 정리 및 다음 글 안내

알고리즘은 문제를 해결하는 명확한 절차이며, 효율성은 알고리즘 선택의 핵심 기준입니다. 다음 글에서는 알고리즘의 효율성을 객관적으로 측정하는 도구인 시간 복잡도와 Big-O 표기법을 다룹니다.
