...
- **secure coding은 정확히 무엇을 뜻할까요?**
  - secure coding은 정확히 무엇을 뜻할까요 — 본문에서 구체적으로 다룹니다.
- **위협 모델과 공격 표면은 코드 설계와 어떤 관계가 있을까요?**
  - secure coding 원칙이 실제 설계로 이어지려면 "무엇이 위험한가"를 구조적으로 분류할 수 있어야 합니다. 여기서는 Microsoft가 제안한 STRIDE 모델을 기준으로 위협을 분류하고, 공격 표면을 코드 수준에서 식별하는 방법을 다룹니다.
- **OWASP Top 10은 입문자가 어떤 관점으로 읽어야 할까요?**
  - OWASP Top 10은 입문자가 어떤 관점으로 읽어야 할까요 — 본문에서 구체적으로 다룹니다.
- **한눈에 보는 구조에서 가장 흔한 실수는 무엇일까요?**
  - 이 그림은 secure coding을 구성하는 기본 흐름을 한 장에 모아 둔 것입니다. 사용자가 보낸 입력은 먼저 검증을 통과해야 하고, 핵심 로직은 인증과 인가 판단 안에서만 실행돼야 하며, 저장과 로그는 그 결과를 안전하게 다뤄야 합니다.
- **바꾸기 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 1. **클라이언트 검증만 믿는 경우**: 브라우저 검증은 편의 기능일 뿐입니다. 서버는 항상 다시 확인해야 합니다.
...
- **Secure Coding 101 (1/10): Secure Coding이란 무엇인가? (현재 글)**
