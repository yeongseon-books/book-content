...
> 연속분포의 핵심은 개념을 외우는 일이 아니라, 그 개념이 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- 대표적 연속분포 비교에서 가장 흔한 실수는 무엇일까요?
- 구간으로 읽는 습관이 핵심입니다을 실무에 적용할 때 주의할 점은 무엇일까요?
- 5단계로 보는 연속분포의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **정규분포**: 가장 중요한 연속분포입니다. 종 모양의 대칭 분포로 측정 오차, 키, 점수, 평균 분포 등 많은 자연현상이 정규분포에 가까습니다.
- **지수분포**: 대기시간, 고장 간격, 생존 시간을 모델링합니다. 무기억성을 가지며, 포아송 프로세스의 도착 간격으로 해석됩니다.
- **감마분포**: 지수분포를 일반화한 형태로, 여러 대기시간의 합을 모델링할 때 사용됩니다. shape 파라미터로 분포의 모양을 조절할 수 있습니다.
...
정규분포에서 가장 유명한 규칙은 **68-95-99.7 규칙**(3-sigma rule)입니다. 평균 μ와 표준편차 σ를 가진 정규분포 N(μ, σ²)에서:
...
시각화를 통해 각 분포의 모양과 파라미터의 영향을 직관적으로 파악할 수 있습니다. 정규분포는 종 모양, 지수분포는 오른쪽 기울어진 모양, 감마분포는 shape에 따라 모양이 달라집니다.
...
여섯째, CDF와 역함수(ppf)의 관계를 혼동하기 쉽습니다. CDF는 "이 값 이하일 확률"을 돌려주고, ppf는 "이 확률에 해당하는 값"을 돌려줍니다. 둘은 역함수 관계입니다.

## 지수분포와 포아송분포의 관계

지수분포와 포아송분포는 독립적인 분포가 아니라, 같은 프로세스를 다른 관점에서 본 것입니다. 포아송 프로세스(Poisson process)라는 확률적 모형 안에서 둘은 자연스럽게 연결됩니다.

**관계**:

- **포아송분포**: 단위 구간(예: 1시간) 안에 발생하는 **사건 횟수**를 모델링합니다.
- **지수분포**: 사건 사이의 **대기시간(간격)**을 모델링합니다.
...
실무에서 로그 오류 횟수는 포아송으로, 다음 오류까지의 간격은 지수분포로 볼 수 있습니다. 동일한 현상을 다른 각도에서 본 것일 뿐입니다.
...
- **연속형 값을 확률적으로 모델링한다는 말은 무엇일까요?**
  - 연속형 값을 확률적으로 모델링한다는 말은 무엇일까요 — 본문에서 구체적으로 다룹니다.
- **확률밀도함수는 왜 확률처럼 보이지만 확률이 아닐까요?**
  - 확률밀도함수는 왜 확률처럼 보이지만 확률이 아닐까요 — 본문에서 구체적으로 다룹니다.
- **균등, 정규, 지수, 감마분포는 각각 어떤 상황에서 쓰일까요?**
  - 균등, 정규, 지수, 감마분포는 각각 어떤 상황에서 쓰일까요 — 본문에서 구체적으로 다룹니다.
- **대표적 연속분포 비교에서 가장 흔한 실수는 무엇일까요?**
  - 다음은 대표적인 연속분포들의 특징을 비교한 표입니다. 각 분포의 PDF 형태, 매개변수, 기댓값과 분산, 그리고 주요 사용 상황을 한 눈에 볼 수 있습니다.
- **구간으로 읽는 습관이 핵심입니다을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - "키가 180일 확률"이라고 말하고 싶어질 때가 있습니다. 하지만 연속형에서는 한 점의 확률이 0입니다. 대신 "180 이상일 확률"처럼 구간으로 바꿔 읽어야 합니다. 이 구간 사고가 생기면 PDF와 CDF의 역할도 훨씬 자연스럽게 보입니다.
