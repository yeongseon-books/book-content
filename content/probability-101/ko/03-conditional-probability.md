...
> 조건부확률의 핵심은 개념을 외우는 일이 아니라, 그 개념이 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- 독립 vs 종속 사건에서 가장 흔한 실수는 무엇일까요?
- 기저율 오류 시뮬레이션을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다중 조건의 체이닝의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
체이닝의 핵심은 앞의 결과가 뒤의 조건을 바꾼다는 사실입니다. 비복원 추출에서는 앞의 결과가 남은 표본공간을 축소시키므로 각 단계의 확률이 달라집니다.
...
조건부확률은 맥락을 다루는 도구입니다. 이 글에서 남겨야 할 핵심은 세 가지입니다. 조건이 붙으면 분모가 바뀐다는 점, 조건의 방향을 바꾸면 전혀 다른 질문이 된다는 점, 그리고 기저율을 함께 봐야 숫자의 의미를 제대로 읽을 수 있다는 사실입니다.
...
- **조건부확률은 왜 분모가 바뀌는 문제일까요?**
  - `P(A|B)`는 전체 집단이 아니라 B가 일어난 경우만 남겨 다시 비율을 계산하는 값입니다. 2x2 분할표에서도 `P(양성|질병)`의 분모는 질병 있는 50명이고, `P(질병|양성)`의 분모는 양성 판정 50명입니다.
  - 스팸 필터 예시에서도 기준 집단이 바뀝니다. 단어를 보기 전 `P(스팸)=0.300`이었지만 `"당첨"`을 본 뒤에는 `P(스팸|"당첨")=0.857`이 되어, 전체 메일이 아니라 `"당첨"`이 나온 메일 안에서 다시 계산합니다.
- **`P(A|B)`와 `P(B|A)`는 왜 전혀 다른 값일 수 있을까요?**
  - 두 식은 서로 다른 질문입니다. 글에서도 `P(양성|질병)`은 검사 민감도이고 `P(질병|양성)`은 양성예측도라서 같은 표를 보고도 분모와 해석이 달라집니다.
  - 날씨와 교통체증 트리에서 `P(체증|비)=0.8`이어도 `P(비|체증)=0.632`입니다. 한쪽은 비가 왔을 때 체증이 생길 확률이고, 다른 쪽은 체증이 보였을 때 원인이 비일 확률입니다.
  - 기저율 오류 시뮬레이션도 이 방향 차이를 보여 줍니다. 민감도 99%만 보면 `P(양성|질병)`을 본 것이고, 실제로 궁금한 `P(질병|양성)`은 유병률 0.1%에서 훨씬 낮게 나옵니다.
- **곱셈정리는 어떤 상황에서 자연스럽게 등장할까요?**
  - 트리의 한 경로를 계산할 때 바로 등장합니다. `P(비 ∩ 체증) = P(비) × P(체증|비) = 0.3 × 0.8 = 0.240`처럼 경로 확률을 곱으로 계산했습니다.
  - 조건이 연쇄로 붙는 비복원 추출에서도 같은 구조를 씁니다. 스페이드 3장을 연속으로 뽑을 확률을 `13/52 × 12/51 × 11/50`으로 계산한 부분이 전형적인 예시입니다.
  - 교집합을 조건부확률로 다시 쓸 때도 곱셈정리가 기본입니다. 그래서 전체확률법칙과 베이즈 정리의 분자·분모 계산이 모두 이 식에서 출발합니다.
...
- **Probability 101 (3/10): 조건부확률 (현재 글)**
