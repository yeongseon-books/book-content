...
- 포함-배제 원리는 실무에서 어떤 문제를 해결할까요?

잘못된 확률 답의 상당수는 잘못된 표본공간에서 나옵니다. 주사위 두 개를 던질 때 `(1,6)`과 `(6,1)`을 같은 결과로 볼지 다른 결과로 볼지부터 분명하지 않으면, 이후 계산은 멀쩡해 보여도 전부 다른 문제를 푼 셈이 됩니다. 표본공간을 집합으로 정리해 두면 확률 문제가 갑자기 집합 연산 문제로 바뀝니다. 사건은 부분집합이 되고, 합사건은 합집합이 되며, 곱사건은 교집합이 됩니다.
...
교집합의 공식은 A와 B가 독립일 때만 곱셈으로 쓸 수 있습니다. 독립이 아니면 조건부확률을 써야 합니다: `P(A ∩ B) = P(A | B) × P(B)`. 여집합은 가장 간단하지만 가장 유용한 공식 중 하나입니다. 여집합을 이용하면 복잡한 사건을 더 간단한 사건으로 바꿀 수 있습니다.
...
실무에서는 여러 조건이 겹칠 때 사용자 수를 올바르게 세기 위해 이 원리를 씁니다. 예를 들어 "A 기능 사용자 또는 B 기능 사용자"를 세려면 두 기능을 모두 쓰는 사용자를 한 번만 세어야 합니다.
## 문제를 집합으로 바꾸면 쉬워집니다
...
방법 1은 균등한 확률을 가정할 수 있어 계산이 간단하지만, 방법 2는 각 결과에 서로 다른 확률을 부여해야 합니다.
...
실무 시스템에서도 사건 정의는 출발점입니다. A/B 테스트의 그룹 정의, 사기 탐지 규칙의 발동 사건, 검색 평가의 관련성 사건처럼 결국 지표는 어떤 사건을 어떻게 정의했는지에서 시작합니다. 강한 팀일수록 계산식보다 먼저 이벤트 정의를 문서로 남깁니다.
...
- [ ] 확률 문제를 풀기 전에 표본공간을 먼저 정의하는 습관이 있다
- [ ] 합사건, 곱사건, 여사건을 집합 연산으로 구분할 수 있다
- [ ] 상호배반과 독립의 차이를 코드와 수식으로 설명할 수 있다
- [ ] 순서 유무와 복원 여부를 문제에 명시한 뒤 계산에 들어간다
- [ ] 포함-배제 원리를 두 사건 이상에서 적용할 수 있다
- [ ] 드 모르간의 법칙을 확률 문제에 활용할 수 있다
- [ ] 몬테카를로 시뮬레이션으로 이론값을 검증할 수 있다

## 정리

표본공간과 사건은 확률의 문법입니다. 이 글에서 남겨야 할 핵심은 세 가지입니다. 표본공간을 먼저 적어야 한다는 점, 사건은 집합으로 정의할 때 가장 분명해진다는 점, 그리고 상호배반과 독립은 닮아 보여도 전혀 다른 질문이라는 사실입니다.
...
- **표본공간은 왜 확률 계산보다 먼저 나와야 할까요?**
  - 가능한 모든 결과를 먼저 적어야 분모가 정해지고, 그 위에서 사건과 확률을 정의할 수 있습니다.
- **사건을 집합으로 쓰면 무엇이 쉬워질까요?**
  - 합집합, 교집합, 여집합 연산으로 복잡한 사건을 체계적으로 분해하고 계산할 수 있습니다.
- **합사건, 곱사건, 여사건은 각각 어떤 집합 연산에 대응될까요?**
  - 합사건은 합집합(A∪B), 곱사건은 교집합(A∩B), 여사건은 여집합(Aᶜ)으로 대응됩니다.
- **상호배반과 독립은 왜 전혀 다른 개념일까요?**
  - 상호배반은 동시에 일어날 수 없다는 뜻(A∩B=∅)이고, 독립은 한쪽이 다른 쪽에 영향을 주지 않는다는 뜻(P(A∩B)=P(A)·P(B))입니다. 상호배반이면 오히려 강하게 종속됩니다.
- **순서를 구분하느냐 마느냐가 왜 답을 바꿀까요?**
  - 동전 두 개를 던질 때 순서를 구분하면 4가지 결과, 무시하면 3가지이지만 확률이 비균등해집니다. 등확률 가정이 깨지면 답이 달라집니다.
- **포함-배제 원리는 실무에서 어떤 문제를 해결할까요?**
  - 여러 조건이 겹치는 사용자를 중복 없이 세거나, A/B 테스트에서 교집합을 정확히 처리할 때 포함-배제 원리가 필수입니다.

<!-- toc:begin -->
## 시리즈 목차

- [Probability 101 (1/10): 확률이란 무엇인가?](./01-what-is-probability.md)
- **Probability 101 (2/10): 사건과 표본공간 (현재 글)**
