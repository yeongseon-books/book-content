...
> 이산분포의 핵심은 개념을 외우는 일이 아니라, 그 개념이 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- 대표적 이산분포 비교에서 가장 흔한 실수는 무엇일까요?

- 분포 선택 가이드의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
포아송 과정의 가장 중요한 성질은 "구간을 늘리면 λ도 비례해서 커진다"는 점입니다. 1시간에 λ=8이면 2시간은 λ=16이 됩니다. 이 성질 덕분에 용량 계획이나 SLA 설정에 포아송을 쉽게 적용할 수 있습니다.
...
이산분포는 카운트 데이터를 읽는 기본 사전입니다. 이 글에서 남겨야 할 핵심은 세 가지입니다. 문제 상황을 분포에 매핑하는 감각이 가장 중요하다는 점, 각 분포는 서로 다른 생성 과정을 요약한다는 점, 그리고 모수를 정하면 평균·분산·확률이 한꺼번에 따라온다는 사실입니다. 이 세 가지를 기억하면 새로운 분포를 만나도 빠르게 파악할 수 있습니다. 음이항분포, 초기하분포처럼 아직 다루지 않은 분포도 같은 틀 안에서 이해할 수 있습니다.
...
- **0과 1의 한 번 실험은 왜 베르누이분포로 시작할까요?**
  - 0과 1의 한 번 실험은 왜 베르누이분포로 시작할까요 — 본문에서 구체적으로 다룹니다.
- **여러 번의 성공 횟수는 언제 이항분포가 될까요?**
  - 여러 번의 성공 횟수는 언제 이항분포가 될까요 — 본문에서 구체적으로 다룹니다.
- **첫 성공까지의 시도 수는 왜 기하분포로 읽을까요?**
  - 첫 성공까지의 시도 수는 왜 기하분포로 읽을까요 — 본문에서 구체적으로 다룹니다.
- **대표적 이산분포 비교에서 가장 흔한 실수는 무엇일까요?**
  - 다음은 대표적인 이산분포들의 특징을 비교한 표입니다. 각 분포가 어떤 매개변수를 가지고, PMF는 어떤 형태이며, 기댓값과 분산은 어떻게 계산되고, 어떤 상황에서 사용되는지 한 눈에 볼 수 있습니다.
...
- **Probability 101 (7/10): 이산분포 (현재 글)**
