...
- 모델이 출력하는 0.8이라는 점수를 어디까지 믿어야 할까요?
...
확률의 기본 문법을 잡으려면 순서가 중요합니다. 가능한 결과를 먼저 정하고(표본공간 Ω), 그다음 관심 있는 결과를 정의하고(사건 A ⊆ Ω), 마지막에 확률을 부여합니다(P(A) ∈ [0, 1]). 이 순서가 빠지면 같은 문제를 두 사람이 전혀 다른 문제로 읽게 됩니다. 빈도주의는 반복 실험의 장기 상대도수로 확률을 해석하고, 베이지안은 현재 가진 정보 아래에서 믿음의 정도로 확률을 해석합니다.
...
확률의 가장 기본적인 약속은 값이 0과 1 사이에 있고, 전체 질량의 합이 1이라는 사실입니다. 이 약속이 무너지면 확률분포라고 부를 수 없습니다. 공리를 모두 외우지 않더라도, 적어도 전체 합이 1이어야 한다는 감각은 처음부터 몸에 붙여 두는 편이 좋습니다.
...
- [ ] 빈도주의와 베이지안의 차이를 실무 예시와 함께 말할 수 있다
- [ ] 확률과 가능도를 혼동하지 않고 구분할 수 있다
- [ ] 간단한 시뮬레이션으로 확률 직관을 확인할 수 있다
- [ ] 전체 확률 질량의 합이 1이라는 조건을 코드로 검증할 수 있다
- [ ] 표본공간 설계가 잘못되면 답이 틀리는 이유를 설명할 수 있다

## 정리

확률은 불확실성을 숫자로 정리하는 언어입니다. 이 글에서 먼저 가져가야 할 핵심은 세 가지입니다. 확률은 표본공간 위에서 정의된다는 점, 반복 속 비율과 믿음의 갱신이라는 두 관점이 함께 존재한다는 점, 그리고 작은 코드 실험이 추상 개념을 빠르게 현실로 끌어내린다는 사실입니다.
...
- **확률은 정확히 무엇을 나타내는 숫자이고, 왜 0과 1 사이여야 할까요?**
  - 확률은 불확실성을 숫자로 표현하는 방식입니다. 콜모고로프 공리에 의해 0 이상 1 이하이며 전체 질량의 합이 1이어야 분포로서 의미를 가집니다.
- **표본공간과 사건을 먼저 정해야 하는 이유는 무엇일까요?**
  - 표본공간을 정해야 어떤 결과가 가능한지 분명하고, 그 위에서 사건과 확률을 정의할 수 있습니다. 표본공간이 다르면 같은 문장도 다른 문제가 됩니다.
- **빈도주의와 베이지안은 같은 현상을 어떻게 다르게 읽을까요?**
  - 빈도주의는 반복 속 비율을, 베이지안은 정보 갱신을 강조합니다. 실무에서는 상황에 따라 두 관점을 섞어 씁니다.
- **확률과 가능도(likelihood)는 기호가 비슷한데 무엇이 다를까요?**
  - 확률은 가정이 주어졌을 때 데이터를 보고, 가능도는 데이터가 주어졌을 때 가정이 얼마나 그럴듯한지를 비교합니다. 합 제약도 다릅니다.
- **모델이 출력하는 0.8이라는 점수를 어디까지 믿어야 할까요?**
  - 모델 점수는 불확실성을 정량화한 값이므로 가정, 해석, 한계를 함께 봐야 합니다. 0.99도 사실상 1이 아니며, 운영 환경에서 이 차이가 비용을 만듭니다.
- **시뮬레이션만으로 해석적 해 없이 확률을 추정할 수 있을까요?**
  - 몬테카를로 시뮬레이션은 충분히 반복하면 비율로 확률을 추정할 수 있습니다. π 추정과 생일 문제에서 확인했습니다.

<!-- toc:begin -->
## 시리즈 목차

- **Probability 101 (1/10): 확률이란 무엇인가? (현재 글)**
