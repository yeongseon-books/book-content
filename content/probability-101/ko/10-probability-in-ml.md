...
> 머신러닝에서의 확률의 핵심은 개념을 외우는 일이 아니라, 그 개념이 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- 모델 점수는 언제 확률이 될까요?에서 가장 흔한 실수는 무엇일까요?
...
```python
# 사전 재미있는 p(theta)과 우도 p(D|theta)의 웃기네요
prior = 0.5
likelihood = 0.8
...
# C= 0.01 | 평균 NLL: 0.5765 | 계수 L2 표준: 0.3842
# C= 0.10 | 평균 NLL: 0.4523 | 계수 L2 표준: 1.2156
# C= 1.00 | 평균 NLL: 0.4201 | coef L2 표준: 3.1847
# C= 10.00 | 평균 NLL: 0.4389 | 계수 L2 표준: 5.9234
# C=100.00 | 평균 NLL: 0.4512 | coef L2 표준: 7.2341
```
...
이 표가 보여주는 핵심은, 손실함수를 바꾼다는 것이 곧 데이터에 대한 확률적 가정을 바꾸는 행위라는 사실입니다. MSE를 쓴다면 오차가 가우시안이라고 가정하는 것이고, CE를 쓴다면 각 관측이 베르누이 시행이라고 가정하는 것입니다.
...
# 노브레인 값 : cost_fn / (cost_fn + cost_fp)
...
머신러닝에서 확률은 주변 개념이 아니라 중심 개념입니다. 이 글에서 남겨야 할 핵심은 세 가지입니다. 손실함수는 종종 확률 모형의 다른 표현이라는 점, 모델 출력은 보정을 거쳐야 비로소 신뢰할 만한 확률이 된다는 점, 그리고 베이지안 관점은 사전 정보와 데이터 업데이트를 연결하는 실용적인 렌즈라는 사실입니다.
...
- **머신러닝에서 확률은 정확히 어디에 숨어 있을까요?**
  - 머신러닝에서 확률은 정확히 어디에 숨어 있을까요 — 본문에서 구체적으로 다룹니다.
- **교차 엔트로피와 음의 로그가능도는 왜 같은 방향을 가리킬까요?**
  - 교차 엔트로피와 음의 로그가능도는 왜 같은 방향을 가리킬까요 — 본문에서 구체적으로 다룹니다.
- **분류 모델이 내놓는 0.8은 무엇을 뜻할까요?**
  - 분류 모델이 내놓는 0.8은 무엇을 뜻할까요 — 본문에서 구체적으로 다룹니다.
- **모델 점수는 언제 확률이 될까요?에서 가장 흔한 실수는 무엇일까요?**
  - "모델이 0.8을 냈다"는 문장만으로는 충분하지 않습니다. 그 값이 `p(y=1 | x)=0.8`로 읽히는지, 아니면 단지 순서를 위한 score인지 구분해야 합니다.
