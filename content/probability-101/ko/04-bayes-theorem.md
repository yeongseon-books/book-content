...
> 베이즈 정리의 핵심은 개념을 외우는 일이 아니라, 그 개념이 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- Python 예제: 의료 검진 양성 판정에서 가장 흔한 실수는 무엇일까요?
- Naive Bayes 분류기 예제을 실무에 적용할 때 주의할 점은 무엇일까요?
- 식보다 먼저 봐야 할 직관의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
베이즈 정리의 강력함은 데이터가 순차적으로 들어와도 같은 형태로 계속 갱신할 수 있다는 사실입니다. 이번 단계의 사후확률이 다음 단계의 사전확률이 됩니다.
...
첫 번째 사후확률은 다음 단계의 사전확률이 됩니다. 베이즈 정리가 강력한 이유 중 하나는 데이터가 순차적으로 들어와도 같은 형태로 계속 갱신할 수 있다는 사실입니다.
...
베이즈 정리는 학습의 수학입니다. 이 글에서 남겨야 할 핵심은 세 가지입니다. 사후확률은 새 데이터와 기존 믿음을 함께 반영한다는 점, 기저율이 작으면 검사 결과의 해석도 달라진다는 점, 그리고 한 번 구한 사후확률이 다음 단계의 사전확률이 된다는 사실입니다.
...
- **베이즈 정리는 어떤 질문에 답하는 식일까요?**
  - 베이즈 정리는 "데이터 B를 본 뒤 가설 A의 확률을 얼마로 갱신할 것인가"에 답합니다. 글의 기본식 `P(A|B) = P(B|A) × P(A) / P(B)`가 바로 그 규칙입니다.
  - 의료 검진 예시에서는 `P(양성|질병)`이 아니라 `P(질병|양성)`을 구하는 데 쓰였습니다. 즉 검사 결과를 본 뒤 실제 질병 확률을 다시 계산하는 식입니다.
  - 순차 갱신 예시처럼 이 질문은 한 번으로 끝나지 않습니다. 첫 사후확률이 다음 사전확률이 되어 `0.010 → 0.154 → 0.783 → 0.280`으로 믿음이 계속 업데이트됩니다.
- **사전확률, 우도, 사후확률은 각각 무엇을 뜻할까요?**
  - 사전확률은 데이터를 보기 전 믿음입니다. 글의 표에서 `P(질병)=0.01`, 드물 질병 시나리오의 유병률 0.1%가 여기에 해당합니다.
  - 우도는 가설이 참일 때 데이터가 나올 가능성입니다. 진단 문제에서는 `P(양성|질병)=0.99` 같은 민감도가 우도 역할을 합니다.
  - 사후확률은 데이터를 본 뒤 갱신된 믿음입니다. 표의 `P(질병|양성)=0.168`이나 드물 질병 시나리오의 9.0%가 바로 최종 판단값입니다.
- **기저율이 작으면 왜 양성 판정의 의미가 달라질까요?**
  - 기저율이 작으면 거짓 양성이 전체 양성에서 차지하는 비중이 커집니다. 민감도 99%, 특이도 99%라도 유병률 0.1%에서는 양성 후 확률이 9.0%에 그쳤습니다.
  - PPV 표에서도 유병률 0.0010일 때 PPV가 0.019라서 양성의 98.1%가 오경보이고, 유병률 0.1000에서는 PPV가 0.688로 크게 올라갑니다.
  - 그래서 검사 성능 숫자만 보는 것으로는 부족하고, `P(양성)`을 만드는 사전확률까지 넣어야 양성 판정의 실제 의미를 읽을 수 있습니다.
