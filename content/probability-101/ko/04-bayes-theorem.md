---
series: probability-101
episode: 4
title: 베이즈 정리
status: content-ready
targets:
  tistory: true
  medium: true
  hashnode: true
  mkdocs: true
  ebook: true
language: ko
tags:
  - Probability
  - Bayes
  - Inference
  - Posterior
  - Beginner
seo_description: 베이즈 정리의 식과 직관, 사전 우도 사후 관계를 의료 검사 예시와 코드로 풀어내고 흔한 오해를 정리한 입문 글
last_reviewed: '2026-05-11'
---

# 베이즈 정리

> Probability 101 시리즈 (4/10)


## 이 글에서 다룰 문제

베이즈 정리는 불확실성을 갱신하는 가장 일관된 규칙입니다. 스팸 분류, 의료 진단, A/B 테스트, RL 모두 이 식을 사용합니다.

> *Bayes' theorem is the engine of inference.*

## 전체 흐름
```mermaid
flowchart LR
    Prior["Prior P(H)"] --> Update["Likelihood P(D|H)"]
    Update --> Post["Posterior P(H|D)"]
    Post --> NewPrior["Becomes new prior"]
```

## Before/After

**Before**: “검사 양성 → 병이 있다.”

**After**: P(병|+) = P(+|병)·P(병) / P(+) — 기저율이 작으면 PPV도 작습니다.

## 5단계 베이즈

### 1단계 — 사전 / 우도

```python
prior = 0.01           # P(disease)
sens = 0.99            # P(+|disease)
spec = 0.95            # P(-|no disease)
```

### 2단계 — 증거 P(+)

```python
p_pos = sens * prior + (1 - spec) * (1 - prior)
print("P(+):", p_pos)
```

### 3단계 — 사후 P(질병 | +)

```python
post = sens * prior / p_pos
print("P(disease | +):", post)
```

### 4단계 — 두 번째 양성 갱신

```python
prior2 = post
p_pos2 = sens * prior2 + (1 - spec) * (1 - prior2)
post2 = sens * prior2 / p_pos2
print("after 2 positives:", post2)
```

### 5단계 — 오즈 형태

```python
prior_odds = 0.01 / 0.99
likelihood_ratio = sens / (1 - spec)
post_odds = prior_odds * likelihood_ratio
print("posterior odds:", post_odds, "P:", post_odds / (1 + post_odds))
```

## 이 코드에서 주목할 점

- 기저율이 작으면 민감한 검사라도 PPV가 낮을 수 있습니다.
- 사후확률은 다음 사전확률이 됩니다. 이것이 순차 갱신입니다.
- 오즈 형태는 계산을 단순화합니다.

## 자주 하는 실수 5가지

1. P(D|H)와 P(H|D)를 같은 값으로 봅니다.
2. 기저율을 무시합니다.
3. 사전확률이 없는 것처럼 다룹니다.
4. 우도와 확률을 혼동합니다.
5. 순차 갱신에서 독립 가정을 빠뜨립니다.

## 실무에서는 이렇게 쓰입니다

스팸 필터(Naive Bayes), A/B 테스트(Bayesian), 의료 진단, RL의 belief state처럼 베이즈 추론은 probabilistic ML의 핵심입니다.

## 체크리스트

- [ ] 베이즈 정리를 식으로 안다.
- [ ] 사전, 우도, 사후를 구분한다.
- [ ] PPV 계산을 안다.
- [ ] 순차 갱신을 할 수 있다.

## 정리 및 다음 단계

베이즈 정리는 학습의 수학입니다. 다음 글에서는 확률변수로 수치적 결과를 다룹니다.

<!-- toc:begin -->
- [확률이란 무엇인가?](./01-what-is-probability.md)
- [사건과 표본공간](./02-events-and-sample-space.md)
- [조건부확률](./03-conditional-probability.md)
- **베이즈 정리 (현재 글)**
- 확률변수 (예정)
- 기대값과 분산 (예정)
- 이산분포 (예정)
- 연속분포 (예정)
- 대수의 법칙과 중심극한정리 (예정)
- 머신러닝에서의 확률 (예정)
<!-- toc:end -->

## 참고 자료

- [3Blue1Brown — Bayes' theorem](https://www.3blue1brown.com/lessons/bayes-theorem)
- [Wikipedia — Bayes' theorem](https://en.wikipedia.org/wiki/Bayes%27_theorem)
- [Stanford CS109 — Notes](https://web.stanford.edu/class/cs109/)
- [Kevin Murphy — Probabilistic ML](https://probml.github.io/pml-book/book1.html)

Tags: Probability, Bayes, Inference, Posterior, Beginner
