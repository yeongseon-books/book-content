...
> 기대값과 분산의 핵심은 개념을 외우는 일이 아니라, 그 개념이 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- 표준편차는 분산과 어떻게 다를까요?
- 기댓값의 주요 성질에서 가장 흔한 실수는 무엇일까요?
- 중심만 보면 부족합니다을 실무에 적용할 때 주의할 점은 무엇일까요?
- 5단계로 보는 기대값과 분산의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
여기서 중요한 감각은 기대값이 반드시 실제로 나올 수 있는 값일 필요는 없다는 사실입니다. 주사위 평균 3.5가 대표적인 예입니다. 평균은 관측 가능한 값이라기보다 중심을 나타내는 요약값입니다.
...
기대값과 분산은 분포의 두 축입니다. 이 글에서 남겨야 할 핵심은 세 가지입니다. 기대값은 중심을, 분산은 퍼짐을 요약한다는 점, 표준편차는 분산보다 해석이 직관적이라는 점, 그리고 선형성은 복잡한 문제를 훨씬 단순하게 만들어 준다는 사실입니다.
...
- **기대값은 왜 분포의 중심이라고 부를까요?**
  - 기댓값은 단순히 평균을 계산하는 공식이 아니라, 강력한 수학적 성질을 가진 연산자입니다
- **분산은 무엇을 얼마나 측정하는 값일까요?**
  - 분산은 평균으로부터 얼마나 떨어져 있는지를 측정하는 값입니다
- **표준편차는 분산과 어떻게 다를까요?**
  - 두 확률변수의 관계를 요약하는 값이 공분산(covariance)입니다. `Cov(X, Y) = E[(X - E[X])(Y - E[Y])]`입니다. 양수면 X가 클 때 Y도 큰 경향, 음수면 반대 경향입니다.
- **기댓값의 주요 성질에서 가장 흔한 실수는 무엇일까요?**
  - 기댓값은 단순히 평균을 계산하는 공식이 아니라, 강력한 수학적 성질을 가진 연산자입니다. 특히 선형성은 계산을 단순화하는 데 매우 유용하며, 독립 확률변수의 곱에 대한 성질은 분산 계산에도 이어집니다.
