---
title: "LLM Fine-tuning 101 (4/6): 학습 루프와 하이퍼파라미터"
...
이 글은 LLM 파인튜닝 101 시리즈의 4번째 글입니다.
...
![LLM Fine-tuning 101 4장 흐름 개요](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/04/04-02-what-you-can-shrink-and-what-you-cannot.ko.png)
*LLM Fine-tuning 101 4장 흐름 개요*
...
샘플 수와 스텝 수는 줄일 수 있습니다. 하지만 **토큰화된 입력, labels, 옵티마이저 스텝, 손실 계산**은 줄이면 안 됩니다. 하나라도 빠지면 학습 검증이 아니라 추론 테스트가 됩니다. 그래서 이 글의 가장 작은 예제도 학습 관련 구성요소만큼은 그대로 유지합니다.

![줄여도 되는 것과 줄이면 안 되는 것](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/04/04-01-what-you-can-shrink-and-what-you-cannot.ko.png)
...
![배치 크기와 그래디언트 누적의 관계](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/04/04-03-what-to-notice-in-this-code.ko.png)
...
![학습 디버깅에서 먼저 볼 출력 판단 흐름](https://yeongseon-books.github.io/book-public-assets/assets/llm-finetuning-101/04/04-04-where-engineers-get-confused.ko.png)
...
## 적용 전후 생성 예시: 학습 스텝이 의미 있게 작동했는지 확인
...
- **`TrainingArguments`에서 한 번의 학습 스텝을 돌리려면 최소 무엇을 설정해야 할까요?**
  - 이 글의 최소 기준선은 `output_dir`, `per_device_train_batch_size`, `max_steps`, `learning_rate`, `save_strategy`, `report_to` 정도입니다. 여기에 실제 `labels`가 들어 있는 데이터셋과 LoRA가 붙은 모델이 연결되어야 비로소 한 번의 진짜 업데이트가 일어납니다. 즉 `Trainer.train()` 한 줄보다 그 앞의 입력 계약이 먼저 완성돼 있어야 합니다.
- **작은 실험에서도 `labels`와 데이터 콜레이터가 왜 중요할까요?**
  - `labels`가 없으면 손실을 계산할 수 없고, 콜레이터가 없으면 길이가 다른 샘플을 배치로 묶는 순간부터 학습이 쉽게 깨집니다. 특히 causal LM에서는 `input_ids`를 복사한 뒤 필요한 구간을 마스킹하는 방식이 손실 계산의 출발점입니다. 샘플 수가 적더라도 이 두 요소를 빼면 학습 검증이 아니라 단순 추론 테스트가 됩니다.
- **학습 루프를 디버깅할 때 어떤 출력부터 읽어야 할까요?**
  - 먼저 `trainable params`가 0이 아닌지, 첫 스텝 `train_loss`가 유한한 값인지, `global_step`이 실제로 증가했는지 봐야 합니다. 그다음에 학습률 로그, eval perplexity, 체크포인트 크기를 보면 어느 층이 흔들리는지 빠르게 좁힐 수 있습니다. 이 순서를 고정하면 NaN, OOM, 배선 오류를 같은 언어로 진단하기 쉬워집니다.
