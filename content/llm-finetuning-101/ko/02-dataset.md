...
데이터셋 단계에서는 양보다 구조가 먼저입니다. 이 글에서는 원본 샘플, 템플릿 적용 텍스트, 토큰화 텐서라는 세 층으로 나눠 파인튜닝 데이터셋을 어떻게 준비해야 하는지 설명합니다.

이 글은 LLM 파인튜닝 101 시리즈의 두 번째 글입니다.
...
> 좋은 fine-tuning(파인튜닝) 데이터셋은 문장 더미가 아니라, 모델이 반복해서 모방해야 하는 **요청-응답 계약**입니다.
