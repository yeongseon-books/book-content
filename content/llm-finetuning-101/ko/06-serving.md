...
- **파인튜닝된 작은 모델을 FastAPI 엔드포인트 뒤에 두기 위한 최소 구조는 무엇일까요?**
  - 최소 구조는 `/health`와 `/generate`를 가진 FastAPI 앱, 시작 시 한 번만 로드되는 베이스 모델과 어댑터, 그리고 요청 스키마를 검증하는 Pydantic 모델입니다. 이 세 층이 있어야 모델이 HTTP 계약 뒤에 들어가고, 팀이 같은 방식으로 호출할 수 있습니다. 글의 예제가 작은 이유도 바로 이 최소 경계를 선명하게 보여 주기 위해서입니다.
- **서빙 코드에서 학습과 추론의 경계는 어디에 그어야 할까요?**
  - 학습은 어댑터를 만들어 내는 단계에서 끝나고, 서빙은 이미 저장된 베이스 모델과 어댑터를 읽어 요청마다 `generate()`를 수행하는 단계에서 시작합니다. 그래서 서빙 코드에서는 `datasets`나 Trainer보다 모델 로드, 요청 검증, 생성 옵션, 에러 처리, 지연 시간 측정이 중심이 됩니다. 같은 모델이라도 학습 관점의 배치 사고를 서빙 코드에 그대로 가져오면 운영 경계가 흐려집니다.
- **브라우저를 열지 않고도 엔드포인트를 어떻게 검증할 수 있을까요?**
  - 메모리 안에서 앱을 호출하는 `TestClient`로 `/health`와 `/generate`를 먼저 확인하면 됩니다. 이후 실제 프로세스에서는 `curl` 두 줄만으로 상태, 요청 스키마, 기본 생성 경로를 반복 검증할 수 있습니다. 이 방식이 좋은 이유는 CI에서도 같은 명령을 그대로 써서 서빙 회귀를 빠르게 잡을 수 있기 때문입니다.
