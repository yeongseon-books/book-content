...
중요한 것은 절대값이 아니라, (1) 평가가 학습과 분리되어 있고, (2) 같은 데이터로 전후를 비교하며, (3) CI에서도 같은 계산이 반복된다는 사실입니다.
...
## 적용 전후 생성 비교: 정량 지표의 의미를 확인하는 앵커
...
## 적용 전후 생성 품질 비교: 평가 보고서에 포함할 최소 예시
...
- **파인튜닝 직후 가장 먼저 봐야 할 정량 신호인 perplexity는 어떻게 계산할까요?**
  - 평가 데이터셋에서 배치별 loss를 모아 평균을 낸 뒤 `exp(mean loss)`를 계산하면 됩니다. 그래서 글의 `perplexity()` 함수도 `model.eval()`과 `torch.no_grad()` 안에서 손실을 수집한 뒤 마지막에 `math.exp(...)`를 적용했습니다. 핵심은 절대값 자체보다 같은 hold-out 세트로 학습 전후를 반복 비교하는 데 있습니다.
- **학습 전후 perplexity 비교만으로는 왜 평가가 충분하지 않을까요?**
  - perplexity는 토큰 예측의 낯섦을 보여 주지만, 형식 준수, 사실성, 안전성, 사용자 만족까지 대신 설명해 주지는 못합니다. 실제로 이 글에서도 골든 세트 점수, format pass rate, safety 기준을 함께 두어 지표 충돌 시 판단 규칙을 정했습니다. 즉 perplexity는 빠른 회귀 감지선이지, 배포 결론 전체가 아닙니다.
- **작은 데모 모델에서도 왜 별도의 평가 루프를 유지해야 할까요?**
  - 생성 샘플 몇 개는 우연히 좋아 보일 수 있지만, 별도 평가 루프가 없으면 다음 실험과 공정하게 비교할 방법이 없습니다. 작은 모델이라도 hold-out perplexity와 골든 세트 채점을 자동화해 두면 6편 서빙 전에 배포 게이트로 바로 연결할 수 있습니다. 평가 루프는 모델 크기와 무관하게 재현성과 롤백 판단을 위해 필요합니다.
