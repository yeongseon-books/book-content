...
평가는 생성 예시 몇 개를 보는 일로 끝나지 않습니다. 이 글에서는 모델 내부 신호와 사용자 관점 품질을 분리해 파인튜닝 결과를 어떻게 재현 가능하게 검증할지 설명합니다.

이 글은 LLM 파인튜닝 101 시리즈의 다섯 번째 글입니다.
...
> Perplexity는 모델이 다음 토큰을 얼마나 낯설지 않게 받아들이는지를 보여 주는 지표일 뿐, 사람이 출력을 좋다고 느낄지까지 직접 보장하지는 않습니다.
