...
이 글은 LLM 파인튜닝 101 시리즈의 첫 번째 글입니다.
...
- **LoRA가 풀 파인튜닝보다 훨씬 가벼운 이유를 어떻게 계산할 수 있을까요?**
  - 핵심은 베이스 모델 전체를 학습하지 않고, 대상 선형 레이어마다 저랭크 행렬 `A`, `B`만 추가한다는 사실입니다. 그래서 파라미터 수를 셀 때는 전체 가중치가 아니라 `r * (in + out)`이 레이어마다 얼마나 더해지는지 계산하면 됩니다. 이 글에서 GPT-2 small 급 구조에 `r=8`을 넣었을 때 비율이 약 1.5%로 나온 이유도 바로 그 산식 때문입니다.
- **프롬프트로 해결할 수 있는 문제와 파인튜닝이 필요한 문제를 어떻게 구분할 수 있을까요?**
  - 말투, 출력 형식, 도메인 표현처럼 반복적으로 같은 행동을 학습시켜야 하는 문제는 LoRA가 잘 맞습니다. 반대로 최신 사실 주입이나 외부 문서 의존성이 큰 문제는 프롬프트나 RAG 쪽이 더 자연스럽습니다. 그래서 글에서도 스타일 보정, 지식 최신성, 큰 능력 변화라는 세 갈래로 의사결정을 나눴습니다.
- **GPU 없이도 1편에서 무엇을 검증할 수 있을까요?**
  - 파라미터 계산이 맞는지, rank를 두 배로 올렸을 때 trainable 비율이 선형에 가깝게 늘어나는지, 그리고 어떤 문제에 LoRA를 쓸지 의사결정 기준이 분명한지를 검증할 수 있습니다. 글 후반의 작은 Trainer 뼈대까지 읽어 두면 이후 장에서 데이터 문제와 학습 루프 문제를 더 빨리 분리할 수 있습니다. 즉 1편의 검증 대상은 GPU 성능이 아니라 계산 감각과 선택 기준입니다.
