...
LLM 파인튜닝은 모델을 한 번 더 학습시키는 작업처럼 보이지만, 실제로는 무엇을 바꾸고 무엇을 그대로 둘지 먼저 나누는 문제입니다. 이 글에서는 그 출발점을 세 가지 변수로 정리해 파인튜닝을 어떤 계산 감각으로 봐야 하는지 설명합니다.
...
> Full fine-tuning(풀 파인튜닝)이 건물을 통째로 다시 짓는 일이라면, LoRA는 몇 개의 핵심 기둥에 보강재를 덧대는 방식입니다.
