...
- **LoRA가 풀 파인튜닝보다 훨씬 가벼운 이유를 어떻게 계산할 수 있을까요?**
  - 핵심은 베이스 모델 전체를 학습하지 않고, 대상 선형 레이어마다 저랭크 행렬 `A`, `B`만 추가한다는 점입니다. 그래서 파라미터 수를 셀 때는 전체 가중치가 아니라 `r * (in + out)`이 레이어마다 얼마나 더해지는지 계산하면 됩니다. 이 글에서 GPT-2 small 급 구조에 `r=8`을 넣었을 때 비율이 약 1.5%로 나온 이유도 바로 그 산식 때문입니다.
- **프롬프트로 해결할 수 있는 문제와 파인튜닝이 필요한 문제를 어떻게 구분할 수 있을까요?**
