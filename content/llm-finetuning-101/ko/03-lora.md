...
- **`LoraConfig`에서 실제로 이해해야 할 필드는 무엇일까요?**
  - 이 글 기준으로는 `r`, `lora_alpha`, `lora_dropout`, `target_modules`, `bias`, `task_type`가 핵심입니다. 그중에서도 실제 사고를 가장 많이 만드는 것은 `target_modules`이고, 표현력과 비용 균형을 가장 크게 바꾸는 것은 `r`과 `alpha` 조합입니다. 즉 `LoraConfig`는 옵션 모음이 아니라 배선 위치와 보정 강도를 정하는 계약입니다.
- **`target_modules`를 잘못 지정하면 어떤 문제가 생길까요?**
  - 가장 흔한 경우는 에러 없이 `trainable params: 0`이 나와 학습이 도는 척만 하는 상황입니다. 반대로 의도하지 않은 레이어까지 넓게 잡으면 어댑터가 커지고 실험 비교도 흐려집니다. 그래서 GPT-2의 `c_attn`, `c_proj`처럼 실제 모듈 이름을 `named_modules()`로 확인한 뒤 붙이는 절차를 먼저 밟았습니다.
- **작은 GPT-2 계열 모델에서는 학습 가능한 파라미터 비율이 얼마나 낮아질까요?**
  - 이 글의 tiny GPT-2 예제에서는 대체로 1~3% 안쪽이 자연스러운 범위입니다. 실제로 `c_attn`, `c_proj`에 `r=8`을 붙였을 때도 1%대 비율이 출력돼 1편의 손계산 감각과 크게 어긋나지 않았습니다. 중요한 것은 정확히 몇 자리 수냐보다, 설정을 바꿨을 때 비율이 예상 방향으로 움직이는지 확인하는 일입니다.
