...
- **챗봇은 모델 외에 어떤 구성 요소를 더 필요로 할까요?**
  - 챗봇은 모델 외에 어떤 구성 요소를 더 필요로 할까요 — 본문에서 구체적으로 다룹니다.
- **multi-turn prompt format은 왜 직접 설계해야 할까요?**
  - multi-turn prompt format은 왜 직접 설계해야 할까요 — 본문에서 구체적으로 다룹니다.
- **FastAPI lifespan으로 모델을 한 번만 로드하면 무엇이 좋아질까요?**
  - FastAPI lifespan으로 모델을 한 번만 로드하면 무엇이 좋아질까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 글이 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 많은 입문자가 모델을 학습시키는 것과 서비스를 만드는 것을 별개의 일처럼 느낍니다. 하지만 실제 애플리케이션에서는 두 문제가 정확히 만납니다.
- **핵심 관점을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 챗봇을 "모델에 프롬프트를 넣고 답을 받는 도구"로만 보면 중요한 절반이 빠집니다. 더 정확한 관점은 이렇습니다. **챗봇은 모델, 대화 히스토리, 스트리밍 I/O, 사용자 인터페이스가 함께 움직이는 작은 시스템**입니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 챗봇을 "모델에 프롬프트를 넣고 답을 받는 도구"로만 보면 중요한 절반이 빠집니다. 더 정확한 관점은 이렇습니다. **챗봇은 모델, 대화 히스토리, 스트리밍 I/O, 사용자 인터페이스가 함께 움직이는 작은 시스템**입니다.
