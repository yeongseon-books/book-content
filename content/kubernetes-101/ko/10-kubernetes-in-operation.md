...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/272"
...
- 단계별로 운영 기본기 살펴보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글은 kubernetes-101 시리즈의 한 단계로, 핵심 개념을 실무 맥락에서 정리했습니다. 여기서 다룬 원칙들은 독립적으로도 유용하지만, 시리즈 전체와 연결될 때 더 큰 그림이 보입니다.

## 처음 질문으로 돌아가기

- **liveness, readiness, startup probe는 어떤 역할을 나눌까요?**
  - liveness는 "이 컨테이너를 죽이고 재시작해야 하는가"를, readiness는 "지금 트래픽을 받아도 되는가"를, startup은 "초기 부팅이 끝났는가"를 각각 본다는 점에서 책임이 다릅니다. 본문에서 강조했듯이 셋을 같은 경로로 잡으면 부팅이 느린 앱이 계속 죽거나, 워밍업이 끝나기 전에 트래픽을 받아 5xx가 터지는 일이 반복됩니다.
- **RBAC와 NetworkPolicy는 왜 운영의 기본 경계일까요?**
  - RBAC가 "누가 API 서버에 어떤 객체를 만지게 할지"를 제한한다면, NetworkPolicy는 "어떤 Pod가 어떤 Pod에 트래픽을 보낼 수 있는지"를 제한합니다. 본문에서 본 것처럼 둘 다 기본값이 "허용"에 가까워 운영자가 명시적으로 좁히지 않으면 실수 한 번이 전체 클러스터로 번질 수 있어, "기본 차단·필요한 것만 허용"이 운영 안전선이 됩니다.
- **메트릭, 로그, 트레이스는 왜 함께 봐야 할까요?**
  - 메트릭은 "지금 무엇이 얼마나 나쁜지", 로그는 "어떤 메시지가 났는지", 트레이스는 "어느 서비스 구간에서 시간이 새는지"를 보여 줍니다. 본문에서 강조했듯이 셋을 같은 시간축에서 묶지 않으면 배포 직후의 5xx 급증을 보고도 어느 서비스의 어느 호출이 원인인지 분리하기 어렵고, 결국 "클러스터가 느립니다" 수준의 모호한 결론에 머무릅니다.
