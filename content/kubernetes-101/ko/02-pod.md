...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 Pod와 컨테이너는 정확히 어떻게 다를, 둘째 왜 Kubernetes는 컨테이너가 아니라 Pod를 기본 단위로 삼을, 셋째 사이드카 패턴은 어떤 상황에서 필요할입니다. 한눈에 보는 구조에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **Pod와 컨테이너는 정확히 어떻게 다를까요?**
  - Pod와 컨테이너는 정확히 어떻게 다를까요 — 본문에서 단계별로 설명합니다.
- **왜 Kubernetes는 컨테이너가 아니라 Pod를 기본 단위로 삼을까요?**
  - 왜 Kubernetes는 컨테이너가 아니라 Pod를 기본 단위로 삼을까요 — 본문에서 단계별로 설명합니다.
- **사이드카 패턴은 어떤 상황에서 필요할까요?**
  - 사이드카 패턴은 어떤 상황에서 필요할까요 — 본문에서 단계별로 설명합니다.
- **한눈에 보는 구조에서 가장 흔한 실수는 무엇일까요?**
  - 이 구조의 핵심은 Pod 안의 컨테이너가 완전히 독립적이지 않다는 사실입니다. 같은 Pod에 들어간 컨테이너는 네트워크 네임스페이스와 볼륨을 공유합니다. 그래서 하나의 애플리케이션 본체와 그 옆에서 돕는 보조 컨테이너를 함께 묶는 패턴이 자연스럽게 나옵니다.
- **도입 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
