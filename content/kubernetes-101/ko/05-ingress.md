...
- 단계별로 호스트와 경로 라우팅 구성하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 Ingress와 IngressController는 왜 따로 이해해야 할, 둘째 여러 서비스를 하나의 도메인 아래에서 어떻게 나눌 수 있을, 셋째 `host`, `path`, `pathType`은 어떤 차이를 만들입니다. 한눈에 보는 구조에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **Ingress와 IngressController는 왜 따로 이해해야 할까요?**
  - 외부 로드 밸런서는 보통 클러스터 앞단에서 트래픽을 받아들이고, IngressController는 Ingress 규칙을 실제 프록시 동작으로 바꿉니다
- **여러 서비스를 하나의 도메인 아래에서 어떻게 나눌 수 있을까요?**
  - 이 글에서는 Ingress를 여러 서비스를 하나의 외부 진입점 뒤에 두고, 도메인과 경로 기준으로 HTTP 요청을 나누는 규칙 객체로 정리했습니다
- **`host`, `path`, `pathType`은 어떤 차이를 만들까요?**
  - 외부 로드 밸런서는 보통 클러스터 앞단에서 트래픽을 받아들이고, IngressController는 Ingress 규칙을 실제 프록시 동작으로 바꿉니다
- **한눈에 보는 구조에서 가장 흔한 실수는 무엇일까요?**
  - 외부 로드 밸런서는 보통 클러스터 앞단에서 트래픽을 받아들이고, IngressController는 Ingress 규칙을 실제 프록시 동작으로 바꿉니다. 결국 Ingress는 "어디로 보낼지"를 선언하고, Controller는 "어떻게 보낼지"를 실행합니다.
- **도입 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - Ingress가 없으면 서비스마다 외부 LoadBalancer를 따로 두기 쉽습니다
- **단계별로 호스트와 경로 라우팅 구성하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 이 예제는 `example.com/api`를 `api` 서비스로 보내고, 나머지 `/` 요청은 `web` 서비스로 보냅니다
