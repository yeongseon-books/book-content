...
> Ingress는 외부 HTTP 요청을 도메인과 경로 기준으로 나누는 규칙이고, IngressController는 그 규칙을 실제 프록시 설정으로 실행하는 구성요소입니다.

- Ingress와 IngressController는 왜 따로 이해해야 할까요?
- 여러 서비스를 하나의 도메인 아래에서 어떻게 나눌 수 있을까요?
- `host`, `path`, `pathType`은 어떤 차이를 만들까요?
- TLS 종료를 Ingress에서 처리하면 무엇이 단순해질까요?
- 왜 서비스마다 LoadBalancer를 하나씩 두는 방식이 곧 비싸지고 복잡해질까요?
...
![한눈에 보는 구조](https://yeongseon-books.github.io/book-public-assets/assets/kubernetes-101/05/05-01-concept-at-a-glance.ko.png)
*Ingress는 외부 진입점 뒤에서 여러 Service를 도메인과 경로 기준으로 나누는 HTTP 라우팅 계층입니다.*
