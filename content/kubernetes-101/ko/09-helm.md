...
- 단계별로 작은 차트 만들어 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 환경마다 YAML을 복사하는 방식은 왜 드리프트를 만들, 둘째 Chart와 `values.yaml`은 어떤 책임을 나눌, 셋째 `install`, `upgrade`, `rollback`은 어떤 흐름으로 이어질입니다. 한눈에 보는 구조에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **환경마다 YAML을 복사하는 방식은 왜 드리프트를 만들까요?**
  - 환경마다 YAML을 복사하는 방식은 왜 드리프트를 만들까요 — 본문에서 단계별로 설명합니다.
- **Chart와 `values.yaml`은 어떤 책임을 나눌까요?**
  - Chart와 `values.yaml`은 어떤 책임을 나눌까요 — 본문에서 단계별로 설명합니다.
- **`install`, `upgrade`, `rollback`은 어떤 흐름으로 이어질까요?**
  - `install`, `upgrade`, `rollback`은 어떤 흐름으로 이어질까요 — 본문에서 단계별로 설명합니다.
- **한눈에 보는 구조에서 가장 흔한 실수는 무엇일까요?**
  - Helm은 값을 받아 템플릿을 렌더링하고, 그 결과를 Kubernetes API에 적용합니다. 그래서 Helm을 이해할 때는 결과 YAML과 입력 값, 그리고 공통 템플릿의 책임을 분리해서 보는 편이 좋습니다.
- **도입 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 도입 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요 — 본문에서 단계별로 설명합니다.
- **단계별로 작은 차트 만들어 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 단계별로 작은 차트 만들어 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요 — 본문에서 단계별로 설명합니다.
