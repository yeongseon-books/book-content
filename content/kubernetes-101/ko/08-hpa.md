...
- 단계별로 CPU 기반 HPA 구성하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 트래픽이 바뀔 때마다 사람이 직접 파드 수를 조절하면 왜 느리고 비싸질, 둘째 HPA는 어떤 지표를 보고 스케일 아웃과 스케일 인을 결정할, 셋째 resource requests가 없으면 왜 제대로 동작하지 않을입니다. 한눈에 보는 구조에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **트래픽이 바뀔 때마다 사람이 직접 파드 수를 조절하면 왜 느리고 비싸질까요?**
  - 트래픽이 바뀔 때마다 사람이 직접 파드 수를 조절하면 왜 느리고 비싸질까요 — 본문에서 단계별로 설명합니다.
- **HPA는 어떤 지표를 보고 스케일 아웃과 스케일 인을 결정할까요?**
  - HPA는 어떤 지표를 보고 스케일 아웃과 스케일 인을 결정할까요 — 본문에서 단계별로 설명합니다.
- **resource requests가 없으면 왜 제대로 동작하지 않을까요?**
  - resource requests가 없으면 왜 제대로 동작하지 않을까요 — 본문에서 단계별로 설명합니다.
- **한눈에 보는 구조에서 가장 흔한 실수는 무엇일까요?**
  - HPA는 파드를 직접 만들고 지우기보다 Deployment의 원하는 개수를 조정합니다. 즉, 자동 스케일링의 판단 계층이라고 보는 편이 더 정확합니다. 뒤의 실제 파드 생성과 유지 작업은 여전히 Deployment가 맡습니다.
- **도입 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 도입 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요 — 본문에서 단계별로 설명합니다.
- **단계별로 CPU 기반 HPA 구성하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 단계별로 CPU 기반 HPA 구성하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요 — 본문에서 단계별로 설명합니다.
