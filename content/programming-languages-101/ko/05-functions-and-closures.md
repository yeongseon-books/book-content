...
- 클로저를 다섯 각도에서 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **일급 함수와 고차 함수는 무엇이 다를까요?**
  - 일급 함수는 함수를 변수에 담고, 인자로 넘기고, 반환값으로 돌려줄 수 있다는 성질이고, 고차 함수는 그 성질을 실제로 활용해 함수를 받거나 반환하는 함수입니다. `make_adder`와 `memo`는 함수를 반환하고 감싸는 예시라서, 일급 함수 위에 고차 함수가 서 있다는 점을 잘 보여 줍니다.
- **클로저는 정확히 무엇을 캡처할까요?**
  - 클로저는 값 복사본이 아니라 바깥 스코프의 바인딩과 그 환경을 붙잡습니다. `add10`과 `add20`이 서로 다른 `x`를 기억하는 예시, 그리고 `get`과 `inc`가 같은 `state` 딕셔너리를 함께 바라보는 예시가 이 점을 분명하게 보여 줍니다.
- **왜 반복문 안에서 만든 람다가 같은 값만 출력할까요?**
  - `for i in range(3): fns.append(lambda: i)`는 세 람다가 각각 값을 복사한 것이 아니라 같은 `i` 바인딩을 공유한 상태로 남기기 때문에, 호출 시점에는 모두 마지막 값 2를 읽습니다. `lambda i=i: i`처럼 기본 인자로 정의 시점 값을 고정하면 `[0, 1, 2]`로 바뀌는 이유도 바로 이 늦은 바인딩 차이입니다.
