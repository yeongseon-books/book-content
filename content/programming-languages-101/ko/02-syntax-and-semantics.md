...
- 작은 표현식을 직접 파싱해 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **구문과 의미의 경계는 정확히 어디일까요?**
  - `ast.parse("total = 3 +")`가 바로 `SyntaxError`를 내는 지점까지가 구문 단계이고, `divide(10, 0)`처럼 구문은 합법이지만 실행하다가 실패하는 지점부터가 의미 단계입니다. 같은 AST에 `evaluate`와 `evaluate_strange`처럼 서로 다른 평가기를 붙일 수 있다는 예시가 두 층위를 선명하게 갈라 줍니다.
- **토큰, 문법, AST는 어떤 순서로 이어질까요?**
  - `tokenize("3 + 4 * 2")`는 먼저 `('NUM', '3')`, `('OP', '+')` 같은 토큰 목록을 만들고, 이어서 문법 규칙 `expr -> term -> factor`를 따라 파서가 `('+', 3, ('*', 4, 2))`라는 트리를 만듭니다. 그다음에야 평가기가 이 AST를 읽어 실제 값 11을 계산하므로, 토큰화와 파싱은 구문 단계의 준비 작업입니다.
- **합법적인 코드인데도 의도와 다르게 동작하는 이유는 무엇일까요?**
  - 구문이 맞다는 사실은 문장 형태만 보장할 뿐, 그 코드가 우리가 기대한 의미로 해석된다는 보장은 하지 않습니다. `('+', 3, ('*', 4, 2))`에 보통 평가기를 붙이면 11이지만, `+`를 곱셈처럼 해석한 `evaluate_strange`를 붙이면 24가 되는 예시가 그 차이를 보여 줍니다.
