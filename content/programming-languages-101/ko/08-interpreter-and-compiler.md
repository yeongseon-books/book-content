...
- 파이썬 실행 내부를 직접 들여다보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **인터프리터와 컴파일러의 가장 짧은 차이는 무엇일까요?**
  - 이 글의 기준으로 컴파일러는 소스 코드를 실행 전에 다른 형태로 번역하고, 인터프리터는 그 결과나 중간 표현을 실행 단계에서 한 단계씩 처리합니다. Python이 `.py`를 바이트코드로 만든 뒤 VM이 실행하는 흐름은 두 전략이 한 시스템 안에 함께 들어갈 수 있음을 보여 줍니다.
- **Python은 실제로 어떤 실행 경로를 거칠까요?**
  - 본문은 `.py → tokenize → parse → AST → compile → .pyc bytecode → VM executes`라는 경로를 직접 제시했고, `dis.dis(add)`로 `LOAD_FAST`, `BINARY_OP`, `RETURN_VALUE` 같은 바이트코드도 확인했습니다. 또 `compile(PY_SRC, "<inline>", "exec")` 예시는 한 번 번역한 코드를 여러 번 재사용할 때 비용 구조가 어떻게 달라지는지 보여 줬습니다.
- **`.pyc` 파일은 정확히 무엇일까요?**
  - `.pyc`는 독립 실행 파일이 아니라 헤더와 직렬화된 코드 객체를 담은 캐시된 바이트코드 파일입니다. import 시점에는 이 결과를 재사용해 파싱과 컴파일 비용을 줄이지만, 실제 실행은 여전히 Python 가상 머신이 맡습니다.
