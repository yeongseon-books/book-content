...
- 객체 수명을 직접 따라가 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **스택과 힙은 어떻게 다를까요?**
  - 스택은 `work()` 같은 함수 호출과 함께 생겼다가 반환 시점에 프레임이 사라지는 영역이고, 힙은 `bytearray(1024)` 같은 객체가 놓여 참조가 남아 있는 동안 계속 살아 있는 영역입니다. 그래서 함수가 끝났다는 사실만으로는 힙 객체 수명이 자동 설명되지 않고, 누가 그 객체를 붙잡고 있는지까지 따로 봐야 합니다.
- **Python의 참조 카운팅은 언제 객체를 즉시 해제할까요?**
  - `t = Tag(); ref = t`처럼 참조가 늘어난 뒤 `del ref, t`로 모든 참조가 사라지면, CPython은 참조 수가 0이 되는 순간 `__del__`을 바로 실행할 수 있습니다. `sys.getrefcount(t)`가 함수 인자 때문에 실제보다 1 크게 보인다는 설명까지 함께 보면 언제 즉시 정리되는지 감이 잡힙니다.
- **순환 참조는 왜 가비지 컬렉터가 따로 필요할까요?**
  - `a.peer = b`와 `b.peer = a`처럼 서로만 가리키는 객체는 바깥 이름을 지워도 참조 수가 0이 되지 않아서, 참조 카운팅만으로는 회수되지 않습니다. 그래서 `gc.collect()` 같은 추적 기반 수집기가 필요하고, `cache[i] = b"x" * 1024` 예시는 GC가 있어도 남은 참조가 있으면 객체가 계속 살아 있음을 보여 줍니다.
