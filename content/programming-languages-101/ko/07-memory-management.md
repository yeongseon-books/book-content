...
**핵심 질문**: `del x`라고 적었는데, 그 객체는 정말 그 자리에서 사라질까요?

> 메모리 관리는 "이 객체는 언제 살아 있고, 언제 사라지는가"를 결정하는 규칙입니다. 언어마다 답이 다르지만 — 수동 해제, 참조 카운팅, 가비지 컬렉션 — 모두 같은 문제를 풉니다. 살아 있는 것만 남기고, 죽은 것은 회수한다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 스택과 힙의 차이를 한 줄로 답하는 법
- Python의 참조 카운팅이 일하는 방식
- 가비지 컬렉터(GC)가 순환 참조를 푸는 방식
- "메모리 누수"가 GC 언어에서도 일어나는 이유
- 수동 해제(C/C++), RAII, GC의 트레이드오프
...
- **Stack**: 함수 호출에 묶여 자동으로 생기고 사라지는 영역.
- **Heap**: 명시적으로 할당되고, 더 이상 안 쓸 때 회수되는 영역.
- **Reference counting**: 한 객체를 가리키는 참조 수를 세고, 0이면 즉시 회수.
- **Garbage collection**: 살아 있는(reachable) 객체를 추적해 그 외를 회수.
- **Cycle**: A→B→A처럼 서로를 가리키는 참조 모양. 단순 카운팅으로는 못 푼다.
...
- "이 객체의 소유자는 누구인가?"를 먼저 묻습니다.
- 캐시를 보면 곧장 상한과 만료 정책을 묻습니다.
- 누수가 의심되면 메트릭부터 봅니다 — 추측 디버깅을 안 합니다.
- GC가 있는 언어에서도 자원 누수는 존재한다는 사실을 기억합니다.
- `with`/`finally`로 자원의 수명을 코드 모양에 새겨 둡니다.
...
1. 위 실습 2단계의 순환을 `weakref`로 한쪽만 약한 참조로 바꾸고, `gc.collect()` 없이도 정리되는지 확인해 보세요.
2. 1만 개의 객체를 만들었다 지우면서 `tracemalloc`으로 사용량을 측정해 보세요. 어떤 패턴이 보이는지 한 문단으로 정리하세요.
3. 캐시 한 개를 골라 LRU(`functools.lru_cache`) 기반으로 바꾸고, 상한을 둔 전후의 동작 차이를 적어 보세요.
