...
- 타입을 단계적으로 붙여 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **타입은 정확히 어떤 역할을 할까요?**
  - `discount(price: int, rate: float) -> float` 같은 시그니처는 호출 계약을 코드에 적어 두고, `mypy`가 잘못된 호출을 실행 전에 막게 해 줍니다. `LineItem`과 `parse_line_item` 예시처럼 경계 안쪽에서는 더 구체적인 도메인 타입으로 좁혀 안전한 계산과 도구 지원을 동시에 얻는 역할도 합니다.
- **정적 타입과 동적 타입은 무엇을 언제 검사할까요?**
  - 정적 타입은 `discount("1000", 0.1)`처럼 잘못된 호출을 코드 실행 전에 검사기가 잡는 방식이고, 동적 타입은 실제 실행 경로가 그 지점에 도달했을 때 오류가 드러나는 방식입니다. 다만 외부 입력은 여전히 런타임 검증이 필요해서, 이 글은 `parse_line_item`으로 값을 확인한 뒤 `subtotal` 안에서는 구체 타입을 신뢰하는 흐름을 보여 줬습니다.
- **강한 타입과 약한 타입은 왜 다른 축일까요?**
  - 정적/동적은 검사가 언제 일어나는지에 대한 축이고, 강한/약한은 암묵적 형변환을 얼마나 허용하는지에 대한 축이라서 서로 다른 질문을 다룹니다. 그래서 정적 타입 언어도 약할 수 있고 동적 타입 언어도 강할 수 있으며, 이 글은 그 차이를 타입 추론·유니온·제네릭과 별개로 구분해 보도록 구성했습니다.
