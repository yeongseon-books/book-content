...
- 두 모델을 단계적으로 따라가기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **객체를 가장 간단히 정의하면 무엇일까요?**
  - 이 글에서 객체는 상태와 동작을 한 단위로 묶은 것으로 정의했습니다. `User`가 `name`, `age`, `greet()`를 함께 들고 있는 예시와, 클로저로 `name`과 `greet`를 묶은 `make_user` 예시가 객체의 핵심이 클래스 키워드보다 묶음 자체에 있음을 보여 줍니다.
- **클래스 기반 모델과 프로토타입 기반 모델은 메서드 탐색이 어떻게 다를까요?**
  - 클래스 기반 모델에서는 `B().hi()`가 `B.__mro__` 순서대로 상위 클래스를 올라가며 `A.hi`를 찾고, `super()`도 그 MRO의 다음 항목으로 이동합니다. 프로토타입 기반 모델에서는 `lookup(inst, "hi")`가 `__proto__` 체인을 따라 객체에서 객체로 위임하므로, 탐색 기준이 클래스가 아니라 개별 객체입니다.
- **Python에서 클래스 자체가 객체라는 말은 무슨 뜻일까요?**
  - `type(A)`가 `<class 'type'>`로 나오는 예시처럼, Python의 클래스는 정의 문법으로만 존재하는 선언이 아니라 실제 런타임 객체입니다. 그래서 `A.tag = "v1"`처럼 클래스에 속성을 붙이거나 메타프로그래밍으로 동작을 조정할 수 있습니다.
