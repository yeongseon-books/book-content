...
- 이 코드에서 먼저 볼 점의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **왜 우리는 기계어 대신 고급 언어를 사용할까요?**
  - 어셈블리 예시에서는 `mov`, `add`, 레지스터를 직접 다뤘지만, Python 예시에서는 `total = 3 + 4`처럼 이름과 연산만으로 같은 계산을 표현했습니다. 고급 언어는 이런 추상화 덕분에 기계 명령보다 문제 구조, 상태, 책임을 먼저 생각하게 해 주는 도구입니다.
- **프로그래밍 언어는 정확히 어떤 추상화를 제공할까요?**
  - 이 글에서 본 추상화는 변수 이름인 `total`, 루프와 조건문, `EvenDoubler.total()` 같은 객체, `reduce`와 `filter` 같은 함수 조합, SQL의 `SELECT SUM(n*2)` 같은 선언적 질의입니다. 언어는 이런 장치를 통해 세부 기계 동작을 감추고 사람이 다루기 좋은 단위로 문제를 다시 표현하게 합니다.
- **같은 문제를 명령형, 객체지향, 함수형, 선언형으로 풀면 무엇이 달라질까요?**
  - 명령형 예제는 `for`와 `total += n * 2`로 절차를 드러냈고, 객체지향 예제는 `EvenDoubler` 안에 데이터와 계산 책임을 묶었습니다. 함수형 예제는 `filter`와 `reduce`로 데이터 흐름을 앞세웠고, 선언형 예제는 SQL로 원하는 결과만 적고 실행 계획은 DBMS에 맡겼습니다.
