...
- 먼저 알아둘 용어에서 가장 흔한 실수는 무엇일까요?
- 먼저 보는 예시을 실무에 적용할 때 주의할 점은 무엇일까요?
- 세 언어를 다섯 축으로 비교하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **언어 설계를 볼 때 어떤 축으로 평가해야 할까요?**
  - 이 글은 좋은 언어 설계를 일관성, 단순성, 표현력, 안전성, 성능이라는 다섯 축으로 나눠 보자고 제안했습니다. Python·Go·Rust를 한 표에 놓고 메모리 관리, 실행 모델, 타입, 객체 모델, 함수 지원을 비교한 부분이 이 축들을 실제 평가 도구로 쓰는 방법입니다.
- **일관성, 단순성, 표현력, 안전성, 성능은 왜 서로 충돌할까요?**
  - Python의 리스트 컴프리헨션은 표현력을 높이는 대신 처음 읽는 사람에게는 규칙 부담을 늘릴 수 있고, Rust의 `Option<&i32>`는 안전성을 높이는 대신 반드시 처리해야 할 문법과 개념을 늘립니다. 본문이 반복해서 보여 준 핵심은 어떤 축을 올리면 다른 축의 비용이 함께 따라오므로, 좋은 설계는 균형보다 우선순위를 솔직하게 드러내는 일이라는 사실입니다.
- **Python, Go, Rust는 같은 문제에 왜 다른 답을 내릴까요?**
  - `total_len` 예시만 봐도 Python은 짧은 표현력, Go는 명시성과 운영 단순성, Rust는 타입과 소유권을 통한 안전성에 더 무게를 둡니다. 이어지는 비교 표에서도 Python은 gradual typing과 interpreter + bytecode, Go는 단순한 static typing과 AOT, Rust는 rich static typing과 ownership을 택했는데, 이것이 같은 문제에 대한 서로 다른 설계 답입니다.
