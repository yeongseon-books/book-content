...
- **일관성, 단순성, 표현력, 안전성, 성능은 왜 서로 충돌할까요?**
  - Python의 리스트 컴프리헨션은 표현력을 높이는 대신 처음 읽는 사람에게는 규칙 부담을 늘릴 수 있고, Rust의 `Option<&i32>`는 안전성을 높이는 대신 반드시 처리해야 할 문법과 개념을 늘립니다. 본문이 반복해서 보여 준 핵심은 어떤 축을 올리면 다른 축의 비용이 함께 따라오므로, 좋은 설계는 균형보다 우선순위를 솔직하게 드러내는 일이라는 점입니다.
- **Python, Go, Rust는 같은 문제에 왜 다른 답을 내릴까요?**
