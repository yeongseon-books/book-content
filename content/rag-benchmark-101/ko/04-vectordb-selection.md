...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/92"
...
- **VectorDB는 기능 목록이 아니라 어떤 운영 조건으로 비교해야 할까요?**
  - 실제 서비스에서는 인덱스 선택이 검색 품질만의 문제가 아닙니다. 설치 방식, 백업, 복구, 확장, 메타데이터 필터링, 업데이트 빈도까지 함께 봐야 합니다.
- **같은 임베딩과 corpus에서 VectorDB만 바꿔 보려면 무엇을 고정해야 할까요?**
  - 같은 임베딩과 corpus에서 VectorDB만 바꿔 보려면 무엇을 고정해야 할까요 — 본문에서 구체적으로 다룹니다.
- **정확도, latency, 필터링, 운영 복잡도가 충돌할 때 어떤 기준으로 선택해야 할까요?**
  - 실제 서비스에서는 인덱스 선택이 검색 품질만의 문제가 아닙니다. 설치 방식, 백업, 복구, 확장, 메타데이터 필터링, 업데이트 빈도까지 함께 봐야 합니다.
- **왜 이 주제가 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 벡터 검색 비용은 코퍼스가 커질수록 급격히 커집니다. 문서 수가 적을 때는 어떤 인덱스를 쓰든 큰 차이가 없어 보일 수 있습니다. 하지만 규모가 커지면 brute-force 방식의 flat 검색은 곧 병목이 됩니다.
- **기본 멘탈 모델을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - VectorDB 비교의 골격은 아래와 같습니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 여기서 recall은 1편에서 본 hit rate와 다릅니다. hit rate는 골드 정답 문서가 들어왔는지를 묻고, 여기서의 recall은 **ANN 결과가 exact 검색 결과를 얼마나 잘 복제했는지**를 봅니다.
