...
- **임베딩 모델을 리더보드 점수만 보고 고르면 왜 위험할까요?**
  - 임베딩 비교의 핵심 원칙은 **한 번에 한 변수만 바꾸기**입니다.
- **같은 corpus와 query에서 모델만 바꾸려면 어떤 조건을 고정해야 할까요?**
  - 같은 corpus와 query에서 모델만 바꾸려면 어떤 조건을 고정해야 할까요 — 본문에서 구체적으로 다룹니다.
- **품질이 좋아져도 latency나 비용이 커지면 어떻게 판단해야 할까요?**
  - 품질이 좋아져도 latency나 비용이 커지면 어떻게 판단해야 할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 주제가 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 임베딩 모델은 RAG 품질의 출발점입니다. 청크 전략, 검색기, LLM이 모두 같아도 임베딩 모델 하나만 바뀌면 검색 품질과 최종 답변 품질이 함께 흔들립니다.
- **기본 멘탈 모델을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 임베딩 비교의 핵심 원칙은 **한 번에 한 변수만 바꾸기**입니다.
- **핵심 개념의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 일반적으로 작은 모델은 차원이 작고 빠르며, 큰 모델은 차원이 크고 더 느립니다. 하지만 그 차이가 실제 질문 세트에서 얼마만큼의 품질 차이로 이어지는지는 직접 측정해 보기 전까지 알 수 없습니다.
