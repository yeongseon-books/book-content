...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/115"
...
좋은 에이전트의 마지막 조건은 똑똑함이 아니라 안전하게 배포하고 안전하게 되돌릴 수 있다는 사실입니다.
...
Production Harness의 핵심은 "무엇이 배포되었는지"를 한 파일로 설명할 수 있어야 한다는 사실입니다. 코드 SHA만으로는 부족합니다. 프롬프트, 도구 레지스트리, 정책, eval 세트를 함께 묶어야 rollback이 정확해집니다.
...
여기서 중요한 점은 테스트 종류가 서로 대체되지 않는다는 사실입니다. eval이 좋아도 tracing contract가 깨져 있으면 운영 중 사고를 재현할 수 없습니다.
