...
에이전트에게 모호한 지시를 던지면 가끔은 그럴듯한 결과가 나옵니다. 문제는 그 결과가 재현되지 않는다는 점입니다. 어느 날은 괜찮고 어느 날은 틀리는 시스템은 자동화가 아니라 추측입니다.
...
이 글은 Harness Engineering 101 시리즈의 2번째 글입니다.
이 글의 핵심은 단순합니다. 에이전트에게 Goal을 주지 말고, 실행 가능한 TaskSpec을 주어야 합니다.
## 이 글에서 다룰 문제
- 비즈니스 Goal과 에이전트 Task를 왜 분리해서 다뤄야 할까요?
- 실행 가능한 Task는 어떤 필드를 반드시 가져야 할까요?
- 태스크 크기가 너무 크거나 모호할 때 어떤 기준으로 쪼개야 할까요?
- 자연어 완료 조건을 어떻게 자동 검증 가능한 기준으로 바꿀 수 있을까요?
- TaskSpec을 프롬프트, 검증기, eval 데이터셋의 단일 기준으로 쓰면 무엇이 좋아질까요?
## 왜 이 글이 중요한가
...
## Task Harness를 이해하는 가장 좋은 방법: Goal을 실행 단위로 번역하는 계층으로 보는 것입니다
