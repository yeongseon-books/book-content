...
Agent에게 어떤 정보를 주고 어떤 정보를 숨겨야 효율과 안전을 동시에 잡을까요?

이 글은 그 질문에 답하기 위해 Context Harness의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **관련 정보만 주는 게 1순위** — noise가 모델을 혼란시킵니다.
- **최소 권한 원칙을 컨텍스트에** — 필요 없는 시크릿·데이터를 숨깁니다.
- **동적 컨텍스트 구성이 표준** — 전체를 매번 주는 건 비용·성능 모두 깨집니다.
- **출처를 함께 전달** — 근거 없는 답을 모델이 만들지 않게 합니다.
- **컨텍스트 길이를 의식** — 과도한 길이가 핵심 정보를 묻습니다.
