...
이 글은 Harness Engineering 101 시리즈의 3번째 글입니다.
결국 컨텍스트 품질은 길이보다 밀도로 결정됩니다.
## 이 글에서 다룰 문제
- 컨텍스트 윈도우가 커져도 왜 항상 부족하게 느껴질까요?
- 시스템 프롬프트, 태스크 명세, 대화 이력, retrieved context, 도구 스키마는 어떤 비율로 다뤄야 할까요?
- 긴 대화 이력은 sliding window, summarization, selective recall 중 무엇으로 관리해야 할까요?
- RAG는 retrieval 이후 어떤 단계가 더 필요할까요?
- 어떤 정보는 왜 의도적으로 컨텍스트에서 빼야 할까요?
## 왜 이 글이 중요한가
...
## Context Harness를 이해하는 가장 좋은 방법: 무한한 메모리가 아니라 제한된 예산을 배분하는 작업으로 보는 것입니다
...
좋은 Context Harness는 정보량이 아니라 정보 밀도를 관리합니다. 모델이 봐야 할 내용을 많이 주는 것이 아니라, 판단에 필요한 내용을 가장 압축된 형태로 주는 설계입니다.
