...
실제 문서 수집은 PDF 하나로 끝나지 않습니다. 운영 메모는 TXT로 오고, 팀 문서는 Markdown으로 남고, 외부 보고서는 PDF로 들어오므로 포맷 차이를 흡수하는 설계가 먼저 필요합니다.

이 글은 문서 수집과 인덱싱 101 시리즈의 다섯 번째 글입니다. 여기서는 여러 포맷을 각자 읽되, 최종 출력은 하나의 공통 `Document` 계약으로 맞추는 방법을 설명합니다.

> 다중 포맷 파이프라인의 핵심은 서로 다른 입력을 하나의 공통 `Document` 계약으로 밀어 넣는 데 있습니다.

## 이 글에서 다룰 문제

- PDF, TXT, MD를 하나의 파이프라인으로 어떻게 묶을 수 있을까요?
- 포맷마다 로더가 달라도 공통 `Document` 형태가 왜 중요할까요?
- 포맷 분기와 메타데이터 정규화는 어느 단계에서 처리해야 할까요?
