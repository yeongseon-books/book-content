...
이 글은 문서 수집과 인덱싱 101 시리즈의 첫 번째 글입니다.
...
생성과 추출을 한 스크립트에 묶어 두면 예제를 다시 돌리기 쉽고, 출력 검증도 간단해집니다. 아래 예제는 샘플 PDF를 직접 생성한 뒤 페이지별 텍스트와 메타데이터를 추출하는 최소 파이프라인입니다.
...
| 실수 | 왜 생기는가 | 올바른 접근 |
| --- | --- | --- |
| 모든 PDF를 OCR로 처리 | "안전하다"는 인식에서 비롯됨 | 텍스트 레이어 존재 여부를 먼저 확인하고 필요할 때만 OCR 적용 |
| 문자 수만으로 품질 판단 | 추출이 됐으면 좋다고 가정 | 공백 비율, 제어 문자 비율, 읽기 순서까지 함께 점검 |
| 페이지 번호를 메타데이터에서 빠뜨림 | 본문만 넘기면 된다는 생각 | `page`, `source`, `extraction_path`를 파싱 경계에서 고정 |
| 표와 본문을 같은 방식으로 청킹 | 구조 차이를 인식하지 못함 | `content_kind=table`과 `content_kind=text`를 분리해 저장 |
| 라이브러리를 하나만 써서 결과 검증 안 함 | 설정이 간단해서 그냥 씀 | 대표 샘플로 pypdf와 pdfplumber 결과를 비교해 기준선 확보 |
...
파싱 단계에서 페이지를 문자열로만 넘기면 다음 단계에서 페이지 경계와 출처를 잃기 쉽습니다. 파싱 결과를 바로 청킹 입력 계약으로 직렬화해 두는 편이 좋습니다.
...
표는 텍스트 추출에서 가장 자주 깨지는 구조입니다. 셀 순서가 바뀌거나 열 머리글이 누락되면, 이후 검색에서 값은 나오지만 의미가 어긋나는 답변이 생깁니다. 표를 찾았을 때는 원문 문단과 분리해 별도 레코드로 저장하고, 셀을 명시적으로 직렬화하는 편이 좋습니다.
...
실무 PDF는 텍스트만 뽑는 경로 하나로 끝나지 않는 경우가 많습니다. 같은 파일이라도 라이브러리별로 줄바꿈, 표 셀 경계, 공백 처리 결과가 다르게 나올 수 있기 때문입니다. 초기에 한 가지 라이브러리만 고정하기보다, 대표 샘플 몇 개를 두고 결과를 비교해 기준선을 정하는 편이 안전합니다.
...
위 코드는 정답을 고르는 코드가 아니라, 차이를 기록하는 코드입니다. 페이지별 문자 수 차이가 크게 나는 파일만 우선 수동 검토 대상으로 올리면, 전체 문서를 눈으로 읽지 않고도 위험 샘플을 빠르게 골라낼 수 있습니다.
...
PDF 파싱의 첫 목표는 시각 문서를 검증 가능한 문자열 목록으로 바꾸는 일입니다. 이 글에서는 파싱 흐름부터 품질 게이트, OCR fallback 라우팅, 운영 점검 쿼리까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
...
- **PDF 파일은 왜 단순한 텍스트 파일처럼 읽을 수 없을까요?**
  - PDF는 인쇄를 위한 시각 레이아웃 형식이라 텍스트가 스트림으로 묶여 있고 읽기 순서가 명시되지 않습니다. 텍스트 레이어가 없는 스캔본은 픽셀만 담고 있어 OCR 없이는 추출이 불가능합니다.
- **페이지 구조와 메타데이터를 추출 단계에서 보존하지 않으면 RAG에서 무엇이 막힐까요?**
  - `page`, `source`, `extraction_path`가 없으면 검색 결과에 출처를 표시하기 어렵고, 품질 저하 원인을 페이지 단위로 좁힐 수 없습니다. 표와 본문이 섞이면 청킹 경계도 잘못 잡힙니다.
- **파싱 결과가 좋아 보일 때도 어떤 검증을 먼저 해야 할까요?**
  - 문자 수 외에 공백 비율, 제어 문자 비율, 두 파서 간 문자 수 차이를 함께 확인해야 합니다. 숫자가 크더라도 읽기 순서가 깨지거나 반복 머리글이 본문에 섞일 수 있습니다.
