...
이 글은 문서 수집과 인덱싱 101 시리즈의 3번째 글입니다.
...
- 핵심은 같은 질의라도 필터가 바뀌면 결과 집합이 달라진다는 사실입니다.

## 자주 하는 실수

| 실수 | 왜 생기는가 | 올바른 접근 |
| --- | --- | --- |
| 메타데이터 없이 벡터만 저장 | 임베딩 품질에만 집중 | 수집 단계에서 `source`, `category`, `quarter`를 함께 저장 |
| 필드 값 정규화 생략 | `Q4`와 `2024Q4`가 같다고 가정 | 인덱싱 전에 정규화 함수를 강제해 값 형식을 통일 |
| 필터 가능 필드와 표시 필드 혼동 | 모든 필드를 필터에 쓰려는 경향 | 필터 대상 필드와 표시 전용 필드를 설계 시 명시적으로 분리 |
| 테넌트 필터 누락 | 개발 환경에서는 필요 없다는 생각 | 백엔드에서 최소 보안 조건을 무조건 강제 |
| 스키마 변경 후 기존 인덱스 방치 | 변경이 작다고 생각 | `schema_version`을 메타데이터에 저장하고 마이그레이션 경로 분리 |
...
필드가 느슨하면 검색 기능은 돌아가도 운영 중에 결과 해석이 어려워집니다. 특히 `source`나 `version`이 빠진 문서는 나중에 재색인 대상 판별과 감사 로그 연결이 힘들어집니다. 수집 단계에서 최소 계약을 코드로 강제하는 편이 안전합니다.
...
### 인덱싱 시점에 메타데이터 정규화 강제하기

필터가 많은 시스템에서는 인덱싱 단계에서 메타데이터 정규화를 함께 처리해야 합니다. 같은 의미를 가진 값이 `Q4`, `2024-Q4`, `2024Q4`로 섞여 있으면 필터 정확도가 떨어지고 캐시 적중률도 나빠집니다.
...
문서 저장소를 분리하지 않더라도, 최소한 어떤 필드를 필터 대상으로 삼을지 사전에 정해 두어야 합니다.
...
스키마 정의, 값 정규화, 복합 필터 조합, 인덱싱 검증이 한 세트로 맞물려야 실제 서비스에서 예측 가능한 검색 동작을 얻을 수 있습니다. 다음 글에서는 이런 메타데이터 계약을 유지한 채, 변경된 문서만 다시 처리하는 증분 인덱싱 흐름으로 넘어가겠습니다.

## 처음 질문으로 돌아가기

- **메타데이터 스키마는 왜 임베딩 후가 아니라 수집 단계에서 먼저 설계해야 할까요?**
  - 임베딩 후에는 벡터 DB에 이미 저장된 문서를 다시 인덱싱해야 하므로 비용이 큽니다. 수집 단계에서 어떤 필드로 필터링할지 미리 정해야 인덱스 구조와 쿼리 계약이 함께 설계될 수 있습니다.
- **필터는 벡터 유사도 검색 전에 후보군을 어떻게 바꿀까요?**
  - 필터는 유사도 점수 계산 전에 `category`, `quarter` 같은 조건으로 후보 집합을 먼저 좁힙니다. 같은 질의라도 필터 조건이 다르면 완전히 다른 문서 집합이 유사도 랭킹에 올라옵니다.
- **필수 메타데이터가 빠지면 검색과 출처 표시에 어떤 문제가 생길까요?**
  - `source`가 없으면 검색 결과에 출처를 표시할 수 없고, `tenant`가 없으면 다른 조직 문서가 후보에 섞입니다. `version`이 없으면 재색인 대상을 식별하기 어렵고 롤백이 불가능해집니다.
