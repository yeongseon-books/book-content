...
예를 들어 회사 사무실에 스위치와 PC 10대가 연결되어 있으면 그것은 네트워크입니다. 이 네트워크가 ISP를 통해 다른 네트워크와 연결되고, BGP로 경로를 교환하면 인터넷의 일부가 됩니다.
...
핵심은 인터넷이 "하나의 시스템"이 아니라 "약속(IP)으로 이어진 네트워크의 연합"이라는 점입니다. RFC 1122에서 정의하는 인터넷 호스트 요구사항도 이 전제에서 출발합니다.
