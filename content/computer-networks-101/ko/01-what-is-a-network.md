...
**핵심 질문**: "네트워크"라는 단어를 들었을 때 머릿속에 무엇이 그려져야, 다음에 나오는 IP·TCP·DNS·HTTP가 모두 같은 그림 위에 놓일까요?

> 네트워크는 케이블이 아니라 약속의 묶음입니다. 두 컴퓨터가 데이터를 주고받기 위해 누가 먼저 말하고, 어떤 단위로 자르고, 어떻게 다시 합칠지를 약속한 것이 프로토콜이고, 그 약속들이 층층이 쌓인 것이 계층 모델입니다. 이 시리즈를 따라가기 전에 이 그림을 머릿속에 한 번 그려 둡니다.
...
| 프로토콜 | 두 노드가 통신하기 위한 약속(형식 + 절차) |
| 계층 모델 | 통신 책임을 층으로 나누어 단순화한 구조 |
| 호스트 | 통신의 양쪽 끝(컴퓨터, 서버, 휴대폰 등) |
| 라우터 | 패킷을 다음 네트워크로 넘기는 장비 |
...
시니어 엔지니어는 네트워크 사고를 들으면 가장 먼저 "어느 층의 문제인가?"를 묻습니다. 서비스 5xx인지, TCP 재전송 폭증인지, DNS resolve 실패인지, 라우팅 문제인지 — 층을 분리하지 못하면 토론은 영원히 헛돕니다. 계층 모델은 책에 박힌 다이어그램이 아니라, 사고 진단의 1단계 분류표입니다.

또한 시니어는 모든 추상의 비용을 의식합니다. HTTP 요청 한 번에는 DNS, TCP handshake, TLS handshake, HTTP 헤더 전송, 라우터 N홉의 비용이 모두 들어 있습니다. 평소에는 보이지 않지만 사고가 나면 정확히 이 단위로 비용이 청구됩니다.
...
1. 자주 가는 사이트에 traceroute를 돌려 보고 라우터 수와 평균 RTT를 정리하세요. 한 문단으로 결과를 설명합니다.

2. 위 5단계의 client/server를 실행하면서 tcpdump로 캡처하고, SYN/SYN-ACK/ACK 핸드셰이크를 직접 확인합니다.

3. "방금 실패한 HTTP 요청"이 있다고 가정하고, 어느 층부터 어떤 도구로 진단할지 절차를 5단계로 적어 보세요.
