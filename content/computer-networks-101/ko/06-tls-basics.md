...
- **TLS가 보장하는 세 가지는 무엇일까요?**
  - TLS가 보장하는 세 가지는 무엇일까요 — 본문에서 구체적으로 다룹니다.
- **핸드셰이크는 어떤 순서로 진행될까요?**
  - 핸드셰이크는 어떤 순서로 진행될까요 — 본문에서 구체적으로 다룹니다.
- **인증서, CA, 체인, trust store는 어떤 관계일까요?**
  - 인증서, CA, 체인, trust store는 어떤 관계일까요 — 본문에서 구체적으로 다룹니다.
- **핵심 그림에서 가장 흔한 실수는 무엇일까요?**
  - 비대칭 합의로 대칭 세션 키를 만들고, 그 뒤부터는 그 키로 빠르게 데이터를 암호화합니다.
- **적용 전후 비교을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 인증서를 부정하게 발급한 것을 발견하는 방법이 Certificate Transparency입니다. 모든공인 CA는 발급한 인증서를 공개 CT 로그에 등록해야 합니다.
...
- **Computer Networks 101 (6/10): TLS 기초 (현재 글)**
