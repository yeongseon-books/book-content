...
- **글로벌 CSS, CSS Modules, CSS-in-JS, Tailwind는 어떤 차이를 가질까요?**
  - 이 글은 전역 규칙을 넓게 쓰는 글로벌 CSS, 이름 충돌을 줄이는 CSS Modules, 컴포넌트 안에서 스타일을 정의하는 CSS-in-JS, 유틸리티 클래스를 조합하는 Tailwind를 서로 다른 운영 방식으로 정리했습니다. 어떤 도구를 쓰든 핵심은 스타일 표현이 팀 전체에서 일관되게 읽히고, 컴포넌트와 토큰 체계 안으로 수렴해야 한다는 점입니다.
- **디자인 토큰은 왜 프로젝트가 커질수록 더 중요해질까요?**
