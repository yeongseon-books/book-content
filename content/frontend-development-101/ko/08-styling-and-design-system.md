...
- 페이지별 임의 스타일에서 토큰 기반 시스템으로을 실무에 적용할 때 주의할 점은 무엇일까요?
...
- **글로벌 CSS, CSS Modules, CSS-in-JS, Tailwind는 어떤 차이를 가질까요?**
  - 이 글은 전역 규칙을 넓게 쓰는 글로벌 CSS, 이름 충돌을 줄이는 CSS Modules, 컴포넌트 안에서 스타일을 정의하는 CSS-in-JS, 유틸리티 클래스를 조합하는 Tailwind를 서로 다른 운영 방식으로 정리했습니다. 어떤 도구를 쓰든 스타일 표현은 팀 전체에서 일관되게 읽혀야 하고, 컴포넌트와 토큰 체계 안으로 수렴해야 합니다.
- **디자인 토큰은 왜 프로젝트가 커질수록 더 중요해질까요?**
  - `--color-primary`나 Tailwind `colors.primary`처럼 이름 붙은 토큰이 있으면 버튼 색, 표면색, 간격 기준이 한곳에서 관리됩니다. 본문이 page A와 page B의 미세하게 다른 파란색을 문제로 든 이유도, 토큰이 없으면 작은 시각 차이가 누적되어 다크 모드와 전역 수정 비용이 급격히 커지기 때문입니다.
- **컴포넌트 라이브러리는 어떤 구조로 운영되는 편이 좋을까요?**
  - `Button` 같은 공용 컴포넌트가 토큰을 소비하며 스타일의 진실의 출처 역할을 해야 페이지마다 개별 구현이 늘어나지 않습니다. 여기에 Storybook 문서화와 lint 규칙을 더하면 variant가 왜 필요한지, 하드코딩 색이 남았는지, 다크 모드가 맞는지까지 체계적으로 운영할 수 있습니다.
