...
- 문서형 웹에서 앱형 프론트엔드로 이동한 이유을 실무에 적용할 때 주의할 점은 무엇일까요?
...
프론트엔드는 예전의 정적인 문서 페이지를 넘어, 브라우저 안에서 상태와 화면을 계속 동기화하는 실행 환경으로 확장됐습니다. 아래 비교를 보면 왜 라우팅, 상태, 번들링이 모두 같은 프론트엔드 이야기로 묶이는지 더 선명해집니다.
...
예전 웹과 현대 프론트엔드의 가장 큰 차이는 화면을 바꾸는 책임이 서버에서 브라우저 런타임 쪽으로 크게 이동했다는 점입니다. 이 관점을 잡아 두면 뒤의 컴포넌트, 라우팅, API 호출 글들이 같은 계층 이야기로 이어집니다.
...
- **프론트엔드와 백엔드의 경계는 정확히 어디에서 나뉠까요?**
  - 이 글에서 프론트엔드는 브라우저 안에서 HTML, CSS, JavaScript가 실행되며 사용자가 직접 만나는 계층입니다. 백엔드는 그 뒤에서 데이터를 제공하더라도, 버튼 클릭 후 `textContent`가 바뀌는 경험과 DevTools에서 보이는 정적 파일 응답은 프론트엔드 책임에 속합니다.
- **브라우저는 HTML, CSS, JavaScript를 어떤 순서와 역할로 조합할까요?**
  - 예제의 `index.html`이 구조를 만들고, `style.css`가 그 구조의 모양을 정리하며, `app.js`가 클릭 이벤트에 반응해 제목을 `Hello, frontend!`로 바꿉니다. 브라우저는 이 세 층을 결합해 DOM을 만들고 화면을 렌더링한 뒤, 사용자 입력이 들어오면 JavaScript로 동작을 다시 붙입니다.
- **DOM, 렌더링, SPA 같은 단어는 왜 프론트엔드 입문에서 계속 등장할까요?**
  - DOM은 브라우저가 HTML을 읽어 만든 트리이고, 렌더링은 그 구조와 CSS를 실제 픽셀로 그리는 과정이며, SPA는 그 위에서 전체 새로고침 없이 화면 조각만 바꾸는 방식입니다. 뒤 글들에서 라우팅, 상태, 번들링을 이해하려면 이 세 단어가 같은 브라우저 실행 흐름의 서로 다른 층이라는 감각이 먼저 필요합니다.
