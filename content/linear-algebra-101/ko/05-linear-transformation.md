...
- 대표 선형변환에서 가장 흔한 실수는 무엇일까요?
- 읽기 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다섯 단계로 변환 읽기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
선형변환의 핵심은 입력 벡터 하나가 아니라 공간 전체가 함께 바뀐다는 사실입니다. 격자는 평행성과 비율을 유지한 채 회전하거나 늘어나거나 기울어집니다.
...
- **행렬을 곱한다는 말은 공간에 어떤 변화를 주는 걸까요?**
  - 행렬을 벡터에 곱한다는 것은 좌표 몇 개를 계산하는 일이 아니라 공간의 점과 격자를 함께 바꾸는 일입니다. 예를 들어 회전 행렬 `R`을 `[1, 0]`에 곱하면 결과가 대략 `[0.707, 0.707]`가 되어, 길이는 유지한 채 방향만 바뀝니다.
- **회전, 확대, 반사, 전단은 행렬 모양으로 어떻게 드러날까요?**
  - 회전은 `[[cosθ, -sinθ], [sinθ, cosθ]]`, 확대는 `np.diag([2.0, 0.5])`, 반사는 `[[1,0],[0,-1]]`, 전단은 `[[1,1],[0,1]]`처럼 행렬 모양 자체에 효과가 드러납니다. 대각 성분은 축별 스케일을, 부호 반전은 반사를, 비대각 성분은 기울임을 읽게 해 줍니다.
- **변환의 합성은 왜 행렬 곱으로 표현될까요?**
  - 본문에서 `M = R @ S`로 먼저 스케일을 적용하고 그다음 회전한 결과를 하나의 행렬로 묶었습니다. 이렇게 합성을 행렬 곱으로 쓰면 여러 단계 변환도 한 번의 `M @ v`로 표현할 수 있고, 순서가 바뀌면 결과도 바뀐다는 점까지 그대로 보존됩니다.
