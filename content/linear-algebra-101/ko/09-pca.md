...
- 중심화는 PCA에서 빠지면 안 되는 전처리입니다.
- SVD는 수치적으로 비교적 안정적인 방법입니다.
- 분산 설명률을 기준으로 `k`를 고르는 편이 좋습니다.

## 자주 하는 실수 5가지

1. **중심화나 스케일링을 빼먹는 실수**
2. **스케일이 큰 피처가 주성분을 지배한다는 점을 놓치는 실수**
3. **주성분 부호가 임의적이라는 사실을 잊는 실수**
4. **비선형 관계가 강한 데이터에 PCA를 그대로 적용하는 실수**
5. **근거 없이 `k`를 정하는 실수**
