...
- 다섯 단계로 고유분해 읽기을 실무에 적용할 때 주의할 점은 무엇일까요?
...
## 실전 연결: 고유값 계산 절차를 단계적으로 검증하기

고유값 문제를 실무에 연결할 때는 계산 결과 자체보다 검증 루틴이 더 중요합니다. 특히 공분산 행렬 분석에서는 대칭 구조를 활용해 `eigh`를 사용하고, 고유벡터 직교성과 재구성 가능성을 함께 확인하는 습관이 필요합니다.
...
print('eigenvalues:', vals)
print('orthogonal:', np.allclose(vecs.T @ vecs, np.eye(vecs.shape[1])))
print('reconstruct:', np.allclose(C, vecs @ np.diag(vals) @ vecs.T))
```

절차를 요약하면 중심화, 공분산 계산, 고유값 정렬, 직교성 검증, 재구성 확인 순서입니다. 이 다섯 단계를 지키면 계산 과정이 투명해지고, 주성분분석으로 이어질 때도 해석이 일관됩니다.

또한 큰 고유값에 대응하는 고유벡터가 데이터 분산의 지배 축이라는 점을 기억해야 합니다. 이 해석은 단순 수학 문제를 넘어, 어떤 방향의 변동이 모델 성능을 좌우하는지 판단하는 실무 근거가 됩니다.
...
- **행렬을 반복해서 적용할 때 왜 어떤 방향은 유지될까요?**
  - 어떤 방향은 행렬이 그 벡터를 다른 방향으로 꺾지 않고 크기만 바꾸기 때문에 반복 적용해도 축이 유지됩니다. 본문에서 거듭제곱 반복으로 `M @ v`를 계속 계산했을 때 지배적인 방향이 남는 모습이 바로 그 현상입니다.
- **고유벡터와 고유값은 각각 무엇을 뜻할까요?**
  - 고유벡터는 `A v = λ v`를 만족하는 특별한 방향이고, 고유값 `λ`는 그 방향이 얼마나 늘어나거나 줄어드는지를 나타냅니다. `np.linalg.eig(A)`로 구한 뒤 `np.allclose(A @ v, lambda * v)`를 확인한 절차가 정의 자체를 코드로 검증한 예입니다.
- **대칭행렬에서 결과가 특히 깔끔해지는 이유는 무엇일까요?**
  - 대칭행렬 `S = [[2,1],[1,2]]`에서는 `np.linalg.eigh(S)`를 썼을 때 고유벡터들이 서로 직교하고 `svc.T @ svc ≈ I`가 성립했습니다. 그래서 축이 서로 깔끔하게 분리되고, PCA처럼 직교한 주축을 써야 하는 문제에서 특히 다루기 쉬워집니다.
