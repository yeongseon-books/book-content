...
- 읽기 전과 후에서 가장 흔한 실수는 무엇일까요?
- 다섯 단계로 분해 읽기을 실무에 적용할 때 주의할 점은 무엇일까요?
- 작은 수치 예시로 다시 보기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
## 실전 연결: 특이값분해로 저랭크 근사 만들기

특이값분해는 이론 설명에서 끝나지 않고, 실제 압축과 잡음 제거에서 바로 사용됩니다. 핵심은 큰 특이값 몇 개만 남겨 저랭크 근사 행렬을 만드는 것입니다.
...
k = 2
A2 = U[:, :k] @ np.diag(S[:k]) @ Vt[:k, :]
rel_err = np.linalg.norm(A - A2) / np.linalg.norm(A)
print('top-2 singular values:', S[:k])
print('relative error:', rel_err)
```

추천 시스템 맥락에서는 `U[:, :k]`와 `Vt[:k, :]`를 사용자 잠재벡터와 아이템 잠재벡터로 해석할 수 있습니다. 이미지 압축에서는 `k`를 줄일수록 저장량이 줄지만 재구성 오차는 커집니다. 즉 `k` 선택은 품질과 비용의 균형 문제입니다.

추가로, 특이값 스펙트럼을 시각화해 급격히 꺾이는 지점을 찾으면 합리적인 `k` 후보를 빠르게 정할 수 있습니다. 이 절차는 주성분분석의 분산 설명률 판단과도 직접 연결됩니다.
...
- **왜 역행렬보다 분해를 먼저 떠올려야 할까요?**
  - 역행렬 하나를 직접 구하는 것보다 문제에 맞는 조각으로 나눠 푸는 편이 더 안정적이고 해석도 쉽기 때문입니다. 본문에서도 `np.linalg.inv`를 기본 해법으로 두지 않고 `LU`, `QR`, `SVD`를 통해 방정식 풀이와 근사를 설명했습니다.
- **LU, QR, 고유분해, SVD는 각각 어디에 잘 맞을까요?**
  - `lu(A)`는 `Ax=b` 같은 연립방정식, `np.linalg.qr(A)`는 최소제곱, `np.linalg.eig(A)`는 변환 축 해석, `np.linalg.svd(A)`는 직사각형 행렬과 저랭크 근사에 자연스럽습니다. 같은 행렬이라도 질문이 다르면 먼저 떠올려야 할 분해가 달라진다는 점이 핵심이었습니다.
- **모든 분해가 모든 행렬에 적용되는 것은 아닐까요?**
  - 아닙니다. 본문에서 강조했듯이 SVD는 거의 모든 행렬에 적용되지만, LU나 고유분해는 행렬 성질에 따라 그대로 쓸 수 없는 경우가 있습니다. 그래서 `np.allclose(A_reconstructed, A)` 같은 재구성 검증과 함께, 대칭인지 직사각형인지부터 먼저 읽는 습관이 중요합니다.
