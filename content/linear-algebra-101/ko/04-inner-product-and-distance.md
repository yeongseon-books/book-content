...
- 거리 측도에서 가장 흔한 실수는 무엇일까요?
- 읽기 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?
- 다섯 단계로 비교 기준 익히기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- **내적은 왜 숫자 하나로 나올까요?**
  - 내적은 `v @ w`처럼 좌표별 곱을 모두 더해 하나의 스칼라로 압축하기 때문입니다. 예제의 `v = [1,2,3]`, `w = [4,5,6]`에서는 결과가 `32.0`으로 나왔고, 이 한 숫자가 두 벡터의 정렬 정도를 요약했습니다.
- **코사인 유사도는 내적과 어떻게 연결될까요?**
  - 코사인 유사도는 `(v @ w) / (np.linalg.norm(v) * np.linalg.norm(w))`처럼 내적을 두 벡터 길이로 나눈 값이라, 같은 내적이라도 크기 영향은 제거하고 방향만 남깁니다. 그래서 본문 예시에서 약 `0.975`가 나온 것은 두 벡터의 방향이 꽤 비슷하다는 뜻입니다.
- **유클리드 거리와 맨해튼 거리는 무엇이 다를까요?**
  - 유클리드 거리는 `np.linalg.norm(v - w)`로 차이 벡터의 직선 길이를 재고, 맨해튼 거리는 `np.sum(np.abs(v - w))`로 좌표별 이동량을 더합니다. 같은 `v, w`에서도 하나는 약 `5.196`, 다른 하나는 `9.0`이었으니, 두 값은 서로 다른 “가까움”의 정의를 보여 준 셈입니다.
