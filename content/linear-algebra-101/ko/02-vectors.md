...
- **노름**: 벡터의 길이입니다. 보통 먼저 유클리드 노름(L2)을 배웁니다.
- **단위벡터**: 노름이 1인 벡터입니다.
...
| 노름 | 벡터 길이 | `np.linalg.norm(v)` |
| 정규화 | 방향 유지, 길이를 1로 | `v / np.linalg.norm(v)` |
...
# L1 노름도 확인
norm_v_l1 = np.abs(v).sum()
print("||v||_1:", norm_v_l1)  # 7.0
```
...
print('raw dot:', x @ y)            # 10.0 (크기 차이 반영)
print('normalized dot:', x_n @ y_n) # 1.0 (방향만 남음)
```
...
실무에서는 벡터 하나가 아니라 여러 벡터를 한 번에 비교하는 경우가 많습니다. 행렬 연산으로 배치 유사도를 계산하는 패턴이 핵심입니다.
...
print('diagonal (self-similarity):', np.diag(sim))  # 모두 1에 가까움
```

대각 원소는 항상 1에 가깝고, 비대각 원소가 클수록 의미가 가까운 벡터라고 읽을 수 있습니다. 같은 원리로 질의 벡터와 문서 벡터의 내적을 계산하면 벡터 검색의 기본 랭킹이 됩니다.
...
# 차원별 평균 거리 변화 확인 (차원의 저주)
for d in [2, 10, 50, 100]:
    samples = rng.normal(size=(100, d))
    dists = [np.linalg.norm(samples[0] - samples[i]) for i in range(1, 20)]
    print(f'{d:3d}차원: 평균 거리 = {np.mean(dists):.2f}, std = {np.std(dists):.2f}')
```

차원이 올라갈수록 거리의 평균과 표준편차 비율이 비슷해집니다. 이 현상을 '차원의 저주(curse of dimensionality)'라고 하며, 고차원 데이터를 다룰 때 반드시 고려해야 합니다.
...
| 실수 | 증상 | 올바른 접근 |
| --- | --- | --- |
| 영벡터를 정규화 | `0/0` 오류 또는 `nan` 발생 | 정규화 전 `np.linalg.norm(v) > 0` 확인 |
| 행벡터와 열벡터를 같은 것으로 취급 | 형상 불일치로 오류 발생 | `v.shape`를 `(n,)`, `(n,1)`, `(1,n)` 구분 |
| 브로드캐스팅에 기대어 형상 무시 | 의도치 않은 결과 | 모든 연산 전 `shape` 출력 확인 |
| 내적과 원소별 곱 혼동 | `np.dot`과 `*` 결과 혼동 | `@`는 행렬/벡터 곱, `*`는 원소별 곱 명확히 구분 |
| 크기 비교와 방향 비교를 구분 안 함 | 잘못된 유사도 선택 | 문제 정의부터 "방향인가 크기인가" 결정 |
...
- **벡터는 단순한 리스트와 무엇이 다를까요?**
  - 벡터는 단순 배열이 아니라 덧셈과 스칼라곱이 정의된 기하학적 대상입니다. 이로 인해 벡터 공간의 구조가 생기고, 같은 언어로 여러 차원의 문제를 표현할 수 있습니다. 파이썬 리스트는 이 수학적 구조를 갖지 않습니다.

- **벡터 덧셈과 스칼라곱은 기하학적으로 무엇을 뜻할까요?**
  - 덧셈은 화살표 두 개를 이어 붙이는 것이고, 스칼라곱은 벡터의 길이와 방향을 조절하는 것입니다. 이 두 연산이 닫혀 있어야 벡터 공간이 성립합니다. 그래서 '선형' 조합이라는 말이 나옵니다.

- **노름은 왜 길이 이상의 의미를 가질까요?**
  - 노름은 벡터의 크기를 정량화하는 도구입니다. 정규화에서는 방향만 유지하고 크기를 1로 표준화하므로, 방향 비교 문제에서는 정규화된 벡터를 씁니다. 반면 수량 데이터에서는 노름이 유지해야 할 정보이므로 맥락에 따라 해석이 달라집니다.

<!-- toc:begin -->
## 시리즈 목차

- [Linear Algebra 101 (1/10): 선형대수란 무엇인가?](./01-what-is-linear-algebra.md)
- **Linear Algebra 101 (2/10): 벡터 (현재 글)**
