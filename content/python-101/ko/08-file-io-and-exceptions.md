...
파일 핸들과 예외, 두 가지 외부 자원과 외부 신호를 어떻게 짧고 안전한 코드로 다룰 수 있을까요?

이 글은 그 질문에 답하기 위해 `with` 문과 좁은 `except` 절이라는 두 도구를 중심으로, 파일을 열고 읽고 쓰는 단계와 그 사이에서 발생하는 실패를 분류하는 방식을 함께 살펴봅니다.
...
- **자원은 컨텍스트 매니저로 묶는다.** 파일·소켓·락·DB 커넥션 모두 같은 패턴입니다. `with` 없이 자원을 여는 코드는 리뷰에서 의심부터 합니다.
- **예외는 좁게 잡고, 모르면 위로 보낸다.** `except Exception:`은 디버깅을 어렵게 만드는 가장 흔한 원인입니다. 잡는 이유와 회복 전략이 명확할 때만 잡습니다.
- **인코딩은 항상 명시한다.** 운영 환경과 개발 환경의 로케일이 다를 때 가장 먼저 의심되는 자리이므로, `utf-8`을 기본값처럼 둡니다.
- **큰 파일은 스트리밍으로 다룬다.** `read()`로 한 번에 올리는 코드는 작은 입력에서는 잘 동작하다가 운영 데이터에서 메모리 한계를 만납니다. 처음부터 줄 단위 순회로 작성합니다.
- **경로 조작은 `pathlib`로 통일한다.** 문자열 `+`나 `os.path` 혼용은 코드베이스 전체의 일관성을 떨어뜨립니다. 하나로 정해 놓으면 잔실수가 사라집니다.
