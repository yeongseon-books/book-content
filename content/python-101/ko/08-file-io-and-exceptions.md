...
이런 상황을 무시하고 짠 코드는 평소에는 잘 동작하다가 운영 환경에서 갑자기 망가지는 편입니다. 핸들을 닫지 않으면 장시간 실행되는 서버에서 file descriptor가 누수되고, 예외를 무시하면 데이터가 절반만 쓰인 채 끝나기도 합니다. 반대로 모든 예외를 `except:`로 한꺼번에 삼키면 진짜 버그가 조용히 숨어버립니다.
...
- **`with`는 정상 종료와 예외 발생 어느 쪽이든 `__exit__`을 호출해 핸들을 닫아 줍니다.** 그래서 `try`/`finally`로 직접 `close()`를 호출하는 일을 거의 하지 않게 됩니다.
...
- `f.readlines()` — 모든 줄을 리스트로 한꺼번에 읽습니다.
...
- `except:`가 모든 예외를 삼켜서, 파일 권한 문제든 코드 버그든 빈 문자열로 묻혀 버립니다.
...
- **`except:`로 모든 예외 잡기** — 진짜 버그까지 함께 삼켜집니다. 잡고 싶은 예외 클래스만 명시하세요.
