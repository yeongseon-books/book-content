...
<!-- a-grade-intro:begin -->
**핵심 질문**: 함수 시그니처를 설계할 때 다섯 인자 형태(positional, keyword, default, `*args`, `**kwargs`)를 어떤 기준으로 골라야 할까요?

> 함수 시그니처는 호출자와 본문 사이의 계약이며, `/`와 `*` 구분자는 그 계약의 강도를 단계별로 조절하는 도구입니다. 계약을 느슨하게 두면 호출부가 깨지기 쉽고, 인자 이름을 바꾸기만 해도 모든 호출부를 따라가야 하는 비용이 발생합니다.
<!-- a-grade-intro:end -->
...
- `def`로 함수를 정의하고 `return`이 어떤 값을 돌려주는지 통제하는 법
- positional, keyword, default, `*args`, `**kwargs` 다섯 가지 인자 형태와 호출 시 결합 규칙
- positional-only(`/`)와 keyword-only(`*`) 구분자가 API 설계에 어떤 영향을 주는지
- mutable 기본값 함정과 안전한 대체 패턴
- `lambda`가 적절한 자리와 그렇지 않은 자리
- 간단한 type hint를 함수 시그니처에 붙여 의도를 문서화하는 법

## 이 글에서 답할 질문

- positional, keyword, default, `*args`, `**kwargs` 다섯 인자 형태는 호출 시 어떤 순서로 결합되는가?
- positional-only(`/`)와 keyword-only(`*`) 구분자는 API 설계에 어떤 차이를 만드는가?
- mutable 기본값 함정은 왜 생기며, 안전한 대체 패턴은 무엇인가?
- `lambda`가 적절한 자리와 일반 `def`를 써야 하는 자리는 어떻게 가르는가?
- type hint를 함수 시그니처에 붙이면 호출자에게 어떤 가치가 생기는가?
...
- **시그니처는 코드 변경 비용을 미리 정해 둡니다** — positional만 노출하면 인자 이름 변경이 안전하고, keyword-only로 잠그면 인자 순서 변경이 안전합니다. 이 둘을 의식적으로 골라 둔 시그니처는 1년 뒤 리팩터링 비용이 다릅니다.
- **`*args`/`**kwargs`는 데코레이터·wrapper 전용 도구로 봅니다** — 일반 함수에 쓰면 IDE 자동완성이 멈추고 타입 검사기가 도와주지 못합니다. "무엇이든 받겠다"는 시그니처는 "무엇을 받는지 모르겠다"는 신호입니다.
- **mutable 기본값은 정의 시점 평가 규칙의 한 사례일 뿐입니다** — 같은 함정이 클래스 변수, 데코레이터 인자에서도 똑같이 재현됩니다. "기본값은 정의 시점에 한 번 평가"를 외워 두면 비슷한 버그를 모두 예방합니다.
- **boolean 자리 인자는 항상 keyword-only로 받습니다** — `f("a", True, False)` 같은 호출은 6개월 후 본인도 못 읽습니다. `*` 뒤로 옮기는 한 줄이 호출부를 영구히 가독성 있게 만듭니다.
- **`return None`이 의도라면 명시합니다** — "부수효과만 있는 함수"임을 docstring과 시그니처(`-> None`)로 표시합니다. 침묵으로 `None`이 흘러가면 호출부의 `if result:` 같은 코드가 조용히 잘못 동작합니다.
...
1. **안전한 누적기.**
   `def append_item(item, items=None) -> list:` 시그니처로, 호출자가 `items`를 주면 그 리스트에 추가하고, 주지 않으면 새 리스트를 만들어 반환하는 함수를 작성하세요.
   - 성공 기준: `append_item(1) == [1]`, `append_item(2) == [2]` (호출 사이에 누적되지 않음).

2. **keyword-only 옵션.**
   `def join_path(*parts, sep="/") -> str:` 시그니처로, 부분 문자열들을 `sep`으로 잇는 함수를 작성하세요. `sep`은 keyword로만 받게 합니다.
   - 성공 기준: `join_path("a", "b", "c") == "a/b/c"`, `join_path("a", "b", sep="-") == "a-b"`.

3. **lambda를 def로 리팩터.**
   `sorted(users, key=lambda u: (u["score"], u["name"]))`을 같은 동작을 하는 명시적 함수 `def by_score_then_name(u): ...`로 분리하고, `sorted(users, key=by_score_then_name)` 형태로 호출하세요.
   - 성공 기준: 두 호출이 같은 결과를 돌려줍니다.
