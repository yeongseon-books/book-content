...
<!-- a-grade-intro:begin -->
**핵심 질문**: Python을 "그냥 설치"하는 것은 왜 위험할까요?

> system Python은 OS가 신뢰하고 쓰는 자원입니다. 거기에 직접 패키지를 설치하면 OS의 어떤 도구가 망가질지 예측할 수 없습니다. 그래서 프로젝트마다 venv로 독립된 환경을 만들고, 그 안에서만 패키지를 설치합니다.
<!-- a-grade-intro:end -->
...
- system Python과 project Python은 왜 분리해서 써야 하는가?
- macOS, Windows, Linux 각각에서 Python 3.12를 안전하게 설치하는 절차의 핵심 차이는 무엇인가?
- venv는 정확히 어떤 문제를 해결하며, 활성화되었는지 어떻게 검증하는가?
- `python`과 `python3` 중 어느 쪽을 명령으로 써야 하며 왜 그런가?
- `requirements.txt`로 환경을 재현한다는 것은 정확히 무엇을 보장한다는 뜻인가?
...
- **system Python에는 절대 손대지 않습니다.** 권한 오류는 "sudo를 쓰라"는 신호가 아니라, "잘못된 곳에 설치하려 한다"는 신호로 읽습니다.
- **활성화는 검증과 한 쌍입니다.** `(.venv)` 프롬프트만 보고 안심하지 않고, 항상 `which python`으로 확인하는 습관을 가집니다.
- **버전은 명시적으로 박습니다.** `python3.12`, `requests==2.32.3`, `.python-version` 파일 — 모호함이 협업과 미래의 자신을 구합니다.
- **표준 도구를 먼저 익힙니다.** uv·poetry는 빠르고 강력하지만, 그 도구들도 venv 위에서 동작합니다. 표준을 알면 어떤 도구로 옮겨도 원리가 보입니다.
- **`.venv/`는 산출물입니다.** 소스가 아닙니다. git에는 `requirements.txt`만 올리고, 환경은 명령으로 재현합니다. "내 컴퓨터에서는 되는데"의 90%가 이 한 줄로 사라집니다.
...
1. **두 개의 venv를 만들어 격리 확인**
   ...
   - 성공 기준: 두 venv가 서로 다른 버전의 requests를 가리키고, 한쪽 변경이 다른 쪽에 영향을 주지 않습니다.
