...
- **왜 system Python과 프로젝트용 `.venv`를 분리해야 하고, 그 경계는 어떤 명령으로 바로 확인할 수 있을까요?**
  - 입문자가 가장 많이 겪는 문제는 문법이 아니라 실행 환경 불일치입니다. VS Code에서 실행한 결과와 터미널에서 실행한 결과가 다르면, 코드 자체보다 먼저 인터프리터 경로를 확인해야 합니다.
- **활성화가 정말 성공했다는 사실을 `which python`, `sys.executable`, `pip --version` 출력에서 어떻게 읽어야 할까요?**
  - 활성화가 정말 성공했다는 사실을 `which python`, `sys.executable`, `pip --version` 출력에서 어떻게 읽어야 할까요 — 본문에서 구체적으로 다룹니다.
- **`sudo pip install`, 버전 없는 `python3 -m venv`, `.venv/` 커밋 같은 실수는 왜 재현성과 협업을 망가뜨릴까요?**
  - "권한이 없다"는 오류를 보면 `sudo`를 붙이고 싶어집니다. 절대 하지 마세요. 권한 오류 자체가 "지금 system Python을 건드리고 있다"는 신호입니다. 폴더로 들어가서 venv를 만들면 됩니다.
- **멘탈 모델에서 가장 흔한 실수는 무엇일까요?**
  - > Python은 한 대의 컴퓨터에 여러 개가 동시에 존재할 수 있고, 각 프로젝트는 자기만의 Python을 하나씩 들고 있어야 합니다.
...
- **Python 101 (1/10): 왜 Python인가, 그리고 설치와 venv (현재 글)**
