...
- **클래스와 객체: 데이터와 동작을 함께 묶기를 운영 관점에서 볼 때 먼저 어떤 경계를 확인해야 할까요?**
  - 먼저 클래스가 공유할 행동과 인스턴스가 따로 가질 상태를 구분해야 합니다. `User.role = "member"` 같은 클래스 속성과 `self.name`, `self.email` 같은 인스턴스 속성을 섞지 않고, `Cart.add()`와 `Cart.total()`처럼 상태를 다루는 메서드를 경계로 두어야 객체 책임이 선명해집니다.
- **클래스와 객체: 데이터와 동작을 함께 묶기에서 예제나 다이어그램으로 검증해야 할 핵심 신호는 무엇일까요?**
  - 핵심 신호는 메서드 바인딩과 객체 표현이 실제로 어떻게 드러나는지입니다. `User("Ada", "a@x").label()`이 `Ada <a@x>`를 돌려주고, `__repr__`을 넣었을 때 REPL에 `User('Ada', 'a@x')`가 보이며, `@dataclass`를 붙인 뒤 `u == User("Ada", "a@x")`가 `True`가 되는지 확인하면 클래스·메서드·비교 규칙이 한 번에 검증됩니다.
- **클래스와 객체: 데이터와 동작을 함께 묶기를 실제 시스템에 적용할 때 어떤 실패를 먼저 막아야 할까요?**
  - 가장 먼저 막아야 할 실패는 메서드에서 `self`를 빼먹거나, 클래스 속성에 `items = []` 같은 가변 객체를 두어 인스턴스끼리 상태가 새는 경우입니다. 또 `__repr__` 없이 로그에 메모리 주소만 남기는 문제, `__eq__` 없이 값 비교가 객체 동일성 비교로 흘러가는 문제, 상속 계층을 깊게 쌓아 메서드 출처를 추적하기 어려워지는 문제도 초기에 차단해야 합니다.
