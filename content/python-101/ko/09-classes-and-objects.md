...
데이터와 동작이 항상 같이 움직여야 한다면, 이 둘을 한 단위로 묶어 호출자가 매번 같은 인자를 챙기지 않게 만드는 가장 단순한 방법은 무엇일까요?

이 글은 그 질문에 답하기 위해 `class` 문, `__init__`/`self`, dunder 메서드, 그리고 `@dataclass`까지를 차례로 살펴봅니다.
...
- `__init__`이 인스턴스 초기화에서 어떤 역할을 하는지 설명할 수 있습니다.
- `self` 매개변수가 무엇을 가리키는지, 그리고 왜 첫 매개변수인지 설명할 수 있습니다.
- 인스턴스 속성과 클래스 속성을 구분해 사용할 수 있습니다.
- `__repr__`, `__str__`, `__eq__` 같은 dunder 메서드의 기본 역할을 설명할 수 있습니다.
- 단일 상속과 메서드 오버라이드의 동작을 설명할 수 있습니다.
- 단순 데이터 묶음에는 `@dataclass`를 사용할 수 있습니다.

## 이 글에서 답할 질문

- `__init__`이 인스턴스 초기화 단계에서 정확히 어떤 일을 하는가?
- `self`는 무엇을 가리키며, 왜 메서드의 첫 매개변수로 명시되는가?
- 인스턴스 속성과 클래스 속성을 혼동하면 어떤 사고가 생기는가?
- `__repr__`, `__str__`, `__eq__`는 각각 어떤 시점에 호출되며 어떤 인상을 결정하는가?
- 단순 데이터 묶음에 `@dataclass`를 쓰는 게 직접 `__init__`/`__repr__`을 작성하는 것보다 좋은 이유는 무엇인가?
...
- **함수가 충분하면 클래스를 만들지 않는다.** 데이터와 동작이 단단히 짝을 이룰 때만 클래스가 값을 합니다. 그렇지 않으면 함수와 dict 조합이 더 단순합니다.
- **상속보다 합성을 먼저 본다.** 깊은 상속은 메서드의 출처 추적을 어렵게 만듭니다. 다른 객체를 속성으로 두는 합성으로 풀리면 그쪽이 우선입니다.
- **dunder 메서드는 인터페이스 계약이다.** `__eq__`, `__hash__`, `__iter__`를 정의하는 순간 그 객체는 Python의 표준 문법(`==`, `set`, `for`)에 들어옵니다. 이 점을 의식하고 정의합니다.
- **가변 기본값은 `__init__` 안에서 만든다.** 클래스 본문에 `items = []`을 두면 인스턴스가 리스트를 공유합니다. 이 함정은 거의 모든 코드베이스에서 한 번씩 등장합니다.
- **데이터 모델에는 `@dataclass`를 기본값으로.** 손으로 쓴 `__init__`/`__repr__`/`__eq__`는 보일러플레이트입니다. 정말 손작성이 필요한 이유가 있을 때만 떼어 냅니다.
...
1. `Point` 클래스를 만들어 `__init__`으로 `x`, `y`를 받고, `distance_to(other)` 메서드로 두 점 사이의 거리를 돌려주세요.
2. `Point`에 `__repr__`과 `__eq__`를 직접 정의해 `Point(1, 2) == Point(1, 2)`가 `True`가 되도록 만들어 보세요.
3. `Point`를 `@dataclass`로 다시 작성하고, 동작이 동일한지 REPL에서 확인해 보세요.
4. `Animal` 클래스를 만들고, `Dog(Animal)`이 `speak()` 메서드를 오버라이드하도록 만들어 보세요. `super().speak()`도 호출해 보세요.
