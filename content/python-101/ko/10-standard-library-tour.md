...
외부 패키지를 추가하기 전에, 표준 라이브러리가 이미 답을 가지고 있을까요?

이 글은 그 질문에 답하기 위해 작은 스크립트에서 가장 자주 마주치는 다섯 모듈을 골라, 각자가 어떤 도메인을 담당하는지를 짧게 훑어봅니다.
...
- `datetime`으로 날짜와 시간을 다룰 때 흔히 빠지는 함정은 무엇인가?
- `pathlib.Path`가 문자열 경로 조작보다 좋은 점은 어떤 코드에서 두드러지는가?
- `json.dumps`와 `json.loads`는 dict와 JSON 문자열을 어떤 규칙으로 변환하는가?
- `Counter`, `defaultdict`, `deque`는 각각 어떤 종류의 코드를 짧게 만들어 주는가?
- `itertools.chain`, `groupby`, `combinations`는 어떤 반복 패턴을 압축해 주는가?
...
- **외부 패키지 전에 표준 라이브러리를 돌아본다.** 의존성은 공짜가 아니므로, 항상 이미 들어 있는 도구가 충분한지 먼저 확인합니다.
- **시간대는 입력 경계에서 명시한다.** naive `datetime`은 운영 환경에서 조용한 버그의 단골입니다. 코드 내부는 aware `datetime`을 유지합니다.
- **경로는 문자열로 다루지 않는다.** `os.path` 혼용이나 `+` 조합은 코드베이스 일관성을 깨뜨립니다. `pathlib`로 통일하는 편이 쿨립니다.
- **이터레이터의 일회성을 의식한다.** `itertools` 결과는 한 번 소비되면 비워집니다. 두 번 보고 싶으면 `list(...)`로 먼저 매터라이즈합니다.
- **표준 라이브러리 문서를 북마크하고 되돌아본다.** 고수의 조건은 새 패키지를 외우는 것이 아니라, 이미 있는 도구를 제때 꺼내 쓰는 것입니다.
