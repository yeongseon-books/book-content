...
이 글은 Python 3.12 기준으로 작성했습니다. 모든 예제는 venv를 활성화한 REPL이나 짧은 스크립트에서 그대로 실행할 수 있습니다.
...
```mermaid
flowchart LR
    a["변수 a"] --> obj1["int 객체: 42"]
    b["변수 b"] --> obj1
    c["변수 c"] --> obj2["str 객체: 'hi'"]
    style obj1 fill:#e8f4fd,stroke:#1f77b4
    style obj2 fill:#fff4e6,stroke:#d97706
```
...
- [ ] REPL에서 `a = 1000; b = 1000; print(a == b, a is b)`를 실행해 보았다
