...
모듈과 패키지는 이 문제를 풀기 위한 Python의 기본 도구입니다. 파일 단위로 코드를 쪼개고, 디렉터리 단위로 묶고, 필요한 부분만 골라서 가져오는 구조를 만듭니다. CLI 스크립트, 웹 서버, 데이터 파이프라인 어느 쪽이든 결국 이 구조 위에 올라갑니다. 작은 프로젝트에서 미리 익혀두면, 큰 프로젝트로 옮겨갈 때 새로 배워야 할 내용이 거의 없습니다.
...
모든 모듈에는 `__name__`이라는 문자열이 자동으로 붙습니다. 다른 파일에서 import되어 실행될 때는 모듈 이름이 들어가고, `python file.py`처럼 직접 실행될 때만 `"__main__"`이 들어갑니다.
...
이 구조는 프로젝트가 커져도 거의 바뀌지 않습니다. 처음에 `myapp/__init__.py` 한 줄로 시작해도, 나중에 하위 패키지를 늘리는 방식으로 자연스럽게 확장됩니다.
