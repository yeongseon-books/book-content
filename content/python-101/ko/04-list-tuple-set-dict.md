...
<!-- a-grade-intro:begin -->
**핵심 질문**: list, tuple, set, dict는 어떤 기준으로 골라야 할까요?

> 네 자료구조는 가변성·순서·중복 허용·해시 가능성이라는 네 축으로 구분됩니다. 자료구조를 고른다는 것은 이 네 축에서 어떤 보장을 받고 어떤 보장을 포기할지를 의식적으로 결정하는 일입니다. 잘못 고르면 코드가 무거워지거나 O(n) 검색이 누적되어 데이터가 커질수록 느려집니다.
<!-- a-grade-intro:end -->
...
- **자료구조 선택은 의도 표현이다** — list냐 tuple이냐, set이냐 list냐를 고르는 순간 "이 묶음은 바뀔 수 있는가, 순서가 의미가 있는가"를 코드 위에 적어 둡니다. 타입만 봐도 의도가 드러나야 합니다.
- **`b = a`는 절대 복사가 아니다** — alias 사고는 디버깅이 가장 어려운 부류입니다. 복사 의도가 있다면 `list(a)`·`a[:]`·`copy.deepcopy`를 명시적으로 씁니다.
- **mutable 기본 인자는 함수 정의 시점에 한 번만 만들어진다** — 호출마다 새 빈 리스트가 만들어진다는 직관을 배신합니다. 기본값은 항상 `None`으로 두고 본문에서 초기화합니다.
- **`d[key]`와 `d.get(key)`는 의미가 다르다** — 전자는 "키가 반드시 있어야 한다"는 단언이고 후자는 "없을 수도 있다"는 인정입니다. 잘못 쓰면 의도와 반대인 코드가 됩니다.
- **O(1) 멤버십이 필요한 자료는 set이나 dict로 보관한다** — list `in` 검사가 핫 루프에 들어가면 데이터가 커질수록 응답 시간이 선형으로 늘어납니다. 자료구조 선택 하나로 막을 수 있는 사고입니다.
