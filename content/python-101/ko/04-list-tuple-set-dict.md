...
- **list를 `=`으로 "복사"한 뒤 한쪽을 바꿨는데 양쪽이 함께 바뀌는 alias 사고?**
  - 1. **`b = a`로 list를 "복사"한다.** `b = a`는 같은 객체에 이름표를 하나 더 붙일 뿐입니다
- **함수의 기본 인자로 `def f(items=[]):`를 썼다가, 호출이 누적될수록 리스트가 자라는 문제?**
  - 리스트, 튜플, 셋, 딕셔너리는 모두 자주 쓰이지만 목적이 다릅니다
- **dict에서 없는 키를 `d[key]`로 꺼내다가 `KeyError`로 멈추는 일?**
  - - **카운팅·그루핑.** `Counter`, `defaultdict(list)`는 직접 짜는 dict 누락 키 패턴을 줄여 줍니다
...
- **Python 101 (4/10): list, tuple, set, dict (현재 글)**
