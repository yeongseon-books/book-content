...
- **`if value:`와 `if value is None:`는 언제 완전히 다른 의미가 될까요?**
  - 이 글은 truthy·falsy 규칙 때문에 `0`, `""`, `[]`, `None`이 모두 `if value:`에서는 거짓으로 묶인다고 보여 주었습니다. 그래서 값이 비어 있는지만 보려는 것인지, 정말로 "아예 주어지지 않음"만 구분하려는 것인지에 따라 `if value:`와 `if value is None:`를 나눠 써야 합니다.
- **`zip`, `enumerate`, comprehension을 쓰면 인덱스를 직접 돌리던 루프가 어떻게 더 읽기 쉬워질까요?**
  - 본문은 `while i < len(names)` 형태를 `zip(names, scores)`와 comprehension으로 바꿔, "두 시퀀스를 짝지어 60점 이상만 모은다"는 의도를 한 줄에 드러냈습니다. `enumerate(zip(...), start=1)` 예제까지 보면 인덱스 관리, 길이 계산, 누적 리스트 조작을 직접 적지 않아도 같은 작업을 더 또렷하게 표현할 수 있습니다.
- **`while True`, `zip(..., strict=True)`, `continue`를 쓸 때 어떤 버그를 먼저 경계해야 할까요?**
  - `while True`는 탈출 조건이 빠지면 무한 루프로 이어지고, `zip`은 기본적으로 짧은 쪽에 맞춰 조용히 끝나므로 길이 불일치를 놓치기 쉽습니다. 또한 `continue`는 후처리를 건너뛸 수 있으므로, 본문이 강조한 것처럼 종료 조건·길이 검증·후속 작업 위치를 함께 점검해야 예측 가능한 제어 흐름을 유지할 수 있습니다.
...
- **Python 101 (5/10): 제어 흐름: if, for, while, comprehension (현재 글)**
