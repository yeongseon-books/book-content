...
<!-- a-grade-intro:begin -->
**핵심 질문**: 분기와 루프를 짤 때 어떤 기준으로 `if`·`for`·`while`·comprehension을 고르나요?

> 제어 흐름을 짤 때는 "이 분기·반복이 무엇을 결정하느냐"를 truthy/falsy 한 단계와 종료 조건 한 단계로 분리해 두면, 컴프리헨션과 일반 루프 사이의 선택도 같은 잣대로 답이 나옵니다. 이 두 잣대를 분리하지 못하면 if 사다리가 길어지고 comprehension에 부수효과가 섞여 들어옵니다.
<!-- a-grade-intro:end -->
...
```mermaid
flowchart TB
    Q1{"결정이 1회인가, 반복인가?"} -->|1회| If["if / elif / else"]
    Q1 -->|반복| Q2{"순회할 이터러블이 있는가?"}
    Q2 -->|예| Q3{"변환만 하는가, 부수효과가 있나?"}
    Q2 -->|아니오| While["while + break"]
    Q3 -->|변환만| Comp["comprehension"]
    Q3 -->|부수효과| For["일반 for 루프"]
```
...
- **루프 선택은 "이터러블이 있는가"로 끝난다** — 이터러블이 먼저 있으면 `for`, 조건이 있을 때까지 돌아야 하면 `while`. 이 경계가 흐려지면 off-by-one과 무한 루프가 늘어납니다.
- **comprehension은 "입력 → 새 컴테이너"로만 쓴다** — print, 파일 쓰기, 외부 상태 변경이 섞이면 일반 `for`로 돌아갑니다. 몇 자를 줄이는 것이 의도를 가리는 대가보다 널지 않습니다.
- **truthy/falsy는 "비어 있다"는 의미지 "없다"는 의미가 아니다** — `0`이 유효한 값이고 `None`이 "압도적으로 없음"일 때는 `if value is None:`을 명시합니다. `if value:`와 `is None`을 혼동하면 0이 누락됩니다.
- **순회 중 시퀀스를 수정하지 않는다** — 의도가 "필터링"이면 새 리스트를 만듭니다. 이 규칙 하나로 원소 건너뛰기·무한 루프·자료구조 상태 손상 세 가지 버그를 동시에 예방합니다.
- **`zip(..., strict=True)`로 속으로 궁했던 가정을 드러낸다** — 길이가 같을 것이라는 암묵적 가정이 깨지면 조용히 짧은 쪽에 맞춰 끝나지 않고 즉시 `ValueError`를 던지게 합니다. 명시적 실패가 무성의 누락보다 항상 안전합니다.
