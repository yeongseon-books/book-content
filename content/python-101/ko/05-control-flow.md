...
- **제어 흐름: if, for, while, comprehension를 운영 관점에서 볼 때 먼저 어떤 경계를 확인해야 할까요?**
  - 먼저 이 코드가 한 번만 결정을 내리는 분기인지, 같은 작업을 반복하는 루프인지부터 구분해야 합니다. 그다음 순회 대상이 이미 있으면 `for`, 종료 조건이 바뀔 때까지 돌아야 하면 `while`, 새 컬렉션으로 변환만 하면 comprehension으로 두어 `zip(names, scores)`나 `remaining -= 1` 같은 핵심 제어점을 명확히 잡는 편이 안전합니다.
- **제어 흐름: if, for, while, comprehension에서 예제나 다이어그램으로 검증해야 할 핵심 신호는 무엇일까요?**
  - 본문 예제에서는 `bool(v)` 결과로 `0`, `""`, `[]`, `None`이 어떻게 갈리고, `[0]`은 왜 truthy인지가 첫 번째 신호입니다. 또 `enumerate(zip(names, roles), start=1)`의 묶음 결과, `zip(..., strict=True)`의 길이 검증, `[f"{name}:{score}" for name, score in zip(names, scores) if score >= 60]`처럼 필터와 변환이 한 줄에서 어떻게 읽히는지를 확인해야 합니다.
- **제어 흐름: if, for, while, comprehension를 실제 시스템에 적용할 때 어떤 실패를 먼저 막아야 할까요?**
  - 가장 먼저 막아야 할 실패는 `while True`에 탈출 조건을 빼먹어 무한 루프가 되는 경우와, 순회 중 리스트를 수정해 원소를 건너뛰는 경우입니다. 여기에 `if value:`로 `0`과 `None`을 혼동하는 분기, `zip`이 짧은 쪽에서 조용히 멈추는 문제, `continue` 때문에 후처리가 건너뛰는 문제까지 함께 점검해야 합니다.
