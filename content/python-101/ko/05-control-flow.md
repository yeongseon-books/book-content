...
분기와 루프는 거의 모든 코드의 절반 이상을 차지합니다. 그래서 이 부분이 어수선하면 나머지가 아무리 좋아도 읽기 어려워집니다. Python은 다른 언어에 비해 분기·루프 문법이 단출한 대신, `for`-`else`나 comprehension처럼 처음 보면 낯선 도구가 몇 개 있습니다. 이 도구를 미리 정리해 두면 if 사다리를 길게 쓰지 않아도 되고, 일반 루프와 comprehension을 의도적으로 갈아탈 수 있습니다.
...
이 두 형태를 자유롭게 오갈 수 있다면 분기·루프 코드의 절반은 정리됩니다.
...
- `for`는 이터러블 위에서 돌고, `while`은 조건이 truthy인 동안 돕니다. `enumerate`/`zip`/`range`로 표현력을 키웁니다.
