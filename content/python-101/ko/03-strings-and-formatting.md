...
이 그림에서는 문자열과 포매팅를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.
...
- **한국어가 들어 있는 파일을 읽다가 `UnicodeDecodeError`로 멈춥니다?**
  - 본문의 기준은 문자열과 포매팅를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **큰 로그를 만드는 동안 `+` 연결을 반복해서 메모리와 시간을 낭비합니다?**
