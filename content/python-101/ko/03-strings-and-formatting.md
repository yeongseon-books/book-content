...
<!-- a-grade-intro:begin -->
**핵심 질문**: Python의 `str`과 `bytes`는 왜 섞어 쓰면 안 될까요?

> `str`은 Unicode 코드 포인트의 시퀀스, `bytes`는 0~255 정수의 시퀀스입니다. 메모리 안에서는 항상 `str`로만 다루고, 외부 세계(파일·네트워크)와의 경계에서만 `encode`/`decode`합니다. 이 한 가지 규칙이 인코딩 오류와 포매팅 사고의 90%를 막아 줍니다.
<!-- a-grade-intro:end -->
...
- Python 3의 `str`이 왜 "Unicode 문자열"이고, `bytes`와 어떻게 다른지
- 작은따옴표·큰따옴표·삼중 따옴표·raw 문자열·byte 리터럴의 쓰임
- `split`, `join`, `strip`, `replace`, `find`, `startswith` 같은 핵심 메서드
- 슬라이싱으로 문자열 일부를 꺼내는 방법과, `str`이 불변(immutable)이라는 의미
- f-string과 format spec(`:>10`, `:.2f`, `:%Y-%m-%d`, `!r`)을 자유롭게 쓰는 법
- `str.format`과 `%` 포매팅의 자리, 그리고 왜 f-string을 우선하는지
- 인코딩 오류(`UnicodeDecodeError`)와 줄바꿈·공백 처리에서 자주 겪는 함정
- `re` 모듈로 정규표현식과 처음 만나는 법

대부분의 예시는 `>>>`가 붙은 REPL 블록이며, 그대로 따라 입력하면 같은 결과가 나옵니다. `>>>`가 없는 코드 블록은 설명을 위한 발췌이므로, 변수가 미리 정의돼 있다고 가정합니다.

## 이 글에서 답할 질문

- Python 3의 `str`이 "Unicode 문자열"이라는 말과 `bytes`와의 차이는 정확히 무엇을 의미하는가?
- f-string과 `str.format`, `%` 포매팅 중 어느 것을 우선해야 하며 왜 그런가?
- format spec(`:>10`, `:.2f`, `:%Y-%m-%d`, `!r`)은 각각 어떤 출력을 만들어 주는가?
- `UnicodeDecodeError`는 어떤 상황에서 터지며 어떻게 회복하는가?
- 정규표현식과 일반 문자열 메서드 중 무엇을 먼저 써야 하는지의 1차 기준은 무엇인가?
...
- **인코딩 경계는 한 곳에 모읍니다.** `decode`는 입력 함수에서, `encode`는 출력 함수에서만 합니다. 비즈니스 로직 한가운데에 `encode`가 흩어져 있으면 디버깅이 지옥이 됩니다.
- **f-string은 출력 도구이지 템플릿 도구가 아닙니다.** 외부에서 채워야 하는 템플릿(메일 본문, 다국어 메시지)은 `str.format` 또는 `string.Template`을 씁니다. f-string은 정의 시점에 변수를 캡처해 버립니다.
- **사용자 입력은 절대 f-string으로 SQL/HTML/shell에 끼우지 않습니다.** 항상 placeholder, escape 함수, 전용 라이브러리를 사용합니다. 이것은 취향이 아니라 보안의 기본입니다.
- **`+` 반복 대신 `join`을 씁니다.** 큰 문자열을 만드는 자리에 `for`와 `+=`이 보이면 거의 항상 잘못된 패턴입니다. 리스트에 모아 `"".join(...)`으로 한 번에 합칩니다.
- **로깅은 `logger.info("user=%s", uid)`처럼 인자 분리 형식을 선호합니다.** 로그 레벨이 꺼졌을 때 포매팅 비용이 들지 않고, 구조화된 로깅으로 옮길 때도 자연스럽게 이어집니다.
...
1. **이름·나이 표 출력기**
   `[("ada", 30), ("bob", 28), ("charlie", 41)]` 같은 리스트를 받아 이름은 왼쪽 정렬 폭 10, 나이는 오른쪽 정렬 폭 3으로 출력하는 함수 `print_table(rows)`을 작성하세요.
   - 성공 기준: `print_table([("ada", 30)])`이 `'ada        |  30'`을 출력합니다.

2. **CSV 한 줄 파서**
   `"ada, 30, 1700000.5"` 형태의 한 줄을 받아 `(name: str, age: int, salary: float)` 튜플로 돌려주는 `parse_row(line: str)`을 작성하세요. 양쪽 공백은 제거하고, 숫자가 아니면 `ValueError`를 그대로 올라가게 둡니다.
   - 성공 기준: `parse_row("ada, 30, 1700000.5")`이 `('ada', 30, 1700000.5)`을 돌려줍니다.

3. **이메일 추출기**
   본문 문자열을 받아 모든 이메일 주소를 리스트로 돌려주는 `extract_emails(text: str) -> list[str]`을 작성하세요. `re.findall`과 raw 문자열을 사용하세요.
   - 성공 기준: `extract_emails("ada@example.com과 bob@example.org")`이 `['ada@example.com', 'bob@example.org']`을 돌려줍니다.
