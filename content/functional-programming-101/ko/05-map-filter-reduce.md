...
이 글은 Functional Programming 101 시리즈의 다섯 번째 글입니다.
...
- `reduce`는 왜 강력하지만 동시에 남용되기 쉬울까요?

> 멘탈 모델: `map`은 변환, `filter`는 선택, `reduce`는 집계입니다. 세 함수를 역할별로 구분해 생각하면 대부분의 반복 로직을 더 작고 읽기 쉬운 단위로 나눌 수 있습니다.
...
Python에서는 단순한 경우 컴프리헨션이 더 관용적이지만, 도구 선택은 개념 이해 위에 있어야 합니다. 어떤 상황에서 무엇이 더 읽기 쉬운지 판단하려면 먼저 역할 자체를 분리해서 볼 수 있어야 합니다.
