...
- **`map`, `filter`, `reduce`는 각각 어떤 역할을 맡을까요?**
  - `map`, `filter`, `reduce`는 각각 어떤 역할을 맡을까요 — 본문에서 구체적으로 다룹니다.
- **반복문으로 쓰던 데이터를 선언형 파이프라인으로 어떻게 바꿀 수 있을까요?**
  - 반복문으로 쓰던 데이터를 선언형 파이프라인으로 어떻게 바꿀 수 있을까요 — 본문에서 구체적으로 다룹니다.
- **리스트 컴프리헨션과 `map`/`filter`는 언제 각각 더 적합할까요?**
  - 리스트 컴프리헨션과 `map`/`filter`는 언제 각각 더 적합할까요 — 본문에서 구체적으로 다룹니다.
- **개념 개요에서 가장 흔한 실수는 무엇일까요?**
  - > 세 함수는 모두 반복을 숨기지만, 무엇을 하려는지는 서로 다릅니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - > 세 함수는 모두 반복을 숨기지만, 무엇을 하려는지는 서로 다릅니다.
