...
- **`map`, `filter`, `reduce`는 각각 어떤 역할을 맡을까요?**
  - `map`은 모든 원소를 다른 값으로 바꾸고, `filter`는 조건에 맞는 원소만 남기며, `reduce`는 여러 값을 하나로 축약합니다. 섭씨를 화씨로 바꾸는 `map`, 짝수만 고르는 `filter`, 주문 총액과 단어 빈도를 만드는 `reduce` 예제가 세 역할을 분리해서 보여 줍니다.
- **반복문으로 쓰던 데이터를 선언형 파이프라인으로 어떻게 바꿀 수 있을까요?**
  - 먼저 선택, 변환, 집계 중 어떤 단계가 필요한지 나눈 뒤 순서대로 조립하면 됩니다. 가격 목록에서 `filter(lambda p: p >= 3000)`로 대상을 고르고 `map(lambda p: int(p * 0.9))`로 할인한 예제와, 주문 데이터를 `with_total → expensive → grand_total`로 흘린 예제가 바로 그 구조입니다.
- **리스트 컴프리헨션과 `map`/`filter`는 언제 각각 더 적합할까요?**
  - 단순한 변환과 조건 필터링은 `[x ** 2 for x in numbers if x % 2 == 0]`처럼 컴프리헨션이 더 자연스럽습니다. 반대로 `str.upper`처럼 이미 있는 함수를 그대로 적용하거나 지연 평가되는 iterator 흐름을 유지하고 싶을 때는 `map`과 `filter`가 더 간결합니다.
