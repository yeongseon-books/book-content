...
- 클로저와 `partial` 선택 흐름을 실무에 적용할 때 주의할 점은 무엇일까요?
...
- **클로저는 바깥 스코프의 변수를 어떻게 기억할까요?**
  - 클로저는 내부 함수가 자유 변수에 대한 참조를 붙잡고 있기 때문에 바깥 함수가 끝난 뒤에도 값을 기억합니다. `make_counter()`가 `count`를 `nonlocal`로 누적하고, `make_greeter()`가 각기 다른 `greeting`을 유지하는 예제가 그 메커니즘을 그대로 보여 줍니다.
- **자유 변수와 cell object는 디버깅에서 왜 중요한 단서일까요?**
  - 클로저가 왜 예상과 다른 값을 내는지 확인할 때 `co_freevars`와 `__closure__[0].cell_contents`는 직접적인 증거가 됩니다. `hello`와 `bye`가 각각 어떤 `greeting`을 붙잡고 있는지 확인한 예제처럼, 루프 변수 공유나 잘못된 상태 캡처 문제를 추적할 때 매우 유용합니다.
- **`functools.partial`은 어떤 상황에서 클로저보다 더 적합할까요?**
  - 새로운 상태를 기억할 필요 없이 기존 함수의 일부 인자만 고정하면 될 때는 `partial`이 더 적합합니다. `square = partial(power, exponent=2)`, `api_get = partial(send_request, "GET", ...)`, `record_audit = partial(publish_audit, policy.audit_channel)`처럼 고정값이 코드에 바로 드러나서 클로저보다 읽기 쉽습니다.
