...
- **어떤 조건을 만족해야 함수를 순수 함수라고 부를 수 있을까요?**
  - 순수 함수는 같은 입력에 항상 같은 출력을 내고, 함수 바깥 상태를 바꾸지 않아야 합니다. 전역 `discount_rate`에 의존하던 `apply_discount()`를 인자 기반 함수로 바꾼 예제와 `square()` 호출을 값으로 치환해도 의미가 유지되는 참조 투명성 예제가 이 기준을 분명하게 보여 줍니다.
- **코드에서 부수효과는 어떤 형태로 나타나며 어떻게 식별할 수 있을까요?**
  - 이 글에서는 난수와 현재 시간처럼 숨은 입력에 의존하는 경우, `global counter`를 바꾸는 경우, 파일에 쓰는 IO, `items.append()`처럼 인자를 제자리에서 수정하는 경우를 모두 부수효과로 짚었습니다. 코드가 입력만으로 설명되지 않거나 호출 뒤 외부 값이 달라지면 먼저 부수효과를 의심하면 됩니다.
- **순수한 계산과 IO를 분리하면 테스트는 어떻게 단순해질까요?**
  - `calculate_subtotal()`, `calculate_tax()`, `calculate_total()`, `format_receipt()`를 순수 코어로 두고 `print_receipt()`만 출력하게 만들면 테스트는 계산 결과만 보면 끝납니다. BMI 예제처럼 입력과 출력에만 `assert`를 쓰면 mock이나 복잡한 환경 준비 없이도 핵심 규칙을 바로 검증할 수 있습니다.

<!-- toc:begin -->
## 시리즈 목차

- [Functional Programming 101 (1/10): 함수형 프로그래밍이란 무엇인가?](./01-what-is-fp.md)
- **Functional Programming 101 (2/10): 순수 함수와 부수효과 (현재 글)**
