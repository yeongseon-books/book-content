...
**핵심 질문**: 데이터를 한꺼번에 메모리에 올리지 않고 필요할 때만 계산할 수 있을까요?

> 지연 평가는 값이 실제로 필요한 시점까지 계산을 미루는 전략입니다. Python의 제너레이터는 지연 평가의 핵심 도구로, 메모리 효율적인 데이터 처리를 가능하게 합니다. 이 글에서는 제너레이터의 원리와 itertools를 활용한 지연 파이프라인을 다룹니다.
...
- **제너레이터 우선** — 메모리·시간 모두에서 보통 유리합니다.
- **itertools 활용** — 표준 도구로 체인을 간결하게 표현합니다.
- **부수효과 위험** — 지연 평가는 부수효과 시점을 흐리므로 순수성을 강제합니다.
- **디버깅 어려움** — 지연 시퀀스 디버깅은 list 변환으로 일시 강제합니다.
- **리소스 종료** — 파일 핸들 등 자원 종료를 잊지 않습니다.
...
1. 소수를 무한히 생성하는 제너레이터를 작성하고 `islice`로 처음 20개를 출력하세요.
2. 대용량 CSV 파일을 줄 단위로 읽어 특정 열의 평균을 계산하는 지연 파이프라인을 구현하세요.
3. `itertools.chain`과 `groupby`를 활용하여 여러 파일의 로그를 날짜별로 그룹화하세요.
