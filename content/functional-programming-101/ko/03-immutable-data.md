...
- **Python에서 mutable 타입과 immutable 타입은 어떻게 구분할까요?**
  - Python에서 mutable 타입과 immutable 타입은 어떻게 구분할까요 — 본문에서 구체적으로 다룹니다.
- **`tuple`, `frozenset`, `NamedTuple`, `frozen dataclass`는 각각 언제 유용할까요?**
  - `tuple`, `frozenset`, `NamedTuple`, `frozen dataclass`는 각각 언제 유용할까요 — 본문에서 구체적으로 다룹니다.
- **함수 경계에서 원본 변경을 막으려면 어떤 패턴이 필요할까요?**
  - 함수 경계에서 원본 변경을 막으려면 어떤 패턴이 필요할까요 — 본문에서 구체적으로 다룹니다.
- **개념 개요에서 가장 흔한 실수는 무엇일까요?**
  - > Python에서는 모든 값을 똑같이 다루면 안 됩니다. 처음부터 "이 값은 바뀌어도 되는가"를 구분해야 설계가 단단해집니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - > Python에서는 모든 값을 똑같이 다루면 안 됩니다. 처음부터 "이 값은 바뀌어도 되는가"를 구분해야 설계가 단단해집니다.
