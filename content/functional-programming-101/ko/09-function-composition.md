...
- 파이프라인이 읽히는 방식을 실무에 적용할 때 주의할 점은 무엇일까요?
...
- **함수 합성은 수학적으로 어떤 의미를 가지며 Python에서는 어떻게 구현할까요?**
  - 함수 합성은 여러 함수를 `f(g(h(x)))`처럼 하나의 계산으로 묶는 방식이며, 이 글에서는 `compose()`와 `pipe()`를 직접 구현해 그 흐름을 보여 줍니다. `add_one`, `double`, `to_str`를 묶어 같은 결과를 만드는 예제가 수학적 정의와 Python 구현을 바로 연결합니다.
- **`compose`와 `pipe`는 무엇이 다르고, 왜 `pipe`가 더 읽기 쉬운 경우가 많을까요?**
  - `compose`는 오른쪽에서 왼쪽으로 읽고, `pipe`는 왼쪽에서 오른쪽으로 읽습니다. `load_data → calculate_totals → filter_passing → sort_by_score → format_output` 예제처럼 실행 순서와 코드 순서가 일치하면 리뷰할 때 중간 단계를 훨씬 따라가기 쉽습니다.
- **데이터 처리와 텍스트 처리에서 파이프라인은 어떤 장점을 줄까요?**
  - 텍스트 처리에서는 `strip_whitespace`, `normalize_spaces`, `replace_special` 같은 작은 함수를 조합해 slug 규칙을 단계별로 바꿀 수 있습니다. 데이터 처리에서는 학생 성적 파이프라인과 주문 이벤트 정산 파이프라인처럼 각 단계가 독립 테스트 가능해서, 어느 변환에서 값이 달라졌는지 바로 좁혀 갈 수 있습니다.
