...
**핵심 질문**: 같은 SQL 인데 *0.1초* 와 *30초* 의 차이는 어디서 오고, 우리는 *EXPLAIN* 을 어떻게 *읽고* *고쳐야* 할까요?

> *튜닝은 *플랜을 보지 않으면 추측* 이다. 추측은 *대부분 틀린다*.*
...
- **B-tree index**: 가장 흔한 *균형 트리* 인덱스.
- **Seq scan**: *전체 행* 을 *순차로* 읽기.
- **Index scan**: 인덱스로 *빠르게* 찾고 *행을 가져옴*.
- **Selectivity**: 조건이 *얼마나 좁히는가*.
- **Covering index**: 쿼리가 *인덱스만으로* 끝남.
...
1. *email* 검색이 *Seq scan* 인 이유를 *EXPLAIN* 으로 확인.
2. *user_id + created_at DESC* 합성 인덱스로 *최근 주문* 쿼리를 튜닝.
3. *deleted_at IS NULL* 부분 인덱스가 *왜 작은지* 설명.
