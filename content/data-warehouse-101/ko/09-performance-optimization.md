...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 Warehouse 성능은 어떤 패턴에서 가장 크게 갈릴, 둘째 같은 결과를 더 적은 비용으로 읽는 방법은 무엇일, 셋째 bytes scanned와 shuffle, spill은 왜 먼저 확인할입니다. 이 글에서 배울 것에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **Warehouse 성능은 어떤 패턴에서 가장 크게 갈릴까요?**
  - - 성능을 좌우하는 핵심 패턴 - 비용을 줄이는 다섯 가지 관점 - 쿼리 플랜을 읽는 기본 감각 - 최적화 실습 5단계 - 입문 단계에서 자주 나오는 실수 5가지 Warehouse는 읽은 양에 따라 비용이 커집니다
- **같은 결과를 더 적은 비용으로 읽는 방법은 무엇일까요?**
  - - 성능을 좌우하는 핵심 패턴 - 비용을 줄이는 다섯 가지 관점 - 쿼리 플랜을 읽는 기본 감각 - 최적화 실습 5단계 - 입문 단계에서 자주 나오는 실수 5가지 Warehouse는 읽은 양에 따라 비용이 커집니다
- **bytes scanned와 shuffle, spill은 왜 먼저 확인할까요?**
  - Warehouse 성능 개선 요청이 오면 많은 팀이 먼저 인덱스를 떠올립니다. 하지만 열 지향 DW에서는 스캔 범위와 데이터 이동이 더 큰 병목입니다. 아래 표는 우선순위를 정리한 기준입니다.
...
- **Data Warehouse 101 (9/10): 성능 최적화 (현재 글)**
