...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 Warehouse 설계는 왜 grain 한 줄에서 시작해야 할, 둘째 fact, dimension, schema, partition, ETL, mart는 어떤 순서로 연결될, 셋째 전자상거래 도메인에서 어떤 dimension을 공통 자산으로 볼 수 있을입니다. 이 글에서 배울 것에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **Warehouse 설계는 왜 grain 한 줄에서 시작해야 할까요?**
  - > *fact_orders grain*: one row per *order*.
- **fact, dimension, schema, partition, ETL, mart는 어떤 순서로 연결될까요?**
  - - 전자상거래 예제로 end-to-end 설계를 보는 법 - grain → dimension → schema → partition → ETL → mart의 순서 - 각 조각이 대시보드로 어떻게 이어지는지 - 설계 실습 5단계 - 입문 단계에서 자주 나오는 실수 5가지 개별 개념을 아는 것만으로는 설계를 설명하기 어렵습니다
- **전자상거래 도메인에서 어떤 dimension을 공통 자산으로 볼 수 있을까요?**
  - - 전자상거래 예제로 end-to-end 설계를 보는 법 - grain → dimension → schema → partition → ETL → mart의 순서 - 각 조각이 대시보드로 어떻게 이어지는지 - 설계 실습 5단계 - 입문 단계에서 자주 나오는 실수 5가지 개별 개념을 아는 것만으로는 설계를 설명하기 어렵습니다
