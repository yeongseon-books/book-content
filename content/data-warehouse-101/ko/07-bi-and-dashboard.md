...
- 데이터 웨어하우스에 데이터를 잘 쌓아도, 왜 의사결정은 여전히 느릴까요?
- BI 도구와 대시보드는 각각 어떤 역할을 맡고, 어디서 자주 실패할까요?
- 좋은 대시보드는 무엇이 다르고, 왜 질문 하나에 화면 하나가 중요할까요?
- KPI, 추세, 드릴다운을 어떤 순서로 배치해야 숫자가 판단으로 이어질까요?
- 실무에서 자주 망가지는 대시보드는 어떤 공통 실수를 반복할까요?

데이터 웨어하우스는 저장소이지만, BI는 그 저장소의 얼굴입니다. 수십억 행을 정확하게 모델링해도 마지막 화면이 시끄럽고 기준이 제각각이면 회의는 결론보다 숫자 해석에 더 많은 시간을 씁니다. 반대로 질문 하나를 화면 하나로 정리한 대시보드는 숫자를 설명 자료가 아니라 판단 도구로 바꿉니다.

대시보드의 성패는 조회 수가 아니라 실제로 내려진 결정 수로 봐야 합니다. 많이 열어 보는 화면이 좋은 화면이 아니라, 빠르게 같은 결론에 도달하게 만드는 화면이 좋은 화면입니다.

> 좋은 대시보드는 데이터를 많이 보여 주는 화면이 아니라, 질문 하나에 답을 하나로 모아 주는 화면입니다.
...
```mermaid
flowchart LR
    DW["웨어하우스 / 데이터 마트"] --> Semantic["시맨틱 레이어"]
    Semantic --> BI["BI 도구"]
    BI --> Dashboard["대시보드"]
    Dashboard --> Decision["의사결정"]
```

웨어하우스나 데이터 마트가 원천 데이터를 정리해 두면, 시맨틱 레이어가 그 위에 공통 지표 정의를 얹습니다. BI 도구는 그 정의를 바탕으로 화면을 만들고, 대시보드는 그 화면을 사람의 질문에 맞게 정리합니다. 마지막에 중요한 것은 차트가 아니라 의사결정입니다.
...
대시보드는 화면부터 만들면 거의 항상 복잡해집니다. 먼저 질문을 한 줄로 고정해야 합니다.

```text
"How much did this month's revenue grow *vs last month*?"
```

이 질문이 명확하면 어떤 KPI가 필요한지, 어떤 비교 기준이 필요한지, 어디까지 세부 분석을 열어 줄지 자연스럽게 정해집니다.

### 2단계 — 질문을 받쳐 줄 모델을 확인합니다

질문이 정해졌다면 이제 그 질문을 어떤 테이블과 어떤 집계로 답할지 확인합니다. 화면 설계보다 먼저 데이터 모델을 점검해야 같은 지표가 화면마다 달라지는 일을 막을 수 있습니다.
...
이 쿼리는 월별 매출 집계를 만드는 가장 단순한 출발점입니다. 중요한 것은 SQL이 복잡하냐가 아니라, 대시보드의 모든 숫자가 같은 정의를 공유하느냐입니다.

### 3단계 — 첫 화면에 KPI 카드를 놓습니다

첫 화면은 사용자가 3초 안에 읽을 수 있어야 합니다. 현재 값 하나만 두면 해석이 어렵기 때문에 비교 기준을 함께 보여 줍니다.
...
현재 값, 전월 대비, 전년 동월 대비 정도면 대부분의 경영 지표는 맥락이 생깁니다. 숫자 하나만 던져 놓는 화면보다 훨씬 빠르게 읽힙니다.

### 4단계 — 추세 차트로 변화 방향을 보여 줍니다

KPI 카드는 현재 상태를 알려 주지만, 변화의 흐름은 설명하지 못합니다. 그래서 직전 12개월 추세 같은 시계열 차트를 바로 아래에 배치하는 경우가 많습니다.
...
이 단계의 목적은 디테일을 늘리는 것이 아니라 방향성을 보여 주는 데 있습니다. 성장세인지, 계절성이 있는지, 특정 시점에 꺾였는지 한눈에 보이면 다음 질문이 훨씬 또렷해집니다.

### 5단계 — 드릴다운으로 원인을 열어 줍니다

요약과 추세만으로는 행동이 정해지지 않을 때가 많습니다. 어떤 카테고리가 기여했는지, 어떤 채널이 감소했는지처럼 원인을 더 깊게 볼 수 있어야 합니다.
...
이 드릴다운은 KPI가 말해 준 결과를 해석하는 출발점이 됩니다. 첫 화면에서 바로 모든 세부 차트를 펼치는 대신, 필요할 때만 원인 분석으로 내려가게 만드는 편이 읽기 쉽습니다.
