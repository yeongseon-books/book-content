...
Data Warehouse 설계는 운영 시스템 분리(OLTP)와 분석 시스템 분리(OLAP)를 기준으로 나뉩니다. 입력과 처리의 경계, 검증과 운영 신호의 경계를 명확히 하면 각 시스템을 독립적으로 최적화할 수 있습니다.
...
위 표에서 중요한 것은 어느 쪽이 더 고급인지가 아니라, 실패 비용이 완전히 다르다는 점입니다. 운영 DB의 실패는 고객 결제 실패로 이어지고, 분석 DB의 실패는 리포트 지연으로 이어집니다. 둘 다 중요하지만 대응 방식이 다르기 때문에 저장소를 분리하는 것이 합리적입니다.
