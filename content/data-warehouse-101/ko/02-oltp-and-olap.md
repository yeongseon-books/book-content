...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 OLTP와 OLAP는 어떤 워크로드를 다르게 처리할, 둘째 행 저장과 열 저장은 어느 쿼리에서 차이가 커질, 셋째 하나의 엔진으로 두 요구를 함께 처리하면 왜 곤란할입니다. 이 글에서 배울 것에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **OLTP와 OLAP는 어떤 워크로드를 다르게 처리할까요?**
  - - OLTP와 OLAP의 워크로드 차이 - 행 저장과 열 저장의 트레이드오프 - 두 시스템을 분리해야 하는 이유 - 비교 실습 5단계 - 입문 단계에서 자주 나오는 실수 5가지 OLTP는 지금 한 건을 빠르게 처리해야 하고, OLAP는 과거 전체를 한 번에 훑어야 합니다
- **행 저장과 열 저장은 어느 쿼리에서 차이가 커질까요?**
  - 행 저장은 한 건을 읽고 쓰는 비용이 낮습니다. 반면 열 저장은 특정 컬럼을 대량으로 읽는 비용이 낮습니다. 아래 예시는 동일한 비즈니스 질문에서도 어느 저장 구조가 유리한지 보여 줍니다.
- **하나의 엔진으로 두 요구를 함께 처리하면 왜 곤란할까요?**
  - - OLTP와 OLAP의 워크로드 차이 - 행 저장과 열 저장의 트레이드오프 - 두 시스템을 분리해야 하는 이유 - 비교 실습 5단계 - 입문 단계에서 자주 나오는 실수 5가지 OLTP는 지금 한 건을 빠르게 처리해야 하고, OLAP는 과거 전체를 한 번에 훑어야 합니다
...
- **Data Warehouse 101 (2/10): OLTP와 OLAP (현재 글)**
