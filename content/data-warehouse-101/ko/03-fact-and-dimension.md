...
Fact 테이블의 입력(이벤트)과 Dimension 테이블의 변경(속성 변경)이 명확히 분리되면 스키마 유지 비용이 획기적으로 낮아집니다. 이 글에서는 이 글에서 배울 것부터 실전 앵커: 모델, 파이프라인, 성능 검증까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.

## 처음 질문으로 돌아가기

- **Fact 테이블과 Dimension 테이블은 무엇을 다르게 담을까요?**
  - Fact와 Dimension을 분리하는 이유는 "정규화가 좋아서"가 아니라, 측정과 맥락의 변경 주기가 다르기 때문입니다.
- **측정값과 속성을 분리하면 어떤 이점이 생길까요?**
  - - Fact 테이블의 정의 - Dimension 테이블의 정의 - 둘을 분리했을 때 얻는 이점 - 모델링 실습 5단계 - 입문 단계에서 자주 나오는 실수 5가지 분석 질문은 대개 얼마나 많이 일어났는지와 어떤 기준으로 나눌 것인지로 읽힙니다
- **분석 모델에서 grain을 먼저 정해야 하는 이유는 무엇일까요?**
  - 팀 내 모델 리뷰에서 가장 먼저 확인해야 할 문장은 grain 선언입니다. 예를 들어 "fact_sales는 주문-상품 라인 단위 한 행"이라고 쓰면, 다음 질문들이 자동으로 정리됩니다.
...
- **Data Warehouse 101 (3/10): Fact와 Dimension (현재 글)**
