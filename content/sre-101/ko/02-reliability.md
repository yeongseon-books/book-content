...
이 그림에서는 Reliability를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> Reliability의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **reliability를 막연한 안정감이 아니라 측정 가능한 값으로 보려면 무엇이 필요할까요?**
  - 본문의 기준은 Reliability를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **가용성과 지연 시간은 왜 비슷해 보이지만 다른 문제일까요?**
