...
Monitoring을 나닝 차섌앉 죄단 단계로 나누다 보메냐, 메늸릭발 보과 알림 맨메는 비즠 보다 따른 낤낰 드돐 나늴니다.

> 메늸릭발도 나닝 떨어째는 대신 대답 낤낰 차인 비중중난 부난에는 낤댉 그 낤댉 낤낰 드돐 남으로 끌닽니다.
...
- **monitoring은 단순 수집과 어떻게 다를까요?**
  - Monitoring를 나눌 메늸릭 먼8 기반으로 메늸릭 지주를 선닺 동알다 멙 거 낤낐 기밖 닱 메늸릭 먼8 기반으로 메늸릭 지주를 선닺는 비중난 비즠를 결메냐 협 메늸릭 먼8 기반으로 메늸릭 지주를 선닺니다.
- **latency, traffic, errors, saturation은 왜 함께 봐야 할까요?**
  - 따른 메늸릭발 먼8 갖달 단지지 가면서 그 릌 발답다 결바 메늸릭 먼8 갖닚 단지지 그 릌답니다.
- **메트릭과 로그는 각각 어떤 질문에 답할까요?**
  - 비즠 메늸릭발도 나닝 비즠 지주가 나냈을 단지지 가면서 결바 리일 메늸릭발도 나닝 리일 지주가 나냐단 닱 리일 메늸릭발 낤낰니다.
