...
if should_proceed("canary", current_metrics):
    print("Pass: Canary 통과, pilot 단계로 진행")
else:
    print("Fail: Canary 실패, 롤백 필요")
```
...
장애가 발생했을 때 무엇이 잘못됐는지 알 수 없다면, 빠른 복구는 불가능합니다. 메트릭, 로그, 트레이스는 사후 도구가 아니라 설계 단계에서 결정해야 할 인터페이스입니다.
...
- [ ] operability는 왜 기능 뒤에 붙이는 옵션이 아니라 설계 요소인지 이해했다
- [ ] 관측성, 자동화, 안전한 변경, 회복력은 왜 함께 봐야 할 수 있다
- [ ] 운영 가능한 시스템을 점검할 때 어떤 질문부터 던져야 할 수 있다
- [ ] 왜 이 주제가 중요한가에서 가장 흔한 실수는 무엇인지 이해했다
- [ ] 한 문장으로 잡는 멘탈 모델을 실무에 적용할 때 주의할 점은 무엇인지 이해했다

---

## 정리

이 글은 sre-101 시리즈의 한 단계로, 핵심 개념을 실무 맥락에서 정리했습니다. 여기서 다룬 원칙들은 독립적으로도 유용하지만, 시리즈 전체와 연결될 때 더 큰 그림이 보입니다.

## 처음 질문으로 돌아가기

- **operability는 왜 기능 뒤에 붙이는 옵션이 아니라 설계 요소일까요?**
  - 본문에서 강조했듯이 운영 가능성은 코드를 다 짠 다음 모니터링·알람을 "끼워 넣는" 방식으로 얻어지지 않습니다. 어떤 신호를 노출할지, 어떤 변경이 안전한지, 실패 시 어떻게 회복할지를 인터페이스·데이터 모델·배포 구조 단계에서 결정해야만 비용 없이 운영할 수 있는 시스템이 만들어집니다.
- **관측성, 자동화, 안전한 변경, 회복력은 왜 함께 봐야 할까요?**
  - 본문 예시처럼 네 축은 서로를 보완하기 때문입니다. 관측성이 없으면 자동화가 무엇을 해야 할지 모르고, 안전한 변경(카나리·롤백)이 없으면 자동화가 사고를 더 빨리 퍼뜨리며, 회복력이 없으면 관측해도 손쓸 수가 없습니다. 한 축만 강해 봐야 다른 축의 약점이 전체 운영 비용을 결정하기 때문에 동시에 봐야 합니다.
- **운영 가능한 시스템을 점검할 때 어떤 질문부터 던져야 할까요?**
  - 본문에서 본 것처럼 "실패는 어디서, 어떤 신호로, 누가, 얼마 만에 인지하는가", "변경은 점진적으로 적용·롤백 가능한가", "사람이 새벽에 깨지 않아도 자동으로 회복되는 구간은 어디까지인가"를 먼저 묻습니다. 이 질문에 명확히 답할 수 없는 부분이 곧 다음 분기 운영 부담의 원천이라, 설계 단계에서 운영을 함께 고려하면 나중에 사람이 손수 해야 할 일이 훨씬 줄어듭니다.
