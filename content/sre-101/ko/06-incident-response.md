...
이 그림에서는 Incident Response를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.

> Incident Response의 핵심은 기능 이름이 아니라, 어떤 경계에서 무엇을 검증하고 어떤 신호를 남길지 정하는 데 있습니다.
...
- **장애 대응은 왜 개인 역량보다 팀 구조에 더 크게 좌우될까요?**
  - 본문의 기준은 Incident Response를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **심각도는 왜 느낌이 아니라 영향 기준으로 정의해야 할까요?**
