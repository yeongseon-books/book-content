...
- **환경 변수 분리**: API 키를 코드에 직접 적어두면 GitHub에 올릴 때 전 세계에 공개됩니다. 반드시 환경 변수로 처리하고, 로컬에서는 `.env` 파일을 사용하세요.
...
로컬 `.env` 파일에 있던 `OPENAI_API_KEY`를 Vercel 설정 화면에 입력해야 합니다.

*   **Key**: `OPENAI_API_KEY`
*   **Value**: `sk-xxxx...` (내 실제 키)
...
1.  `.env` 파일에 실제 API 키를 적고 로컬 개발을 진행합니다.
2.  `.gitignore` 파일에 반드시 `.env`를 추가하여 GitHub에 업로드되지 않게 막습니다.
3.  서버(Vercel, Azure)에는 각 플랫폼의 'Environment Variables' 메뉴를 통해 키값을 따로 입력합니다.
