...
`system`과 `user`를 함께 쓰는 가장 작은 예제를 실행해 보겠습니다. 다음 글의 프롬프트 엔지니어링으로 자연스럽게 이어지는 실습입니다.
...
로컬 실습 단계를 넘어서면, 모델 호출을 서비스 엔드포인트로 감싸야 합니다. 아래 예시는 가장 작은 FastAPI 백엔드에서 OpenAI 호출을 안전하게 처리하는 패턴입니다.
...
초기에는 문자열 하나로 충분해 보이지만, 요구사항이 늘어나면 프롬프트가 가장 먼저 복잡해집니다. 템플릿을 별도 파일로 분리하고 변수만 주입하는 패턴이 유지보수에 유리합니다.
...
실무에서는 성공보다 실패 경로가 더 중요합니다. 실패를 유형별로 나누고 사용자 메시지와 내부 로그를 분리해야 장애 대응 속도가 올라갑니다.
...
| 실수 | 증상 | 올바른 접근 |
| --- | --- | --- |
| API 키를 코드에 하드코딩 | 저장소 노출 → 무단 과금 | 환경변수 또는 시크릿 관리 서비스 사용 |
| `finish_reason` 무시 | 응답이 중간에 잘려도 인지 못함 | `finish_reason == "length"` 이면 `max_tokens` 재검토 |
| 토큰 사용량 미기록 | 비용 급증 시 원인 불명 | 모든 호출에서 `usage` 필드 저장 |
| 오류를 단일 `except Exception`으로 처리 | 인증·한도·네트워크 실패 구분 불가 | `RateLimitError`, `APIConnectionError`, `APIStatusError` 분리 |
| 응답을 문자열로만 취급 | `finish_reason`, `model` 버전 정보 손실 | 응답 객체 전체 구조 파싱 후 필요 필드 저장 |
| 타임아웃 미설정 | 네트워크 문제 시 무한 대기 | `OpenAI(timeout=20.0)` 으로 클라이언트 초기화 |
...
- [ ] 클라이언트 초기화 시 `timeout`을 명시했다.
- [ ] 실패 유형별로 사용자 메시지와 내부 로그를 분리했다.
...
- **ChatGPT 웹사이트를 쓰는 것과 AI API를 붙이는 것은 무엇이 다를까요?**
  - ChatGPT는 완성된 제품이고, AI API를 붙이는 것은 외부 모델 서비스를 내 기능 안으로 편입하는 개발 작업입니다. 인증, 요청 형식, 응답 파싱, 타임아웃, 비용 기록이 모두 개발자 책임이 됩니다.
- **OpenAI API를 호출하려면 어떤 준비가 필요할까요?**
  - 계정 가입, API 키 발급, 환경변수 주입, SDK 설치가 필요합니다. 비밀 키를 코드에 하드코딩하지 않는 습관이 가장 중요합니다.
- **첫 번째 요청은 어떤 형식으로 보내고, 어디서 응답을 읽어야 할까요?**
  - `messages` 배열에 `role`과 `content`를 담아 보내고, 응답에서는 `choices[0].message.content`, `finish_reason`, `usage`를 함께 읽어야 합니다.
