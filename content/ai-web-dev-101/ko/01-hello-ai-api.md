...
- 왜 첫 호출을 제대로 이해해야 할까에서 가장 흔한 실수는 무엇일까요?
- SDK 메서드보다 먼저 봐야 할 것: JSON 요청과 JSON 응답을 실무에 적용할 때 주의할 점은 무엇일까요?
- 가장 작은 성공 경로: SDK 설치와 첫 호출의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
## 운영 가능한 첫 호출로 바꾸는 체크리스트

첫 호출이 성공했다는 사실만으로는 운영 준비가 끝났다고 말하기 어렵습니다. 로컬에서 한 번 응답을 받는 것과, 서비스 트래픽 안에서 안정적으로 호출하는 것은 전혀 다른 문제이기 때문입니다. 최소한 요청 식별자, 시간 초과, 재시도 기준, 비용 로그는 초반부터 함께 붙여 두어야 이후 기능 확장에서 품질이 흔들리지 않습니다.

다음 예시는 가장 작은 FastAPI 백엔드에서 OpenAI 호출을 감싸는 형태입니다.
...
def ask(req: AskRequest):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "당신은 한국어 기술 문서 도우미입니다."},
...
핵심은 모델 호출을 그대로 노출하지 않고, 서비스 경계에서 실패 형태를 통제하는 것입니다. 나중에 모델을 바꾸거나 파라미터를 조정해도 API 계약이 흔들리지 않도록 응답 스키마를 먼저 고정해 두는 편이 안전합니다.

## 프롬프트 템플릿을 코드 바깥으로 분리하기

초기에는 문자열 하나로 충분해 보이지만, 요구사항이 늘어나면 프롬프트가 가장 먼저 복잡해집니다. 특히 톤, 금지 항목, 출력 형식이 동시에 늘어나면 코드 리뷰가 어려워집니다. 그래서 템플릿을 별도 파일로 분리하고 변수만 주입하는 패턴이 유지보수에 유리합니다.
...
`prompts/answer_v1.txt` 예시는 다음처럼 작성할 수 있습니다.

```text
당신은 {product_name}의 고객지원 도우미입니다.
답변은 항상 한국어로 작성합니다.
근거가 없으면 추측하지 말고 모른다고 말합니다.
다음 주제는 답변하지 않습니다: {forbidden_topics}
출력 형식:
1) 한 줄 요약
2) 핵심 설명 3개 이하
3) 필요한 다음 행동
...
이 방식은 프롬프트 변경 이력을 Git diff에서 명확히 보여 줍니다. 운영 이슈가 생겼을 때 코드 변경인지 프롬프트 변경인지 원인 분리가 빨라집니다.

## 실패 시나리오별 대응 표준

실무에서는 성공보다 실패 경로가 더 중요합니다. 아래처럼 실패를 유형별로 나누고 사용자 메시지와 내부 로그를 분리해야 장애 대응 속도가 올라갑니다.

| 실패 유형 | 사용자 응답 | 내부 로그 필드 | 재시도 전략 |
| --- | --- | --- | --- |
| 인증 실패(401) | 잠시 후 다시 시도해 달라고 안내 | `error_type=auth`, `key_version` | 즉시 중단 |
| 요청 제한(429) | 트래픽이 많다고 안내 | `error_type=rate_limit`, `retry_after` | 지수 백오프 1~3회 |
| 시간 초과 | 응답 지연 안내 | `error_type=timeout`, `timeout_sec` | 짧은 질문으로 1회 재시도 |
| 모델 내부 오류(5xx) | 일시적 오류 안내 | `error_type=upstream_5xx`, `model` | 회로 차단기 기준 적용 |

이 표를 팀 운영 문서에 그대로 옮겨 두면, 온콜 담당자가 코드 없이도 동일한 판단을 반복할 수 있습니다.

## 비용 관측: 토큰과 지연 시간을 함께 본다

토큰 비용만 보면 사용자 체감 품질을 놓치고, 지연 시간만 보면 비용 폭증을 놓칩니다. 그래서 요청 단위로 두 지표를 묶어 저장하는 습관이 필요합니다.
...
초반에는 콘솔 출력만으로도 충분합니다. 다만 필드 이름을 일관되게 유지해야 Datadog, Grafana, CloudWatch로 옮길 때 추가 비용이 줄어듭니다.

## 운영 로그 샘플과 해석

초기 운영에서는 로그 한 줄이 가장 빠른 디버깅 도구입니다. 아래처럼 요청 ID를 포함한 구조화 로그를 남기면 사용자 문의와 서버 기록을 쉽게 연결할 수 있습니다.
...
이 예시에서 핵심은 `status=timeout`이 연속되는 시간대를 비용 그래프와 함께 보는 것입니다. 비용이 늘지 않는데 지연만 증가한다면 네트워크 또는 공급자 이슈일 가능성이 큽니다. 반대로 지연과 토큰이 동시에 증가하면 프롬프트 길이 팽창을 먼저 의심해야 합니다.
...
이제 모델을 한 번 투명하게 호출해 본 만큼, 다음 글에서는 같은 모델이라도 왜 프롬프트 설계에 따라 결과가 크게 달라지는지 봅니다.
...
- **왜 첫 호출을 제대로 이해해야 할까에서 가장 흔한 실수는 무엇일까요?**
  - 입문 단계에서는 모델 성능보다 서비스 경계가 더 먼저 헷갈립니다. 채팅 UI는 이 경계를 감춰 주지만, 런타임에서 실제로 일어나는 일은 단순합니다. 애플리케이션이 인증 헤더와 JSON 본문을 담아 HTTP 요청을 보내고, 모델 서비스가 JSON 응답을 돌려줍니다.
- **SDK 메서드보다 먼저 봐야 할 것: JSON 요청과 JSON 응답을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - OpenAI Python SDK는 편리하지만, 실제 계약을 바꾸지는 않습니다. `client.chat.completions.create()`는 결국 JSON 요청을 만들고 JSON 응답을 Python 객체로 감싼 결과를 돌려줍니다.
- **가장 작은 성공 경로: SDK 설치와 첫 호출의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 먼저 Python 가상환경과 SDK를 준비합니다.
...
- **AI Web Development 101 (1/7): AI API 첫 걸음 — OpenAI API로 첫 번째 요청 보내기 (현재 글)**
