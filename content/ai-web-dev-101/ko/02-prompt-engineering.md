...
원하는 출력을 안정적으로 받기 위해 프롬프트를 어떻게 구조화해야 할까요?

이 글은 그 질문에 답하기 위해 프롬프트 엔지니어링의 핵심 개념과 실무 고려사항을 단계별로 살펴봅니다.

<!-- a-grade-intro:end -->

## 시니어 엔지니어는 이렇게 생각합니다

- **역할·맥락·과제·형식을 분리한다** — 한 문단에 섞지 말고 섹션을 나눠야 모델이 의도를 정확히 파싱합니다.
- **출력 형식을 먼저 고정한다** — JSON·표·마크다운 중 하나로 형식을 못 박아야 다운스트림 파싱이 안정됩니다.
- **예시는 1~3개면 충분하다** — few-shot은 너무 많으면 비용이 늘고 편향이 생기므로 대표성 있는 소수만 보여줍니다.
- **실패 케이스를 명시한다** — '모를 때는 모른다고 답하라'를 넣어야 환각이 줄어듭니다.
- **프롬프트는 코드처럼 버전 관리한다** — 변경 영향이 크므로 diff·테스트·릴리스 단위로 관리합니다.
