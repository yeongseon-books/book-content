...
> 프롬프트는 영리한 문장이 아니라 실행 가능한 계약입니다 — `system`은 오래 유지되는 규칙, `user`는 이번 작업의 입력이고, 이 분리가 애플리케이션 코드에서 모델 동작을 안정적으로 만드는 핵심입니다.
...
프롬프트 엔지니어링을 문장 센스로만 접근하면 팀 단위 협업에서 바로 막힙니다. 실무에서는 같은 기능을 여러 사람이 만지기 때문에, 템플릿을 명시적 계약으로 다뤄야 합니다.
...
역할, 목표, 제약, 출력 형식을 분리하면 모델 버전을 바꿔도 품질 편차를 줄일 수 있습니다.
...
프롬프트 품질을 팀으로 관리하려면 "좋아 보인다" 대신 공통 평가표가 필요합니다.
...
평가표가 프롬프트와 함께 버전 관리된다는 사실이 핵심입니다. 프롬프트를 바꾸면 평가표도 같이 바꾸고, 테스트 결과를 PR에 남기면 이후 회귀를 빠르게 찾을 수 있습니다.

## 프롬프트 버전 관리 규칙

프롬프트를 코드처럼 다루려면 버전 규칙이 필요합니다. `answer_v1`, `answer_v2`처럼 번호를 올리고, 변경 이유를 한 줄로 남기는 방식이 실무에서 가장 단순합니다.

- `v1 -> v2`: 출력 JSON 필드 `confidence` 추가
- `v2 -> v3`: 금지 주제 응답 문구를 정책 문구로 통일
- `v3 -> v4`: RAG 근거 인용 형식 `source_ids` 배열로 고정

이 변경 이력은 나중에 평가 점수 하락 원인을 찾을 때 매우 큰 도움을 줍니다.

## 프롬프트 품질 회귀 테스트

배포 전에 같은 입력 세트로 점수를 비교하는 스크립트가 있으면 프롬프트 변경의 품질 하락을 막을 수 있습니다.
...
def check_case(answer: str, case: dict) -> bool:
...
| 실수 | 증상 | 올바른 접근 |
| --- | --- | --- |
| `system` 없이 `user`만 사용 | 매 호출마다 역할과 어조가 달라짐 | `system`에 장기 규칙, `user`에 현재 작업 분리 |
| 출력 형식 미지정 | JSON 파싱 실패, 후처리 불안정 | `반드시 JSON 객체 하나만 출력하라` 명시 |
| 금지 규칙 없이 "하지 마"만 적음 | 금지 항목을 우회 표현으로 답변 | `대신 이렇게 답하라`는 대체 동작 지정 |
| `temperature=1.0` 고정 사용 | 분류/추출 작업에서 결과 불안정 | 작업 성격에 맞게 0.0~0.3 사용 |
| 프롬프트를 코드 문자열로 관리 | 변경 이력 추적 불가 | 별도 파일로 분리 후 Git 버전 관리 |
| 같은 프롬프트를 모든 작업에 적용 | 분류 작업과 창작 작업이 같은 품질로 처리됨 | 작업 유형별 프롬프트 템플릿 분리 |
...
- **프롬프트는 단순한 질문과 무엇이 다를까요?**
  - 프롬프트는 역할, 맥락, 출력 형식, 제약 조건을 포함하는 실행 계약입니다. 단순 질문은 맥락이 없어 모델이 일반론으로 흐르게 됩니다.
- **`system`과 `user` 역할은 각각 어떤 책임을 가질까요?**
  - `system`은 장기 유지 규칙(역할, 어조, 금지 사항)을, `user`는 현재 실행할 작업과 입력값을 담습니다.
- **좋은 프롬프트는 어떤 정보를 빠뜨리지 않을까요?**
  - 역할, 맥락(독자·도메인·금지사항), 출력 형식, 제약 조건과 실패 기준을 모두 포함해야 합니다.
