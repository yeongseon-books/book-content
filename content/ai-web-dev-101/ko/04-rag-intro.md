...
내 데이터를 모델에 안전하게 결합해 정확한 답을 얻으려면 RAG를 어떻게 설계해야 할까요?

이 글은 그 질문에 답하기 위해 RAG 입문의 핵심 개념과 실무 고려사항을 단계별로 살펴봅니다.
...
- **검색 품질이 답변 품질의 상한이다** — 벡터 검색이 잘못된 문서를 가져오면 어떤 모델도 좋은 답을 줄 수 없습니다.
- **청크 전략은 도메인에 맞춘다** — 크기·오버랩·메타데이터를 도메인 특성에 맞게 조정해야 의미가 보존됩니다.
- **출처를 항상 함께 보여준다** — 근거 링크 없이 답만 주면 신뢰가 무너지므로 인용을 기본 UI로 둡니다.
- **키워드 검색을 함께 쓴다** — BM25 같은 lexical 검색과 결합한 하이브리드가 단일 벡터 검색보다 안정적입니다.
- **재인덱싱 파이프라인을 먼저 짠다** — 데이터가 바뀔 때 인덱스를 갱신할 수 없다면 RAG는 곧 낡은 답을 줍니다.
