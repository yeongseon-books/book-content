...
ChatGPT에게 우리 회사 매뉴얼이나 어제 나온 뉴스를 물어보면 "잘 모르겠습니다"라고 답합니다. 자연스러운 결과입니다. 그 정보들은 모델이 학습할 때 세상에 없었거나, 학습 데이터에 포함되지 않았기 때문입니다.

그런데 '학습'을 다시 시키지 않고도 AI에게 새로운 정보를 알려줄 방법이 있다면 어떨까요? "여기 참고서가 있으니 이걸 읽고 질문에 답해 달라"고 부탁하는 방식입니다. 이것이 바로 **RAG(Retrieval-Augmented Generation, 검색 증강 생성)**의 핵심입니다.

이 글은 AI 웹 개발 입문 시리즈의 4번째 글입니다. 여기서는 모델이 모르는 최신 정보와 내부 문서를 답변에 연결하는 RAG의 기본 구조를 설명합니다.
...
여러분이 아주 똑똑한 대학생이라고 해봅시다. 머릿속에 지식은 많지만, 특정 회사의 내부 보안 규정까지 다 외울 수는 없습니다. 이때 누군가 규정을 묻는다면 어떻게 할까요?
...
공무원 시험을 위해 전공 서적을 통째로 외우는 것이 파인튜닝이라면, 오픈북 테스트를 보는 방식이 RAG입니다. 보통 후자가 더 빠르고 정확합니다.

---

## 임베딩(Embedding)이란?

컴퓨터는 텍스트를 그대로 이해하지 못합니다. 숫자만 다룰 수 있으므로 텍스트를 숫자의 나열, 즉 벡터로 바꿔야 합니다.
...
- **프롬프트 인젝션**: 검색된 문서 안에 "앞선 지시를 무시하라" 같은 악성 문장이 섞여 있을 수 있습니다. 검색 결과는 어디까지나 참고 자료로 다루고, 실행 명령으로 믿지 않도록 프롬프트에 경계를 명확히 적어두세요.
...
RAG는 이제 AI 웹 서비스의 표준적인 접근으로 자리 잡았습니다. 거창한 데이터셋이 없어도 텍스트 파일 몇 개만으로 실용적인 챗봇을 만들 수 있기 때문입니다.

다음 글에서는 이 RAG 시스템을 더 고도화해 수백 페이지의 PDF를 읽고 분석하는 서비스를 만드는 방법을 살펴보겠습니다.
