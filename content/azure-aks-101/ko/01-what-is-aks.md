...
이 그림에서는 Azure Kubernetes Service란? — 직접 운영하지 않아도 되는 Kubernetes를 운영 흐름 안에서 어디에 배치해야 하는지 봅니다. 핵심은 개념을 따로 외우는 것이 아니라 입력, 처리, 검증, 운영 신호가 어떤 경계로 이어지는지 확인하는 데 있습니다.
