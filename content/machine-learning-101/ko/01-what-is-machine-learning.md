...
다음 글에서는 지도학습과 비지도학습을 비교하면서, 레이블이 있을 때와 없을 때 어떤 식으로 문제를 나눠 생각해야 하는지 봅니다.

## 처음 질문으로 돌아가기

- **머신러닝은 정확히 무엇을 학습한다고 봐야 할까요?**
  - 이 글에서 머신러닝은 데이터에서 함수를 추정해 새로운 입력에 예측을 내놓는 과정입니다. `load_iris`, `LogisticRegression`, `fit`, `predict` 예제로 보면 모델이 외운 규칙집을 손으로 쓰는 대신 `X`와 `y`에서 결정 경계를 학습한다는 점이 분명해집니다.
- **일반화는 왜 훈련 성능과 다른 개념일까요?**
  - 본문에서 `model.score(X, y)`로 확인한 값은 훈련 데이터 점수라서, 이미 본 샘플을 얼마나 잘 맞추는지만 보여 줍니다. 일반화는 이 점수와 별개로 보지 못한 데이터에서도 같은 성능이 유지되는지 묻는 개념이며, 그래서 분리된 데이터와 교차검증을 함께 써야 합니다.
- **통계, 규칙 기반 코드, 머신러닝은 어디서 갈릴까요?**
  - 규칙 기반 코드는 `if-else`처럼 사람이 규칙을 직접 적는 방식이고, 머신러닝은 같은 자리를 데이터에서 학습한 함수로 채웁니다. 통계는 분포와 관계를 설명하는 데 강하고, 머신러닝은 그 지식을 예측 파이프라인으로 연결해 추천·사기 탐지·수요 예측처럼 새 입력에 답을 내는 데 더 초점을 둡니다.

<!-- toc:begin -->
## 시리즈 목차

- **Machine Learning 101 (1/10): Machine Learning이란 무엇인가? (현재 글)**
