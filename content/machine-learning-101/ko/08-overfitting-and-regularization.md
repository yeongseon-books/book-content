...
- **과적합과 과소적합은 어떤 신호로 구분할까요?**
  - 과적합은 훈련 점수는 높지만 테스트 점수가 크게 떨어질 때 드러나고, 과소적합은 둘 다 낮을 때 보입니다. 본문에서도 train-test 간격과 학습 곡선을 먼저 보라고 했듯이, 점수의 절대값보다 두 곡선이 어떻게 벌어지는지가 핵심 신호입니다.
- **편향-분산 트레이드오프는 무엇을 뜻할까요?**
  - 편향은 모델이 너무 단순해서 놓치는 구조이고, 분산은 데이터 변화에 따라 모델이 지나치게 흔들리는 정도입니다. 그래서 글에서 말한 정규화와 데이터 추가는 분산을 줄이는 방향이고, 반대로 과소적합일 때는 모델 용량이나 피처 표현을 늘려 편향을 줄여야 합니다.
- **Ridge, Lasso, ElasticNet은 어떻게 다를까요?**
  - `Ridge(alpha=1.0)`는 모든 계수를 부드럽게 줄이는 L2 규제이고, `Lasso(alpha=0.01)`는 일부 계수를 0으로 만들어 피처 선택 효과까지 냅니다. ElasticNet은 둘을 섞은 방식이라서, 상관된 피처가 많아 Lasso 선택이 불안정할 때 더 실용적인 절충안이 됩니다.
...
- **Machine Learning 101 (8/10): 과적합과 정규화 (현재 글)**
