...
**핵심 질문**: *훈련 점수 99%, 테스트 점수 60%* — *모델 탓일까요, 데이터 탓일까요*?

> *Overfitting 은 *훈련 데이터의 노이즈* 까지 외운 결과이고, *Regularization* 이 *모델의 자유도* 를 줄여 *일반화* 를 회복합니다.*
...
- *학습 곡선* 이 *진단의 시작*.
- *데이터 추가* 가 *최강 정규화*.
- *Dropout/Augmentation* 도 *정규화의 한 형태*.
- *RidgeCV* 로 *alpha 자동* 결정.
- *과소적합* 은 *모델 키워야* 해결.
...
1. *PolynomialFeatures(degree=10)* + *Ridge* 로 *과적합* 을 재현하세요.
2. *RidgeCV* 와 *수동 alpha* 결과를 비교하세요.
3. *Lasso* 가 *0 으로 만든 피처* 를 나열하세요.
