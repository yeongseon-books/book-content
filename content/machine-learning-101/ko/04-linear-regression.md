...
- **선형 회귀 식은 어떤 방식으로 예측값을 만들까요?**
  - 선형 회귀는 본문 첫 수식처럼 `y_hat = Xw + b`로 각 피처에 가중치 `w`를 곱하고 절편 `b`를 더해 예측값을 만듭니다. Numpy 예제의 `X_b`와 `theta = np.linalg.inv(X_b.T @ X_b) @ X_b.T @ y`는 이 직선을 데이터에 가장 잘 맞추는 계수를 직접 구하는 과정입니다.
- **평균제곱오차와 최소제곱 해는 무엇을 최소화할까요?**
  - 평균제곱오차는 실제값 `y`와 예측값 `y_hat`의 차이를 제곱해 평균낸 값이라서 큰 오차에 더 큰 페널티를 줍니다. 최소제곱 해는 이 MSE가 가장 작아지는 `w`와 `b`를 찾는 해이며, 그래서 이상치가 있을 때 점수가 크게 흔들릴 수 있다는 경고가 함께 붙습니다.
- **R-squared는 정확히 무엇을 설명할까요?**
  - `R^2`는 모델이 타깃 분산 중 얼마를 설명했는지 보여 주는 비율입니다. 그래서 `r2_score(yte, pred)`가 낮다면 단순히 점수가 낮다는 뜻을 넘어서, 직선 하나로는 설명되지 않는 비선형 패턴이나 빠진 피처가 남아 있다는 신호로 읽어야 합니다.
