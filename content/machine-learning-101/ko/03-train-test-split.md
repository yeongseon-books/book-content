...
다음 글에서는 지도학습의 가장 기본적인 모델인 Linear Regression을 봅니다.

## 처음 질문으로 돌아가기

- **훈련 세트, 검증 세트, 테스트 세트는 각각 무엇을 맡을까요?**
  - 훈련 세트는 `LogisticRegression(max_iter=1000).fit(Xtr, ytr)`처럼 모델 파라미터를 학습하는 데 쓰입니다. 검증 세트는 하이퍼파라미터와 임계값을 고르는 중간 점검용이고, 테스트 세트는 마지막에 한 번만 열어 일반화 성능을 확인하는 최종 홀드아웃입니다.
- **`random_state`를 왜 항상 고정하라고 할까요?**
  - 본문에서 `train_test_split(..., random_state=42)`를 고정한 이유는 같은 데이터 분할을 다시 만들기 위해서입니다. 시드를 고정하지 않으면 실행마다 train/test 구성이 바뀌어 점수 차이가 모델 개선인지 우연한 분할 차이인지 구분하기 어려워집니다.
- **`stratify`는 클래스 불균형에서 어떤 도움을 줄까요?**
  - `stratify=y`를 넣으면 train과 test가 원래 데이터의 클래스 비율을 비슷하게 유지합니다. 특히 양성 비율이 작은 문제에서는 한쪽 분할에 양성이 거의 사라지는 일을 막아 주므로, 테스트 점수가 실제 운영 분포를 더 잘 반영하게 됩니다.
...
- **Machine Learning 101 (3/10): 훈련/테스트 분할 (현재 글)**
