...
- **결정 트리는 피처 공간을 어떤 기준으로 나눌까요?**
  - 결정 트리는 하나의 피처와 임계값을 골라 데이터를 둘로 나누는 분할을 반복하면서 피처 공간을 잘게 쪼갭니다. 본문 예제의 `DecisionTreeClassifier(max_depth=4)`는 이런 분할을 제한된 깊이 안에서 쌓아 예측 규칙을 만드는 방식입니다.
- **Gini와 entropy는 무엇을 측정할까요?**
  - Gini와 entropy는 노드 안에 클래스가 얼마나 섞여 있는지, 즉 불순도를 재는 기준입니다. 트리는 이 값이 가장 크게 줄어드는 분할을 선호하므로, 두 지표 모두 "한 번 나누었을 때 레이블이 얼마나 더 깔끔해지는가"를 수치로 표현한다고 보면 됩니다.
- **단일 트리는 왜 쉽게 과적합될까요?**
  - 단일 트리는 깊이를 풀어 두면 훈련 데이터를 거의 그대로 외울 정도로 세밀한 규칙을 만들 수 있습니다. 그래서 본문에서도 `max_depth`를 가장 중요한 손잡이로 강조했고, 여러 트리를 평균내는 `RandomForestClassifier(n_estimators=200)`가 보통 더 안정적인 테스트 점수를 냅니다.
