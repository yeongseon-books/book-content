...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 분류에서는 어떤 지표를 언제 써야 하고, 정확도만으로는 왜 부족할, 둘째 회귀에서는 MAE, MSE, RMSE, R-squared를 어떤 기준으로 골라야 할, 셋째 혼동 행렬은 어떤 구조를 보여 주고, 어떻게 읽어야 할입니다. 적용 전과 후에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **분류에서는 어떤 지표를 언제 써야 할까요?**
  - 분류에서는 어떤 지표를 언제 써야 할까요 — 본문에서 단계별로 설명합니다.
- **회귀에서는 MAE, MSE, RMSE, R-squared를 어떻게 나눠 읽을까요?**
  - 회귀에서는 MAE, MSE, RMSE, R-squared를 어떻게 나눠 읽을까요 — 본문에서 단계별로 설명합니다.
- **혼동 행렬은 어떤 구조를 보여 줄까요?**
  - 혼동 행렬은 어떤 구조를 보여 줄까요 — 본문에서 단계별로 설명합니다.
