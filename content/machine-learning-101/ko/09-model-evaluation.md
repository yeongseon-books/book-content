...
- **분류에서는 어떤 지표를 언제 써야 할까요?**
  - 분류에서는 정확도 하나로 끝내지 않고 `classification_report`, `roc_auc_score`, `average_precision_score`를 문제 성격에 맞게 골라야 합니다. 오탐 비용이 크면 정밀도, 미탐 비용이 크면 재현율을 더 중시하고, 클래스 불균형이 크면 ROC-AUC보다 PR-AUC가 더 솔직한 신호가 될 수 있습니다.
- **회귀에서는 MAE, MSE, RMSE, R-squared를 어떻게 나눠 읽을까요?**
  - 본문의 장난감 예제처럼 MAE는 평균적인 절대 오차 크기를, MSE와 RMSE는 큰 오차에 더 민감한 손실을 보여 줍니다. `R^2`는 오차 단위가 아니라 설명된 분산의 비율이므로, 회귀 평가는 보통 MAE나 RMSE 같은 오차 지표와 함께 읽어야 균형이 맞습니다.
- **혼동 행렬은 어떤 구조를 보여 줄까요?**
  - 혼동 행렬은 `TP`, `FP`, `FN`, `TN` 네 칸으로 어떤 종류의 실패가 얼마나 나왔는지 바로 보여 줍니다. 그래서 이 글에서도 단순 점수보다 "지금 놓치는 쪽이 많은지, 잘못 경보를 울리는 쪽이 많은지"를 먼저 읽어야 임계값과 운영 정책을 제대로 조정할 수 있다고 설명했습니다.
