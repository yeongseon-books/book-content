...
이 글은 머신러닝 101 시리즈의 10번째 글입니다. 여기서는 문제 정의부터 데이터, 피처, 모델, 평가, 배포, 모니터링까지 이어지는 7단계 워크플로를 정리하고, `Pipeline`이 왜 유지보수성과 재현성의 중심인지 봅니다.
...
- 모델 재학습 주기는 어떤 기준으로 정해야 할까요?
- 배포 후 성능이 떨어지는 원인을 어떻게 진단할까요?
...
- **정확도가 높아도 왜 많은 ML 프로젝트는 배포에 실패할까요?**
  - 이 글은 실패 원인을 모델 점수보다 앞뒤 단계에서 찾습니다. 노트북에서 `pipe.score(Xte, yte)`가 높아도 문제 정의가 약하거나, 저장한 `model.joblib`가 재현되지 않거나, 배포 후 drift를 보지 않으면 실제 서비스에서는 금방 무너질 수 있습니다.
- **문제 정의, 데이터, 모델, 배포, 모니터링을 왜 하나의 루프로 봐야 할까요?**
  - 본문은 문제 정의부터 데이터, 피처, 모델, 평가, 배포, 모니터링까지 7단계를 한 번의 직선이 아니라 반복되는 루프로 설명했습니다. 마지막 단계의 drift 신호가 다시 데이터 수집과 모델 재학습 기준을 바꾸기 때문에, 앞단과 뒷단을 분리하면 운영 중 품질 저하를 제때 되돌릴 수 없습니다.
- **`Pipeline`은 왜 전처리 누수를 막는 핵심 도구일까요?**
  - `Pipeline([("scaler", StandardScaler()), ("clf", LogisticRegression(...))])`처럼 전처리와 모델을 한 객체로 묶으면 학습 때 쓴 변환 순서가 추론 때도 그대로 재현됩니다. 그래서 스케일링이 노트북 셀마다 따로 흩어져 생기는 누수를 막고, `joblib.dump`와 재로딩까지 포함한 배포 경로를 더 안정적으로 만들 수 있습니다.
