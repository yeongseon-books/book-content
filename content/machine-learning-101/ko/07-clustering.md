...
다음 글에서는 Overfitting과 Regularization을 통해 모델이 잡음을 외우는 문제와 이를 제어하는 방법을 봅니다.

## 처음 질문으로 돌아가기

- **정답 레이블이 없는데 군집이 좋은지 어떻게 판단할까요?**
  - 이 글에서는 KMeans의 `inertia_`와 `silhouette_score(X, km.labels_)`를 함께 보고, 숫자 하나로 끝내지 않았습니다. 군집화는 정답이 없기 때문에 응집도와 분리도 지표를 본 뒤, 시각화와 도메인 해석으로 그 묶음이 실제 의미를 가지는지 다시 확인해야 합니다.
- **KMeans와 DBSCAN은 어떤 상황에서 다르게 써야 할까요?**
  - KMeans는 `n_clusters=3`처럼 군집 수를 미리 정하고 중심점에 가까운 샘플을 묶는 방식이라서 비교적 둥근 군집에 잘 맞습니다. DBSCAN은 `eps`, `min_samples`로 밀도를 보며 군집을 만들고 `-1` 노이즈도 따로 표시하므로, 이상치가 많거나 군집 모양이 불규칙할 때 더 자연스럽게 동작할 수 있습니다.
- **`K`는 어떤 기준으로 정해야 할까요?**
  - 본문은 `K=3`을 임의로 고정하지 않고 Elbow 곡선과 Silhouette 점수를 함께 보라고 권합니다. 둘이 다른 답을 가리키면 수학적 점수만 따르지 말고, 그 군집 수가 고객 세그먼트나 운영 정책으로 설명 가능한지도 같이 따져야 합니다.
...
- **Machine Learning 101 (7/10): 군집화 (현재 글)**
