...
- **0 또는 1을 예측하는데 왜 이름은 회귀일까요?**
  - 로지스틱 회귀는 클래스 레이블을 바로 찍기 전에 먼저 선형 점수를 계산하고, 그 값을 확률로 바꾸는 연속 함수 모델입니다. 그래서 최종 용도는 분류이지만 내부에서는 선형 결합과 확률 추정을 다루므로 이름에 회귀가 남아 있습니다.
- **시그모이드는 선형 점수를 어떻게 확률로 바꿀까요?**
  - 본문 수식 `σ(z) = 1 / (1 + e^{-z})`는 어떤 실수 `z`도 0과 1 사이 값으로 압축합니다. 그래서 `predict_proba(Xte)`를 보면 모델이 계산한 선형 점수가 클래스 1일 확률로 바뀌고, `z=0` 근처에서는 0.5를 경계로 예측이 갈립니다.
- **왜 0.5 임계값을 항상 정답처럼 쓰면 안 될까요?**
  - 글의 임계값 루프처럼 같은 확률이라도 `0.3`, `0.5`, `0.7` 중 어디를 기준으로 자르느냐에 따라 정밀도와 재현율이 달라집니다. 사기 탐지처럼 놓치면 손실이 큰 문제는 0.5보다 낮은 임계값이 더 적합할 수 있으므로, cutoff는 도메인 비용과 함께 정해야 합니다.
