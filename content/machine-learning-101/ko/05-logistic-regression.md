...
- Python 예제: predict_proba로 확률 확인에서 가장 흔한 실수는 무엇일까요?
- 로지스틱 vs 선형 회귀을 실무에 적용할 때 주의할 점은 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 0 또는 1을 예측하는데 왜 이름은 회귀일, 둘째 시그모이드는 선형 점수를 어떻게 확률로 바꿀, 셋째 왜 0.5 임계값을 항상 정답처럼 쓰면 안 될입니다. 시그모이드 함수의 직관에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **0 또는 1을 예측하는데 왜 이름은 회귀일까요?**
  - 0 또는 1을 예측하는데 왜 이름은 회귀일까요 — 본문에서 단계별로 설명합니다.
- **시그모이드는 선형 점수를 어떻게 확률로 바꿀까요?**
  - 시그모이드는 선형 점수를 어떻게 확률로 바꿀까요 — 본문에서 단계별로 설명합니다.
- **왜 0.5 임계값을 항상 정답처럼 쓰면 안 될까요?**
  - 왜 0.5 임계값을 항상 정답처럼 쓰면 안 될까요 — 본문에서 단계별로 설명합니다.
- **Python 예제: predict_proba로 확률 확인에서 가장 흔한 실수는 무엇일까요?**
  - ```python
from sklearn.datasets import load_breast_cancer
from sklearn.model_selection import train_test_split
...
- **로지스틱 vs 선형 회귀을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 로지스틱 vs 선형 회귀을 실무에 적용할 때 주의할 점은 무엇일까요 — 본문에서 단계별로 설명합니다.
