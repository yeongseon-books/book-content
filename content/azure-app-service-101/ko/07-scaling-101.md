...
- 수직 확장(scale up)과 수평 확장(scale out)은 어떤 신호와 비용 구조에서 차이가 날까요?
- 어떤 메트릭(metric)(CPU, queue, custom metric)을 기준으로 auto-scale 규칙을 잡아야 할까요?
- 인스턴스가 scale in될 때 ARR Affinity 세션은 어떻게 될까요?
- Premium의 always-ready 기능은 cold start를 얼마나 줄여 줄까요?
- 확장 상한(scale ceiling)과 비용 상한(cost ceiling)을 동시에 지키려면 어떻게 설계해야 할까요?
