...
- GoF의 23개 패턴은 왜 세 묶음으로 나뉠까에서 가장 흔한 실수는 무엇일까요?
- 코드 한 줄로 보는 패턴의 가치을 실무에 적용할 때 주의할 점은 무엇일까요?
- 패턴이 만드는 진짜 효과: 합의 속도의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 디자인 패턴은 결국 무엇을 가리키는 말일, 둘째 패턴 이름을 외우는 것과 패턴을 이해하는 것은 어떻게 다를, 셋째 같은 문제에 패턴을 쓰는 팀과 안 쓰는 팀의 차이는 어디에서 생길입니다. 디자인 패턴을 한 문장으로 다시 정의하기에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **Design Patterns 101 (1/10): 디자인 패턴이란 무엇인가? (현재 글)**
