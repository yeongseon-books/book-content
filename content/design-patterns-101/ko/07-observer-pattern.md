...
- Observer가 풀려는 진짜 문제: 발신자와 수신자의 결합 끊기에서 가장 흔한 실수는 무엇일까요?
- Python에서 Observer를 표현하는 세 가지 방식을 실무에 적용할 때 주의할 점은 무엇일까요?
- Observer가 조용히 만드는 세 가지 장애의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
- [ ] 이벤트 기반 시스템에서 Observer가 쓰이는 예를 말할 수 있습니다.
- [ ] Python에서 Observer를 구현하는 방법을 보여줄 수 있습니다.

## 정리

이 글에서 다룬 핵심은 세 가지입니다. 첫째 Observer 패턴을 도입하면 정확히 어떤 결합이 끊어지고, 대신 어떤 비용이 생길, 둘째 동기 Observer와 비동기 이벤트 버스는 언제 갈라져야 할, 셋째 구독자가 예외를 던지거나 느려지면 발행자에게 무슨 일이 생길입니다. Observer가 풀려는 진짜 문제: 발신자와 수신자의 결합 끊기에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **동기 Observer와 비동기 이벤트 버스는 언제 갈라져야 할까요?**
  - 동기 Observer는 `publish()`가 모든 구독자를 순차 호출한 뒤에야 리턴합니다. 구독자가 10ms씩 걸리는 게 5개면 발행자는 50ms를 기다립니다. 이 지연이 허용 가능한지가 동기/비동기를 가르는 기준입니다.
- **구독자가 예외를 던지거나 느려지면 발행자에게 무슨 일이 생길까요?**
  - 구독자가 예외를 던지거나 느려지면 발행자에게 무슨 일이 생길까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **Observer가 풀려는 진짜 문제: 발신자와 수신자의 결합 끊기에서 가장 흔한 실수는 무엇일까요?**
  - 직접 호출은 가장 단순한 통신입니다. `A`가 `B.do()`를 부르면 끝입니다. 문제는 `A`가 `B`, `C`, `D`, `E`를 전부 알아야 할 때 시작됩니다. 수신자가 늘어날 때마다 발신자를 열어야 하고, 수신자 하나를 제거할 때도 발신자를 수정해야 합니다.
- **Python에서 Observer를 표현하는 세 가지 방식을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 15줄이면 동작하는 Observer입니다. 토픽별 콜백 리스트를 순회하며 호출합니다. 단순하지만 실무에서 이대로 쓰면 세 가지 문제가 생깁니다.
- **Observer가 조용히 만드는 세 가지 장애의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 동기 Observer에서 구독자 하나가 외부 API를 호출하며 3초를 기다리면, 발행자도 3초를 기다립니다.
