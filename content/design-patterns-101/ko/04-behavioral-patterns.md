...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 Command가 단순한 함수 호출과 다른 점은 무엇일, 둘째 State와 Strategy는 코드 모양이 거의 같은데, 왜 별도 패턴으로 분류될, 셋째 Python에서 Iterator 패턴을 명시적으로 구현할 일이 거의 없는 이유는 무엇일입니다. 객체 간 책임 분배가 어려운 이유에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
