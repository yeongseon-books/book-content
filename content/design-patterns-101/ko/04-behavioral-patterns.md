...
Behavioral 패턴은 런타임에 객체들이 어떻게 협업하는지를 다루고, 누가 시작하고 누가 응답하며 누가 상태를 들고 가는지를 깔끔하게 정리합니다. 이 글에서는 객체 간 책임 분배가 어려운 이유부터 패턴별 트레이드오프 정리까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
