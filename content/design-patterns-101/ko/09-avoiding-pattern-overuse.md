...
```python
# 예시 파일: 4_refactor_to_pattern.py
# 분기 5개가 비슷한 모양으로 자라면, 그제야 Strategy로 정리합니다
```
...
```python
# 예시 파일: 5_remove_pattern.py
# 사용처가 하나뿐이라면 Strategy/Factory를 다시 함수로 접습니다.
```
...
1. **요구사항보다 빠른 추상화.** 미래를 상상해서 짜는 코드.
...
좋은 라이브러리는 적은 패턴을 정확히 씁니다. requests, FastAPI, pytest를 보면 거대한 추상화 대신 작은 합성으로 풉니다. 신입과 시니어의 차이는 패턴을 얼마나 많이 아는가가 아니라 언제 미루는지를 아는가입니다.
...
- [ ] 변화가 세 번 이상 반복되었는가?
- [ ] 함수 분리로 충분하지 않은가?
- [ ] 패턴 이름이 역할을 정확히 가리키는가?
- [ ] 사용처가 하나뿐이라면 다시 접을 수 있는가?

## 정리 및 다음 단계

패턴은 어휘입니다. 마지막 글에서는 Python의 일급 함수, 모듈, Protocol 같은 도구가 GoF의 많은 패턴을 어떻게 자연스럽게 풀어내는지 — Pythonic 패턴 — 을 봅니다.
