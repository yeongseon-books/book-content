...
패턴은 자신이 추가하는 복잡도보다 더 큰 고통을 덜어 줄 때만 가치 있고, 모든 간접화 계층은 유연성·테스트성·명료함 중 하나로 비용을 지불해야 합니다. 이 글에서는 패턴이 문제를 부르기 시작하는 신호부터 패턴을 참는 것도 실력입니다까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
