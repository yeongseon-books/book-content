...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/185"
...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 상속 대신 합성을 쓰면 구체적으로 무엇이 달라질, 둘째 Decorator와 Proxy는 둘 다 "감싸는" 패턴인데, 언제 어느 쪽을 골라야 할, 셋째 Structural 패턴을 도입했을 때 잃는 것은 무엇일입니다. 객체를 묶을 때 생기는 두 가지 문제에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **상속 대신 합성을 쓰면 구체적으로 무엇이 달라질까요?**
  - 상속 대신 합성을 쓰면 구체적으로 무엇이 달라질까요 — 본문에서 구체적으로 다룹니다.
- **Decorator와 Proxy는 둘 다 "감싸는" 패턴인데, 언제 어느 쪽을 골라야 할까요?**
  - Proxy는 실제 객체와 **동일한 인터페이스**를 노출하면서, 그 앞에서 접근 제어, 캐시, 지연 로딩 같은 부가 책임을 수행합니다. Decorator와 비슷해 보이지만 의도가 다릅니다. Decorator는 "기능을 추가"하고, Proxy는 "접근을 제어"합니다.
- **Structural 패턴을 도입했을 때 잃는 것은 무엇일까요?**
  - 패턴은 공짜가 아닙니다. 도입할 때 잃는 것을 미리 알아야 합니다.
...
- **Design Patterns 101 (3/10): Structural 패턴 (현재 글)**
