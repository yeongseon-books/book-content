...
Structural 패턴은 클래스와 객체를 어떻게 조합하느냐에 관한 것이라, 시스템이 커져도 모든 연결을 다시 잇지 않아도 됩니다. 이 글에서는 객체를 묶을 때 생기는 두 가지 문제부터 각 패턴이 요구하는 비용까지 이 원칙을 구체적으로 살펴봤습니다. 핵심은 개념을 외우는 것이 아니라 실무에서 어떤 판단을 바꾸는지 이해하는 데 있습니다.
