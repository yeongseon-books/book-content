...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 모든 `if/elif` 분기가 Strategy 후보일까요, 아니면 특정 조건을 만족해야 할, 둘째 Python에서 Strategy를 클래스로 만드는 것과 함수로 만드는 것은 언제 갈라질, 셋째 Strategy를 런타임에 교체하면 어떤 운영상 이점이 생길입니다. 분기 폭발이 정말 Strategy 후보인지 가려내는 방법에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **모든 `if/elif` 분기가 Strategy 후보일까요, 아니면 특정 조건을 만족해야 할까요?**
  - 모든 `if/elif`가 Strategy 후보는 아닙니다. 저는 세 가지 조건을 동시에 만족하는지 먼저 확인합니다.
- **Python에서 Strategy를 클래스로 만드는 것과 함수로 만드는 것은 언제 갈라질까요?**
  - 클래스 Strategy는 **상태를 가져야 할 때** 적합합니다. `BulkPricing`의 `threshold`나 `TieredPricing`의 `tiers`처럼 알고리즘 자체에 설정값이 필요한 경우입니다.
- **Strategy를 런타임에 교체하면 어떤 운영상 이점이 생길까요?**
  - 클래스 Strategy는 **상태를 가져야 할 때** 적합합니다. `BulkPricing`의 `threshold`나 `TieredPricing`의 `tiers`처럼 알고리즘 자체에 설정값이 필요한 경우입니다.
- **분기 폭발이 정말 Strategy 후보인지 가려내는 방법에서 가장 흔한 실수는 무엇일까요?**
  - 모든 `if/elif`가 Strategy 후보는 아닙니다. 저는 세 가지 조건을 동시에 만족하는지 먼저 확인합니다.
...
- **Design Patterns 101 (5/10): Strategy 패턴 (현재 글)**
