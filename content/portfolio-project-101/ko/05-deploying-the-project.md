...
예를 들어 Fly.io, Render, Railway 중 하나를 골라 배포 기준 플랫폼으로 정해 둘 수 있습니다.
...
예를 들어 `DATABASE_URL`, `SECRET_KEY` 같은 값은 코드가 아니라 배포 환경 변수로 분리해야 합니다.
...
예를 들어 `https://app.fly.dev/healthz` 같은 상태 확인 주소를 따로 정해 두면 점검이 쉬워집니다.
...
이 방식의 장점은 의존성 버전, OS, 런타임 차이를 모두 컨테이너로 격리한다는 사실입니다. "제 환경에서는 되는데요"라는 대화를 원천적으로 차단합니다.
...
- **포트폴리오 프로젝트에서 공개 URL이 왜 필수에 가까울까요?**
  - 초기 제품이나 개인 서비스도 작은 단계에서는 단순한 플랫폼을 많이 씁니다
- **호스팅 플랫폼은 화려함보다 어떤 기준으로 골라야 할까요?**
  - 호스팅 플랫폼은 화려함보다 어떤 기준으로 골라야 할까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **시크릿과 환경 변수는 왜 코드가 아니라 배포 환경에서 관리해야 할까요?**
  - 포트폴리오 배포의 핵심은 거대한 인프라가 아닙니다
  - 전자는 설명 중심 프로젝트입니다. 후자는 확인 중심 프로젝트입니다. 포트폴리오에서 강한 쪽은 거의 항상 후자입니다. 실제로 열어 볼 수 있기 때문입니다.
...
- **Portfolio Project 101 (5/10): 배포하기 (현재 글)**
