...
이 그림에서는 테스트와 문서가 코드 신뢰도를 어떻게 높이는지 붕니다. 테스트 없는 코드는 기능이고, 테스트 있는 코드는 사례입니다.
...
- **단위 테스트, 통합 테스트, 전체 흐름 테스트는 각각 무엇을 증명할까요?**
  - 필수는 아니지만, 있으면 급격히 신뢰도가 올라갑니다. 전체 커버리지 90% 이상보다는 핸심 로직 테스트 30%가 효씬 더 강한 신호입니다.
- **자동 검증은 왜 작은 포트폴리오에서도 중요한 기준이 될까요?**
  - 가장 중요한 비즈니스 로직부터 시작하세요. 사용자가 느낄 수 있는 핸심 기능 몇 개만 테스트해도 'CI/CD 파이프라인이 있는 프로젝트'라는 신호를 좵니다.
- **API 문서나 변경 기록은 어떤 신뢰를 더해 줄까요?**
  - README는 진스, API 문서는 선택, 아키텍처 문서는 선택입니다만, 있으면 더 좋습니다. 처음 읽는 사람이 로컬에서 실행할 수 있을 정도면 충분합니다.
