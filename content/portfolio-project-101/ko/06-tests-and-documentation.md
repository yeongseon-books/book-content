...
예를 들어 `로그인 → 생성 → 삭제`처럼 핵심 사용자 흐름을 세 단계로 적어 두면 전체 흐름 테스트 범위가 선명해집니다.
...
문서는 최소한 아래 세 가지가 있으면 기본 틀이 잡힙니다.
...
다음 글에서는 이런 프로젝트에서 왜 그런 선택을 했는지 남기는 기술적 의사결정 기록을 다룹니다.
...
이 구성의 장점은 "검증 후 실행"을 자동화한다는 사실입니다. 테스트가 깨지면 서버가 떠도 배포 성공으로 간주하지 않게 만들 수 있습니다.
...
- **Portfolio Project 101 (6/10): 테스트와 문서화 (현재 글)**
