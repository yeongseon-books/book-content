...
**핵심 질문**: Python은 동적 타입 언어인데, 왜 타입을 명시하는 것이 좋을까요?

> 타입 힌트는 변수, 함수 인자, 반환값의 기대 타입을 코드에 명시하는 Python의 기능입니다. 런타임에는 강제되지 않지만, 정적 분석 도구와 IDE가 이를 활용하여 버그를 사전에 잡아줍니다. 이 글에서는 타입 힌트의 개념과 도입 이유, 기본 문법을 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 타입 힌트의 정의와 등장 배경 (PEP 484)
- 변수와 함수에 타입 힌트를 적용하는 기본 문법
- 타입 힌트가 런타임에 미치는 영향
- IDE와 정적 분석 도구가 타입 힌트를 활용하는 방식
...
- **문서화 가치** — 타입 힌트는 가장 강력한 문서입니다.
- **점진 도입** — 공개 API부터 적용하는 편이 ROI가 큽니다.
- **런타임 무영향** — 힌트는 기본적으로 런타임 동작을 바꾸지 않습니다.
- **도구 결합** — mypy·pyright와 결합해야 가치가 살아납니다.
- **Public 우선** — 내부 코드보다 공개 API에 먼저 적용합니다.
...
1. 이름(str), 나이(int), 이메일(str)을 받아 소개 문자열을 반환하는 함수를 타입 힌트로 작성하세요.
2. 정수 리스트를 받아 평균(float)을 반환하는 함수를 타입 힌트로 작성하세요.
3. 타입 힌트가 있는 함수에 잘못된 타입을 전달했을 때 mypy가 어떤 오류를 출력하는지 확인하세요.
