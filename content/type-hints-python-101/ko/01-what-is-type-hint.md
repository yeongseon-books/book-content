...
- **타입 힌트는 정적 타입 언어의 타입 선언과 무엇이 다를까요?**
  - 팀 코드베이스에 타입 힌트를 도입할 때는 공개 API 함수, 도메인 모델, 경계 레이어 순으로 적용하면 리뷰 비용을 줄일 수 있습니다.
- **PEP 484가 해결하려던 문제는 무엇이었을까요?**
  - PEP 484가 해결하려던 문제는 무엇이었을까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **변수, 매개변수, 반환값에 어떤 문법으로 타입을 붙일까요?**
  - 팀 코드베이스에 타입 힌트를 도입할 때는 공개 API 함수, 도메인 모델, 경계 레이어 순으로 적용하면 리뷰 비용을 줄일 수 있습니다.
- **왜 이 주제가 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - Python은 유연합니다. 같은 변수에 문자열도 넣고, 리스트도 넣고, `None`도 넣을 수 있습니다.
- **바꾸기 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 타입 힌트가 없으면 함수 계약을 시그니처에서 읽을 수 없습니다.
- **단계별로 익히기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 기본 문법은 `매개변수: 타입`, `-> 반환타입`입니다. 중요한 점은 Python이 이 힌트를 실행 시점에 강제하지 않는다는 사실입니다.
...
- **Type Hints in Python 101 (1/10): Python type hint란 무엇인가? (현재 글)**
