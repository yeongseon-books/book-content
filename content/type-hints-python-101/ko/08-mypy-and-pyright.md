...
**핵심 질문**: 타입 힌트를 작성했는데 실제로 검증하지 않으면 주석과 다를 바가 없지 않을까요?

> 타입 힌트는 런타임에 강제되지 않습니다. 잘못된 타입을 전달해도 Python 인터프리터는 아무 경고 없이 실행합니다. 타입 힌트의 진정한 가치는 정적 분석 도구가 코드 실행 전에 오류를 잡아줄 때 발휘됩니다. 이 글에서는 대표적인 타입 검사 도구인 mypy와 pyright의 설정과 활용법을 다룹니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- mypy와 pyright의 차이와 선택 기준
- 프로젝트에 mypy를 설치하고 설정하는 방법
- strict 모드와 점진적 도입 전략
- CI 파이프라인에 타입 검사를 통합하는 방법
...
| 용어 | 설명 |
| --- | --- |
| mypy | Python 공식 타입 검사 도구 |
| pyright | Microsoft가 개발한 고속 타입 검사 도구 |
| strict 모드 | 모든 함수에 타입 힌트를 요구하는 엄격 설정 |
| stub 파일 | `.pyi` 확장자로 타입 정보만 제공하는 파일 |
| type: ignore | 특정 줄의 타입 오류를 무시하는 주석 |
...
시니어 엔지니어는 타입 검사 도구를 "있으면 좋은 것"이 아니라 테스트와 동급의 필수 인프라로 취급합니다. 새 프로젝트에서는 첫날부터 strict 모드를 적용하고, 기존 프로젝트에서는 새로 작성하는 코드부터 타입 힌트를 강제합니다.

도구 선택에서는 팀 전체가 하나의 도구를 기준으로 통일하는 것이 중요합니다. mypy와 pyright는 판단이 미묘하게 다른 경우가 있으므로, 두 도구를 모두 통과해야 한다고 요구하면 불필요한 마찰이 생깁니다. 하나를 CI 기준으로 정하고 나머지는 보조로 사용합니다.
...
1. 간단한 Python 파일을 작성하고 mypy를 실행하여 의도적으로 3가지 타입 오류를 발생시킨 뒤 수정하세요.

2. `pyproject.toml`에 mypy 설정을 추가하고, 테스트 디렉터리에는 느슨한 규칙을, 소스 디렉터리에는 strict 모드를 적용하세요.

3. GitHub Actions 워크플로우 파일을 작성하여 push 시 자동으로 mypy 검사가 실행되도록 설정하세요.
