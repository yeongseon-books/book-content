...
### 6단계: strict를 저장소 전체에 한 번에 켜지 않기

대형 저장소에서 가장 흔한 실패는 strict를 전체 루트에 바로 켠 뒤, 쏟아지는 오류를 팀이 감당하지 못하는 경우입니다. 더 현실적인 방법은 **기준선을 찍고, 핵심 디렉터리부터 단계적으로 올리는 것**입니다.
...
핵심은 `legacy`를 영구 예외 구역으로 두는 것이 아니라, **어느 디렉터리가 다음 strict 후보인지 매 분기 다시 정하는 것**입니다. 이렇게 해야 strict가 선언만 있고 실제로는 확장되지 않는 상태를 피할 수 있습니다.

### 7단계: `type: ignore`를 격리하고 이유를 남기기

실무에서는 서드파티 스텁 부족, 동적 메타프로그래밍, 프레임워크 한계 때문에 일부 무시 주석이 필요할 수 있습니다. 문제는 이유 없는 `type: ignore`가 쌓이면 "타입 시스템이 못 잡는 영역"이 아니라 "아무도 다시 보지 않는 영역"이 된다는 점입니다.

```python
from third_party_sdk import build_client


client = build_client()  # type: ignore[no-untyped-call]  # SDK v2에 스텁이 없음
```

이런 줄은 세 가지 기준으로 관리하는 편이 좋습니다.

- 오류 코드를 함께 적어 어떤 종류를 억제했는지 남깁니다.
- PR 설명에 왜 무시가 필요한지 한 문장으로 남깁니다.
- 분기별로 `warn_unused_ignores = true`를 켜 두고, 더 이상 필요 없는 무시 주석을 걷어냅니다.

### 8단계: false positive를 triage하는 순서 정하기

타입 오류가 생겼을 때 무조건 주석부터 붙이면 안 됩니다. 보통은 아래 순서가 가장 안전합니다.

1. 실제 버그인지 확인합니다.
2. 타입이 너무 넓은 시그니처인지 봅니다 (`Any`, `object`, 큰 Union).
3. 스텁이나 라이브러리 버전 문제인지 확인합니다.
4. 도구 한계가 맞다면 최소 범위의 ignore를 붙입니다.

특히 mypy와 pyright가 다르게 말할 때는 둘 중 하나가 틀렸다고 단정하기보다, **우리 팀의 표준 게이트가 어느 도구인지** 먼저 정하고 그 기준으로 해결하는 편이 운영 비용이 낮습니다.
...
- [mypy 문서 — 오류 코드와 ignore](https://mypy.readthedocs.io/en/stable/error_codes.html)
