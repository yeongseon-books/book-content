...
- **`int`, `str`, `float`, `bool`, `bytes`, `None`은 어떻게 적을까요?**
  - `int`, `str`, `float`, `bool`, `bytes`, `None`은 어떻게 적을까요 — 본문에서 구체적으로 다룹니다.
- **`list`, `dict`, `tuple`, `set`은 왜 원소 타입까지 붙여야 할까요?**
  - `list`, `dict`, `tuple`, `set`은 왜 원소 타입까지 붙여야 할까요 — 본문에서 구체적으로 다룹니다.
- **고정 길이 튜플과 가변 길이 튜플은 어떻게 다를까요?**
  - 고정 길이 튜플과 가변 길이 튜플은 어떻게 다를까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 주제가 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - 실무 Python 코드는 대부분 컬렉션과 함께 움직입니다. API 응답은 딕셔너리와 리스트로 오고, 설정은 중첩 딕셔너리로 들어오며, 쿼리 결과는 튜플 목록으로 다뤄집니다.
- **바꾸기 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 컨테이너의 내부 타입을 적는 순간, 분석기는 `item.name`, `item.price`, `result`의 키와 값까지 추적할 수 있습니다.
- **단계별로 익히기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - 복잡한 타입도 결국은 이런 기본 타입을 조합해서 표현합니다.
...
- **Type Hints in Python 101 (2/10): 기본 타입과 collection 타입 (현재 글)**
