...
- 한 모듈을 적용 전후로 단단하게 만들기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?

## 왜 이 주제가 중요한가

레거시 코드베이스는 대개 두 가지 문제를 함께 가집니다. 하나는 공개 함수 시그니처가 비어 있어 호출자가 계약을 알 수 없다는 점이고, 다른 하나는 `Any`가 한 군데에서 시작해 아래 코드 전체로 전염된다는 사실입니다.
...
여기서 중요한 변화는 `...` 같은 빈 본문을 두지 않고, 실제로 복사해 실행할 수 있는 코드를 제공한다는 사실입니다. 좋은 예시는 조언이 아니라 동작하는 기준선이어야 합니다.
...
- **타입 힌트는 어디에 먼저 붙여야 투자 대비 효과가 클까요?**
  - 이 글은 지역 변수보다 공개 함수 시그니처와 반환 타입을 먼저 단단하게 만들라고 답합니다. `find_user(user_id: int) -> UserRecord | None`, `create_order(self, payload: OrderPayload) -> OrderSummary`처럼 다른 모듈이 바로 읽는 계약부터 고정해야 리팩터링 효과가 가장 큽니다.
- **`Any`가 퍼진 코드는 어떤 순서로 줄여야 할까요?**
  - 본문에서는 `get_config() -> Any` 같은 경계 함수를 먼저 구체 타입으로 바꾸고, 이어서 `parse_order_id()`와 `require_user_email()` 같은 helper로 `Union`과 `None` 분기를 끌어올렸습니다. 이렇게 해야 `payload.get()` 중심의 느슨한 흐름이 `OrderPayload`, `OrderSummary`, 명시적 예외로 정리되며 `Any` 확산이 끊깁니다.
- **반환 타입, 공개 API, 내부 헬퍼 중 무엇을 먼저 단단하게 만들어야 할까요?**
  - 우선순위는 공개 API와 반환 타입이 먼저이고, 내부 헬퍼는 그 계약을 지키기 위해 뒤따르는 순서입니다. 이 글의 hardening pass도 시그니처 보강 → `Any` 제거 → `parse_order_id()`·`require_user_email()` 같은 helper 분리 → `mypy order_service.py` 통과 검증으로 마무리되었습니다.
