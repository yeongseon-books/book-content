...
# email: Optional[str] — 사용하기 전에 확인 필요
...
```python
# Python 3.10+: Union/Optional 대신 | 사용합니다.
...
- **반환값이 `None`일 수 있다는 사실을 타입에 어떻게 드러낼까요?**
  - 반환값이 `None`일 수 있다는 사실을 타입에 어떻게 드러낼까요 — 본문에서 구체적으로 다룹니다.
- **하나의 값이 여러 타입 중 하나일 수 있을 때 어떤 문법을 써야 할까요?**
  - 아래 예시는 타입 힌트가 문서가 아니라 검증 가능한 계약이라는 점을 분명하게 보여 줍니다.
- **Python 3.10+의 `X | Y` 문법은 언제 유용할까요?**
  - Python 3.10+의 `X | Y` 문법은 언제 유용할까요 — 본문에서 구체적으로 다룹니다.
- **왜 이 주제가 중요한가에서 가장 흔한 실수는 무엇일까요?**
  - Python 런타임 오류 중에는 `None` 관련 예외가 매우 많습니다. 함수가 찾지 못한 값을 `None`으로 반환했는데 호출자는 항상 객체가 온다고 가정하고 메서드를 호출해 버리는 식입니다.
- **바꾸기 전과 후을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 여기서 중요한 점은 `None` 가능성을 숨기지 않는다는 사실입니다.
- **단계별로 익히기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - `Optional[str]`는 정확히 `str | None`입니다. 선택적 매개변수라는 말이 아니라, 값 자체가 비어 있을 수도 있다는 설명에 가깝습니다.
...
- **Type Hints in Python 101 (3/10): Optional과 Union (현재 글)**
