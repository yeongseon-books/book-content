...
**핵심 질문**: *컨테이너 가 사라지면* *데이터* 는 *어디로* 갈까요?

> *상태 는 *컨테이너 안* 이 아니라 *볼륨* 에 둡니다. *Volume*, *Bind mount*, *tmpfs* 의 *차이* 를 알면 *데이터 사고* 를 막습니다.*
...
- **Volume**: *Docker* 가 *관리* 하는 영속 저장소.
- **Bind mount**: *호스트 경로* 를 *직접* 마운트.
- **tmpfs**: *메모리* 위 *임시* 저장소.
- **Driver**: *NFS*, *EBS* 등 *외부 저장소* 연결.
- **Mount propagation**: *마운트 전파* 모드.
...
1. *Volume* 과 *Bind mount* 의 *차이* 한 줄로.
2. *tmpfs* 의 *적합한 사용 사례* 한 가지.
3. *DB 데이터* 가 *컨테이너 내부* 에 있을 때의 *위험* 한 줄로.
