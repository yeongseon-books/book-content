...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/244"
...
이 글은 Containers 101 시리즈의 2번째 글입니다.
...
- 빠른 검증과 장애 신호을 실무에 적용할 때 주의할 점은 무엇일까요?
...
- **컨테이너 이미지는 왜 굳이 여러 레이어로 나뉠까요?**
  - 이미지 메타데이터를 읽어서 구조를 확인합니다. 실무에서는 문제를 감으로 추측하기보다 `inspect` 출력부터 보는 습관이 중요합니다.
- **레이어 하나는 정확히 어떤 역할을 할까요?**
  - 이미지 최적화의 핵심은 "작게 만든다"가 아니라 "변경 비용을 통제한다"입니다. 대부분의 팀에서 배포가 느려지는 이유는 애플리케이션 코드 양보다 Dockerfile 레이어 설계가 비효율적이기 때문입니다.
- **OverlayFS는 이 레이어들을 어떻게 겹쳐 보이게 만들까요?**
  - 레이어는 단순 파일 묶음이 아니라 변경 델타입니다. OverlayFS는 lowerdir(읽기 전용 레이어들)과 upperdir(쓰기 가능 레이어)을 합쳐 하나의 파일시스템처럼 보여 줍니다.
- **적용 전후에서 가장 흔한 실수는 무엇일까요?**
  - 즉, 레이어는 단순한 저장 형식이 아니라 빌드 속도와 배포 효율을 좌우하는 핵심 메커니즘입니다. 레이어 순서를 설계하는 것이 곧 빌드 비용을 설계하는 것입니다.
- **빠른 검증과 장애 신호을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 빠른 검증과 장애 신호을 실무에 적용할 때 주의할 점은 무엇일까요 — 본문에서 구체적으로 다룹니다.

<!-- toc:begin -->
## 시리즈 목차

- [Containers 101 (1/10): Container란 무엇인가?](./01-what-is-a-container.md)
- **Containers 101 (2/10): Image와 Layer (현재 글)**
