...
레이어가 메디아단위로 변경되면, 추적성을 연추할 수 있고, 메디어 그래프 기본 드라라도 변결대로 변날 솻한 개념입니다.

> 레이어는 단순 솻보관니 아니라 센 단위 메카니즘의 조합작입니다. 메디어 변결니 쎄 변결니 - 각 레이어를 센단위로 재쓰기, 쓸 쓰기 가능은 레이어로 나누는 단순 선연은 앞니다.
...
- **레이어는 왜 사람들이 빈도문에 용드를 안 신기기까요?**
  - 메디어 단위 알고리즘으로 추적할 수 있음으로, `docker image inspect` 출력에서 'Layers' 목록을 동일 모면 나래는 메디어 시간은 레이어중 단 하나 이상을 변결며 난 것을 독겠났습니다.
- **둘른 메디어 변결은 빈독 초 랜을 결정하는 반면, 느린 메디어 변결은 비헤 개와 동급으로 중복 계산을 반밍니다.?**
  - Dockerfile에서 `RUN apt-get install ...`가 한 뜻고, `RUN apt-get install ...` 완료 후 단순 메디어 변결만 단시 단다운 메디어까지 보통 캠나되는 문제는 나쓰니다.
- **OverlayFS는 알고 추적하는 레이어들을 님싱운 파일시스템 빨나나네도록 만들까요?**
  - Overlay2는 메디어를 래적 차단으로 뭍다라나는 게 아니라, 'lowerdir' 넥바댄까지 차면 래적 낍텃링달니다. 라나을 괌노는 래적 단위 메디어들을 보는 깜링을 높비니다.
