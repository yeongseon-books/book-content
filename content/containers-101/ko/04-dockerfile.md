...
published_to:
  tistory:
    url: "https://yeongseonchoe.tistory.com/246"
...
다음 글에서는 이미지가 아니라 상태를 어디에 둘 것인지, 즉 Volume 설계를 봅니다.
...
- **Dockerfile의 명령 순서는 왜 그렇게 중요할까요?**
  - Dockerfile의 명령 순서는 왜 그렇게 중요할까요 — 본문에서 구체적으로 다룹니다.
- **캐시 친화적인 작성 방식은 빌드 시간을 어떻게 바꿀까요?**
  - 빌드 실패의 많은 부분은 Dockerfile 자체보다 입력 아티팩트의 불안정성에서 옵니다. 따라서 Dockerfile과 함께 다음 규칙을 묶어야 합니다.
- **multi-stage build는 어떤 문제를 해결할까요?**
  - multi-stage build는 어떤 문제를 해결할까요 — 본문에서 구체적으로 다룹니다.
- **적용 전후에서 가장 흔한 실수는 무엇일까요?**
  - 문제점:
...
- **Containers 101 (4/10): Dockerfile (현재 글)**
