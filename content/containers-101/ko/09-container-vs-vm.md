...
- 어떤 워크로드는 컨테이너가 더 맞고, 어떤 워크로드는 VM이 더 맞을까요?
- Firecracker, Kata 같은 하이브리드 접근은 왜 등장했을까요?

> 컨테이너는 호스트 커널을 공유하기 때문에 가볍고 빠르며, VM은 자체 커널을 가지기 때문에 더 강한 격리를 제공합니다. 둘은 경쟁 관계라기보다 보완 관계에 가깝습니다.
