...
**핵심 질문**: 같은 *격리* 라면 *컨테이너* 와 *VM* 중 *무엇* 을 *언제* 써야 할까요?

> *컨테이너* 는 *커널 공유* 로 *가벼움*, *VM* 은 *전용 커널* 로 *강한 격리* 를 제공합니다.
...
- **hypervisor**: *VM* 을 띄우는 *가상화 계층*.
- **guest kernel**: *VM 안* 의 *전용 커널*.
- **container**: *호스트 커널 공유* 의 *프로세스 격리*.
- **microVM**: *가벼운 VM* (Firecracker).
- **gVisor / Kata**: *컨테이너 + 추가 격리*.
...
1. *커널 공유* 가 *왜* 가볍게 만드는지 한 줄로.
2. *VM* 이 *컨테이너* 보다 *유리한 사례* 한 가지.
3. *Firecracker* 의 *위치* 한 줄로.
