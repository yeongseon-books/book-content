...
## 처음 질문으로 돌아가기
- **컨테이너와 VM의 격리 수준은 어디가 다를까요?**
  - VM은 호스트 kernel과 완전히 다른 guest kernel을 부팅하고, 컨테이너는 호스트 kernel을 공유합니다. 따라서 컨테이너는 더 빠르지만 격리 수준이 낮습니다.
- **부팅 시간과 리소스 사용은 왜 다를까요?**
  - VM은 OS 전체를 부팅하므로 초 단위, 컨테이너는 프로세스 시작만 하므로 밀리초 단위입니다. 메모리도 VM은 GB 단위, 컨테이너는 MB 단위입니다.
- **언제 컨테이너를 쓰고 언제 VM을 쓸까요?**
  - 개발/테스트/배포 파이프라인은 컨테이너, 완전한 격리가 필요하거나 다른 OS를 실행해야 하면 VM입니다. 보안 정책상 격리가 필수면 컨테이너 위의 VM도 고려합니다.
