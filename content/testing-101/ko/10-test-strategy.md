...
**핵심 질문**: 테스트는 *무조건 많이* 짜야 좋을까요?

> 좋은 전략은 *적게 짜고 많이 잡는* 구조입니다. 핵심은 *어디에 투자할지* 정하는 일입니다.
...
- *테스트 피라미드* 의 의미와 분포
- 레이어별 *ROI* 와 *투자 우선순위*
- 팀이 따르는 *반복 의식(rituals)*
- 경계에 두는 *계약 테스트(contract test)*
- 흔한 함정 5가지
...
- **Test pyramid**: *단위 많고 / 통합 적고 / E2E 더 적은* 분포 모델.
- **ROI**: 테스트 *한 건이 잡는 버그 대비 비용*.
- **Critical path**: *돈이 흐르는* 핵심 흐름 (결제, 로그인 등).
- **Contract test**: *시스템 경계* 에서 *입출력 형식* 을 검증.
- **Flaky budget**: *허용 가능한 불안정 비율* (예: 0.5%).
...
- *위험 기반* 으로 테스트한다. 균등하게 분배하지 않는다.
- *빠른 피드백* 이 *모든 결정* 의 기준이다.
- *경계* 에 *계약 테스트* 를 두면 *조직 간 비용* 이 준다.
- *플레이키 테스트* 는 *깨진 창문* 이다. 즉시 고친다.
- 전략은 *살아 있는 문서* 다.
...
1. 본인 프로젝트의 *테스트 분포* 를 측정하고 *피라미드 형태* 인지 확인하세요.
2. *Critical path* 3가지를 정의하고 E2E로 덮였는지 확인하세요.
3. 팀에 도입할 *주간 의식* 한 가지를 제안하세요.
