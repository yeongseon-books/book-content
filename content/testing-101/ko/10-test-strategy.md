...
이 그림에서는 테스트의 종류와 개수를 어떻게 균형 잡을지 보여줍니다. 비용 대비 신뢰도를 고려하여 단위, 통합, E2E 테스트의 비율을 결정합니다.
...
- **테스트 피라미드는 왜 분포가 중요할까요?**
  - 본문의 기준은 테스트 전략 세우기를 한 덩어리 개념으로 보지 않고 입력, 처리, 검증, 운영 신호가 만나는 경계로 나누어 확인하는 것입니다.
- **단위, 통합, E2E 계층에 어떤 비율로 투자해야 할까요?**
  - 가장 빠른 단위 테스트를 많이, 느린 E2E는 적게 유지하되, 팀의 위험도에 따라 비율을 조정합니다.
- **중요한 사용자 경로는 어떻게 정할까요?**
  - 처음부터 완벽한 전략을 세우기보다는 경험하며 배우고, 메트릭을 보며 지속해서 개선하는 것이 현실적입니다.
