...
소프트웨어에는 기억력이 없습니다. 한 번 고쳐도 다음 사람이 다시 깨뜨릴 수 있습니다. 회귀 테스트는 팀의 기억입니다.

> 회귀 테스트가 없는 팀은 같은 버그를 오래 반복해서 고치게 됩니다.
...
```python
# tests/test_regression.py 파일
def test_repro_negative_price_breaks_total():
    cart = Cart(); cart.add(Item(price=-1))
    assert cart.total() >= 0   # 현재는 실패한다
```
...
- 테스트 이름에 버그 ID를 넣어 추적 가능하게 합니다.
- 회귀 테스트는 작고 명확해야 하며 재현 한 가지에만 집중합니다.
- 무조건 추가하는 것이 아니라 재발 가능성을 보고 결정합니다.

## 자주 하는 실수 5가지

1. **버그를 고치고 테스트는 안 쓴다.** 회귀의 시작입니다.
2. **재현 테스트가 너무 크다.** 최소 케이스로 줄이세요.
3. **회귀 테스트만 쌓이고 정리되지 않는다.** 유지 비용이 폭발합니다.
4. **스냅샷을 생각 없이 갱신한다.** 왜 바뀌었는지 확인하지 않으면 의미 없는 박제가 됩니다.
5. **회귀 테스트를 느린 E2E로 만든다.** 가능한 한 낮은 단계에 두어야 합니다.

## 실무에서는 이렇게 쓰입니다

대부분의 팀은 이슈 → 재현 테스트 → 수정 흐름을 기본 PR 템플릿으로 강제합니다. 같은 모듈에서 회귀가 반복되면 설계 문제의 신호로 봅니다.

## 체크리스트

- [ ] 최근 버그 수정에 회귀 테스트가 있다.
- [ ] 테스트 이름에 이슈 ID나 이유가 들어 있다.
- [ ] 재현 테스트가 작고 결정적이다.
- [ ] 회귀 테스트가 가능한 단위 레벨이다.

## 정리 및 다음 단계

회귀 테스트는 팀의 기억입니다. 다음 글에서는 이 모든 테스트를 자동으로 돌리는 CI로 옮깁니다.
