---
series: testing-101
episode: 1
title: "Testing 101 (1/10): 테스트란 무엇인가?"
status: content-ready
targets:
  tistory: true
  medium: false
  hashnode: false
  mkdocs: true
  ebook: true
language: ko
tags:
  - Testing
  - Quality
  - Software
  - Basics
  - Engineering
seo_description: 테스트의 정의, 종류, 자동화의 필요성을 처음부터 설명하는 Testing 101 시리즈의 시작.
last_reviewed: '2026-05-12'
---

# Testing 101 (1/10): 테스트란 무엇인가?

처음 테스트를 배울 때는 코드보다 절차부터 떠올리기 쉽습니다. 서버를 띄우고, 브라우저를 열고, 회원가입 버튼을 눌러 보고, 로그인이 되는지 확인하는 식입니다. 이 방식은 처음 한두 번은 통합니다. 그런데 기능이 늘고 사람이 늘면 곧 같은 질문이 따라옵니다. 어제 확인한 기능을 오늘도 다시 손으로 눌러 봐야 할까요?

변경이 잦은 코드베이스에서는 수작업 확인만으로 품질을 지키기 어렵습니다. 확인 범위가 넓어질수록 빠뜨리는 항목이 생기고, 그 공백은 배포 뒤에 사고로 돌아옵니다.

이 글은 Testing 101 시리즈의 첫 번째 글입니다. 여기서는 테스트가 무엇인지, 자동 테스트가 왜 필요한지, 그리고 뒤이어 나올 단위 테스트·통합 테스트·E2E 테스트가 어떤 자리에 놓이는지부터 정리하겠습니다.

![Testing 101 1장 흐름 개요](https://yeongseon-books.github.io/book-public-assets/assets/testing-101/01/01-01-diagram.ko.png)
*Testing 101 1장 흐름 개요*
> 테스트의 본질은 한 번의 확인이 아니라, 같은 검증을 여러 번 재현할 수 있는 자동화된 절차입니다.

## 이 글에서 다룰 문제

- 테스트는 정확히 무엇을 검증하는 코드일까요?
- 수동 확인과 자동 테스트는 무엇이 다를까요?
- 단위 테스트, 통합 테스트, E2E 테스트는 어떻게 구분할까요?
- 좋은 테스트가 갖춰야 할 조건은 무엇일까요?
- 테스트를 처음 도입할 때 어떤 순서가 현실적일까요?

테스트가 없으면 모든 수정이 추측에 가까워집니다. 회원가입을 고쳤는데 결제가 깨지고, 결제를 고쳤는데 로그인이 깨지는 일이 반복됩니다. 문제는 버그가 생긴다는 사실만이 아닙니다. 팀이 변경을 두려워하게 된다는 점이 더 큽니다.

자동 테스트는 변경을 멈추지 않으면서 위험을 줄이는 장치입니다. 코드를 고친 뒤 바로 다시 돌려 볼 수 있고, 다른 사람이 만든 변경까지 같은 기준으로 확인할 수 있습니다. 그래서 테스트는 기능 개발의 반대편에 있는 부담이 아니라, 계속 바꿀 수 있게 해 주는 안전장치에 가깝습니다.

## 한눈에 보는 구조

가장 단순한 흐름은 이렇습니다. 프로덕션 코드가 있고, 그 코드를 검증하는 테스트 코드가 있습니다. 테스트 러너는 테스트를 모아서 실행하고, 통과 여부와 실패 위치를 알려 줍니다.

```text
┌─────────────────┐     ┌──────────────────┐     ┌────────────────┐
│ Production Code │ ←── │   Test Code      │ ←── │  Test Runner   │
│  (src/)         │     │  (tests/)        │     │  (pytest)      │
└─────────────────┘     └──────────────────┘     └────────────────┘
                                                         │
                                                         ▼
                                                  ┌────────────────┐
                                                  │  결과 리포트    │
                                                  │  Pass / Fail   │
                                                  └────────────────┘
```

중요한 점은 테스트도 코드라는 사실입니다. 기대하는 동작을 사람이 말로 설명하는 대신 실행 가능한 형태로 남겨 둔 것입니다. 테스트가 코드이기 때문에 버전 관리가 됩니다. PR에서 리뷰할 수 있고, CI에서 자동 실행할 수 있습니다.

| 용어 | 의미 | 예시 |
|------|------|------|
| 테스트 | 기대하는 동작을 코드로 표현한 검증 절차 | `assert add(2, 3) == 5` |
| 단언문(assertion) | 실제 값이 기대와 맞는지 확인하는 표현 | `assert result == expected` |
| 테스트 러너 | 테스트를 수집하고 실행하는 도구 | pytest, unittest, Jest |
| 픽스처(fixture) | 테스트에서 재사용할 데이터나 상태 | DB 연결, 임시 파일, 테스트 사용자 |
| 커버리지(coverage) | 테스트가 프로덕션 코드를 어디까지 실행했는지 보여 주는 지표 | line coverage 85% |
| SUT(System Under Test) | 현재 테스트 대상이 되는 코드 단위 | 함수, 클래스, 모듈 |
| 테스트 스위트(suite) | 관련 테스트를 묶은 집합 | `tests/test_auth.py` 파일 전체 |

## 수동 확인과 자동 테스트 비교

**수동 확인 중심**

```text
1. 로컬에서 서버를 띄운다
2. 브라우저에서 회원가입 → 로그인 → 결제를 눌러 본다
3. "잘 되네"라고 판단하고 PR을 머지한다
4. 사흘 뒤 다른 변경 때문에 같은 흐름이 다시 깨진다
```

**자동 테스트 중심**

```bash
$ pytest
collected 142 items
.................................... 142 passed in 3.4s
```

차이는 단순합니다. 수동 확인은 그 순간에만 유효합니다. 자동 테스트는 다음 주에도, 반년 뒤에도, 다른 사람이 같은 코드를 건드렸을 때도 다시 실행됩니다. 팀 단위로 작업할수록 이 차이가 크게 벌어집니다.

| 비교 항목 | 수동 확인 | 자동 테스트 |
|-----------|-----------|-------------|
| 반복 비용 | 매번 사람이 시간 투입 | 한 번 작성, 무한 반복 |
| 범위 | 기억에 의존, 빠뜨림 발생 | 정의된 시나리오 전부 실행 |
| 피드백 속도 | 분~시간 | 초~분 |
| 신규 인원 활용 | 매뉴얼 필요 | `pytest` 한 줄 |
| 회귀 감지 | 우연히 발견 | 즉시 실패 |

## 다섯 단계로 첫 자동 테스트 만들기

### 1단계 — 검증할 함수 준비

```python
# src/calc.py
def add(a: int, b: int) -> int:
    return a + b
```

### 2단계 — 테스트 파일 작성

```python
# tests/test_calc.py
from src.calc import add

def test_add_positive_numbers():
    assert add(2, 3) == 5

def test_add_with_zero():
    assert add(0, 7) == 7

def test_add_negative_numbers():
    assert add(-1, -2) == -3
```

### 3단계 — 실행

```bash
pip install pytest
pytest -v
# tests/test_calc.py::test_add_positive_numbers PASSED
# tests/test_calc.py::test_add_with_zero PASSED
# tests/test_calc.py::test_add_negative_numbers PASSED
```

### 4단계 — 일부러 깨뜨려 보기

```python
def add(a: int, b: int) -> int:
    return a - b   # 버그
```

```bash
pytest -v
# FAILED tests/test_calc.py::test_add_positive_numbers - assert -1 == 5
```

### 5단계 — 다시 고치고 신뢰 확인

`add` 함수를 원래대로 되돌린 뒤 `pytest`를 다시 실행합니다. 모든 테스트가 초록색으로 돌아오면, 테스트가 실제로 버그를 잡아냈다는 경험을 한 번 얻게 됩니다.

실패를 먼저 보고 수정 뒤 통과하는 이 사이클이 테스트 신뢰의 출발점입니다. 처음부터 초록색 테스트만 추가하면 그 테스트가 실제로 버그를 잡는지 확인할 방법이 없습니다.

## pytest 기본 패턴 — AAA와 파라미터화

좋은 테스트는 Arrange(준비), Act(실행), Assert(검증) 세 단계로 나뉩니다. 이 패턴이 자리 잡히면 다른 사람이 테스트를 읽을 때 흐름이 바로 보입니다.

```python
import pytest
from src.calc import add

# AAA 패턴 적용
def test_add_two_positive_integers():
    # Arrange
    a, b = 3, 4
    # Act
    result = add(a, b)
    # Assert
    assert result == 7

# 여러 입력을 하나로 묶기
@pytest.mark.parametrize("a,b,expected", [
    (2, 3, 5),
    (0, 0, 0),
    (-1, 1, 0),
    (-5, -3, -8),
])
def test_add_parametrized(a, b, expected):
    assert add(a, b) == expected
```

파라미터화는 같은 모양의 테스트를 함수 하나로 줄이면서도 케이스마다 독립적으로 실행되게 합니다. 실패 시 어떤 입력이 원인인지 바로 볼 수 있어 디버깅이 편합니다.

## 테스트의 세 가지 레벨

자동 테스트는 검증 범위에 따라 세 레벨로 나뉩니다. 이 시리즈 뒤편에서 각각을 깊이 다루지만, 여기서 큰 그림을 먼저 잡아 두면 이후 글의 위치가 선명해집니다.

```text
                ┌───────────┐
               /  E2E Test   \         ← 적고, 느리고, 비싸다
              /───────────────\
             / Integration Test\       ← 중간
            /───────────────────\
           /     Unit Test       \     ← 많고, 빠르고, 싸다
          └─────────────────────────┘
```

| 레벨 | 범위 | 속도 | 주요 검증 대상 |
|------|------|------|----------------|
| Unit | 함수 하나, 클래스 하나 | ms 단위 | 로직 정확성 |
| Integration | 모듈 간 경계 (DB, API, 큐) | 초 단위 | 계약 준수 |
| E2E | 사용자 시나리오 전체 | 분 단위 | 비즈니스 흐름 완결성 |

이 비율은 고정값이 아닙니다. 프로젝트 성격에 따라 integration이 두꺼울 수도, E2E가 핵심일 수도 있습니다. 중요한 것은 각 레벨이 **다른 종류의 위험**을 잡는다는 사실입니다.

## 좋은 테스트의 조건

모든 테스트가 가치 있지는 않습니다. 유지보수 비용이 가치보다 크면 테스트가 오히려 짐이 됩니다. 좋은 테스트는 다음 네 가지 성질을 갖습니다.

1. **빠릅니다.** 느린 테스트는 실행을 피하게 됩니다. 단위 테스트는 전체 스위트가 수 초 안에 끝나야 합니다.
2. **결정적(deterministic)입니다.** 같은 코드에서 항상 같은 결과가 나와야 합니다. 시간, 네트워크, 랜덤에 의존하면 flaky test가 됩니다.
3. **독립적입니다.** 테스트 A의 결과가 테스트 B에 영향을 주면 안 됩니다. 실행 순서에 따라 결과가 바뀌면 디버깅이 극도로 어려워집니다.
4. **실패 메시지가 명확합니다.** 실패 시 어떤 기대가 어떻게 깨졌는지 한 줄로 읽을 수 있어야 합니다.

```python
# 나쁜 예: 실패 시 "AssertionError" 한 줄만 나옴
def test_bad():
    assert process(data)

# 좋은 예: 실패 시 기대값과 실제값이 모두 보임
def test_good():
    result = process(data)
    assert result.status == "completed", f"Expected completed, got {result.status}"
```

## 저장소 구조와 테스트 피라미드 배치

개념을 이해한 뒤 가장 먼저 부딪히는 문제는 "우리 저장소에 어떤 비율로 테스트를 두어야 하는가"입니다. 팀에서 실제로 쓰는 디렉터리 구조를 먼저 잡으면 전략이 선명해집니다.

```text
project/
├─ src/
│  ├─ domain/
│  ├─ service/
│  └─ api/
├─ tests/
│  ├─ unit/         # 빠른 피드백, 대다수 케이스
│  ├─ integration/  # DB, 메시지 브로커, 외부 API 경계
│  └─ e2e/          # 핵심 사용자 여정만
└─ pyproject.toml
```

실무에서는 보통 테스트 수량 기준으로 단위 테스트가 70~85%, 통합 테스트가 10~20%, E2E 테스트가 5~10%를 차지하도록 시작합니다. 절대 비율은 프로젝트 성격에 따라 달라지지만, "빠른 테스트를 두껍게, 느린 테스트를 얇게"라는 원칙은 변하지 않습니다.

```python
# tests/unit/test_tax.py
def test_calculate_vat_for_standard_rate():
    assert calculate_vat(10000, 0.1) == 1000

# tests/integration/test_order_repository.py
def test_save_order_persists_to_db(db_session):
    repo = OrderRepository(db_session)
    order = Order(id='o-1', total=15000)
    repo.save(order)
    assert repo.get('o-1').total == 15000

# tests/e2e/test_checkout_flow.py
def test_checkout_flow(page):
    page.goto('http://localhost:8000')
    page.get_by_role('button', name='장바구니 담기').click()
    page.get_by_role('button', name='결제하기').click()
    expect(page.get_by_text('결제가 완료되었습니다')).to_be_visible()
```

## 픽스처 패턴을 처음부터 분리하는 이유

초기 프로젝트에서는 테스트 데이터 준비를 함수마다 직접 작성해도 당장 큰 문제가 없습니다. 하지만 테스트가 200개를 넘으면 준비 코드 중복이 빠르게 증가하고, 데이터 형태가 바뀔 때 수정 범위가 폭발합니다. 그래서 `pytest` fixture는 입문 단계부터 패턴으로 정착시키는 편이 장기적으로 훨씬 저렴합니다.

```python
import pytest

@pytest.fixture
def user_factory():
    def _build(**overrides):
        data = {
            'id': 'u-100',
            'email': 'test@example.com',
            'role': 'member',
            'active': True,
        }
        data.update(overrides)
        return User(**data)
    return _build

def test_admin_can_publish(user_factory):
    admin = user_factory(role='admin')
    assert can_publish(admin) is True

def test_member_cannot_publish(user_factory):
    member = user_factory(role='member')
    assert can_publish(member) is False
```

`factory fixture`는 같은 기본 데이터를 공유하면서도 테스트별 변형을 간단히 만들 수 있다는 장점이 있습니다. 이 패턴을 도입하면 테스트가 길어지지 않고, 비즈니스 규칙 변화에도 수정 지점을 줄일 수 있습니다.

## 커버리지 보고서를 나침반으로 쓰는 방법

커버리지는 품질 점수표가 아니라 투자 우선순위를 정하는 지표입니다. 리포트를 읽으면 "어떤 파일을 다음 주에 먼저 보호해야 하는지"를 합의할 수 있습니다.

```bash
pytest --cov=src --cov-report=term-missing
```

```text
Name                       Stmts   Miss  Cover   Missing
---------------------------------------------------------
src/domain/pricing.py         82     19    77%   45-52, 71-79
src/service/order.py         104      6    94%   88-90, 131-133
src/utils/time.py             24      1    96%   18
---------------------------------------------------------
TOTAL                        210     26    88%
```

이 보고서에서 `pricing.py`가 핵심 도메인이라면 우선순위는 명확합니다. 단순 유틸리티보다 가격 계산의 누락 분기를 먼저 보강해야 합니다. 즉, 커버리지 숫자 자체보다 "어떤 위험을 먼저 닫았는가"가 중요합니다.

## CI에 테스트를 연결할 때 최소 기준

테스트가 로컬에서만 실행되면 결국 사람의 의지에 의존하게 됩니다. 팀 품질 기준으로 굳히려면 PR마다 자동 실행되어야 합니다. 가장 단순한 출발점은 아래와 같습니다.

```yaml
name: test
on:
  pull_request:
  push:
    branches: [main]

jobs:
  pytest:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      - uses: actions/setup-python@v5
        with:
          python-version: '3.12'
      - run: pip install -r requirements-dev.txt
      - run: pytest -q --maxfail=1
```

처음부터 복잡한 매트릭스를 넣기보다, "모든 PR에서 단위 테스트는 반드시 돈다"는 합의를 먼저 자동화하는 것이 좋습니다. 이후 프로젝트가 커지면 통합 테스트 병렬화, 캐시, 커버리지 업로드를 단계적으로 추가하면 됩니다.

## 자주 하는 실수

| 실수 | 증상 | 올바른 접근 |
|------|------|------------|
| 테스트를 나중에 몰아서 작성 | 맥락이 사라져 케이스를 빠뜨림, 배포 전 급조된 테스트 | 코드 작성 직후 또는 직전에 테스트를 함께 작성 |
| 단언문 없는 테스트 | 커버리지는 높은데 버그가 계속 발생 | 모든 테스트에 의미 있는 `assert` 포함 |
| 테스트끼리 상태 공유 | 실행 순서 바꿀 때만 실패하는 불안정한 테스트 | 각 테스트를 독립적으로 격리 |
| 외부 의존을 단위 테스트에 포함 | 테스트가 느려지고 네트워크 상태에 따라 결과 변동 | mock/stub으로 대체하거나 통합 테스트로 분류 |
| 브라우저 클릭으로 "테스트 완료" | 재현 불가능, 팀 자산이 아님 | 자동 테스트로 변환하여 코드로 남김 |
| 100% 커버리지 집착 | 단언문 없는 테스트 양산, 빈 테스트 함수 | 커버리지는 방향 지표로만 사용 |

## 테스트를 처음 도입할 때의 현실적 조언

이미 테스트 없이 운영 중인 프로젝트에 테스트를 넣으려면 전체를 한 번에 커버하려 들지 마세요. 다음 순서가 현실적입니다.

1. **새로 작성하는 코드부터** 테스트를 붙입니다. 기존 코드는 일단 놔둡니다.
2. **버그가 나올 때마다** 해당 버그를 재현하는 테스트를 먼저 작성한 뒤 수정합니다.
3. **리팩토링 전에** 현재 동작을 검증하는 테스트를 추가합니다.
4. **CI에 연결합니다.** 로컬에서만 돌리면 잊힙니다. PR마다 자동 실행되게 하면 빠뜨림이 줄어듭니다.

완벽한 커버리지가 목표가 아닙니다. 팀이 "테스트를 돌리는 습관"을 갖는 것이 첫 번째 목표입니다.

## 운영 관점에서 생각하기

현업에서는 테스트를 단순한 검사 도구보다 변경 비용을 낮추는 장치로 봅니다. 팀이 코드를 자주 배포하려면, 사람의 기억과 수동 체크리스트 대신 자동 검증이 먼저 갖춰져야 합니다.

강한 팀일수록 PR에서 먼저 묻는 질문이 있습니다. 무엇을 바꿨는가보다 무엇을 검증했는가입니다. 테스트가 없으면 리뷰어는 구현 의도와 실패 조건을 파악하기 어려워지고, 머지 뒤의 위험도 크게 올라갑니다.

테스트는 코드와 함께 진화해야 합니다. 기능이 바뀌었는데 테스트가 그대로라면 테스트는 안전장치가 아니라 오경보 장치가 됩니다. 팀에서는 요구사항 변경 PR에 테스트 변경이 함께 포함되는지를 리뷰 기준으로 두는 편이 좋습니다.

버그 티켓이 닫힐 때 반드시 재현 테스트를 남기는 습관도 중요합니다. 수정 코드만 머지하면 같은 원인의 버그가 다른 경로에서 재발합니다. 재현 테스트를 함께 남기면 팀 지식이 실행 가능한 형태로 축적됩니다.

CI에서는 실패를 빠르게 보여 주는 순서가 중요합니다. 일반적으로 단위 테스트를 먼저 실행하고, 그 다음 통합 테스트, 마지막으로 느린 E2E를 배치하면 평균 피드백 시간이 줄어듭니다. 파이프라인 설계도 테스트 전략의 일부입니다.

## 직접 검증해 볼 것

1. `add` 함수를 일부러 `return a - b`로 바꾼 뒤 `pytest -v`를 다시 실행해 봅니다. 실패가 바로 보이지 않으면 테스트가 실제 버그를 감시하지 못하고 있다는 뜻입니다.
2. 두 테스트 이름만 읽고도 함수 계약이 떠오르는지 확인합니다. 이름이 동작을 설명하지 못하면 테스트는 문서 역할도 못 합니다.
3. `pytest -q` 실행 시간을 적어 둡니다. 입문 단계의 테스트가 이미 오래 걸린다면, 이후 시리즈에서 더 많은 테스트를 추가할수록 습관으로 굳기 어렵습니다.

**예상 결과:** 의도적으로 넣은 버그에서는 최소 1개 테스트가 즉시 실패하고, 원래 구현으로 되돌리면 다시 모두 통과해야 합니다.

## 운영 체크리스트

- [ ] `pytest`를 한 번 설치하고 실행해 봤습니다.
- [ ] 코드를 일부러 깨뜨린 뒤 실패 메시지를 읽어 봤습니다.
- [ ] 테스트 하나가 한 가지 동작만 확인하도록 작성했습니다.
- [ ] 테스트 실행 시간이 몇 초 안에 끝나는지 확인했습니다.
- [ ] CI에 테스트가 연결되어 있거나 연결 계획을 세웠습니다.

## 연습 문제

1. `subtract(a, b)` 함수를 만들고 테스트 세 개를 작성해 보세요.
2. 일부러 버그를 넣고 어떤 실패 메시지가 나오는지 확인해 보세요.
3. `@pytest.mark.parametrize`를 사용해 경계값 입력 다섯 개를 하나의 테스트 함수로 묶어 보세요.
4. 동료에게 테스트가 왜 필요한지 한 문단으로 설명해 보세요.

## 정리

테스트는 코드를 믿게 만드는 장치가 아니라, 변경을 겁내지 않게 만드는 장치입니다. 수동 확인을 자동 검증으로 바꾸는 순간부터 팀은 같은 기능을 더 자주, 더 안전하게 고칠 수 있습니다. 다음 글에서는 이 전체 그림에서 가장 작은 단위인 단위 테스트를 자세히 보겠습니다.

<!-- toc:begin -->
## 시리즈 목차

- **Testing 101 (1/10): 테스트란 무엇인가? (현재 글)**
- [Testing 101 (2/10): 단위 테스트](./02-unit-test.md)
- [Testing 101 (3/10): 통합 테스트](./03-integration-test.md)
- [Testing 101 (4/10): E2E 테스트](./04-e2e-test.md)
- [Testing 101 (5/10): 테스트 더블](./05-test-double.md)
- [Testing 101 (6/10): Mock과 Stub](./06-mock-and-stub.md)
- [Testing 101 (7/10): 테스트 커버리지](./07-test-coverage.md)
- [Testing 101 (8/10): 회귀 테스트](./08-regression-test.md)
- [Testing 101 (9/10): CI에서 테스트 실행하기](./09-tests-in-ci.md)
- [테스트 전략 세우기](./10-test-strategy.md)

<!-- toc:end -->

## 참고 자료

- 실습 예제 저장소(book-examples): https://github.com/yeongseon-books/book-examples/tree/main/testing-101/ko
### 공식 문서
- [pytest documentation](https://docs.pytest.org/)
- [Python `unittest` documentation](https://docs.python.org/3/library/unittest.html)

### 실무 참고
- [Martin Fowler — The Practical Test Pyramid](https://martinfowler.com/articles/practical-test-pyramid.html)
- [Google Testing Blog](https://testing.googleblog.com/)

Tags: Testing, Quality, Software, Basics, Engineering
