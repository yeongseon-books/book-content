...
Mock과 Stub을 혼동하면 과한 Mock 때문에 깨지기 쉬운 테스트를 만들게 됩니다. 차이를 알면 올바른 도구를 올바른 자리에 둘 수 있습니다.

> 좋은 테스트는 무엇이 깨졌는지 한 줄로 말해 줍니다.
...
- `return_value` 는 Stub 역할을 하고, `assert_called_*` 는 Mock 역할을 합니다.
- `patch` 는 수술 같은 도구이므로 최소 범위로 사용해야 합니다.
- `side_effect` 로 오류 경로도 검증할 수 있습니다.

## 자주 하는 실수 5가지

1. **Mock에 너무 자세한 기대를 건다.** 리팩터링이 불가능해집니다.
2. **`patch` 를 너무 넓게 건다.** 다른 테스트가 오염됩니다.
3. **반환값과 호출 검증을 동시에 한 테스트에 다 넣는다.**
4. **돈, 이메일, SMS처럼 부수효과가 큰 동작을 Mock 없이 진짜 호출한다.**
5. **테스트가 모든 라인을 다 mock 한다.** 그건 테스트가 아닙니다.

## 실무에서는 이렇게 쓰입니다

대부분의 새 테스트는 Stub이나 Fake를 우선하고, 상호작용이 본질인 곳에만 Mock을 씁니다. 이메일, 결제, 푸시 알림처럼 부수효과 자체가 검증 대상인 경우가 여기에 해당합니다.

## 체크리스트

- [ ] Stub과 Mock의 목적 차이를 한 줄로 설명할 수 있다.
...
Mock과 Stub은 목적이 다른 도구입니다. 다음 글에서는 얼마나 테스트했는가를 측정하는 커버리지를 다룹니다.
