...
모델이 자체 안전 장치를 갖고 있어도 출력은 그대로 신뢰할 수 없습니다. 미묘한 jailbreak, 욕설 인용, 부적절한 조언은 응답 단계에서 다시 걸러야 합니다.

이 글은 AI Safety & Guardrails 101 시리즈의 3번째 글입니다. 여기서는 모델 출력을 검증되지 않은 입력으로 보고, 별도 모더레이션 계층을 어떻게 붙일지 살펴봅니다.
...
모델 출력은 검증되지 않은 입력입니다. 사용자에게 보내기 전에 별도 모더레이션 계층에서 한 번 더 검사해야 합니다. 이번 글에서는 OpenAI Moderation API, Detoxify, 그리고 자체 정책 분류기를 결합한 출력 필터링 패턴을 살펴봅니다.
...
# 예: {"flagged": True, "categories": {"violence": True}, ...}
```
...
detox = Detoxify("original")  # 다국어 모델은 "multilingual" 사용

def detox_score(text: str) -> dict:
    return detox.predict(text)  # 예: {"toxicity": 0.02, "severe_toxicity": 0.001, ...}
...
Llama Guard는 정책 텍스트를 그대로 받아서 분류하므로 회사별 정책(금융 자문 금지, 경쟁사 언급 금지 등)도 카테고리로 확장할 수 있습니다.
...
여러 judge 호출을 병렬화하면 지연 시간을 줄일 수 있습니다. 예를 들어 Moderation API, Detoxify, Policy judge를 동시에 실행할 수 있습니다.

---

## Section 4 — Streaming 응답의 모더레이션

Streaming 응답은 토큰 단위로 바로 흘러갑니다. 모두 받은 뒤 검증하면 사용자가 이미 내용을 본 뒤입니다.
...
### 옵션 B: 전체 응답을 받아 검증한 뒤 전송 (delayed delivery)
...
기본 원칙은 단순합니다. 사용자에게 보여 준 텍스트는 되돌릴 수 없습니다. 위험이 큰 도메인일수록 옵션 B를 선택하는 편이 안전합니다.
...
- 구체적인 차단 사유는 노출하지 않습니다. 우회 힌트를 줄 수 있기 때문입니다.
- 카테고리별로 적절한 안내를 제공합니다. 자해는 응급 연락처, 정책 차단은 재시도 안내가 적절합니다.
- 다시 시도할 수 있다는 점을 알려 줍니다. 대화가 끝났다는 인상을 주지 않는 편이 좋습니다.
- 상세 사유는 로그에 남깁니다. 디버깅과 모니터링에는 내부 기록이 필요합니다.
...
class ModerationLog:
    timestamp: datetime
    response_excerpt: str  # 처음 200자만 저장
    flagged_category: str
    score: float
    user_complaint: bool = False  # 사용자가 "왜 차단됐는지 모르겠다"고 신고함
...
False positive rate가 5%를 넘으면 임계값을 다시 조정하거나 카테고리 정책을 재검토해야 합니다.

---

## 흔한 실수

1. Moderation API의 `flagged`만 봅니다. 카테고리별 임계값을 도메인에 맞게 조정하지 않으면 의료·금융 챗봇에는 너무 느슨해집니다.
2. Streaming 응답을 검증하지 않습니다. 사용자가 다 본 뒤에야 문제를 알게 되므로 청크 버퍼나 delayed delivery를 사용해야 합니다.
3. 차단 사유를 그대로 노출합니다. 우회 힌트가 되므로 일반 메시지로 응답하는 편이 안전합니다.
4. 자체 정책을 표준 카테고리에 억지로 넣습니다. "환불 약속 금지" 같은 회사 정책은 별도 LLM judge가 필요합니다.
5. False positive를 측정하지 않습니다. 너무 엄격해도 신뢰를 잃으므로 사용자 신고와 함께 추적해야 합니다.

---

## 핵심 요약

- 모델 출력은 검증되지 않은 입력이므로 별도 모더레이션 계층이 필요합니다.
- OpenAI Moderation API로 빠르게 시작할 수 있지만, 카테고리별 임계값은 도메인에 맞춰 조정해야 합니다.
- Detoxify, Llama Guard 같은 self-hostable 대안도 활용할 수 있습니다.
- 회사별 정책은 자체 LLM judge로 따로 검증하는 편이 안전합니다.
- Streaming 응답은 청크 버퍼나 delayed delivery 방식으로 처리해야 합니다.
- 차단 메시지는 카테고리에 맞게 안내하고 false positive rate를 계속 모니터링해야 합니다.
