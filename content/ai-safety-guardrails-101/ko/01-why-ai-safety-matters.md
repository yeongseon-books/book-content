...
이 그림에서는 LLM 호출을 비신뢰 경계로 두고 입력, 모델 호출, 출력, 운영 로그 주변에 guardrail을 배치하는 관점을 봅니다. AI Safety의 출발점은 모델을 믿는 것이 아니라 모델이 만든 데이터를 다시 검증하는 것입니다.
...
이 구조의 장점은 정책이 코드에서 명시적으로 보인다는 점입니다. 정책이 프롬프트 문장 속에 숨어 있으면 운영자가 실제 집행 규칙을 확인하기 어렵습니다.
...
이 순서의 장점은 정책을 크게 바꾸지 않고도 위험을 빠르게 낮출 수 있다는 점입니다. 또한 각 단계에서 block rate, false positive rate, P95 지연을 같이 관찰하면 팀이 품질과 안전을 동시에 조절할 수 있습니다.
