...
Jailbreak 시도를 어떻게 탐지해야 신호 손실을 최소화하면서 방어할 수 있을까요?

이 글은 그 질문에 답하기 위해 Jailbreak 탐지의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **알려진 패턴 사전이 출발점** — DAN·prompt smuggling 등을 자동 검사합니다.
- **LLM 분류기가 보조 방어** — 휴리스틱이 못 잡는 변형을 잡습니다.
- **false positive 비용을 의식** — 너무 엄격하면 정상 사용자를 막습니다.
- **입력 길이·구조 신호를 활용** — 이상치 입력은 jailbreak일 확률이 높습니다.
- **탐지 결과를 학습 데이터로** — 공격은 진화하므로 방어도 진화합니다.
