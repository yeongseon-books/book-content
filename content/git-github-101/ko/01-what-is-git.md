...
> Git을 명령 목록으로 접근하면 금방 막힙니다 — Git은 파일 변경을 시간 순으로 보관하고 되돌리는 도구이고, 모든 상태는 'working directory · staging · repository'라는 세 영역 사이의 이동으로 설명된다는 그림이 먼저 잡혀야 add·commit·push가 비로소 자연스럽게 읽힙니다.
...
Git은 개인 프로젝트의 안전망이면서 팀 협업의 공통어입니다. 어제 작업을 한 번에 되돌릴 수 있다는 장점도 크지만, 더 중요한 이유는 같은 변경을 팀이 같은 방식으로 읽고 토론할 수 있기 때문입니다.
...
- **버전 관리 도구는 정확히 어떤 문제를 해결할까요?**
  - 버전 관리 도구는 정확히 어떤 문제를 해결할까요 — 본문에서 구체적으로 다룹니다.
- **Git이 분산 버전 관리 도구라고 부르는 이유는 무엇일까요?**
  - Git이 분산 버전 관리 도구라고 부르는 이유는 무엇일까요 — 본문에서 구체적으로 다룹니다.
- **Git의 스냅샷 모델은 "변경된 줄만 저장하는 도구"와 무엇이 다를까요?**
  - Git의 스냅샷 모델은 "변경된 줄만 저장하는 도구"와 무엇이 다를까요 — 본문에서 구체적으로 다룹니다.
- **핵심 관점에서 가장 흔한 실수는 무엇일까요?**
  - Git을 한 문장으로 줄이면 **파일의 스냅샷을 시간 순서대로 저장하는 도구**입니다. 각 commit은 그 순간 추적 중이던 파일들의 상태를 찍어 둔 사진이라고 보면 됩니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 개념을 용어로만 외우면 금방 섞이므로, 실제 동작을 한 번에 묶어서 보는 편이 낫습니다.
...
- **Git & GitHub 101 (1/10): Git이란 무엇인가? 버전 관리의 시작 (현재 글)**
