...
버전 관리는 대개 명령부터 외우기 시작하지만, 실제로는 왜 이런 도구가 필요한지부터 감이 잡혀야 오래 남습니다. Git도 마찬가지로, 명령 목록보다 먼저 "시간 순서대로 변경을 보관하고 되돌리는 도구"라는 그림이 머릿속에 들어와야 이후가 쉬워집니다.

이 글은 Git & GitHub 101 시리즈의 첫 번째 글입니다. 여기서는 Git 자체를 어떤 멘탈 모델로 이해해야 하는지부터 차근차근 정리합니다.

## 핵심 질문

Git이 다른 버전 관리 도구와 비교해 무엇을 다르게 하고, 왜 표준이 되었을까요?

이 글은 그 질문에 답하기 위해 Git의 본질의 핵심 결정과 운영 함정을 살펴봅니다.
