...
- **스냅샷 기반이 분기·병합을 단순화** — 차이가 아니라 상태를 저장하는 모델이 협업을 살립니다.
- **분산 저장소가 협업의 자유도** — 각자의 로컬에서 완전한 이력을 가집니다.
- **커밋이 의미 단위로 잘려야 가치** — 기록의 가독성이 후속 작업의 효율을 결정합니다.
- **브랜치는 비용이 거의 없다** — 실험을 두려워하지 않게 만드는 핵심 자산입니다.
- **학습 곡선은 모델 이해에서 풀린다** — 명령어가 아니라 자료구조를 먼저 이해합니다.
