...
브랜치를 어떻게 운영해야 실험은 자유롭고 main은 안전할까요?

이 글은 그 질문에 답하기 위해 branch 운영의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- branch는 "파일 복사"가 아니라면 내부적으로 무엇인가?
- `git branch`로 branch를 만드는 것과 `git switch`로 옮기는 것은 어떻게 분리되는가?
- `HEAD`는 branch와 어떤 방식으로 함께 움직이는가?
- `git checkout`과 `git switch`의 관계는 무엇이며 왜 둘 다 남아 있는가?
- 두 branch의 차이를 `git log`와 `git diff`로 어떻게 비교하는가?
...
> branch는 폴더 복사가 아니라 "특정 commit을 가리키는 움직이는 포인터"이며, `HEAD`는 그 포인터들 중 "지금 작업하고 있는 곳"을 가리키는 또 하나의 포인터입니다.
branch는 commit을 가리키는 **포인터**입니다. 새 commit이 만들어지면 현재 branch 포인터가 한 칸 앞으로 이동합니다.

![Mental Model](../../../assets/git-github-101/04/04-01-mental-model.ko.png)
...
- **branch는 포인터일 뿐** — 비용이 거의 없는 가벼운 도구로 인식합니다.
- **이름이 의도를 드러내야 한다** — 브랜치 이름 컨벤션이 협업의 마찰을 줄입니다.
- **checkout과 switch의 의미 차이** — 최신 명령으로 의도를 명확히 합니다.
- **브랜치 비교가 PR 전 자기 검토** — merge 전에 diff를 스스로 확인합니다.
- **오래된 브랜치는 빨리 정리** — 장수 브랜치가 갈수록 통합 비용을 키웁니다.
