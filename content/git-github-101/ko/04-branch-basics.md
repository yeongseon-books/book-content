...
이 방식은 실험 흔적을 commit 단위로 남깁니다. 팀 토론에서 "어떤 문구가 왜 바뀌었는지"를 감이 아니라 이력으로 확인할 수 있습니다.
...
- **branch는 왜 파일 복사본이 아니라 포인터라고 할까요?**
  - branch를 포인터라고 이해해도, 실제로는 히스토리 모양이 머릿속에서 잘 안 그려지는 경우가 많습니다. 그래서 텍스트 다이어그램으로 현재 위치를 반복해서 읽는 연습이 효과적입니다.
- **`git branch`와 `git switch`는 역할이 어떻게 다를까요?**
  - `git branch`와 `git switch`는 역할이 어떻게 다를까요 — 본문에서 구체적으로 다룹니다.
- **`HEAD`는 branch와 어떤 관계를 가질까요?**
  - `HEAD`는 branch와 어떤 관계를 가질까요 — 본문에서 구체적으로 다룹니다.
- **핵심 그림에서 가장 흔한 실수는 무엇일까요?**
  - 이 그림에서 먼저 붙잡을 것은 두 가지입니다.
- **핵심 개념을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 이 그림에서 먼저 붙잡을 것은 두 가지입니다.
...
- **Git & GitHub 101 (4/10): branch 기초 - 만들고 옮기고 비교하기 (현재 글)**
