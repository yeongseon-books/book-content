...
> remote가 생기는 순간 Git은 '한 대의 PC 안 도구'에서 '같은 history를 여러 기기·여러 사람이 공유하는 협업 도구'로 바뀝니다 — push·fetch·pull·clone은 별개 기능이 아니라 '로컬과 원격의 history를 어느 방향으로 동기화할 것인가'라는 한 가지 질문의 네 가지 답입니다.
...
- **remote는 정확히 무엇이고 왜 첫 이름이 보통 `origin`일까요?**
  - remote는 정확히 무엇이고 왜 첫 이름이 보통 `origin`일까요에 대해 본문에서 실무 예시와 함께 답합니다.
- **빈 GitHub 저장소를 로컬 저장소에 연결하는 순서는 어떻게 될까요?**
  - remote는 다른 위치의 저장소를 가리키는 별칭이고, 첫 이름은 보통 `origin`입니다
- **`git push -u origin main`은 한 번에 무엇을 두 가지나 설정할까요?**
  - 이제 많이 쓰는 실무 패턴을 하나 더 보겠습니다. 회사나 오픈소스에서 자주 쓰는 구조는 아래와 같습니다.
- **핵심 그림에서 가장 흔한 실수는 무엇일까요?**
  - 세 가지를 함께 기억하면 충분합니다.
...
- **Git & GitHub 101 (6/10): GitHub repository 만들기 - remote, push, pull 한 번에 익히기 (현재 글)**
