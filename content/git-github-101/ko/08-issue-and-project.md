...
- GitHub Issue가 무엇이고 commit, PR과 어떻게 다른지
- Issue를 만들고 label과 milestone을 붙이는 순서
- PR 본문의 `Closes #42` 한 줄이 merge 시 issue를 자동으로 닫아 주는 원리
- Project board를 만들어서 issue를 Kanban 흐름에 올려 두는 방법
- 혼자 쓰는 저장소에서도 issue를 쓰면 좋은 이유

이 글이 끝나면 issue로 작업을 정의하고, PR로 그 작업을 끝내고, Project board로 진행 상황을 추적하는 한 사이클을 직접 돌릴 수 있습니다.
...
- GitHub Issue는 commit·PR과 어떤 점에서 본질적으로 다른가?
- label과 milestone은 issue 운영에서 각각 어떤 역할을 하는가?
- `Closes #42` 한 줄이 PR merge 시 issue를 자동으로 닫는 원리는 무엇인가?
- Project board의 컬럼은 issue의 어떤 상태 변화를 반영하는가?
- 혼자 쓰는 저장소에서도 issue를 쓰면 무엇이 좋아지는가?
...
- **Issue는 일감의 단일 출처** — 대화·결정·결과를 한 곳에 모읍니다.
- **Issue 템플릿이 마찰을 줄인다** — 필요한 정보를 처음부터 받습니다.
- **Label이 분류·필터의 1순위** — 검색·자동화의 기반이 됩니다.
- **Project board로 흐름을 가시화** — WIP를 제한해 처리량을 높입니다.
- **PR과 Issue를 연결한다** — 변경의 이유가 자동으로 추적됩니다.
...
1. `vacation-notes`에 issue 두 개를 새로 만들고 한쪽에는 `Closes #N`을 넣은 PR을, 다른 한쪽에는 `Closes`를 빼먹은 PR을 만들어 보세요. merge한 뒤 두 issue의 상태가 어떻게 다른지 비교해 봅니다.
2. Project board를 하나 만들고 column을 `Triage`, `This week`, `Done` 세 개로 직접 정의해 보세요. 본인의 작업 흐름에 맞춘 column 이름을 정하는 연습이 됩니다.
