...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 프로세스 메모리는 어떤 구역으로 나뉘어 있을, 둘째 `malloc`과 `free`, 가비지 컬렉션은 각각 무엇을 맡을, 셋째 메모리 누수와 단편화는 어떻게 다른 문제일입니다. 기본 모델에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **프로세스 메모리는 어떤 구역으로 나뉘어 있을까요?**
  - 메모리 문제는 언어 런타임 지표만 보면 원인을 놓치기 쉽습니다. OS 관점의 레이아웃과 할당기 동작을 함께 보아야 합니다.
- **`malloc`과 `free`, 가비지 컬렉션은 각각 무엇을 맡을까요?**
  - - 가비지 컬렉션은 "참조가 끊긴" 객체만 회수합니다 - 누수는 "안 쓰는데 참조가 살아 있는" 상태에서 만들어집니다 - 단편화는 총량이 아니라 "연속 공간"의 문제입니다 - 약한 참조와 회수 정책(LRU 등)은 누수의 가장 흔한 처방입니다
- **메모리 누수와 단편화는 어떻게 다른 문제일까요?**
  - 메모리 문제는 언어 런타임 지표만 보면 원인을 놓치기 쉽습니다. OS 관점의 레이아웃과 할당기 동작을 함께 보아야 합니다.
- **기본 모델에서 가장 흔한 실수는 무엇일까요?**
  - > 프로세스 메모리는 크게 네 영역으로 나뉩니다. 코드(text), 전역 변수(data/bss), heap, stack. heap은 동적 할당, stack은 함수 호출에 따라 자동으로 자라고 줄어듭니다.
- **같은 코드를 다르게 읽는 법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 며칠 후 OOM. 누수는 명시적 free가 없는 GC 언어에서도 똑같이 발생합니다.
- **단계별로 확인하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - `ru_maxrss`는 프로세스가 실제로 점유한 RAM의 최대값입니다.
