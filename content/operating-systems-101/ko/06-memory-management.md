...
**핵심 질문**: 프로세스가 사용하는 메모리는 어디에서 와서, OS는 한정된 RAM을 어떻게 나누어 줄까요?

> 메모리는 CPU 다음으로 자주 부족해지는 자원입니다. stack과 heap의 구분, malloc과 free의 의미, 그리고 메모리 누수가 어떻게 만들어지는지를 모르면 "갑자기 느려지는 서비스"의 원인을 영원히 추적하지 못합니다. 이 글은 메모리 관리의 기본기를 한 번에 정리합니다.

<!-- a-grade-intro:end -->

## 이 글에서 배울 것

- 프로세스 메모리 레이아웃 — text, data, heap, stack
- malloc/free와 가비지 컬렉션의 차이
- 메모리 누수와 단편화가 만들어지는 방식
- OS가 메모리를 나누는 두 축 — 분할(partitioning)과 회수(reclamation)
...
| 용어 | 설명 |
| --- | --- |
| 가상 주소 | 프로세스가 보는 주소 (실제 RAM 주소와 다름) |
| heap | 동적 할당 영역, 명시적 free 또는 GC 필요 |
| stack | 함수 호출에 따라 자동 관리되는 영역 |
| 단편화 | 빈 공간은 충분한데 연속된 큰 블록이 없는 상태 |
| 메모리 누수 | 더 이상 안 쓰는데 회수되지 않은 메모리 |
...
시니어 엔지니어는 "이 객체는 누가, 언제 회수하는가"를 코드 리뷰의 기본 질문으로 삼습니다. 회수 책임이 명시되지 않은 객체는 잠재적 누수입니다. 캐시를 추가할 때는 capacity와 eviction policy를 같이 적습니다. 둘 중 하나만 빠져 있어도 시간 문제로 OOM이 옵니다.

또한 시니어는 "메모리는 CPU와 달리 그래프에 늦게 나타난다"는 사실을 압니다. CPU는 즉시 100%로 튀지만, 메모리 누수는 며칠~몇 주에 걸쳐 잠식합니다. 그래서 RSS 추세를 모니터링 대상에 반드시 넣고, 일정 기울기 이상이면 알람을 겁니다.
...
1. `resource.getrusage`로 누수 코드의 RSS가 시간에 따라 어떻게 증가하는지 그래프로 그려 보세요. 한 문단으로 패턴을 정리합니다.

2. 무제한 dict 캐시를 `functools.lru_cache(maxsize=...)`로 바꾸고 같은 부하를 줬을 때 RSS 차이를 측정합니다.

3. 컨테이너에서 메모리 한도를 64MB로 묶고, 그 안에서 안전하게 도는 캐시 상한을 결정해 보세요. 한도 책정 근거를 적습니다.
