...
- **프로세스 메모리는 어떤 구역으로 나뉘어 있을까요?**
  - 기본적으로 프로세스 메모리는 text, data/bss, heap, stack으로 나뉘고, 실무에서는 여기에 mmap 영역과 스레드별 스택까지 함께 봐야 합니다. 글의 도식과 `/proc/<pid>/smaps` 예제는 힙이 커지는지, 파일 매핑이 늘어나는지처럼 문제 위치를 구역별로 가르는 기준을 제공합니다.
- **`malloc`과 `free`, 가비지 컬렉션은 각각 무엇을 맡을까요?**
  - `malloc`과 `free`는 개발자가 직접 할당과 해제를 짝지어야 하는 방식이고, 가비지 컬렉션은 참조가 완전히 끊긴 객체만 대신 회수합니다. 그래서 무제한 dict 캐시를 `lru_cache(maxsize=10_000)`로 바꾸거나 `WeakValueDictionary`를 쓰는 예제처럼, 회수 정책과 참조 구조를 설계하지 않으면 GC가 있어도 누수는 남습니다.
- **메모리 누수와 단편화는 어떻게 다른 문제일까요?**
  - 누수는 더 이상 쓰지 않는 객체를 참조가 붙잡고 있어 RSS가 계속 증가하는 문제이고, 단편화는 총량은 남아 있어도 큰 연속 공간을 못 구하는 문제입니다. 1MB 블록 1000개를 잡았다가 절반씩 비우는 예제에서 "500MB가 비었지만 1GB 연속 블록은 어렵다"는 설명이 두 현상의 차이를 정확히 보여 줍니다.
