...
프로세스가 "소유"하는 자원:
- 가상 주소 공간 (코드, 힙, 매핑된 파일)
- 파일 디스크립터 테이블
- 신호(signal) 처리기 설정
- 사용자/그룹 권한 (uid, gid)

스레드가 "추가로" 가지는 자원:
- 자신만의 스택 (지역 변수, 함수 호출 프레임)
- 레지스터 세트 (PC, SP, 범용 레지스터)
...
프로세스                          스레드
---------                         -------
느린 생성 (메모리 복사)            빠른 생성 (스택만 추가)
격리 보장 (한 쪽 crash 독립)      격리 없음 (한 쪽 corrupt → 전체 위험)
통신 비용 높음 (pipe/socket)      통신 비용 낮음 (공유 메모리 직접)
GIL 제약 없음                     CPython GIL로 동시 실행 제한
```
...
NEW ──────────────→ READY ──────────────────→ RUNNING
                      ↑                           |
                      |  I/O 완료, 신호 수신       | I/O 요청, sleep, 락 대기
                      └──────── BLOCKED ←─────────┘
                                                   |
                                             TERMINATED
```

OS 스케줄러는 READY 상태의 프로세스 중 다음 실행 대상을 고릅니다. BLOCKED 상태에서는 CPU를 소비하지 않습니다.

현재 실행 중인 프로세스 상태 확인:

```bash
# S: sleeping(blocked), R: running, Z: zombie, D: uninterruptible sleep
ps aux | awk '{print $8, $11}' | sort | uniq -c | sort -rn | head -10
```
...
| 대규모 병렬 I/O (수천 개) | asyncio | 스레드 수 제한 없이 확장 |
...
buf = bytearray(200 * 1024 * 1024)  # 200MB

def rss():
    return resource.getrusage(resource.RUSAGE_SELF).ru_maxrss

print(f"Before fork: RSS = {rss()} KB")

pid = os.fork()
if pid == 0:
    print(f"Child before write: RSS = {rss()} KB")
    for i in range(0, len(buf), 4096):
        buf[i] = 1  # 페이지 단위로 CoW 유발
    print(f"Child after write:  RSS = {rss()} KB")
    os._exit(0)

os.waitpid(pid, 0)
print(f"Parent after child: RSS = {rss()} KB")
```

이 패턴은 워커 모델의 시작 지연과 메모리 폭증을 동시에 만들 수 있습니다. gunicorn 같은 멀티프로세스 서버에서 RSS가 워커 수에 비례해 선형 증가하면 CoW 파괴를 의심합니다.
...
while true; do
  ...
  echo "$(date +%T)  threads=$threads  rss=${rss}KB"
  sleep 1
done
```
...
# 부모 PID 추적
ps -o pid,ppid,stat,comm | awk '$3 ~ /Z/'
```

좀비 프로세스 자체는 CPU를 쓰지 않지만, 프로세스 테이블 슬롯을 점유합니다. 수백 개가 쌓이면 새 프로세스 생성이 실패합니다. 해결책은 부모가 `waitpid`를 호출하거나, 부모가 죽어서 init(PID 1)이 인양하는 것입니다.
...
- **프로세스는 어떤 자원을 자기 것으로 가지고 있을까요?**
  - 프로세스는 가상 주소 공간(코드/힙/스택), 파일 디스크립터 테이블, 신호 처리기 설정, 사용자/그룹 권한을 자기 것으로 갖습니다. 이 자원들은 다른 프로세스와 공유되지 않습니다. 프로세스가 종료되면 OS가 이 자원을 모두 회수합니다.
- **스레드는 무엇을 공유하고 무엇은 따로 가질까요?**
  - 스레드는 같은 프로세스 안에서 코드, 힙, 전역 변수, 파일 디스크립터를 공유합니다. 반면 스택과 레지스터 세트(PC, SP 포함)는 스레드마다 따로 가집니다. 공유하는 영역에 동시에 쓰면 경쟁 상태가 생기는 이유가 이것입니다.
- **`fork`와 `exec`는 왜 두 단계로 나뉘어 있을까요?**
...
4. 좀비 프로세스를 의도적으로 만들어 보고 (`waitpid` 없이 `fork`), `ps`로 Z 상태를 확인한 뒤 부모가 종료될 때 어떻게 사라지는지 관찰해 보세요.
