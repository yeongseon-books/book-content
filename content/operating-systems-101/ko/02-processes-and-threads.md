...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 프로세스는 어떤 자원을 자기 것으로 가지고 있을, 둘째 스레드는 무엇을 공유하고 무엇은 따로 가질, 셋째 `fork`와 `exec`는 왜 두 단계로 나뉘어 있을입니다. 기본 모델에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.
...
- **스레드는 무엇을 공유하고 무엇은 따로 가질까요?**
  - 프로세스와 스레드는 "무엇이 더 빠른가"보다 "무엇을 격리하고 무엇을 공유할 것인가"로 고르는 편이 정확합니다.
- **`fork`와 `exec`는 왜 두 단계로 나뉘어 있을까요?**
  - > 한 프로세스는 자기만의 가상 주소 공간, 파일 디스크립터 테이블, 신호 처리기, 권한을 가집니다
- **기본 모델에서 가장 흔한 실수는 무엇일까요?**
  - > 한 프로세스는 자기만의 가상 주소 공간, 파일 디스크립터 테이블, 신호 처리기, 권한을 가집니다. 그 안에 한 개 이상의 스레드가 있고, 스레드는 메모리와 fd는 공유하지만 자신만의 스택과 레지스터 상태를 갖습니다.
- **같은 코드를 다르게 읽는 법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 이 한 줄은 둘 중 어느 것을 골라야 하는지 알려 주지 않습니다.
