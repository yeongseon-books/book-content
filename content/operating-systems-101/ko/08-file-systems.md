...
- **inode와 디렉터리 엔트리는 파일을 어떻게 표현할까요?**
  - 디렉터리 엔트리는 이름을 inode 번호에 연결하는 표이고, inode는 권한·크기·블록 포인터 같은 파일 메타데이터를 담는 실제 객체입니다. `ln a.txt b.txt` 뒤에 `ls -li`를 보면 두 이름이 같은 inode 번호를 가리키는데, 이것이 "이름 둘, 파일 하나"인 하드 링크의 정체입니다.
- **페이지 캐시와 `fsync`는 각각 어디까지를 보장할까요?**
  - 일반적인 `write`는 우선 페이지 캐시에만 반영되므로, 호출이 끝나도 전원 장애 시 데이터가 사라질 수 있습니다. `flush + os.fsync(f.fileno())`는 그 내용을 저장 장치까지 밀어 내리라고 요구하는 단계이고, 원자적 저장 절차에서는 이어서 `os.replace`와 디렉터리 `fsync(dirfd)`까지 해야 이름 변경의 내구성도 확보됩니다.
- **저널링은 충돌 이후 어떤 복구를 가능하게 할까요?**
  - 저널링 파일시스템은 크래시 뒤에도 메타데이터 일관성을 복구해 파일시스템 자체가 깨지는 일을 줄여 줍니다. 다만 애플리케이션 레코드가 완전하게 남는 것까지 자동 보장하지는 않으므로, 글에서 강조한 tmp 파일 + atomic rename + checksum 같은 패턴을 애플리케이션이 직접 설계해야 합니다.
