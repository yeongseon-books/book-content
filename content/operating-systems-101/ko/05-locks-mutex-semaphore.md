...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 뮤텍스, 재진입 락, 세마포어, 조건 변수는 무엇이 다를, 둘째 데드락은 어떤 조건이 겹칠 때 만들어질, 셋째 락을 오래 잡으면 왜 처리량과 지연 시간이 같이 나빠질입니다. 기본 모델에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **뮤텍스, 재진입 락, 세마포어, 조건 변수는 무엇이 다를까요?**
  - 뮤텍스, 재진입 락, 세마포어, 조건 변수는 무엇이 다를까요 — 본문에서 구체적으로 다룹니다.
- **데드락은 어떤 조건이 겹칠 때 만들어질까요?**
  - 데드락은 어떤 조건이 겹칠 때 만들어질까요 — 본문에서 구체적으로 다룹니다.
- **락을 오래 잡으면 왜 처리량과 지연 시간이 같이 나빠질까요?**
  - 락을 오래 잡으면 왜 처리량과 지연 시간이 같이 나빠질까요 — 본문에서 구체적으로 다룹니다.
- **기본 모델에서 가장 흔한 실수는 무엇일까요?**
  - > Mutex는 한 시점에 한 흐름만 임계 구역에 들어가게 합니다. Semaphore는 N개까지 동시에 들어갈 수 있게 합니다.
- **같은 코드를 다르게 읽는 법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 두 함수가 동시에 돌면 데드락이 만들어집니다.
- **단계별로 확인하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - `with m:` 블록이 임계 구역입니다. 빠지면 자동으로 풀립니다.
