...
- **뮤텍스, 재진입 락, 세마포어, 조건 변수는 무엇이 다를까요?**
  - 뮤텍스는 한 번에 한 흐름만 임계 구역에 넣고, RLock은 같은 스레드의 재진입을 허용하며, 세마포어는 `Semaphore(3)`처럼 N개까지 동시 진입을 허용합니다. 조건 변수는 "재고가 1개 이상일 때까지 기다린다"처럼 상태 변화 자체를 기다리는 도구라서, 단순 배타 락과 목적이 다릅니다.
- **데드락은 어떤 조건이 겹칠 때 만들어질까요?**
  - 데드락은 상호 배제, 점유 대기, 비선점, 순환 대기가 동시에 성립할 때 생깁니다. `t1()`은 `a` 다음 `b`를 잡고 `t2()`는 `b` 다음 `a`를 잡는 예제에서 두 스레드가 서로의 락 해제를 기다리며 영원히 멈추는 이유가 바로 이 네 조건 때문입니다.
- **락을 오래 잡으면 왜 처리량과 지연 시간이 같이 나빠질까요?**
  - 락 안에서 `time.sleep(0.5)` 같은 긴 작업이나 I/O를 하면 다른 스레드가 모두 입구에서 막혀 병렬성이 사라집니다. 이런 상황에서는 처리량이 떨어질 뿐 아니라 `futex` 대기와 락 호송이 늘어나서 tail latency도 함께 악화됩니다.
