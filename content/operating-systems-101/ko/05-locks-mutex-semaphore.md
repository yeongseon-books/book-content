...
경쟁 상태를 막으려면 결국 공유 구역의 출입 규칙을 정해야 합니다. 문제는 규칙을 만들었다고 해서 자동으로 안전해지지 않는다는 사실입니다.
...
`with m:` 블록이 임계 구역입니다. 블록을 빠지면 자동으로 풀립니다.
...
## 시스템 관찰: 데드락 탐지와 락 경합 분석

### `futex` 대기로 락 경합 확인하기

```bash
strace -f -c -e trace=futex -p <PID>
```

`futex` 호출이 전체 시간의 50% 이상을 차지하면, 락 경합이 심각한 수준입니다. 락 분할(lock striping) 또는 읽기-쓰기 락으로 교체를 검토해야 합니다.
...
데드락과 락 경합을 줄이는 최소 규약입니다.

1. **락 순서 전역 규약**: `L_user -> L_order -> L_inventory` 식으로 팀 전체가 동의한 순서를 코드에 주석으로 명시합니다.
2. **락 타임아웃 기본값**: `Lock.acquire(timeout=0.2)` 식으로 200ms 이상이면 로그 경고를 남깁니다.
3. **임계 구역 내 I/O 금지**: 락을 잡은 상태에서 네트워크/파일 I/O를 하지 않습니다.
4. **예외 경로 보장**: 항상 `with lock:` 또는 `try/finally`를 씁니다.
...
| 단방향 데이터 흐름 | 과설계 | 적합 |
| 처리량이 중요한 I/O | 부적합 | 적합 |
| 실시간 조건 동기화 | Condition 변수 | 가능하나 복잡 |

## 처음 질문으로 돌아가기

- **뮤텍스, 재진입 락, 세마포어, 조건 변수는 무엇이 다를까요?**
  - Mutex는 단 하나의 진입만 허용하는 이진 잠금입니다. RLock은 같은 스레드가 여러 번 잡을 수 있어 재귀 함수에 적합합니다. Semaphore는 동시 진입 개수를 N으로 제한해 자원 풀 관리에 씁니다. Condition variable은 "이 조건이 참이 될 때까지 기다려라"는 술어 기반 대기를 제공합니다.
- **데드락은 어떤 조건이 겹칠 때 만들어질까요?**
  - 상호 배제(한 번에 한 흐름만 자원 사용), 점유 대기(자원을 쥔 채 다른 자원 요청), 비선점(강제로 뺏을 수 없음), 순환 대기(A→B→C→A 식의 의존 고리)라는 네 조건이 모두 동시에 성립하면 데드락이 발생합니다. 순환 대기를 깨는 것이 가장 실용적인 해법입니다.
- **락을 오래 잡으면 왜 처리량과 지연 시간이 같이 나빠질까요?**
  - 한 스레드가 락을 잡고 있으면 나머지 스레드는 모두 대기 상태로 전환됩니다. 대기 스레드가 많아질수록 OS가 컨텍스트 스위치와 스케줄링을 반복합니다. 이 현상을 "락 호송(lock convoy)"이라 부르며, 처리량은 내려가고 tail latency는 올라갑니다.
...
- **Operating Systems 101 (5/10): 락, 뮤텍스, 세마포어 (현재 글)**
