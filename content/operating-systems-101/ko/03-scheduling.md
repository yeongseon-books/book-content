...
- **스케줄러는 어떤 목표들 사이에서 균형을 잡을까요?**
  - 스케줄러는 응답성, 처리량, 공정성, 전력 소비 사이에서 타협점을 찾습니다. FCFS와 SRTF 간트 차트 비교처럼 어떤 정책은 짧은 작업의 대기 시간을 줄여 체감 품질을 높이고, 어떤 정책은 전체 시스템의 공정성을 더 중시합니다.
- **선점, 타임 슬라이스, 우선순위는 실제로 어떤 차이를 만들까요?**
  - 선점이 있으면 더 높은 우선순위 작업이 깨어났을 때 현재 작업을 끊고 CPU를 빼앗을 수 있고, 타임 슬라이스는 한 작업이 코어를 독점하지 못하게 합니다. `nice -n 19`, `taskset`, `chrt -r` 예제가 보여 주듯 우선순위·어피니티·정책을 바꾸면 같은 코드라도 CPU 점유율과 지연 특성이 달라집니다.
- **컨텍스트 스위치는 왜 보이지 않아도 비용이 클까요?**
  - 컨텍스트 스위치는 실행 흐름만 바꾸는 것이 아니라 레지스터 상태를 저장하고 다른 작업의 상태를 복원하는 작업이라서 누적 비용이 큽니다. `/usr/bin/time -v`에서 voluntary·involuntary context switch 수를 따로 보는 이유도, 스레드를 많이 늘렸을 때 실제 병목이 계산이 아니라 스위치 폭증일 수 있기 때문입니다.
