...
### `futex` 대기 비율로 락 경합 측정

```bash
strace -f -c -e trace=futex python3 race_test.py
```

락 경합이 심하면 `futex` 비중이 높게 나타납니다. 이 수치가 높으면 락 분할(lock striping) 또는 불변 메시지 전달 구조로 재설계할 근거가 됩니다.

### 원자성 세 가지 축의 실패 패턴 비교

| 실패 유형 | 발생 조건 | 증상 |
| --- | --- | --- |
| 원자성 위반 | read-modify-write 사이에 스케줄러 개입 | 카운터 손실, 부분 업데이트 |
| 가시성 위반 | CPU 캐시 또는 레지스터에 값이 묶임 | 한 스레드의 쓰기가 다른 스레드에 안 보임 |
| 순서성 위반 | 컴파일러/CPU 명령 재배치 | 초기화 전 객체 접근, 조건 검사 우회 |

### thread-local 누산 패턴으로 경쟁 회피
...
threads = [threading.Thread(target=add_local) for _ in range(8)]
for t in threads: t.start()
for t in threads: t.join()

# 각 스레드 로컬 값을 합산
# (실제로는 스레드 종료 전에 중앙에 보고하는 구조 필요)
```

락 없이 각 스레드가 자기 카운터를 갖고, 마지막에만 합치는 패턴입니다. 메트릭 수집에서 자주 씁니다.

## 처음 질문으로 돌아가기

- **경쟁 상태는 정확히 언제 발생한다고 말할 수 있을까요?**
  - 두 개 이상의 실행 흐름이 공유 상태에 동시에 접근하고, 그 중 적어도 하나가 쓰기일 때, 적절한 동기화 없이 발생합니다. "동시에"의 기준은 실제 동시가 아니라 인터리빙이 가능한 모든 경우입니다.
- **원자성, 가시성, 순서성은 어떤 식으로 서로 다른 실패를 만들까요?**
  - 원자성이 깨지면 한 작업의 중간 상태가 다른 스레드에 노출됩니다(잃어버린 업데이트). 가시성이 깨지면 스레드 A의 쓰기가 스레드 B에 보이지 않습니다(무한 루프). 순서성이 깨지면 명령 재배치로 초기화 전 객체에 접근합니다(널 포인터, 불완전 초기화).
- **왜 "한 줄짜리 코드"도 안전하다고 볼 수 없을까요?**
  - 소스 코드 한 줄은 여러 개의 기계어 명령으로 컴파일됩니다. `count += 1`은 load, add, store 세 단계입니다. 스케줄러는 이 세 단계 사이 어디서든 다른 스레드를 실행할 수 있습니다.
...
- **Operating Systems 101 (4/10): 동시성과 경쟁 상태 (현재 글)**
