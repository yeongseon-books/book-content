...
- **경쟁 상태는 정확히 언제 발생한다고 말할 수 있을까요?**
  - 경쟁 상태는 둘 이상의 흐름이 같은 상태를 공유하고, 그중 적어도 하나가 쓰기를 하며, 최종 결과가 인터리빙 순서에 따라 달라질 때 발생합니다. 8개 스레드가 `count += 1`을 100,000번씩 수행해도 결과가 800,000보다 작고 실행마다 달라지는 예제가 그 정의를 그대로 보여 줍니다.
- **원자성, 가시성, 순서성은 어떤 식으로 서로 다른 실패를 만들까요?**
  - 원자성이 깨지면 `count += 1`처럼 읽기-더하기-쓰기 사이에 다른 스레드가 끼어들어 누락 업데이트가 생기고, 가시성이 깨지면 `stop = True` 변경이 다른 흐름에 늦게 보일 수 있습니다. 순서성 문제까지 겹치면 코드에 적은 순서와 CPU가 실제로 관찰하는 순서가 어긋나므로, 락이나 메시지 전달로 실행 순서를 명시해야 합니다.
- **왜 "한 줄짜리 코드"도 안전하다고 볼 수 없을까요?**
  - 파이썬 한 줄도 실제로는 여러 단계의 바이트코드로 쪼개집니다. `dis.dis(add_one)`에서 `LOAD_FAST`, `INPLACE_ADD`, `STORE_SUBSCR`가 따로 보이듯 중간 단계마다 스케줄러가 다른 스레드를 끼워 넣을 수 있으므로, 겉모양만 보고 원자적이라고 판단하면 안 됩니다.
