...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 가상 주소와 물리 주소는 왜 굳이 분리되어 있을, 둘째 페이지, 페이지 테이블, TLB는 어떤 역할 분담을 할, 셋째 minor fault와 major fault는 비용이 어떻게 다를입니다. 기본 모델에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **가상 주소와 물리 주소는 왜 굳이 분리되어 있을까요?**
  - 가상 주소와 물리 주소는 왜 굳이 분리되어 있을까요 — 본문에서 구체적으로 다룹니다.
- **페이지, 페이지 테이블, TLB는 어떤 역할 분담을 할까요?**
  - 가상 주소 변환은 그림으로만 보면 추상적이지만, 숫자를 넣어 한 번 계산하면 정확히 이해됩니다.
- **minor fault와 major fault는 비용이 어떻게 다를까요?**
  - minor fault와 major fault는 비용이 어떻게 다를까요 — 본문에서 구체적으로 다룹니다.
- **기본 모델에서 가장 흔한 실수는 무엇일까요?**
  - > 모든 프로세스는 자신만의 가상 주소 공간을 가집니다. 가상 주소는 페이지 단위(보통 4KB)로 잘려 페이지 테이블을 통해 물리 주소로 매핑됩니다.
- **같은 코드를 다르게 읽는 법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 같은 RSS여도 page fault 패턴이 다르면 체감 속도가 다릅니다.
- **단계별로 확인하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - minor fault는 RAM에서 새 페이지를 잡는 비용, major fault는 디스크에서 읽어오는 비용입니다. major가 늘면 swap 의심.
