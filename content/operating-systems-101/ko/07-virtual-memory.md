...
- **가상 주소와 물리 주소는 왜 굳이 분리되어 있을까요?**
  - 프로세스마다 고유한 가상 주소 공간을 주면, 두 프로세스가 같은 가상 주소를 써도 서로 다른 물리 메모리를 가리킵니다. 덕분에 프로세스 간 메모리 침범이 불가능해지고, 프로그램은 자기가 메모리 전체를 혼자 쓰는 것처럼 가정할 수 있습니다.
- **페이지, 페이지 테이블, TLB는 어떤 역할 분담을 할까요?**
  - 페이지는 가상 주소 공간을 4KB 단위로 나눈 조각입니다. 페이지 테이블은 각 페이지의 가상 주소를 물리 주소로 변환하는 매핑표입니다. TLB는 최근에 쓴 변환 결과를 캐싱해 페이지 테이블 조회 횟수를 줄이는 CPU 내 하드웨어 캐시입니다.
- **minor fault와 major fault는 비용이 어떻게 다를까요?**
