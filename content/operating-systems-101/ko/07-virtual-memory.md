...
```text
minor fault (소프트 폴트)
  - 페이지가 이미 RAM에 있지만 페이지 테이블에 매핑이 없는 경우
  - 예: 처음 접근하는 익명 페이지, fork 이후 CoW 페이지
  - 비용: 수 마이크로초

major fault (하드 폴트)
  - 페이지가 RAM에 없어서 디스크(swap 또는 파일)에서 읽어야 하는 경우
  - 예: swap으로 쫓겨난 페이지, 파일 백업 mmap 최초 접근
  - 비용: 수 밀리초 ~ 수십 밀리초
```

major fault가 초당 수백 건 이상 발생하면 시스템 응답성이 심각하게 저하됩니다.
...
## 시스템 관찰: 페이지 테이블, TLB, swap을 운영에서 읽기

### 페이지 테이블 워크 계산 예시

가상 주소 변환을 숫자로 직접 계산하면 추상적인 그림이 구체화됩니다.
...
```bash
# perf로 TLB miss 측정
perf stat -e dTLB-load-misses,iTLB-load-misses python3 my_workload.py
```

TLB miss 비율이 높게 나오면:
- 순차 접근 패턴으로 지역성을 높입니다.
- 큰 페이지(huge page, `madvise MADV_HUGEPAGE`)를 검토합니다.
...
# 페이지 폴트와 swap 동향
vmstat 1
# si(swap in), so(swap out), pgfault 확인

# 프로세스별 페이지 폴트
pidstat -r 1
# minflt/s, majflt/s: 초당 minor/major fault 수
```

`si`(swap in)이 지속적으로 0이 아니면 RAM이 실제 워킹셋보다 작은 상태입니다.

### 실무에서 swap 없이 운영하는 이유
...
swap이 있으면 OOM-kill 대신 swap으로 넘어가 응답 시간이 수십~수백 배 느려질 수 있습니다. 데이터베이스, 실시간 서비스는 swap을 비활성화하고 메모리 한도를 넉넉하게 잡는 것이 일반적입니다.

### `/proc/<pid>/smaps`로 메모리 구간 분석

```bash
PID=$(pgrep -f "python3 app.py" | head -n 1)
# 가장 큰 메모리 구간 찾기
awk '/^Size:/ {size=$2} /^Rss:/ {rss=$2; if(rss>0) print rss, size}' \
    /proc/$PID/smaps | sort -rn | head -5
```

힙 증가인지 mmap 증가인지 분리할 수 있습니다. Private_Dirty가 크면 실제로 수정된 페이지가 많다는 뜻입니다.

### mmap vs read: 언제 어떤 것을 쓸까

| 상황 | 권장 방식 | 이유 |
| --- | --- | --- |
| 큰 파일 순차 읽기 | read + 버퍼링 | 페이지 캐시 prefetch 효과 |
| 랜덤 접근이 많은 큰 파일 | mmap | 필요한 페이지만 로드 |
| 여러 프로세스가 같은 파일 공유 | mmap (MAP_SHARED) | 물리 페이지 공유 |
| 파일 전체를 한 번만 처리 | read + streaming | mmap 설정 오버헤드 없음 |

## 처음 질문으로 돌아가기

- **가상 주소와 물리 주소는 왜 굳이 분리되어 있을까요?**
  - 프로세스마다 고유한 가상 주소 공간을 주면, 두 프로세스가 같은 가상 주소를 써도 서로 다른 물리 메모리를 가리킵니다. 덕분에 프로세스 간 메모리 침범이 불가능해지고, 프로그램은 자기가 메모리 전체를 혼자 쓰는 것처럼 가정할 수 있습니다. 또한 물리 메모리보다 큰 가상 공간을 제공해 swap과 demand paging이 가능해집니다.
- **페이지, 페이지 테이블, TLB는 어떤 역할 분담을 할까요?**
  - 페이지는 가상 주소 공간을 4KB 단위로 나눈 조각입니다. 페이지 테이블은 각 페이지의 가상 주소를 물리 주소로 변환하는 매핑표입니다. TLB는 최근에 쓴 변환 결과를 캐싱해 페이지 테이블 조회 횟수를 줄이는 CPU 내 하드웨어 캐시입니다. TLB hit이면 변환이 수 사이클, miss이면 페이지 테이블 워크로 수십 사이클이 걸립니다.
- **minor fault와 major fault는 비용이 어떻게 다를까요?**
  - minor fault는 매핑은 있지만 RAM에 페이지가 없어 새 프레임을 할당하는 경우로, 수 마이크로초 수준입니다. major fault는 페이지가 swap에 있거나 파일에서 읽어야 해서 디스크 I/O가 발생하는 경우로, 수십~수백 밀리초에 달합니다. major fault가 급증하면 시스템 응답성이 무너집니다.
