...
**핵심 질문**: 모든 프로세스가 자신만의 거대한 메모리를 가진 것처럼 보이는데, OS는 한정된 RAM으로 그 환상을 어떻게 만들까요?

> 가상 메모리는 OS가 만들어 내는 가장 큰 환상입니다. 모든 프로세스에게 "너만의 4GB"를 약속하지만 RAM은 8GB, 16GB뿐입니다. 페이지, 페이지 테이블, TLB, 그리고 page fault가 이 환상을 떠받칩니다. 작동 원리를 알면 "프로세스는 멀쩡한데 시스템이 굳는" 현상이 왜 발생하는지 보입니다.
...
- 가상 주소와 물리 주소의 분리
- 페이지, 페이지 테이블, TLB의 역할
- page fault와 swap이 만들어 내는 비용
- 메모리 매핑 파일(mmap)과 copy-on-write의 활용
...
| 용어 | 설명 |
| --- | --- |
| 페이지 | 가상/물리 메모리를 나누는 단위 (보통 4KB) |
| 페이지 테이블 | 가상 페이지 → 물리 프레임 매핑표 |
| TLB | 페이지 테이블 lookup을 캐시하는 CPU 내부 캐시 |
| page fault | 매핑이 없거나 페이지가 RAM에 없어 OS가 개입해야 함 |
| swap | RAM이 부족할 때 페이지를 디스크에 내보냈다가 다시 가져오는 것 |
...
시니어 엔지니어는 "메모리 부족"이라는 보고를 받으면 RSS만 보지 않습니다. 먼저 page fault 비율, swap 사용량, 그리고 working set이 RAM을 넘었는지를 확인합니다. 가상 메모리는 환상이지만, 환상이 깨지는 순간(major fault, swap-in)을 정확히 짚어내지 못하면 진단이 영원히 헛돕니다.

또한 시니어는 데이터 구조를 짤 때 접근 패턴을 같이 설계합니다. 메모리는 단순한 1차원 공간이 아니라 페이지/캐시 라인이라는 단위로 동작하는 계층 구조이기 때문입니다. 이 사실 하나로 같은 알고리즘이 수 배 빨라집니다.
...
1. 같은 2D 배열을 행 우선/열 우선으로 채우는 시간을 측정하고, 차이가 왜 발생하는지 한 문단으로 설명합니다.

2. 1GB 파일을 read와 mmap으로 각각 처리하고, RSS와 시간을 비교합니다.

3. swap이 켜진 환경과 꺼진 환경에서 같은 메모리 부하를 줬을 때 응답 시간 차이를 관찰합니다.
