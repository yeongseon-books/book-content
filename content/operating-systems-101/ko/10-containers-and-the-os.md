...
- 기본 모델에서 가장 흔한 실수는 무엇일까요?
- 같은 코드를 다르게 읽는 법을 실무에 적용할 때 주의할 점은 무엇일까요?
- 단계별로 확인하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?
...
이 글은 operating-systems-101 시리즈의 한 단계로, 핵심 개념을 실무 맥락에서 정리했습니다. 여기서 다룬 원칙들은 독립적으로도 유용하지만, 시리즈 전체와 연결될 때 더 큰 그림이 보입니다.

## 처음 질문으로 돌아가기

- **컨테이너와 가상 머신은 격리 방식이 어떻게 다를까요?**
  - 가상 머신은 하이퍼바이저 위에 게스트 OS를 통째로 올려 하드웨어 수준에 가까운 격리를 만들고, 컨테이너는 호스트 커널을 공유한 채 프로세스 관점에서 격리를 만듭니다. 그래서 컨테이너는 시작이 빠르고 가볍지만, 커널 취약점과 시스템 콜 표면을 호스트와 함께 쓰므로 seccomp·capability·rootless 같은 추가 방어선이 중요합니다.
- **namespace는 "무엇이 보이는가"를, cgroup은 "얼마나 쓸 수 있는가"를 어떻게 나눌까요?**
  - namespace는 PID, 네트워크, 마운트처럼 프로세스가 어떤 세계를 보게 할지 정하고, cgroup은 그 프로세스가 CPU와 메모리를 얼마나 쓸 수 있는지 제한합니다. 컨테이너 안에서 `ps -ef`를 보면 PID 1이 `sh`로 보이지만, `memory.max`가 64MB로 고정되고 `--cpus=0.5`에서 quota throttling이 걸리는 예제가 두 역할 분담을 선명하게 보여 줍니다.
- **overlayfs는 왜 컨테이너 이미지를 가볍게 느끼게 만들까요?**
  - overlayfs는 읽기 전용 레이어들을 여러 컨테이너가 공유하고, 변경분만 상단 writable layer에 얹는 방식으로 동작합니다. 같은 베이스 이미지를 쓰는 컨테이너가 디스크를 중복 점유하지 않고, 실제 수정 시점에만 copy-on-write가 일어나기 때문에 이미지가 빠르게 내려받히고 가볍게 느껴집니다.
