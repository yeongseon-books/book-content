...
이 글에서 다룬 핵심은 세 가지입니다. 첫째 컨테이너와 가상 머신은 격리 방식이 어떻게 다를, 둘째 namespace는 "무엇이 보이는가"를, cgroup은 "얼마나 쓸 수 있는가"를 어떻게 나눌, 셋째 overlayfs는 왜 컨테이너 이미지를 가볍게 느끼게 만들입니다. 기본 모델에서 시작해 실무 적용까지 이어지는 흐름을 따라가면 이 주제의 전체 그림이 잡힙니다.

## 처음 질문으로 돌아가기

- **컨테이너와 가상 머신은 격리 방식이 어떻게 다를까요?**
  - 컨테이너와 가상 머신은 격리 방식이 어떻게 다를까요 — 본문에서 구체적으로 다룹니다.
- **namespace는 "무엇이 보이는가"를, cgroup은 "얼마나 쓸 수 있는가"를 어떻게 나눌까요?**
  - namespace는 "무엇이 보이는가"를, cgroup은 "얼마나 쓸 수 있는가"를 어떻게 나눌까요 — 본문에서 구체적으로 다룹니다.
- **overlayfs는 왜 컨테이너 이미지를 가볍게 느끼게 만들까요?**
  - 컨테이너 장애를 볼 때는 "애플리케이션이 죽었는가"만 묻지 말고, 어느 OS 계층에서 문제가 시작됐는지를 먼저 나누는 편이 빠릅니다.
- **기본 모델에서 가장 흔한 실수는 무엇일까요?**
  - > VM은 하이퍼바이저 위에 게스트 OS를 통째로 올립니다. 컨테이너는 호스트 커널을 그대로 쓰고, namespace로 "보이는 것"을 격리하고, cgroup으로 "쓸 수 있는 자원"을 제한합니다.
- **같은 코드를 다르게 읽는 법을 실무에 적용할 때 주의할 점은 무엇일까요?**
  - 차이를 알면 컨테이너의 한계를 인정하고 적절한 추가 보호(seccomp, rootless, gVisor 등)를 적용합니다.
- **단계별로 확인하기의 핵심 원리를 한 문장으로 설명하면 무엇일까요?**
  - PID namespace 덕분에 컨테이너 안의 프로세스는 자기만의 PID 공간을 가집니다.
