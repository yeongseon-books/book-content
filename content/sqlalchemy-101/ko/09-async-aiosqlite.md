...
- **AsyncEngine·AsyncSession이 비동기 진입점** — create_async_engine으로 시작합니다.
- **async 드라이버가 필수** — asyncpg·aiosqlite 등 dialect별 매칭이 다릅니다.
- **await 누락이 가장 흔한 사고** — 타입 체커를 적극 활용합니다.
- **Session은 task당 하나** — 공유 시 race condition으로 직결됩니다.
- **greenlet 마법은 한계가 있다** — blocking 호출을 섞지 않습니다.
