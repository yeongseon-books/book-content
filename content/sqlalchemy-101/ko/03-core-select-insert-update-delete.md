...
- **select() 함수가 진입점** — 체이닝으로 쿼리를 구성합니다.
- **execute() 결과는 Result 객체** — scalar·all·one 등의 의도를 명시합니다.
- **bind parameter가 SQL 인젝션 방어** — literal SQL을 절대 직접 합치지 않습니다.
- **returning이 race를 줄인다** — INSERT·UPDATE 후 즉시 결과를 받습니다.
- **bulk 연산은 별도 API** — ORM의 unit of work를 우회해 성능을 얻습니다.
