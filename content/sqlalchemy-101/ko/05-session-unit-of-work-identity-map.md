...
Session·Unit of Work·Identity Map을 이해하면 어떤 ORM 사고를 예방할 수 있을까요?

이 글은 그 질문에 답하기 위해 Session 내부 동작의 핵심 결정과 운영 함정을 살펴봅니다.

<!-- a-grade-intro:end -->

## 이 글에서 답할 질문

- `Session`은 커넥션, 트랜잭션과 어떤 관계입니까?
- Unit of Work란 정확히 무엇이고, 언제 SQL을 실제로 보냅니까?
- `flush()`와 `commit()`은 어떻게 다릅니까?
- Identity Map은 무엇을 보장합니까? 같은 PK를 두 번 조회하면 같은 객체가 옵니까?
- `expire_on_commit=True`(기본값)는 어떤 부작용을 만듭니까?
- `sessionmaker`는 왜 필요합니까?
- ORM 객체의 transient/pending/persistent/detached 상태는 어떻게 전이합니까?

## 왜 중요한가

![핵심 개념](../../../assets/sqlalchemy-101/05/05-02-why-it-matters.ko.png)
...
![Mental model](../../../assets/sqlalchemy-101/05/05-03-mental-model.ko.png)
...
![핵심 개념](../../../assets/sqlalchemy-101/05/05-04-core-concepts.ko.png)
...
이 묶음 처리가 Unit of Work의 핵심입니다. 100개의 객체를 INSERT하면 100번의 round-trip 대신 한 번의 INSERT로 묶이며, 그 사이에 발생한 UPDATE와 DELETE도 의존성 순서를 고려해 순서가 결정됩니다.
...
![단계별 실습](../../../assets/sqlalchemy-101/05/05-05-step-by-step-walkthrough.ko.png)
...
- **Session은 트랜잭션의 작업 영역** — with 블록으로 생명주기를 명시합니다.
- **Unit of Work가 변경을 모은다** — flush 시점이 SQL 실행 시점입니다.
- **Identity Map이 동일성 보장** — 같은 PK는 같은 객체로 돌아옵니다.
- **expire·refresh의 의미를 안다** — 캐시된 상태와 DB 상태의 분리를 이해합니다.
- **Session per request가 표준 패턴** — 웹 앱의 동시성 모델과 정합합니다.
...
1. `Session(engine, autoflush=False)`로 만든 세션에서 `add` 후 SELECT를 호출해 보고, 그 다음에 `flush()`를 호출했을 때 SELECT 결과가 어떻게 달라지는지 관찰해 보세요. autoflush가 끄지 않으면 동작이 어떻게 바뀝니까?
