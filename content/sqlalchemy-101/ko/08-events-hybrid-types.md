...
- SQLAlchemy event 시스템으로 모델·세션·엔진 수준의 라이프사이클 훅을 거는 방법
- `@validates` 데코레이터로 입력값을 검증하는 패턴
- `hybrid_property`로 Python과 SQL 양쪽에서 쓸 수 있는 파생 속성을 만드는 방법
- `TypeDecorator`로 컬럼 타입을 감싸 암호화·정규화·JSON 직렬화 등을 투명하게 처리하는 방법
- 이벤트와 커스텀 타입을 production에서 안전하게 운영하기 위한 규칙
...
- SQLAlchemy event는 mapper, session, engine 세 층 중 어디에서 어떤 일을 잡을 수 있는가?
- `@validates`는 어느 시점에 호출되며 어떤 종류의 검증·정규화에 적합한가?
- `hybrid_property`는 Python 표현식과 SQL 표현식 양쪽을 어떻게 동시에 정의하는가?
- `TypeDecorator`로 암호화·정규화·JSON 직렬화 같은 변환을 컬럼 차원에서 감추면 무엇이 좋아지는가?
- 이벤트와 커스텀 타입을 production에 둘 때 지켜야 할 안전 규칙은 무엇인가?
...
- **event listener가 cross-cutting 자리** — before_insert·after_update가 자주 쓰입니다.
- **hybrid_property가 Python·SQL 양쪽** — 한 정의로 두 컨텍스트를 만족합니다.
- **TypeDecorator가 직렬화 경계** — JSON·암호화 등 변환을 캡슐화합니다.
- **validates는 가벼운 검증에** — 복잡한 도메인 규칙은 별도 레이어로 분리합니다.
- **event는 강력하지만 추적 어려움** — 남용 시 디버깅 비용이 커집니다.
...
1. 비밀번호를 저장할 때 자동으로 bcrypt 해시로 변환하고 읽을 때는 그대로 두는 `BcryptString` `TypeDecorator`를 작성하세요.
2. `Order` 모델에 `total_amount = unit_price * quantity` 를 `hybrid_property`로 정의하고 where 절에서 사용해 보세요.
3. `before_cursor_execute` / `after_cursor_execute`로 1초 이상 걸린 쿼리를 별도 로그 파일에 남기는 핸들러를 작성하세요.
