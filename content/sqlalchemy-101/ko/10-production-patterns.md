...
- connection pool 핵심 옵션(`pool_size`, `max_overflow`, `pool_pre_ping`, `pool_recycle`)
- pool 종류 선택 기준(`QueuePool` vs `NullPool`, SQLite의 `StaticPool`)
- OpenTelemetry로 SQL 쿼리를 trace에 연결하기
- slow query와 N+1을 production에서 잡는 패턴
...
- **Pool 튜닝이 안정성의 기반** — size·timeout·recycle을 워크로드에 맞춥니다.
- **관측은 SQL·트랜잭션 메트릭부터** — P99 query time이 가장 빠른 신호입니다.
- **Alembic으로 스키마 변경 추적** — production 변경은 코드로만 진행합니다.
- **read replica 라우팅을 의식적으로** — session·engine 분리가 표준입니다.
...
1. `pool_size=2`, `max_overflow=0`으로 줄인 뒤 동시 요청 5개를 보내 `pool_timeout`이 어떻게 동작하는지 직접 재현하세요.
2. slow query 임계를 200ms → 50ms로 점진적으로 좁히면서 어떤 쿼리가 신호로 잡히는지 기록하세요.
3. `SQLITE_BUSY` 시나리오를 만들기 위해 두 thread에서 동시에 write를 시도하고, retry 데코레이터로 회복되는지 확인하세요.
