...
relationship과 back_populates를 어떻게 써야 양방향 탐색이 안전할까요?

이 글은 그 질문에 답하기 위해 ORM relationships의 핵심 결정과 운영 함정을 살펴봅니다.
...
- `back_populates`와 `backref`의 차이는 무엇이고, 왜 `back_populates`를 권장합니까?
- 일대다 관계에서 `User.orders`에 새 객체를 append하면 어떤 일이 일어납니까?
- 다대다 관계는 어떻게 정의합니까? Association table은 ORM에 어떻게 노출됩니까?
- `cascade="all, delete-orphan"`은 언제 켜야 안전합니까?
- 관계 탐색이 추가 SELECT를 만드는 시점은 언제입니까?
...
- **relationship이 객체 그래프 정의** — FK 위에 navigational API를 얹습니다.
- **back_populates가 양방향 일관성** — 한쪽 변경이 다른 쪽에 자동 반영됩니다.
- **cascade는 신중하게 결정** — delete-orphan이 의도와 다르면 데이터 사고입니다.
- **collection 타입을 의식적으로 선택** — list·set·dict 각각의 의미가 다릅니다.
- **self-referential도 같은 패턴** — tree·graph 구조를 자연스럽게 표현합니다.
...
1. `User.orders`에 `cascade="all, delete-orphan"`을 두지 않은 상태에서 user를 삭제하면 어떤 일이 일어납니까? `PRAGMA foreign_keys=ON` 여부에 따라 결과가 어떻게 달라지는지 직접 확인해 보세요.
2. `Post`-`Tag` 다대다를 association table로 정의한 뒤, "태그가 달린 시각" 컬럼이 필요해진 상황을 가정해 association object로 마이그레이션해 보세요. 코드 변경 범위는 어디까지입니까?
3. `User.orders`에 `order_by=Order.amount.desc()` 옵션을 추가하면 어떤 효과가 있습니까? echo로 SELECT를 확인해 ORDER BY가 추가되는 것을 확인해 보세요.
