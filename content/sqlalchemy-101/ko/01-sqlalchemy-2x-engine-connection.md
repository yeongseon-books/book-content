...
- SQLAlchemy가 Core와 ORM이라는 두 계층으로 이루어져 있고, Engine과 Connection은 그 중 Core의 진입점이라는 것
- `create_engine()`이 만들어내는 객체가 사실은 dialect, DBAPI, connection pool, URL 파서를 묶은 factory라는 것
- 1.x와 2.x의 가장 큰 차이인 `future=True` 기본화, 통일된 `select()`, `Mapped[]` 타입 힌트, native async가 코드 스타일에 어떤 영향을 주는지
- `engine.connect()`와 `engine.begin()`의 차이, 그리고 2.x에서 transaction이 명시적으로 다뤄지는 방식
- `text()`로 raw SQL을 안전하게 실행하는 방법과 named parameter binding의 의미
- SQLite URL 표기(`sqlite:///`, `sqlite:////`, `:memory:`)와 PRAGMA를 event로 적용하는 패턴
...
- SQLAlchemy를 쓰면 `sqlite3`는 더 이상 import할 필요가 없나요?
- `create_engine()`은 즉시 SQLite 파일을 여는가, 아니면 lazy하게 동작하는가?
- 2.x에서 `engine.execute("SELECT 1")`이 사라진 이유는 무엇인가?
- Connection은 ORM의 Session과 어떻게 다른가?
- `with engine.connect()`와 `with engine.begin()` 중 어느 쪽을 기본으로 써야 하는가?
- SQLite의 foreign_keys PRAGMA는 SQLAlchemy 어디에서 켜야 하는가?
...
- **Engine은 풀의 게이트웨이** — 전역에 하나만 두는 패턴이 표준입니다.
- **Connection은 트랜잭션의 단위** — with 블록으로 명시적으로 관리합니다.
- **2.x style은 명시성을 강조** — implicit autocommit이 사라진 이유를 이해합니다.
- **Pool 옵션이 동시성·안정성 결정** — size·overflow·recycle을 처음에 설정합니다.
- **dialect URL이 동작 방식을 좌우** — 드라이버·옵션 차이를 의식합니다.
...
1. `sqlite:///./test.db`로 Engine을 만들고, `users(id, name)` 테이블을 만든 뒤 두 명을 삽입한 다음 `SELECT COUNT(*)` 결과가 2인지 확인하는 스크립트를 작성하세요. 모든 SQL 실행은 `engine.begin()` 또는 `engine.connect()` 컨텍스트 안에서 일어나야 합니다.
2. 같은 코드에서 `engine.begin()`을 `engine.connect()`로 바꾸되 commit을 호출하지 마세요. 다시 실행하면 데이터가 남아 있는지, 사라지는지를 관찰하고 그 이유를 설명하세요.
3. `event.listens_for(engine, "connect")`로 `PRAGMA foreign_keys = ON`을 적용한 Engine과 적용하지 않은 Engine을 각각 만들어, 부모 row가 없는 자식 row INSERT가 어떻게 다르게 동작하는지 비교하세요.
4. `sqlite:///:memory:` Engine을 두 개 만들고 한 쪽에서 만든 테이블이 다른 쪽에서 보이는지 확인하세요. 결과를 통해 in-memory DB의 격리 범위를 설명하세요.
