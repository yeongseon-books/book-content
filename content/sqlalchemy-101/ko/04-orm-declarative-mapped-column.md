...
DeclarativeBase와 mapped_column을 어떻게 써야 모델이 명확하고 타입 안전할까요?

이 글은 그 질문에 답하기 위해 ORM 모델 정의의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **DeclarativeBase가 표준 베이스** — registry 통합과 타이핑이 깔끔해집니다.
- **mapped_column이 컬럼 정의의 기본** — 타입 힌트와 컬럼 옵션이 동시에 표현됩니다.
- **Mapped[T] 타입이 IDE 지원** — 정적 분석이 모델을 이해합니다.
- **__tablename__을 의식적으로 명명** — 복수형·소문자 등 팀 컨벤션을 정합니다.
- **PK·FK는 처음에 결정** — 후속 변경은 마이그레이션 비용이 큽니다.
