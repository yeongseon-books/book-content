...
lazy·joined·selectin을 어떻게 골라야 N+1 문제를 예방할 수 있을까요?

이 글은 그 질문에 답하기 위해 로딩 전략의 핵심 결정과 운영 함정을 살펴봅니다.
...
- **기본 lazy가 N+1의 원인** — 관계 접근마다 추가 쿼리가 나갑니다.
- **joined는 1:1·M:1에 적합** — JOIN으로 한 번에 가져옵니다.
- **selectin은 1:N·M:N에 적합** — IN 쿼리 한 번으로 컬렉션을 채웁니다.
- **쿼리 단위로 전략을 다르게** — 모델 기본값보다 query option이 우선입니다.
- **echo·logging으로 SQL을 본다** — 전략 선택의 효과를 사실로 확인합니다.
